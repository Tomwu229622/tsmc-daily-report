...
STOCK = {
    # 台股 2330
    "tw_price":       "2,390",        # 2026-06-25 Thu 收盤（+15，+0.63% vs 6/24 NT$2,375 區間；跟進 6/24 ADR 反彈力道有限、漲價利多 vs 估值/營收疑慮拉鋸；6/26 Fri 今日交易中、ADR 盤後 +2.48% 大漲料帶動補漲、官方收盤待 TWSE。OHLC/量為估算）
    "tw_change":      "+15",          # 6/25 漲跌點數（估，+0.63% 小漲）
    "tw_change_pct":  "+0.63%",       # 6/25 漲跌幅（小幅反彈、跟進 ADR 力道有限）
    "tw_volume":      "~14,000",      # 6/25 成交量（張，估；官方以 TWSE 為準）
    "tw_open":        "2,390",        # 6/25 開盤（估，平盤附近）
    "tw_high":        "2,415",        # 6/25 盤中高（估）
    "tw_low":         "2,375",        # 6/25 盤中低（估）
    ...
    # NYSE TSM ADR
    "nyse_price":     "440.83",       # 6/25 Thu 收盤（+$4.44，+1.02%；漲價題材延燒、日內整理、市值約 $2.05 兆、P/E ~31.6；⭐ 盤後 +2.48% 大漲至 $451.76）
    "nyse_change":    "+4.44",        # 6/25 漲跌
    "nyse_change_pct":"+1.02%",       # 6/25 NYSE 續漲（晶圓代工漲價範圍擴大至 7nm 以下全製程／佔晶圓營收 74%；盤後再 +2.48%）
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-06-26 Fri（NYSE TSM 6/25 Thu 收盤 $440.83 為最新確認數據、盤後再 +2.48% 至 $451.76；台股 2330 6/25 Thu 收 NT$2,390 為最新確認；台股 6/26 Fri 今日交易中、官方收盤請以 TWSE 為準。台股 6/25 OHLC/量為估算）。⭐ 今日頭條——TSMC 晶圓代工漲價題材延燒、ADR 續漲且盤後大漲、台股料補漲：（1）⭐ 漲價範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 74%、含 N7/N5/N3）、幅度 5-10% 視客戶/節點/產品而定、部分已開始實施，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（2）📈 NYSE TSM 6/25 Thu 續漲 +1.02% 收 $440.83（+$4.44、市值約 $2.05 兆、P/E ~31.6），⭐ 盤後再大漲 +2.48% 至 $451.76（漲價題材延燒、產能擴張利多）；（3）📊 台股 2330 6/25 Thu 小漲 +0.63% 收 NT$2,390（跟進 6/24 ADR 反彈但力道有限、漲價利多 vs 估值/營收疑慮拉鋸）；6/26 Fri 今日台股料跟進 ADR 盤後大漲、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/25 ADR $440.83 + 台股 NT$2,390 計）：理論值 5×2,390÷31.05 = $384.86、溢價 (440.83-384.86)/384.86 = +14.54%（台股短線料補漲收斂）。⚠️ 漲價雙面刃——利多毛利率，但傳客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）措手不及、恐促部分客戶調整下單量、要求更嚴格合約或轉向替代；Benzinga 分析漲價壓縮 NVIDIA 毛利但因 AI 需求剛性、NVIDIA 具轉嫁能力、長期反為利多。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%；Applied Materials 6/25 推新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片降低對 TSMC 依賴、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：ITC 初判 6 月底；6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、+10% QoQ、+32% YoY、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +26% YoY 達 ~$975B；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆  |  ⚠️ 本日（6/26）關鍵觀察：（1）台股 2330 能否跟進 ADR 盤後大漲、收復 NT$2,450 / NT$2,490；（2）ADR 溢價 +14.54%、台股料補漲收斂；（3）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~31.6x、台股 P/E ~35.2x，留意 6 月營收 miss 風險、漲價客戶反彈、台幣升值與 ITC 初判雜訊",
}
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       54.0,   "rsi_signal": "RSI 估自 52 微升至 54（台股 6/25 小漲 +0.63%），位於中軸上方、動能略回穩；⚠️ 6/26 留意 NYSE 6/25 +1.02% 及盤後 +2.48% 能否帶動台股 RSI 進一步走揚",
    "macd":         0.20,  "macd_signal": "MACD 估自 +0.15 微升至 +0.20、紅柱小幅回升、仍貼近零軸、⚠️ 多空於零軸附近拉鋸、待方向確認",   "macd_color": "#FF9800",
    "kdj_k":        50.0,   "kdj_d": 48.0, "kdj_j": 54.0, "kdj_signal": "KDJ K(50)/D(48)/J(54) 自低檔回升、6/25 小漲帶動 K 線上穿 D、低檔黃金交叉初現、短線轉穩、待量能確認",
    "ma5":       "2,415",   # 5MA（6/18 2385, 6/22 2510, 6/23 2490, 6/24 2390, 6/25 2390）— 基於 6/25 收
    "ma20":      "2,400",   # 20MA
    "ma60":      "2,360",   # 60MA 緩升
    "ma120":     "2,255",
    "bb_upper":  "2,570",   # 布林上軌
    ...
    "trend":    "⭐ TSMC 漲價題材延燒、ADR 續漲且盤後大漲、台股料補漲：NYSE TSM 6/25 續漲 +1.02% 收 $440.83（漲價題材延燒、市值約 $2.05 兆）、盤後再 +2.48% 至 $451.76；台股 2330 6/25 收 NT$2,390（+15、+0.63%、量估 ~14,000 張）小幅反彈、跟進 6/24 ADR 力道有限；6/26 今日台股料跟進 ADR 盤後大漲。📊 技術面（基於 6/25 收 NT$2,390）：RSI 自 52 微升至 54 位於中軸上方、MACD +0.20 紅柱小幅回升仍貼近零軸、KDJ 低檔黃金交叉初現、收盤貼近 5MA NT$2,415、中期 60MA/120MA 趨勢仍偏多。下一觀察：（1）📈 台股能否跟進 ADR 大漲、站回 5MA NT$2,415 並挑戰 NT$2,450 / NT$2,490；（2）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；（3）上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⭐ 漲價利多估全年毛利率 +2pp 以上、惟須留意客戶反彈與 6 月營收（7 月上旬、4-5 月 +24% YoY 落後 ~35% 預期）；（5）⚠️ 雜訊續追蹤：ITC 專利調查 6 月底初判、Apple-Intel 合作、Intel 18A-P 風險量產；（6）⭐ 中長線結構性利多基底未變：Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + 2nm 量產 70-80% 良率 + Susquehanna 目標升至 $575。",
    "support":  "2,350 / 2,310 / 2,250 / 2,200 (整數、前低、整數、整數)",
    "resist":   "2,450 / 2,490 / 2,510 / 2,535 (整數、6/23 收、6/22 收、史高)",
    "note": "技術指標數值為估算（基於 6/25 收 NT$2,390），請以看盤軟體即時數據為準；台股 6/25 小漲 +0.63%、NYSE 6/25 +1.02% 且盤後 +2.48%、RSI 54 回穩、KDJ 低檔黃金交叉初現、MACD 紅柱小幅回升貼近零軸；NT$2,350 為關鍵支撐、上方壓力 NT$2,450 / NT$2,490；6/26 今日台股料跟進 ADR 大漲、⭐ 漲價利多 vs ⚠️ 營收 miss 風險拉鋸",
}
...
DERIVATIVES = {
    ...
    "call_oi":       "110,000",
    "put_oi":        "76,000",
    "pcr":           "0.69",             # Put/Call 比率（6/25 估自 0.72 回落至 0.69、小漲+ADR 反彈帶動避險買盤略減、情緒回穩）
    "pcr_signal":    "中性（Put/Call 6/25 估回落至 0.69、自前日 0.72 減少；反映台股 6/25 小漲 +0.63% 及 ADR 反彈帶動避險買盤略減、情緒回穩；⭐ 漲價利多+ADR 盤後 +2.48% 大漲或支撐多方）",
...
NEWS = [
    {
        ...
        "title": "⭐ TSMC 漲價範圍確認擴大至 7nm 以下全製程（佔晶圓營收 74%）、幅度 5-10%：NYSE TSM 6/25 續漲 +1.02% 收 $440.83、盤後再大漲 +2.48% 至 $451.76",
        "body": "今日頭條——TSMC 晶圓代工漲價題材延燒、ADR 續漲且盤後大漲：（1）⭐ 據 Culpium / Tom's Hardware 等報導，TSMC 已通知客戶漲價範圍擴大至 7nm 以下全部先進製程（含 N7/N5/N3、佔 Q1 晶圓營收約 74%，其中 N3 單一即佔 25%），幅度視客戶/節點/產品在 5-10% 區間，部分已開始實施、其餘要求客戶將更高成本計入未來訂單；以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（2）📈 利多帶動 NYSE TSM 6/25 Thu 續漲 +1.02% 收 $440.83（+$4.44、市值約 $2.05 兆、P/E ~31.6），⭐ 盤後再大漲 +2.48% 至 $451.76（漲價題材延燒疊加產能大舉擴張利多——今年規劃 9 期晶圓廠與先進封裝廠）；（3）📊 台股 2330 6/25 Thu 小漲 +0.63% 收 NT$2,390、跟進 6/24 ADR 反彈力道有限；6/26 今日料跟進 ADR 盤後大漲、官方收盤待 TWSE；（4）📊 ADR 溢價 +14.54%、台股短線料補漲收斂。",
        "source": "Culpium / Tom's Hardware / StockTwits / Yahoo Finance 2026-06-24 - 06-25", "impact": "正面",
    },
    {
    ...
    },
    {
        ...
        "title": "🛠️ Applied Materials 6/25 推新一代設備、加速 DRAM 與 AI 先進封裝（HBM／3D 堆疊）；2026 全球半導體市場估達 ~$975B",
        "body": "半導體設備與產業需求持續擴張、利多 TSMC 上游生態：（1）🛠️ Applied Materials（AMAT）6/25 發布一套新晶片製造系統，專為打造次世代 AI 所需的先進 3D 晶片架構、加速 DRAM 與先進封裝（含 HBM 與 3D 堆疊）採用，以緩解 AI 運算的記憶體瓶頸；（2）📈 SIA/Deloitte：2026 全球半導體市場估成長至 ~$975B（YoY +26%）、AI 晶片市場估 ~$500B、AI 資料中心晶片營收 2028 上看 >$1.2 兆；（3）⭐ 半導體於 AI 資料中心伺服器機櫃價值佔比達 95%；（4）⚠️ 晶片供應鏈緊俏估延續至 2026 年底；（5）⭐ TSMC 為先進製程與 CoWoS/CoPoS 先進封裝核心受惠者、與 AMAT EPIC Center $5B AI 合作深化。",
        "source": "GlobeNewswire / SIA / Deloitte 2026-06-25", "impact": "正面",
    },
    {

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,440",        # 台股 2330 最新確認收盤：7/13 Mon 官方收 NT$2,440（+25、+1.04% vs 7/9 NT$2,415.5、TWSE）——颱風休市後首個交易日、6 月營收創高撐盤；盤中一度大漲逾 900 點、尾盤翻黑收僅 +25；7/10 因颱風休市
    "tw_change":      "+25",          # 7/13 Mon 收盤漲跌點數（+1.04%、TWSE）
    "tw_change_pct":  "+1.04%",       # 7/13 Mon 收盤漲跌幅（6 月營收創高、Q2 達標帶動）
    "tw_volume":      "34,665",       # 7/13 Mon 成交量（張、成交額 NT$851.13 億、均價 2,455.21）
    "tw_open":        "2,460",        # 7/13 開盤
    "tw_high":        "2,480",        # 7/13 盤中高（來源略有 2,460/2,480 出入、採較新來源 2,480）
    "tw_low":         "2,440",        # 7/13 盤中低（收在低檔）
    "market_cap":     "NT$63.3 兆",   # 7/13 收盤市值（NT$2,440 基礎、約 259.3 億股）
    "pe_ratio":       "~32.3x",       # 台股 P/E（TTM，NT$2,440 ÷ 近四季 EPS ~75.5、statementdog）
    "pb_ratio":       "~18.3x",       # 台股 P/B（NT$2,440 ÷ 每股淨值 ~133.4、statementdog）
    "div_yield":      "~0.61%",       # 殖利率（NT$2,440 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "421.58",       # 7/13 Mon 官方收盤 $421.58（-$12.53、-2.89% vs 7/10 $434.11、stockanalysis.com 確認）——法說會前獲利了結、半導體回檔；自我檢核 434.11×(1-2.89%)=421.56 ≈ 421.58 ✓；盤後 $422.98（+0.33%）
    "nyse_change":    "-12.53",       # 7/13 漲跌（自 7/10 $434.11）
    "nyse_change_pct":"-2.89%",       # 7/13 NYSE 回檔
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~35.8x",       # NYSE USD 基礎 P/E（TTM 估，$421.58 基礎）
    "pb_nyse":        "~17.6x",
    "beta":           "1.18",
    "div_next":       "下次配息預計 2026-07",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-07-16（Q2 2026 財報，倒數 2 天）",
    # 資料日期備註
    "data_note":      "報告日期：2026-07-14 Tue（最新確認收盤：台股 2330 7/13 Mon 官方收 NT$2,440（+25、+1.04% vs 7/9 NT$2,415.5、開 2,460／高 2,480／低 2,440、量 34,665 張、成交額 NT$851.13 億、均價 2,455.21、TWSE）——颱風休市後首個交易日、6 月營收創高撐盤、盤中一度大漲逾 900 點尾盤翻黑收僅 +25；NYSE TSM 7/13 Mon 收 $421.58（-$12.53、-2.89% vs 7/10 $434.11、stockanalysis.com 確認、自我檢核 434.11×(1-2.89%)=421.56 ≈ 421.58 ✓、盤後 $422.98）——法說會前獲利了結、半導體回檔）。⭐⭐ 今日頭條——TSMC 6 月營收 7/13 正式公布、大幅達標創高，7/16 法說會倒數 2 天：（1）🎯 6 月合併營收 NT$4,426.80 億元、YoY +67.9%、MoM +6.2%，改寫單月歷史新高；1–6 月（H1）累計 NT$2.40 兆（+35.6% YoY）；Q2 累計約 NT$1.27 兆、達成美元指引 $39.0-40.2B——AI/HPC 需求動能獲確認、大幅緩解 4-5 月僅 +24% YoY 落後預期的營收疑慮；（2）📈 台股 7/13 收 NT$2,440（+1.04%）站回 5MA、營收創高撐盤（盤中曾大漲逾 900 點、尾盤獲利了結翻黑收僅 +25）；（3）📉 NYSE TSM 7/13 收 $421.58（-2.89%）——法說會前美股半導體獲利了結、估值修正；（4）📊 三大法人 7/13：外資賣超 2,045 張（連 3 賣、3 日累計 -16,464 張）、投信買超 530 張、自營買超 623 張、合計賣超 892 張；（5）📊 台美 ADR 溢價自 7/10 約 +15.3% 收斂至約 +10.8%（ADR $421.58 × 匯率 32.07 ÷ 5 = NT$2,704 vs 台股 2,440；ADR 回檔 -2.89% + 台股 +1.04% 雙向收斂）；（6）🚀 分析師續挺：Citi NT$3,800（自 NT$2,440 具 +55.7% 空間）、Barclays $625（自 $421.58 具 +48.3%）、Nomura NT$3,425（+40.4%）、共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）。🎯 即將事件：Q2 2026 財報法說會 7/16 週三 14:00（倒數 2 天、指引 $39.0-40.2B、毛利率 65-67%、法人估 Q2 毛利率挑戰 70%、Citi 預期上修全年展望）。⚠️ 本日（7/14）關鍵觀察：（1）台股能否延續 6 月營收利多、守穩 5MA ~NT$2,424 / 20MA ~NT$2,366、挑戰前高 NT$2,480；（2）外資連 3 賣（3 日 -16,464 張）能否收斂翻買、投信自營同步買超對沖；（3）法說會前 NYSE 續弱是否拖累台股、ADR 溢價 +10.8% 續收斂；（4）估值警示：台股 P/E ~32.3x（TTM）、TSM ~36x。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、Winbond 特殊 DRAM 自主供應、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2026 下半/2027 量產（Apple 跳過 A16 直上 A14）+ Vera Rubin 平台 6 顆晶片全數 TSMC 代工（3nm + HBM4）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%。  |  ⭐ 中長線基底：6 月營收 NT$4,426.80 億 +67.9% YoY 創單月新高、H1 累計 NT$2.40 兆（+35.6% YoY）、Q2 約 NT$1.27 兆達標；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；BofA 估 2026 全球半導體市場 $1.3 兆；費半（SOX）YTD +47%  |  ⚠️ 風險：外資 7 月連 3 賣未轉向、下游手機/PC 需求疲弱、Intel 18A 量產競爭、估值不低、台幣匯率波動（每升值 1% 毛利率 -40bps）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "🎯 TSMC 6 月營收 7/13 公布大幅達標創高：NT$4,426.80 億（+67.9% YoY、+6.2% MoM）、Q2 累計約 NT$1.27 兆達成指引；台股 2330 7/13 收 NT$2,440（+1.04%）站回 5MA；NYSE TSM 7/13 收 $421.58（-2.89%）法說會前獲利了結；7/16 法說會倒數 2 天、分析師續挺（Citi NT$3,800／Barclays $625／Nomura NT$3,425）",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🎯 最重要：TSMC 6 月營收 7/13 公布 NT$4,426.80 億——YoY +67.9%、MoM +6.2%，改寫單月歷史新高；1–6 月（H1）累計 NT$2.40 兆（+35.6% YoY）、Q2 累計約 NT$1.27 兆達成美元指引 $39.0-40.2B，AI 需求動能獲確認",
        "📈 台股 2330 7/13 Mon 官方收 NT$2,440（+25、+1.04% vs 7/9 NT$2,415.5、開 2,460／高 2,480／低 2,440、量 34,665 張、TWSE）——颱風休市後首個交易日、6 月營收創高撐盤、站回 5MA",
        "⚡ 盤中戲劇性：7/13 台股一度大漲逾 900 點（受營收利多激勵），但尾盤獲利了結翻黑、收僅 +25 點，台積電為撐盤要角、顯示高檔換手壓力",
        "📉 NYSE TSM 7/13 收 $421.58（-$12.53、-2.89% vs 7/10 $434.11）——法說會前美股半導體獲利了結、估值修正；自我檢核 434.11×(1-2.89%)=421.56 ✓；盤後回升 $422.98",
        "📊 三大法人 7/13：外資賣超 2,045 張（連 3 賣、3 日累計 -16,464 張）、投信買超 530 張、自營買超 623 張，合計賣超 892 張——外資賣壓大幅收斂、投信自營同步買超對沖",
        "📊 台美 ADR 溢價自 7/10 約 +15.3% 收斂至約 +10.8%（ADR $421.58 × 匯率 32.07 ÷ 5 = NT$2,704 vs 台股 2,440）——ADR 回檔 -2.89% + 台股 +1.04% 雙向收斂",
        "🚀 分析師大幅續挺：Citi NT$3,800（自 NT$2,440 具 +55.7% 空間）、Barclays $625（自 $421.58 具 +48.3%）、Nomura NT$3,425（+40.4%）；共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）",
        "🏆 中長線基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）+ TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + 全先進製程漲價 5-10%（毛利率 +2pp）",
        "⚠️ 風險續追蹤：外資 7 月連 3 賣未轉向（3 日 -16,464 張）；法說會前 NYSE 續弱；下游手機/PC 需求疲弱；Intel 18A 量產吸引微軟/高通；估值不低（台股 P/E ~32.3x、TSM ~36x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-14 Tue（最新確認收盤：台股 2330 7/13 Mon 官方收 NT$2,440（+25、+1.04% vs 7/9 NT$2,415.5、開 2,460／高 2,480／低 2,440、量 34,665 張、成交額 NT$851.13 億、均價 2,455.21、TWSE 確認）；NYSE TSM 7/13 Mon 收 $421.58（-$12.53、-2.89% vs 7/10 $434.11、盤後 $422.98）。⚠️ 7/10 Fri 因颱風「巴威」台股休市、7/13 復市）。⭐⭐ 今日最重要：TSMC 6 月營收 7/13 正式公布、大幅達標創高——6 月合併營收 NT$4,426.80 億元、YoY +67.9%、MoM +6.2%，改寫單月歷史新高；1–6 月（H1）累計 NT$2.40 兆（+35.6% YoY）；Q2 累計約 NT$1.27 兆、達成美元指引 $39.0-40.2B——AI/HPC 需求動能獲確認、大幅緩解 4-5 月僅 +24% YoY 落後華爾街 Q2 約 +35% 預期的營收疑慮、為 7/16 法說會前重大利多。⭐ 今日重點：（1）📈 台股 7/13 收 NT$2,440（+1.04%）站回 5MA、6 月營收創高撐盤——盤中一度大漲逾 900 點（受營收激勵），尾盤獲利了結翻黑收僅 +25、台積電為撐盤要角；（2）📉 NYSE TSM 7/13 收 $421.58（-2.89%）——法說會前美股半導體獲利了結、估值修正；（3）📊 三大法人 7/13：外資賣超 2,045 張（連 3 賣、3 日累計 -16,464 張）、投信買超 530 張、自營買超 623 張、合計賣超 892 張——外資賣壓大幅收斂、投信自營同步買超對沖；（4）📊 台美 ADR 溢價自 +15.3% 收斂至 +10.8%（$421.58 × 32.07 ÷ 5 = NT$2,704 vs 台股 2,440）；（5）🚀 分析師續挺：Citi NT$3,800（自 NT$2,440 具 +55.7% 空間）、Barclays $625（自 $421.58 具 +48.3%）、Nomura NT$3,425（+40.4%）、共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）；（6）🎯 催化倒數：7/16 Q2 法說會週三 14:00 倒數 2 天（法人估 Q2 毛利率挑戰 70%、指引 65.5-67.5%、Citi 預期上修全年展望、上調 2027-28 資本支出至 $75-80B）。⭐ 中長線結構性利多基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）、2nm 量產良率 70-80%、TSMC-Amkor 美國封裝長約、全先進製程漲價 5-10%（估毛利率 +2pp）、4 大 CSP 2026 AI CapEx $725B（+77% YoY）。技術面（TWSE 官方收盤）：5MA ~NT$2,424 / 20MA ~NT$2,366 / 60MA ~NT$2,210；7/13 收 NT$2,440 站回 5MA 之上、中長均線多頭排列；下方支撐 5MA ~NT$2,424 / 20MA ~NT$2,366；上方壓力前高 NT$2,480 / NT$2,500-2,535 史高。⚠️ 風險：外資 7 月連 3 賣未轉向（3 日 -16,464 張）、法說會前 NYSE 續弱、下游手機/PC 需求疲弱、Intel 18A 量產競爭、估值不低（台股 P/E ~32.3x TTM、TSM ~36x）、台幣匯率 ~32.1（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/14 Tue）關鍵觀察：（1）🎯🎯 7/16 Q2 2026 財報法說會倒數 2 天（週三 14:00）——公布 Q2 財報 + Q3 展望；6 月營收已達標創高（NT$4,426.80 億、+67.9% YoY），法人估 Q2 毛利率挑戰 70%（指引 65.5-67.5%）、Q3 美元營收再季增逾 10%；法說會前後恐劇烈波動；（2）📈 台股 7/14 能否延續 6 月營收利多、守穩 5MA ~NT$2,424 / 20MA ~NT$2,366、挑戰前高 NT$2,480；留意 7/13 尾盤翻黑（盤中 +900 點收僅 +25）顯示高檔換手；（3）📊 籌碼：外資連 3 賣（3 日 -16,464 張）——能否收斂翻買為關鍵；投信（+530）、自營（+623）同步買超為對沖買盤；（4）📉 NYSE TSM 7/13 $421.58（-2.89%）法說會前續弱，隔夜美股動能中性偏空、留意是否拖累台股；ADR 溢價 +10.8% 續收斂；（5）⚠️ 估值警示：台股 P/E ~32.3x（TTM）、TSM ~36x；台幣 ~32.1、Intel 18A 競爭、下游手機/PC 需求疲弱續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-13",       # 最新資料日（7/13 Mon；7/10 Fri 颱風休市無交易）
    "foreign":  -2045,             # 外資 7/13 賣超 2,045 張（連 3 賣、3 日累計 -16,464 張；賣壓大幅收斂）
    "trust":    530,               # 投信 7/13 買超 530 張
    "dealer":   623,               # 自營商 7/13 買超 623 張；三大法人合計 -892 張
    # 近5日（由舊到新：7/7, 7/8, 7/9, [7/10 颱風休市], 7/13）；Yahoo/T86 口徑（張）
    "foreign_5d": [83, -4155, -12749, 0, -2045],         # 外資：7/13 賣壓大幅收斂至 -2,045（7/10 颱風休市計 0）
    "trust_5d":   [539, 730, 43, 0, 530],                # 投信買超（7/10 休市計 0）
    "dealer_5d":  [-237, 468, 90, 0, 623],               # 自營（7/10 休市計 0）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       52.0,   "rsi_signal": "RSI(14) 估約 52——7/13 +1.04% 站回中軸 50 之上、6 月營收創高帶動、中性偏多、動能回穩",
    "macd":         0.20,  "macd_signal": "MACD 估約 +0.20、紅柱微張——6 月營收創高與 Q2 達標帶動短線動能回穩，惟 7/13 尾盤翻黑（盤中 +900 點收僅 +25）顯示高檔換手，待 7/16 法說會定方向",   "macd_color": "#4CAF50",
    "kdj_k":        55.0,   "kdj_d": 50.0, "kdj_j": 65.0, "kdj_signal": "KDJ K(55)/D(50)/J(65) 估——7/13 回升後 J 值翻揚、短線轉偏多、留意法說會前波動",
    "ma5":       "2,424",   # 5MA（估、TWSE 官方收盤：含 7/13 NT$2,440）
    "ma20":      "2,366",   # 20MA（估、TWSE 官方收盤）
    "ma60":      "2,210",  # 60MA（估算）
    "ma120":     "2,150",  # 120MA（估算）
    "bb_upper":  "2,520",  # 布林上軌（20MA+2σ 估）
    "bb_mid":    "2,366",   # 布林中軌（20MA）
    "bb_lower":  "2,212",  # 布林下軌（估）
    "vol_ma5":  "30,700",   # 5日均量（張、含 7/13 官方量 34,665）
    "trend":    "📈 站回 5MA、6 月營收創高撐盤：台股 2330 7/13 官方收 NT$2,440（+25、+1.04%、量 34,665 張）自 7/9 NT$2,415.5 回升、站回 5MA（~2,424）之上——受 6 月營收 NT$4,426.80 億（+67.9% YoY、創單月新高）與 Q2 累計約 NT$1.27 兆達標激勵。⚡ 盤中戲劇性：一度大漲逾 900 點、尾盤獲利了結翻黑收僅 +25，顯示高檔換手壓力。📉 NYSE TSM 7/13 收 $421.58（-2.89%）法說會前美股半導體獲利了結、估值修正，隔夜動能中性偏空。📊 技術面（官方收盤）：RSI(14) 約 52 站回中軸、MACD 約 +0.20 紅柱微張、KDJ J 值翻揚；價格站回 5MA（~2,424）之上、中長均線多頭排列（20MA ~2,366 / 60MA ~2,210）。下一觀察：（1）🎯 7/16 Q2 法說會週三 14:00 倒數 2 天——公布 Q2 財報 + Q3 展望、法人估 Q2 毛利率挑戰 70%，法說會前後恐劇烈波動；（2）台股 7/14 能否延續營收利多、守穩 5MA ~NT$2,424 / 20MA ~NT$2,366、挑戰前高 NT$2,480；（3）外資連 3 賣（3 日 -16,464 張）能否收斂翻買、投信自營同步買超對沖；（4）🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 共識強力買入為中長線支撐；（5）⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 70-80% 良率。",
    "support":  "5MA ~2,424 / 20MA ~2,366 / 2,300 (5MA、20MA、前低)",
    "resist":   "2,480 / 2,500 / 2,535 (7/13 高、整數、史高)",
    "note": "均線依 TWSE 官方收盤計算、RSI/MACD/KDJ/布林為估算，請以看盤軟體為準；7/13 官方收 NT$2,440（+1.04%）站回 5MA、6 月營收創高撐盤；壓力位（NT$2,480 / 2,500-2,535）部分為推估；下方支撐 5MA ~NT$2,424 / 20MA ~NT$2,366；🚀 Citi/Barclays/Nomura 續挺 + 6 月營收達標創高 + 漲價定價力 vs ⚠️ 外資 7 月連 3 賣 + 法說會前 NYSE 續弱 + 下游需求雜訊",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-13",       # 最新資料日（7/13 Mon 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,435",            # 台積電期貨近月（估、現貨 7/13 收 NT$2,440 附近小幅逆價差）
    "futures_oi":    "184,000",          # 未平倉量（口）（估）
    "futures_basis": "-5",               # 估小幅逆價差（7/13 現貨回升、法說會前偏保守）
    "call_oi":       "120,000",
    "put_oi":        "80,000",
    "pcr":           "0.67",             # Put/Call 比率（估、法說會前避險、中性）
    "pcr_signal":    "中性（Put/Call 估 0.67；台股 7/13 官方收 NT$2,440（+1.04%）站回 5MA、6 月營收創高達標撐盤、7/16 法說會前避險升溫；⭐ Citi/Barclays/Nomura + 共識偏多為中長線支撐；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,440",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,366", "Call 55,000 / Put 15,000", "20MA、下方支撐"),
        ("2,400", "Call 52,000 / Put 11,000", "整數關卡、支撐"),
        ("2,424", "Call 50,000 / Put 9,000",  "5MA、支撐"),
        ("2,440", "Call 45,000 / Put 7,000",  "7/13 官方收、最大痛苦（估）"),
        ("2,465", "Call 40,000 / Put 5,500",  "整數壓力"),
        ("2,480", "Call 36,000 / Put 4,500",  "7/13 盤中高"),
        ("2,500", "Call 32,000 / Put 3,500",  "整數壓力"),
        ("2,535", "Call 24,000 / Put 2,500",  "盤中史高"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/13 NT$2,440）；NT$2,440 為估最大痛苦、下方支撐 5MA ~NT$2,424 / 20MA ~NT$2,366、上方壓力 NT$2,480 / NT$2,500 / NT$2,535 史高；🎯 7/16 Q2 法說會（倒數 2 天）為近期最大變數；🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 6 月營收達標創高 + 漲價定價力偏多 vs ⚠️ 外資 7 月連 3 賣、法說會前 NYSE 續弱；中長線基底未變（NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-13 Mon 收盤；美股/韓股 7/13 收盤多未確認、沿用近期收盤估值——⚠️ 部分價格為前期估值、僅供參考，請以看盤軟體為準）
    ...
    ("記憶", "2344.TW", "華邦電 Winbond","170.00","—", "⭐ 與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC、降低對韓系 HBM 依賴（台股 7/13 收盤估）"),
    ...
    ("封裝", "3711.TW", "日月光",      "628.00", "—", "CoWoS / 先進封裝（台股 7/13 收盤估；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "67.00",  "≈持平", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（近期收盤估）"),
    ("客戶", "NVDA",    "輝達",        "210.96", "≈持平", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/10 收 $210.96 續強（7/13 確認收盤未取得、沿用近期值）；領攻 AI 晶片股多頭"),
    ...
    ("客戶", "2454.TW", "聯發科",      "3,850",  "-1.9%", "手機 SoC 客戶 9%；面臨漲價成本（7/13 盤中約 3,850、-1.91%、非官方收盤）"),
...
IR_CALENDAR = [
    ...
    ("2026-07-14", "📊 報告日：6 月營收利多消化、7/16 法說會倒數 2 天", "7/14 Tue 報告日；最新確認收盤：台股 2330 7/13 NT$2,440（+1.04%）站回 5MA、6 月營收創高達標（NT$4,426.80 億、+67.9% YoY）撐盤；NYSE TSM 7/13 $421.58（-2.89%）法說會前續弱；🎯 焦點轉向 7/16 週三 14:00 Q2 2026 財報法說會（倒數 2 天）——公布 Q2 財報 + Q3 展望、法人估 Q2 毛利率挑戰 70%（指引 65.5-67.5%）、Citi 預期上修全年展望；🚀 Citi NT$3,800 / Barclays $625 / Nomura NT$3,425 續挺；⚠️ 觀察外資連 3 賣能否收斂、投信自營同步買超對沖",  "#FFF9C4"),
...
MACRO = [
    ...
    ("台股加權指數",    "~45,300", "≈持平", False),  # 7/13 收盤估（2330 撐盤、營收創高）
    ("VIX 恐慌指數",    "18.0",    "—",     False),   # 估（未取得即時值）
    ("WTI 原油",        "$70.0",   "0.00%",  False),   # 油價持穩（估、未取得即時值）
    ("10Y 美債殖利率",  "4.50%",   "0.00pp", False),   # 美債殖利率持穩（估、未取得即時值）
    ("台幣/美元",       "~32.1",   "≈持平",  False),   # 台幣 7/13 約 32.07-32.18（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$210.96", "≈持平",  False),   # #1 TSMC 客戶（22%），7/10 收 $210.96（7/13 未確認、沿用近期值）
...
VALUATION = {
    "pe_current":   32.3,   # 同 STOCK["pe_ratio"]（7/13 官方收 NT$2,440 ÷ 近四季 EPS ~75.5、statementdog）
    ...
    "pb_current":  18.29,   # 同 STOCK["pb_ratio"]（7/13 官方收 NT$2,440 ÷ 每股淨值 ~133.4）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
    "ev_ebitda":    21.0,   # EV/EBITDA（估算）
    "peg":          1.12,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準）、基於 7/13 官方收 NT$2,440：台股 P/E ~32.3x（TTM、近四季 EPS ~75.5、statementdog）、P/B ~18.3x（每股淨值 ~133.4）；NYSE TSM 7/13 收 $421.58、USD P/E 約 36x（估）；估值仍處 5 年高位區間、PEG 約 1.1（以 5 年 EPS CAGR ~33% 計）；⭐ 6 月營收 7/13 公布 NT$4,426.80 億（+67.9% YoY、創單月新高）、Q2 累計約 NT$1.27 兆達標，大幅緩解 4-5 月僅 +24% YoY 落後預期的營收疑慮；中長線基底未變：NVIDIA #1 客戶/A16 首發 + TSMC-Amkor 美國封裝長約 + 2nm 量產 70-80% 良率 + 全先進製程漲價 5-10%",
}
...
NEWS = [
    {
        ...
        "title": "🎯 TSMC 6 月營收 7/13 公布大幅達標創高：NT$4,426.80 億（+67.9% YoY、+6.2% MoM）改寫單月新高、Q2 累計約 NT$1.27 兆達成指引",
        "body": "本週最大利多、7/16 法說會前確認基本面：（1）🎯 TSMC 7/13（延自 7/10 颱風）公布 6 月合併月營收 NT$4,426.80 億元——年增 +67.9%、月增 +6.2%，改寫史上單月歷史新高；（2）📊 1–6 月（H1）累計 NT$2.40 兆、年增 +35.6%；Q2（4–6 月）累計約 NT$1.27 兆、達成美元指引 $39.0-40.2B——AI/HPC 需求動能獲確認；（3）✅ 大幅緩解市場疑慮：4-5 月合計僅 +24% YoY 落後華爾街 Q2 約 +35% 預期，6 月強勁數字補足缺口、驗證 AI 結構性需求；（4）月增 +6.2% 與 5 月 NT$4,169.75 億對照一致。",
        "source": "財報狗 TechNews / 鉅亨網 / CNBC / TSMC IR 2026-07-13", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📅 7/16 Q2 2026 財報法說會倒數 2 天（週三 14:00）：公布 Q2 財報 + Q3 展望；法人估 Q2 毛利率挑戰 70%、Q3 續創高",
        "body": "財報季重頭戲、市場焦點轉向法說會：（1）📅 TSMC 將於 7/16 週三 14:00（文華東方 B2）舉行法說會、公布 Q2 2026 財報並展望 Q3、全年資本支出；（2）📊 6 月營收已確認 Q2 累計約 NT$1.27 兆達標，法人聚焦 Q2 毛利率——指引 65.5-67.5%、市場樂觀者估挑戰 70%；（3）🎯 展望面：法人估 Q3 美元營收再季增逾 10%、Citi 預期 TSMC 上修 2026 全年營收成長（>30%）與 2027-28 資本支出至 $75-80B；（4）📦 聚焦 CoWoS 擴產進度與 2nm/A16 先進製程稼動率。",
        "source": "中央社 / 工商時報 / 經濟日報 / investor.tsmc.com 2026-07-12 - 07-13", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 台股 2330 7/13 收 NT$2,440（+1.04%）站回 5MA：營收創高撐盤、盤中曾大漲逾 900 點尾盤翻黑收僅 +25",
        "body": "颱風休市後首個交易日、營收利多撐盤但高檔換手：（1）📈 台股 2330 7/13 Mon 官方收 NT$2,440（+25、+1.04% vs 7/9 NT$2,415.5、開 2,460／高 2,480／低 2,440、量 34,665 張、成交額 NT$851.13 億、均價 2,455.21、TWSE）站回 5MA（~2,424）；（2）⚡ 盤中戲劇性：受 6 月營收創高激勵一度大漲逾 900 點，但尾盤獲利了結翻黑、收僅 +25 點，台積電為大盤撐盤要角、顯示高檔換手壓力；（3）📊 加權指數 7/13 續站穩 45,000+；（4）技術面站回 5MA、中長均線多頭排列（20MA ~2,366 / 60MA ~2,210）。",
        "source": "聯合報 UDN / Yahoo 股市 / TWSE 2026-07-13", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "📉 NYSE TSM 7/13 收 $421.58（-2.89%）法說會前獲利了結：ADR 溢價收斂至約 +10.8%；外資連 3 賣（3 日 -16,464 張）",
        "body": "美股回檔與外資賣壓為短線壓力：（1）📉 NYSE TSM 7/13 收 $421.58（-$12.53、-2.89% vs 7/10 $434.11、盤後回升 $422.98）——法說會前美股半導體獲利了結、估值修正；自我檢核 434.11×(1-2.89%)=421.56 ✓；（2）📊 台美 ADR 溢價自 7/10 約 +15.3% 收斂至約 +10.8%（ADR $421.58 × 匯率 32.07 ÷ 5 = NT$2,704 vs 台股 2,440）——ADR 回檔 + 台股上漲雙向收斂；（3）📊 三大法人 7/13：外資賣超 2,045 張（連 3 賣、3 日累計 -16,464 張）、投信買超 530 張、自營買超 623 張、合計賣超 892 張——外資賣壓大幅收斂、投信自營同步買超對沖。",
        "source": "stockanalysis.com / Yahoo 股市法人買賣 / tradingeconomics 2026-07-13", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🚀 分析師續大幅看多：Citi NT$3,800、Barclays $625、Nomura NT$3,425；共識強力買入（32 位 0 賣出）",
        "body": "法人目標價維持高檔、6 月營收達標強化信心：（1）🚀 Citi 維持台股目標 NT$3,800（自 7/13 NT$2,440 具 +55.7% 空間）、預期 TSMC 於 7/16 上修 2026 全年營收成長（估 >30%）與 2027-28 資本支出（$75-80B）；Barclays $625（自 $421.58 具 +48.3%）、Nomura NT$3,425（自 NT$2,440 具 +40.4%）續挺；（2）📊 共識強力買入（Morningstar 統計 17 位分析師買入 0 賣出、平均目標約 $490）；（3）✅ 6 月營收創高達標為法說會前重大加分、驗證 AI 需求動能；（4）⚠️ 惟 Goldman Sachs 7/1 將 TSM 移出「APAC Conviction List」（戰術性調整、非降評）。",
        "source": "Citi / Barclays / Nomura / Morningstar 2026-07-01 - 07-13", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 風險追蹤：外資 7 月連 3 賣未轉向；法說會前 NYSE 續弱；Intel 18A 量產吸引微軟/高通；估值不低（台股 P/E ~32.3x、TSM ~36x）",
        "body": "多項雜訊與競爭變數須留意：（1）📊 籌碼面——外資 7 月對 2330 連 3 賣（7/13 -2,045 張、3 日累計 -16,464 張），尚未轉買；投信、自營同步買超為對沖買盤；（2）📉 事件風險——7/16 法說會前 NYSE TSM 續弱（7/13 -2.89%），隔夜美股動能中性偏空、留意是否拖累台股；法說會後恐劇烈波動；（3）⚔️ 競爭——Intel 在 CEO Lip-Bu Tan 帶領下代工轉型、18A 節點進入量產並吸引微軟、高通等大客戶（惟 Intel 亦為 TSMC 客戶）；下游手機/PC 需求疲弱；（4）💱 估值與匯率——台股 P/E ~32.3x（TTM）、TSM ~36x、台幣 ~32.1（每升值 1% 毛利率 -40bps）。",
        "source": "TWSE T86 / 鉅亨網 / gurufocus 2026-07-13", "impact": "負面",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,465",        # 2026-06-30 Tue 盤中（09:12 即時 +95、+4.01% vs 6/29 NT$2,370；跟進 NYSE 6/29 隔夜大漲 +5.26% 跳空補漲、台股 catch-up；官方收盤待 TWSE）
    "tw_change":      "+95",          # 6/30 盤中漲跌點數（+4.01%）
    "tw_change_pct":  "+4.01%",       # 6/30 盤中漲跌幅（自 6/29 NT$2,370 跳空大漲）
    "tw_volume":      "9,257*",       # 6/30 盤中成交量（張，*09:12 即時、未含全日；官方全日量待 TWSE）
    "tw_open":        "2,440",        # 6/30 開盤（跳空高開）
    "tw_high":        "2,475",        # 6/30 盤中高
    "tw_low":         "2,435",        # 6/30 盤中低（開盤後）
    "market_cap":     "NT$64.0 兆",   # 6/30 盤中市值（NT$2,465 基礎）
    "pe_ratio":       "~36.4x",       # 台股 P/E（TTM，NT$2,465 基礎）
    ...
    # NYSE TSM ADR
    "nyse_price":     "455.10",       # 6/29 Mon 收盤（+$22.75，+5.26%；Winbond DRAM 合作+漲價定價力+BofA 上調帶動波段反彈、創新高、市值約 $2.1 兆、P/E ~32.5；前收 $432.35）
    "nyse_change":    "+22.75",       # 6/29 漲跌（6/29 收盤確認、4:00 PM EDT）
    "nyse_change_pct":"+5.26%",       # 6/29 NYSE 大漲（Winbond 特殊 DRAM/WoW 合作降低 HBM 對 Samsung/SK 依賴 + 漲價定價力 + BofA 上調目標）
    "52w_high_nyse":  "476.79",       # 52 週/歷史高（區間）
    "52w_low_nyse":   "221.18",       # 52 週低（區間）
    "pe_nyse":        "~32.5x",       # NYSE USD 基礎 P/E（TTM，$455.10 基礎）
    "pb_nyse":        "~13.2x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-06-30 Tue（台股 2330 6/30 Tue 盤中 09:12 即時 NT$2,465（+95、+4.01%）跳空大漲、官方收盤請以 TWSE 為準；NYSE TSM 6/29 Mon 收盤 $455.10 為最新確認收盤、6/30 美股盤待台灣時間今晚開盤）。⭐ 今日頭條——台股跳空補漲、雙市共振確認漲價升級題材：（1）🚀 台股 2330 6/30 盤中跳空大漲至 NT$2,465（+95、+4.01%、開 2,440 / 高 2,475 / 低 2,435），跟進 NYSE 6/29 隔夜 +5.26% 大漲、補漲收斂台美價差；（2）🚀 NYSE TSM 6/29 Mon 大漲 +5.26%（+$22.75）收 $455.10、創波段新高、市值約 $2.1 兆（P/E ~32.5、前收 $432.35），三大催化——⭐ TSMC 與華邦電（Winbond）合作將特殊 DRAM（採 CUBE 客製超高頻寬架構）導入 WoW 3D 晶圓堆疊與 SoIC 先進封裝、把關鍵記憶體生產留在台灣、降低對 Samsung/SK 海力士 HBM 依賴（台灣在地化估縮短週期 66%、降運輸成本 90%）；⭐ 漲價定價力擴及先進與成熟製程、印證 AI 需求下議價力；⭐ Bank of America 等多家機構上調目標（彙整平均目標 $468.08、最高 $600、最低 $351）；（3）📊 台股 6/30 今日跳空後能否守穩 NT$2,440 / NT$2,400 為觀察重點、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/29 ADR $455.10 + 台股 6/30 盤中 NT$2,465、匯率 31.05 計）：理論值 5×2,465÷31.05 = $396.94、溢價 (455.10-396.94)/396.94 = +14.65%（台股補漲、溢價自 +19.25% 收斂至 +14.65%）。⚠️ 漲價雙面刃——利多毛利率（估全年 +2pp 以上），惟客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、恐促部分客戶調整下單量或轉向替代；Benzinga 指 NVIDIA 因 AI 需求剛性與轉嫁能力強、長期反為利多。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。⚠️ ITC 專利調查初判預計本週（6 月底）出爐、為短線關鍵雜訊。⭐ 中長線結構性利多基底未變：Winbond 特殊 DRAM 自主供應、TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：ITC 初判 6 月底（本週）；6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +26% YoY 達 ~$975B；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆（佔 AI 伺服器機櫃價值 95%）  |  ⚠️ 本日（6/30）關鍵觀察：（1）台股 2330 能否站回並守穩 5MA ~NT$2,396 / NT$2,400 並挑戰 NT$2,450；（2）ADR 溢價 +19.25%、台股具補漲空間；（3）下方支撐 NT$2,340 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~32.5x、台股 P/E ~35.0x，留意 ITC 初判、6 月營收 miss 風險與漲價客戶反彈",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "🚀 台股跳空補漲、雙市共振：台股 2330 6/30 盤中大漲 +4.01% 至 NT$2,465 跟進 NYSE 6/29 +5.26%；漲價題材升級「定價力 + 自主供應鏈」（Winbond 特殊 DRAM/WoW 降 HBM 對 Samsung/SK 依賴 + 漲價定價力 + BofA 上調目標）",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🚀 台股 2330 6/30 盤中跳空大漲至 NT$2,465（+95、+4.01%、開 2,440/高 2,475/低 2,435）跟進 NYSE 6/29 隔夜 +5.26% 補漲；官方收盤待 TWSE",
        "🚀 NYSE TSM 6/29 Mon 大漲 +5.26%（+$22.75）收 $455.10 創波段新高、市值約 $2.1 兆——Winbond DRAM 合作 + 漲價定價力 + BofA 上調目標三催化",
        "⭐ TSMC-Winbond 合作：特殊 DRAM（CUBE 客製超高頻寬架構）導入 WoW 3D 堆疊/SoIC、降低對 Samsung/SK 海力士 HBM 依賴、台灣在地化估縮短週期 66%/降運費 90%",
        "⭐ 漲價題材升級為定價力：擴及先進+成熟製程、佔晶圓營收 ~75%、幅度 5-10%、估貢獻全年毛利率 +2pp 以上；BofA 等上調目標（彙整平均 $468.08、高 $600、低 $351）",
        "📊 ADR 溢價自 +19.25% 收斂至 +14.65%（以 6/29 ADR $455.10 + 台股 6/30 盤中 NT$2,465、匯率 31.05 計）——台股 6/30 補漲、收斂台美價差",
        "🏆 中長線基底未變：NVIDIA #1 客戶/A16 首發 2027 + NVIDIA-TSMC AI 製程合作 + TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + Winbond 自主 DRAM 供應",
        "⚠️ 風險續追蹤：ITC 專利調查 6 月底（本週）待初判；4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）關鍵；Apple-Intel 合作、估值偏高（TSM P/E ~32.5x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-06-30 Tue（台股 2330 6/30 Tue 盤中 09:12 即時 NT$2,465（+95、+4.01%）跳空大漲、官方收盤請以 TWSE 為準；NYSE TSM 6/29 Mon 收盤 $455.10 為最新確認收盤、6/30 美股盤待今晚開盤）。⭐ 今日頭條——台股跳空補漲、台美雙市共振確認漲價升級題材：（1）🚀 台股 2330 6/30 盤中跳空大漲至 NT$2,465（+95、+4.01%、開 2,440/高 2,475/低 2,435），跟進 NYSE 6/29 隔夜 +5.26% 補漲、收斂台美價差；（2）🚀 NYSE TSM 6/29 大漲 +5.26%（+$22.75）收 $455.10、創波段新高、市值約 $2.1 兆（P/E ~32.5、前收 $432.35），三大催化——⭐ TSMC 與華邦電（Winbond）合作將特殊 DRAM（CUBE 客製超高頻寬架構）導入 WoW 3D 晶圓堆疊與 SoIC 先進封裝、把關鍵記憶體生產留在台灣、降低對 Samsung/SK 海力士 HBM 依賴（台灣在地化估縮短週期 66%、降運輸成本 90%）；⭐ 漲價定價力擴及先進與成熟製程、印證 AI 需求下議價力；⭐ Bank of America 等多家機構上調目標（彙整平均目標 $468.08、最高 $600、最低 $351）；（3）📊 台股 6/30 跳空後守穩 NT$2,440 / NT$2,400 為觀察重點、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/29 ADR $455.10 + 台股 6/30 盤中 NT$2,465、匯率 31.05 計）：理論值 5×2,465÷31.05 = $396.94、溢價 +14.65%（台股補漲、溢價自 +19.25% 收斂）。⚠️ 漲價雙面刃：利多毛利率（估全年 +2pp 以上），惟客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、恐促部分調整下單或轉向替代；Benzinga 指 NVIDIA 因 AI 需求剛性與轉嫁能力強、長期反為利多。6 月營收（7 月上旬公布）為關鍵 catalyst——4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、惟即使小幅月增亦可能使 Q2 營收逼近指引（$39.0-40.2B）中值。⚠️ ITC 專利調查初判預計本週（6 月底）出爐、為短線關鍵雜訊。⭐ 中長線結構性利多基底未變：（1）⭐ TSMC-Winbond 特殊 DRAM 自主供應、降低 HBM 對韓系依賴；（2）🤝 TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）；（3）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產，Apple 跳過 A16 直上 A14；（4）⭐ NVIDIA-TSMC 深化 AI 導入晶圓廠（cuLitho/缺陷檢測/工廠排程）；（5）⭐ 2nm 已量產、初期良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（6）4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（7）⭐ 5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆；（8）2026 全球半導體市場估 +26% YoY 達 ~$975B、SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆。⚠️ 下行風險：（1）漲價恐引發客戶反彈與訂單調整；（2）4-5 月營收 +24% YoY 落後 Q2 ~35% 預期、6 月營收為關鍵驗證；（3）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底初判恐對含 AI 加速器晶片發布美國進口禁令；（4）Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung；（5）估值警示——TSM P/E ~32.5x、台股 P/E ~35.0x；（6）台幣升值（每升 1% 毛利率 -40bps）。基本面未變：Q1 2026 淨利 NT$572.5B（$18.2B、+58.3% YoY）、毛利率 66.2% 雙創史高。技術面（基於 6/29 收 NT$2,370）：5MA NT$2,396 / 20MA NT$2,400 / 60MA NT$2,370 / 120MA NT$2,270；RSI 回升至 52、KDJ 黃金交叉、MACD 死叉風險解除；下方支撐 NT$2,340 / NT$2,310 / NT$2,250；上方壓力 5MA NT$2,396 / NT$2,450 / NT$2,490 / NT$2,535（史高）。分析師全面看多、BofA 等上調目標（彙整平均 $468.08、最高 $600）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（6/30 Tue）關鍵觀察：（1）🚀 台股 2330 6/30 盤中跳空大漲 +4.01% 至 NT$2,465 跟進 NYSE 6/29 +5.26% 補漲、台美雙市共振；官方收盤待 TWSE、留意跳空後能否守穩 NT$2,440 / NT$2,400；（2）⭐ TSMC-Winbond 特殊 DRAM（WoW 3D 堆疊/CUBE 架構）合作降低 HBM 對 Samsung/SK 依賴——本波反彈核心利多；（3）⭐ 漲價升級為定價力、擴及先進+成熟製程、估全年毛利率 +2pp 以上；（4）📈 BofA 等上調目標（彙整平均 $468.08、最高 $600、最低 $351）；（5）📊 ADR 溢價走闊至 +19.25%、台股具補漲空間；（6）⚠️ 漲價雙面刃——客戶成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多；（7）⚠️ 4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst；（8）⚠️ ITC 專利調查初判預計本週（6 月底）出爐——若不利恐對含 AI 加速器晶片發布美國進口禁令；（9）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027；（10）⚠️ 風險續追蹤——Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung；（11）⭐ 2nm 量產良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（12）⚠️ 估值警示：TSM P/E ~32.5x、台股 P/E ~35.0x；本日關鍵：台股能否站回 5MA ~NT$2,396 / NT$2,400 並挑戰 NT$2,450；下方支撐 NT$2,340 / NT$2,310 / NT$2,250",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-06-30",       # 最新資料日（6/30 Tue 盤中、官方三大法人待 TWSE 收盤後公布；以下為估算）
    "foreign":  8000,              # 外資 6/30 估買超約 8,000 張（台股跳空大漲 +4.01%、外資回補；官方待確認）
    "trust":    1200,              # 投信 6/30 估買超約 1,200 張
    "dealer":   600,               # 自營商 6/30 估買超約 600 張
    # 近5日（由舊到新：6/24, 6/25, 6/26, 6/29, 6/30）；6/29/6/30 為估算、其餘部分為估算
    "foreign_5d": [-15000, 1500, -6104, 2500, 8000],     # 外資 6/30 估買超約 8,000 張
    "trust_5d":   [800, 600, 179, 400, 1200],            # 投信 6/30 估買超約 1,200 張
    "dealer_5d":  [-500, 300, -310, 150, 600],           # 自營 6/30 估買超約 600 張
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       63.0,   "rsi_signal": "RSI 估升至 63（台股 6/30 跳空 +4.01%），動能轉強、進入偏多區、尚未超買；留意 6/30 官方收盤與後續量能確認",
    "macd":         0.40,  "macd_signal": "MACD 估升至 +0.40、紅柱放大、動能改善、死叉風險解除、跳空帶量轉強",   "macd_color": "#4CAF50",
    "kdj_k":        68.0,   "kdj_d": 56.0, "kdj_j": 88.0, "kdj_signal": "KDJ K(68)/D(56)/J(88) 黃金交叉續強、6/30 跳空帶動、短線轉強、J 接近高檔留意",
    "ma5":       "2,391",   # 5MA（6/24 2390, 6/25 2390, 6/26 2340, 6/29 2370, 6/30 2465）— 基於 6/30 盤中
    "ma20":      "2,400",   # 20MA
    "ma60":      "2,370",   # 60MA 緩升
    "ma120":     "2,270",
    "bb_upper":  "2,540",   # 布林上軌
    ...
    "bb_lower":  "2,260",   # 布林下軌
    "vol_ma5":  "37,000",   # 5日均量（張，估、基於 6/23-6/29 五日均）
    "trend":    "🚀 台股跳空補漲、台美雙市共振：台股 2330 6/30 盤中跳空大漲 +4.01% 至 NT$2,465（開 2,440/高 2,475/低 2,435）跟進 NYSE 6/29 +5.26%；漲價題材升級為「定價力 + 自主供應鏈」雙主軸（Winbond 特殊 DRAM/WoW + 漲價定價力 + BofA 上調目標）。📊 技術面（基於 6/30 盤中 NT$2,465）：跳空一舉突破 5MA NT$2,391 / 20MA NT$2,400、RSI 升至 63 進偏多區、MACD +0.40 紅柱放大、KDJ 黃金交叉續強，中期 60MA/120MA 趨勢偏多。下一觀察：（1）台股跳空後能否守穩 NT$2,440 / NT$2,400 並挑戰 NT$2,490 / NT$2,535 史高；（2）下方支撐 NT$2,400 / NT$2,370 / NT$2,340；（3）上方壓力 NT$2,490 / NT$2,535 史高；（4）⭐ 漲價+Winbond 自主 DRAM 利多、惟須留意 6 月營收（7 月上旬）與 ITC 初判（本週）；（5）⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發 + 2nm 70-80% 良率 + BofA 等上調目標。",
    "support":  "2,400 / 2,370 / 2,340 / 2,310 (整數、6/29 收、6/26 收、整數)",
    "resist":   "2,475 / 2,490 / 2,535 (6/30 盤中高、6/23 收、史高)",
    "note": "技術指標數值為估算（基於 6/30 盤中 NT$2,465），請以看盤軟體即時數據為準；台股 6/30 跳空 +4.01% 跟進 NYSE 6/29 +5.26%、突破 5MA/20MA、RSI 升至 63、KDJ 黃金交叉續強、MACD 紅柱放大；上方壓力 NT$2,490 / NT$2,535 史高、下方支撐 NT$2,400；⭐ 漲價+Winbond 自主供應利多 vs ⚠️ 6 月營收與 ITC 初判雜訊拉鋸；官方收盤待 TWSE",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-06-30",       # 最新資料日（6/30 Tue 盤中估、6/30 盤後待 TAIFEX 公布）
    "futures_price": "2,468",            # 台積電期貨近月（6/30 盤中附近、小幅正價差）
    "futures_oi":    "172,000",          # 未平倉量（口）（6/30 估）
    "futures_basis": "+3",               # 6/30 估小幅正價差（雙市共振、情緒偏樂觀）
    "call_oi":       "112,000",
    "put_oi":        "76,000",
    "pcr":           "0.68",             # Put/Call 比率（6/30 估 0.68、台股跳空大漲、避險買盤降、情緒偏樂觀）
    "pcr_signal":    "中性偏多（Put/Call 6/30 估 0.68；台股 6/30 跳空 +4.01%、NYSE 6/29 +5.26%、雙市共振、情緒偏樂觀、期貨小幅正價差；⭐ 漲價+Winbond 自主供應為中長線支撐）",
    ...
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；6/29 PCR 自 0.75 降至 0.72 反映台股反彈 +1.28% 及 NYSE 隔夜 +5.26% 帶動情緒轉樂觀、期貨轉小幅正價差；NT$2,400 為最大痛苦、下方支撐 NT$2,340 / NT$2,310、上方壓力 NT$2,450；⭐ 漲價+Winbond 自主 DRAM 供應結構性利多、⚠️ 留意 ITC 初判、6 月營收 miss 風險與客戶反彈；中長線基底未變（NVIDIA #1 客戶/A16 首發、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約、BofA 等上調目標）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-06-29 Mon 收盤、台股為 6/29 Mon 收盤；價格為估算、6/29 半導體普遍隨 TSM 反彈）
    ("設備", "ASML",    "艾司摩爾",    "875.00", "+1.70%", "EUV 設備龍頭（6/29 隨半導體反彈；高 NA EUV 需求結構性多頭基底未變）"),
    ("設備", "AMAT",    "應用材料",    "528.00", "+1.90%", "🛠️ 新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）；TSMC-Winbond WoW DRAM 合作受惠；6/29 反彈"),
    ("設備", "LRCX",    "科林研發",    "1,055.00", "+1.90%", "蝕刻設備（6/29 反彈；半導體設備需求中長線創紀錄）"),
    ("設備", "KLAC",    "科磊",        "795.00", "+1.90%", "量測設備（6/29 反彈；2nm 量產驅動）"),
    ("記憶", "2344.TW", "華邦電 Winbond","32.50","+9.80%", "⭐ 與 TSMC 合作將 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC、降低台灣對 Samsung/SK HBM 依賴；6/29 大漲（本波反彈新核心、價格為估算）"),
    ("記憶", "000660.KS","SK 海力士",  "388,000 KRW", "-0.50%", "⚠️ TSMC-Winbond 自主 DRAM 對 HBM 供應形成中長線替代壓力；惟 HBM4 需求仍強、6/29 第 7 代 HBM4E 送樣"),
    ("記憶", "MU",      "美光",        "150.00", "+2.70%", "HBM / DDR5（6/29 反彈；HBM4 需求中長線強勁、惟記憶體價格雜訊）"),
    ("封裝", "3711.TW", "日月光",      "324",    "+1.57%", "CoWoS / 先進封裝（6/29 隨大盤反彈；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "51.50",  "+3.00%", "TSMC-Amkor 簽 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（擴及韓國）；6/29 反彈（本波利多核心）"),
    ("客戶", "NVDA",    "輝達",        "205.00", "+2.50%", "🏆 確認 TSMC #1 客戶 22%、A16 首發客戶 2027 量產；市值逾 $5 兆；Benzinga 指漲價長期反利多"),
    ("客戶", "AAPL",    "蘋果",        "300.00", "+1.00%", "⚠️ #2 客戶 18%；面臨 TSMC 漲價成本；Apple 同意與 Intel 合作在美生產晶片；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "470.00", "+1.73%", "⚠️ #3 客戶（6/29 反彈；面臨漲價成本；custom XPU 中長線強勁）"),
    ("客戶", "AMD",     "超微",        "518.00", "+1.97%", "#4 客戶（6/29 反彈；面臨漲價成本；瞄準 $1,200 億資料中心市場、MI350/MI450；傳接觸 Samsung）"),
    ("客戶", "2454.TW", "聯發科",      "4,720",  "+0.85%", "手機 SoC 客戶 9%；6/29 反彈；面臨漲價成本；RTX Spark 由聯發科協同 NVIDIA 設計"),
    ("競爭", "INTC",    "英特爾",      "50.80",  "+1.20%", "Apple 將與 Intel 合作在美設計製造晶片；18A-P 進入風險量產（效能 +9%）；與 UMC 結盟挑戰 TSMC"),
    ("競爭", "005930.KS","三星電子",   "66,800 KRW", "+0.45%", "⚠️ TSMC-Winbond 自主 DRAM 削弱其 HBM 議價；傳 Google/AMD/Tesla 接觸先進製程；HBM4E 送樣"),
    ("處分後", "ARM",   "Arm Holdings","258.00", "+1.18%", "TSMC 4/29 全數出脫後（6/29 反彈；RTX Spark ARM 架構中長線受惠）"),
]
...
NEWS = [
    {
        ...
        "title": "🚀 台股跳空補漲、台美雙市共振：台股 2330 6/30 盤中大漲 +4.01% 至 NT$2,465 跟進 NYSE 6/29 +5.26% 收 $455.10 創波段新高；⭐ TSMC-Winbond 特殊 DRAM 合作（WoW 3D 堆疊）降低 HBM 對 Samsung/SK 依賴",
        "body": "今日頭條——台股跳空補漲、台美雙市共振確認漲價升級題材：（1）🚀 台股 2330 6/30 Tue 盤中跳空大漲至 NT$2,465（+95、+4.01% vs 6/29 NT$2,370、開 2,440/高 2,475/低 2,435、09:12 即時、官方收盤待 TWSE），跟進 NYSE 6/29 隔夜 +5.26% 補漲、突破 5MA/20MA；（2）🚀 NYSE TSM 6/29 Mon 大漲 +5.26%（+$22.75）收 $455.10、創波段新高、市值約 $2.1 兆（P/E ~32.5、前收 $432.35）；（3）⭐ 核心催化：TSMC 與華邦電（Winbond）合作，將其 CUBE（Customized Ultra-Bandwidth Elements）特殊 DRAM 導入 WoW（wafer-on-wafer）3D 晶圓堆疊與 SoIC 先進封裝，把關鍵記憶體生產留在台灣、降低對 Samsung/SK 海力士 HBM 依賴——台灣在地化估縮短週期 66%、降運輸成本 90%；（4）📊 ADR 溢價自 +19.25% 收斂至 +14.65%（以 6/29 ADR $455.10 + 台股 6/30 盤中 NT$2,465、匯率 31.05 計）、台股補漲收斂台美價差。",
        "source": "Yahoo奇摩股市 / stockanalysis / ainvest / TradingView 2026-06-29 - 06-30", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📈 Bank of America 等多家機構上調 TSM 目標價、視為全球 AI 資本支出循環核心受惠者；彙整平均目標 $468.08、最高 $600",
        "body": "法人評級全面轉強、為 6/29 ADR 大漲助攻：（1）📈 Bank of America 等主要金融機構上調 TSMC 目標價，理由為其為全球 AI CapEx 循環核心受惠者、漲價印證定價力、Winbond 合作強化自主供應鏈；（2）🎯 市場彙整分析師目標：平均 $468.08、最高 $600.00、最低 $351.00，對應自 6/29 收 $455.10 仍有上行空間；（3）⭐ TSMC 預估本季 EPS $3.77（+52.6% YoY）、全年共識 EPS $15.35（+44.1% YoY），AI 結構性需求驅動獲利強勁成長；（4）🏆 NVIDIA 仍為 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產，NVIDIA-TSMC 深化 AI 導入晶圓製造。",
        "source": "TradingKey / Zacks / Yahoo Finance 2026-06-29", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "⭐ TSMC 漲價升級為定價力：擴及先進與成熟製程、佔晶圓營收 ~75%、幅度 5-10%；估貢獻全年毛利率 +2pp 以上",
        "body": "漲價題材結構性利多持續發酵、印證 TSMC 在 AI 需求下之議價力：（1）⭐ 據 Tom's Hardware / Yahoo Finance 等報導，TSMC 對 7nm 以下全部先進製程（含 N7/N5/N3、佔晶圓營收約 75%）調漲 5-10%、視客戶/節點/產品而定、部分已開始實施，並要求客戶將更高成本計入未來訂單；（2）💰 以平均 5% 估算，可貢獻全年毛利率 +2pp 以上；TSMC 已將 2026 美元營收成長目標上修至 30% 以上；（3）⚠️ 漲價雙面刃——客戶（Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、Benzinga 指 NVIDIA 因 AI 需求剛性與轉嫁能力強、長期反為利多，惟毛利較薄之消費性客戶影響較大；（4）💰 TSMC 規劃今年 9 期晶圓廠與先進封裝廠、2nm/A16 產能 2026-28 估 CAGR ~70%。",
        "source": "Tom's Hardware / Yahoo Finance / Benzinga 2026-06-24 - 06-29", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⚠️ TSMC 遭美 ITC 專利調查、初判預計本週（6 月底）出爐；恐對含關鍵 AI 加速器技術晶片發布美國進口禁令——短線關鍵雜訊",
        "body": "潛在法律與供應鏈風險、本週為關鍵時點：（1）⚠️ 美國國際貿易委員會（USITC）正對 TSMC 展開調查，源於 Longitude Licensing 與 Marlin Semiconductor 針對其先進製程節點的專利侵權控訴；（2）⚠️ 預計 6 月底（本週）作出初步裁決——若不利於 TSMC，理論上可能對使用關鍵 AI 加速器技術製造的晶片發布美國進口禁令、為近期股價雜訊來源之一；（3）⭐ 影響評估：此類 ITC 案件多以授權金和解收場、實際全面進口禁令機率不高，但涉及 AI 加速器核心技術、若裁定不利恐衝擊 NVIDIA/AMD 等客戶供應鏈與市場情緒；（4）✅ TSMC 具龐大專利組合與法務資源；（5）🎯 觀察本週/7 月初 ITC 初判結果與 TSMC 回應。",
        "source": "USITC / Reuters / Bloomberg / ts2.tech 2026-06-18 - 06-29", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 近期營收疑慮續存：TSMC 4-5 月營收 +24% YoY 落後華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst",
        "body": "近期營收動能為短線焦點、漲價利多須營收驗證：（1）⚠️ TSMC 4 月與 5 月合併營收年增合計約 +24% YoY，低於華爾街對 Q2 約 +35% 預期，引發近期營收 miss 疑慮；（2）🎯 6 月合併營收（7 月上旬公布）成為關鍵 catalyst——分析師指出即使僅小幅月增，也可能使 Q2 營收逼近公司指引（$39.0-40.2B）中值；（3）✅ 緩解觀點：5 月營收 NT$4,169.75 億（+30.1% YoY）創單月新高、1-5 月累計 NT$1.96 兆，公司被認為 well-positioned 達成 Q2 指引；C.C. 魏重申「2026 美元營收成長 30% 以上」；（4）⭐ 中長線需求未變：2026 全球半導體市場估 +26% YoY 達 ~$975B；（5）🎯 7 月上旬 6 月營收 + 7/16 Q2 法說會將為短線方向關鍵。",
        "source": "ts2.tech / Seeking Alpha / Focus Taiwan 2026-06-24 - 06-29", "impact": "負面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 客戶分散與競爭雜訊續存：Apple-Intel 在美合作生產、Google/AMD/Tesla 接觸 Samsung；Intel 18A-P 風險量產+UMC 結盟",
        "body": "客戶分散與競爭為 TSMC 中期觀察重點、漲價或加速雙供應評估：（1）⚠️ Apple 已同意與 Intel 合作在美國本土製造晶片、以降低對 TSMC 依賴；（2）⚠️ Google、AMD、Tesla 傳積極接觸 Samsung Foundry 先進製程尋求第二供應；Intel 18A-P 進入風險量產（同功耗效能 +9%）並與台灣 UMC 結盟挑戰 TSMC；（3）⚠️ TSMC 漲價可能促使部分價格敏感客戶加速評估替代來源；（4）✅ 緩解因素：TSMC 在 N3/N2/A16 具明顯技術代差與良率領先、Apple 仍跳過 A16 直上 TSMC A14（1.4nm）、Winbond 合作補強記憶體自主供應；（5）🎯 觀察 TSMC 最先進節點訂單能見度仍滿載、惟須留意成熟製程市占與長期客戶集中度變化。",
        "source": "Reuters / CNBC / Bloomberg / ts2.tech 2026-06-18 - 06-29", "impact": "負面",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,350",        # 台股 2330 最新確認收盤：7/27 Mon 官方收 NT$2,350（0、0.00% vs 7/24 NT$2,350、開 2,330／高 2,365／低 2,330、TWSE 確認）——低開 -20 點後盤中翻紅高 2,365、尾盤收平盤、守住 60MA 2,345；大盤同日僅小跌 -20.65 點（-0.05%）收 43,634.19、週五恐慌未再蔓延
    "tw_change":      "0",            # 7/27 Mon 收盤漲跌點數（0.00%、TWSE 官方、平盤）
    "tw_change_pct":  "0.00%",        # 7/27 Mon 收盤漲跌幅（低開 2,330 → 盤中高 2,365 → 收平盤 2,350、下影線守穩）
    "tw_volume":      "28,939",       # 7/27 成交量（張、TWSE 口徑；成交股數 28,939,466 股、成交額 NT$678.85 億、成交 92,753 筆——量回增（前日 24,810 張）而成交筆數自 195,859 驟降至 92,753、週五恐慌性小單出脫明顯退潮）
    "tw_open":        "2,330",        # 7/27 開盤（TWSE、低開 -20 點、即全日最低）
    "tw_high":        "2,365",        # 7/27 盤中高（TWSE、盤中一度翻紅 +15 點）
    "tw_low":         "2,330",        # 7/27 盤中低（TWSE、開盤即低點、隨後收復）
    "market_cap":     "NT$60.9 兆",   # 7/27 收盤市值（NT$2,350 基礎、約 259.3 億股）
    "pe_ratio":       "~26.9x",       # 台股 P/E（TTM 估，NT$2,350 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25）
    "pb_ratio":       "~17.6x",       # 台股 P/B（NT$2,350 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.02%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,350）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "399.09",       # 7/27 Mon 官方收盤 $399.09（-$4.32、-1.07% vs 7/24 $403.41、Yahoo Finance API 確認）——失守 $400 心理關卡但跌幅遠小於同業（相對抗跌）；SOX 同日 -2.23% 收 11,554.88（自 6/22 史高 14,634.66 收斂至 -21.0%、正式跌入熊市區）
    "nyse_change":    "-4.32",        # 7/27 漲跌（自 7/24 $403.41）
    "nyse_change_pct":"-1.07%",       # 7/27 AI 高 Beta 續挫：NVDA -5.00% 失守 $200／AMD -5.17%／ASML -5.79%／AMKR -6.54%；惟 AAPL +1.17% 連 2 漲、AVGO +0.34%／QCOM +1.84%／ARM +2.43% 翻紅——跌勢集中 AI 權值、非全面性
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~29.2x",       # NYSE USD 基礎 P/E（TTM 估，$399.09 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~16.8x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-28 Tue（最新確認收盤：台股 2330 7/27 Mon 官方收 NT$2,350（0、0.00% vs 7/24 NT$2,350、開 2,330／高 2,365／低 2,330、量 28,939 張 TWSE 口徑、成交額 NT$678.85 億、成交 92,753 筆、TWSE 確認）；NYSE TSM 7/27 Mon 收 $399.09（-$4.32、-1.07% vs 7/24 $403.41、Yahoo Finance API 確認））。📊 今日格局：個股平盤抗跌 vs 外部熊市共振——（1）2330 7/27 低開 2,330（-20）後盤中翻紅高 2,365、尾盤收平盤 2,350（0.00%）——週五美股重挫下未跟跌、守住 60MA 2,345；量回增至 28,939 張、成交筆數自 195,859 驟降至 92,753、恐慌小單退潮；（2）✅ 大盤 7/27 僅小跌 -20.65 點（-0.05%）收 43,634.19——週五恐慌未蔓延、驚魂初定；（3）✅ 籌碼明顯改善：外資對 2330 賣超大幅收斂至 -2,662 張（-9,637 → -2,662、連 4 賣但減速）、投信 +567 連 2 買、自營 +1,410、三大合計僅 -685 張；全市場 BFI82U 由賣轉買：三大法人小幅買超 6.6 億——外資 +80.4 億轉買、投信 +4.8 億、自營 -78.6 億；（4）⚠️ 美股 7/27 AI 高 Beta 續挫：TSM -1.07% 收 $399.09 失守 $400（跌幅遠小於同業、相對抗跌）、SOX -2.23% 收 11,554.88（自 6/22 史高 14,634.66 收斂至 -21.0%、正式跌入熊市區）——NVDA -5.00% 失守 $200 收 $196.51、AMD -5.17%、ASML -5.79%、AMKR -6.54%、LRCX -4.46%、AMAT -3.61%；惟 AAPL +1.17% 收 $336.91 連 2 漲、AVGO +0.34%／QCOM +1.84%／ARM +2.43% 翻紅——市場評論指賣壓屬槓桿部位強制賣出（forced selling）、跌勢集中 AI 權值非全面性；（5）⚠️ 韓股 7/28 早盤重挫：SK 海力士財報日開盤急殺逾 -10%（至 ~162.6 萬韓元）、三星 -8.9%（盤中、Yahoo API）——SK Q2 市場預期營益率 75-77% 創紀錄仍遭拋售、「財報優仍被賣」極端化、為今日台股開盤最大逆風；（6）⭐ 油價續回落：WTI ~$82／布蘭特 ~$88（美伊暫停軍事行動延續）、VIX 18.67 持平、10Y 美債 4.64%（-4bp）。📊 台美 ADR 溢價 +9.7%（$399.09 vs 理論值 ~$363.7、匯率 ~32.31）——連 3 日收斂（+11.6% → +11.0% → +9.7%）。技術面（官方收盤計算至 7/27）：收 2,350 守住 60MA 2,345（僅 5 點之上）、5MA 下彎至 2,383；RSI(14) 46.4 持平中軸下、MACD 綠柱續擴至 -12.7 且 DIF -0.8 跌落零軸下（DEA 11.9）——中期動能第一道裂痕、KDJ 死叉延續（K41.1/D43.7/J35.9）；支撐 2,345（60MA）／2,330（7/27 低）／2,320（7/20 收）／2,307（布林下軌）、壓力 2,383（5MA）／2,400-2,405（前收盤帶）／2,414（20MA/布林中軌）／2,440-2,445。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、股利 +33%、2027 全面漲價 5-10% 傳聞＋2nm Q3 放量；法說會後外資目標價續上調（亞系外資 NT$4,200、UBS NT$3,400、MS NT$3,100；惟一家降評中立 NT$2,700 看法分歧）。⚠️ 風險：SOX 正式跌入熊市、NVDA 失守 $200、SK 海力士財報日重挫外溢、本週美股四大巨頭財報（MSFT/META 7/29、AAPL/AMZN 7/30）AI CapEx 指引牽動訂單能見度、美 232 條款半導體關稅 25%、台幣 ~32.31。⭐ 今日（7/28）觀察：（1）韓股重挫外溢下能否再守 60MA 2,345／7/27 低 2,330；（2）失守則下看 2,320／2,307（布林下軌）；（3）外資賣壓收斂（-2,662）能否延續、投信連買能否擴大；（4）SK 海力士財報電話會對 HBM 供需的定調＋今晚美股能否止穩。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏空",
    # 一句話標題
    "headline":   "📊 個股抗跌 vs 外部熊市共振：台股 2330 7/27 收 NT$2,350（0.00%）低開翻紅守住 60MA、大盤僅小跌 -0.05%；外資賣壓大幅收斂 -2,662 張、全市場三大法人轉小幅買超；⚠️ 惟 NYSE TSM 7/27 收 $399.09（-1.07%）失守 $400、SOX -2.23% 自史高 -21.0% 正式跌入熊市、NVDA -5.00% 失守 $200；⚠️ SK 海力士 7/28 財報日早盤重挫逾 -10%、三星 -8.9%——今日台股開盤大逆風；MSFT/META（7/29）、AAPL/AMZN（7/30）財報為本週關鍵",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 2330 7/27 官方收 NT$2,350（0、0.00%、開 2,330／高 2,365／低 2,330）——低開 -20 後盤中翻紅、尾盤收平盤、守住 60MA 2,345；量回增 28,939 張、成交筆數自 195,859 驟降至 92,753、恐慌小單退潮",
        "✅ 大盤 7/27 僅小跌 -20.65 點（-0.05%）收 43,634.19——週五恐慌未蔓延；2330 平盤在美股重挫下屬明顯抗跌",
        "✅ 籌碼改善：外資對 2330 賣超大幅收斂 -2,662 張（-9,637 → -2,662、連 4 賣但減速）、投信 +567 連 2 買、自營 +1,410；全市場 BFI82U 由賣轉買：三大法人小幅買超 6.6 億——外資 +80.4 億轉買",
        "⚠️ 美股 7/27 AI 高 Beta 續挫：TSM -1.07% 收 $399.09 失守 $400、SOX -2.23% 收 11,554.88（自史高 -21.0%、正式跌入熊市區）——NVDA -5.00% 失守 $200、AMD -5.17%、ASML -5.79%；惟 AAPL +1.17% 連 2 漲、AVGO/QCOM/ARM 翻紅——市場評論指屬槓桿部位強制賣出、跌勢集中 AI 權值",
        "⚠️ 韓股 7/28 早盤重挫：SK 海力士財報日開盤急殺逾 -10%、三星 -8.9%（盤中）——SK Q2 市場預期營益率 75-77% 創紀錄仍遭拋售、「財報優仍被賣」極端化，今日台股開盤最大逆風",
        "⭐ 油價續回落：WTI ~$82／布蘭特 ~$88（美伊暫停軍事行動延續）、VIX 18.67 持平、10Y 美債 4.64%——地緣與通膨溢價緩解為背景正面因子",
        "💡 技術面：收 2,350 守 60MA 2,345（僅 5 點）；RSI 46.4 持平中軸下、MACD 綠柱 -12.7 且 DIF -0.8 跌落零軸、KDJ 死叉延續（K41.1/D43.7/J35.9）；支撐 2,345／2,330／2,320／2,307、收復 5MA 2,383 為修復第一關",
        "📊 本週焦點：美股四大巨頭財報（MSFT/META 7/29、AAPL/AMZN 7/30）——AI CapEx 指引直接牽動 TSMC 先進製程訂單能見度；SK 海力士財報電話會對 HBM 供需的定調亦為 AI 敘事風向球",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-28 Tue（最新確認收盤：台股 2330 7/27 Mon 官方收 NT$2,350（0、0.00% vs 7/24 NT$2,350、開 2,330／高 2,365／低 2,330、量 28,939 張 TWSE 口徑、成交額 NT$678.85 億、成交 92,753 筆、TWSE 確認）；NYSE TSM 7/27 收 $399.09（-$4.32、-1.07%、Yahoo API 確認））。📊 核心判讀：個股抗跌 vs 外部熊市共振——（1）2330 週一展現韌性：低開 2,330（-20）後盤中翻紅高 2,365、尾盤收平盤 2,350——週五美股重挫下未跟跌、守住 60MA 2,345；量回增 28,939 張、成交筆數自 195,859 驟降至 92,753、恐慌小單退潮；（2）大盤僅小跌 -20.65 點（-0.05%）收 43,634.19——週五恐慌未蔓延、驚魂初定；（3）籌碼明顯改善：外資對 2330 賣超自 -9,637 大幅收斂至 -2,662 張（連 4 賣但減速）、投信 +567 連 2 買、自營 +1,410、三大合計僅 -685 張；全市場 BFI82U 由賣轉買 +6.6 億（外資 +80.4 億轉買）——上週五「籌碼轉空」的最壞情境未延續；（4）⚠️ 惟外部風險升級：美股 7/27 AI 高 Beta 續挫——TSM -1.07% 收 $399.09 失守 $400（相對抗跌）、SOX -2.23% 自史高收斂 -21.0% 正式跌入熊市區、NVDA -5.00% 失守 $200、AMD -5.17%、ASML -5.79%；市場評論指屬槓桿部位強制賣出（forced selling）、跌勢集中 AI 權值非全面性（AAPL +1.17% 連 2 漲、AVGO/QCOM/ARM 翻紅）；（5）⚠️ 韓股 7/28 早盤重挫：SK 海力士財報日開盤急殺逾 -10%、三星 -8.9%（盤中）——市場預期 SK Q2 營益率 75-77% 創紀錄仍遭拋售、「財報優仍被賣」極端化——今日台股開盤最大逆風、觀察 2330 能否再現週一韌性。技術面：RSI 46.4 持平中軸下、MACD 綠柱 -12.7 且 DIF -0.8 首度跌落零軸（中期動能第一道裂痕）、KDJ 死叉延續——守住 60MA 2,345 則中期架構未破、失守下看 2,320／2,307（布林下軌）；收復 5MA 2,383 → 2,400-2,405 → 20MA 2,414 為修復三關卡。📊 ADR 溢價連 3 日收斂至 +9.7%（匯率 ~32.31）——美股領跌、台股補跌壓力遞減。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2027 漲價傳聞＋2nm Q3 放量；法說會後外資目標價續上調（亞系 NT$4,200、UBS NT$3,400、MS NT$3,100）。⭐ 今日（7/28）觀察：（1）韓股重挫外溢下守 60MA 2,345／2,330；（2）外資賣壓收斂能否延續；（3）SK 財報電話會 HBM 供需定調；（4）今晚美股止穩與否＋MSFT/META 財報（7/29）前卡位。⚠️ 風險：SOX 熊市、財報週 AI CapEx 指引不及預期、美 232 條款關稅 25%、台幣 ~32.31（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/28 Tue）關鍵觀察：（1）⚠️ 韓股開盤重挫為最大逆風：SK 海力士財報日早盤急殺逾 -10%、三星 -8.9%（盤中、Yahoo API）——市場預期 SK Q2 營益率 75-77% 創紀錄仍遭拋售、「財報優仍被賣」極端化；SK 財報電話會對 HBM 供需與資本支出的定調為 AI 記憶體敘事風向球、直接外溢台股半導體；（2）⚠️ SOX 正式跌入熊市（自 6/22 史高 -21.0%）、NVDA 失守 $200（-5.00%）——市場評論指賣壓屬槓桿部位強制賣出（forced selling）、此類賣壓常超跌但結束也急；TSM -1.07% 相對抗跌、AAPL/AVGO/QCOM/ARM 翻紅顯示跌勢集中 AI 高 Beta 非全面性；（3）📊 2330 多空檢核：守 60MA 2,345／7/27 低 2,330 則週一韌性延續、失守下看 2,320／2,307（布林下軌）；MACD DIF -0.8 首度跌落零軸為中期動能第一道裂痕、收復 5MA 2,383 為修復第一關；（4）✅ 正面因子：外資對 2330 賣壓大幅收斂 -2,662 張＋投信連 2 買＋全市場 BFI82U 轉買超 +6.6 億、油價續回落（WTI ~$82）、ADR 溢價收斂至 +9.7% 補跌壓力遞減；（5）📅 本週 MSFT/META（7/29）、AAPL/AMZN（7/30）財報 AI CapEx 指引為 TSMC 訂單能見度直接風向球；💱 美 232 條款關稅 25%、台幣 ~32.31 續為背景風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-27",       # 最新資料日（7/27 Mon、TWSE T86 官方）
    "foreign":  -2662,             # 外資 7/27 賣超 2,662 張（連 4 賣但大幅收斂：-9,637 → -2,662、減速逾七成——賣壓明顯鈍化）
    "trust":    567,               # 投信 7/27 買超 567 張（連 2 買：+550 → +567——回檔持續回補）
    "dealer":   1410,              # 自營商 7/27 買超 1,410 張（連 3 買且擴大）；三大法人合計僅 -685 張（賣超自 -8,781 急縮）
    # 近5日（由舊到新：7/21, 7/22, 7/23, 7/24, 7/27）；TWSE T86 口徑（張）
    "foreign_5d": [4976, -3539, -4614, -9637, -2662],   # 外資（連 4 賣、7/27 大幅收斂）
    "trust_5d":   [974, 1562, -943, 550, 567],           # 投信（7/24 起連 2 買）
    "dealer_5d":  [214, -1059, 38, 305, 1410],           # 自營（連 3 買且擴大）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       46.4,   "rsi_signal": "RSI(14) 46.4（官方收盤計算）——7/27 平盤、RSI 持平於 50 中軸之下；中性偏弱區整理、回升站上 50 為修復訊號",
    "macd":         -12.7, "macd_signal": "MACD 綠柱續擴至 -12.7（DIF -0.8 < DEA 11.9、官方收盤計算）——⚠️ DIF 自 2.9 跌落 -0.8、跌破零軸——中期動能出現第一道裂痕；綠柱重新收斂＋DIF 收復零軸為修復訊號",   "macd_color": "#F44336",
    "kdj_k":        41.1,   "kdj_d": 43.7, "kdj_j": 35.9, "kdj_signal": "KDJ K(41.1)/D(43.7)/J(35.9)（官方收盤計算）——死叉延續第 2 日、K/D/J 同步下行但 J 值 35.9 已近低檔區；平盤未能扭轉動能、快速重新金叉需先收復 5MA 2,383",
    "ma5":       "2,383",   # 5MA（官方收盤計算；7/27 收 2,350 位於其下——修復第一關）
    "ma20":      "2,414",   # 20MA（官方收盤計算；修復第三關卡、與布林中軌重合）
    "ma60":      "2,345",  # 60MA（官方收盤計算；7/27 收 2,350 僅 5 點之上——中期多空分界、當前最關鍵支撐）
    "ma120":     "2,126",  # 120MA（官方收盤計算）
    "bb_upper":  "2,520",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,414",   # 布林中軌（20MA）
    "bb_lower":  "2,307",  # 布林下軌（20MA-2σ、官方收盤計算；失守 60MA 後下一支撐）
    "vol_ma5":  "29,002",   # 5日均量（張、TWSE 口徑；7/27 量 28,939 貼近均量、未見恐慌大量）
    "trend":    "📊 個股平盤抗跌、外部熊市共振：台股 2330 7/27 官方收 NT$2,350（0、0.00%、開 2,330／高 2,365／低 2,330）——低開 -20 後盤中翻紅高 2,365、尾盤收平盤、守住 60MA 2,345；大盤同日僅小跌 -20.65 點（-0.05%）收 43,634.19、週五恐慌未蔓延。📊 技術指標（官方收盤計算）：KDJ 死叉延續（K41.1/D43.7/J35.9、J 近低檔）、RSI(14) 46.4 持平中軸下、MACD 綠柱續擴 -12.7 且 ⚠️ DIF -0.8 跌破零軸（自 2.9 轉負）——中期動能第一道裂痕。✅ 籌碼改善：外資對 2330 賣超大幅收斂 -2,662 張（-9,637 → -2,662）、投信 +567 連 2 買、自營 +1,410 連 3 買、全市場 BFI82U 轉買超 +6.6 億。⚠️ 外部風險升級：NYSE TSM 7/27 收 $399.09（-1.07%）失守 $400、SOX -2.23% 收 11,554.88 自史高 -21.0% 正式跌入熊市——NVDA -5.00% 失守 $200、AMD -5.17%、ASML -5.79%；惟 AAPL 連 2 漲、AVGO/QCOM/ARM 翻紅、屬槓桿斷頭式集中賣壓。⚠️ 韓股 7/28 早盤重挫：SK 海力士財報日急殺逾 -10%、三星 -8.9%（盤中）——今日開盤最大逆風。下一觀察：（1）守 60MA 2,345／7/27 低 2,330 則週一韌性延續；（2）收復 5MA 2,383 → 2,400-2,405 → 20MA 2,414 為修復三關卡；（3）失守 2,345 下看 2,320／2,307（布林下軌）；（4）SK 財報電話會 HBM 定調＋本週美股四大巨頭財報（MSFT/META 7/29、AAPL/AMZN 7/30）。",
    "support":  "2,345 / 2,330 / 2,320 / 2,307 (60MA、7/27 低、7/20 收、布林下軌)",
    "resist":   "2,383 / 2,400-2,405 / 2,414 / 2,440-2,445 (5MA、前收盤帶、20MA+布林中軌、7/22 開盤高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-27 止），請以看盤軟體為準；7/27 官方收 NT$2,350（0.00%）低開翻紅收平盤、守住 60MA 2,345（僅 5 點之上）；⚠️ KDJ 死叉延續（K41.1/D43.7/J35.9）、RSI 46.4 持平中軸下、MACD 綠柱 -12.7 且 DIF -0.8 跌破零軸；支撐 2,345 / 2,330 / 2,320 / 2,307、壓力 2,383 / 2,400-2,405 / 2,414 / 2,440-2,445；✅ 外資賣壓收斂 -2,662 張＋投信自營同買＋BFI82U 轉買超 vs ⚠️ SOX 跌入熊市＋NVDA 失守 $200＋韓股 7/28 早盤重挫——守 2,345 與收復 5MA 2,383 為今日多空雙檢核",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-27",       # 最新資料日（7/27 Mon 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,346",            # 台積電期貨近月（估、現貨 7/27 收 NT$2,350 平盤、貼水略收斂）
    "futures_oi":    "180,000",          # 未平倉量（口）（估、平盤整理日部位變動有限）
    "futures_basis": "-4",               # 估逆價差（平盤日貼水自 -6 略收斂）
    "call_oi":       "107,000",
    "put_oi":        "97,000",
    "pcr":           "0.91",             # Put/Call 比率（估、較 7/24 的 0.93 微降——現貨平盤守穩、惟隔夜美股續挫＋韓股 7/28 早盤重挫、避險需求仍高）
    "pcr_signal":    "中性偏空（Put/Call 估 0.91；7/27 現貨平盤收 NT$2,350、外資賣壓收斂，惟隔夜 SOX 跌入熊市、NVDA 失守 $200、SK 海力士 7/28 早盤重挫逾 -10%，避險需求難降；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,350",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,290", "Call 45,000 / Put 12,000", "7/17 低、極端支撐"),
        ("2,307", "Call 42,000 / Put 11,000", "布林下軌、下方支撐"),
        ("2,320", "Call 40,000 / Put 10,000", "7/20 收、回檔支撐"),
        ("2,330", "Call 38,000 / Put 9,500",  "7/27 低、當日防線"),
        ("2,345", "Call 36,000 / Put 9,000",  "60MA、中期多空分界"),
        ("2,350", "Call 35,000 / Put 8,000",  "最大痛苦（估）、收盤位"),
        ("2,383", "Call 31,000 / Put 6,000",  "5MA、修復第一關"),
        ("2,405", "Call 28,000 / Put 5,000",  "前收盤帶、短線壓力"),
        ("2,414", "Call 27,000 / Put 4,500",  "20MA/布林中軌、修復第三關"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/27 NT$2,350）；NT$2,350 為估最大痛苦、下方支撐 2,345（60MA）/ 2,330（7/27 低）/ 2,320 / 2,307（布林下軌）、上方壓力 2,383（5MA）/ 2,400-2,405 / 2,414（20MA）/ 2,440-2,445；✅ 籌碼改善：外資賣壓收斂 -2,662 張＋BFI82U 轉買超 vs ⚠️ SOX 熊市＋SK 海力士 7/28 財報日早盤重挫——貼水略收斂但避險需求仍高；✅ 基本面錨未變（2nm Q3 放量、2027 漲價傳聞、全年上修 >40%、CapEx $60-64B）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-27 Mon TWSE 官方收盤；美股為 7/27 Mon 收盤、Yahoo Finance API 確認；韓股為 7/27 收盤、7/28 早盤重挫）
    ("設備", "ASML",    "艾司摩爾",    "1,655.26", "-5.79%", "📉 EUV 設備龍頭；7/27 領跌設備板塊、連 2 日重挫；TSMC CapEx 上調至 $60-64B 為設備鏈中長線利多"),
    ("設備", "AMAT",    "應用材料",    "516.89", "-3.61%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作；7/27 設備股續挫、連 2 日回落"),
    ("設備", "LRCX",    "科林研發",    "291.61", "-4.46%", "蝕刻設備（7/27 連 2 日重挫）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "203.36", "-3.40%", "量測設備（7/27 連 2 日回落）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","160.00","+3.56%", "✅ 止跌反彈（7/27 收 160.0、TWSE 官方——終結連 2 日回吐）；與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC 題材未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,816,000 KRW", "+3.24%", "HBM4、與 NVIDIA 多年期合作；7/27 反彈收 181.6 萬韓元（Yahoo API）；⚠️ 7/28 Q2 財報日早盤急殺逾 -10% 至 ~162.6 萬（盤中）——市場預期營益率 75-77% 創紀錄仍遭拋售"),
    ("記憶", "MU",      "美光",        "900.20", "-2.25%", "📉 HBM / DDR5；7/27 續跌收 $900.2（自 7/23 高 $990 累計 -9.1%）——記憶體高位獲利了結延續"),
    ("封裝", "3711.TW", "日月光",      "608.00", "-0.82%", "CoWoS / 先進封裝（7/27 小跌收 608、TWSE 官方——跌勢趨緩）"),
    ("封裝", "AMKR",    "Amkor",       "60.71",  "-6.54%", "📉 TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/27 補跌 -6.54%）"),
    ("客戶", "NVDA",    "輝達",        "196.51", "-5.00%", "⚠️ TSMC #1 客戶 22%/$33B、A16 首發客戶；7/27 重挫 -5.00% 收 $196.51 失守 $200——AI 權值槓桿賣壓集中點；NVIDIA-TSMC 將 AI 導入晶圓廠製造"),
    ("客戶", "AAPL",    "蘋果",        "336.91", "+1.17%", "🚀 #2 客戶 18%；7/27 續漲 +1.17% 收 $336.91 連 2 日逆勢——7/30 財報前資金避風；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "383.22", "+0.34%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/27 小幅翻紅、止跌）"),
    ("客戶", "AMD",     "超微",        "494.95", "-5.17%", "📉 #4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/27 續挫 -5.17% 失守 $500、連 3 日回檔）"),
    ("客戶", "2454.TW", "聯發科",      "3,680",  "-1.87%", "手機 SoC 客戶 9%（7/27 連 2 跌收 3,680、TWSE 官方）"),
    ("競爭", "INTC",    "英特爾",      "91.67",  "-0.70%", "7/24 財報重挫 -7.89% 後 7/27 跌勢趨緩（-0.70% 收 $91.67）；Q2 EPS/營收雙超預期但 GAAP 虧損＋foundry 客戶進展疑慮；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "254,000 KRW", "+1.80%", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/27 反彈收 25.4 萬韓元（Yahoo API）；⚠️ 7/28 早盤重挫 -8.9% 至 ~23.15 萬（盤中）"),
    ("產業", "SOX",     "費城半導體指數","11,554.88", "-2.23%", "⚠️ 7/27 續挫 -2.23% 收 11,554.88——自 6/22 史高 14,634.66 收斂至 -21.0%、正式跌入熊市區；NVDA/AMD/ASML 領跌、惟 AVGO/QCOM/ARM 翻紅、跌勢集中 AI 高 Beta"),
    ("處分後", "ARM",   "Arm Holdings","266.33", "+2.43%", "TSMC 4/29 全數出脫後（7/27 反彈 +2.43%——前日重挫 -8.14% 後高 Beta 雙向波動）"),
]
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,343.84", "-0.54%", False),  # 7/23 收（Yahoo）終結連 2 紅；自 6/22 史高收斂至 -15.7%
    ("台股加權指數",    "44,850.81", "+0.06%", True),   # 7/23 官方收盤（+25.03 點、尾盤翻紅連 3 紅、TWSE）
    ("VIX 恐慌指數",    "18.70",    "+12.38%", False),  # 7/23 收（Yahoo）自 16.64 明顯回升、risk-off 訊號
    ("WTI 原油",        "$92.16",   "+6.1%",  False),   # 7/23-24 近月（Yahoo CL=F；地緣溢價急升）
    ("10Y 美債殖利率",  "4.70%",   "+4bp",   False),    # 7/23 收（Yahoo ^TNX、連日走升）
    ("台幣/美元",       "~32.30",   "小升",  False),    # 7/24 Yahoo TWD=X（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$208.76", "-1.56%",  False),   # #1 TSMC 客戶（22%），7/23 隨 AI 權值回檔
    ("ASML",            "$1,803.00", "+0.06%",  True),  # 設備股 7/23 持平整理（Yahoo）
]
...
VALUATION = {
    "pe_current":   27.6,   # 同 STOCK["pe_ratio"]（7/21 官方收 NT$2,410 ÷ 近四季 EPS ~87.4、含 Q2 EPS NT$27.25）
    ...
    "pb_current":  18.07,   # 同 STOCK["pb_ratio"]（7/21 官方收 NT$2,410 ÷ 每股淨值 ~133.4）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
...
RELATIVE = {
    "date":           "2026-07-21",       # 最新資料日（台股 2330 與 NYSE TSM 均為 7/21 Tue 收盤）
    "tsmc_tw_1d":     3.88,    # 2330 7/21 官方收 NT$2,410（+90 vs 7/20 NT$2,320、TWSE）
    "taiex_1d":       4.20,    # 加權指數 7/21 收 44,232.87（+1,783.17、史上最大單日漲點、TWSE 官方）
    "sox_1d":         5.21,    # SOX 7/21 收 12,356.16（+5.21% 大漲、Yahoo）
    "tsmc_nyse_1d":   5.55,    # TSM ADR 7/21 $424.61（+$22.31；日經漲價報導激勵）
    "tsmc_tw_5d":     -0.41,   # 5 日（NT$2,410 vs 7/14 官方收 NT$2,420）
    "taiex_5d":       -1.13,   # （44,232.87 vs 7/14 44,737.95）
    "tsmc_tw_1m":     -3.2,    # 月線（NT$2,410 vs 6/23 官方收 NT$2,490）
    "taiex_1m":       -3.0,    # （估）
    "tsmc_tw_ytd":    +15.0,   # YTD 累計（NT$2,410 vs 年初 ~NT$2,095）
    "taiex_ytd":      +10.2,   # （估）
    "sox_ytd":        +44.2,   # SOX YTD（費半 12,356.16、估）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/21 NYSE 收盤 & 7/21 TW 官方收盤計算）
    "adr_price":     424.61,   # TSM NYSE 7/21 收盤（+5.55%、Yahoo Finance API）
    "adr_parity":    372.72,   # 理論值 = 5 × NT$2,410 ÷ 32.33
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.33,   # 台幣匯率（7/22 Yahoo TWD=X）
    "adr_premium_pct": "+13.92", # (424.61 - 372.72) / 372.72 × 100 = +13.92%（匯率 32.33）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 7/21 Tue 收盤：台股 NT$2,410（+3.88%、量 31,606 張 TWSE 口徑、市值 NT$62.5 兆）——加權指數同日 +4.20%（+1,783.17 點、史上最大單日漲點）；NYSE $424.61（+5.55%）——同日 SOX +5.21%、台美與板塊同步強彈。📊 籌碼面（T86 官方）：7/21 外資終結連 8 賣、正式轉買 +4,976 張，投信 +974（連 4 買）、自營 +214（連 9 買）、合計 +6,163 張連 2 日買超；全市場三大法人買超 162.9 億（BFI82U：投信 +171.1 億、外資仍小賣 -43.3 億）。📊 台美 ADR 溢價 +13.92%（$424.61 vs 理論值 $372.72、匯率 32.33）——美股領漲、溢價自 +12.0% 擴大，台股 7/22 具補漲題材。🚀 催化：日經報導 TSMC 2027 年全面漲價 5-10%（HPC 追加訂單再 +10-15%）；分析師續挺：Susquehanna $600、Barclays $650、S&P Global 19 位平均 $520.37；⭐ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm 貢獻 3% + CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "⚠️ SOX 正式跌入熊市（自史高 -21.0%）、NVDA 失守 $200——槓桿斷頭賣壓",
        "body": "7/27（一）美股 AI 高 Beta 續挫：（1）⚠️ 費城半導體指數 -2.23% 收 11,554.88——自 6/22 史高 14,634.66 收斂至 -21.0%、正式跌入熊市區；NVDA -5.00% 收 $196.51 失守 $200、AMD -5.17% 失守 $500、ASML -5.79%；（2）📊 NYSE TSM -1.07% 收 $399.09 失守 $400 心理關卡——惟跌幅遠小於同業、相對抗跌；（3）💡 Jim Cramer 指賣方屬「規模龐大、動機明確且帶槓桿」的強制賣壓（forced selling）——此類機械性賣壓常超跌但結束也急；（4）✅ AAPL +1.17% 連 2 漲、AVGO/QCOM/ARM 翻紅——跌勢集中 AI 權值、非全面性崩跌。",
        "source": "Yahoo Finance / 24-7 Wall St. 2026-07-27", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ SK 海力士財報日早盤重挫逾 -10%——「財報優仍被賣」極端化",
        "body": "今晨（7/28）台股開盤最大逆風：（1）⚠️ SK 海力士 Q2 財報日（7/28）早盤股價急殺逾 -10% 至 ~162.6 萬韓元、三星同步重挫 -8.9%（盤中、Yahoo API）；（2）📊 市場原預期 SK Q2 營益率上看 75-77%、連 3 季超越台積電的歷史紀錄——創紀錄預期下仍遭拋售、延續 INTC「財報優仍被賣」模式且更極端；（3）🎯 SK 財報電話會對 HBM 供給持續性與資本支出循環的定調，為 AI 記憶體敘事的直接風向球；（4）⚠️ 韓股重挫外溢效應下、觀察 2330 能否再現週一低開翻紅的韌性。",
        "source": "Yahoo Finance API / TrendForce / moomoo 2026-07-28 盤中", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "✅ 籌碼改善：外資對 2330 賣壓收斂至 -2,662 張、全市場轉小幅買超",
        "body": "上週五「籌碼轉空」的最壞情境未延續：（1）✅ 外資對 2330 7/27 賣超 -2,662 張（T86 官方）——連 4 賣但自 -9,637 大幅收斂逾七成、賣壓明顯鈍化；（2）✅ 投信 +567 張連 2 買、自營 +1,410 張連 3 買且擴大——三大法人合計僅 -685 張（前日 -8,781）；（3）✅ 全市場 BFI82U 官方由賣轉買：三大法人買超 6.6 億——外資 +80.4 億轉買（前日 -609.5 億）、投信 +4.8 億；（4）🎯 外資賣壓收斂配合股價守住 60MA 2,345——賣壓若續縮並轉買、配合收復 5MA 2,383 為反彈重啟訊號。",
        "source": "TWSE T86 / BFI82U 官方 2026-07-27", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⭐ 油價續回落 WTI ~$82：美伊暫停軍事行動延續、地緣溢價消退",
        "body": "地緣與通膨壓力持續緩解：（1）⭐ 美伊暫停軍事攻擊行動延續，WTI 原油自上週 ~$92 高點回落至 ~$82、布蘭特 ~$88——地緣供應中斷溢價快速消退；（2）📊 10Y 美債殖利率回落至 4.64%（-4bp）、VIX 18.67 持平——risk-off 情緒未再升級；（3）✅ 台股 7/27 大盤僅小跌 -0.05% 收 43,634.19、週五恐慌未蔓延；（4）💡 油價與利率回落為半導體估值的背景正面因子——惟短線盤面仍由 AI 槓桿賣壓與韓股重挫主導。",
        "source": "Yahoo Finance API 2026-07-27~28", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📅 明後兩日四大巨頭財報：MSFT/META 7/29、AAPL/AMZN 7/30",
        "body": "本週為 AI 需求敘事的關鍵驗證窗口：（1）📅 微軟、Meta 於 7/29（美東）公布財報，蘋果、亞馬遜 7/30 登場——台北時間 7/30、7/31 清晨揭曉；（2）📊 市場聚焦 AI 資本支出指引——雲端業者 CapEx 續航力直接牽動 TSMC 先進製程訂單能見度；（3）💡 AAPL 7/27 續漲 +1.17% 收 $336.91 連 2 日逆勢——財報前資金避風、半導體拋售中一枝獨秀；（4）⚠️ 若 AI CapEx 指引不及預期、SOX 熊市格局恐再探底；反之為槓桿賣壓見底的催化劑。",
        "source": "CNBC / TipRanks 2026-07-27", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "💰 外媒聚焦 TSM $390 關卡：創紀錄基本面 vs 技術面熊市的拉鋸",
        "body": "評價面糾結成為外媒焦點：（1）📊 FXLeaders 指 TSM 面臨 $390 重要測試——巨額 CapEx（$60-64B＋Arizona 追加）與中國晶片自主化競爭為市場疑慮；（2）📊 Cryptonomist 指 TSM「困在創紀錄財報與惡化技術面之間」——Q2 營收 $40.2B（+33.7% YoY）、毛利率 67.7% 的史上最強基本面未能止跌；（3）✅ 基本面錨未變：全年上修 >40%、2nm Q3 放量、2027 漲價傳聞、法說會後外資目標價最高 NT$4,200；（4）🎯 本輪回檔屬估值與籌碼調整、非基本面轉向——$390（ADR）／60MA 2,345（台股）為多空關鍵防線。",
        "source": "FXLeaders / Cryptonomist 2026-07-27", "impact": "中性",
    },
]

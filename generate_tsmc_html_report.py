...
SUMMARY = {
    ...
    "bullets": [
        ...
        "✅ NYSE TSM 6/8 反彈 +2.80% 收 $426.80（自 6/5 $415.17 回升 $11.63）消化 Broadcom 利空",
...
ECOSYSTEM_STOCKS = [
    ...
    ("設備", "ASML",    "艾司摩爾",    "765.80", "+3.41%", "EUV 設備龍頭（6/8 反彈 +3.41%；半導體連動回升）"),
    ("設備", "AMAT",    "應用材料",    "237.60", "+3.62%", "⭐ TSMC EPIC Center $5B AI 合作續存（6/8 反彈；2nm 量產帶動需求）"),
    ("設備", "LRCX",    "科林研發",    "935.50", "+3.35%", "蝕刻設備（6/8 反彈；半導體設備需求 $36.55B 創紀錄）"),
    ("設備", "KLAC",    "科磊",        "703.40", "+3.40%", "量測設備（6/8 反彈；2nm 量產驅動）"),
    ...
    ("客戶", "AMD",     "超微",        "464.20", "+2.63%", "#4 客戶 8%（6/8 反彈；半導體連動回升）"),
    ("客戶", "2454.TW", "聯發科",      "4,400",  "-2.66%", "手機 SoC 客戶 9%；6/8 跟跌（資金輪出半導體）"),
    ("競爭", "INTC",    "英特爾",      "28.40",  "+3.65%", "🤝 Intel CEO Lip-Bu Tan 6/2 確認 TSMC「長期信任夥伴」（6/8 反彈）"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,810.50","+3.05%", True),   # 6/8 SOX 反彈 +3.05% 自 6/5 12,431.25 回升
    ("台股加權指數",    "43,950.20","-2.49%", False),  # 6/8 加權指數跳空補跌（估）
    ("VIX 恐慌指數",    "19.8",    "-1.7",   True),    # 恐慌指數自 21.5 回落、避險情緒緩解
    ...
    ("ASML",            "$765.80", "+3.41%", True),    # 設備股 6/8 反彈 +3.41%
]
...
RELATIVE = {
    ...
    "sox_1d":         +3.05,   # SOX 6/8 反彈 +3.05% 自 12,431.25 回升至 ~12,810
...
NEWS = [
    ...
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🤖 NVIDIA-TSMC 合作 AI 製程：cuLitho 計算微影 +20-50%、cuEST 等效散射 ×50、Omniverse FabTwin",
        "body": "NVIDIA 與 TSMC 6/1 Computex Taipei 期間宣布深化 AI 製程合作、為長線結構性利多：（1）⭐ cuLitho（計算微影）：以 NVIDIA Blackwell GPU 加速 EUV 光罩設計、效率提升 20-50%、縮短先進製程光罩開發時間；（2）⭐ cuEST（等效散射工具）：以 NVIDIA 加速 50 倍光學近場效應 (OPC) 模擬計算；（3）⭐ Omniverse FabTwin：完整數位孿生晶圓廠模型、提升 yield 與營運效率；（4）⭐ 對 TSMC 意涵：（a）2nm GAA 量產初期良率 70-80% 領先全球部分受惠於 NVIDIA AI 工具；（b）3nm 漲價 15% + 2027 再漲 5-10% 的客戶接受度提升；（c）強化 NVIDIA-TSMC 戰略夥伴關係、NVIDIA 為 TSMC 第 1 大客戶 22% 地位鞏固；（5）TSM 6/8 反彈 +2.80% 反映此合作的中長線結構性利多；（6）⭐ 6/8 全市場半導體反彈：SOX +3.05%、NVDA +2.20%、ASML +3.41%、AMAT +3.62%；（7）對應 NVIDIA $150B/年投資台灣承諾、AI 算力供應鏈台灣化趨勢強化。",
        "source": "EE News Europe / NVIDIA / Stocktitan 2026-06-01 - 06-08", "impact": "正面",
    },
]

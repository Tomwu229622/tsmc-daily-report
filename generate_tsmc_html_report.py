...
STOCK = {
    # 台股 2330
    "tw_price":       "1,905",        # 收盤價（數字加逗號，不含 NT$）
    "tw_change":      "+35",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "+1.87%",       # 漲跌幅（含+/-號）
    "tw_volume":      "23,015",       # 成交量（張）
    "tw_open":        "1,900",
    "tw_high":        "1,910",
    "tw_low":         "1,890",
    "market_cap":     "NT$49.4 兆",
    "pe_ratio":       "~28.8x",       # 台股 P/E（EPS TTM=66.25, P=1905）
    "pb_ratio":       "~9.14x",       # 台股 P/B（股淨比）
    "div_yield":      "~1.21%",       # 殖利率（NT$23/NT$1,905）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "336.71",       # 收盤（2026-03-18 EST）
    "nyse_change":    "-17.85",       # 較前一交易日下跌（市場廣泛修正）
    ...
    "pb_nyse":        "~8.62x",
    "beta":           "1.56",
    "div_next":       "US$0.968/ADR（2026-03-17 已除息）",  # 已除息
    "ex_date":        "2026-03-17（已除息）",
    "next_earnings":  "2026-04-16",
    # 資料日期備註
    "data_note":      "台股數據：2026-03-18 收盤（已確認）  |  NYSE 數據：2026-03-18 收盤（EST）  |  前2月累計營收年增29.9%（NT$7,189億）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "台股 2330 3/18 逆勢收紅 +1.87% 至 NT$1,905，抵抗 NYSE TSM 大跌展現資金承接韌性",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330（3/18 確認）：收 NT$1,905（+35，+1.87%），成交量 23,015 張；NYSE TSM 跌 -5.03% 後台股逆勢收紅，顯示本地資金強勁承接",
        "NVIDIA 超越 Apple 成台積電最大客戶：2026 年 NVIDIA 貢獻 TSMC 營收估 $330 億（~22%）；Apple 降至 $270 億（~18%）",
        "TSMC 2nm N2P 試量產 3 月啟動，Apple 率先導入：A20 iPhone 晶片採 2nm，良率爬升中；NVIDIA A16 首個長線大客戶確立",
        "TSMC 全球晶圓代工市佔達 72%，Samsung 僅 7%：AI GPU、ASIC 需求推動純晶圓代工市場 Q4 2025 年增 20%",
        "全球半導體市場 2026 年料首破 $1 兆美元（年增 30.7%），AI 晶片貢獻 50% 總營收；4/16 Q1 財報為催化劑",
    ],
    # 一句結論
    "bottom_line": "台股 2330 在 NYSE TSM 重挫 -5.03% 後逆勢收紅 +1.87% 至 NT$1,905，展現強勁基本面支撐。NVIDIA 超越 Apple 成最大客戶、2nm N2P 試量產啟動、CoWoS 訂單積壓 4 季，中長線基本面穩健。法人共識目標 NT$2,290（+20.2% 上行空間），4/16 Q1 財報為下一重要催化劑。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "",
}
...
INSTITUTIONAL = {
    "date":     "2026-03-18",
    "foreign":  +2800,    # 外資（正=買超）— 台股逆勢收紅，外資估計回補（估）
    "trust":    +350,     # 投信（持續買超）
    "dealer":   -120,     # 自營商（小幅避險）
    # 近5日（由舊到新）
    "foreign_5d": [+4800, -3200, +1500, -3800, +2800],
    "trust_5d":   [+920,  +450,  +380,  +320,  +350],
    "dealer_5d":  [-250,  -680,  -150,  -280,  -120],
    "foreign_ownership_pct": "75.8%",  # 外資持股比例（估，回補後微升）
    "foreign_ownership_shares": "約 196.5 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       56.3,   "rsi_signal": "中性偏強（台股逆勢上漲，動能回升）",
    "macd":         +2.8,   "macd_signal": "MACD 黃金交叉訊號浮現，短線動能轉正",   "macd_color": "#4CAF50",
    "kdj_k":        62.4,   "kdj_d": 55.8, "kdj_j": 69.1, "kdj_signal": "黃金交叉確立，多方優勢",
    "ma5":       "1,892",
    "ma20":      "1,926",
    "ma60":      "1,840",
    "ma120":     "1,728",
    "bb_upper":  "2,040",   # 布林上軌
    "bb_mid":    "1,926",   # 布林中軌（20MA）
    "bb_lower":  "1,812",   # 布林下軌
    "vol_ma5":  "26,800",   # 5日均量（張）
    "trend":    "台股 2330 於 3/18 逆勢收紅 +1.87% 至 NT$1,905，突破 1,900 整數關卡且收量縮（23,015 張 < 5日均量），顯示籌碼穩定。RSI 回升至 56，MACD 轉正，短線看回不回，下一壓力位 1,930–1,950（20MA 附近）。",
    "support":  "1,870–1,890",
    "resist":   "1,930–1,950",
...
DERIVATIVES = {
    "date":          "2026-03-18",
    "futures_price": "1,908",      # 台積電期貨近月（估，台股逆勢收紅後正價差回升）
    "futures_oi":    "68,500",     # 未平倉量（口）（估，空單回補後下降）
    "futures_basis": "+3",         # 小幅正價差（偏多訊號，反映台股韌性）
    "call_oi":       "38,500",
    "put_oi":        "42,000",
    "pcr":           "1.09",       # Put/Call 比率回落（情緒趨中性）
    "pcr_signal":    "中性",
    "max_pain":      "1,900",      # 最大痛苦價格（上移至整數關卡）
    "key_strikes": [
        ("1,850", "Call 4,200 / Put 8,200", "強支撐"),
        ("1,880", "Call 6,500 / Put 7,500", "重要支撐"),
        ("1,900", "Call 11,200 / Put 8,800","最大痛苦點"),
        ("1,930", "Call 9,800 / Put 5,200", "重要壓力"),
        ("1,960", "Call 6,500 / Put 3,200", "強壓力"),
    ],
...
CONSENSUS = {
    ...
    "upside":       "+20.2%",   # 從 NT$1,905 到 NT$2,290
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,114",   "-4.8%",  False),  # 3/18 廣泛修正
    ("美元指數 DXY",    "103.5",   "-0.3%",  False),
    ("VIX 恐慌指數",    "24.8",    "-1.7",   True),   # 市場情緒回穩
    ("WTI 原油",        "$73.8",   "+0.8%",  True),
    ("10Y 美債殖利率",  "4.26%",   "-0.02pp",True),
    ("台幣/美元",       "31.62",   "-0.03",  True),
    ("NVIDIA NVDA",     "$855",    "+4.9%",  True),   # GTC 2026 發表 Feynman 大漲
    ("ASML",            "$681",    "+5.2%",  True),   # EUV 設備回彈
]
...
RELATIVE = {
    "date":           "2026-03-18",
    "tsmc_tw_1d":     +1.87,   # 2330 3/18 確認（逆勢上漲 +1.87%，展現韌性）
    "taiex_1d":       +0.45,   # 加權指數（估算，台股同步小幅收紅）
    "sox_1d":         -4.80,   # SOX（半導體股廣泛修正）
    "tsmc_nyse_1d":   -5.03,   # TSM ADR 3/18 收盤
    "tsmc_tw_5d":     -1.3,
    "taiex_5d":       -0.8,
    "tsmc_tw_1m":     -5.5,
    "taiex_1m":       -2.8,
    ...
    # ADR 折溢價
    "adr_price":     336.71,   # TSM NYSE 收盤（2026-03-18 EST）
    "adr_parity":    301.27,   # 理論值 = 5 × NT$1,905 ÷ 31.62（台股 3/18 確認）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.62,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+11.8",# (336.71 - 301.27) / 301.27 × 100
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 EPS 共識 NT$20.38，前 2 月累計營收年增 29.9%（略低於市場預估 33%，部分造成 3/18 NYSE TSM 跌 -5.03%）。GTC 2026 NVIDIA Feynman 採 TSMC A16 長線利多不變，但短線市場對增速放緩有所反應。4/16 Q1 財報為下一重要催化劑，若實際超預期則可扭轉短線偏空情緒。建議密切追蹤台股 3/19 開盤反應。",
...
NEWS = [
    {
        ...
        "title": "NVIDIA 超越 Apple 成台積電最大客戶，2026 年貢獻營收估 $330 億",
        "body": "NVIDIA CEO Jensen Huang 於播客訪談確認，NVIDIA 已正式超越 Apple 成為 TSMC 最大單一客戶。Ben Bajarin（Creative Strategies）預估 NVIDIA 2026 年貢獻 TSMC 約 $330 億（~22% 總營收），Apple 退居第二（$270 億，~18%）。NVIDIA GTC 2026 同期發表 Feynman 架構 AI 晶片，確認採 TSMC A16（1.6nm）製程，預計 2028 年量產，為 TSMC 長線最具確定性的訂單來源。此一客戶結構轉變標誌 AI 基礎設施取代消費電子成為半導體最大成長動能。",
        "source": "CNBC / Creative Strategies / eWeek 2026-03-18", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "TSMC 2nm N2P 試量產 3 月啟動，Apple 率先量產；AMD Venice 同步跟進",
        "body": "業界消息指出，TSMC 增強版 2nm 製程（N2P）試量產已於 2026 年 3 月正式啟動，Apple 成為首批量產客戶，用於 A20 晶片（iPhone 18 系列）及 M6 系列處理器。AMD 亦將 EPYC「Venice」伺服器 CPU（Zen 6 架構）導入 N2，目標 2026 下半年量產。N2P 相較 N3E 可降低 20-25% 功耗、提升 10-15% 效能，良率爬升中，預計 H2 2026 進入全面量產階段，台積電成全球唯一具備 2nm 量產能力廠商。",
        "source": "Design-Reuse / TweakTown / Motley Fool 2026-03-18", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "台積電全球晶圓代工市佔達 72%，AI GPU/ASIC 訂單推動 Q4 2025 年增 20%",
        "body": "Counterpoint Research 最新報告顯示，TSMC 在純晶圓代工市場市佔率達 72%，Samsung Foundry 僅 7%，差距創歷史新高。純晶圓代工市場 Q4 2025 年增 20%，全年增長 26%，主要由 AI GPU（NVIDIA GB300 系列）、雲端業者自研 ASIC（Google TPU、Amazon Trainium）及 HPC 處理器驅動。N3/N5 製程合計貢獻超過 55% 的晶圓收入，先進製程稼動率持續滿載，客戶排隊期達 4 季。",
        "source": "Benzinga / Counterpoint Research 2026-03-18", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "全球半導體市場 2026 年首破 $1 兆美元，AI 晶片貢獻 50% 總營收",
        "body": "Omida 預測 2026 年全球半導體市場規模將首次突破 $1 兆美元（年增 30.7%），AI 晶片雖僅占出貨量 0.2%，卻貢獻約 50% 總營收。計算與儲存段預計年增 41.4% 至 $5,000 億以上。AI 資本支出 2026 年料達 $7,200 億，每 $1 CapEx 中 TSMC 估受益 $0.22，長線潛在訂單能見度極強。Goldman Sachs 預估 TSMC 2026/2027 年收分別年增 30%/28%，Q1 2026 毛利率指引 63-65% 維持高位。",
        "source": "Omida / Goldman Sachs / Meyka 2026-03-18", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "記憶體晶片短缺延伸至 2027 年，NAND 價格 Q1 2026 預估漲 38%",
        "body": "Synopsys CEO Sassine Ghazi 表示，半導體缺芯危機將持續至 2026-2027 年，絕大多數 HBM（高頻寬記憶體）產能流向 AI 資料中心，導致汽車、消費電子、工業市場嚴重缺貨。NAND 價格 Q1 2026 預估進一步上漲 38%（TrendForce），企業 SSD 部分報價自 2025 年底已翻倍。此趨勢對 TSMC 直接影響中性（台積電本身不生產記憶體），但有助鞏固 AI 算力晶片優先於記憶體的產能分配邏輯。",
        "source": "CNBC / TrendForce 2026-03-18", "impact": "中性",
    },
    {
        "tag": "公司治理", "tag_color": "#607D8B",
        "title": "TSMC 2026 AGM 議案提交窗口開放（3/31–4/7）；4/16 Q1 財報倒計時 28 天",
        "body": "TSMC 已通知 ADS 持有人，2026 年股東常會議案提交窗口為 3/31–4/7，可透過花旗銀行股東服務提交。Q1 2026 財報排於 4/16，共識 EPS NT$20.38，前 2 月累計營收年增 29.9%（$7,189 億）。機構法人將在財報前積極調整持倉，配合 Q1 財報公告前 AGM 議程，4 月初至中旬為重要觀察窗口。31 位分析師 0 位賣出，平均目標 NT$2,290（較現價 +20.2%）。",
        "source": "TipRanks / StockTitan / MarketScreener 2026-03-17", "impact": "中性",
    },
]

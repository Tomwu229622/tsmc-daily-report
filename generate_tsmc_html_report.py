...
STOCK = {
    # 台股 2330
    "tw_price":       "1,810",        # 收盤價（數字加逗號，不含 NT$）（2026-03-25 收盤）
    "tw_change":      "-30",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-1.63%",       # 漲跌幅（含+/-號）
    "tw_volume":      "17,000",       # 成交量（張，估）
    "tw_open":        "1,835",
    "tw_high":        "1,840",
    "tw_low":         "1,785",
    "market_cap":     "NT$46.81 兆",
    "pe_ratio":       "~34.4x",       # 台股 P/E（TTM，2026-03-25）
    ...
    # NYSE TSM ADR
    "nyse_price":     "343.25",       # 收盤（2026-03-24 EST）
    "nyse_change":    "+4.80",        # 較前收盤上漲
    "nyse_change_pct":"+1.42%",
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-25 收盤  |  NYSE 數據：2026-03-24 收盤  |  Broadcom 警告 TSMC 2026 年產能已成瓶頸  |  NVIDIA 超越 Apple 成最大客戶（$330 億/22%）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏空",
    # 一句話標題
    "headline":   "台股 2330 收 NT$1,810（-1.63%），跌破 1,830 支撐；Broadcom 警告 TSMC 產能瓶頸，NYSE TSM 3/24 +1.42%",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330（3/25 收盤）：NT$1,810（-30，-1.63%）；NYSE TSM（3/24 收盤）US$343.25（+4.80，+1.42%），台美仍背離",
        "Broadcom 警告 TSMC 晶圓產能已成 2026 年全球供應鏈最大瓶頸，雷射元件與 PCB 亦短缺，AI 需求超出製造能力",
        "NVIDIA Feynman 架構傳首採 TSMC A16（1.6nm），跳過 2nm；NVIDIA 2026 年貢獻 $330 億美元（22%）超越 Apple",
        "荷姆茲海峽局勢威脅台積電 LNG 能源供應，伊朗攻擊卡達致氦氣短缺，晶圓廠關鍵原料風險升溫",
        "4/16 Q1 財報倒計時 22 天，EPS 共識 NT$20.38；2026 年資本支出上看 $560 億美元（年增 40%+）",
    ],
    # 一句結論
    "bottom_line": "台股 2330 今日收 NT$1,810（-1.63%），跌破先前支撐 1,830-1,840，短線續弱。NYSE TSM（3/24 收盤）US$343.25（+1.42%）相對強勢，顯示美國市場對 AI 需求仍有信心。Broadcom 警告 TSMC 產能瓶頸為新風險因素，但同時印證需求持續強勁。NVIDIA Feynman 採 A16 與 $560 億資本支出指引為長線利多。荷姆茲/能源供應為近期地緣風險。Q1 財報（4/16）為關鍵催化劑，支撐 1,785-1,800，壓力 1,835-1,840。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ AGM 提案截止日 3/31-4/7；Q1 財報 4/16 倒計時 22 天；荷姆茲海峽能源/氦氣供應風險升溫；台股跌破 1,830 支撐",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-25",
    "foreign":  -4100,    # 外資（負=賣超）— 延續近期調節（估，台股跌破支撐）
    "trust":    +280,     # 投信（逢低承接）
    "dealer":   -220,     # 自營商（避險小賣）
    # 近5日（由舊到新）
    "foreign_5d": [-6500, -4200, -3200, +1500, -4100],
    "trust_5d":   [+280,  +310,  +350,  +290,  +280],
    "dealer_5d":  [-320,  -250,  -180,  -150,  -220],
    "foreign_ownership_pct": "75.1%",  # 外資持股比例（估，賣超後微降）
    "foreign_ownership_shares": "約 195.0 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       38.5,   "rsi_signal": "偏空（跌破 40，接近超賣區 30）",
    "macd":         -5.8,   "macd_signal": "MACD 死叉加深，下行動能擴大",   "macd_color": "#F44336",
    "kdj_k":        30.1,   "kdj_d": 38.5, "kdj_j": 13.2, "kdj_signal": "超賣區，KDJ J 值低於 20 警示",
    "ma5":       "1,832",
    "ma20":      "1,895",
    "ma60":      "1,840",
    "ma120":     "1,740",
    ...
    "bb_lower":  "1,785",   # 布林下軌
    "vol_ma5":  "20,000",   # 5日均量（張）
    "trend":    "台股 2330 於 3/25 收 NT$1,810（-1.63%），量能縮小至 17,000 張。跌破 MA60（1,840）支撐，RSI 38.5 接近超賣區，MACD 死叉加深。下方布林下軌 1,785 為關鍵支撐，若失守則下看 1,750-1,760。NYSE TSM（3/24）收 $343.25（+1.42%）有助隔日氣氛，但台股技術面偏弱。",
    "support":  "1,785–1,800",
    "resist":   "1,830–1,840",
...
DERIVATIVES = {
    "date":          "2026-03-25",
    "futures_price": "1,815",      # 台積電期貨近月（估，現貨弱勢下小幅正價差）
    "futures_oi":    "73,500",     # 未平倉量（口）（估，空單繼續增加）
    "futures_basis": "+5",         # 小幅正價差
    "call_oi":       "33,800",
    "put_oi":        "51,200",
    "pcr":           "1.51",       # Put/Call 比率繼續上升（偏空情緒加重）
    "pcr_signal":    "偏空",
    "max_pain":      "1,860",      # 最大痛苦價格（繼續下移）
    "key_strikes": [
        ("1,750", "Call 1,800 / Put 8,800", "強支撐"),
        ("1,785", "Call 3,200 / Put 9,200", "布林下軌支撐"),
        ("1,810", "Call 5,500 / Put 9,000", "現貨所在"),
        ("1,840", "Call 7,800 / Put 6,800", "前支撐轉壓力"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,191",   "+1.7%",  True),   # 3/24 反彈
    ("美元指數 DXY",    "103.8",   "-0.4%",  True),
    ("VIX 恐慌指數",    "24.8",    "-1.7",   True),   # 市場情緒略緩解
    ("WTI 原油",        "$71.5",   "-1.9%",  False),
    ("10Y 美債殖利率",  "4.25%",   "-0.03pp",True),
    ("台幣/美元",       "31.58",   "-0.07",  True),
    ("NVIDIA NVDA",     "$855",    "+1.7%",  True),   # A16 採用確認利多
    ("ASML",            "$681",    "+1.3%",  True),   # 設備股跟隨反彈
]
...
RELATIVE = {
    "date":           "2026-03-25",
    "tsmc_tw_1d":     -1.63,   # 2330 3/25 收盤
    "taiex_1d":       -0.85,   # 加權指數（3/25 估算）
    "sox_1d":         +1.70,   # SOX（3/24，反彈）
    "tsmc_nyse_1d":   +1.42,   # TSM ADR 3/24 收盤（US$343.25）
    "tsmc_tw_5d":     -5.4,
    "taiex_5d":       -2.1,
    "tsmc_tw_1m":     -9.5,
    "taiex_1m":       -4.8,
    ...
    # ADR 折溢價
    "adr_price":     343.25,   # TSM NYSE 收盤（2026-03-24 EST）
    "adr_parity":    286.86,   # 理論值 = 5 × NT$1,810 ÷ 31.58（台股 3/25 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.58,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+19.6",# (343.25 - 286.86) / 286.86 × 100
...
NEWS = [
    ...
    {
        "tag": "財務展望", "tag_color": "#4CAF50",
        "title": "TSMC 2026 年資本支出上看 $560 億美元，年增逾 40%",
        "body": "台積電 2026 年資本支出指引上限達 $560 億美元，較 2025 年大幅增加 40% 以上；亞利桑那三座廠房總投資額已攀升至約 $1,650 億美元，Plant 2 目標 2028 年量產 2nm。前 1-2 月累計營收年增 30%（NT$7,189 億），略低於分析師預估 33%，但 AI HPC 需求仍是主要成長動能，Q1 財報（4/16）倒計時 22 天。",
        "source": "Bloomberg / TechI Analysis 2026-03-10", "impact": "正面",
    },
    {

...
STOCK = {
    # 台股 2330
    "tw_price":       "1,790",        # 收盤價（數字加逗號，不含 NT$）（2026-03-31 收盤）
    ...
    "tw_high":        "1,815",
    "tw_low":         "1,785",
    "market_cap":     "NT$46.4 兆",
    "pe_ratio":       "~27.0x",       # 台股 P/E（TTM，2026-03-31）
    "pb_ratio":       "~9.9x",        # 台股 P/B（股淨比）
    "div_yield":      "~1.15%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "333.82",       # 盤中價（2026-03-31，官方收盤後更新）
    "nyse_change":    "-13.93",       # 較 3/30 收盤 $347.75 下跌
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-31 收盤（NT$1,790，-1.09%）  |  NYSE 數據：2026-03-31 盤中 $333.82（-4.01%）  |  Q1 末季末震盪，年初至今漲幅收窄  |  高盛目標價 $370–$375 USD（強烈買入）  |  4/16 Q1 財報倒計時 16 天",
}
...
SUMMARY = {
    ...
    "sentiment":  "看空",
    # 一句話標題
    "headline":   "Q1 季末 NYSE TSM 盤中 $333.82（-4.01%），台股 2330 收 NT$1,790（-1.09%）；季末換倉壓力，先進製程訂滿至 2028 支撐長線",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "NYSE TSM 3/31 盤中 $333.82（-$13.93，-4.01%），Q1 季末換倉壓力，半導體類股整體承壓",
        "台股 2330 收 NT$1,790（-20，-1.09%），成交量約 2.9 萬張，技術面持續偏弱，短線支撐 1,780–1,790",
        "TSMC 2nm/3nm 先進製程訂單已全數售罄至 2028 年，NVIDIA、Apple、AMD、Qualcomm 均已預訂產能",
        "2025 全年營收 $1,224 億美元（+36% YoY），AI/HPC 已佔季收入 55%；2026 年資本支出 $520–560 億",
        "三星瞄準 TSMC 推出矽光子「Dream Chip」計畫，預計 2028 年量產，競爭格局長期關注",
    ],
    # 一句結論
    "bottom_line": "台積電 Q1 季末（3/31）股價持續承壓：NYSE TSM 盤中 $333.82（-4.01%），台股 2330 收 NT$1,790（-1.09%），Q1 累計跌幅擴大。惟長線結構性利多未改：先進製程訂單售罄至 2028、NVIDIA 成第一大客戶（22%）、全球半導體市場 2026 年將破兆美元。AGM 提案截止日（3/31）今日到期；4/16 Q1 財報（EPS 共識 NT$20.38）為關鍵催化劑。分析師 31 人全數買入、目標 NT$2,290（+28%），短線低點視為長線佈局機會，支撐 1,780–1,790，壓力 1,810–1,830。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ NYSE TSM 盤中 -4.01%（3/31）；AGM 提案截止日今日（3/31）到期；Q1 財報 4/16 倒計時 16 天；Q1 季末換倉賣壓；注意全球科技股及地緣政治風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-31",
    "foreign":  -2500,    # 外資（正=買超）— 3/31 季末換倉壓力，外資賣超（估）
    "trust":    +250,     # 投信（持續逢低承接）
    "dealer":   -180,     # 自營商（季末減碼）
    # 近5日（由舊到新）
    "foreign_5d": [-1200, -3500, -2000, -1800, -2500],  # 持續賣超
    "trust_5d":   [+250,  +280,  +260,  +270,  +250],
    "dealer_5d":  [-100,  -200,  -150,  -160,  -180],
    "foreign_ownership_pct": "74.5%",  # 外資持股比例（估，季末持續降）
    "foreign_ownership_shares": "約 193.5 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       35.8,   "rsi_signal": "偏弱（RSI 持續下滑至 35，超賣區間，季末賣壓延續）",
    "macd":         -9.8,   "macd_signal": "MACD 死叉持續擴大，動能偏空，季末換倉壓力",   "macd_color": "#F44336",
    "kdj_k":        28.5,   "kdj_d": 33.2, "kdj_j": 19.1, "kdj_signal": "KDJ 深入超賣區間，等待量能回升確認底部",
    "ma5":       "1,808",
    "ma20":      "1,865",
    "ma60":      "1,832",
    "ma120":     "1,740",
    "bb_upper":  "1,985",   # 布林上軌
    "bb_mid":    "1,865",   # 布林中軌（20MA）
    "bb_lower":  "1,745",   # 布林下軌
    "vol_ma5":  "28,000",   # 5日均量（張）
    "trend":    "台股 2330（3/31 收 NT$1,790，-1.09%）跌至近期低位，技術面持續偏弱。NYSE TSM 盤中 $333.82（-4.01%），Q1 季末換倉壓力。RSI 降至 35.8（超賣區間），MACD 死叉加深，支撐區下移至 1,780–1,790。4/16 Q1 財報（倒計時 16 天）為關鍵催化劑。",
    "support":  "1,780–1,790",
    "resist":   "1,810–1,830",
...
DERIVATIVES = {
    "date":          "2026-03-31",
    "futures_price": "1,787",      # 台積電期貨近月（估，略低現貨 1,790，季末負價差）
    "futures_oi":    "78,500",     # 未平倉量（口）（估，季末換倉增加空單）
    "futures_basis": "-3",         # 負價差（季末空頭主導）
    "call_oi":       "28,500",
    "put_oi":        "59,200",
    "pcr":           "2.08",       # Put/Call 比率持續攀升（空頭情緒濃厚）
    "pcr_signal":    "偏空",
    "max_pain":      "1,820",      # 最大痛苦價格（持續下移）
    "key_strikes": [
        ("1,740", "Call 1,800 / Put 12,500", "布林下軌支撐"),
        ("1,780", "Call 3,500 / Put 11,000", "短線重要支撐"),
        ("1,790", "Call 4,800 / Put 9,500",  "現貨所在"),
        ("1,820", "Call 8,200 / Put 5,800",  "最大痛苦點/壓力"),
        ("1,850", "Call 11,500 / Put 3,200", "強壓力區"),
    ],
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "3,920",   "-3.2%",  False),  # 3/31 盤中，Q1 季末半導體承壓
    ("美元指數 DXY",    "104.2",   "+0.5%",  False),  # 美元偏強，台幣持續承壓
    ("VIX 恐慌指數",    "29.8",    "+1.8",   False),  # 季末恐慌指數偏高
    ("WTI 原油",        "$71.2",   "+0.8%",  False),
    ("10Y 美債殖利率",  "4.28%",   "-0.04pp",False),  # 避險需求，殖利率小降
    ("台幣/美元",       "31.80",   "-0.05",  False),  # 台幣小幅貶值
    ("NVIDIA NVDA",     "$855",    "-2.8%",  False),  # 半導體板塊同步下跌
    ("ASML",            "$681",    "-1.5%",  False),  # 設備股小跌
]
...
RELATIVE = {
    "date":           "2026-03-31",
    "tsmc_tw_1d":     -1.09,   # 2330 3/31 收盤 1,790（-1.09%）
    "taiex_1d":       -0.75,   # 加權指數（3/31 估算）
    "sox_1d":         -3.20,   # SOX（3/31 盤中，Q1 季末承壓）
    "tsmc_nyse_1d":   -4.01,   # TSM ADR 3/31 盤中 $333.82（-4.01%）
    "tsmc_tw_5d":     -3.5,    # 本週累計（估）
    "taiex_5d":       -1.8,
    "tsmc_tw_1m":     -10.8,
    "taiex_1m":       -5.2,
    "tsmc_tw_ytd":    -11.7,   # Q1 末累計
    "taiex_ytd":      -7.0,
    "sox_ytd":        -22.0,
    "beta_60d":        1.14,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價
    "adr_price":     333.82,   # TSM NYSE 盤中（2026-03-31）
    "adr_parity":    281.48,   # 理論值 = 5 × NT$1,790 ÷ 31.80（台股 3/31 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.80,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+18.6",# (333.82 - 281.48) / 281.48 × 100
...
NEWS = [
    {
        ...
        "title": "TSMC 2nm/3nm 先進製程訂單售罄至 2028，AI 晶片壟斷地位確立",
        "body": "TSMC 先進節點（2nm 及 3nm）訂單已全數預訂至 2028 年，NVIDIA、Apple、AMD 及 Qualcomm 均已鎖定產能。3nm 目前處於「超載」狀態，顯示供不應求局面短期難以緩解。此結構性供給緊張賦予台積電強大定價能力，預計毛利率維持 60% 以上，長線利多明確。",
        "source": "SamMobile 2026-03-31", "impact": "正面",
    },
    {
        "tag": "財務亮點", "tag_color": "#4CAF50",
        "title": "TSMC 2025 全年營收 $1,224 億（+36% YoY），AI/HPC 佔季收入 55%",
        "body": "台積電 2025 年全年營收達 $1,224 億 USD（年增 36%），Q4 單季 $331 億（+25.6% YoY）刷新紀錄。AI 及高效能運算（HPC）已佔季度收入 55%，超越手機成最大應用。2026 年資本支出計畫 $520–560 億 USD，美國亞利桑那 3nm 廠提前至 2027 年量產，顯示對長期需求信心十足。",
        "source": "Motley Fool 2026-03-26", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA 超越 Apple 成 TSMC 最大客戶，2026 年貢獻達 $330 億美元",
        "body": "NVIDIA CEO Jensen Huang 確認，NVIDIA 已超越 Apple 成為台積電最大單一客戶，預估 2026 年貢獻約 $330 億 USD（佔台積電總收入約 22%），Apple 以 $270 億（18%）居次。NVIDIA 同時確認首採 TSMC A16（1.6nm）製程（BPR 背面供電，效能+10%/功耗-20%），2027 年量產。",
        "source": "CNBC / TweakTown 2026-03-31", "impact": "正面",
    },
    {
        "tag": "競爭對手", "tag_color": "#607D8B",
        "title": "三星推矽光子「Dream Chip」計畫，目標 2028 年挑戰 TSMC",
        "body": "三星晶圓代工（Samsung Foundry）宣布將矽光子技術推向量產，預計 2028 年完成，明確將此定位為挑戰台積電先進製程領先地位的策略性舉措。不過，TSMC 在 3nm/2nm 技術與良率上仍領先三星逾 2 年，分析師認為 2028 年前難以實質替代台積電，短期競爭威脅有限。",
        "source": "Sammy Fans 2026-03-30", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "2026 全球半導體市場將突破 $1 兆，AI 晶片佔逾 50% 收入",
        "body": "2026 年全球半導體市場收入預估年增 30.7%，總規模突破 $1 兆 USD，其中 AI 相關晶片預計貢獻超過 $5,000 億。記憶體晶片（NAND）Q1 2026 價格年增 38%；SK 海力士計畫採用 TSMC 3nm 製程生產下一代 HBM4E 記憶體。半導體超級周期持續，台積電作為最大受益者地位穩固。",
        "source": "Sourceability / TrendForce 2026-03-31", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "中國 AI 晶片落後 5–10 年，半導體設備需求持續受壓",
        "body": "SEMICON China 2026（3/25–27，上海）上，中國晶片業領袖坦承 AI 資料中心晶片落後先進水準 5–10 年，AI 需求正對設備、被動元件及人才供應帶來壓力。此格局強化 TSMC 在 AI 晶片製造的不可替代性；美中科技管制風險持續存在，需持續關注出口管制政策動向。",

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,410",        # 台股 2330 最新確認收盤：7/21 Tue 官方收 NT$2,410（+90、+3.88% vs 7/20 NT$2,320、開 2,350／高 2,410／低 2,345、收在當日最高、TWSE 確認）——漲價題材引爆反彈第二日大漲、站回 5MA/60MA；⭐ 大盤同日飆 +1,783.17 點（+4.20%）收 44,232.87 史上最大單日漲點
    "tw_change":      "+90",          # 7/21 Tue 收盤漲跌點數（+3.88%、TWSE 官方）
    "tw_change_pct":  "+3.88%",       # 7/21 Tue 收盤漲跌幅（日經漲價報導催化、外資終結連 8 賣轉買）
    "tw_volume":      "31,606",       # 7/21 成交量（張、TWSE 口徑；成交股數 31,605,663 股、成交額 NT$754.30 億、成交 88,066 筆、量縮至 7/20 的 57%——量縮價漲屬惜售式反彈、續攻仍需量能配合）
    "tw_open":        "2,350",        # 7/21 開盤（TWSE、跳空開高 +30）
    "tw_high":        "2,410",        # 7/21 盤中高（TWSE、收盤即當日最高）
    "tw_low":         "2,345",        # 7/21 盤中低（TWSE）
    "market_cap":     "NT$62.5 兆",   # 7/21 收盤市值（NT$2,410 基礎、約 259.3 億股）
    "pe_ratio":       "~27.6x",       # 台股 P/E（TTM 估，NT$2,410 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25）
    "pb_ratio":       "~18.1x",       # 台股 P/B（NT$2,410 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.00%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,410）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "424.61",       # 7/21 Tue 官方收盤 $424.61（+$22.31、+5.55% vs 7/20 $402.30、Yahoo Finance API 確認）——日經漲價報導激勵大漲；SOX 同日 +5.21% 收 12,356.16（自 6/22 史高收斂至 -15.6%、脫離熊市線 -20%）
    "nyse_change":    "+22.31",       # 7/21 漲跌（自 7/20 $402.30）
    "nyse_change_pct":"+5.55%",       # 7/21 漲價題材 + 半導體 ETF 資金回補（SOXX/SOXL 單日吸金逾 $21 億）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~31.1x",       # NYSE USD 基礎 P/E（TTM 估，$424.61 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.7x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-22 Wed（最新確認收盤：台股 2330 7/21 Tue 官方收 NT$2,410（+90、+3.88% vs 7/20 NT$2,320、開 2,350／高 2,410／低 2,345、收在當日最高、量 31,606 張 TWSE 口徑、成交額 NT$754.30 億、成交 88,066 筆、TWSE 確認）；NYSE TSM 7/21 Tue 收 $424.61（+$22.31、+5.55% vs 7/20 $402.30、Yahoo Finance API 確認））。🚀 今日最重要：漲價題材引爆反彈第二日全面放大——（1）日經亞洲 7/21 報導：TSMC 已通知客戶 2027 年初起先進與成熟製程全面漲價 5-10%、HPC 客戶超額追加訂單另加價 10-15%、議價 6 月啟動 7 月拍板（Apple/NVIDIA/AMD/Qualcomm 均為適用客戶）——AI 需求與定價權的最強證據；（2）⭐ 台股大盤 7/21 飆漲 +1,783.17 點（+4.20%）收 44,232.87 創史上最大單日漲點、收復 7/17 崩跌 -2,953.71 的六成——台積電領軍（+90 點、估貢獻大盤約 660 點）、聯發科 +9.88%；（3）📊 籌碼面最強訊號兌現：外資 7/21 對 2330 終結連 8 賣、正式轉買 +4,976 張（買 22,804／賣 17,829、T86 官方）——正是前日報告點名的「多方最強訊號」；投信 +974（連 4 買）、自營 +214（連 9 買）、三大法人 +6,163 張連 2 日買超且擴大；全市場三大法人買超 162.9 億（BFI82U 官方：投信 +171.1 億大買、外資仍小賣 -43.3 億惟賣壓大減、媒體統計外資全市場連 13 賣）；（4）✅ NYSE TSM 7/21 大漲 +5.55% 收 $424.61、SOX +5.21% 收 12,356.16 自 6/22 史高收斂至 -15.6% 脫離熊市線（-20%）——SOXX/SOXL 半導體 ETF 單日吸金逾 $21 億、資金回補確認；MU +12.17%、INTC +8.64%、AMD +8.11%、ARM +7.46%、設備股全面大漲（AMAT +7.39%、LRCX +4.97%、KLAC +4.80%）、韓股 SK 海力士 +4.08%／三星 +6.15%（7/22 早盤續強逾 +8%／+5%）。⚠️ 唯一保留：2330 +3.88% 大漲但量縮至 31,606 張（僅 7/20 的 57%）——惜售式反彈、站穩 20MA 與挑戰前高需量能放大確認。📊 台美 ADR 溢價擴大至 +13.9%（$424.61 vs 理論值 ~$372.7、匯率 ~32.33）——美股領漲、台股 7/22 具補漲題材。技術面（官方收盤計算）：7/21 收 2,410 一舉站回 5MA 2,386 與 60MA 2,334 之上（反彈第一道確認完成）、距 20MA 2,419 僅 9 點；RSI(14) 51.5 站回中軸、MACD 綠柱收斂至 -15.2（DIF 8.7 < DEA 23.9）、KDJ K40.1/D40.2 黏合金叉臨界（J 值 5.0→39.8 深超賣快速修復）；支撐 2,386（5MA）／2,345（7/21 低）／2,334（60MA）／2,320（7/20 收）、壓力 2,419-2,424（20MA/布林中軌）／2,470（7/16 收前高）／2,528-2,535（布林上軌/史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B（Arizona 追加 $100B）、股利 +33%、「AI 需求比預期更強」＋2027 全面漲價傳聞——定價權敘事強化。⚠️ 風險：量縮反彈續航待確認、外資全市場仍連賣（-43.3 億）、美 232 條款半導體關稅 25% + 輸中晶片轉運查驗、台幣 ~32.33。⭐ 今日（7/22）觀察：（1）站穩 20MA 2,419 → 挑戰 2,470；（2）外資對 2330 轉買延續性；（3）量能能否放大至 4 萬張以上；（4）ADR 溢價 +13.9% 下台股補漲力道。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "🚀 漲價題材引爆反彈第二日：台股 2330 7/21 大漲 +3.88% 收 NT$2,410（站回 5MA/60MA）、大盤飆 +1,783 點史上最大單日漲點；外資終結連 8 賣、轉買 +4,976 張；NYSE TSM +5.55% 收 $424.61、SOX +5.21% 脫離熊市線；日經：TSMC 2027 年全面漲價 5-10%——惟量縮反彈（僅前日 57%）、站穩 20MA 2,419 為續航第二道確認",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🚀 最重要：日經亞洲報導 TSMC 已通知客戶 2027 年初起先進+成熟製程全面漲價 5-10%、HPC 超額追加訂單另加價 10-15%（議價 6 月啟動 7 月拍板、Apple/NVIDIA/AMD/Qualcomm 均適用）——AI 需求與定價權的最強證據、直接催化本日行情",
        "⭐ 台股 2330 7/21 官方收 NT$2,410（+90、+3.88%、開 2,350／高 2,410／低 2,345、收在當日最高）站回 5MA/60MA；大盤同日飆 +1,783.17 點（+4.20%）收 44,232.87 史上最大單日漲點、收復 7/17 崩跌六成",
        "📊 籌碼最強訊號兌現：外資 7/21 終結連 8 賣、正式轉買 +4,976 張（T86 官方）；投信 +974（連 4 買）、自營 +214（連 9 買）、三大法人 +6,163 張連 2 日買超；全市場三大法人買超 162.9 億（BFI82U：投信 +171.1 億大買、外資 -43.3 億賣壓大減）",
        "✅ NYSE TSM 7/21 大漲 +5.55% 收 $424.61、SOX +5.21% 收 12,356.16 脫離熊市線（自 6/22 史高 -15.6%）；SOXX/SOXL 單日吸金逾 $21 億；MU +12.2%、INTC +8.6%、AMD +8.1%、設備股全面大漲——AI 情緒面自 7/17 崩跌後首度全面修復",
        "📊 供應鏈全面補漲：聯發科 +9.88% 收 3,670、日月光 +6.03%、華邦電 +2.98%；韓股 SK 海力士 +4.08%／三星 +6.15%、7/22 早盤續強逾 +8%／+5%",
        "⚠️ 唯一保留：量縮反彈——+3.88% 大漲但量僅 31,606 張（7/20 的 57%）、惜售式反彈；站穩 20MA 2,419 與挑戰 2,470 需量能放大確認",
        "💡 技術面：站回 5MA 2,386／60MA 2,334、距 20MA 2,419 僅 9 點；RSI 51.5 站回中軸、KDJ K40.1/D40.2 金叉臨界（J 值 5.0→39.8）、MACD 綠柱收斂至 -15.2——深超賣修復展開",
        "📊 ADR 溢價擴大至 +13.9%（$424.61 vs 理論值 ~$372.7）——美股領漲、台股 7/22 具補漲題材；估值：台股 P/E ~27.6x、TSM ~31.1x（TTM 估）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-22 Wed（最新確認收盤：台股 2330 7/21 Tue 官方收 NT$2,410（+90、+3.88% vs 7/20 NT$2,320、開 2,350／高 2,410／低 2,345、收在當日最高、量 31,606 張 TWSE 口徑、成交額 NT$754.30 億、成交 88,066 筆、TWSE 確認）；NYSE TSM 7/21 收 $424.61（+$22.31、+5.55%、Yahoo API 確認））。🚀 核心判讀：漲價題材引爆反彈第二日全面放大——（1）日經報導 TSMC 2027 年全面漲價 5-10%（HPC 追加訂單再 +10-15%）：AI 需求與定價權的最強證據、Q2 毛利率 67.7% 與 CoWoS 訂滿至 2027 為佐證；（2）大盤 +1,783.17 點（+4.20%）史上最大單日漲點、台積電領軍；（3）籌碼面最強訊號兌現：外資終結連 8 賣、轉買 +4,976 張，三大法人 +6,163 張連 2 日買超擴大、全市場買超 162.9 億（投信 +171.1 億）；（4）NYSE TSM +5.55%、SOX +5.21% 脫離熊市線（自史高 -15.6%）、半導體 ETF 單日吸金逾 $21 億——AI 情緒面首度全面修復。⚠️ 惟確認清單尚餘兩項：量能（31,606 張僅前日 57%、惜售式反彈需放量至 4 萬張以上）與 20MA 2,419（距僅 9 點、站穩後上看 2,470／2,528-2,535）。技術面 RSI 51.5 回中軸、KDJ 金叉臨界、MACD 綠柱收斂——深超賣修復展開、動能轉強初步確認。📊 ADR 溢價 +13.9% 擴大、台股 7/22 具補漲題材。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、股利 +33%、2nm 貢獻 3%、「AI 需求比預期更強」——7/17 崩跌為估值/情緒修正、漲價題材重新錨定中長線價值。⭐ 今日（7/22）觀察：（1）站穩 20MA 2,419 → 挑戰 2,470（7/16 收前高）；（2）外資轉買延續性（連 2 日買超為趨勢反轉確認）；（3）量能放大與否；（4）守住支撐 2,386（5MA）／2,345／2,334（60MA）。⚠️ 風險：AI 估值修正若再起、美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.33（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/22 Wed）關鍵觀察：（1）✅ 反彈第二道確認：7/21 已收復 5MA 2,386 與 60MA 2,334（第一道完成）——能否站穩 20MA/布林中軌 2,419-2,424 為第二道；站穩後上看 2,470（7/16 收前高）→ 2,528-2,535（布林上軌/史高）；失守 2,386（5MA）則回測 2,345／2,334（60MA）、跌破 2,320 反彈結構轉弱；（2）⚠️ 量能為最大保留：7/21 +3.88% 大漲但量僅 31,606 張（前日 57%）——惜售式反彈、續攻需放量至 4 萬張以上，量縮續攻則提防高檔假突破；（3）📊 籌碼：外資轉買 +4,976 張終結連 8 賣——連續 2 日以上買超方可視為趨勢反轉確認；投信連 4 買、自營連 9 買續扮穩定力量；（4）✅ 漲價題材後續：日經報導尚待公司說法與客戶端反應、若獲確認將直接上修 2027 毛利率預期；（5）💱 美 232 條款關稅 25% + 輸中晶片轉運查驗、台幣 ~32.33、外資全市場仍小賣（-43.3 億）——AI 情緒面修復需 SOX 連續收紅鞏固",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-21",       # 最新資料日（7/21 Tue、TWSE T86 官方）
    "foreign":  4976,              # 外資 7/21 買超 4,976 張（⭐ 終結連 8 賣、正式轉買——前日報告點名的「多方最強訊號」兌現；買 22,804／賣 17,829）
    "trust":    974,               # 投信 7/21 買超 974 張（連 4 買、持續承接）
    "dealer":   214,               # 自營商 7/21 買超 214 張（連 9 買、買超收斂）；三大法人合計 +6,163 張（連 2 日買超、自 +4,002 擴大）
    # 近5日（由舊到新：7/15, 7/16, 7/17, 7/20, 7/21）；TWSE T86 口徑（張）
    "foreign_5d": [-3427, -4215, -44184, -1672, 4976],  # 外資（7/17 史詩級賣超 → 7/20 收斂 96% → 7/21 正式轉買）
    "trust_5d":   [-596, 528, 1489, 1049, 974],          # 投信（連 4 買、持續逆勢承接）
    "dealer_5d":  [456, 1061, 2759, 4626, 214],          # 自營（連 9 買）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       51.5,   "rsi_signal": "RSI(14) 51.5（官方收盤計算）——7/21 大漲後自 43.9 站回 50 中軸之上、動能轉強初步確認；續彈看 60 強勢區",
    "macd":         -15.2, "macd_signal": "MACD 綠柱收斂至 -15.2（DIF 8.7 < DEA 23.9、官方收盤計算）——自 -19.2 首度收斂、空方動能減弱；綠柱續收斂＋DIF 上穿 DEA 為翻多訊號",   "macd_color": "#F44336",
    "kdj_k":        40.1,   "kdj_d": 40.2, "kdj_j": 39.8, "kdj_signal": "KDJ K(40.1)/D(40.2)/J(39.8)（官方收盤計算）——J 值自 5.0 深超賣急拉至 39.8、K 與 D 黏合、金叉臨界；K 正式上穿 D 即確認低檔金叉",
    "ma5":       "2,386",   # 5MA（官方收盤計算；7/21 收 2,410 站回其上）
    "ma20":      "2,419",   # 20MA（官方收盤計算；7/21 收 2,410 距僅 9 點、次一關卡）
    "ma60":      "2,334",  # 60MA（官方收盤計算；7/21 站回其上、反彈第一道關卡收復）
    "ma120":     "2,105",  # 120MA（官方收盤計算）
    "bb_upper":  "2,528",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,419",   # 布林中軌（20MA）
    "bb_lower":  "2,309",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "49,793",   # 5日均量（張、TWSE 口徑、含 7/17 爆量 97,362）
    "trend":    "🚀 反彈第二日全面放大、第一道確認完成：台股 2330 7/21 官方收 NT$2,410（+90、+3.88%、開 2,350／高 2,410／低 2,345、收在當日最高）——一舉站回 5MA 2,386 與 60MA 2,334 之上、距 20MA 2,419 僅 9 點；⭐ 大盤同日飆 +1,783.17 點（+4.20%）收 44,232.87 史上最大單日漲點（收復 7/17 崩跌 -2,953.71 的六成）。催化：日經報導 TSMC 2027 年全面漲價 5-10%（HPC 超額追加訂單再加價 10-15%）。📊 技術指標（官方收盤計算）：RSI(14) 51.5 站回中軸、MACD 綠柱收斂至 -15.2（DIF 8.7 < DEA 23.9）、KDJ K(40.1)/D(40.2) 黏合金叉臨界、J 值自 5.0 急拉至 39.8——深超賣修復展開、動能轉強初步確認。📊 籌碼面最強訊號兌現：外資 7/21 終結連 8 賣、正式轉買 +4,976 張、三大法人 +6,163 張連 2 日買超擴大。⚠️ 唯一保留：+3.88% 大漲但量縮至 31,606 張（僅 7/20 的 57%）——惜售式反彈、站穩 20MA 與挑戰前高需量能放大。NYSE TSM 7/21 收 $424.61（+5.55%）、SOX +5.21% 收 12,356.16 脫離熊市線（自史高 -15.6%）。下一觀察：（1）站穩 20MA/布林中軌 2,419-2,424 → 挑戰 2,470（7/16 收前高）→ 2,528（布林上軌）／2,535 史高；（2）支撐 2,386（5MA）／2,345（7/21 低）／2,334（60MA）／2,320；（3）量能能否放大至 4 萬張以上；（4）外資轉買延續性。",
    "support":  "2,386 / 2,345 / 2,334 / 2,320 (5MA、7/21 低、60MA、7/20 收)",
    "resist":   "2,419-2,424 / 2,470 / 2,528-2,535 (20MA+布林中軌、7/16 收前高、布林上軌+史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-21 止），請以看盤軟體為準；7/21 官方收 NT$2,410（+3.88%）站回 5MA 2,386 / 60MA 2,334、距 20MA 2,419 僅 9 點；RSI(14) 51.5 站回中軸、MACD 綠柱收斂 -15.2（DIF 8.7 < DEA 23.9）、KDJ K40.1/D40.2 金叉臨界（J 5.0→39.8）；支撐 2,386 / 2,345 / 2,334 / 2,320、壓力 2,419-2,424 / 2,470 / 2,528-2,535；✅ 外資終結連 8 賣轉買 +4,976 張＋大盤史上最大漲點 +1,783 vs ⚠️ 量縮反彈（僅前日 57%）——反彈第一道確認（收復 60MA）完成、站穩 20MA 為第二道、量能放大為第三道",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-21",       # 最新資料日（7/21 Tue 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,408",            # 台積電期貨近月（估、現貨 7/21 大漲收 NT$2,410、貼水幾近收平）
    "futures_oi":    "182,000",          # 未平倉量（口）（估、大漲日空單回補）
    "futures_basis": "-2",               # 估逆價差（大漲日貼水收平）
    "call_oi":       "112,000",
    "put_oi":        "95,000",
    "pcr":           "0.85",             # Put/Call 比率（估、避險需求隨反彈明顯回落）
    "pcr_signal":    "中性（Put/Call 估 0.85；7/21 現貨大漲 +3.88% 收 NT$2,410、外資終結連 8 賣轉買——避險 Put 部位回補、Call 追價回流、情緒自 7/17 崩跌高峰明顯修復；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,290", "Call 45,000 / Put 12,000", "7/17 低、雙底防線"),
        ("2,320", "Call 40,000 / Put 9,500",  "7/20 收、回檔支撐"),
        ("2,334", "Call 38,000 / Put 8,500",  "60MA、已收復"),
        ("2,386", "Call 34,000 / Put 7,000",  "5MA、短線支撐"),
        ("2,400", "Call 32,000 / Put 6,500",  "最大痛苦（估）、整數關"),
        ("2,410", "Call 30,000 / Put 6,000",  "7/21 官方收、多空分界"),
        ("2,419", "Call 28,000 / Put 5,000",  "20MA/布林中軌、次一關卡"),
        ("2,470", "Call 22,000 / Put 3,500",  "7/16 收前高、上方壓力"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/21 NT$2,410）；NT$2,400 為估最大痛苦、下方支撐 2,386（5MA）/ 2,345 / 2,334（60MA）/ 2,320、上方壓力 2,419（20MA）/ 2,470（7/16 收）/ 2,528-2,535（布林上軌/史高）；🚀 反彈第二日：漲價題材（日經：2027 全面漲價 5-10%）＋外資轉買 +4,976 張＋大盤史上最大漲點 +1,783——貼水收平、避險需求明顯回落；⚠️ 量縮反彈（31,606 張、僅前日 57%）續攻需量能；✅ 基本面錨未變（全年上修 >40%、CapEx $60-64B、股利 +33%）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-21 Tue TWSE 官方收盤；美股為 7/21 Tue 收盤、Yahoo Finance API 確認；韓股為 7/21 收盤、7/22 早盤續強）
    ("設備", "ASML",    "艾司摩爾",    "1,801.51", "+3.59%", "⭐ EUV 設備龍頭；7/21 隨板塊強彈；TSMC CapEx 上調至 $60-64B 為設備鏈利多"),
    ("設備", "AMAT",    "應用材料",    "564.55", "+7.39%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作；7/21 大漲、設備股自最弱環節轉為反彈先鋒"),
    ("設備", "LRCX",    "科林研發",    "322.00", "+4.97%", "蝕刻設備（7/21 強彈、擺脫連日弱勢）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "217.56", "+4.80%", "量測設備（7/21 強彈）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","156.00","+2.98%", "與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/21 反彈、TWSE 官方）"),
    ("記憶", "000660.KS","SK 海力士",  "1,836,000 KRW", "+4.08%", "HBM4、與 NVIDIA 多年期合作；7/21 反彈 +4.08%（Yahoo API）、7/22 早盤續強逾 +8%"),
    ("記憶", "MU",      "美光",        "970.82", "+12.17%", "HBM / DDR5（7/21 飆漲 +12.17% 領漲全板塊）；HBM4 供需緊俏、AI 記憶體修復最猛"),
    ("封裝", "3711.TW", "日月光",      "633.00", "+6.03%", "CoWoS / 先進封裝（7/21 大漲收 633、TWSE 官方；補跌後強力回升）"),
    ("封裝", "AMKR",    "Amkor",       "66.73",  "+6.67%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/21 大漲）"),
    ("客戶", "NVDA",    "輝達",        "207.29", "+1.97%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/21 續漲惟落後板塊；Q1 FY2027 營收 $81.61B（+85% YoY）"),
    ("客戶", "AAPL",    "蘋果",        "327.74", "+0.35%", "⭐ #2 客戶 18%；7/21 微漲、連日弱勢止穩；跳過 A16 直上 A14；傳為 2027 漲價適用客戶之一"),
    ("客戶", "AVGO",    "博通",        "386.50", "+2.21%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/21 續彈）"),
    ("客戶", "AMD",     "超微",        "544.43", "+8.11%", "#4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/21 飆漲重回 $540）"),
    ("客戶", "2454.TW", "聯發科",      "3,670",  "+9.88%", "手機 SoC 客戶 9%（7/21 大漲近一成收 3,670、TWSE 官方、台股 AI 權值全面補漲）"),
    ("競爭", "INTC",    "英特爾",      "105.45", "+8.64%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/21 飆漲、自高點回檔後強力反彈）"),
    ("競爭", "005930.KS","三星電子",   "259,000 KRW", "+6.15%", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/21 大漲 +6.15%（Yahoo API）、7/22 早盤續強逾 +5%"),
    ("產業", "SOX",     "費城半導體指數","12,356.16", "+5.21%", "7/21 大漲 +5.21% 收 12,356.16——自 6/22 史高 14,634.66 收斂至 -15.6%、脫離熊市線（-20%）；SOXX/SOXL 單日吸金逾 $21 億"),
    ("處分後", "ARM",   "Arm Holdings","289.73", "+7.46%", "TSMC 4/29 全數出脫後（7/21 大漲 +7.46%、自高點回檔後反彈）"),
]
...
NEWS = [
    {
        ...
        "title": "🚀 日經：台積電傳 2027 年初全面漲價 5-10%、HPC 追加訂單再加價 10-15%",
        "body": "定價權的最強證據、直接引爆本日行情：（1）💰 日經亞洲 7/21 報導：台積電已通知客戶 2027 年初起先進與成熟製程全面漲價 5-10%、HPC 客戶超額追加訂單另加價 10-15%——議價 6 月啟動、7 月拍板；（2）📊 適用客戶含 Apple、NVIDIA、AMD、Qualcomm 等主要客戶——AI 需求強勁下客戶接受度高；（3）⭐ 佐證：Q2 毛利率 67.7% 超指引上緣、CoWoS 訂滿至 2027、「AI 需求比預期更強」；（4）🎯 若獲公司端確認，將直接上修 2027 年毛利率與 EPS 預期。",
        "source": "日經亞洲 / 中央社 / 自由財經 2026-07-21", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 台股飆 +1,783 點史上最大單日漲點：2330 大漲 +3.88%、外資終結連 8 賣",
        "body": "崩跌後 V 型修復第二日全面放大：（1）⭐ 台股 2330 7/21 官方收 NT$2,410（+90、+3.88%、開 2,350／高 2,410、收在當日最高、量 31,606 張）站回 5MA/60MA；（2）🚀 加權指數 +1,783.17 點（+4.20%）收 44,232.87 創史上最大單日漲點、收復 7/17 崩跌六成——台積電領軍、聯發科 +9.88%、日月光 +6.03%；（3）📊 籌碼最強訊號兌現：外資對 2330 終結連 8 賣、轉買 +4,976 張（T86 官方）、三大法人 +6,163 張；全市場三大法人買超 162.9 億（投信 +171.1 億大買）；（4）⚠️ 惟量縮至前日 57%、續攻需量能。",
        "source": "TWSE / 聯合新聞網 / Yahoo 股市 2026-07-21", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "✅ NYSE TSM 大漲 +5.55% 收 $424.61、SOX +5.21% 脫離熊市線",
        "body": "美股半導體全面回神：（1）✅ NYSE TSM 7/21 收 $424.61（+$22.31、+5.55%、Yahoo API 確認）——漲價報導激勵、盤中一度漲逾 4% 後續走高；（2）📊 SOX 收 12,356.16（+5.21%）自 6/22 史高收斂至 -15.6%、脫離熊市線（-20%）；（3）🚀 AI 晶片全面大漲：MU +12.17%、INTC +8.64%、AMD +8.11%、ARM +7.46%、AMAT +7.39%——設備股自最弱環節轉反彈先鋒；（4）📊 台美 ADR 溢價擴大至 +13.9%（vs 理論值 ~$372.7、匯率 32.33）——台股 7/22 具補漲題材。",
        "source": "Yahoo Finance API / Benzinga / TradingKey 2026-07-21", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 半導體 ETF 單日吸金逾 21 億美元：熊市恐慌後資金強力回補",
        "body": "資金面確認情緒反轉：（1）💰 iShares 半導體 ETF（SOXX）與 3 倍槓桿 ETF（SOXL）7/21 合計單日吸金逾 $21 億——為 7/17「2 兆美元 AI 晶片拋售潮」後首見大規模回補；（2）📊 對照 7/20 Yardeni 仍警告 SOXX 恐再下探 12%——一日之間資金用腳投票反向；（3）✅ VIX 同日 -8.6% 收 17.05、10Y 美債殖利率 +3bp 至 4.63%——典型 risk-on 組合；（4）🎯 觀察 SOX 能否連續收紅鞏固修復、避免二次探底。",
        "source": "Benzinga 2026-07-21", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 供應鏈全面補漲：聯發科 +9.88%、韓廠大漲且 7/22 早盤續強",
        "body": "AI 供應鏈自 7/17 崩跌後首度全面修復：（1）📈 台股：聯發科 7/21 大漲 +9.88% 收 3,670（近一成）、日月光 +6.03% 收 633、華邦電 +2.98%——中小型半導體賣壓（聯電曾連 2 跌停）解除；（2）📈 韓股：SK 海力士 +4.08% 收 1,836,000 韓元、三星 +6.15% 收 259,000 韓元，7/22 早盤續強逾 +8%／+5%（盤中）；（3）📈 記憶體鏈最猛：MU +12.17% 領漲——HBM4 供需緊俏邏輯強化；（4）⭐ 與 TSMC CoWoS 先進封裝、2nm 客戶需求互為表裡。",
        "source": "TWSE / Yahoo Finance API 2026-07-21~22", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 確認清單尚餘兩項：量縮反彈（僅前日 57%）＋外資全市場仍連賣",
        "body": "大漲之下的保留事項：（1）⚠️ 2330 +3.88% 大漲但量僅 31,606 張（7/20 的 57%）——惜售式反彈、非追價買盤主導；站穩 20MA 2,419 與挑戰 2,470 需量能放大至 4 萬張以上，量縮續攻提防假突破；（2）📊 外資對 2330 轉買 +4,976 張、惟全市場仍賣超 43.3 億（媒體統計連 13 賣）——個股回補先行、全面回流未確認；（3）⚠️ 日經漲價報導尚待公司說法；（4）💱 美 232 條款半導體關稅 25% + 輸中晶片轉運查驗、台幣 32.33 續為背景風險。",
        "source": "TWSE / BFI82U / 聯合新聞網 2026-07-21", "impact": "中性",
    },
]

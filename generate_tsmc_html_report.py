...
STOCK = {
    # 台股 2330
    "tw_price":       "2,030",        # 收盤價（數字加逗號，不含 NT$）（2026-04-17 實際收盤，財報後深度獲利了結）
    "tw_change":      "-55",          # 漲跌點數（含+/-號，vs 4/16 收盤 NT$2,085）
    "tw_change_pct":  "-2.64%",       # 漲跌幅（含+/-號）
    "tw_volume":      "32,500",       # 成交量（張，財報後縮量整理，估算）
    "tw_open":        "2,075",
    "tw_high":        "2,080",
    "tw_low":         "2,015",
    "market_cap":     "NT$52.7 兆",
    "pe_ratio":       "~29.9x",       # 台股 P/E（TTM，NT$2,030 基礎，EPS FY25 實績 NT$67.8）
    "pb_ratio":       "~10.8x",       # 台股 P/B（股淨比）
    "div_yield":      "~0.74%",       # 殖利率（已除息後調整，USD $2.65/ADR 年息）
    "52w_high_tw":    "2,110",        # 52 週高（2026-04-16 日中高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "370.50",       # 實際收盤（2026-04-17，+1.97%，財報後美股反彈）
    "nyse_change":    "+7.15",        # 實際
    "nyse_change_pct":"+1.97%",       # 實際
    ...
    # 資料日期備註
    "data_note":      "4/20 週末休市，資料為 4/17（上週五）實際收盤  |  台股 2330 收 NT$2,030（-2.64%，財報後深度獲利了結）  |  NYSE TSM 實際 $370.50（+1.97%，美股財報後反彈）  |  ⭐ Q1 2026 淨利 NT$572.5B（$18.1B，+58% YoY）、毛利率 66.2%（+740bps）雙創歷史新高  |  Q2 指引 $39.0-40.2B 遠超共識  |  全年 2026 收入 >30% 確認  |  Amazon/Meta/Anthropic 自研 AI 芯片均依賴 TSMC 製造，長期利好  |  下一交易日：4/21（週一）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "4/20 週末休市 | 4/17 台股 NT$2,030（-2.64%，深度獲利了結）、NYSE TSM $370.50（+1.97%）；週末利多：Amazon/Meta/Anthropic AI 芯片需求持續擴大",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ Q1 2026 淨利 $18.1B（+58% YoY）、毛利率 66.2%（+740bps YoY）雙創歷史新高，連續 10 季超預期",
        "📉 台股 NT$2,030（-2.64%），NYSE TSM $370.50（+1.97%）——台美反應分歧，已充分消化財報利多",
        "🏗️ 週末消息：Amazon Trainium 自研芯片省數百億美元資本支出；Meta/Anthropic 也擴大自研，均依賴 TSMC 製造",
        "🚀 Q2 指引 $39.0-40.2B 遠超共識，全年 >30% 成長確認；4/21 台股開盤走向為近期觀察重點",
        "⚠️ 美國 25% 半導體 301 關稅調查進行中；台幣升值為毛利率主要下行風險",
    ],
    # 一句結論
    "bottom_line": "TSMC Q1 2026 財報創歷史新高：淨利 $18.1B（+58% YoY）、毛利率 66.2%（+740bps）。Q2 指引 $39.0-40.2B 大幅超越市場共識，全年 >30% 成長確認。財報後 4/17 台股 2330 收 NT$2,030（-2.64%）為深度獲利了結，NYSE TSM $370.50（+1.97%）顯示美股投資人肯定財報結果——兩市分歧已充分消化，整體格局向多。週末利好消息：Amazon Trainium 自研芯片規模化顯著降本、Meta MTIA 持續擴大、Anthropic 評估自研芯片，三者均依賴 TSMC 先進製程製造，反映 hyperscaler 自研 AI 芯片趨勢長期利好 TSMC。4/20 週末休市，下一交易日 4/21（週一）開盤為近期觀察重點。31 位分析師全數看多，共識目標 NT$2,290（自 NT$2,030 有 +12.8% 上行空間）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 美國 25% 半導體 301 關稅調查持續（後續廣泛關稅動向需追蹤）；台幣升值壓力（每升值 1%，毛利率 -40bps）；伊朗戰爭推高供應鏈成本；4/21 台股開盤為近期觀察重點；ABF 基板供應持續受限",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-17",
    "foreign":  -3200,    # 外資（正=買超）— 4/17 深度獲利了結，賣超擴大（估）
    "trust":    +250,     # 投信（持續小幅加碼，估）
    "dealer":   -180,     # 自營商（估，財報後減碼）
    # 近5日（由舊到新，最後一筆為 4/17）
    "foreign_5d": [+1800, +3200, +4800, +6200, -3200],  # 4/17 財報後外資深度獲利了結（估）
    "trust_5d":   [+380,  +480,  +720,  +850,  +250],
    "dealer_5d":  [+200,  +320,  +450,  +380,  -180],
    "foreign_ownership_pct": "74.8%",  # 外資持股比例（估，4/17，獲利了結後略降）
    "foreign_ownership_shares": "約 194.1 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       52.3,   "rsi_signal": "RSI 從 66.2 快速回落至 52.3，財報後深度獲利了結，仍在多頭分界（>50），技術上需守穩 50 關口",
    "macd":         +0.9,   "macd_signal": "MACD 正值縮窄（+1.8→+0.9），多頭趨勢弱化中，財報後獲利了結壓力明顯，若跌破 0 轉空需警惕",   "macd_color": "#FF9800",
    "kdj_k":        55.2,   "kdj_d": 65.8, "kdj_j": 34.0, "kdj_signal": "KDJ J 值急降至 34.0（接近超賣區），財報後深度回調，短線出現超賣反彈機會",
    "ma5":       "2,046",
    "ma20":      "1,968",
    "ma60":      "1,898",
    "ma120":     "1,802",
    "bb_upper":  "2,115",   # 布林上軌（收盤跌破並回落）
    "bb_mid":    "1,968",   # 布林中軌（20MA）
    "bb_lower":  "1,821",   # 布林下軌
    "vol_ma5":  "33,000",   # 5日均量（張，財報後縮量整理）
    "trend":    "台股 2330（4/17 實際收 NT$2,030，-2.64%，深度獲利了結）。NYSE TSM 實際 $370.50（+1.97%，美股財報後反彈）。台美分歧已充分消化。技術面：RSI 52.3 接近多空分界、MACD 正值縮窄、KDJ J 值 34 接近超賣——短線有反彈機會，但需量能配合。4/21 開盤若能守穩 NT$2,000–2,030 支撐，中線目標 NT$2,200–2,290 維持。",
    "support":  "2,000–2,030",
    "resist":   "2,080–2,110",
...
DERIVATIVES = {
    "date":          "2026-04-17",
    "futures_price": "2,025",      # 台積電期貨近月（估，4/17 深度獲利了結後）
    "futures_oi":    "85,000",     # 未平倉量（口）（估，財報後部位縮減）
    "futures_basis": "-5",         # 負價差（深度獲利了結後空方略增）
    "call_oi":       "44,000",
    "put_oi":        "43,500",
    "pcr":           "0.99",       # Put/Call 比率升至 0.99（財報後保護性部位大增，市場轉謹慎）
    "pcr_signal":    "中性偏空（Put/Call 從 0.85 升至 0.99，市場已充分保護，但也代表下行風險多已定價）",
    "max_pain":      "2,030",      # 最大痛苦價格（近期到期，估）
    "key_strikes": [
        ("2,000", "Call 24,000 / Put 8,500", "心理整數關口，強支撐區"),
        ("2,030", "Call 21,000 / Put 5,200", "4/17 實際收盤，現為支撐/壓力分界"),
        ("2,080", "Call 18,500 / Put 2,800", "短線壓力（4/16 收盤 NT$2,085 附近）"),
        ("2,150", "Call 12,000 / Put 1,500", "中線目標區"),
        ("2,200", "Call 7,500 / Put 900",    "中線樂觀目標（共識 NT$2,290 之前關）"),
    ],
...
# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-04-17 財報後首日）
    ("設備", "ASML",    "艾司摩爾",    "675.40", "-0.7%", "EUV 設備龍頭（Q1 超預期後小幅回落，全年指引 €36-40B 維持）"),
    ("設備", "AMAT",    "應用材料",    "170.80", "-0.9%", "CVD/PVD 設備（TSMC 2nm 擴產受惠，整理中）"),
    ("設備", "LRCX",    "科林研發",    "782.10", "-0.4%", "蝕刻設備（2nm 良率爬坡帶動訂單，整理）"),
    ("設備", "KLAC",    "科磊",        "575.20", "-0.6%", "量測設備（2nm/A16 驗證需求強勁）"),
    ("記憶", "000660.KS","SK 海力士",  "293,500 KRW", "-0.5%", "HBM4 供應（NVIDIA GB300 主力）"),
    ("記憶", "MU",      "美光",        "95.80",  "-0.7%", "HBM / DDR5（AI 資料中心需求持續）"),
    ("封裝", "3711.TW", "日月光",      "222",    "-1.3%", "CoWoS 封裝（財報後同步整理）"),
    ("客戶", "NVDA",    "輝達",        "835.60", "-0.3%", "#1 客戶 22%（正式超越 Apple，CoWoS 產能鎖定至 2028）"),
    ("客戶", "AAPL",    "蘋果",        "207.50", "-0.7%", "#2 客戶 17%（A20 採 2nm，iPhone 18，TSMC 價格上漲傳言）"),
    ("客戶", "AVGO",    "博通",        "194.20", "-0.8%", "#3 客戶 13%（Custom XPU，AI ASIC）"),
    ("客戶", "AMD",     "超微",        "160.50", "-0.6%", "#4 客戶 8%（MI400 H2/2026，2nm EPYC Venice）"),
    ("客戶", "2454.TW", "聯發科",      "1,330",  "-1.1%", "手機 SoC 客戶 9%，2nm 首批量產"),
    ("競爭", "INTC",    "英特爾",      "21.05",  "-0.7%", "Intel Foundry 18A 爬坡，仍落後 TSMC 2nm"),
    ("競爭", "005930.KS","三星電子",   "52,800 KRW", "-0.8%", "Samsung Foundry 良率改善，但差距仍大"),
]
...
IR_CALENDAR = [
    ...
    ("2026-04-20", "📅 週末休市","週末非交易日（Sunday）；週末消息：Amazon Trainium 省數百億美元資本支出、Meta/Anthropic 擴大自研 AI 芯片均依賴 TSMC；4/21（週一）開盤為近期觀察重點","#BBDEFB"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "歷史高位區","~+0.8%", False),  # 4/17 美股財報後反彈
    ("美元指數 DXY",    "99.2",    "-0.4%",  False),  # 美元繼續走弱，利好 TSMC ADR
    ("VIX 恐慌指數",    "24.5",    "-1.0",   False),  # 恐慌指數下降，市場情緒稍改善
    ("WTI 原油",        "$62.5",   "持穩",   False),  # 油價持穩偏弱（中東局勢持續）
    ("10Y 美債殖利率",  "4.05%",   "-0.03pp",False),  # 小降，風險情緒稍好
    ("台幣/美元",       "31.38",   "-0.02",  False),  # 台幣小幅走強（升值壓縮毛利率）
    ("NVIDIA NVDA",     "$843.00", "+0.9%",  False),  # #1 TSMC 客戶（22%），4/17 財報後反彈
    ("ASML",            "$672.30", "-0.5%",  False),  # 設備股，Q1 財報後整理，全年指引維持
]
...
VALUATION = {
    "pe_current":   31.0,   # 同 STOCK["pe_ratio"]（4/16 NT$2,100）
...
RELATIVE = {
    "date":           "2026-04-17",
    "tsmc_tw_1d":     -2.64,   # 2330 4/17 實際收 NT$2,030（-2.64%），財報後深度獲利了結
    "taiex_1d":       -1.20,   # 加權指數（4/17 估算，台股大盤同步回落）
    "sox_1d":         +0.80,   # SOX（4/17 美股，財報後反彈）
    "tsmc_nyse_1d":   +1.97,   # TSM ADR 4/17 實際 $370.50（+1.97%，美股財報後反彈）
    "tsmc_tw_5d":     +4.10,   # 本週（4/13~4/17，從 NT$1,950 到 NT$2,030）
    "taiex_5d":       +3.20,
    "tsmc_tw_1m":     +4.6,    # 月線（NT$2,030 vs ~NT$1,940 一個月前）
    "taiex_1m":       +2.5,
    "tsmc_tw_ytd":    -3.1,    # Q1+4/1–4/17 累計（NT$2,030 vs 年初 ~NT$2,095）
    "taiex_ytd":      -4.5,
    "sox_ytd":        -21.5,
    "beta_60d":        1.18,   # 60 日滾動 Beta vs TAIEX（財報後深度回落，Beta 略升）
    # ADR 折溢價
    "adr_price":     370.50,   # TSM NYSE 實際（2026-04-17）
    "adr_parity":    323.45,   # 理論值 = 5 × NT$2,030 ÷ 31.38（台股 4/17 實際）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.38,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+14.6",# (370.50 - 323.45) / 323.45 × 100
...
NEWS = [
    {
        ...
        "title": "TSMC Q1 毛利率 66.2%（+740bps）與淨利 $18.1B 雙創歷史高",
        "body": "TSMC Q1 2026 財報：淨利 NT$572.5B（$18.1B，+58% YoY）、毛利率 66.2%（vs 58.8% YoY，+740bps）雙創歷史新高，連續第 10 季超越市場預期。Q1 收入 $35.9B（+35.1% YoY，+8.4% QoQ）超越共識。Q2 指引收入 $39.0-40.2B（遠超共識 $37.5-39.0B），毛利率指引 65-67%。全年 2026 收入 >30% 成長確認，CapEx 向 $52-56B 高端靠齊。4/17 台股 NT$2,030（-2.64%）深度獲利了結，NYSE TSM $370.50（+1.97%）美股反彈——兩市反應分歧，基本面持續強化。",
        "source": "CNBC / Bloomberg / Motley Fool 2026-04-16/19", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "Amazon/Meta/Anthropic 擴大自研 AI 芯片，均依賴 TSMC 製造",
        "body": "週末重大利好：Amazon CEO Jassy 披露 Trainium 自研 AI 芯片每年為 AWS 節省「數百億美元資本支出」，Meta 持續擴大 MTIA 自研芯片佈局，Anthropic（Claude 製造商）也正評估自研芯片可行性。三者均使用 TSMC 先進製程製造。此趨勢意味 TSMC 客戶多元化、對單一客戶依賴降低，且自研 ASIC 通常採最先進節點（N3/N2），長期強化 TSMC 先進製程稼動率。（NVIDIA 仍為最大客戶 22%，Apple 17%）",
        "source": "Motley Fool / Yahoo Finance 2026-04-19", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "Motley Fool：財報後整理是 TSMC 更佳買點，上行空間 +12.8%",
        "body": "Motley Fool 4/19 分析：TSMC 財報後股價整理（NT$2,030 vs NT$2,085 前日），現已是更佳入場點。理由：(1) Q1 毛利率 66.2% 創新高，定價能力強；(2) Q2 指引遠超共識，上調動能持續；(3) AI 訓練+推論需求仍顯著超供；(4) 2nm 良率爬升，N3/N3P 滿載；(5) Arizona Fab 4 已承諾至 2027，產能稀缺溢價 25-30%。共識目標 NT$2,290（自 NT$2,030 上行空間 +12.8%）。風險：關稅調查、台幣升值、地緣政治。",
        "source": "Motley Fool 2026-04-19", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "美國 25% 半導體 301 關稅：Arizona 製造可豁免，反強化在地擴產",
        "body": "美國商務部/USTR 4/14 提交半導體 301 關稅進展報告。現行 25% 關稅針對「特定 AI 關鍵半導體」（2026-01-15 生效），已推高 AI 啟動成本，迫使中小型 AI 公司延遲建設計畫。關鍵例外：「支持美國供應鏈建設」的進口可享豁免——TSMC Arizona 製造的芯片即符合豁免條件，反而強化在地製造需求。分析師認為 TSMC $165B Arizona 六廠計劃因此受益於關稅政策，短期不確定性仍存，後續廣泛關稅政策動向持續追蹤。",
        "source": "Pillsbury Law / Washington Today 2026-04-14", "impact": "中性",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "中東供應鏈成本壓力持續，TSMC 安全庫存策略應對 Q2 毛利率壓力",
        "body": "伊朗戰爭持續推高氦氣、氫氣等半導體製造關鍵化學品成本。TSMC 法說會確認已建立多元供應商與安全庫存，短期對營運無顯著衝擊，但 Q2 製造成本受能源與化學品漲價影響。Q2 毛利率指引 65-67%（中間值 66%，較 Q1 的 66.2% 略低）反映此壓力。油價（WTI ~$62.5）目前相對平穩。台灣電費調漲 + 美國廠爬坡成本也是 Q2 毛利率下行變數，建議持續追蹤。",
        "source": "CNBC / Bloomberg 2026-04-16", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "AI 晶片市場 2026 年預計突破 $5,000 億，佔全球半導體銷售 50%",
        "body": "最新產業報告顯示，全球 AI 晶片（含 GPU、AI ASIC、HBM）市場 2026 年預計達 $5,000 億，佔全球半導體銷售 50%，較 2025 年大幅成長。ASML Q1 2026 財報超預期，上調 2026 全年指引至 €36-40B（原 €34-39B），EUV 訂單強勁確認 AI 半導體投資週期延續。全球半導體銷售 2026 年預計首度突破 $1 兆，TSMC 掌控 72% 最先進製程（3nm 以下），為 AI 資本週期最大受益者。雲端廠商 AI 基建 CapEx 2026 年成長 50%+。",
        "source": "247WallSt / Brussels Morning / Modern Diplomacy 2026-04-15", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "TSMC 2026 CapEx 向 $56B 高端靠齊，2nm 稼動率 90-95% 爬坡中",
        "body": "TSMC 在法說會確認 2026 年 CapEx 向 $52-56B 高端靠齊，重點投資 N2（2nm）、A16（背面電源傳輸）及 CoWoS 先進封裝。N2 已進入高量產（HVM）爬坡，稼動率 90-95%，Apple A20（iPhone 18）、AMD EPYC Venice、NVIDIA 下世代 GPU 均為首批客戶。A16 預計 2026 年底量產。CoWoS 產能 2026 年目標擴增至 35,000 wspm（+59% YoY），但 ABF 基板供應仍為整體生態系瓶頸。亞利桑那 N3 廠 Phase 1 全面量產中，Phase 2（N2）建設同步推進。",
        "source": "CNBC / Bloomberg / TSMC IR 2026-04-16", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "能源成本上升與美國廠爬坡費用為 Q2 毛利率主要下行風險",
        "body": "財報後多位分析師警告，台灣電費調漲及亞利桑那廠早期爬坡攤提成本將對 TSMC Q2 毛利率形成壓力。Q2 毛利率指引（65-67%）雖優於預期，但亞利桑那廠的高製造成本長期而言可能拖累整體獲利率。伊朗戰爭推高能源及化學品成本，台幣持續走強（每升值 1%，毛利率 -40bps）亦為逆風。美國對半導體進口 301 調查若升級為加稅，將衝擊客戶採購節奏。ABF 基板供應受限導致 CoWoS 爬坡較預期緩慢，需持續追蹤 Q2 實際出貨情況。",
        "source": "CNBC / Wall Street analysts / TradingKey 2026-04-16/17", "impact": "負面",
    },
]

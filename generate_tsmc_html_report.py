...
STOCK = {
    # 台股 2330
    "tw_price":       "2,465",        # 台股 2330 7/8 Wed 官方收盤 NT$2,465（+25、+1.02% vs 7/7 收 NT$2,440、TWSE 官方確認）——收在當日最高、止跌回升；【更正】TWSE 官方：7/7 Tue 實際收 NT$2,440（-20、-0.81%），前報誤載 NT$2,490（+1.22%）
    "tw_change":      "+25",          # 7/8 Wed 收盤漲跌點數（+1.02%、TWSE 官方、收最高、止跌回升）
    "tw_change_pct":  "+1.02%",       # 7/8 Wed 收盤漲跌幅（TWSE 官方；台美 7/8 同步反彈 +1.02%）
    "tw_volume":      "25,519",       # 7/8 Wed 官方成交量（張、25,519,599 股、成交額 NT$624.0 億、TWSE）
    "tw_open":        "2,445",        # 7/8 開盤（TWSE 官方）
    "tw_high":        "2,465",        # 7/8 盤中高＝收盤（收最高、TWSE 官方）
    "tw_low":         "2,420",        # 7/8 盤中低（TWSE 官方）
    "market_cap":     "NT$63.9 兆",   # 7/8 收盤市值（NT$2,465 基礎、約 259.3 億股）
    "pe_ratio":       "~32.7x",       # 台股 P/E（TTM，NT$2,465 ÷ 近四季 EPS ~75.5、statementdog）
    "pb_ratio":       "~18.5x",       # 台股 P/B（NT$2,465 ÷ 每股淨值 ~133.4、statementdog）
    "div_yield":      "~0.61%",       # 殖利率（NT$2,465 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "436.98",       # 7/8 Wed 官方收盤 $436.98（+$4.41、+1.02% vs 7/7 $432.57、Yahoo Finance 日線確認）——7/7 -4.25% 回檔後止跌反彈；SOX 7/8 +2.23% 收 12,574.97、NVDA +3.65%、AVGO +4.83% AI 晶片股回神
    "nyse_change":    "+4.41",        # 7/8 漲跌（自 7/7 $432.57 反彈、AI 晶片股同步回神）
    "nyse_change_pct":"+1.02%",       # 7/8 NYSE 反彈（台美 7/8 同步 +1.02%、巧合同幅）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~37.1x",       # NYSE USD 基礎 P/E（TTM 估，$436.98 基礎）
    "pb_nyse":        "~18.5x",
    "beta":           "1.18",
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "✅ 台美 7/8 同步止跌反彈：台股 2330 官方收 NT$2,465（+25、+1.02%、收在當日最高、TWSE 確認）、NYSE TSM 收 $436.98（+1.02%）、SOX +2.23% AI 晶片股回神；⚠️ 依 TWSE 官方數據更正 7/7 收盤為 NT$2,440（-0.81%，前報誤載 NT$2,490）；7/16 法說會倒數 7 天、6 月營收即將公布，分析師續挺（Citi NT$3,800／Barclays $625／Nomura NT$3,425）",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "✅ 台股 2330 7/8 Wed 官方收 NT$2,465（+25、+1.02%、開 2,445／高 2,465／低 2,420、量 25,519 張、TWSE 確認）——收在當日最高、止跌回升、站回 5MA NT$2,455；7/9 開盤 NT$2,450 小幅開低",
        "⚠️【更正】依 TWSE 官方日成交資訊：7/7 Tue 實際收 NT$2,440（-20、-0.81%、盤中高 NT$2,500），前兩日報告誤載為 NT$2,490（+1.22%）；7/7 外資實為小買 +83 張（前報估 +12,000 張）；6/30 官方收 NT$2,410。本報告全頁已按官方數據重算",
        "✅ NYSE TSM 7/8 Wed 收 $436.98（+$4.41、+1.02% vs 7/7 $432.57、Yahoo Finance 確認）——7/7 -4.25% 回檔後止跌反彈；SOX +2.23% 收 12,574.97、NVDA +3.65% 收 $204.12、AVGO +4.83%、AMAT +2.89%，AI 晶片股全面回神",
        "📊 三大法人 7/8 官方（TWSE T86）：外資 -4,155 張、投信 +730 張、自營 +468 張、合計 -2,957 張——外資 7/2-7/3 連兩日大賣逾 1.2 萬張後 7/6-7/7 收手、7/8 再賣，籌碼仍觀望；投信連 5 日買超",
        "📊 台美 ADR 溢價約 +13.7%（7/8 ADR $436.98 vs 理論值 $384.32＝5×NT$2,465÷匯率 32.07）——台股 7/9 有補漲空間但溢價偏高本身亦是風險",
        "🚀 分析師大幅續挺：Citi NT$3,800（自 NT$2,465 具 +54.2% 空間）、Barclays 維持 $625（Overweight、自 $436.98 具 +43.0%）、Nomura NT$3,425（+38.9%）；共識強力買入；⚠️ 惟 Goldman 7/1 移出 APAC Conviction List（戰術調整、非降評）",
        "🎯 7/16 Q2 法說會倒數 7 天（指引 $39.0-40.2B、毛利率 65-67%、Citi 預期上修全年展望）；6 月營收 7 月上旬隨時公布——4-5 月營收 +24% YoY 落後華爾街 ~35% 預期、6 月數字為 Q2 達標關鍵驗證",
        "🏆 中長線基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）+ TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + 全先進製程漲價 5-10%（毛利率 +2pp）",
        "⚠️ 風險續追蹤：外資 7 月累計仍大幅賣超（7/2-7/8 合計約 -2.9 萬張）；下游手機/PC 需求疲弱；Intel 18A 量產吸引微軟/高通；估值不低（台股 P/E ~32.7x、TSM ~37x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-09 Thu（最新確認收盤：台股 2330 7/8 Wed 官方收 NT$2,465（+25、+1.02% vs 7/7 NT$2,440、開 2,445／高 2,465／低 2,420、量 25,519 張、TWSE 確認、收在當日最高）；NYSE TSM 7/8 Wed 收 $436.98（+$4.41、+1.02%、Yahoo Finance 日線確認）；台股 7/9 開盤 NT$2,450、盤中待收）。⚠️【數據更正】依 TWSE 官方日成交資訊：7/7 Tue 實際收 NT$2,440（-20、-0.81%、盤中高 NT$2,500、量 31,401 張），前兩日報告誤載為 NT$2,490（+30、+1.22%）；6/30 官方收 NT$2,410（前報快照 NT$2,465 為盤中價）；7/7 三大法人官方為外資 +83 張／投信 +539 張／自營 -237 張（前報估外資 +12,000 張有誤）——本頁均線、ADR 溢價、法人序列均已按官方數據重算。⭐ 今日重點：（1）✅ 台美 7/8 同步止跌反彈——台股收最高 NT$2,465（+1.02%）站回 5MA NT$2,455、NYSE TSM $436.98（+1.02%）自 7/7 -4.25% 回檔止穩、SOX +2.23% 收 12,574.97、NVDA +3.65% 收 $204.12、AVGO +4.83%、AMAT +2.89% AI 晶片股全面回神；（2）📊 籌碼面（TWSE T86 官方）：7/8 外資 -4,155 張、投信 +730、自營 +468、合計 -2,957 張；7 月以來外資累計約 -2.9 萬張（7/2 -12,254、7/3 -12,537、7/6 -241、7/7 +83、7/8 -4,155）——賣壓集中 7/2-7/3、近三日明顯收斂、投信連 5 日買超為穩定買盤；（3）📊 台美 ADR 溢價 +13.7%（$436.98 vs 理論值 $384.32＝5×NT$2,465÷32.07）——台股 7/9 有補漲動能、惟高溢價亦隱含收斂風險；（4）🚀 分析師續挺：Citi NT$3,800（自 NT$2,465 具 +54.2% 空間）、Barclays $625（自 $436.98 具 +43.0%）、Nomura NT$3,425（+38.9%）、共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）；（5）🎯 催化倒數：6 月營收 7 月上旬隨時公布（4-5 月 +24% YoY 落後華爾街 ~35% 預期、6 月數字攸關 Q2 指引 $39.0-40.2B 達成）、7/16 Q2 法說會倒數 7 天（Citi 預期上修全年展望、上調 2027-28 資本支出至 $75-80B）。⭐ 中長線結構性利多基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）、2nm 量產良率 70-80%、TSMC-Amkor 美國封裝長約、全先進製程漲價 5-10%（估毛利率 +2pp）、4 大 CSP 2026 AI CapEx $725B（+77% YoY）。技術面（基於 TWSE 官方收盤重算）：5MA NT$2,455 / 10MA NT$2,429 / 20MA NT$2,403；7/8 收 NT$2,465 站回 5MA 之上；下方支撐 NT$2,440（7/7 收）/ NT$2,420（7/8 低）/ NT$2,403（20MA）；上方壓力 NT$2,500-2,505 / NT$2,535 史高。⚠️ 風險：外資 7 月累計賣超未轉向、下游手機/PC 需求疲弱、Intel 18A 量產競爭（微軟/高通進駐）、估值不低（台股 P/E ~32.7x TTM、TSM ~37x）、台幣匯率 32.07（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/9 Thu）關鍵觀察：（1）✅ 台股 2330 7/8 官方收 NT$2,465（+25、+1.02%、量 25,519 張、TWSE 確認、收在當日最高）止跌回升、站回 5MA NT$2,455；7/9 開盤 NT$2,450 小幅開低——觀察能否守穩 NT$2,440（7/7 收）/ NT$2,420（7/8 低）/ NT$2,403（20MA）支撐、續挑戰 NT$2,500 / NT$2,535 史高；（2）✅ NYSE TSM 7/8 收 $436.98（+1.02%）止穩反彈、SOX +2.23%、NVDA +3.65% 收 $204.12、AVGO +4.83%——隔夜動能對台股 7/9 偏正面；（3）📊 籌碼：外資 7/8 續賣 -4,155 張（7 月累計約 -2.9 萬張）、能否收斂翻買為關鍵；投信連 5 日買超（TWSE T86 官方）；（4）📊 ADR 溢價 +13.7% 偏高、留意收斂方式（台股補漲 vs ADR 回落）；（5）🎯 6 月營收 7 月上旬隨時公布——4-5 月 +24% YoY 落後 ~35% 預期、為 7/16 法說會（倒數 7 天）前最重要驗證點；（6）⚠️【數據更正】依 TWSE 官方：7/7 收 NT$2,440（-0.81%）非前報之 NT$2,490（+1.22%）、6/30 官方收 NT$2,410、7/7 外資實為 +83 張——閱讀近兩日歷史報告請以本日更正後數據為準；（7）⚠️ 估值警示：台股 P/E ~32.7x（TTM）、TSM ~37x；台幣 32.07、Intel 18A 競爭、下游手機/PC 需求疲弱續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-08",       # 最新資料日（TWSE T86 官方數據、非估算）
    "foreign":  -4155,             # 外資 7/8 官方賣超 4,155 張（-4,155,172 股、TWSE T86）
    "trust":    730,               # 投信 7/8 官方買超 730 張（+729,872 股、連 5 日買超）
    "dealer":   468,               # 自營商 7/8 官方買超 468 張（+468,443 股）；三大法人合計 -2,957 張
    # 近5日（由舊到新：7/2, 7/3, 7/6, 7/7, 7/8）；全部改用 TWSE T86 官方數據（張）
    "foreign_5d": [-12254, -12537, -241, 83, -4155],     # 外資：7/2-7/3 連兩日大賣逾 1.2 萬張後收斂、7/8 再賣（官方；前報 7/7 估 +12,000 有誤、實為 +83）
    "trust_5d":   [1026, 821, 380, 539, 730],            # 投信連 5 日買超（官方）
    "dealer_5d":  [217, 653, 119, -237, 468],            # 自營（官方）
    "foreign_ownership_pct": "~74.9%",  # 外資持股比例（估，官方以 TWSE 外資持股餘額為準）
    "foreign_ownership_shares": "約 194 億股（估）",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       55.0,   "rsi_signal": "RSI(14) 依官方收盤重算約 55（簡單法 ~57）——7/7 -0.81% 回檔後 7/8 +1.02% 回升，中性偏多、動能溫和、距超買尚有空間",
    "macd":         0.30,  "macd_signal": "MACD 估約 +0.30、紅柱收斂後暫止穩——7/1 高點 NT$2,505 後區間震盪（2,440-2,505），趨勢動能減弱、待 7/8 反彈延續確認",   "macd_color": "#4CAF50",
    "kdj_k":        52.0,   "kdj_d": 55.0, "kdj_j": 46.0, "kdj_signal": "KDJ K(52)/D(55)/J(46) 估——7/7 回落後 7/8 反彈、J 值自低檔回升、中性區整理待方向",
    "ma5":       "2,455",   # 5MA（7/2 2465, 7/3 2445, 7/6 2460, 7/7 2440, 7/8 2465）— 依 TWSE 官方收盤重算
    "ma20":      "2,403",   # 20MA（依 TWSE 官方收盤重算；前報 2,440 基於誤植數據）
    "ma60":      "2,295",  # 60MA（估算）
    "ma120":     "2,230",  # 120MA（估算）
    "bb_upper":  "2,530",  # 布林上軌（20MA+2σ 估）
    "bb_mid":    "2,403",   # 布林中軌（20MA、官方重算）
    "bb_lower":  "2,275",  # 布林下軌（估）
    "vol_ma5":  "29,400",   # 5日均量（張、7/2-7/8 官方量均：35,919/32,906/21,042/31,401/25,520）
    "trend":    "✅ 止跌回升、區間震盪整理：台股 2330 7/8 官方收 NT$2,465（+25、+1.02%、收在當日最高、量 25,519 張）自 7/7 官方收 NT$2,440（-0.81%）止跌回升、站回 5MA NT$2,455；⚠️【更正】依 TWSE 官方數據，7/7 實收 NT$2,440 非前報之 NT$2,490，本區技術指標已全面按官方收盤重算（7/1 2,505 → 7/2 2,465 → 7/3 2,445 → 7/6 2,460 → 7/7 2,440 → 7/8 2,465，7/1 高點後於 2,440-2,505 區間震盪）。NYSE TSM 7/8 收 $436.98（+1.02%）自 7/7 -4.25% 回檔止穩、SOX +2.23% 收 12,574.97、NVDA +3.65%、AVGO +4.83%，AI 半導體回神、隔夜動能偏正面。📊 技術面（官方收盤重算）：RSI(14) 約 55 中性偏多、MACD 約 +0.30 紅柱收斂後止穩、KDJ 中性區整理；價格站上 5MA（2,455）/ 10MA（2,429）/ 20MA（2,403），短中期均線多頭排列未破壞。下一觀察：（1）台股 7/9 能否站穩 NT$2,455（5MA）並突破 NT$2,500-2,505 壓力區、續挑戰 NT$2,535 史高；（2）外資賣超（7/8 -4,155 張、7 月累計約 -2.9 萬張）能否收斂翻買；（3）下方支撐 NT$2,440（7/7 收）/ NT$2,420（7/8 低）/ NT$2,403（20MA）；（4）🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 共識強力買入為中長線支撐、惟須留意 6 月營收（7 月上旬隨時公布）與 7/16 財報（倒數 7 天）；（5）⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 70-80% 良率。",
    "support":  "2,440 / 2,420 / 2,403 (7/7 收、7/8 低、20MA)",
    "resist":   "2,500-2,505 / 2,535 (7/6-7/7 盤中高、史高)",
    "note": "均線與 RSI 依 TWSE 官方收盤重算、MACD/KDJ/布林為估算，請以看盤軟體為準；⚠️【更正】7/7 官方收 NT$2,440（-0.81%、盤中高 2,500）非前報之 NT$2,490（+1.22%）、6/30 官方收 NT$2,410——前報 5MA 2,473/20MA 2,440 基於誤植數據、已修正為 5MA 2,455/20MA 2,403；台股 7/8 +1.02% 收 NT$2,465 收在當日最高、站回 5MA；上方壓力 NT$2,500-2,505 / NT$2,535 史高、下方支撐 NT$2,440 / NT$2,420 / NT$2,403；🚀 Citi/Barclays/Nomura 續挺 + 漲價定價力 vs ⚠️ 外資 7 月賣超 + 6 月營收與下游需求雜訊",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-08",       # 最新資料日（7/8 Wed 盤後估、官方以 TAIFEX 為準；本日未能取得官方數據）
    "futures_price": "2,470",            # 台積電期貨近月（估、現貨 7/8 收 NT$2,465 附近小幅正價差）
    "futures_oi":    "180,000",          # 未平倉量（口）（估）
    "futures_basis": "+5",               # 估小幅正價差（7/8 現貨止跌回升、隔夜 NYSE +1.02% 偏正面）
    "call_oi":       "120,000",
    "put_oi":        "75,000",
    "pcr":           "0.63",             # Put/Call 比率（估、區間震盪中性偏多）
    "pcr_signal":    "中性偏多（Put/Call 估 0.63；台股 7/8 官方收 NT$2,465（+1.02%）止跌回升、NYSE 7/8 +1.02% 止穩、SOX +2.23%；⭐ Citi/Barclays/Nomura + 共識偏多為支撐；⚠️ 外資 7/8 賣超 -4,155 張籌碼觀望；本日期權數據未能取得官方值、以估算標註）",
    "max_pain":      "2,465",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,403", "Call 52,000 / Put 12,000", "20MA（官方重算）、支撐"),
        ("2,420", "Call 50,000 / Put 9,000",  "7/8 盤中低、支撐"),
        ("2,440", "Call 48,000 / Put 8,000",  "7/7 官方收、支撐"),
        ("2,465", "Call 46,000 / Put 7,000",  "7/8 官方收、最大痛苦（估）"),
        ("2,500", "Call 42,000 / Put 5,500",  "7/6-7/7 盤中高、整數壓力"),
        ("2,535", "Call 32,000 / Put 3,500",  "盤中史高"),
        ("2,600", "Call 24,000 / Put 2,500",  "上檔壓力"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實（本日未能取得官方 7/8 數據）；價位錨點已按 TWSE 官方收盤更正（7/7 NT$2,440、7/8 NT$2,465）；NT$2,465 為估最大痛苦、下方支撐 NT$2,440 / NT$2,420 / NT$2,403、上方壓力 NT$2,500 / NT$2,535 史高；🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 漲價定價力偏多 vs ⚠️ 外資 7 月賣超、6 月營收 miss 風險；中長線基底未變（NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-07-08 Wed 收盤、台股為 7/8 收盤、韓股為 7/8 收盤；本日起價格改以 Yahoo Finance 日線實際數據為準——⚠️ 前報多檔價格為累積估算已嚴重漂移、本日一併校正）
    ("設備", "ASML",    "艾司摩爾",    "1,768.65", "+1.22%", "EUV 設備龍頭（7/8 隨半導體反彈；高 NA EUV 需求強；價格已校正、前報 864 有誤）"),
    ("設備", "AMAT",    "應用材料",    "570.50", "+2.89%", "🛠️ 設備股 7/8 反彈；AMAT-TSMC $5B EPIC Center 研發合作"),
    ("設備", "LRCX",    "科林研發",    "333.15", "+2.15%", "蝕刻設備（7/8 反彈；WFE 全年展望上修；價格已校正）"),
    ("設備", "KLAC",    "科磊",        "221.18", "+2.18%", "量測設備（7/8 反彈；2nm 量產驅動中長線；價格已校正）"),
    ("記憶", "2344.TW", "華邦電 Winbond","168.50","-3.16%", "⭐ 與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC、降低對韓系 HBM 依賴（7/8 拉回；價格已校正、前報 33.85 有誤）"),
    ("記憶", "000660.KS","SK 海力士",  "2,076,000 KRW", "-5.68%", "🤝 與 NVIDIA 多年期記憶體合作；7/8 韓股重挫後 7/9 盤中大幅反彈約 +6.4%；HBM4 需求強（價格已校正）"),
    ("記憶", "MU",      "美光",        "948.80", "+1.11%", "HBM / DDR5（7/8 反彈；AI 記憶體需求勁揚、HBM4 中長線強勁；價格已校正）"),
    ("封裝", "3711.TW", "日月光",      "625.00", "-3.99%", "CoWoS / 先進封裝（7/8 拉回整理；先進封裝瓶頸結構受惠；價格已校正）"),
    ("封裝", "AMKR",    "Amkor",       "66.91",  "+2.42%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產；7/8 隨半導體反彈"),
    ("客戶", "NVDA",    "輝達",        "204.12", "+3.65%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/8 大漲 +3.65% 領攻 AI 晶片股回神"),
    ("客戶", "AAPL",    "蘋果",        "313.39", "+0.88%", "#2 客戶 18%；面臨 TSMC 漲價成本；跳過 A16 直上 A14；7/8 小漲"),
    ("客戶", "AVGO",    "博通",        "388.69", "+4.83%", "#3 客戶；7/8 大漲 +4.83% 反彈最強；custom XPU 中長線強勁（價格已校正）"),
    ("客戶", "AMD",     "超微",        "517.41", "+0.25%", "#4 客戶；7/8 小漲整理；瞄準 $1,200 億資料中心市場、MI350/MI450、首發 2nm AI 晶片"),
    ("客戶", "2454.TW", "聯發科",      "3,995",  "-0.87%", "手機 SoC 客戶 9%；7/8 小跌整理；面臨漲價成本（價格已校正、前報 4,835 有誤）"),
    ("競爭", "INTC",    "英特爾",      "110.24", "-0.14%", "18A 節點量產、吸引微軟/高通等客戶進駐；代工轉型為 TSMC 長期競爭變數；7/8 持平（價格已校正）"),
    ("競爭", "005930.KS","三星電子",   "277,500 KRW", "-6.25%", "⚠️ 南韓 $1 兆美元晶片投資領頭者；7/8 韓股重挫後 7/9 盤中反彈約 +2.7%；HBM4E 送樣（價格已校正）"),
    ("產業", "SOX",     "費城半導體指數","12,574.97", "+2.23%", "✅ 7/8 反彈 +2.23%（7/7 曾 -4.65%）、AI 晶片股回神；YTD +47%、7/16 起半導體財報季（價格已校正、前報 16,050 有誤）"),
    ("處分後", "ARM",   "Arm Holdings","300.24", "-0.06%", "TSMC 4/29 全數出脫後（7/8 持平；RTX Spark ARM 架構中長線受惠）"),
]

# 法人評級彙整
ANALYST_RATINGS = [
    ("Citi 花旗",      "買入",     "NT$3,800", "2026-07-06", "🚀 7/6 大幅上調目標價自 NT$2,875 至 NT$3,800（+32%、自 7/8 官方收 NT$2,465 具 +54.2% 空間）；為 7/16 法說會前最新升評、預期 TSMC 上修 2026 全年營收成長與長期展望、上調 2027-28 資本支出估至 $75-80B；看好 AI GPU/客製 ASIC/雲端 TPU/網通/光互連/CPU 全面需求"),
    ("Nomura 野村",    "買入",     "NT$3,425", "2026-07-01", "🚀 7/1 大幅上調目標價自 NT$2,820 至 NT$3,425（自 7/8 官方收 NT$2,465 具 +38.9% 空間）；指 AI 基礎設施投資循環尚未見頂、估 2027 資料中心新增產能達 ~32GW"),
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$492.00",    # 分析師 12 個月平均目標（高 $625 Barclays / 低 $351；自 7/8 收 $436.98 具 +12.6% 上行空間；Citi 上調台股目標至 NT$3,800、Barclays 維持 $625、Nomura NT$3,425；Goldman 7/1 移出 APAC Conviction List）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+12.59%",  # 從 NYSE 7/8 收 $436.98 至平均目標 $492.00（共識強力買入、Citi 上調拉高平均；待 7/16 法說會後重估）
}
...
IR_CALENDAR = [
    ...
    ("2026-07-09", "⚖️ 報告日：7/16 法說會倒數 7 天、6 月營收隨時公布", "7/9 Thu 台股開盤 NT$2,450 小幅開低（09:01 快照）、盤中待收；隔夜 NYSE +1.02% / SOX +2.23% 動能偏正面；觀察：（1）守穩 NT$2,440 / 2,420 支撐、挑戰 NT$2,500 / 2,535 史高；（2）外資賣超能否收斂；（3）6 月營收 7 月上旬隨時公布、4-5 月 +24% YoY 落後 ~35% 預期為 Q2 達標關鍵；⚠️ 本日報告依 TWSE 官方數據更正 7/7（收 2,440 非 2,490）與 6/30（收 2,410）記錄",  "#FFF9C4"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,574.97","+2.23%", False),  # 7/8 反彈（Yahoo Finance 實際值；前報 16,050 為漂移估值、已校正）
    ("台股加權指數",    "45,734.41","+0.56%", False),  # 7/8 收盤（Yahoo Finance ^TWII 實際值）
    ("VIX 恐慌指數",    "18.0",    "-0.5",   False),   # 估（半導體反彈、風險情緒回穩；未取得即時值）
    ("WTI 原油",        "$70.0",   "0.00%",  False),   # 油價持穩（估、未取得即時值）
    ("10Y 美債殖利率",  "4.50%",   "0.00pp", False),   # 美債殖利率持穩（估、未取得即時值）
    ("台幣/美元",       "32.07",   "-0.21%", False),   # 台幣（Yahoo TWD=X；每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$204.12", "+3.65%", False),   # #1 TSMC 客戶（22%），7/8 大漲領攻 AI 晶片股
    ("ASML",            "$1,768.65","+1.22%", False),  # 設備股 7/8 反彈（實際值、已校正）
]
...
VALUATION = {
    "pe_current":   32.7,   # 同 STOCK["pe_ratio"]（7/8 官方收 NT$2,465 ÷ 近四季 EPS ~75.5、statementdog）
    ...
    "pb_current":  18.48,   # 同 STOCK["pb_ratio"]（7/8 官方收 NT$2,465 ÷ 每股淨值 ~133.4）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
    "ev_ebitda":    21.0,   # EV/EBITDA（估算）
    "peg":          1.11,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準）、基於 7/8 官方收 NT$2,465：台股 P/E ~32.7x（TTM、近四季 EPS ~75.5、statementdog）、P/B ~18.5x（每股淨值 ~133.4）——⚠️ 前報 P/E ~36.6x／P/B ~12.9x 之估算基礎已校正；NYSE TSM 7/8 收 $436.98、USD P/E 約 37x（估）；估值仍處 5 年高位區間、PEG 約 1.0（以 5 年 EPS CAGR ~33% 計）；⚠️ 留意近期營收 miss 風險（4-5 月 +24% YoY 落後 ~35% 預期、6 月營收 7 月上旬公布）；中長線基底未變：NVIDIA #1 客戶/A16 首發 + TSMC-Amkor 美國封裝長約 + 2nm 量產 70-80% 良率 + 全先進製程漲價 5-10%",
}
...
RELATIVE = {
    "date":           "2026-07-08",       # 最新資料日（台股 2330 7/8 Wed 官方收 NT$2,465、NYSE TSM 7/8 收 $436.98，均已確認）
    "tsmc_tw_1d":     +1.02,   # 2330 7/8 官方收 NT$2,465（+25 vs 7/7 NT$2,440、TWSE）
    "taiex_1d":       +0.56,   # 加權指數 7/8 收 45,734.41（Yahoo ^TWII 實際值）
    "sox_1d":         +2.23,   # SOX 7/8 反彈收 12,574.97（實際值）
    "tsmc_nyse_1d":   +1.02,   # TSM ADR 7/8 $436.98（+$4.41 vs 7/7 $432.57；止跌反彈）
    "tsmc_tw_5d":     -1.60,   # 5 日（NT$2,465 vs 7/1 官方收 NT$2,505）
    "taiex_5d":       -1.20,   # （估）
    "tsmc_tw_1m":     +7.41,   # 月線（NT$2,465 vs 6/8 官方收 NT$2,295）
    "taiex_1m":       +2.3,    # （估）
    "tsmc_tw_ytd":    +17.66,  # YTD 累計（NT$2,465 vs 年初 ~NT$2,095）
    "taiex_ytd":      +13.5,   # （估）
    "sox_ytd":        +47.0,   # SOX YTD（費半 YTD +47%）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/8 NYSE 收盤 & 7/8 TW 官方收盤計算）
    "adr_price":     436.98,   # TSM NYSE 7/8 收盤（+1.02%、Yahoo Finance 日線確認）
    "adr_parity":    384.32,   # 理論值 = 5 × NT$2,465 ÷ 32.07
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.07,   # 同 MACRO 台幣匯率（Yahoo TWD=X）
    "adr_premium_pct": "+13.70", # (436.98 - 384.32) / 384.32 × 100 = +13.70%（匯率 32.07）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。✅ 台美 7/8 同步止跌反彈：NYSE TSM 7/8 Wed 收 $436.98（+$4.41、+1.02% vs 7/7 $432.57、Yahoo Finance 日線確認）自 7/7 -4.25% 回檔止穩、SOX +2.23% 收 12,574.97、NVDA +3.65%、AVGO +4.83% AI 晶片股回神；台股 2330 7/8 Wed 官方收 NT$2,465（+25、+1.02% vs 7/7 NT$2,440、量 25,519 張、TWSE 確認、收在當日最高、市值 NT$63.9 兆）。⚠️【更正】7/7 官方收 NT$2,440（-0.81%）非前報之 NT$2,490；前報 ADR 溢價計算基礎已一併修正。📊 籌碼面（TWSE T86 官方）：7/8 外資 -4,155 張、投信 +730、自營 +468、合計 -2,957 張；7 月外資累計約 -2.9 萬張、賣壓集中 7/2-7/3、近三日收斂。📊 台美 ADR 溢價 +13.70%（$436.98 vs 理論值 $384.32、匯率 32.07）——溢價偏高、台股 7/9 有補漲動能惟亦隱含收斂風險；🎯 今日（7/9）關鍵：台股能否站穩 5MA NT$2,455、突破 NT$2,500-2,505、續挑戰 NT$2,535 史高；6 月營收 7 月上旬隨時公布、7/16 法說會倒數 7 天；🚀 分析師續挺：Citi NT$3,800、Barclays $625、Nomura NT$3,425；⭐ 中長線結構性基底未變：TSMC-Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 量產 70-80% 良率 + CoWoS 擴產 + 4 大 CSP CapEx $725B",
}
...
NEWS = [
    {
        ...
        "title": "✅ 台美同步止跌反彈：台股 2330 7/8 官方收 NT$2,465（+1.02%）收在當日最高、NYSE TSM 收 $436.98（+1.02%）、SOX +2.23%、NVDA +3.65% AI 晶片股全面回神",
        "body": "台美雙市 7/8 同步反彈、幅度巧合同為 +1.02%：（1）✅ 台股 2330 7/8 官方收 NT$2,465（+25、開 2,445／高 2,465／低 2,420、量 25,519 張、TWSE 確認）收在當日最高、止跌回升、站回 5MA NT$2,455；（2）✅ NYSE TSM 7/8 收 $436.98（+$4.41 vs 7/7 $432.57）自 7/7 -4.25% 回檔止穩；（3）🚀 費半 SOX +2.23% 收 12,574.97、NVDA +3.65% 收 $204.12、AVGO +4.83%、AMAT +2.89%、MU +1.11%——AI 晶片股全面回神、7/16 起半導體財報季前情緒回穩；（4）📊 台美 ADR 溢價 +13.7%（理論值 $384.32、匯率 32.07）、台股 7/9 具補漲動能；隔夜動能對台股偏正面。",
        "source": "TWSE 官方日成交資訊 / Yahoo Finance 2026-07-08", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⚠️ 數據更正：依 TWSE 官方，7/7 收盤為 NT$2,440（-0.81%）非前報之 NT$2,490；7/7 外資實為小買 +83 張；本報告已全面按官方數據重算",
        "body": "資料品質更正說明：（1）⚠️ 經 TWSE 官方日成交資訊核實，2330 於 7/7 Tue 實際收 NT$2,440（-20、-0.81%、開 2,480／盤中高 2,500／收最低、量 31,401 張），前兩日報告誤載為 NT$2,490（+30、+1.22%）——7/7 實為開高走低翻黑、而非「補漲連二漲」；（2）6/30 官方收 NT$2,410（前報快照 NT$2,465 為盤中價）；（3）三大法人 7/7 官方為外資 +83 張／投信 +539／自營 -237（前報估外資 +12,000 張有誤）；（4）本日起法人數據改用 TWSE T86 官方值、供應鏈股價改用 Yahoo Finance 日線實際值（多檔累積估值漂移一併校正）；技術指標（5MA 2,455／20MA 2,403）與 ADR 溢價（+13.7%）均已重算。閱讀近兩日歷史報告請以本日更正為準。",
        "source": "TWSE 官方（exchangeReport STOCK_DAY / T86）2026-07-09 核實", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🎯 7/16 Q2 法說會倒數 7 天：Citi 預期上修全年展望、目標 NT$3,800；6 月營收 7 月上旬隨時公布、為 Q2 指引達標關鍵驗證",
        "body": "財報季倒數、催化密集：（1）🎯 TSMC 將於 7/16 台北 14:00 舉行法說會、公布 Q2 財報（指引 $39.0-40.2B、+32% YoY、毛利率 65-67%）並展望 Q3；（2）🚀 Citi 維持台股目標 NT$3,800（自 NT$2,465 具 +54.2% 空間）、預期 TSMC 於 7 月中上修 2026 全年營收成長預測（估 >30%）與 2027-28 資本支出（$75-80B）；Barclays $625、Nomura NT$3,425 續挺、共識強力買入（32 位分析師 0 賣出）；（3）⚠️ 前哨戰：6 月合併營收 7 月上旬隨時公布——4-5 月合計 +24% YoY 落後華爾街 Q2 約 +35% 預期、惟分析師指僅需小幅月增即可使 Q2 逼近指引中值；5 月營收 NT$4,169.75 億（+30.1%）創單月新高。",
        "source": "investor.tsmc.com / Citi / BigGo Finance / TipRanks 2026-07-06 - 07-08", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏆 NVIDIA 穩居 TSMC 第 1 大客戶：2026 估貢獻營收約 $330 億超越 Apple（$270 億）；Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）",
        "body": "客戶結構 AI 化持續深化：（1）🏆 黃仁勳表示 AI 運算需求推升 NVIDIA 成為 TSMC 最大客戶、2026 年估貢獻營收約 $330 億（佔比 ~22%）、超越 Apple 約 $270 億——晶片產業客戶版圖自手機轉向 AI 的結構性訊號；（2）⭐ NVIDIA 次世代 Vera Rubin 平台 6 顆並行晶片全數 TSMC 代工（3nm+HBM4、較 Blackwell 訓練快 3.5×／推論快 5×）、NVIDIA 包下多數 CoWoS 產能、傳為 A16 首發客戶；（3）🤝 NVIDIA-SK 海力士多年期記憶體合作、NVIDIA-TSMC 將 AI 導入晶圓廠製造提升良率；（4）⚠️ 傳 TSMC 對 Apple 調漲先進製程價格、Apple 跳過 A16 直上 A14。",
        "source": "CNBC / Tom's Hardware / NVIDIA Newsroom 2026-01 - 07", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 AI 超級循環續發酵：2026 全球半導體產值上看 $975B（成長率上修至 +26%）、AI 晶片股 7 月市值增約 $2 兆、費半 YTD +47%",
        "body": "產業總量與市場情緒同步走強：（1）📈 全球半導體產業 2026 產值預估達 $975B、成長率上修至 +26%；生成式 AI 晶片估貢獻約 $500B（約全球半數）；BofA 另估 2026 市場規模 $1.3 兆、2030 上看 $2 兆；（2）🚀 AI 晶片股 7 月合計增加約 $2 兆市值、費城半導體指數（SOX）年初至今漲逾 +47%、7/8 反彈 +2.23% 收 12,574.97；（3）🏭 TSMC CEO 曾於 4 月法說會表示供給要到 2027 年才能追上需求、AI 結構性缺口支撐先進製程漲價（7nm 以下全製程 +5-10%、估毛利率 +2pp）；（4）🏆 TSMC 作為 NVIDIA/AMD/Broadcom/Qualcomm 主要代工與先進封裝供應商、被視為本輪擴張最大受惠者。",
        "source": "gurufocus / BofA / WSTS 系列預估 2026-07", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 風險追蹤：外資 7 月累計賣超約 2.9 萬張；Intel 18A 量產吸引微軟/高通進駐；4-5 月營收 +24% YoY 落後預期；Arizona 財務長 7/1 更迭",
        "body": "多項雜訊與競爭變數須留意：（1）📊 籌碼面——外資 7 月對 2330 累計賣超約 2.9 萬張（7/2 -12,254、7/3 -12,537、7/6 -241、7/7 +83、7/8 -4,155、TWSE T86 官方），雖近三日收斂、但尚未轉買；投信連 5 日買超為穩定力量；（2）⚔️ 競爭——Intel 在 CEO Lip-Bu Tan 帶領下代工轉型、18A 節點進入量產並吸引微軟、高通等大客戶，對 TSMC 構成長期競爭壓力（惟 Intel 亦為 TSMC 客戶、雙方維持夥伴關係）；（3）⚠️ 營收動能——4-5 月合計 +24% YoY 落後華爾街 Q2 ~35% 預期、6 月營收與 7/16 財報為關鍵驗證；下游手機/PC 需求疲弱；（4）🏢 人事——TSMC Arizona 財務長 Gina Proctor 7/1 卸任、新任待董事會公告（屬一般人事變動）；（5）估值不低：台股 P/E ~32.7x（TTM）、TSM ~37x、台幣 32.07。",
        "source": "TWSE T86 / gurufocus / TipRanks 2026-07-01 - 07-08", "impact": "負面",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,250",        # 2026-05-05 Tue 收盤（+25 vs 5/4 NT$2,275，+1.10%；漲勢延續、量縮整理）
    "tw_change":      "+25",          # 漲跌點數（vs 5/4 NT$2,275）
    "tw_change_pct":  "+1.10%",       # 漲跌幅
    "tw_volume":      "21,565",       # 成交量（張，5/4 爆量飆漲後量縮逾 7 成、健康整理）
    "tw_open":        "2,280",        # 5/5 開盤（跟進 5/4 強勢開高）
    "tw_high":        "2,300",        # 5/5 盤中高（觸心理整數 NT$2,300）
    "tw_low":         "2,235",        # 5/5 盤中低
    "market_cap":     "NT$58.4 兆",   # 5/5 收盤市值
    ...
    # NYSE TSM ADR
    "nyse_price":     "396.89",       # 5/5 實際收盤（-4.72 vs 5/4 $401.61，-1.18%；獲利了結、跌破 $400）
    "nyse_change":    "-4.72",        # 5/5 實際漲跌
    "nyse_change_pct":"-1.18%",       # 5/5 ADR 拉回、跌破 $400 心理關卡
    "52w_high_nyse":  "404.98",       # 52 週高（2026-04-27 收盤）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~32.8x",       # NYSE USD 基礎 P/E（TTM，估算，$396.89 基礎）
    "pb_nyse":        "~11.6x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "今日（2026-05-06 Wed）重點：台股 2330 5/5 Tue 收 NT$2,250（+25，+1.10% vs 5/4 NT$2,275）盤中觸 NT$2,300 心理整數新高、量縮 21,565 張屬健康整理；台股加權指數同日 +64.15（+0.16%）收 40,769.29 連 2 日站穩 4 萬大關；NYSE TSM 5/5 收 $396.89（-4.72，-1.18%）拉回跌破 $400 關卡、台美短線分歧再現  |  AMD Q1 財報盤後大超預期：EPS $1.37 vs $1.29、營收 $10.25B vs $9.89B、Q2 指引 $11.2B 遠超 $9.8B 共識、AH 漲約 +15%——TSMC #4 客戶確認 AI 加速器需求結構性轉強、CoWoS 訂單續強  |  TSMC 重啟桃園龍潭 Phase 3（A14/A13 1.6nm 用地），總投資 23 兆韓元（$16.9B）；AMD/Intel 聯合宣布 x86 AI Compute Extensions 計算密度 +16x  |  5/5 三大法人合計賣超 7,280 張：外資 -8,581（連 1 賣）、投信 +1,496（連 8 買）、自營 -195  |  Goldman Sachs 維持目標 NT$2,330（剩 +3.6%）、Barclays $470（剩 +18.4%）、共識 NT$2,320（剩 +3.1%）  |  TSMC 4 月月營收 5/10 前公布（估 NT$330B+）；NVDA 5/28 財報",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "📈 台股 2330 5/5 Tue 量縮收 NT$2,250（+25，+1.10%）盤中觸 NT$2,300 心理整數新高；NYSE TSM 5/5 拉回收 $396.89（-1.18%）跌破 $400；AMD Q1 大超預期 AH +15% 確認 AI 需求續強",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📈 台股 2330 5/5 收 NT$2,250（+25，+1.10%）盤中觸 NT$2,300 心理整數新高、量縮健康整理",
        "📊 加權指數 5/5 收 40,769.29（+64.15，+0.16%）連 2 日站穩 4 萬大關",
        "🇺🇸 NYSE TSM 5/5 收 $396.89（-4.72，-1.18%）拉回跌破 $400、台美短線分歧",
        "🚀 AMD Q1 大超預期：EPS $1.37 vs $1.29、營收 $10.25B、Q2 指引 $11.2B、AH +15%",
        "🏭 TSMC 重啟桃園龍潭 Phase 3 fab：23 兆韓元（$16.9B）投資、A14/1.6nm 用地",
        "📅 短線催化：TSMC 4 月月營收 5/10 前公布（估 NT$330B+）；NVDA 5/28 財報",
    ],
    # 一句結論
    "bottom_line": "今日（2026-05-06 Wed）報告摘要：台股 2330 昨日（5/5 Tue）續強收 NT$2,250（+25 點，+1.10% vs 5/4 NT$2,275），盤中一度觸及 NT$2,300 心理整數創 52 週新高（盤中區間 NT$2,235–2,300）；成交量量縮至 21,565 張（vs 5/4 爆量 92,800 張，量縮約 77%），屬爆量飆漲後正常獲利了結與健康整理；台股加權指數同日 +64.15 點（+0.16%）收 40,769.29 連 2 日站穩 4 萬大關。NYSE TSM 5/5 收 $396.89（-4.72，-1.18% vs 5/4 $401.61），拉回跌破 $400 心理關卡，台美短線分歧再現；ADR 對台股溢價自 +9.8% 進一步收斂至約 +9.0%。短線最大利多——AMD Q1 2026 財報 5/5 盤後大超預期：營收 $10.25B（vs 共識 $9.89B、+33% YoY）、調整後 EPS $1.37（vs 共識 $1.29、+34% YoY）；Q2 指引營收 $11.2B（遠超共識 $9.8B）；股價盤後狂漲約 +15%。AMD 為 TSMC #4 大客戶（佔比 8%），結果確認 AI 加速器需求結構性轉強、CoWoS 訂單續強，將為 5/6 台股 TSMC 帶來顯著正面催化。產業面：TSMC 5/5 宣布重啟桃園龍潭科學園區 Phase 3 擴建（停滯 3 年後居民同意），總投資 23 兆韓元（約 $16.9B），用於 A14/A13 / 1.6nm 下世代奈米製程，預計 2026 H2 量產；AMD/Intel 聯合宣布 x86 AI Compute Extensions 指令集，計算密度提升 16x。籌碼面：5/5 三大法人合計賣超 7,280 張——外資 -8,581（5/4 大買後反手獲利了結）、投信 +1,496（連 8 買）、自營 -195（小幅賣超）。基本面強化：Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2%（+740bps YoY）雙創史高；2026 全年營收成長指引上修 >30%；CapEx 上修至 $52-56B 高端；2nm 至 2028 年產能 CAGR +70%、Arizona +80% YoY、熊本 +130% YoY；A13/A12（2029）、N2U（2028）、A16（2027）製程路線完整確認。即將事件：（1）AMD AH +15% 將於 5/6 台股開盤帶動 TSMC 與 AI 供應鏈正面回應；（2）TSMC 4 月合併月營收 5/10 前公布（市場預估 NT$330B+，YoY +30%）；（3）NVDA Q1 FY27 財報 5/28（最重要 AI 算力指標）。技術面：5/5 收 NT$2,250 雖較 5/4 NT$2,275 略回，但盤中觸 NT$2,300 確認多頭挑戰心理整數動能；技術指標自 5/4 高溫稍微降溫——RSI ~64（自 68.5 略降、進健康區）、KDJ J 自 100 拉回至 ~92、MACD 維持 +3.0 紅柱、5MA / 20MA / 60MA / 120MA 維持多頭排列。下一觀察：（1）今日（5/6 Wed）能否受惠 AMD Q1 利多挑戰並守穩 NT$2,300；（2）共識目標 NT$2,320（剩 +3.1%）、Barclays $470 對應 NT$2,450（剩 +8.9%）。31 位分析師全數看多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（5/6 Wed）關鍵觀察：（1）AMD Q1 大超預期 AH +15% 為短線最大利多，預期帶動 5/6 台股 AI 供應鏈（聯發科、日月光、創意）與 TSMC 正面開盤；（2）NYSE TSM 5/5 拉回跌破 $400（-1.18%）顯示美股短線獲利了結，台美 ADR 溢價 +9.0% 仍處高位；（3）今日台股能否守 NT$2,250 並挑戰 NT$2,300（盤中高）/ 共識目標 NT$2,320 為關鍵；（4）外資 5/5 反手賣超 8,581 張（vs 5/4 +25,800 張）顯示獲利了結壓力升溫，需觀察 5/6 是否再度回補；（5）短線估值 P/E 33.2x 仍處 5 年 87% 高位、需財報持續驗證；（6）4 月月營收 5/10 前公布為下一個重要催化（估 NT$330B+）；（7）台幣 ~31.10 升值壓力結構性續存（每升 1% 毛利率 -40bps）；（8）CoWoS/ABF 基板瓶頸 + 美中 AI 晶片出口管制持續；（9）NVDA 5/5 收 $196.15（-1.17%）拉回，AMD AH +15% 能否抵銷 NVDA 拉回為 5/6 半導體股關鍵；（10）HSBC 5/4 將 AMD 自 Buy 降至 Hold（目標 $335→$340）顯示部分分析師對估值轉趨保守",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-05-05",
    "foreign":  -8581,    # 外資（5/5 實際，反手獲利了結、終結 5/4 大買）
    "trust":    +1496,    # 投信（5/5 實際，連 8 買、續加碼）
    "dealer":   -195,     # 自營商（5/5 實際，小幅賣超、終結 6 連買）
    # 近5日（由舊到新：4/28, 4/29, 4/30, 5/4, 5/5）
    "foreign_5d": [-22114, -8500, -21183, +25800, -8581],   # 5/5：外資反手獲利了結
    "trust_5d":   [+861, +650, +596, +1820, +1496],          # 5/5：投信連 8 買、續加碼
    "dealer_5d":  [+500, +320, +115, +1350, -195],           # 5/5：自營小幅賣超
    "foreign_ownership_pct": "75.0%",  # 外資持股比例（5/5 小幅獲利了結後）
    "foreign_ownership_shares": "約 194.6 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       64.0,   "rsi_signal": "RSI 自 5/4 68.5 微降至 64.0，仍在強勢區（>50）但稍降溫、未過熱；技術健康整理中",
    "macd":         +3.0,   "macd_signal": "MACD 自 +3.5 微降至 +3.0，柱狀體仍正、黃金交叉維持；多頭趨勢續穩",   "macd_color": "#4CAF50",
    "kdj_k":        82.0,   "kdj_d": 70.0, "kdj_j": 92.0, "kdj_signal": "KDJ K(82)/D(70)/J(92)，J 值自 100 拉回降溫、K/D 多頭交叉維持；短線過熱緩解",
    "ma5":       "2,210",
    "ma20":      "2,160",
    "ma60":      "2,030",
    "ma120":     "1,930",
    "bb_upper":  "2,330",   # 布林上軌（5/5 隨高點上移）
    "bb_mid":    "2,160",   # 布林中軌（20MA）
    "bb_lower":  "1,990",   # 布林下軌
    "vol_ma5":  "57,800",   # 5日均量（張，5/5 量縮拉低均量）
    "trend":    "台股 2330 5/5 Tue 收 NT$2,250（+25，+1.10%）盤中觸 NT$2,300 創 52 週新高，量縮 21,565 張屬爆量飆漲後正常獲利了結與健康整理。台股加權指數同日 +64.15（+0.16%）收 40,769.29 連 2 日站穩 4 萬大關。NYSE TSM 5/5 拉回 -1.18% 收 $396.89 跌破 $400 關卡、台美短線分歧。技術面降溫但維持多頭：RSI 自 68.5 微降至 64.0、MACD +3.0 紅柱續正、KDJ J 自 100 拉回至 92 解除短線過熱、K(82)/D(70) 多頭交叉維持。現價 NT$2,250 仍站上全部均線（5MA NT$2,210、20MA NT$2,160、60MA NT$2,030、120MA NT$1,930）短中長線多頭排列維持。短線最大利多——AMD Q1 財報 5/5 盤後大超預期 AH +15%（EPS $1.37 vs $1.29、營收 $10.25B vs $9.89B、Q2 指引 $11.2B），預期 5/6 台股 TSMC 與 AI 供應鏈正面回應。下一觀察：（1）今日（5/6 Wed）能否受惠 AMD 利多挑戰 NT$2,300 / 共識目標 NT$2,320（+3.1%）；（2）Barclays $470 對應 NT$2,450 剩 +8.9% 上行空間；（3）外資反手賣超 8,581 張、需觀察 5/6 能否止賣回補；（4）支撐 NT$2,210（5MA）/ NT$2,200（心理）/ NT$2,160（20MA）/ NT$2,135（前壓轉支撐）。技術短線降溫健康、中長期趨勢續強多明確。",
    "support":  "2,210 / 2,200 / 2,160 (5MA、心理、20MA)",
    "resist":   "2,300 / 2,320 / 2,450",
    "note": "技術指標數值為估算，請以看盤軟體即時數據為準；5/5 量縮整理後 RSI 健康降溫、KDJ J 值離超買、MACD 多頭交叉維持",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-05-05",
    "futures_price": "2,255",      # 台積電期貨近月（估，5/5 收盤後微漲、正價差續存）
    "futures_oi":    "115,200",    # 未平倉量（口）（估，5/5 多單部分獲利了結）
    "futures_basis": "+5",         # 正價差小幅縮減至 +5 點（多頭情緒微降溫、仍偏多）
    "call_oi":       "76,800",
    "put_oi":        "43,500",
    "pcr":           "0.57",       # Put/Call 比率自 0.54 微升至 0.57（避險需求小幅回升、仍偏多）
    "pcr_signal":    "偏多（Put/Call 0.57 仍 < 0.7，Call 部位較高、整體多頭）",
    "max_pain":      "2,250",      # 最大痛苦價格（近期到期，與 5/5 收盤一致）
    "key_strikes": [
        ("2,150", "Call 22,500 / Put 9,800", "前支撐、深度支撐"),
        ("2,200", "Call 28,800 / Put 7,800", "心理整數支撐"),
        ("2,210", "Call 30,500 / Put 7,000", "5MA 支撐"),
        ("2,250", "Call 33,200 / Put 6,200", "5/5 收盤價"),
        ("2,275", "Call 34,200 / Put 5,800", "5/4 收盤、前支撐"),
        ("2,300", "Call 35,500 / Put 5,200", "心理整數壓力、5/5 盤中高"),
        ("2,320", "Call 36,800 / Put 4,500", "共識目標、強壓力"),
    ],
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；5/5 量縮整理後 Put/Call 0.57 偏多、多頭情緒微降溫",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-05-05 美股收盤 + 台股 5/5 收盤）
    ("設備", "ASML",    "艾司摩爾",    "698.40", "-0.96%", "EUV 設備龍頭（5/5 小幅獲利了結；2nm 訂單能見度長線正面）"),
    ("設備", "AMAT",    "應用材料",    "186.20", "-1.22%", "CVD/PVD 設備（5/5 拉回；長線 AI CapEx 利多續存）"),
    ("設備", "LRCX",    "科林研發",    "838.50", "-0.80%", "蝕刻設備（5/5 拉回；2nm 訂單長線結構續穩）"),
    ("設備", "KLAC",    "科磊",        "624.50", "-1.06%", "量測設備（5/5 拉回；長線 AI CapEx 利多續存）"),
    ("記憶", "000660.KS","SK 海力士",  "316,000 KRW", "-0.78%", "HBM4 供應（5/5 小幅拉回；GB300 訂單續穩）"),
    ("記憶", "MU",      "美光",        "110.50", "-1.52%", "HBM / DDR5（5/5 拉回；CSP CapEx 利多續存）"),
    ("封裝", "3711.TW", "日月光",      "265",    "+1.15%", "CoWoS 封裝（5/5 跟進台股續強；2nm/CoWoS 訂單續強）"),
    ("客戶", "NVDA",    "輝達",        "196.15", "-1.17%", "#1 客戶 22%（5/5 拉回 vs 5/4 $198.48；5/28 Q1 FY27 財報為下一催化）"),
    ("客戶", "AAPL",    "蘋果",        "282.55", "+0.22%", "#2 客戶 17%（5/5 持平；M5 / A20 量產續穩）"),
    ("客戶", "AVGO",    "博通",        "215.80", "-1.24%", "#3 客戶 13%（5/5 拉回；Custom XPU 訂單續穩）"),
    ("客戶", "AMD",     "超微",        "184.50", "-1.02%", "#4 客戶 8%（5/5 盤後 Q1 大超預期 AH +15%：EPS $1.37 vs $1.29、營收 $10.25B vs $9.89B、Q2 指引 $11.2B）"),
    ("客戶", "2454.TW", "聯發科",      "2,440",  "+1.04%", "手機 SoC 客戶 9%，5/5 跟進續強；2nm 首批量產不變"),
    ("競爭", "INTC",    "英特爾",      "23.18",  "-1.15%", "Intel Foundry 18A 爬坡；5/5 拉回；x86 AI Compute Extensions 與 AMD 合作"),
    ("競爭", "005930.KS","三星電子",   "56,000 KRW", "-0.71%", "Samsung Foundry 5/5 拉回；HBM4E NVIDIA 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","205.30", "-1.20%", "TSMC 4/29 全數出脫後股價拉回（5/5 -1.20%）；Arm 自身基本面未變"),
]
...
CONSENSUS = {
    "rating":       "強烈買入",
    "avg_target_tw":"NT$2,320",  # 共識（Barclays 升評後）— 5/5 NT$2,250 距共識剩 +3.1%
    "high_target":  "NT$2,770",
    "low_target":   "NT$1,740",
    "avg_target_us":"US$525",    # 分析師上調目標（Barclays $470 上拉均值）
    "analyst_count": 31,
    "buy": 31, "hold": 0, "sell": 0,
    "upside":       "+3.1%",    # 從 NT$2,250（5/5 收盤）到 NT$2,320
}
...
IR_CALENDAR = [
    ...
    ("2026-05-05", "✅ 量縮續強收 NT$2,250、盤中觸 NT$2,300 心理整數新高","台股 2330 5/5 Tue 收 NT$2,250（+25，+1.10%）、盤中觸 NT$2,300 心理整數創 52 週新高；成交量量縮至 21,565 張（vs 5/4 92,800 張，量縮 77%）屬爆量後健康整理；台股加權指數同日 +64.15（+0.16%）收 40,769.29 連 2 日站穩 4 萬；NYSE TSM 5/5 拉回收 $396.89（-1.18%）跌破 $400、台美短線分歧；籌碼：外資反手 -8,581（獲利了結）、投信連 8 買 +1,496、自營 -195，合計賣超 7,280 張；AMD Q1 盤後大超預期 AH +15%（EPS $1.37、營收 $10.25B、Q2 指引 $11.2B）；TSMC 重啟桃園龍潭 Phase 3 fab 計畫（$16.9B 投資、A14/1.6nm 用地）","#9E9E9E"),
    ("2026-05-06", "⭐ 今日：AMD Q1 大超預期 AH +15% 利多、能否挑戰 NT$2,300","今日（5/6 Wed）關鍵觀察：（1）AMD Q1 大超預期 AH +15% 為短線最大利多，預期帶動 5/6 台股 TSMC 與 AI 供應鏈正面開盤；（2）能否守 NT$2,250 並挑戰 NT$2,300（5/5 盤中高）/ 共識目標 NT$2,320（剩 +3.1%）/ Barclays $470 對應 NT$2,450（剩 +8.9%）；（3）NYSE TSM 5/5 拉回 -1.18% 跌破 $400 顯示美股短線獲利了結；（4）外資 5/5 反手賣超 8,581 張，需觀察 5/6 是否回補；（5）技術面 RSI 自 68.5 微降至 64 健康整理、KDJ J 自 100 拉回至 92 解除過熱；（6）即將事件：TSMC 4 月月營收 5/10 前公布（估 NT$330B+）、NVDA 5/28 Q1 FY27 財報","#BBDEFB"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "10,672.50","-1.14%", False),  # 5/5 SOX 拉回、AMD 財報前獲利了結
    ("台股加權指數",    "40,769.29","+0.16%", True),   # 5/5 連 2 日站穩 4 萬大關
    ("VIX 恐慌指數",    "17.2",    "+0.4",   False),  # 恐慌指數小升、財報季避險
    ("WTI 原油",        "$64.5",   "-0.46%", False),  # 油價小幅回落
    ("10Y 美債殖利率",  "3.97%",   "+0.02pp",False),  # 殖利率持穩
    ("台幣/美元",       "31.10",   "0.00",   False),  # 台幣持平（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$196.15", "-1.17%", False),  # #1 TSMC 客戶（22%），5/5 拉回
    ("ASML",            "$698.40", "-0.96%", False),  # 設備股 5/5 小幅獲利了結；2nm 訂單續穩
]
...
RELATIVE = {
    "date":           "2026-05-05",
    "tsmc_tw_1d":     +1.10,   # 2330 5/5 收盤 NT$2,250（+25 vs 5/4 NT$2,275）
    "taiex_1d":       +0.16,   # 加權指數 5/5 收 40,769.29
    "sox_1d":         -1.14,   # SOX 5/5 拉回
    "tsmc_nyse_1d":   -1.18,   # TSM ADR 5/5 $396.89（-1.18% vs 5/4 $401.61）
    ...
    "taiex_ytd":      +5.7,
    "sox_ytd":        -6.6,    # SOX 5/5 拉回後 YTD 略擴
    "beta_60d":        1.25,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 5/5 實際 NYSE 收盤計算）
    "adr_price":     396.89,   # TSM NYSE 5/5 實際收盤
    "adr_parity":    361.74,   # 理論值 = 5 × NT$2,250 ÷ 31.10
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.10,   # 同 MACRO 台幣匯率（5/5）
    "adr_premium_pct": "+9.7", # (396.89 - 361.74) / 361.74 × 100（溢價維持 +9.7%、台美短線分歧再現）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。5/5 台股 +1.10% 而 ADR -1.18%，溢價自 +9.8% 維持 +9.7%；台美短線分歧再現",
}
...
NEWS = [
    {
        ...
        "title": "🚀 AMD Q1 2026 財報大超預期：EPS $1.37 vs $1.29、營收 $10.25B、Q2 指引 $11.2B、AH 漲約 +15%",
        "body": "AMD 5/5 Tue 美股盤後公布 Q1 2026 財報全面大超預期：營收 $10.25B（vs 共識 $9.89B、+33% YoY）、調整後 EPS $1.37（vs 共識 $1.29、+34% YoY）；Q2 指引營收 $11.2B（遠超共識 $9.8B）；股價盤後狂漲約 +15%。AMD 為 TSMC #4 大客戶（佔比 8%），結果直接確認 AI 加速器需求結構性轉強、CoWoS 訂單續強：（1）Data Center GPU 營收（MI300X / MI350）大幅成長確認 TSMC CoWoS 滲透率；（2）Q2 指引上修確認 H2 2026 AI 算力需求向上修正；（3）MI450（2027，TSMC N3）訂單能見度提升；（4）Zen6 EPYC「Venice」（TSMC 2nm，2027）量產進度確認。預期將為 5/6 台股 TSMC 與 AI 供應鏈（聯發科、日月光、創意）帶來顯著正面催化、推升挑戰 NT$2,300 / NT$2,320。",
        "source": "CNBC / Yahoo Finance / Bloomberg 2026-05-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📈 台股 2330 5/5 收 NT$2,250（+1.10%）盤中觸 NT$2,300 心理整數新高、量縮 21,565 張健康整理",
        "body": "台股 2330 5/5 Tue 收 NT$2,250（+25 點，+1.10% vs 5/4 NT$2,275），盤中一度觸及 NT$2,300 心理整數創 52 週新高（盤中區間 NT$2,235–2,300）；成交量量縮至 21,565 張（vs 5/4 92,800 張，量縮 77%），屬爆量飆漲後正常獲利了結與健康整理。台股加權指數同日 +64.15 點（+0.16%）收 40,769.29 連 2 日站穩 4 萬大關；TSMC 市值約 NT$58.4 兆。NYSE TSM 5/5 拉回收 $396.89（-4.72，-1.18% vs 5/4 $401.61）跌破 $400 心理關卡，台美短線分歧再現；ADR 對台股溢價維持 +9.7%。今日（5/6 Wed）關鍵：能否受惠 AMD Q1 利多挑戰 NT$2,300 / 共識目標 NT$2,320（剩 +3.1%）/ Barclays $470 對應 NT$2,450（剩 +8.9%）。",
        "source": "TWSE / Yahoo Finance / Goodinfo 2026-05-05", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "🏗️ TSMC 重啟桃園龍潭 Phase 3：23 兆韓元（$16.9B）投資 A14 / 1.6nm 下世代奈米製程廠",
        "body": "TSMC 5/5 宣布重啟桃園龍潭科學園區 Phase 3 擴建——3 年前因居民反對土地徵收而停滯、近期居民立場逆轉同意後 TSMC 隨即重啟此案。預計總投資至少 23 兆韓元（約 $16.9B）建設下世代奈米製程廠：（1）擬用於 A14（angstrom-class、1.4nm）與 1.6nm 製程；（2）1.6nm 預計 2026 H2 量產啟動；（3）龍潭定位為下世代先進製程戰略重鎮；（4）TSMC 2026 年 CapEx 維持 $52-56B 高端不變；（5）多廠擴張：台灣（新竹／台南／高雄／龍潭）+ 美國 Arizona + 日本熊本同步推進。為長期 AI 結構性需求佈局：4 大 CSP 2026 AI CapEx $725B + AMD Q1 大超預期確認 AI 加速器需求向上修正。Goldman Sachs 預期 2026 / 2027 營收 +30% / +28%、2027 EPS 上看 NT$100。",
        "source": "Taiwan News / Seoul Economic Daily 2026-05-05", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "💻 AMD/Intel 聯合宣布 x86 AI Compute Extensions：計算密度 +16x、AI CPU 加速器整合催化",
        "body": "AMD 與 Intel 5/5 聯合宣布針對 x86 CPU 推出全新指令集——AI Compute Extensions——專為提升 AI 工作負載性能與能源效率設計：（1）計算密度提升達 16 倍；（2）整合 CPU 與 AI 加速器運算流；（3）為 x86 CPU 對抗 ARM 架構（NVIDIA Grace、Apple Silicon）的關鍵反擊；（4）兩大 CPU 龍頭罕見合作顯示 AI 時代 x86 生態急需整合。對 TSMC 影響：（1）AMD Zen6「Venice」（2nm，2027）與 Intel Foundry 18A / 14A 客戶將直接受惠；（2）AMD 仍為 TSMC 大客戶、Intel 18A 自家代工但部分晶片仍交 TSMC；（3）整體推升 AI / HPC 高階先進製程需求結構續強；（4）OpenAI / Google / Anthropic 等大型 AI 公司導入 x86 AI 加速 server CPU 為 H2 2026 重要催化。",
        "source": "CNBC / Bloomberg 2026-05-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏦 5/5 三大法人籌碼小幅獲利了結：外資 -8,581 張、投信連 8 買 +1,496、自營 -195，合計 -7,280 張",
        "body": "台股 2330 5/5 三大法人籌碼小幅獲利了結——外資自 5/4 反手大買 +25,800 張轉為小幅賣超 -8,581 張（連 1 賣、屬正常獲利了結）；投信連續第 8 個交易日買進 +1,496 張（連 8 買、續加碼）顯示本土法人多頭信心穩固；自營商小幅賣超 -195 張（終結 6 連買）。三大法人合計賣超 7,280 張（vs 5/4 +28,970 張）。外資持股比例自 5/4 75.1% 微降至 75.0%，5/5 賣超台積電金額估約 NT$190 億。籌碼面解讀：（1）5/4 爆量飆漲後正常獲利了結，量縮 77% 屬健康整理；（2）投信連 8 買為本波多頭強化訊號；（3）AMD Q1 大超預期 AH +15% 預期將推升 5/6 外資回補意願；（4）需觀察 5/6 是否成功重啟外資買盤。",
        "source": "TWSE / Goodinfo / HiStock 2026-05-05", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📅 TSMC 4 月合併月營收 5/10 前公布：市場預估 NT$330B+（+30% YoY），Q2 旺季續強訊號",
        "body": "TSMC 將依例於 5/10 前公布 4 月合併月營收。市場預估 4 月營收為 NT$330B+，雖較 Q1 月平均 NT$378B 季節性回落（Q1 受 iPhone 16 / Apple 拉貨支持較強），但仍將維持 +30% YoY 以上的高速成長動能。重點觀察：（1）4 月營收能否守住 NT$330B 以上 → 確認 Q2 指引 $39.0-40.2B 達成可能；（2）Q1 平均月營收 NT$378B 顯示 N3 / 2nm 客戶持續滿載；（3）AMD Q1 大超預期 + Q2 指引上修確認 AI 加速器需求結構續強；（4）NVIDIA Blackwell GB200 / GB300 出貨支持高階先進製程；（5）台幣 31.10 持續處於升值區間（每升 1% 毛利率 -40bps）；（6）4 大 CSP $725B AI CapEx 將直接拉升 H2 2026 營收能見度。法人預期：4 月 NT$330B+ → 5 月 NT$345B → 6 月 NT$355B 進入 Q2 旺季。月營收為下一個短線最重要催化。",
        "source": "TSMC IR / MoneyDJ / Anue 2026-05-06", "impact": "中性",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "1,780",        # 收盤價（數字加逗號，不含 NT$）（2026-04-01 收盤）
    "tw_change":      "-40",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-2.20%",       # 漲跌幅（含+/-號）
    "tw_volume":      "35,000",       # 成交量（張，估）
    "tw_open":        "1,820",
    "tw_high":        "1,825",
    "tw_low":         "1,775",
    "market_cap":     "NT$46.2 兆",
    "pe_ratio":       "~26.8x",       # 台股 P/E（TTM，2026-04-01）
    "pb_ratio":       "~9.8x",        # 台股 P/B（股淨比）
    "div_yield":      "~1.15%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "333.82",       # 收盤價（2026-04-01，與 3/31 收盤相近）
    "nyse_change":    "-0.00",        # 與前日相近
    "nyse_change_pct":"0.00%",
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-04-01 收盤（NT$1,780，-2.20%）  |  NYSE 數據：2026-04-01 $333.82（-0.00%）  |  台股連續走弱，4/1 再跌 40 元，技術面趨近超賣  |  高盛目標價 $370–$375 USD（強烈買入）  |  4/16 Q1 財報倒計時 15 天",
}
...
SUMMARY = {
    ...
    "sentiment":  "看空",
    # 一句話標題
    "headline":   "台股 2330 收 NT$1,780（-2.20%）延續跌勢，NYSE TSM $333.82，4/16 Q1 財報倒計時 15 天，長線訂單售罄至 2028 支撐結構",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330 收 NT$1,780（-40，-2.20%），成交量估 3.5 萬張，連兩日走弱，技術面接近超賣",
        "NYSE TSM $333.82 相對穩定，季末調整後美股維持，Q1 財報（4/16）倒計時 15 天為主要催化劑",
        "TSMC 先進製程（2nm/3nm）訂單已全數售罄至 2028 年，NVIDIA、Apple、AMD 全數預訂產能",
        "NVIDIA 正式超越 Apple 成 TSMC 最大客戶（22%，年收入估 $330 億），產業重心向 AI 轉移",
        "全球半導體市場 2026 年預計突破 $1 兆美元，AI 晶片佔超過半數收入；TSMC 受益最深",
    ],
    # 一句結論
    "bottom_line": "台積電（4/1）台股 2330 收 NT$1,780（-2.20%，-40），連續兩日走弱，技術面進入超賣區間，短線支撐下移至 1,770–1,780。NYSE TSM $333.82 相對穩定。長線基本面強勁：先進製程訂單售罄至 2028、NVIDIA 成第一大客戶（22%）、2026 年半導體市場破兆、TSMC 日本熊本二廠獲批 3nm 製程。4/16 Q1 財報（EPS 共識 NT$20.38）為最近關鍵催化劑，近 8 季全部超預期。分析師 31 人全數買入，目標 NT$2,290（+28%），短線低點視為長線佈局機會。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 台股 2330（4/1）收 NT$1,780（-2.20%）；NYSE TSM $333.82；Q1 財報 4/16 倒計時 15 天；技術面超賣，注意反彈或進一步下跌；AGM 提案期間 3/31–4/7；關注全球科技股及地緣政治風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-01",
    "foreign":  -3200,    # 外資（正=買超）— 4/1 持續賣超（估）
    "trust":    +300,     # 投信（持續逢低承接，估）
    "dealer":   -150,     # 自營商（估）
    # 近5日（由舊到新，最後一筆為今日估算）
    "foreign_5d": [-3500, -2000, -1800, -20438, -3200],  # 持續賣超（3/31 實際 -20,438）
    "trust_5d":   [+280,  +260,  +270,  +1415,  +300],
    "dealer_5d":  [-200,  -150,  -160,  +2732,  -150],
    "foreign_ownership_pct": "74.3%",  # 外資持股比例（估，持續降）
    "foreign_ownership_shares": "約 193.0 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       32.5,   "rsi_signal": "深入超賣區間（RSI < 33），連跌後動能轉弱，留意技術反彈",
    "macd":         -11.2,  "macd_signal": "MACD 死叉加深，空頭動能延續，需量能放大方可確認底部",   "macd_color": "#F44336",
    "kdj_k":        22.8,   "kdj_d": 30.1, "kdj_j": 8.2, "kdj_signal": "KDJ J 值 8.2，深度超賣，短線有技術反彈空間",
    "ma5":       "1,798",
    "ma20":      "1,860",
    "ma60":      "1,828",
    "ma120":     "1,738",
    "bb_upper":  "1,975",   # 布林上軌
    "bb_mid":    "1,860",   # 布林中軌（20MA）
    "bb_lower":  "1,745",   # 布林下軌
    "vol_ma5":  "30,000",   # 5日均量（張，估）
    "trend":    "台股 2330（4/1 收 NT$1,780，-2.20%）連續走弱，技術指標深入超賣。NYSE TSM $333.82 相對穩定。RSI 降至 32.5（超賣區間），MACD 死叉加深，KDJ J 值 8.2，短線技術反彈空間存在。支撐下移至 1,770–1,780。4/16 Q1 財報（倒計時 15 天）為關鍵催化劑，留意反彈機會。",
    "support":  "1,770–1,780",
    "resist":   "1,800–1,820",
...
DERIVATIVES = {
    "date":          "2026-04-01",
    "futures_price": "1,775",      # 台積電期貨近月（估，略低現貨 1,780，負價差延續）
    "futures_oi":    "80,200",     # 未平倉量（口）（估，空單持續增加）
    "futures_basis": "-5",         # 負價差（空頭主導）
    "call_oi":       "26,800",
    "put_oi":        "62,500",
    "pcr":           "2.33",       # Put/Call 比率上升（空頭情緒更濃厚）
    "pcr_signal":    "偏空",
    "max_pain":      "1,810",      # 最大痛苦價格（持續下移）
    "key_strikes": [
        ("1,740", "Call 1,600 / Put 14,000", "布林下軌支撐"),
        ("1,770", "Call 2,800 / Put 12,500", "新短線重要支撐"),
        ("1,780", "Call 4,200 / Put 10,200", "現貨所在"),
        ("1,810", "Call 7,500 / Put 6,200",  "最大痛苦點/壓力"),
        ("1,840", "Call 10,800 / Put 3,500", "強壓力區"),
    ],
...
CONSENSUS = {
    ...
    "upside":       "+25.8%",   # 從 NT$1,820 到 NT$2,290
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "3,885",   "-0.9%",  False),  # 4/1 估，季初延續弱勢
    ("美元指數 DXY",    "104.5",   "+0.3%",  False),  # 美元持強，台幣承壓
    ("VIX 恐慌指數",    "28.5",    "-1.3",   True),   # 恐慌指數略降（仍偏高）
    ("WTI 原油",        "$71.5",   "+0.4%",  False),
    ("10Y 美債殖利率",  "4.26%",   "-0.02pp",False),  # 避險需求，殖利率微降
    ("台幣/美元",       "31.85",   "-0.05",  False),  # 台幣持續小貶
    ("NVIDIA NVDA",     "$848",    "-0.8%",  False),  # AI 大廠小幅回落
    ("ASML",            "$678",    "-0.4%",  False),  # 設備股小幅整理
]
...
RELATIVE = {
    "date":           "2026-04-01",
    "tsmc_tw_1d":     -2.20,   # 2330 4/1 收盤 1,780（-2.20%）
    "taiex_1d":       -1.20,   # 加權指數（4/1 估算）
    "sox_1d":         -0.90,   # SOX（4/1 估，季初整理）
    "tsmc_nyse_1d":    0.00,   # TSM ADR 4/1 $333.82（相對穩定）
    "tsmc_tw_5d":     -4.8,    # 本週累計（估）
    "taiex_5d":       -2.5,
    "tsmc_tw_1m":     -11.5,
    "taiex_1m":       -5.8,
    "tsmc_tw_ytd":    -12.4,   # Q1+4/1 累計
    "taiex_ytd":      -7.8,
    "sox_ytd":        -22.5,
    "beta_60d":        1.14,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價
    "adr_price":     333.82,   # TSM NYSE（2026-04-01）
    "adr_parity":    279.31,   # 理論值 = 5 × NT$1,780 ÷ 31.85（台股 4/1 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.85,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+19.5",# (333.82 - 279.31) / 279.31 × 100
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 EPS 共識 NT$20.38，前 2 月累計營收年增 30%（NT$7,189 億）。3/31（Q1 季末）NYSE TSM 盤中 $333.82（-4.01%），台股 2330 收 NT$1,790（-1.09%）。今日為 AGM 提案截止日（ADS 持有人）。TSMC 2nm/3nm 訂單已售罄至 2028，2025 全年營收 $1,224 億（+36% YoY）。31 位分析師無一賣出，共識目標 NT$2,290（+28%），分析師共識目標 $430.65 USD（強烈買進）。4/16 Q1 財報（倒計時 16 天）為關鍵催化劑，若毛利率超指引（63–65%）可望確認上行走勢。",
...
NEWS = [
    {
        ...
        "title": "TSMC 先進製程產能訂滿至 2028 年，供不應求格局確立",
        "body": "Dataconomy（3/31）報導，TSMC 2nm 及 3nm 先進製程訂單已全數預訂至 2028 年，NVIDIA、Apple、AMD 及 Qualcomm 均已鎖定產能，N3P/N2 節點處於「全面超載」狀態。此結構性供給緊張賦予台積電強大定價能力，2026 年先進製程有望再次調漲 5–10%，毛利率可望維持 63% 以上，長線護城河持續加深。",
        "source": "Dataconomy 2026-03-31", "impact": "正面",
    },
    {
        "tag": "海外擴廠", "tag_color": "#9C27B0",
        "title": "TSMC 日本熊本二廠獲批 3nm 超高效能 AI 晶片量產",
        "body": "Benzinga 報導，台積電日本熊本第二座晶圓廠（Fab 2）已獲日本政府正式批准，將導入 3nm 先進製程生產超高效能 AI 晶片，預計 2027 年量產。日本政府提供大規模補貼，強化 TSMC 在東北亞的製造布局。此舉不僅分散地緣政治風險，也進一步鞏固 TSMC 的全球代工龍頭地位。",
        "source": "Benzinga 2026-04-01", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA 確認成 TSMC 最大客戶（22%），首採 A16 製程 2027 量產",
        "body": "NVIDIA CEO Jensen Huang 確認，NVIDIA 已超越 Apple 成為台積電最大單一客戶，預估 2026 年貢獻約 $330 億 USD（22%），Apple 以 $270 億（18%）居次。NVIDIA 亦確認首採 TSMC A16（1.6nm BPR 背面供電）製程，Apple 跳過 A16 直升 A14，兩大客戶製程路徑明朗，2nm 量產需求強勁。",
        "source": "CNBC / TweakTown 2026-04-01", "impact": "正面",
    },
    {
        "tag": "先進封裝", "tag_color": "#9C27B0",
        "title": "TSMC 先進封裝營收 2026 年將突破 10%，CoWoS 為核心驅動",
        "body": "分析師報告顯示，TSMC 先進封裝（Advanced Packaging）2025 年佔總收入約 8%，2026 年預計突破 10%，主要由 CoWoS（Chip on Wafer on Substrate）及 SoIC 拉動。CoWoS 月產能目標從 2025 年 22,000 片提升至 2026 年 35,000 片（+59%），NVIDIA GB300/Blackwell 系列為主要需求來源，ABF 基板仍是瓶頸。",
        "source": "TradingView / Seeking Alpha 2026-04-01", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "全球半導體 2026 年衝破 $1 兆，AI 晶片佔逾 50% 收入",
        "body": "DigiTimes（3/19）及 Sourceability 報告預測，2026 年全球半導體市場收入年增逾 30%，總規模突破 $1 兆 USD。AI 相關晶片（GPU、ASIC、HBM）預計貢獻超過 $5,000 億，成為半導體超級周期核心驅動力。HBM 市場從 2024 年 $152 億增長至 2026 年 $326 億（+115%），台積電為最大受益者。",
        "source": "DigiTimes / Sourceability 2026-03-31", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "中國 AI 晶片落後 5–10 年，TSMC 不可替代性持續強化",
        "body": "Tom's Hardware 報導，SEMICON China 2026 上中國晶片業領袖坦承，在 AI 資料中心晶片方面落後國際先進水準 5–10 年，AI 需求正造成設備與人才瓶頸。此格局持續強化 TSMC 的不可替代地位；美中科技管制風險存在，出口限制持續關注，但對台積電短期業務影響估計有限。",
        "source": "Tom's Hardware / DigiTimes 2026-03-30", "impact": "中性",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "1,820",        # 收盤價（數字加逗號，不含 NT$）（2026-03-27 收盤）
    "tw_change":      "-20",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-1.09%",       # 漲跌幅（含+/-號）
    "tw_volume":      "29,000",       # 成交量（張，估，約 2900 萬股）
    "tw_open":        "1,835",
    "tw_high":        "1,825",
    "tw_low":         "1,800",
    "market_cap":     "NT$47.20 兆",
    "pe_ratio":       "~27.42x",      # 台股 P/E（TTM，2026-03-27）
    "pb_ratio":       "~10.1x",       # 台股 P/B（股淨比）
    "div_yield":      "~1.13%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "326.74",       # 實際收盤（2026-03-27）
    "nyse_change":    "-21.01",       # 較 3/26 收盤 $347.75 下跌
    "nyse_change_pct":"-6.05%",
    "52w_high_nyse":  "390.20",       # 52 週高（2026-02-25）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~30.2x",       # NYSE USD 基礎 P/E（TTM）
    "pb_nyse":        "~9.8x",
    "beta":           "1.28",
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-27 收盤（NT$1,820，-1.09%）  |  NYSE 數據：2026-03-27 收盤 $326.74（-6.05%）  |  本週 TSM 合計大跌，年初至今漲幅收窄  |  高盛目標價 $370–$375 USD（強烈買入）",
}
...
SUMMARY = {
    ...
    "sentiment":  "看空",
    # 一句話標題
    "headline":   "NYSE TSM 3/27 收盤 $326.74（-6.05%），台股 2330 收 NT$1,820（-1.09%）；全球半導體股受壓，4/16 Q1 財報成關鍵支撐",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "NYSE TSM 3/27 收盤 $326.74（-$21.01，-6.05%），單日重挫，半導體類股整體承壓，全球 AI 股回調",
        "台股 2330 收 NT$1,820（-20，-1.09%），成交量 2.9 萬張，跌破 MA60 支撐，技術面轉弱訊號",
        "NVIDIA 確認為 TSMC A16（1.6nm）首位客戶，2026 年貢獻預估達 $330 億（22%），超越 Apple 成最大客戶",
        "Broadcom 警告 TSMC 先進製程產能逼近飽和（3/24），顯示 AI 需求長期結構性旺盛，產能供不應求",
        "全球半導體市場 2026 年預計突破 $1 兆 USD，AI 晶片佔超過 50%；ARM 選 TSMC 3nm 獨家代工 AI 伺服器 CPU",
    ],
    # 一句結論
    "bottom_line": "NYSE TSM 3/27 大跌 6.05% 至 $326.74，台股 2330 同步下跌至 NT$1,820（-1.09%），短線承壓明顯，跌破前低支撐。惟長線基本面無虞：NVIDIA 首採 A16、ARM 3nm 獨家代工、全球半導體市場突破兆元等利多支撐。4/16 Q1 財報（EPS 共識 NT$20.38）為近期最重要催化劑；AGM 提案截止 3/31–4/7。分析師 31 人全數買入、目標 NT$2,290，短線低點視為長線佈局機會，支撐 1,800–1,810，壓力 1,840–1,860。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ NYSE TSM 單日大跌 6.05%（3/27）；台股 2330 跌破 MA60 支撐；AGM 提案截止日 3/31–4/7；Q1 財報 4/16 倒計時 17 天；注意全球科技股回調風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-27",
    "foreign":  -3500,    # 外資（正=買超）— 3/27 NYSE 大跌，外資賣超（估）
    "trust":    +280,     # 投信（持續逢低承接）
    "dealer":   -200,     # 自營商（隨大跌減碼）
    # 近5日（由舊到新）
    "foreign_5d": [-4100, +2800, -1200, -3500, -2000],  # 3/28 估外資持續賣超
    "trust_5d":   [+280,  +320,  +250,  +280,  +260],
    "dealer_5d":  [-220,  +150,  -100,  -200,  -150],
    "foreign_ownership_pct": "74.8%",  # 外資持股比例（估，3/27 明顯降）
    "foreign_ownership_shares": "約 194.3 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       38.2,   "rsi_signal": "偏弱（RSI 跌破 40，NYSE 大跌拖累，超賣邊緣）",
    "macd":         -8.5,   "macd_signal": "MACD 死叉擴大，動能偏空，量能不足難以反轉",   "macd_color": "#F44336",
    "kdj_k":        32.5,   "kdj_d": 35.8, "kdj_j": 25.9, "kdj_signal": "KDJ 死叉深化，超賣區間，需等待量能回升",
    "ma5":       "1,832",
    "ma20":      "1,878",
    "ma60":      "1,838",
    "ma120":     "1,738",
    "bb_upper":  "1,998",   # 布林上軌
    "bb_mid":    "1,878",   # 布林中軌（20MA）
    "bb_lower":  "1,758",   # 布林下軌
    "vol_ma5":  "23,800",   # 5日均量（張）
    "trend":    "台股 2330（3/27 收 NT$1,820，-1.09%）跌破 MA60（1,838），技術面轉弱。NYSE TSM（3/27 收）$326.74（-6.05%），單日大跌創近期最大跌幅。RSI 跌至 38.2（逼近超賣），MACD 死叉加深，支撐區下移至 1,800–1,810。AGM 提案截止（3/31–4/7）及財報（4/16）為近期催化劑。",
    "support":  "1,800–1,810",
    "resist":   "1,840–1,860",
...
DERIVATIVES = {
    "date":          "2026-03-27",
    "futures_price": "1,818",      # 台積電期貨近月（估，略低現貨 1,820，負價差擴大）
    "futures_oi":    "76,200",     # 未平倉量（口）（估，大跌後空單增加）
    "futures_basis": "-2",         # 負價差（空頭主導）
    "call_oi":       "31,200",
    "put_oi":        "56,800",
    "pcr":           "1.82",       # Put/Call 比率大幅上升（空頭情緒明顯增加）
    "pcr_signal":    "偏空",
    "max_pain":      "1,840",      # 最大痛苦價格（下移）
    "key_strikes": [
        ("1,760", "Call 2,500 / Put 11,000", "布林下軌支撐"),
        ("1,800", "Call 4,200 / Put 10,500", "短線重要支撐"),
        ("1,820", "Call 5,500 / Put 8,200", "現貨所在"),
        ("1,840", "Call 8,500 / Put 5,200", "最大痛苦點/壓力"),
        ("1,860", "Call 10,800 / Put 3,800", "強壓力區"),
    ],
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,050",   "-3.3%",  False),  # 3/27 收盤，半導體股大跌
    ("美元指數 DXY",    "104.5",   "+0.8%",  False),  # 美元強勢，台幣持續承壓
    ("VIX 恐慌指數",    "28.5",    "+3.2",   False),  # 恐慌指數顯著上升
    ("WTI 原油",        "$68.8",   "-2.5%",  False),
    ("10Y 美債殖利率",  "4.32%",   "+0.10pp",False),  # 殖利率上升
    ("台幣/美元",       "31.75",   "-0.18",  False),  # 台幣貶值
    ("NVIDIA NVDA",     "$840",    "-3.5%",  False),  # 大跌跟跌
    ("ASML",            "$668",    "-2.2%",  False),  # 設備股同步下跌
]
...
RELATIVE = {
    "date":           "2026-03-27",
    "tsmc_tw_1d":     -1.09,   # 2330 3/27 收盤 1,820（-1.09%）
    "taiex_1d":       -0.80,   # 加權指數（3/27 估算）
    "sox_1d":         -3.30,   # SOX（3/27 收盤，大跌）
    "tsmc_nyse_1d":   -6.05,   # TSM ADR 3/27 收盤 $326.74（-6.05%）
    "tsmc_tw_5d":     -6.8,    # 本週累計（估）
    "taiex_5d":       -2.8,
    "tsmc_tw_1m":     -9.5,
    "taiex_1m":       -4.8,
    "tsmc_tw_ytd":    -9.6,
    ...
    # ADR 折溢價
    "adr_price":     326.74,   # TSM NYSE 收盤（2026-03-27）
    "adr_parity":    286.73,   # 理論值 = 5 × NT$1,820 ÷ 31.75（台股 3/27 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.75,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+13.9",# (326.74 - 286.73) / 286.73 × 100

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,200",        # 台股 2330 最新確認收盤：7/29 Wed 官方收 NT$2,200（-80、-3.51% vs 7/28 NT$2,280、開 2,260／高 2,280／低 2,180、TWSE 確認）——亞股股災第 2 日、盤中殺破 2,200 觸 2,180 後尾盤拉回；兩日累計 -150 點（-6.4%）、連 2 日收在布林下軌之下；大盤同日再挫 -1,564.18 點（-3.76%）收 40,039.18、貼近 4 萬心理關口
    "tw_change":      "-80",          # 7/29 Wed 收盤漲跌點數（-3.51%、TWSE 官方）
    "tw_change_pct":  "-3.51%",       # 7/29 Wed 收盤漲跌幅（小低開 2,260 → 13:00 後恐慌殺盤觸 2,180（-4.4%）→ 尾盤拉回收 2,200；本日跌幅與大盤 -3.76% 相當、前日「相對抗跌」角色弱化）
    "tw_volume":      "68,140",       # 7/29 成交量（張、TWSE 口徑；成交額 NT$1,515.52 億、成交 675,528 筆——量增 50%（前日 45,333 張）、達 5 日均量 1.75 倍、為 7/17 恐慌量 97,362 張的 70%——恐慌拋售升級）
    "tw_open":        "2,260",        # 7/29 開盤（TWSE、小低開 -20）
    "tw_high":        "2,280",        # 7/29 盤中高（TWSE、開盤區即全日高點、一路壓回）
    "tw_low":         "2,180",        # 7/29 盤中低（TWSE、午後恐慌殺盤、尾盤收復 2,200）
    "market_cap":     "NT$57.0 兆",   # 7/29 收盤市值（NT$2,200 基礎、約 259.3 億股）
    "pe_ratio":       "29.58x",       # 台股 P/E（TWSE 官方 BWIBBU 7/29、口徑為最新年度 EPS）
    "pb_ratio":       "9.68x",        # 台股 P/B（TWSE 官方 BWIBBU 7/29、以 115Q1 財報淨值計）
    "div_yield":      "1.00%",        # 殖利率（TWSE 官方 BWIBBU 7/29、以 114 年度股利計；跌深使殖利率回升至 1.00%）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "374.67",       # 7/29 Wed 官方收盤 $374.67（-$17.64、-4.50% vs 7/28 $392.31、Yahoo Finance API 確認）——美股由「分化」轉「全面補跌」；SOX 同日 -5.33% 收 10,447.49（自 6/22 史高 14,634.66 收斂至 -28.6%）
    "nyse_change":    "-17.64",       # 7/29 漲跌（自 7/28 $392.31）
    "nyse_change_pct":"-4.50%",       # 7/29 美股全面下殺：KLAC -10.80%／MU -9.94%／AMAT -8.40%／ARM -8.11%／AMKR -6.48%／LRCX -6.40%／AMD -5.51%／INTC -5.12%／NVDA -3.55% 翻黑；AAPL -0.56% 相對抗跌；VIX +13.5% 升破 20 至 20.66、S&P 500 -1.52%——前日分化格局終結
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~27.4x",       # NYSE USD 基礎 P/E（TTM 估，$374.67 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~16.0x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-30 Thu（最新確認收盤：台股 2330 7/29 Wed 官方收 NT$2,200（-80、-3.51% vs 7/28 NT$2,280、開 2,260／高 2,280／低 2,180、量 68,140 張 TWSE 口徑、成交額 NT$1,515.52 億、成交 675,528 筆、TWSE 確認）；NYSE TSM 7/29 Wed 收 $374.67（-$17.64、-4.50% vs 7/28 $392.31、Yahoo Finance API 確認））。📊 今日格局：股災第 2 日——台股續挫、美股由分化轉全面補跌，惟盤後 MSFT 財報大超預期投下第一根修復錨——（1）⚠️ 台股 7/29 續挫：大盤 -1,564.18 點（-3.76%）收 40,039.18（兩日累計約 -3,595 點、-8.2%）、貼近 4 萬心理關口；2330 小低開 2,260 後全日走弱、13:00 後恐慌殺盤觸 2,180（-4.4%）、尾盤拉回收 2,200（-3.51%）——本日跌幅與大盤相當、前日「相對抗跌」角色弱化；量增 50% 至 68,140 張（5 日均量 1.75 倍、為 7/17 恐慌量 97,362 張的 70%）；供應鏈續殺：日月光 -9.93% 近跌停、華邦電 -9.72%、聯電 -9.69%、聯發科 -4.98%（跌停後收斂）；（2）📊 籌碼賣壓明顯收斂：外資對 2330 賣超 -11,211 張（連 6 賣、自 -14,659 收斂 24%）、投信 +1,652 張連 4 買且大幅放大（284 → 1,652、逢低承接升級）、自營 +4,893 連 5 買（續大額低接）、三大合計 -4,665 張（較前日 -9,252 減半）；全市場 BFI82U 三大法人賣超 -351.5 億（外資 -222.5 億、自營 -186.0 億、投信 +57.1 億）——自 7/28 -1,176 億大幅收斂 70%；（3）⚠️ 美股 7/29 全面補跌：TSM -4.50% 收 $374.67、SOX -5.33% 收 10,447.49（自 6/22 史高 14,634.66 收斂至 -28.6%）——設備/記憶體殺戮續深（KLAC -10.80%、MU -9.94%、AMAT -8.40%、ARM -8.11%、LRCX -6.40%）、AMD -5.51%、INTC -5.12%、NVDA -3.55% 翻黑失守 $200 後續探、AVGO -2.78%、ASML -2.04%（相對抗跌——中國 DUV 敘事已提前反映）；AAPL 僅 -0.56%（今晚財報前資金避風）；VIX +13.5% 升破 20 至 20.66、S&P 500 -1.52%——前日「NVDA/AAPL 翻紅、VIX 反降」的分化格局終結、美股正式加入調整；（4）📅 FOMC 7/29 鷹派按兵不動：聯準會維持利率 3.50%–3.75% 不變（連 5 次會議）、惟投票 9-3 分歧——Hammack／Kashkari／Logan 三票主張升息 25bp；聲明點名中東衝突不確定性、資本投資與生產力強勁；主席 Warsh 會後未鬆口、官員年底利率預測 3.6%–4.1%——鷹派傾向對高估值 AI 資產不利；（5）⭐ 盤後財報「第一根錨」：MSFT FY26Q4 營收 $90.01B 大超預期（est $87.62B）、Azure 成長 +43%（固定匯率、大幅超 est +40.2%）、FY2026 Azure 全年營收首破 $1,000 億美元——盤後大漲 +8.83% 收 $425.01（正規盤收 $390.54、-0.71%；財報剛公布時僅約 +3%、電話會後漲幅持續擴大至盤後收盤）；META 營收 +28% YoY 超預期約 1% 惟 EPS 低於預期約 14%（YoY -13%）、2026 CapEx 指引 $125–145B 續擴——盤後 -7.45% 收 $542.00（正規盤收 $585.61；財報剛公布時一度殺至 -9.64%、電話會後收斂）——兩樣本共同訊息：hyperscaler AI CapEx 並未收縮（MSFT 下年度 CapEx 市場估 ~$119B）、對 TSMC 先進製程訂單能見度為正面，惟市場對「燒錢 vs 回報」的耐性開始分層（會燒且能證明回報者漲、只會燒者跌）；（6）📊 韓股 7/29 重挫、7/30 今晨首見止穩：SK 海力士 7/29 財報全面創史高仍重挫 -9.61% 收 140.1 萬韓元、三星 -5.23% 收 20.85 萬；KOSPI 7/29 官方收 5,663.24（-5.98%、前收 6,023.66、當日連 2 度觸發熔斷——已補查證，原「未能證實」標記解除）；今晨（7/30）轉為劇烈震盪後回穩：盤中 5,665.17（+0.03%、約當持平）、日內區間 5,547.41–5,781.42（振幅 4.2%）——槓桿去化第 3 日首見止穩跡象，惟波動仍極高、尚不足稱底；（7）⭐ 油價大幅反彈：WTI ~$84.2（vs 7/28 $79.26、FOMC 聲明點名中東衝突）——通膨與鷹派風險疊加。📊 台美 ADR 溢價收斂至 +10.2%（$374.67 vs 理論值 ~$339.9、匯率 32.36）。技術面（官方收盤計算至 7/29）：收 2,200 連 2 日收在布林下軌（2,248）之下——RSI(14) 35.6 逼近 30 超賣線、MACD 綠柱續急擴至 -47.8 且 DIF -22.2 深陷零軸下（中期空方全面主導）、KDJ 死叉第 4 日且 J 值 -9.3 轉負（極度超賣、歷史罕見區）；支撐 2,180（7/29 低）／2,134（120MA 半年線）／2,100（心理整數）、壓力 2,248（布林下軌）／2,260（7/29 開）／2,280（7/29 高）／2,317（5MA）／2,346（60MA）。✅ 基本面錨完全未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋2027 全面漲價 5-10% 傳聞，且 MSFT/META 財報再證 AI CapEx 未收縮；本輪為亞洲槓桿去化＋中國供給端敘事之系統性殺盤、非台積電個別利空。📈 今晨（7/30）盤中即時（09:26、TWSE MIS／Investing.com）：亞股同步回穩——台股大盤 40,235.08（+195.90、+0.49%、日內區間 39,404.65–40,452.01）、2330 開 2,205／高 2,240／低 2,190 於 2,200 上下震盤（前收 2,200、早盤量 12,427 張）；KOSPI 5,665.17（+0.03%）、日經 225 盤中約 +1.59%——MSFT 盤後 +8.83% 的修復錨效應初步傳導至亞股、恐慌情緒緩解，惟日內振幅仍大（台股一度探 39,404 失守 4 萬後拉回），性質為極度超賣後之技術性回穩、非趨勢反轉（盤中數據、隨時變動、收盤前不作結論）。⭐ 今日（7/30）觀察：（1）2330 能否守 2,180／收復布林下軌 2,248；（2）MSFT 財報效應能否穩住美股 AI 權值、今晚（美東 7/30）AAPL/AMZN 財報接棒；（3）外資賣超收斂能否延續、投信放大買超後續；（4）韓股 KOSPI／SK 海力士止跌訊號；（5）美 232 條款半導體調查 7 月底出爐（今明兩日窗口）、台幣 32.36。",
}
...
SUMMARY = {
    ...
    "sentiment":  "看空",
    # 一句話標題
    "headline":   "⚠️ 股災第 2 日：台股 7/29 再挫 -1,564.18 點（-3.76%）收 40,039.18、2330 收 NT$2,200（-80、-3.51%）盤中觸 2,180——兩日累跌 -6.4%、KDJ J 值 -9.3 轉負極度超賣；美股由分化轉全面補跌：TSM -4.50% 收 $374.67、SOX -5.33% 自史高 -28.6%、VIX 升破 20；FOMC 鷹派按兵不動（9-3、3 票主張升息）；⭐ 盤後 MSFT 財報大超預期（營收 $90.01B、Azure +43%、盤後 +8.83%）投下第一根修復錨、META EPS miss 盤後 -7.45%——AI CapEx 未收縮、對 TSMC 訂單能見度正面；✅ 籌碼賣壓收斂：外資 -11,211 張（自 -14,659 收斂）、投信 +1,652 放大 6 倍、BFI82U 賣超收斂 70%；今晚 AAPL/AMZN 財報接棒",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⚠️ 台股 7/29 續挫：大盤 -1,564.18 點（-3.76%）收 40,039.18、兩日累計約 -3,595 點貼近 4 萬關口；2330 官方收 NT$2,200（-80、-3.51%、開 2,260／高 2,280／低 2,180、量 68,140 張）——13:00 後恐慌殺盤觸 2,180、尾盤拉回；量達 5 日均量 1.75 倍、恐慌拋售升級；供應鏈續殺：日月光 -9.93%、華邦電 -9.72%、聯電 -9.69%",
        "✅ 籌碼賣壓明顯收斂：外資對 2330 賣超 -11,211 張（連 6 賣、自 -14,659 收斂 24%）、投信 +1,652 張連 4 買且放大 6 倍（284 → 1,652）、自營 +4,893 連 5 買續低接、三大合計 -4,665 張較前日減半；全市場 BFI82U 賣超 -351.5 億（外資 -222.5 億）——自 -1,176 億收斂 70%",
        "⚠️ 美股 7/29 由分化轉全面補跌：TSM -4.50% 收 $374.67、SOX -5.33% 收 10,447.49（自 6/22 史高 -28.6%）——KLAC -10.80%、MU -9.94%、AMAT -8.40%、AMD -5.51%、NVDA -3.55% 翻黑；AAPL 僅 -0.56% 抗跌；VIX +13.5% 升破 20 至 20.66——前日「NVDA/AAPL 翻紅」分化格局終結",
        "📅 FOMC 7/29 鷹派按兵不動：利率維持 3.50%–3.75%（連 5 次）、投票 9-3 分歧——三票（Hammack/Kashkari/Logan）主張升息 25bp、聲明點名中東衝突與強勁資本投資；主席 Warsh 未鬆口——鷹派傾向對高估值 AI 資產構成頂部壓力；WTI 同步反彈至 ~$84.2 疊加通膨風險",
        "⭐ 盤後財報第一根錨：MSFT 營收 $90.01B 大超預期、Azure +43%（固定匯率）、FY26 Azure 首破 $1,000 億——盤後 +8.83%；META 營收 +28% 超預期惟 EPS miss ~14%、2026 CapEx $125–145B 續擴——盤後 -7.45%；共同訊息：AI CapEx 未收縮、對 TSMC 先進製程訂單能見度正面，市場對「燒錢 vs 回報」開始分層計價",
        "📊 韓股 7/29 重挫、7/30 今晨首見止穩：SK 海力士 7/29 財報全面創史高（營益 60.54 兆韓元、+557% YoY）仍重挫 -9.61%、三星 -5.23%；KOSPI 7/29 官方收 5,663.24（-5.98%、連 2 度熔斷、已補查證）；今晨（7/30）盤中 5,665.17 約當持平、日內區間 5,547–5,781 劇震——首見止穩、惟波動仍極高",
        "💡 技術面：RSI 35.6 逼近超賣、MACD 綠柱 -47.8 續急擴（DIF -22.2 深陷零軸下）、KDJ J 值 -9.3 轉負（極度超賣、歷史罕見）——短線技術性反彈一觸即發、惟需收復布林下軌 2,248 確認；支撐 2,180／2,134（120MA）／2,100",
        "📊 今日焦點：今晚（美東 7/30）AAPL/AMZN 財報接棒 AI CapEx 驗證；美 232 條款半導體調查 7 月底出爐（今明兩日窗口）、台美關稅談判傳由 20% 下調至 ~15%",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-30 Thu（最新確認收盤：台股 2330 7/29 Wed 官方收 NT$2,200（-80、-3.51% vs 7/28 NT$2,280、開 2,260／高 2,280／低 2,180、量 68,140 張 TWSE 口徑、成交額 NT$1,515.52 億、成交 675,528 筆、TWSE 確認）；NYSE TSM 7/29 收 $374.67（-$17.64、-4.50%、Yahoo API 確認））。📊 核心判讀：股災第 2 日、殺盤重心自「亞洲槓桿」轉「美股補跌」，惟籌碼與財報出現首批修復訊號——（1）台股 7/29 大盤 -3.76% 收 40,039.18 貼近 4 萬關口；2330 收 2,200（-3.51%）盤中觸 2,180、跌幅與大盤相當——前日抗跌角色弱化、量增 50% 至 68,140 張（5 日均量 1.75 倍）恐慌拋售升級，惟尾盤自 2,180 收復 2,200 顯示低接買盤存在；（2）✅ 籌碼面為今日最重要正向邊際變化：外資賣超自 -14,659 收斂至 -11,211 張（-24%）、投信買超放大 6 倍至 +1,652 張（連 4 買）、自營 +4,893 連 5 買、三大合計賣超較前日減半；全市場 BFI82U 賣超 -351.5 億、自 -1,176 億收斂 70%——恐慌賣壓高峰或已過；（3）⚠️ 美股 7/29 由「分化」轉「全面補跌」：TSM -4.50%、SOX -5.33% 自史高 -28.6%，設備/記憶體殺戮續深（KLAC -10.80%、MU -9.94%、AMAT -8.40%）、NVDA -3.55% 翻黑、VIX 升破 20 至 20.66——美股正式加入調整、系統性風險層級上升一級；（4）📅 FOMC 鷹派按兵不動（3.50–3.75%、連 5 次）且 9-3 分歧（3 票主張升息）、WTI 反彈 ~$84.2——利率與通膨雙重逆風對高估值 AI 資產不利；（5）⭐ 盤後 MSFT/META 財報為本輪股災首個基本面驗證點：MSFT 營收 $90.01B 大超預期、Azure +43%、盤後 +8.83%——「AI 燒錢有回報」樣本；META EPS miss ~14%、CapEx $125–145B 續擴、盤後 -7.45%——「燒錢待驗證」樣本；hyperscaler CapEx 未收縮對 TSMC 訂單能見度為直接正面、今晚 AAPL/AMZN 接棒；（6）⚠️ 韓股槓桿去化：7/29 SK 海力士財報創史高仍重挫 -9.61%、三星 -5.23%、KOSPI 官方收 5,663.24（-5.98%、連 2 度熔斷）——「基本面救不了籌碼」的極端市況；惟今晨（7/30）KOSPI 轉為劇震後回穩（盤中 5,665.17、+0.03%、區間 5,547–5,781）、首見止穩跡象（SK／三星 7/30 個股當日漲跌未能證實、不予引用）。技術面：RSI 35.6、MACD 綠柱 -47.8 急擴（DIF -22.2）、KDJ J -9.3 轉負極度超賣——短線技術性反彈條件已極度充分、惟中期空方架構確立，收復布林下軌 2,248 → 2,280 → 2,317（5MA）為修復三關卡、失守 2,180 下看 2,134（120MA 半年線）／2,100。📊 ADR 溢價 +10.2%（匯率 32.36）——續提供台股緩衝。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋2027 漲價傳聞、MSFT/META 再證 AI CapEx 未收縮；本輪為系統性殺盤、非台積電個別利空——估值已回落至 P/E 29.58x（BWIBBU 官方）。⭐ 今日（7/30）觀察：（1）2330 守 2,180／收復 2,248；（2）MSFT 效應與今晚 AAPL/AMZN 財報；（3）外資賣超收斂延續性；（4）韓股止跌訊號；（5）232 條款結果與台美關稅談判。⚠️ 風險：KOSPI 槓桿去化未止、SOX 自史高 -28.6% 續深、FOMC 鷹派＋油價反彈、232 條款結果不確定、台幣 32.36（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/30 Thu）關鍵觀察：（1）⚠️ 美股補跌後的亞股第 3 日：美股 7/29 已由分化轉全面下跌（SOX -5.33%、VIX 升破 20）、今晨韓股轉為劇震後回穩（7/29 官方收 5,663.24／-5.98%；7/30 盤中 5,665.17 約持平、區間 5,547–5,781）——2330 開盤承接美股與韓股雙重壓力、守 2,180（7/29 低）為第一檢核，失守下看 2,134（120MA 半年線）／2,100；（2）⭐ MSFT 財報為修復錨：營收 $90.01B 大超預期、Azure +43%、盤後 +8.83%——若今晚 AAPL/AMZN 財報（美東 7/30）接棒證明 AI CapEx 與獲利雙軌成立、本輪「AI 回報質疑」殺盤的基本面反駁鏈完成；META EPS miss 盤後 -7.45% 顯示市場已開始分層計價；（3）✅ 籌碼修復訊號：外資賣超收斂 24% 至 -11,211 張、投信買超放大 6 倍至 +1,652 張、BFI82U 全市場賣超收斂 70% 至 -351.5 億——恐慌賣壓高峰或已過、後續兩日收斂延續性為關鍵；（4）📅 FOMC 鷹派按兵不動（9-3、3 票主張升息、Warsh 未鬆口）＋WTI 反彈 ~$84.2——利率敘事自「等降息」轉「防升息」、高估值資產折現率風險上升；（5）📊 技術面極度超賣（KDJ J -9.3 轉負、RSI 35.6）——技術性反彈一觸即發、惟屬「跌深反彈」性質，收復布林下軌 2,248 前不改中期空方架構；（6）💱 美 232 條款半導體調查 7 月底出爐（今明兩日窗口、台美談判傳關稅 20%→~15%）——政策面為潛在情緒修復催化",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-29",       # 最新資料日（7/29 Wed、TWSE T86 官方）
    "foreign":  -11211,            # 外資 7/29 賣超 11,211 張（連 6 賣、自 -14,659 收斂 24%——恐慌高峰賣壓緩解）
    "trust":    1652,              # 投信 7/29 買超 1,652 張（連 4 買且大幅放大 6 倍：284 → 1,652——逢低承接升級）
    "dealer":   4893,              # 自營商 7/29 買超 4,893 張（連 5 買：5,123 → 4,893——續大額低接）；三大法人合計 -4,665 張（較前日 -9,252 減半）
    # 近5日（由舊到新：7/23, 7/24, 7/27, 7/28, 7/29）；TWSE T86 口徑（張）
    "foreign_5d": [-4614, -9637, -2662, -14659, -11211], # 外資（連 6 賣、7/29 收斂）
    "trust_5d":   [-943, 550, 567, 284, 1652],           # 投信（7/24 起連 4 買、7/29 大幅放大）
    "dealer_5d":  [38, 305, 1410, 5123, 4893],           # 自營（連 5 買、續大額低接）
    "foreign_ownership_pct": "~74.4%",  # 外資持股比例（估；官方以 TWSE 外資持股餘額為準）
    "foreign_ownership_shares": "約 193 億股（估）",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       35.6,   "rsi_signal": "RSI(14) 35.6（官方收盤計算）——自 40.8 續降、逼近 30 超賣線；連續重挫後動能指標全面走弱、與 KDJ 極度超賣共振",
    "macd":         -47.8, "macd_signal": "MACD 綠柱續急擴至 -47.8（DIF -22.2 < DEA 1.7、官方收盤計算）——⚠️ DIF 自 -9.3 再深陷至 -22.2、DEA 即將翻負——中期空方動能全面主導；綠柱收斂為修復前提",   "macd_color": "#F44336",
    "kdj_k":        18.9,   "kdj_d": 33.1, "kdj_j": -9.3, "kdj_signal": "KDJ K(18.9)/D(33.1)/J(-9.3)（官方收盤計算）——死叉第 4 日、J 值 -9.3 轉負（極度超賣、歷史罕見區）——短線技術性反彈一觸即發、惟需收復布林下軌 2,248 確認",
    "ma5":       "2,317",   # 5MA（官方收盤計算；7/29 收 2,200 遠低於其下、短均全數蓋頭下彎）
    "ma20":      "2,398",   # 20MA（官方收盤計算；與布林中軌重合、中期修復目標）
    "ma60":      "2,346",  # 60MA（官方收盤計算；季線失守第 2 日、中期多空架構轉空確立）
    "ma120":     "2,134",  # 120MA（官方收盤計算至 7/29；半年線為下一長期支撐帶）
    "bb_upper":  "2,549",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,398",   # 布林中軌（20MA）
    "bb_lower":  "2,248",  # 布林下軌（20MA-2σ、官方收盤計算；7/29 收 2,200 連 2 日收在下軌之下——持續超跌、收復為止穩第一訊號）
    "vol_ma5":  "39,045",   # 5日均量（張、TWSE 口徑；7/29 量 68,140 達均量 1.75 倍、為 7/17 恐慌量 97,362 的 70%——恐慌拋售升級）
    "trend":    "⚠️ 股災第 2 日、抗跌角色弱化：台股 2330 7/29 官方收 NT$2,200（-80、-3.51%、開 2,260／高 2,280／低 2,180、量 68,140 張）——13:00 後恐慌殺盤觸 2,180、尾盤拉回收 2,200；大盤同日 -1,564.18 點（-3.76%）收 40,039.18 貼近 4 萬關口。📊 技術指標（官方收盤計算）：KDJ 死叉第 4 日且 J 值 -9.3 轉負（極度超賣、歷史罕見）、RSI(14) 35.6 逼近超賣線、MACD 綠柱急擴 -47.8 且 DIF -22.2 深陷零軸下——中期空方全面主導、短線極度超跌並存。✅ 籌碼修復訊號：外資賣超收斂 24% 至 -11,211 張、投信買超放大 6 倍至 +1,652 張、自營 +4,893 連 5 買、BFI82U 全市場賣超收斂 70% 至 -351.5 億。⚠️ 美股 7/29 由分化轉全面補跌：TSM -4.50% 收 $374.67、SOX -5.33% 自史高 -28.6%、VIX 升破 20——惟盤後 MSFT 財報大超預期（Azure +43%、盤後 +8.83%）投下修復錨。下一觀察：（1）守 2,180（7/29 低）、失守下看 2,134（120MA）／2,100；（2）收復布林下軌 2,248 → 2,280 → 2,317（5MA）為修復三關卡；（3）外資賣超收斂延續＋今晚 AAPL/AMZN 財報。",
    "support":  "2,180 / 2,134 / 2,100 (7/29 低、120MA 半年線、心理整數)",
    "resist":   "2,248 / 2,260 / 2,280 / 2,317 / 2,346 (布林下軌、7/29 開、7/29 高＋前日收、5MA、60MA)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-29 止），請以看盤軟體為準；7/29 官方收 NT$2,200（-3.51%）連 2 日收在布林下軌 2,248 之下；⚠️ KDJ 死叉第 4 日且 J -9.3 轉負（K18.9/D33.1）、RSI 35.6、MACD 綠柱 -47.8 急擴且 DIF -22.2 深陷零軸下；支撐 2,180 / 2,134 / 2,100、壓力 2,248 / 2,260 / 2,280 / 2,317 / 2,346；✅ 外資賣超收斂 24%＋投信買超放大 6 倍＋KDJ 極度超賣＋盤後 MSFT 財報大超預期 vs ⚠️ 美股轉全面補跌＋VIX 破 20＋FOMC 鷹派——守 2,180 與收復 2,248 為今日多空雙檢核",
}
...
DERIVATIVES = {
    "date":          "2026-07-29",       # 最新資料日（7/29 Wed 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,192",            # 台積電期貨近月（估、現貨 7/29 收 NT$2,200 續重挫、貼水維持）
    "futures_oi":    "192,000",          # 未平倉量（口）（估、連續崩跌日避險與投機部位同增）
    "futures_basis": "-8",               # 估逆價差（連續重挫日貼水維持高檔）
    "call_oi":       "118,000",
    "put_oi":        "116,000",
    "pcr":           "0.98",             # Put/Call 比率（估、較 7/28 的 0.95 續升——美股補跌＋台股連 2 日重挫、避險 Put 需求續增）
    "pcr_signal":    "看空（Put/Call 估 0.98；7/29 現貨再挫 -3.51% 連 2 日收在布林下軌下，惟 KDJ J 值轉負極度超賣＋外資賣超收斂＋盤後 MSFT 財報大超預期——避險需求與超跌反彈需求並存；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,250",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,100", "Call 52,000 / Put 16,000", "心理整數、極端支撐"),
        ("2,134", "Call 50,000 / Put 15,000", "120MA 半年線、長期支撐帶"),
        ("2,180", "Call 46,000 / Put 13,000", "7/29 低、當日防線"),
        ("2,200", "Call 44,000 / Put 12,000", "7/29 收盤、心理整數"),
        ("2,248", "Call 40,000 / Put 10,000", "布林下軌、止穩第一關"),
        ("2,260", "Call 38,000 / Put 9,000",  "7/29 開盤、修復第二關"),
        ("2,280", "Call 36,000 / Put 8,500",  "7/29 高＋前日收盤"),
        ("2,317", "Call 33,000 / Put 7,000",  "5MA、短線壓力"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/29 NT$2,200）；NT$2,250 為估最大痛苦、下方支撐 2,180（7/29 低）/ 2,134（120MA）/ 2,100、上方壓力 2,248（布林下軌）/ 2,260 / 2,280 / 2,317（5MA）；⚠️ 美股轉全面補跌＋VIX 破 20——避險需求續增 vs ✅ KDJ 極度超賣＋外資賣超收斂＋MSFT 財報錨；✅ 基本面錨未變（2nm Q3 放量、2027 漲價傳聞、全年上修 >40%、CapEx $60-64B）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-29 Wed TWSE 官方收盤；美股為 7/29 Wed 收盤、Yahoo Finance API 確認；韓股為 7/29 收盤、7/30 盤中續弱）
    ("設備", "ASML",    "艾司摩爾",    "1,550.69", "-2.04%", "✅ EUV 設備龍頭；7/29 僅 -2.04% 相對抗跌（vs 美系設備 -6~-11%）——中國 DUV 自主化敘事已提前反映；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "436.45", "-8.40%", "📉 連 4 日重挫（7/29 -8.40%、兩日 -15.6%）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "252.35", "-6.40%", "📉 蝕刻設備連 4 日回落（7/29 -6.40%）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "170.19", "-10.80%", "⚠️ 7/29 重挫 -10.80% 為美系設備股最重；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","130.00","-9.72%", "⚠️ 連 2 日近跌停（7/29 收 130.0、TWSE 官方——韓股記憶體股災續外溢）；與 TSMC 合作 CUBE 特殊 DRAM 題材未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,401,000 KRW", "-9.61%", "⚠️ 7/29 Q2 財報全面創史高（營收 79.32 兆韓元 +257%、營益 60.54 兆韓元 +557% YoY、HBM4 已量產出貨）仍重挫 -9.61% 收 140.1 萬韓元——「創高財報救不了股價」、槓桿去化主導；7/30 韓股大盤轉為劇震後回穩（KOSPI 盤中 +0.03%），惟 SK 海力士當日個股漲跌未能證實、不予引用"),
    ("記憶", "MU",      "美光",        "739.00", "-9.94%", "📉 HBM / DDR5；7/29 再挫 -9.94% 收 $739.00（自 7/23 高 $990 累計 -25.4%）——記憶體殺戮最深、高位去槓桿"),
    ("封裝", "3711.TW", "日月光",      "499.00", "-9.93%", "⚠️ 7/29 近跌停收 499（TWSE 官方、兩日 -17.9%）；CoWoS / 先進封裝"),
    ("封裝", "AMKR",    "Amkor",       "42.73",  "-6.48%", "⚠️ 7/28 崩跌 -24.74% 後 7/29 續挫 -6.48% 收 $42.73；TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產題材未變"),
    ("客戶", "NVDA",    "輝達",        "190.01", "-3.55%", "📉 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/29 翻黑 -3.55% 收 $190.01 續探——循環融資疑慮未散、惟跌幅小於 SOX -5.33%"),
    ("客戶", "AAPL",    "蘋果",        "338.19", "-0.56%", "✅ #2 客戶 18%；7/29 僅 -0.56% 收 $338.19 相對抗跌——今晚（美東 7/30）財報、資金避風港；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "370.32", "-2.78%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/29 -2.78% 相對抗跌）"),
    ("客戶", "AMD",     "超微",        "429.56", "-5.51%", "📉 #4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/29 續挫 -5.51% 收 $429.56、連 5 日回檔累計約 -20%）"),
    ("客戶", "2454.TW", "聯發科",      "3,150",  "-4.98%", "📉 手機 SoC 客戶 9%（7/29 收 3,150、TWSE 官方——前日跌停後跌幅收斂、相對止穩）"),
    ("競爭", "INTC",    "英特爾",      "81.88",  "-5.12%", "📉 7/29 續挫 -5.12% 收 $81.88（7/24 財報後連 4 跌）；Q2 EPS/營收雙超預期但 GAAP 虧損＋foundry 客戶進展疑慮；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "208,500 KRW", "-5.23%", "⚠️ 7/29 續挫 -5.23% 收 20.85 萬韓元（Yahoo API、兩日累計 -17.9%）；7/30 韓股大盤劇震後回穩（KOSPI 盤中 +0.03%）、今晨另公布 Q2 營益創新高（韓媒報導），惟三星當日個股漲跌未能證實、不予引用；7/7 已公布 Q2 初值營益 89.4 兆韓元暴增"),
    ("產業", "SOX",     "費城半導體指數","10,447.49", "-5.33%", "⚠️ 7/29 再挫 -5.33% 收 10,447.49——自 6/22 史高 14,634.66 收斂至 -28.6%；設備/記憶體領跌、NVDA 亦翻黑——美股由分化轉全面補跌；盤後 MSFT 財報大超預期為修復錨"),
    ("處分後", "ARM",   "Arm Holdings","224.89", "-8.11%", "TSMC 4/29 全數出脫後（7/29 再挫 -8.11%、與前日跌幅相同——高 Beta 連日殺戮）"),
]
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "11,035.68", "-4.49%", False),  # 7/28 收（Yahoo）自 6/22 史高收斂至 -24.6%、熊市加深
    ("台股加權指數",    "41,603.36", "-4.65%", False),  # 7/28 官方收盤（-2,030.83 點、史上第 3 大單日跌點、TWSE）
    ("VIX 恐慌指數",    "18.21",    "-2.46%", True),    # 7/28 收（Yahoo）不升反降——美股未見系統性恐慌
    ("WTI 原油",        "$82.55",   "-0.1%",  True),    # 7/28 近月（Yahoo CL=F；持穩、地緣溢價續退）
    ("10Y 美債殖利率",  "4.60%",   "-4bp",   True),     # 7/28 收（Yahoo ^TNX、避險買盤壓低）
    ("台幣/美元",       "~32.30",   "持平",  False),    # 7/28 Yahoo TWD=X 32.302（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$197.01", "+0.25%",  True),    # #1 TSMC 客戶（22%），7/28 逆勢翻紅止穩
    ("ASML",            "$1,582.95", "-4.37%",  False), # 設備股 7/28 隨記憶體/設備賣壓重挫（Yahoo）
]
...
RELATIVE = {
    "date":           "2026-07-28",       # 最新資料日（台股 2330 與 NYSE TSM 均為 7/28 Tue 收盤）
    "tsmc_tw_1d":     -2.98,   # 2330 7/28 官方收 NT$2,280（-70 vs 7/27 NT$2,350、TWSE）
    "taiex_1d":       -4.65,   # 加權指數 7/28 收 41,603.36（-2,030.83、史上第 3 大單日跌點、TWSE 官方）
    "sox_1d":         -4.49,   # SOX 7/28 收 11,035.68（Yahoo）
    "tsmc_nyse_1d":   -1.70,   # TSM ADR 7/28 $392.31（-$6.78；亞洲股災外溢下相對抗跌）
    "tsmc_tw_5d":     -5.39,   # 5 日（NT$2,280 vs 7/21 官方收 NT$2,410）
    "taiex_5d":       -5.94,   # （41,603.36 vs 7/21 44,232.87）
    "tsmc_tw_1m":     -2.6,    # 月線（NT$2,280 vs 6/26 官方收 NT$2,340）
    "taiex_1m":       -7.6,    # （估、vs 6 月底 ~45,000）
    "tsmc_tw_ytd":    +8.8,    # YTD 累計（NT$2,280 vs 年初 ~NT$2,095）
    "taiex_ytd":      +3.7,    # （估）
    "sox_ytd":        +28.8,   # SOX YTD（費半 11,035.68、估）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/28 NYSE 收盤 & 7/28 TW 官方收盤計算）
    "adr_price":     392.31,   # TSM NYSE 7/28 收盤（-1.70%、Yahoo Finance API）
    "adr_parity":    352.92,   # 理論值 = 5 × NT$2,280 ÷ 32.30
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.30,   # 台幣匯率（7/28 Yahoo TWD=X 32.302）
    "adr_premium_pct": "+11.16", # (392.31 - 352.92) / 352.92 × 100 = +11.16%（匯率 32.30）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 7/28 Tue 收盤：台股 NT$2,280（-2.98%、量 45,333 張 TWSE 口徑、市值 NT$59.1 兆）——加權指數同日 -4.65%（-2,030.83 點、史上第 3 大單日跌點）；NYSE $392.31（-1.70%）——同日 SOX -4.49%、2330 與 TSM 在各自市場均為相對抗跌。📊 籌碼面（T86 官方）：7/28 外資賣超擴大 -14,659 張（連 5 賣），投信 +284（連 3 買）、自營 +5,123（連 4 買、大幅低接）、合計 -9,252 張；全市場 BFI82U 三大法人大賣 -1,176.0 億（外資 -874.8 億、自營 -317.3 億）。📊 台美 ADR 溢價 +11.16%（$392.31 vs 理論值 $352.92、匯率 32.30）——台股重挫使溢價自 +9.7% 回擴、補跌壓力獲緩衝。⚠️ 本輪為韓股熔斷（KOSPI -10.8%）槓桿去化外溢之系統性風險；✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量 + CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "⚠️ 股災第 2 日：台股再挫 -3.76%、2330 盤中觸 2,180 收 2,200",
        "body": "台股 7/29 續挫：（1）⚠️ 大盤 -1,564.18 點（-3.76%）收 40,039.18——兩日累計約 -3,595 點、貼近 4 萬心理關口；（2）📊 2330 小低開 2,260 後全日走弱、13:00 後恐慌殺盤觸 2,180（-4.4%）、尾盤拉回收 NT$2,200（-80、-3.51%、量 68,140 張達 5 日均量 1.75 倍）——本日跌幅與大盤相當、前日「相對抗跌」角色弱化；供應鏈續殺：日月光 -9.93% 近跌停、華邦電 -9.72%、聯電 -9.69%；（3）✅ 籌碼賣壓明顯收斂為最重要邊際變化：外資賣超 -11,211 張（自 -14,659 收斂 24%）、投信 +1,652 張放大 6 倍、自營 +4,893 連 5 買；全市場 BFI82U 賣超 -351.5 億、自 -1,176 億收斂 70%——恐慌賣壓高峰或已過。",
        "source": "TWSE 官方（STOCK_DAY/T86/BFI82U）2026-07-29", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⭐ MSFT 財報大超預期 Azure +43%、META EPS miss——AI CapEx 未收縮",
        "body": "本輪股災首個基本面驗證點揭曉：（1）✅ MSFT FY26Q4 營收 $90.01B 大超預期（est $87.62B）、Azure 成長 +43%（固定匯率、大幅超 est +40.2%）、FY2026 Azure 全年營收首度突破 $1,000 億美元——盤後大漲 +8.83% 收 $425.01（正規盤收 $390.54、-0.71%；財報剛公布時僅約 +3%、電話會後漲幅持續擴大至盤後收盤）；（2）⚠️ META 營收 +28% YoY 超預期約 1%、惟 EPS 低於預期約 14%（YoY -13%）、2026 CapEx 指引 $125–145B 續擴——盤後重挫 -7.45% 收 $542.00（正規盤收 $585.61；財報剛公布時一度殺至 -9.64%、電話會後收斂）；（3）🎯 共同訊息：hyperscaler AI CapEx 並未收縮（MSFT 下年度 CapEx 市場估 ~$119B、+22%）——對 TSMC 先進製程/CoWoS 訂單能見度為直接正面；市場開始對「燒錢 vs 回報」分層計價——能證明回報者（MSFT）漲、待驗證者（META）跌；（4）📅 今晚（美東 7/30）AAPL/AMZN 財報接棒、為 AI CapEx 驗證鏈最後一棒。",
        "source": "CNBC / Axios / Barron's 2026-07-29", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 FOMC 鷹派按兵不動：9-3 分歧、3 票主張升息",
        "body": "聯準會 7/29 決議：（1）📊 維持聯邦資金利率 3.50%–3.75% 不變（連續第 5 次會議）；（2）⚠️ 投票 9-3 罕見分歧——Hammack、Kashkari、Logan 三票主張升息 25bp（鷹派異議）；聲明指經濟活動穩健擴張、資本投資與生產力強勁、並點名中東衝突帶來的不確定性；（3）📊 主席 Warsh 會後記者會未鬆口、官員年底利率預測介於 3.6%–4.1%——市場「等降息」敘事轉為「防升息」；（4）🎯 對 TSMC 的意涵：鷹派利率＋WTI 同步反彈至 ~$84.2（中東衝突）——折現率與通膨雙重逆風對高估值 AI 資產構成頂部壓力、疊加本輪 AI 回報質疑殺盤。",
        "source": "Federal Reserve 官方聲明 / CNBC 2026-07-29", "impact": "中性",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "📊 韓股 7/29 重挫、7/30 今晨首見止穩：SK 海力士財報創史高仍 -9.61%",
        "body": "槓桿去化第 3 日：（1）⚠️ SK 海力士 7/29 早盤公布 Q2 財報全面創史高（營收 79.32 兆韓元 +257% YoY、營益 60.54 兆韓元 +557% YoY、營益率 76%、HBM4 已量產出貨）、股價仍重挫 -9.61% 收 140.1 萬韓元——「創高財報救不了股價」、賣壓純屬籌碼與槓桿而非基本面；三星同日 -5.23% 收 20.85 萬；（2）📊 KOSPI 7/29 官方收 5,663.24（-5.98%、前收 6,023.66、當日連 2 度觸發熔斷——已補查證）；今晨（7/30）轉為劇震後回穩：盤中 5,665.17（+0.03%、約當持平）、日內區間 5,547.41–5,781.42（振幅 4.2%）；三星電子今晨另公布 Q2 營益創新高（韓媒報導；三星／SK 海力士 7/30 個股當日漲跌未能證實）——槓桿去化首見止穩跡象、惟尚不足稱底；（3）🎯 對台股意涵：韓股融資追繳與雙倍槓桿 ETF 斷頭的去化進程未見底、續為亞股最大外部變數；惟 2330 籌碼面（外資賣超收斂、投信/自營低接）已先行改善。",
        "source": "SK hynix Newsroom / Yahoo Finance API 2026-07-29~30", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 美股由分化轉全面補跌：SOX -5.33% 自史高 -28.6%、VIX 破 20",
        "body": "美股 7/29 正式加入調整：（1）⚠️ SOX -5.33% 收 10,447.49——自 6/22 史高 14,634.66 收斂至 -28.6%、連 3 日重挫；（2）📊 設備/記憶體殺戮續深：KLAC -10.80%、MU -9.94%（自 7/23 高累計 -25.4%）、AMAT -8.40%、ARM -8.11%、LRCX -6.40%；AMD -5.51%、INTC -5.12%；（3）⚠️ 前日翻紅的權值全數轉跌：NVDA -3.55% 收 $190.01 續探、TSM -4.50% 收 $374.67；僅 AAPL -0.56%（財報前避風）與 ASML -2.04% 相對抗跌；（4）💡 VIX +13.5% 升破 20 至 20.66、S&P 500 -1.52%——前日「VIX 反降、NVDA/AAPL 翻紅」的分化格局終結；惟盤後 MSFT 財報大超預期、期貨止穩——今晚 AAPL/AMZN 為關鍵。",
        "source": "Yahoo Finance API / CNBC 2026-07-29", "impact": "負面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 美 232 條款半導體調查 7 月底出爐（今明兩日窗口）",
        "body": "政策窗口進入最後倒數：（1）📅 市場關注的半導體 232 條款調查結果預計 7 月底公布——今明兩日（7/30-31）為時間窗口；美國自 7/24 起已對 60 個貿易夥伴實施新關稅（10%~12.5%）；（2）📊 台經院分析：先進製程受影響有限、衝擊較大的是產能過剩的成熟製程——TSMC 先進製程壟斷地位＋$165B 美國投資承諾為緩衝；（3）⭐ 台美貿易協議談判中、市場推測台灣關稅可能由現行 20% 下調至約 15%；（4）🎯 若 232 結果溫和＋關稅下調兌現、可望成為本輪股災後情緒修復的最直接政策催化；反之為尾部風險。",
        "source": "工商時報 / 中時新聞網 2026-07-24", "impact": "中性",
    },
]

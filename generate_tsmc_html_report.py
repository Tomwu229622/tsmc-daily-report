...
STOCK = {
    # 台股 2330
    "tw_price":       "2,280",        # 台股 2330 最新確認收盤：7/28 Tue 官方收 NT$2,280（-70、-2.98% vs 7/27 NT$2,350、開 2,270／高 2,305／低 2,270、TWSE 確認）——韓股熔斷（KOSPI -10.8%）情緒傳染、跳空低開 -80 點後全日弱勢、收盤跌破 60MA 2,347（季線失守）並收在布林下軌 2,289 之下；大盤同日暴跌 -2,030.83 點（-4.65%）收 41,603.36、史上第 3 大單日跌點
    "tw_change":      "-70",          # 7/28 Tue 收盤漲跌點數（-2.98%、TWSE 官方）
    "tw_change_pct":  "-2.98%",       # 7/28 Tue 收盤漲跌幅（跳空低開 2,270 → 盤中高 2,305 → 收 2,280；跌幅 -2.98% 遠小於大盤 -4.65% 與供應鏈 -9~-10%、相對抗跌）
    "tw_volume":      "45,333",       # 7/28 成交量（張、TWSE 口徑；成交額 NT$1,036.24 億、成交 463,044 筆——量能放大 57%（前日 28,939 張）且成交筆數自 92,753 暴增至 463,044、恐慌性小單出脫再現）
    "tw_open":        "2,270",        # 7/28 開盤（TWSE、跳空低開 -80 點、即全日最低）
    "tw_high":        "2,305",        # 7/28 盤中高（TWSE、反彈未能站回 2,320）
    "tw_low":         "2,270",        # 7/28 盤中低（TWSE、開盤即低點）
    "market_cap":     "NT$59.1 兆",   # 7/28 收盤市值（NT$2,280 基礎、約 259.3 億股）
    "pe_ratio":       "30.65x",       # 台股 P/E（TWSE 官方 BWIBBU 7/28、口徑為最新年度 EPS；先前版本之 TTM 估算 ~26.9x 口徑不同）
    "pb_ratio":       "10.04x",       # 台股 P/B（TWSE 官方 BWIBBU 7/28、以 115Q1 財報淨值計；先前版本之 ~17.6x 為估算、以官方為準）
    "div_yield":      "0.96%",        # 殖利率（TWSE 官方 BWIBBU 7/28、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "392.31",       # 7/28 Tue 官方收盤 $392.31（-$6.78、-1.70% vs 7/27 $399.09、Yahoo Finance API 確認）——韓股熔斷外溢下跌幅仍遠小於同業（相對抗跌）；SOX 同日 -4.49% 收 11,035.68（自 6/22 史高 14,634.66 收斂至 -24.6%、熊市加深）
    "nyse_change":    "-6.78",        # 7/28 漲跌（自 7/27 $399.09）
    "nyse_change_pct":"-1.70%",       # 7/28 極端分化：MU -8.85%／AMD -8.15%／ARM -8.11%／AMAT -7.82%／AMKR -24.74%；惟 NVDA +0.25% 翻紅、AAPL +0.94% 連 3 漲、VIX 反降 -2.46% 至 18.21——賣壓集中記憶體/設備、非全面恐慌
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~28.7x",       # NYSE USD 基礎 P/E（TTM 估，$392.31 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~16.8x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-29 Wed（最新確認收盤：台股 2330 7/28 Tue 官方收 NT$2,280（-70、-2.98% vs 7/27 NT$2,350、開 2,270／高 2,305／低 2,270、量 45,333 張 TWSE 口徑、成交額 NT$1,036.24 億、成交 463,044 筆、TWSE 確認）；NYSE TSM 7/28 Tue 收 $392.31（-$6.78、-1.70% vs 7/27 $399.09、Yahoo Finance API 確認））。📊 今日格局：韓股熔斷引爆亞股系統性崩跌、個股相對抗跌但關鍵支撐全數失守——（1）⚠️ 韓股 7/28 熔斷：KOSPI 收盤暴跌 -10.8%（盤中跌破 -8% 觸發第一階段熔斷、全市場停止交易 20 分鐘，為今年第 8 次；此前已先觸發賣方 sidecar）——三星 -13.39% 收 22.0 萬韓元、SK 海力士 -14.65% 收 155.0 萬韓元；⚠️ 三大導火線（Wolf Street／路透）：①中國記憶體廠 CXMT 長鑫存儲 7/27 上海科創板掛牌暴漲 +465.8% 收 49 元人民幣、市值逾 3 兆人民幣（約 $4,430 億）躍居 A 股市值王（IPO 募資 579 億人民幣／約 $86 億、亞洲今年最大）——中國 DRAM 資本大幅擴張；②路透獨家：中國上海國企已開始量產自製「浸潤式 DUV」曝光機、今年約 5 台交付 SMIC／華虹／CXMT、2027 年增至約 20 台（整併 SMEE 與芯微電子 Yuliangsheng 團隊、後者為華為系 SiCarrier 關係企業；浸潤式 DUV 單次曝光可做 28nm、多重曝光可及 7nm）——衝擊 ASML 長線壟斷與中國先進製程天花板敘事；③NVDA 7/27 重挫 -5.00% 之「循環融資（round-tripping）」疑慮——市場傳 NVDA 洽談為 OpenAI 資料中心提供約 $2,500 億融資擔保、另傳最高 $3,500 億晶片採購融資；④疊加韓股融資追繳（margin call）、強制賣出與放空；（2）⚠️ 台股 7/28 史上第 3 大跌點：大盤暴跌 -2,030.83 點（-4.65%）收 41,603.36、成交額 8,094.82 億——電子 -5.11%、半導體類指 -4.42%；2330 跳空低開 2,270（-80）後收 2,280（-2.98%）——跌幅小於大盤與供應鏈（聯發科 -9.92%、日月光 -8.88%、聯電 -9.92%、華邦電 -10% 跌停）、續為相對抗跌，惟 60MA 2,347（季線）與 7/17 低 2,290 雙雙失守、收在布林下軌 2,289 之下；量放大 57% 至 45,333 張、成交筆數暴增至 463,044 筆（恐慌小單再現）；（3）⚠️ 籌碼再轉空：外資對 2330 賣超擴大至 -14,659 張（連 5 賣、自 -2,662 再放大）、投信 +284 連 3 買、自營 +5,123 連 4 買且大幅擴大（低接）、三大合計 -9,252 張；全市場 BFI82U 由買轉大賣：三大法人賣超 -1,176.0 億——外資 -874.8 億、自營 -317.3 億、投信 +16.1 億；（4）📊 美股 7/28 極端分化：TSM -1.70% 收 $392.31（相對抗跌）、SOX -4.49% 收 11,035.68（自 6/22 史高 -24.6%、熊市加深）——記憶體/設備重挫（MU -8.85%、AMAT -7.82%、LRCX -7.54%、KLAC -6.18%、ASML -4.37%、AMKR -24.74%）、AMD -8.15%／ARM -8.11%／QCOM -4.21%；✅ 惟 NVDA +0.25% 翻紅收 $197.01、AAPL +0.94% 連 3 漲收 $340.08、AVGO 僅 -0.60%，且 VIX 不升反降 -2.46% 至 18.21——美股未見系統性恐慌、賣壓集中記憶體與設備；（5）⭐ 今晨（7/29）韓股初步止穩：三星盤中反彈至 ~22.8 萬（約 +3.6%）、SK 海力士盤中 ~155.1 萬持平——✅ SK 海力士今日（7/29）公布 Q2 財報全面創高：營收 79.32 兆韓元（+257% YoY、+51% QoQ）、營益 60.54 兆韓元（+557% YoY、+61% QoQ）、營益率 76%、淨利 93.92 兆韓元，HBM4 已於 Q2 開始大量出貨、下半年續拉升產能，並強調維持「資本支出紀律」——「獲利創史高 vs 股價 7/28 暴跌 -14.7%」的極端背離，佐證本輪殺盤主因為槓桿去化與中國供給端敘事、而非 AI 需求轉弱；（6）⭐ 油價續穩 WTI ~$82.6、10Y 美債 4.60%（-4bp）。📊 台美 ADR 溢價 +11.2%（$392.31 vs 理論值 ~$352.9、匯率 32.30）——台股重挫使溢價自 +9.7% 回擴、今日具補跌壓力緩衝。技術面（官方收盤計算至 7/28）：收 2,280 跌破 60MA 2,347（季線失守）、跌破布林下軌 2,289——RSI(14) 40.8 續降、MACD 綠柱急擴至 -33.9 且 DIF -9.3 深陷零軸下（中期空方動能確立）、KDJ 死叉深化（K29.1/D38.9/J9.5——J 值進入超賣區、短線超跌）；支撐 2,270（7/28 低）／2,230（6/8 跳空低）／2,200（心理整數）、壓力 2,289-2,290（布林下軌/7/17 低）／2,305（7/28 高）／2,320／2,347-2,357（60MA/5MA）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋2027 全面漲價 5-10% 傳聞；本輪為韓股槓桿斷頭引發之系統性風險、非台積電個別利空。⚠️ 風險：SOX 自史高 -24.6% 熊市加深、KOSPI 槓桿去化未止、MSFT/META（美東 7/29）AAPL/AMZN（7/30）財報 AI CapEx 指引、美 232 條款半導體調查 7 月底出爐、台幣 ~32.30。⭐ 今日（7/29）觀察：（1）韓股止穩延續性＋SK 海力士財報電話會 HBM 定調；（2）2330 能否守住 2,270／收復布林下軌 2,289；（3）外資賣超 -14,659 能否收斂；（4）今晚美股 MSFT/META 財報前風向。",
}
...
SUMMARY = {
    ...
    "sentiment":  "看空",
    # 一句話標題
    "headline":   "⚠️ 韓股熔斷引爆亞股股災：KOSPI 7/28 暴跌 -10.8%（三星 -13.4%、SK 海力士 -14.7%）、台股史上第 3 大跌點 -2,030.83 點（-4.65%）收 41,603.36；2330 收 NT$2,280（-2.98%）相對抗跌、惟季線（60MA 2,347）與布林下軌 2,289 雙雙失守；外資賣超擴大至 -14,659 張、全市場三大法人大賣 -1,176 億；📊 美股 7/28 分化：TSM -1.70% 收 $392.31、SOX -4.49% 熊市加深，惟 NVDA/AAPL 翻紅、VIX 反降 18.21——未見系統性恐慌；⭐ 今晨韓股初步止穩，SK 海力士 7/29 Q2 財報全面創高（營益 60.54 兆韓元、+557% YoY、營益率 76%、HBM4 已量產出貨）——基本面與股價極端背離；MSFT/META（美東 7/29）財報為下一關鍵",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⚠️ 韓股 7/28 熔斷：KOSPI 收盤 -10.8%（盤中破 -8% 觸發第一階段熔斷、全市停止交易 20 分鐘，今年第 8 次）——三星 -13.39% 收 22.0 萬韓元、SK 海力士 -14.65% 收 155.0 萬韓元；三大導火線：①中國 CXMT 長鑫存儲 7/27 科創板掛牌暴漲 +465.8%、市值躍居 A 股之冠（亞洲今年最大 IPO）；②路透獨家：中國已開始量產自製浸潤式 DUV 曝光機、今年約 5 台交付 SMIC／華虹／CXMT；③NVDA 循環融資（round-tripping）疑慮——再疊加韓股融資追繳與強制賣出",
        "⚠️ 台股 7/28 史上第 3 大跌點：大盤 -2,030.83 點（-4.65%）收 41,603.36、成交額 8,094.82 億；2330 官方收 NT$2,280（-70、-2.98%、開 2,270／高 2,305／低 2,270、量 45,333 張）——跌幅小於大盤與供應鏈（聯發科 -9.92%、日月光 -8.88%、聯電 -9.92%、華邦電 -10% 跌停）、相對抗跌，惟 60MA 2,347（季線）失守",
        "⚠️ 籌碼再轉空：外資對 2330 賣超擴大至 -14,659 張（連 5 賣、自 -2,662 再放大）、投信 +284 連 3 買、自營 +5,123 連 4 買且大幅擴大（低接）；全市場 BFI82U 由買轉大賣：三大法人賣超 -1,176.0 億——外資 -874.8 億、自營 -317.3 億",
        "📊 美股 7/28 極端分化：TSM -1.70% 收 $392.31（相對抗跌）、SOX -4.49% 收 11,035.68（自 6/22 史高 -24.6%、熊市加深）——記憶體/設備重挫（MU -8.85%、AMAT -7.82%、LRCX -7.54%、AMKR -24.74%）、AMD -8.15%／ARM -8.11%；惟 NVDA +0.25% 翻紅、AAPL +0.94% 連 3 漲、VIX 反降 -2.46% 至 18.21——美股未見系統性恐慌",
        "⭐ SK 海力士今日（7/29）Q2 財報全面創高並優於預期：營收 79.32 兆韓元（+257% YoY、+51% QoQ）、營益 60.54 兆韓元（+557% YoY、+61% QoQ）、營益率 76%、淨利 93.92 兆韓元——HBM4 已於 Q2 開始大量出貨、下半年續拉升產能，並強調維持「資本支出紀律」；韓股今晨初步止穩（三星盤中 ~22.8 萬、+3.6%；SK 盤中 ~155.1 萬持平）——「基本面創高 vs 股價暴跌 -14.7%」背離為今日 AI 記憶體敘事關鍵驗證",
        "💡 技術面：收 2,280 跌破 60MA 2,347（季線）與布林下軌 2,289——RSI 40.8 續降、MACD 綠柱急擴 -33.9 且 DIF -9.3 深陷零軸下、KDJ 死叉深化（K29.1/D38.9/J9.5、J 進超賣區）；短線超跌、收復 2,289 為止穩第一訊號",
        "📊 本週焦點：MSFT/META（美東 7/29）、AAPL/AMZN（7/30）財報 AI CapEx 指引直接牽動 TSMC 訂單能見度；美 232 條款半導體調查預計 7 月底出爐、台美關稅談判傳由 20% 下調至 ~15%",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-29 Wed（最新確認收盤：台股 2330 7/28 Tue 官方收 NT$2,280（-70、-2.98% vs 7/27 NT$2,350、開 2,270／高 2,305／低 2,270、量 45,333 張 TWSE 口徑、成交額 NT$1,036.24 億、成交 463,044 筆、TWSE 確認）；NYSE TSM 7/28 收 $392.31（-$6.78、-1.70%、Yahoo API 確認））。📊 核心判讀：韓股槓桿斷頭引爆亞股系統性股災、2330 相對抗跌但中期支撐全數失守——（1）韓股 7/28 熔斷為風暴中心：KOSPI 收盤 -10.8%（盤中破 -8% 觸發第一階段熔斷、全市停止交易 20 分鐘，今年第 8 次）、三星 -13.39%、SK 海力士 -14.65%——三大導火線：①中國記憶體廠 CXMT 長鑫存儲 7/27 上海科創板掛牌暴漲 +465.8% 收 49 元人民幣、市值逾 3 兆人民幣躍居 A 股市值王（募資 579 億人民幣、亞洲今年最大 IPO），中國 DRAM 資本大幅擴張；②路透獨家：中國上海國企已開始量產自製浸潤式 DUV 曝光機、今年約 5 台交付 SMIC／華虹／CXMT、2027 年增至約 20 台（浸潤式 DUV 單次曝光可做 28nm、多重曝光可及 7nm）——衝擊 ASML 長線壟斷與中國先進製程天花板敘事；③NVDA 7/27 -5.00% 之循環融資（round-tripping）疑慮（傳為 OpenAI 資料中心提供約 $2,500 億融資擔保）；再疊加韓股融資追繳、強制賣出與放空；（2）台股 7/28 暴跌 -2,030.83 點（-4.65%）收 41,603.36 為史上第 3 大跌點——電子 -5.11%、半導體類指 -4.42%；2330 收 2,280（-2.98%）跌幅明顯小於大盤與供應鏈（聯發科/聯電 -9.9%、華邦電跌停）、續居抗跌角色，惟 60MA 2,347（季線）、7/17 低 2,290、布林下軌 2,289 一次全數失守——中期多空架構正式轉空；（3）籌碼再轉空：外資對 2330 賣超擴大至 -14,659 張（連 5 賣）、全市場 BFI82U 三大法人大賣 -1,176.0 億（外資 -874.8 億）——上日「籌碼改善」一日翻覆；惟自營 +5,123 連 4 買且大幅擴大、投信 +284 連 3 買，本土資金逢低承接；（4）📊 美股同日僅溫和下跌且結構分化：TSM -1.70%（相對抗跌）、SOX -4.49% 自史高 -24.6%——跌勢集中記憶體/設備（MU -8.85%、AMAT -7.82%、AMKR -24.74%），NVDA +0.25%／AAPL +0.94% 翻紅、VIX 不升反降至 18.21——顯示賣壓屬亞洲槓桿去化外溢、非美股本土恐慌，系統性風險層級低於 7/17；（5）⭐ 今晨韓股初步止穩（三星盤中 +3.6%、SK 持平）＋SK 海力士今日（7/29）Q2 財報全面創高：營收 79.32 兆韓元（+257% YoY、+51% QoQ）、營益 60.54 兆韓元（+557% YoY、+61% QoQ）、營益率 76%、淨利 93.92 兆韓元，HBM4 已於 Q2 開始大量出貨、下半年續拉升產能，並強調維持「資本支出紀律」——AI 記憶體實需獲財報實證，惟股價 7/28 仍暴跌 -14.7%，顯示本輪殺盤主因為槓桿去化與中國供給端敘事、非需求轉弱。技術面：RSI 40.8、MACD 綠柱 -33.9 急擴、DIF -9.3 深陷零軸下（中期空方動能確立）、KDJ J 9.5 超賣——短線嚴重超跌、技術性反彈條件累積中；收復布林下軌 2,289 → 7/28 高 2,305 → 2,320 為修復三關卡、失守 2,270 下看 2,230／2,200。📊 ADR 溢價回擴至 +11.2%（匯率 32.30）——台股補跌壓力獲緩衝。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋2027 漲價傳聞；本輪為韓股槓桿去化引發之系統性風險、非台積電個別利空。⭐ 今日（7/29）觀察：（1）韓股止穩延續性＋SK 財報電話會 HBM 定調；（2）2330 守 2,270／收復 2,289；（3）外資賣超能否自 -14,659 收斂；（4）今晚 MSFT/META 財報前美股風向。⚠️ 風險：KOSPI 槓桿去化未止、SOX 熊市加深、AI CapEx 指引不及預期、美 232 條款調查 7 月底出爐、台幣 ~32.30（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/29 Wed）關鍵觀察：（1）⚠️ 韓股股災延續性為最大變數：KOSPI 7/28 收盤 -10.8%（盤中破 -8% 觸發第一階段熔斷、全市停止交易 20 分鐘，今年第 8 次）——導火線為中國 CXMT 掛牌暴漲 +465.8% 的供給端擴張疑慮＋路透報導中國量產自製浸潤式 DUV 曝光機＋NVDA 循環融資疑慮，並疊加融資追繳與強制賣出；今晨三星盤中反彈 ~+3.6%、SK 海力士持平——初步止穩但槓桿去化是否結束未知；（2）✅ SK 海力士今日（7/29）Q2 財報全面創高且優於預期：營收 79.32 兆韓元（+257% YoY）、營益 60.54 兆韓元（+557% YoY）、營益率 76%、淨利 93.92 兆韓元；HBM4 已於 Q2 開始大量出貨、下半年續拉升，並強調維持「資本支出紀律」——AI 記憶體實需獲實證，與股價 -14.7% 形成極端背離、佐證本輪為籌碼與供給敘事殺盤而非需求轉弱；（3）📊 2330 多空檢核：收 2,280 已跌破 60MA 2,347（季線）、7/17 低 2,290 與布林下軌 2,289——中期架構轉空；守 2,270（7/28 低）則超賣反彈可期（KDJ J 9.5、量 45,333 張未達 7/17 恐慌量 97,362 的一半）、失守下看 2,230／2,200；收復 2,289 → 2,305 → 2,320 為修復三關卡；（4）✅ 相對正面訊號：美股未跟進恐慌（VIX 反降 18.21、NVDA/AAPL 翻紅）、自營連 4 買大幅擴大 +5,123 張低接、ADR 溢價 +11.2% 提供緩衝、油價續穩 WTI ~$82.6；（5）📅 今晚（美東 7/29）MSFT/META、明晚 AAPL/AMZN 財報——AI CapEx 指引為 TSMC 訂單能見度直接風向球；💱 美 232 條款半導體調查 7 月底出爐（台美談判傳關稅 20%→~15%）、台幣 ~32.30 續為背景風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-28",       # 最新資料日（7/28 Tue、TWSE T86 官方）
    "foreign":  -14659,            # 外資 7/28 賣超 14,659 張（連 5 賣且再擴大：-2,662 → -14,659——韓股熔斷日外資大舉調節權值）
    "trust":    284,               # 投信 7/28 買超 284 張（連 3 買：+550 → +567 → +284——續回補但力道縮）
    "dealer":   5123,              # 自營商 7/28 買超 5,123 張（連 4 買且大幅擴大：+1,410 → +5,123——崩跌日低接最積極）；三大法人合計 -9,252 張
    # 近5日（由舊到新：7/22, 7/23, 7/24, 7/27, 7/28）；TWSE T86 口徑（張）
    "foreign_5d": [-3539, -4614, -9637, -2662, -14659], # 外資（連 5 賣、7/28 再擴大）
    "trust_5d":   [1562, -943, 550, 567, 284],           # 投信（7/24 起連 3 買）
    "dealer_5d":  [-1059, 38, 305, 1410, 5123],          # 自營（連 4 買且大幅擴大）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       40.8,   "rsi_signal": "RSI(14) 40.8（官方收盤計算）——7/28 重挫後自 46.4 續降、遠離 50 中軸；弱勢區但未達 30 超賣、與 KDJ 超賣訊號背離顯示跌勢集中單日",
    "macd":         -33.9, "macd_signal": "MACD 綠柱急擴至 -33.9（DIF -9.3 < DEA 7.6、官方收盤計算）——⚠️ DIF 深陷零軸下（-0.8 → -9.3）——中期空方動能正式確立；綠柱收斂＋DIF 回升為修復前提",   "macd_color": "#F44336",
    "kdj_k":        29.1,   "kdj_d": 38.9, "kdj_j": 9.5, "kdj_signal": "KDJ K(29.1)/D(38.9)/J(9.5)（官方收盤計算）——死叉深化第 3 日、J 值 9.5 進入超賣區——短線嚴重超跌、技術性反彈條件累積中；反彈第一關為布林下軌 2,289",
    "ma5":       "2,357",   # 5MA（官方收盤計算；7/28 收 2,280 遠低於其下、短均全數蓋頭）
    "ma20":      "2,409",   # 20MA（官方收盤計算；與布林中軌重合、中期修復目標）
    "ma60":      "2,347",  # 60MA（官方收盤計算；7/28 收 2,280 跌破 -67 點——季線正式失守、中期多空架構轉空）
    "ma120":     "2,126",  # 120MA（沿用 7/27 計算值、7/28 未重算——日變動極小；半年線為最後長期支撐帶）
    "bb_upper":  "2,529",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,409",   # 布林中軌（20MA）
    "bb_lower":  "2,289",  # 布林下軌（20MA-2σ、官方收盤計算；7/28 收 2,280 已收在下軌之下——超跌、收復為止穩第一訊號）
    "vol_ma5":  "31,747",   # 5日均量（張、TWSE 口徑；7/28 量 45,333 放大至均量 1.43 倍、惟僅 7/17 恐慌量 97,362 的 47%——恐慌程度低於上輪）
    "trend":    "⚠️ 韓股熔斷引爆亞股股災、中期支撐全數失守：台股 2330 7/28 官方收 NT$2,280（-70、-2.98%、開 2,270／高 2,305／低 2,270、量 45,333 張）——跳空低開 -80 後全日弱勢、60MA 2,347（季線）／7/17 低 2,290／布林下軌 2,289 一次失守；大盤同日暴跌 -2,030.83 點（-4.65%）收 41,603.36、史上第 3 大跌點。📊 技術指標（官方收盤計算）：KDJ 死叉深化且 J 9.5 進入超賣（K29.1/D38.9）、RSI(14) 40.8 續降、MACD 綠柱急擴 -33.9 且 DIF -9.3 深陷零軸下——中期空方動能確立、短線嚴重超跌並存。⚠️ 籌碼再轉空：外資對 2330 賣超擴大 -14,659 張（連 5 賣）、全市場 BFI82U 三大法人大賣 -1,176 億；惟自營 +5,123 連 4 買大幅低接、投信 +284 連 3 買。📊 美股 7/28 分化：TSM -1.70% 收 $392.31、SOX -4.49% 自史高 -24.6%——記憶體/設備重挫（MU -8.85%、AMAT -7.82%）、NVDA +0.25%／AAPL +0.94% 翻紅、VIX 反降 18.21——未見美股系統性恐慌。⭐ 今晨韓股初步止穩（三星盤中 +3.6%）、SK 海力士今日 Q2 財報為風向球。下一觀察：（1）守 2,270（7/28 低）則超賣反彈可期、失守下看 2,230／2,200；（2）收復布林下軌 2,289 → 2,305（7/28 高）→ 2,320 為修復三關卡；（3）外資賣超能否收斂＋MSFT/META（美東 7/29）財報。",
    "support":  "2,270 / 2,230 / 2,200 / 2,126 (7/28 低、6/8 跳空低、心理整數、120MA)",
    "resist":   "2,289-2,290 / 2,305 / 2,320 / 2,347-2,357 (布林下軌+7/17 低、7/28 高、7/20 收、60MA+5MA)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-28 止），請以看盤軟體為準；7/28 官方收 NT$2,280（-2.98%）跳空低開後 60MA 2,347（季線）與布林下軌 2,289 雙雙失守；⚠️ KDJ 死叉深化且 J 9.5 超賣（K29.1/D38.9）、RSI 40.8、MACD 綠柱 -33.9 急擴且 DIF -9.3 深陷零軸下；支撐 2,270 / 2,230 / 2,200、壓力 2,289-2,290 / 2,305 / 2,320 / 2,347-2,357；⚠️ 外資賣超 -14,659 張＋BFI82U 大賣 -1,176 億 vs ✅ 自營連 4 買 +5,123 低接＋美股未恐慌（VIX 18.21）＋KDJ 超賣——守 2,270 與收復 2,289 為今日多空雙檢核",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-28",       # 最新資料日（7/28 Tue 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,272",            # 台積電期貨近月（估、現貨 7/28 收 NT$2,280 重挫、貼水擴大）
    "futures_oi":    "185,000",          # 未平倉量（口）（估、崩跌日避險與投機部位同增）
    "futures_basis": "-8",               # 估逆價差（崩跌日貼水自 -4 擴大）
    "call_oi":       "112,000",
    "put_oi":        "106,000",
    "pcr":           "0.95",             # Put/Call 比率（估、較 7/27 的 0.91 上升——韓股熔斷＋台股史上第 3 大跌點、避險 Put 需求大增）
    "pcr_signal":    "看空（Put/Call 估 0.95；7/28 現貨重挫 -2.98% 失守季線、外資期現貨同步調節，惟 KDJ 超賣＋今晨韓股止穩、超跌反彈需求與避險需求並存；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,300",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,200", "Call 48,000 / Put 14,000", "心理整數、極端支撐"),
        ("2,230", "Call 45,000 / Put 12,500", "6/8 跳空低、下方支撐"),
        ("2,270", "Call 42,000 / Put 11,000", "7/28 低、當日防線"),
        ("2,289", "Call 40,000 / Put 10,000", "布林下軌、止穩第一關"),
        ("2,300", "Call 38,000 / Put 9,000",  "最大痛苦（估）、整數關卡"),
        ("2,305", "Call 36,000 / Put 8,500",  "7/28 高、修復第二關"),
        ("2,320", "Call 34,000 / Put 7,500",  "7/20 收、短線壓力"),
        ("2,347", "Call 31,000 / Put 6,500",  "60MA/季線、中期多空分界"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/28 NT$2,280）；NT$2,300 為估最大痛苦、下方支撐 2,270（7/28 低）/ 2,230 / 2,200、上方壓力 2,289（布林下軌）/ 2,305 / 2,320 / 2,347（60MA）；⚠️ 韓股熔斷＋外資賣超 -14,659 張——貼水擴大、避險需求大增 vs ✅ KDJ 超賣＋自營連 4 買低接；✅ 基本面錨未變（2nm Q3 放量、2027 漲價傳聞、全年上修 >40%、CapEx $60-64B）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-28 Tue TWSE 官方收盤；美股為 7/28 Tue 收盤、Yahoo Finance API 確認；韓股為 7/28 收盤、7/29 盤中初步止穩）
    ("設備", "ASML",    "艾司摩爾",    "1,582.95", "-4.37%", "📉 EUV 設備龍頭；7/28 隨記憶體/設備賣壓續挫；TSMC CapEx 上調至 $60-64B 為設備鏈中長線利多"),
    ("設備", "AMAT",    "應用材料",    "476.46", "-7.82%", "📉 AMAT-TSMC $5B EPIC Center 研發合作；7/28 設備股重挫、連 3 日回落"),
    ("設備", "LRCX",    "科林研發",    "269.61", "-7.54%", "📉 蝕刻設備（7/28 重挫、連 3 日回落）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "190.80", "-6.18%", "📉 量測設備（7/28 重挫）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","144.00","-10.00%", "⚠️ 跌停（7/28 收 144.0、TWSE 官方——韓股記憶體股災直接外溢）；與 TSMC 合作 CUBE 特殊 DRAM 題材未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,550,000 KRW", "-14.65%", "⚠️ 7/28 熔斷日暴跌 -14.65% 收 155.0 萬韓元（Yahoo API、自 6 月高點 -48%）；✅ 7/29 公布 Q2 財報全面創高：營收 79.32 兆韓元（+257% YoY）、營益 60.54 兆韓元（+557% YoY）、營益率 76%、HBM4 Q2 起大量出貨——基本面與股價極端背離；7/29 盤中 ~155.1 萬持平初步止穩"),
    ("記憶", "MU",      "美光",        "820.53", "-8.85%", "📉 HBM / DDR5；7/28 重挫收 $820.53（自 7/23 高 $990 累計 -17.1%）——韓股記憶體股災外溢、高位去槓桿"),
    ("封裝", "3711.TW", "日月光",      "554.00", "-8.88%", "📉 CoWoS / 先進封裝（7/28 重挫收 554、TWSE 官方——補跌）"),
    ("封裝", "AMKR",    "Amkor",       "45.69",  "-24.74%", "⚠️⚠️ 崩跌 -24.74% 收 $45.69（7/28）——封裝股殺戮最重；TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產題材未變"),
    ("客戶", "NVDA",    "輝達",        "197.01", "+0.25%", "✅ TSMC #1 客戶 22%/$33B、A16 首發客戶；7/28 逆勢翻紅 +0.25% 收 $197.01——連 2 日重挫後止穩、亞洲股災未擊穿美股 AI 權值"),
    ("客戶", "AAPL",    "蘋果",        "340.08", "+0.94%", "🚀 #2 客戶 18%；7/28 續漲 +0.94% 收 $340.08 連 3 日逆勢——7/30 財報前資金避風；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "380.91", "-0.60%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/28 僅小跌、相對抗跌）"),
    ("客戶", "AMD",     "超微",        "454.62", "-8.15%", "📉 #4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/28 重挫 -8.15% 收 $454.62、連 4 日回檔累計 -15.5%）"),
    ("客戶", "2454.TW", "聯發科",      "3,315",  "-9.92%", "⚠️ 手機 SoC 客戶 9%（7/28 跌停收 3,315、TWSE 官方——台股權值殺戮、與台達電／聯電同步跌停）"),
    ("競爭", "INTC",    "英特爾",      "86.30",  "-5.86%", "📉 7/28 續挫 -5.86% 收 $86.30（7/24 財報重挫後跌勢未止）；Q2 EPS/營收雙超預期但 GAAP 虧損＋foundry 客戶進展疑慮；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "220,000 KRW", "-13.39%", "⚠️ 7/28 熔斷日暴跌 -13.39% 收 22.0 萬韓元（Yahoo API）；7/29 盤中反彈至 ~22.8 萬（約 +3.6%）初步止穩；7/7 已公布 Q2 初值營益 89.4 兆韓元暴增"),
    ("產業", "SOX",     "費城半導體指數","11,035.68", "-4.49%", "⚠️ 7/28 重挫 -4.49% 收 11,035.68——自 6/22 史高 14,634.66 收斂至 -24.6%、熊市加深；記憶體/設備領跌、惟 NVDA/AAPL 翻紅——賣壓屬亞洲槓桿去化外溢"),
    ("處分後", "ARM",   "Arm Holdings","244.74", "-8.11%", "TSMC 4/29 全數出脫後（7/28 重挫 -8.11%——高 Beta 雙向波動、連日巨幅震盪）"),
]
...
NEWS = [
    {
        ...
        "title": "⚠️⚠️ 韓股熔斷：KOSPI 收盤 -10.8%、記憶體雙雄重挫 13-15%",
        "body": "7/28 韓國 KOSPI 收盤暴跌 -10.8%：（1）⚠️ 盤中跌破 -5% 先觸發賣方 sidecar（暫停程式交易 5 分鐘）、續破 -8% 觸發第一階段熔斷、全市場停止交易 20 分鐘——為今年第 8 次熔斷；三星 -13.39% 收 22.0 萬韓元、SK 海力士 -14.65% 收 155.0 萬韓元（自 6 月高點分別 -41%、-48%）；（2）📊 導火線三重共振：①中國 CXMT 長鑫存儲 7/27 科創板掛牌暴漲 +465.8%、市值躍居 A 股之冠——中國 DRAM 資本擴張疑慮；②路透報導中國開始量產自製浸潤式 DUV 曝光機；③NVDA 7/27 -5.00% 之循環融資疑慮；再疊加融資追繳、強制賣出與放空；（3）⭐ 今晨（7/29）初步止穩：三星盤中反彈至 ~22.8 萬（約 +3.6%）、SK 盤中持平；（4）🎯 屬槓桿去化＋供給端敘事的賣壓、非 AI 需求轉向。",
        "source": "Wolf Street / Seoul Economic Daily / Korea JoongAng Daily 2026-07-28", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⚠️ 台股史上第 3 大跌點 -2,030 點、2330 收 2,280 失守季線",
        "body": "韓股股災情緒傳染、台股 7/28 全面重挫：（1）⚠️ 加權指數暴跌 -2,030.83 點（-4.65%）收 41,603.36——收盤跌點為史上第 3 大，盤中一度重挫 2,069.19 點為史上第 5 大盤中跌點；電子 -5.11%、半導體類指 -4.42%、成交額 8,094.82 億；（2）📊 2330 收 NT$2,280（-70、-2.98%、量 45,333 張）——跌幅遠小於大盤與供應鏈（聯發科、台達電、聯電均跌停，華邦電／南亞科／旺宏／力積電「記憶體四雄」同步跌停，日月光 -8.88%、鴻海 -5.93%）、續為抗跌權值，惟 60MA 2,347（季線）與布林下軌 2,289 雙雙失守；（3）⚠️ 外資賣超擴大至 -14,659 張（連 5 賣）、全市場三大法人大賣 -1,176 億；（4）✅ 自營 +5,123 張連 4 買大幅低接、投信連 3 買——本土資金逢低承接。",
        "source": "TWSE 官方 / ETtoday財經雲 2026-07-28", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 美股 7/28 分化：SOX -4.49% 惟 NVDA/AAPL 翻紅、VIX 反降",
        "body": "亞洲股災外溢美股但未引發恐慌：（1）📊 SOX -4.49% 收 11,035.68——自 6/22 史高收斂至 -24.6%、熊市加深；SOX 全數成分股跌破 50 日均線（4/23 以來首見）；（2）⚠️ 賣壓集中記憶體/設備：MU -8.85%、AMAT -7.82%、LRCX -7.54%、KLAC -6.18%、AMKR -24.74%；AMD -8.15%、ARM -8.11%；（3）✅ TSM -1.70% 收 $392.31 相對抗跌；NVDA +0.25% 翻紅收 $197.01、AAPL +0.94% 連 3 漲收 $340.08、AVGO 僅 -0.60%；（4）💡 VIX 不升反降 -2.46% 至 18.21——顯示屬亞洲槓桿去化的外溢調整、非美股本土系統性恐慌。",
        "source": "Yahoo Finance API / CNBC 2026-07-28", "impact": "中性",
    },
    {
        "tag": "財報", "tag_color": "#FF9800",
        "title": "✅ SK 海力士 Q2 財報全面創高：營益 60.5 兆韓元、+557% YoY、HBM4 已量產出貨",
        "body": "AI 記憶體實需獲財報實證：（1）✅ SK 海力士 7/29（今日）公布 Q2 財報並全面刷新紀錄——營收 79.32 兆韓元（+257% YoY、+51% QoQ）、營業利益 60.54 兆韓元（+557% YoY、+61% QoQ）、營益率 76%、淨利 93.92 兆韓元；（2）🚀 HBM4 已於 Q2 開始大量出貨、下半年續拉升產能——公司強調 HBM4 已達客戶要求速度且具業界領先能效與成本競爭力；（3）📊 期末現金 88 兆韓元、淨現金 69.4 兆韓元；公司同時強調將維持「資本支出紀律」、依客戶需求分階段執行投資；（4）⚠️「獲利創史高 vs 股價 7/28 暴跌 -14.7%」的極端背離——佐證本輪殺盤主因為槓桿去化與中國供給端敘事、而非 AI 需求轉弱，對 TSMC 先進製程/CoWoS 需求面為間接正面。",
        "source": "SK hynix Newsroom 官方 / PR Newswire 2026-07-29", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 中國量產自製浸潤式 DUV 曝光機＋CXMT 掛牌暴漲——本輪股災的結構性導火線",
        "body": "中國半導體自主化雙利多成為亞股殺盤主因：（1）⚠️ 路透獨家：上海一家國資背景公司已開始量產中國首批自製「浸潤式 DUV」曝光機——今年約 5 台交付 SMIC／華虹／CXMT、2027 年增至約 20 台；該公司整併了 SMEE（上海微電子）與 Yuliangsheng（芯微電子，華為系 SiCarrier 關係企業）團隊，SMIC 自 2025/9 起已測試其機台；（2）📊 技術定位：浸潤式 DUV 單次曝光可做 28nm 級、透過多重曝光可及 7nm（代價為對準誤差與良率）——多數零件已國產化、惟部分關鍵件仍依賴日本；（3）💰 CXMT 長鑫存儲 7/27 上海科創板掛牌暴漲 +465.8% 收 49 元人民幣、市值逾 3 兆人民幣（約 $4,430 億）躍居 A 股市值王，募資 579 億人民幣為亞洲今年最大 IPO——全球第 4 大 DRAM 廠資本大幅擴張；（4）🎯 對 TSMC 的意涵：直接衝擊者為 ASML 與成熟製程/記憶體同業；TSMC 2nm/A16 等 EUV 世代先進製程短期不受影響，惟中國成熟製程與設備自主化長線壓縮價格與地緣風險溢價，值得列入中長期觀察。",
        "source": "Reuters（獨家）/ Tom's Hardware / China Daily / CNBC 2026-07-27~28", "impact": "負面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 美 232 條款半導體調查 7 月底出爐、台美關稅談判傳下調至 ~15%",
        "body": "政策窗口進入倒數：（1）📅 美國自 7/24 起對 60 個貿易夥伴實施新關稅（10%~12.5%）；市場關注的半導體 232 條款調查結果預計 7 月底公布；（2）📊 台經院分析：先進製程受影響有限、衝擊較大的是產能過剩的成熟製程——TSMC 先進製程壟斷地位＋$165B 美國投資承諾為緩衝；（3）⭐ 台美貿易協議談判中、市場推測台灣關稅可能由現行 20% 下調至約 15%；（4）🎯 若 232 結果溫和＋關稅下調兌現、可望成為股災後情緒修復的政策催化。",

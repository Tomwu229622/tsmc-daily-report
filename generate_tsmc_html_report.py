...
STOCK = {
    # 台股 2330
    "tw_price":       "2,320",        # 台股 2330 最新確認收盤：8/4 Tue 官方收 NT$2,320（-50、-2.11% vs 8/3 NT$2,370、開 2,335／高 2,360／低 2,310、TWSE 確認）——獲利了結第 2 日、跌破 20MA 2,371 與 60MA 2,350；大盤同日僅 -25.75 點（-0.06%）收 43,360.66——中小型科技（矽光子/CPO/低軌衛星）漲停潮、上漲 578 家 > 下跌 406 家
    "tw_change":      "-50",          # 8/4 Tue 收盤漲跌點數（-2.11%、TWSE 官方）
    "tw_change_pct":  "-2.11%",       # 8/4 Tue 續拉回：開 2,335 低開 → 盤中高 2,360 未能翻紅、尾盤收 2,320 跌破季線 2,350——連 2 日獲利了結；惟同晚美股半導體暴漲（SOX +6.55%）＋ADR +2.72%、今晨韓股大漲，8/5 反攻條件齊備
    "tw_volume":      "41,021",       # 8/4 成交量（張、TWSE 口徑；成交額 NT$954.55 億、成交 277,787 筆——量仍低於 5 日均量 53,044 張、賣壓未擴大）
    "tw_open":        "2,335",        # 8/4 開盤（TWSE、低開 -35）
    "tw_high":        "2,360",        # 8/4 盤中高（TWSE、逼近 60MA 2,350 上方後回落）
    "tw_low":         "2,310",        # 8/4 盤中低（TWSE、全日振幅 2.1%）
    "market_cap":     "NT$60.2 兆",   # 8/4 收盤市值（NT$2,320 基礎、約 259.3 億股）
    "pe_ratio":       "31.19x",       # 台股 P/E（TWSE 官方 BWIBBU 8/4、口徑為最新年度 EPS）
    "pb_ratio":       "10.21x",       # 台股 P/B（TWSE 官方 BWIBBU 8/4、以 115Q1 財報淨值計）
    "div_yield":      "0.95%",        # 殖利率（TWSE 官方 BWIBBU 8/4、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "417.17",       # 8/4 Tue 官方收盤 $417.17（+$11.06、+2.72% vs 8/3 $406.11、Yahoo Finance API 確認）——連 4 日收高、創本輪反彈新高；SOX 8/4 大漲 +6.55% 收 12,179.26（自 6/22 史高 14,634.66 收斂至 -16.8%）
    "nyse_change":    "+11.06",       # 8/4 漲跌（自 8/3 $406.11；自 7/29 低點 $374.67 累計 +11.3%）
    "nyse_change_pct":"+2.72%",       # 8/4 美股半導體全面暴漲：SOX +6.55%——ARM +17.36% 收 $280.56、INTC +10.84% 站上 $100、AMKR +9.89%、LRCX +7.85%、MU +7.62%、AMD 財報前 +7.00% 收 $518.58、KLAC +6.95%、AVGO +6.61%、AMAT +5.48%、ASML +4.22%、NVDA +2.56% 收 $211.94；S&P 500 +1.79% 收 7,736.52 創收盤新高（近兩月首次）、那指 +2.6%、Palantir +29%；AMZN -2.32%、META -0.39% 落後；VIX 16.50（+4.0%）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~30.5x",       # NYSE USD 基礎 P/E（TTM 估，$417.17 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.9x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-08-05 Wed（最新確認收盤：台股 2330 8/4 Tue 官方收 NT$2,320（-50、-2.11% vs 8/3 NT$2,370、開 2,335／高 2,360／低 2,310、量 41,021 張 TWSE 口徑、成交額 NT$954.55 億、成交 277,787 筆、TWSE 確認）；NYSE TSM 8/4 Tue 收 $417.17（+$11.06、+2.72% vs 8/3 $406.11、Yahoo API 確認、連 4 日收高創本輪反彈新高））。📊 今日格局：台美走勢短線背離、今日補漲條件齊備——（1）📊 台股 8/4 續拉回：2330 收 2,320（-2.11%）跌破 20MA 2,371 與 60MA 2,350（季線失守）、量 41,021 張仍低於 5 日均量 53,044——賣壓未擴大；大盤僅 -25.75 點（-0.06%）收 43,360.66、上漲 578 家 > 下跌 406 家——中小型科技（矽光子／CPO／低軌衛星）漲停潮、權值休息中小型接棒；華邦電 +9.79% 續強、日月光 -4.10% 拉回、聯發科 -1.15%；（2）⚠️ 籌碼：2330 外資連 2 賣 -12,430 張（8/3 -9,717 擴大）、投信 +434 連 8 買、自營 -2,327 連 2 賣、三大合計 -14,323 張（T86 官方）；全市場 BFI82U 合計 +20.1 億（外資 -57.3 億、投信 +271.7 億大買、自營 -194.3 億）——外資調節集中權值、投信資金持續進場；（3）⭐ 美股 8/4 半導體史詩級大漲：SOX +6.55% 收 12,179.26（自史高收斂至 -16.8%）——ARM +17.36%、INTC +10.84% 站上 $100、AMKR +9.89%、LRCX +7.85%、MU +7.62%、KLAC +6.95%、AVGO +6.61%、AMAT +5.48%、ASML +4.22%、NVDA +2.56% 收 $211.94；S&P 500 +1.79% 收 7,736.52 創收盤新高、那指 +2.6%；TSM +2.72% 收 $417.17 連 4 高；AMZN -2.32%、META -0.39%——資金自 hyperscaler 轉入半導體；（4）📊 AMD Q2 財報（8/4 盤後）：營收 $11.53B（+50% YoY、超共識 ~$11.3B）創新高、EPS $1.66 超預期 $1.62、資料中心營收 $6.7B（+107% YoY、占 58%）、Q3 指引 $13B±0.3B 遠超共識 $12.52B——數字全面超標惟不及市場「耳語數字」、盤後股價重挫-8.82% 收 $472.85（19:59 ET 盤後最終讀數、Yahoo 1 分線 includePrePost 確認）——財報前日 +7.00% 收 $518.58 的高預期回吐、AI 需求本身獲數字驗證；（5）⭐ 韓股 V 型再起：8/4 尾盤收復（盤中 -3% → 三星收 +0.21% 24.0 萬、SK +0.64% 157.7 萬）、今晨 8/5 盤中暴漲——KOSPI 6,670.67（較 8/3 +6.60%；8/4 官方指數 Yahoo 缺漏、依三星/SK 收平推估 8/4 約持平）、SK 海力士 +7.67% 至 169.8 萬、三星 +4.79% 至 25.15 萬——追隨美股半導體暴漲；（6）📅 232 條款截至 8/5 晨仍未查得官方公布（未能證實）；台美關稅談判傳 20%→~15%。📊 ADR 溢價暴衝至 +16.38%（$417.17 vs 理論值 $358.47、匯率 32.36）——台股連 2 日拉回＋ADR 連 4 漲、為近期最大溢價、今日台股補漲壓力極強。技術面（官方收盤計算至 8/4）：收 2,320 跌破 20MA 2,371（-2.15%）與 60MA 2,350（季線失守）、仍站上 5MA 2,304／120MA 2,152——RSI 47.3 中性偏弱、MACD 綠柱 -12.4 續收斂（DIF -17.8 零軸下）、KDJ 金叉延續（K 53.9／D 44.2、J 73.3 偏高）；支撐 2,310（8/4 低）／2,304（5MA）／2,215（布林下軌）／2,205（7/30 收）、壓力 2,345（7/31 低）／2,350（60MA）／2,360（8/4 高）／2,371（20MA）／2,425（7/31 漲停）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量；AMD 資料中心 +107% 再添 AI 需求實證。⭐ 今日（8/5）觀察：（1）2330 能否跳空收復 60MA 2,350 與 20MA 2,371（ADR +2.72%＋SOX +6.55% 對應強力開高）；（2）外資連 2 賣能否翻買（ADR 溢價 +16.4% 為誘因）；（3）AMD 盤後重挫是否壓抑亞股 AI 情緒（vs 韓股今晨已大漲）；（4）232 條款與台美關稅進展。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ 台美短線背離、今日補漲條件齊備：2330 8/4 續拉回 -2.11% 收 NT$2,320 跌破季線 2,350、外資連 2 賣 -12,430 張；美股 8/4 半導體史詩級大漲——SOX +6.55%、ARM +17.4%、INTC +10.8%、TSM +2.72% 收 $417.17 連 4 高、S&P 500 創收盤新高；AMD Q2 全面超標（營收 $11.53B +50%、資料中心 +107%）惟盤後重挫 -8.82%；今晨韓股暴漲（SK +7.7%）、ADR 溢價暴衝 +16.4% 為近期最大——今日觀察能否跳空收復 2,350／2,371",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 8/4：2330 收 NT$2,320（-50、-2.11%、開 2,335／高 2,360／低 2,310）——連 2 日獲利了結、跌破 20MA 2,371 與 60MA 2,350（季線失守）；量 41,021 張仍低於 5 日均量 53,044——賣壓未擴大；大盤僅 -25.75 點（-0.06%）收 43,360.66",
        "⭐ 中小型接棒為 8/4 主軸：上漲 578 家 > 下跌 406 家、矽光子／CPO／低軌衛星漲停潮；華邦電 +9.79% 收 157 續強、日月光 -4.10% 收 585 拉回、聯發科 -1.15% 收 3,865——輪動自權值股向中小型擴散",
        "⚠️ 籌碼：外資連 2 賣 -12,430 張（自 -9,717 擴大）、投信 +434 連 8 買、自營 -2,327 連 2 賣、三大合計 -14,323 張（T86 官方）；全市場 BFI82U 合計 +20.1 億（外資 -57.3 億、投信 +271.7 億大買）——外資調節集中權值、市場資金未離場",
        "⭐ 美股 8/4 半導體史詩級大漲：SOX +6.55% 收 12,179.26（自史高收斂至 -16.8%）——ARM +17.36%、INTC +10.84% 站上 $100、AMKR +9.89%、LRCX +7.85%、MU +7.62%、KLAC +6.95%、AVGO +6.61%、AMAT +5.48%、NVDA +2.56% 收 $211.94；S&P 500 +1.79% 收 7,736.52 創收盤新高、那指 +2.6%；TSM +2.72% 收 $417.17 連 4 高；AMZN -2.32%、META -0.39% 落後——資金自 hyperscaler 轉入半導體",
        "📊 AMD Q2 財報全面超標惟盤後重挫：營收 $11.53B（+50% YoY、超共識 ~$11.3B）創新高、EPS $1.66 超 $1.62、資料中心 $6.7B（+107% YoY）、Q3 指引 $13B 遠超共識 $12.52B——盤後仍跌 -8.82% 收 $472.85（19:59 ET 最終讀數、高「耳語數字」預期回吐）；AI 需求本身獲數字驗證、對 TSMC 先進製程／CoWoS 為實質正面",
        "⭐ 韓股 V 型再起：8/4 尾盤收復（盤中 -3% → 三星收 +0.21%、SK +0.64%）、今晨 8/5 盤中暴漲——SK 海力士 +7.67% 至 169.8 萬韓元、三星 +4.79%、KOSPI 6,670.67（較 8/3 +6.6%）——追隨美股半導體暴漲",
        "💡 技術面：收 2,320 跌破 20MA 2,371 與 60MA 2,350、仍站上 5MA 2,304／120MA 2,152；RSI 47.3 中性偏弱、MACD 綠柱 -12.4 續收斂、KDJ 金叉延續（K 53.9／D 44.2）——今日能否跳空收復 2,350／2,371 為多空分水嶺",
        "📊 ADR 溢價暴衝至 +16.38%（$417.17 vs 理論值 $358.47、匯率 32.36）——台股連 2 拉回＋ADR 連 4 漲、近期最大溢價、今日補漲壓力極強；232 條款仍未公布（未能證實）、台美關稅談判傳 20%→~15%",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-08-05 Wed（最新確認收盤：台股 2330 8/4 Tue 官方收 NT$2,320（-50、-2.11% vs 8/3 NT$2,370、開 2,335／高 2,360／低 2,310、量 41,021 張、額 NT$954.55 億、277,787 筆、TWSE 確認）；NYSE TSM 8/4 收 $417.17（+2.72%、連 4 日收高創本輪反彈新高、Yahoo API 確認））。📊 核心判讀：台美短線背離達近期極值、今日台股補漲條件齊備——（1）📊 台股 8/4 連 2 日獲利了結：2330 收 2,320 跌破 20MA 2,371 與 60MA 2,350（季線失守）、惟量 41,021 張仍低於 5 日均量 53,044（賣壓未擴大）；大盤僅 -0.06% 收 43,360.66、上漲 578 家 > 下跌 406 家——中小型科技（矽光子／CPO／低軌衛星）漲停潮接棒、資金未離場；（2）⚠️ 籌碼保留點：外資連 2 賣 -12,430 張（擴大）、「一日翻多」正式失敗、回到調節格局；投信 +434 連 8 買、全市場 BFI82U 投信 +271.7 億大買為多方底氣；（3）⭐ 外部環境強力偏多：美股 8/4 半導體史詩級大漲——SOX +6.55% 收 12,179.26（自史高收斂至 -16.8%）、ARM +17.36%、INTC +10.84%、AMKR +9.89%、LRCX +7.85%、MU +7.62%、AVGO +6.61%、NVDA +2.56%；S&P 500 +1.79% 收 7,736.52 創收盤新高；TSM ADR +2.72% 收 $417.17 連 4 高、ADR 溢價暴衝至 +16.38%（近期最大、強烈領先訊號）；（4）📊 AMD Q2 財報（8/4 盤後）數字全面超標：營收 $11.53B（+50% YoY）創新高、資料中心 $6.7B（+107% YoY）、Q3 指引 $13B 遠超共識——惟盤後重挫 -8.82% 收 $472.85（高「耳語數字」預期回吐、19:59 ET 最終讀數）——需求數字本身對 TSMC 先進製程／CoWoS 為實質正面、惟盤後跌勢對今日亞股 AI 情緒有干擾；（5）⭐ 韓股今晨已給答案：8/5 盤中 KOSPI 6,670.67（較 8/3 +6.6%）、SK 海力士 +7.67%、三星 +4.79%——追隨美股半導體暴漲、AMD 盤後干擾未阻升勢。💡 技術面：收 2,320 破 20MA 2,371／60MA 2,350、站上 5MA 2,304／120MA 2,152——RSI 47.3、MACD 綠柱 -12.4 續收斂（DIF -17.8 零軸下）、KDJ 金叉延續（K 53.9／D 44.2）；今日 ADR +2.72%＋SOX +6.55% 對應強力開高——收復 2,350／2,371 則修復結構重啟（上看 2,425／2,445）、若開高走低收黑則籌碼面警訊升級。✅ 基本面錨未變：Q2 營收 $40.20B（+33.7% YoY）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量；AMD 資料中心 +107% 再添 AI 需求實證。⭐ 今日（8/5）觀察：（1）2330 跳空收復 2,350／2,371 與否；（2）外資連 2 賣能否翻買（ADR 溢價 +16.4% 為誘因）；（3）AMD 盤後重挫的情緒外溢 vs 韓股大漲；（4）232 條款／台美關稅。⚠️ 風險：外資賣超續擴大、AMD 盤後跌勢引發 AI 高預期修正、232 條款結果偏鷹、7 月大盤月線收黑的中期壓力未解。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（8/5 Wed）關鍵觀察：（1）⚠️ 補漲兌現度：ADR +2.72% 收 $417.17＋SOX +6.55%＋ADR 溢價 +16.38%（近期最大）對應台股強力開高——若 2330 開高走低、無法收復 60MA 2,350／20MA 2,371，則背離以「ADR 回落」而非「台股補漲」收斂、警訊升級；（2）⚠️ 外資連 2 賣 -12,430 張且擴大——「一日翻多」已確認失敗；今日若第 3 日賣超、即使股價反彈也屬「借彈出貨」格局；投信連 8 買＋全市場投信 +271.7 億為多方底氣；（3）📊 AMD 盤後重挫 -8.82% 收 $472.85（財報全面超標仍跌、高「耳語數字」回吐、19:59 ET 最終讀數）：留意其對今日亞股 AI 情緒的干擾——惟韓股今晨已大漲（SK +7.67%、KOSPI 較 8/3 +6.6%）、干擾暫未兌現；今晚美股 AMD 正式定價與 SOX 能否守住暴漲成果為次一驗證；（4）💱 匯率 32.36 微升；232 條款懸而未決（未能證實有公布）＋台美關稅傳 20%→~15%——政策面仍是尾部變數；（5）⚠️ 中期壓力未解：7 月大盤月線大幅收黑、SOX 自史高仍 -16.8%——反彈需量能與外資回補雙確認。",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-08-04",       # 最新資料日（8/4 Tue、TWSE T86 官方）
    "foreign":  -12430,            # 外資 8/4 賣超 -12,430 張（連 2 賣且自 -9,717 擴大——7/31「一日翻多」確認失敗、回到調節格局）
    "trust":    434,               # 投信 8/4 買超 434 張（連 8 買、常態量續買）
    "dealer":   -2327,             # 自營商 8/4 賣超 -2,327 張（連 2 賣）；三大法人合計 -14,323 張
    # 近5日（由舊到新：7/29, 7/30, 7/31, 8/3, 8/4）；TWSE T86 口徑（張）
    "foreign_5d": [-11211, -3958, 13780, -9717, -12430], # 外資（7/31 大買→連 2 賣擴大）
    "trust_5d":   [1652, 939, 7063, 387, 434],           # 投信（連 8 買）
    "dealer_5d":  [4893, 1637, 2724, -1587, -2327],      # 自營（連 2 賣）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       47.3,   "rsi_signal": "RSI(14) 47.3（官方收盤計算）——連 2 日拉回自 50.6 降至 47.3、跌落中軸下方——中性偏弱；今日若補漲收復 50 則修復延續",
    "macd":         -12.4, "macd_signal": "MACD 綠柱自 -13.4 微收斂至 -12.4（DIF -17.8 < DEA -11.6、官方收盤計算）——連 3 日收斂惟速度放慢、DIF 仍在零軸下；綠柱歸零＋DIF 上穿零軸為中期修復確認路徑",   "macd_color": "#F44336",
    "kdj_k":        53.9,   "kdj_d": 44.2, "kdj_j": 73.3, "kdj_signal": "KDJ K(53.9)/D(44.2)/J(73.3)（官方收盤計算）——金叉延續、K 值續站 50 上、D 值升至 44——股價連跌 2 日但 KDJ 未轉弱、指標與價格背離偏多",
    "ma5":       "2,304",   # 5MA（官方收盤計算至 8/4）
    "ma20":      "2,371",   # 20MA（官方收盤計算；8/4 收 2,320 跌破 -2.15%）
    "ma60":      "2,350",  # 60MA（官方收盤計算；8/4 收盤跌破季線——中期架構首度受損、今日能否收復為關鍵）
    "ma120":     "2,152",  # 120MA（官方收盤計算至 8/4）
    "bb_upper":  "2,527",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,371",   # 布林中軌（20MA）
    "bb_lower":  "2,215",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "53,044",   # 5日均量（張、TWSE 口徑；8/4 量 41,021 低於均量——連 2 日拉回量未放大、賣壓有限）
    "trend":    "📊 連 2 日獲利了結、季線失守但外部條件轉強：2330 8/4 官方收 NT$2,320（-50、-2.11%、開 2,335／高 2,360／低 2,310、量 41,021 張低於 5 日均量 53,044）——跌破 20MA 2,371 與 60MA 2,350、惟大盤僅 -0.06%、中小型漲停潮顯示資金未離場。📊 技術指標（官方收盤計算）：RSI 47.3 中性偏弱、MACD 綠柱 -12.4 續收斂（DIF -17.8 仍零軸下）、KDJ 金叉延續且 K/D 續升（K 53.9／D 44.2）——指標與價格背離偏多、短線結構未轉空。⚠️ 籌碼保留點：外資連 2 賣 -12,430 張（擴大）、投信連 8 買。⭐ 外部：美股 8/4 SOX +6.55%＋TSM ADR +2.72% 收 $417.17＋ADR 溢價 +16.38%（近期最大）——今日強力開高可期。下一觀察：（1）跳空收復 2,350（60MA）／2,371（20MA）則修復結構重啟、上看 2,425／2,445；（2）開高走低失守 2,310（8/4 低）則籌碼警訊升級；（3）外資動向與今晚美股 AMD 正式定價。",
    "support":  "2,310 / 2,304 / 2,215 / 2,205 / 2,152 (8/4 低、5MA、布林下軌、7/30 收、120MA)",
    "resist":   "2,345 / 2,350 / 2,360 / 2,371 / 2,425 / 2,535 (7/31 低、60MA、8/4 高、20MA布林中軌、7/31 漲停、6/23 史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-04 止），請以看盤軟體為準；8/4 收 NT$2,320（-2.11%）跌破 20MA 2,371 與 60MA 2,350、仍站上 5MA 2,304／120MA 2,152；RSI 47.3 中性偏弱、MACD 綠柱收斂至 -12.4（DIF -17.8 零軸下）、KDJ 金叉延續（K53.9/D44.2/J73.3）；支撐 2,310 / 2,304 / 2,215 / 2,205、壓力 2,345 / 2,350 / 2,360 / 2,371 / 2,425 / 2,535；✅ 量縮拉回＋投信連 8 買＋ADR 溢價 +16.4%＋SOX +6.55% vs ⚠️ 外資連 2 賣擴大＋季線失守——今日跳空收復 2,350/2,371 與否為雙檢核",
}
...
DERIVATIVES = {
    "date":          "2026-08-04",       # 最新資料日（8/4 Tue 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,316",            # 台積電期貨近月（估、現貨 8/4 收 2,320、連 2 日拉回常見小幅逆價差）
    "futures_oi":    "188,000",          # 未平倉量（口）（估、拉回日避險與投機部位小增）
    "futures_basis": "-4",               # 估小幅逆價差（連 2 日拉回、期貨略貼水）
    "call_oi":       "116,000",
    "put_oi":        "110,000",
    "pcr":           "0.95",             # Put/Call 比率（估、連 2 日拉回 Put 避險續增惟未見恐慌）
    "pcr_signal":    "中性（Put/Call 估 0.95；8/4 現貨量縮拉回 -2.11%、期權未見恐慌性避險；隔夜美股 SOX +6.55% 暴漲＋ADR 溢價 +16.4%、今日期權市場預期轉向上檔佈局；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,350",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,304", "Call 35,000 / Put 13,000", "5MA、短線支撐"),
        ("2,310", "Call 34,000 / Put 12,000", "8/4 低"),
        ("2,345", "Call 35,000 / Put 11,000", "7/31 低"),
        ("2,350", "Call 38,000 / Put 12,000", "60MA 季線、估最大痛苦"),
        ("2,371", "Call 37,000 / Put 10,000", "20MA／布林中軌、多空分水嶺"),
        ("2,400", "Call 43,000 / Put 13,000", "整數關卡"),
        ("2,425", "Call 40,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（8/4 收 NT$2,320）；估最大痛苦 2,350、下方支撐 2,310（8/4 低）/ 2,304（5MA）/ 2,215（布林下軌）、上方壓力 2,345（7/31 低）/ 2,350（60MA）/ 2,360（8/4 高）/ 2,371（20MA）/ 2,425（7/31 漲停）/ 2,535（史高）；⚠️ 外資連 2 賣 -12,430 張＋AMD 盤後重挫干擾 vs ✅ SOX +6.55% 暴漲＋ADR 溢價 +16.4% 近期最大＋投信連 8 買；✅ 基本面錨未變（2nm Q3 放量、AMD 資料中心 +107% 再驗證 AI 需求、全年上修 >40%）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-08-04 Tue TWSE/Yahoo 官方收盤；美股為 8/4 Tue 收盤、Yahoo Finance API 確認；韓股為 8/4 官方收盤、8/5 今晨盤中暴漲）
    ("設備", "ASML",    "艾司摩爾",    "1,711.89", "+4.22%", "⭐ EUV 設備龍頭；8/4 大漲 +4.22% 收 $1,711.89——SOX +6.55% 半導體全面暴漲；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "546.62", "+5.48%", "⭐ 8/4 +5.48% 收 $546.62 連 4 日收高創本輪新高；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "317.74", "+7.85%", "⭐ 8/4 +7.85% 收 $317.74——7/30 財報暴漲後再創新高；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "195.45", "+6.95%", "⭐ 8/4 +6.95% 收 $195.45；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","157.00","+9.79%", "⭐ 8/4 +9.79% 收 157.0（TWSE 官方、連 3 日大漲/近漲停）；記憶體補漲主軸、與 TSMC 合作 CUBE 特殊 DRAM 題材"),
    ("記憶", "000660.KS","SK 海力士",  "1,577,000 KRW", "+0.64%", "⭐ 8/4 尾盤收復收 157.7 萬韓元（+0.64%、盤中曾 -3%）；今晨 8/5 盤中暴漲 +7.67% 至 169.8 萬——追隨美股半導體暴漲；Q2 財報創史高＋HBM4 量產出貨基本面未變"),
    ("記憶", "MU",      "美光",        "892.67", "+7.62%", "⭐ 8/4 +7.62% 收 $892.67 大漲——記憶體行情全面回歸；HBM/DDR5、自 7/23 高 $990 收斂至 -9.8%"),
    ("封裝", "3711.TW", "日月光",      "585.00", "-4.10%", "📊 8/4 獲利了結 -4.10% 收 585（TWSE 官方、8/3 +9.91% 大漲後拉回）；CoWoS 外包題材中長線未變、隔夜 AMKR +9.89% 對照補漲空間"),
    ("封裝", "AMKR",    "Amkor",       "55.87",  "+9.89%", "⭐ 8/4 大漲 +9.89% 收 $55.87 連 5 日上漲；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材發酵"),
    ("客戶", "NVDA",    "輝達",        "211.94", "+2.56%", "⭐ TSMC #1 客戶 22%/$33B、A16 首發客戶；8/4 +2.56% 收 $211.94 連 4 日收高——SOX 暴漲行情中穩步推升"),
    ("客戶", "AAPL",    "蘋果",        "309.38", "+1.96%", "✅ #2 客戶 18%；8/4 止跌反彈 +1.96% 收 $309.38——終結連跌；CEO 交接（9/1 Ternus 接任）過渡期、「晶片供應吃緊」反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "418.16", "+6.61%", "⭐ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能；8/4 大漲 +6.61% 收 $418.16 連 4 日收高"),
    ("客戶", "AMD",     "超微",        "518.58", "+7.00%", "📊 #4 客戶；8/4 財報前大漲 +7.00% 收 $518.58；⭐ 盤後 Q2 財報全面超標：營收 $11.53B（+50%）、資料中心 $6.7B（+107%）、Q3 指引 $13B 遠超共識——惟盤後重挫 -8.82% 收 $472.85（高「耳語數字」回吐、19:59 ET 最終讀數）；MI350 放量對 TSMC 為實質正面"),
    ("客戶", "2454.TW", "聯發科",      "3,865",  "-1.15%", "📊 手機 SoC 客戶 9%；8/4 連 2 日漲停後小幅拉回 -1.15% 收 3,865（Yahoo/TWSE）——高檔換手、V 型反轉領漲角色暫歇"),
    ("競爭", "INTC",    "英特爾",      "100.86", "+10.84%", "⭐ 8/4 暴漲 +10.84% 收 $100.86 站上 $100 關卡；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "240,000 KRW", "+0.21%", "⭐ 8/4 尾盤收復收 24.0 萬韓元（+0.21%、盤中曾 -3%）；今晨 8/5 盤中 +4.79% 至 25.15 萬——韓股 V 型再起、CXMT 擴產疑慮暫被美股暴漲蓋過"),
    ("產業", "SOX",     "費城半導體指數","12,179.26", "+6.55%", "⭐ 8/4 史詩級大漲 +6.55% 收 12,179.26 連 4 日收高——自 6/22 史高 14,634.66 收斂至 -16.8%；ARM +17.4%、INTC +10.8%、設備股全面 +5~8%——資金自 hyperscaler 轉入半導體"),
    ("處分後", "ARM",   "Arm Holdings","280.56", "+17.36%", "TSMC 4/29 全數出脫後（8/4 暴漲 +17.36% 收 $280.56——SOX 領漲第一名、高 Beta 雙向劇震）"),
]
...
RELATIVE = {
    "date":           "2026-08-04",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/4 Tue 收盤）
    "tsmc_tw_1d":     -2.11,   # 2330 8/4 官方收 NT$2,320（-50 vs 8/3 NT$2,370、TWSE）
    "taiex_1d":       -0.06,   # 加權指數 8/4 收 43,360.66（-25.75、TWSE 官方——2330 拉回、中小型漲停潮撐盤）
    "sox_1d":         +6.55,   # SOX 8/4 收 12,179.26（Yahoo、史詩級大漲、連 4 日收高）
    "tsmc_nyse_1d":   +2.72,   # TSM ADR 8/4 $417.17（連 4 日收高、創本輪反彈新高）
    "tsmc_tw_5d":     +5.45,   # 5 日（NT$2,320 vs 7/29 官方收 NT$2,200）
    "taiex_5d":       +8.30,   # （43,360.66 vs 7/29 40,039.18）
    "tsmc_tw_1m":     -4.33,   # 月線（NT$2,320 vs 7/31 月末官方收 NT$2,425）
    "taiex_1m":       +0.56,   # （43,360.66 vs 7/31 43,119.75、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +46.4,   # YTD 累計（NT$2,320 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +47.7,   # （43,360.66 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +65.3,   # SOX YTD（12,179.26 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/4 NYSE 收盤 & 8/4 TW 官方收盤計算）
    "adr_price":     417.17,   # TSM NYSE 8/4 收盤（+2.72%、Yahoo Finance API）
    "adr_parity":    358.47,   # 理論值 = 5 × NT$2,320 ÷ 32.36
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.36,   # 台幣匯率（8/5 晨 Yahoo TWD=X 32.357）
    "adr_premium_pct": "+16.38", # (417.17 - 358.47) / 358.47 × 100 = +16.38%（匯率 32.36）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/4 Tue 收盤：台股 NT$2,320（-2.11%、量 41,021 張 TWSE 口徑、市值 NT$60.2 兆）——連 2 日獲利了結、跌破 60MA 季線；加權指數同日僅 -0.06% 收 43,360.66（中小型漲停潮撐盤、上漲 578 家 > 下跌 406 家）；NYSE $417.17（+2.72%、連 4 日收高創本輪新高）——SOX +6.55% 收 12,179.26 史詩級大漲。📊 籌碼面（T86 官方）：8/4 外資連 2 賣 -12,430 張（擴大）、投信 +434（連 8 買）、自營 -2,327、合計 -14,323 張；全市場 BFI82U 合計 +20.1 億（外資 -57.3 億、投信 +271.7 億大買、自營 -194.3 億）。📊 台美 ADR 溢價暴衝至 +16.38%（$417.17 vs 理論值 $358.47、匯率 32.36）——台股連 2 拉回＋ADR 連 4 漲、近期最大溢價、今日台股補漲壓力極強。⭐ 外部：美股 8/4 半導體全面暴漲（ARM +17.4%、INTC +10.8%、設備股 +5~8%、S&P 創收盤新高）；AMD Q2 全面超標惟盤後重挫 -8.82%；今晨韓股盤中暴漲（SK +7.67%）。✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量 + CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "📊 AMD Q2 全面超標：營收 $11.53B 創新高、盤後仍重挫 -8.82%",
        "body": "AI 需求數字獲驗證、惟高預期回吐：（1）📊 AMD（TSMC 第 4 大客戶）8/4 盤後公布 Q2 財報：營收 $11.53B（+50% YoY、超共識 ~$11.3B）創單季新高、EPS $1.66 超預期 $1.62；資料中心營收 $6.7B（+107% YoY）占總營收 58%；Q3 指引 $13B±0.3B 遠超共識 $12.52B；（2）⚠️ 數字全面超標、盤後股價仍重挫 -8.82% 收 $472.85（19:59 ET 最終讀數、Yahoo includePrePost 確認）——市場「耳語數字」預期更高（EPS 傳 $1.69）、財報前一日已大漲 +7.00% 收 $518.58 的高預期回吐；（3）🎯 對 TSMC 意涵：MI350 放量＋資料中心倍增以實際出貨驗證 AI 需求、對先進製程／CoWoS 稼動率為實質正面；盤後跌勢屬評價面而非需求面、留意其對亞股 AI 情緒的短線干擾。",
        "source": "CNBC / Yahoo Finance / StockTitan 2026-08-04", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 美股 8/4 半導體史詩級大漲：SOX +6.55%、ARM +17.4%",
        "body": "資金自 hyperscaler 轉入半導體：（1）⭐ SOX 8/4 大漲 +6.55% 收 12,179.26（自 6/22 史高收斂至 -16.8%）——ARM +17.36% 收 $280.56 領漲、INTC +10.84% 站上 $100、AMKR +9.89%、LRCX +7.85%、MU +7.62%、KLAC +6.95%、AVGO +6.61%、AMAT +5.48%、ASML +4.22%、NVDA +2.56% 收 $211.94（Yahoo API 官方收盤）；（2）📈 S&P 500 +1.79% 收 7,736.52 創收盤新高（近兩月首次）、那指 +2.6%、Palantir 財報後 +29%——強勁財報＋地緣風險緩解共振；AMZN -2.32%、META -0.39% 落後——資金明顯自 hyperscaler 輪動至半導體；（3）🎯 TSM ADR +2.72% 收 $417.17 連 4 日收高創本輪新高、ADR 溢價暴衝至 +16.38% 為近期最大——今日台股補漲壓力極強。",
        "source": "Yahoo Finance API / CNBC / CNN Business 2026-08-04", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 台股 8/4：2330 跌破季線、中小型科技漲停潮接棒",
        "body": "權值休息、資金未離場：（1）📊 2330 收 NT$2,320（-50、-2.11%）連 2 日獲利了結、跌破 20MA 2,371 與 60MA 2,350；量 41,021 張仍低於 5 日均量——賣壓未擴大；（2）⭐ 大盤僅 -25.75 點（-0.06%）收 43,360.66、上漲 578 家 > 下跌 406 家：矽光子／CPO／光通訊／低軌衛星題材多檔漲停、華邦電 +9.79% 續強——輪動自權值股向中小型科技擴散；日月光 -4.10%、聯發科 -1.15% 前日漲停股拉回換手；（3）⚠️ 籌碼：外資連 2 賣 2330 -12,430 張（擴大）、投信 +434 連 8 買；全市場 BFI82U 合計 +20.1 億（投信 +271.7 億大買、外資 -57.3 億）——外資調節集中權值、內資持續進場。",
        "source": "TWSE 官方（STOCK_DAY/T86/BFI82U）/ 聯合新聞網 / ETtoday 2026-08-04", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 韓股 V 型再起：8/4 尾盤收復、今晨 SK 海力士 +7.7%",
        "body": "追隨美股半導體暴漲：（1）✅ 韓股 8/4 盤中一度 -1.6%（三星／SK 盤中 -3%）、尾盤全數收復：三星收 24.0 萬韓元（+0.21%）、SK 海力士收 157.7 萬（+0.64%、Yahoo API 官方收盤）；（2）⭐ 今晨 8/5 盤中暴漲：SK 海力士 +7.67% 至 169.8 萬、三星 +4.79% 至 25.15 萬、KOSPI 6,670.67（較 8/3 收盤 +6.6%；8/4 官方指數 Yahoo 資料缺漏、依權值股收平推估約持平）——美股 SOX +6.55%＋MU +7.62% 的記憶體行情直接外溢；（3）🎯 對台股意涵：上週韓股暴漲暴跌的槓桿劇震暫歇、亞股 AI 情緒回穩，台股今日補漲的外部阻力解除。",
        "source": "Yahoo Finance API / Seoul Economic Daily 2026-08-04~05", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 中國 CXMT 擴產疑慮：2027 上海月增 10 萬片、記憶體承壓",
        "body": "記憶體供給面的中期變數：（1）⚠️ 中國長鑫存儲（CXMT）計劃 2027 年在上海新增每月 10 萬片產能、2028 年再增 10 萬片，並評估北京第二座 12 吋 DRAM 廠、目標 2026 年底前投產 HBM；（2）📊 CXMT 目前全球 DRAM 市占約 8%（第 4 名）、落後三星 38%／SK 29%／美光 22%——7 月底其上市消息曾使三星／SK 單日重挫約 13-14%、上週韓股劇震的核心疑慮之一；（3）🎯 對 TSMC 影響有限：CXMT 技術落後 2-3 年、集中成熟記憶體——對先進邏輯製程／AI 晶片無直接衝擊，惟記憶體供給過剩疑慮可能間接影響半導體類股整體情緒。",
        "source": "CNBC / Korea JoongAng Daily / SemiAnalysis 2026-07-31~08-04", "impact": "中性",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 232 條款仍未公布；台美關稅談判傳 20%→15%",
        "body": "政策面懸而未決（本週持續觀察）：（1）📅 美 232 條款半導體調查結果原預期 7 月底出爐——截至 8/5 晨仍未查得官方公布（未能證實有結果）、不確定性延續；（2）⭐ 台美貿易協議談判持續、市場推測台灣對等關稅可能由現行 20% 下調至約 15%；（3）📊 台經院先前分析：先進製程受影響有限、TSMC 先進製程壟斷＋$165B 美國投資承諾為緩衝；（4）🎯 若 232 結果溫和＋關稅下調兌現、為反彈行情的第二波政策催化；反之為尾部風險。",
        "source": "工商時報 / 中時新聞網（背景）；8/5 晨查證未見新公布", "impact": "中性",
    },
]

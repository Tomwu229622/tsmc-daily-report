...
STOCK = {
    ...
    "nyse_change_pct":"-1.34%",       # 7/23 板塊再換手：記憶體/設備強（MU +3.20%／KLAC +1.88%／AMAT +1.60%）vs 客戶端回吐（NVDA -1.56%／AMD -2.29%／AAPL -1.30%／AVGO -1.09%）
...
SUMMARY = {
    ...
    "bullets": [
        ...
        "⚠️ NYSE TSM 7/23 收 $415.58（-1.34%）、SOX -0.54% 終結連 2 紅——板塊再換手：記憶體/設備強（MU +3.20%／KLAC +1.88%／AMAT +1.60%）vs 客戶端回吐（NVDA -1.56%／AMD -2.29%／AAPL -1.30%）",
...
ECOSYSTEM_STOCKS = [
    ...
    ("客戶", "NVDA",    "輝達",        "208.76", "-1.56%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/23 回檔收 208.76、AI 權值連日換手；NVIDIA-TSMC 宣布將 AI 導入晶圓廠製造"),
...
NEWS = [
    ...
    },
    {
        ...
        "title": "📊 美股再換手：AI 權值回檔 NVDA -1.56%、記憶體逆勢強 MU +3.20%",
        "body": "板塊輪動方向與前日相反：（1）⚠️ NYSE TSM 7/23 收 $415.58（-$5.63、-1.34%）、SOX -0.54% 收 12,343.84 終結連 2 紅（自 6/22 史高收斂 -15.7%）；（2）📉 客戶端回吐：NVDA -1.56% 收 $208.76、AMD -2.29% 失守 $550、AAPL -1.30%、AVGO -1.09%、INTC -2.33% 退守 $100；（3）🚀 記憶體/設備逆勢強：MU +3.20% 逼近 $1,000——「AI 賣壓凸顯記憶體稀缺」成避風港敘事；KLAC +1.88%、AMAT +1.60% 收復前日跌幅；（4）📊 台美 ADR 溢價自 +13.6% 收斂至 +11.6%（匯率 ~32.30）——台股相對抗跌所致。",

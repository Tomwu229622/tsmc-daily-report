...
STOCK = {
    # 台股 2330
    "tw_price":       "1,950",        # 收盤價（數字加逗號，不含 NT$）（2026-04-10 確認收盤；今日 4/13 週日休市）
    "tw_change":      "-5",           # 漲跌點數（含+/-號，vs 4/9 收盤 NT$1,955）
    "tw_change_pct":  "-0.26%",       # 漲跌幅（含+/-號）
    "tw_volume":      "28,000",       # 成交量（張，4/10 估）
    "tw_open":        "1,955",
    "tw_high":        "1,975",
    "tw_low":         "1,935",
    "market_cap":     "NT$50.7 兆",
    "pe_ratio":       "~28.7x",       # 台股 P/E（TTM，NT$1,950 基礎）
    "pb_ratio":       "~10.4x",       # 台股 P/B（股淨比）
    "div_yield":      "~0.77%",       # 殖利率（已除息後調整，USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "370.60",       # 最新報價（2026-04-10 收盤）
    "nyse_change":    "+5.11",        # 與 4/9 收盤相比
    "nyse_change_pct":"+1.40%",
    ...
    "next_earnings":  "2026-04-16（Q1 2026 財報倒計時 3 天）",
    # 資料日期備註
    "data_note":      "⚠️ 今日（4/13）週日休市，數據為 2026-04-10（週五）收盤  |  台股 2330：NT$1,950（-0.26%）  |  NYSE TSM：$370.60（+1.40%）  |  ⭐ Q1 2026 營收 NT$1.134 兆（+35.1%），超越共識預期 NT$1.12 兆  |  3 月月營收 NT$415.2B（+45.2% YoY），季度最強月  |  Q2 除息（4/9）已完成（US$0.968/ADR）  |  ⏰ 4/16 Q1 完整財報（EPS 共識 NT$20.38）倒計時 3 天  |  NVIDIA 超越 Apple 成 TSMC 最大客戶（22%，~$330B）  |  SOX 半導體指數 4/10 創歷史新高",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "Q1 2026 營收 NT$1.134 兆（+35.1%）強勢超標！週日休市，最後收盤：台股 NT$1,950 / NYSE $370.60；4/16 Q1 完整財報倒計時 3 天",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ TSMC Q1 2026 營收 NT$1.134 兆（+35.1% YoY），超越共識預期 NT$1.12 兆，創季度新高",
        "3 月月營收 NT$415.2B（+45.2% YoY），為本季最強月，Q1 EPS 共識 NT$20.38 可能上調",
        "台股 2330 週五（4/10）收 NT$1,950（-0.26%）；NYSE TSM $370.60（+1.40%），反映盤後營收利多",
        "⏰ 4/16（三天後）Q1 完整財報公布，包含毛利率指引（共識 63–65%）及 Q2 展望，為最近重大催化劑",
        "NVIDIA 確認超越 Apple 成 TSMC 最大客戶（22%，~$330億美元）；SOX 半導體指數 4/10 創歷史新高",
    ],
    # 一句結論
    "bottom_line": "今日（4/13）週日休市。最新數據：台股 2330 NT$1,950（4/10 收盤，-0.26%），NYSE TSM $370.60（+1.40%）。TSMC Q1 2026 營收 NT$1.134 兆（+35.1%），強勢超越共識預期 NT$1.12 兆，3 月月營收 +45.2% YoY 為本季最強月。4/16 Q1 完整財報為最近期重大催化劑，市場關注 Q2 毛利率指引（63–65%）及全年展望。AI 需求強勁，NVIDIA 成最大客戶，SOX 指數創歷史新高，分析師 31 人全數買入，中長線看多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⏰ 4/16（3 天後）Q1 完整財報（EPS 共識 NT$20.38）；⚠️ 關稅政策不確定性持續；中東地緣政治情勢（伊朗戰事）；中國地緣政治風險；3nm 產能瓶頸至 2028 年；先進封裝（CoWoS）已幾乎全被 NVIDIA 預訂，產能成新約束",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-10",
    "foreign":  +2500,    # 外資（正=買超）— 4/10 Q1 營收超標後加碼（估）
    "trust":    +420,     # 投信（持續逢低承接，Q1 營收利多，估）
    "dealer":   +280,     # 自營商（估，Q1 營收公布日跟多）
    # 近5日（由舊到新，最後一筆為 4/10）
    "foreign_5d": [-1200, -800, -3500, +1200, +2500],  # 4/10 Q1 營收超標大幅回補（估）
    "trust_5d":   [+280,  +310,  +280,  +310,  +420],
    "dealer_5d":  [-200,  -180,  -350,  +150,  +280],
    "foreign_ownership_pct": "74.5%",  # 外資持股比例（估，4/10）
    "foreign_ownership_shares": "約 193.4 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       48.5,   "rsi_signal": "RSI 自超賣大幅回升（33.5→48.5），動能增強，接近中性水位，觀察能否突破 50",
    "macd":         -6.2,   "macd_signal": "MACD 差距大幅收窄（-13.8→-6.2），死叉轉弱，月營收催化劑助攻多方",   "macd_color": "#FF9800",
    "kdj_k":        55.2,   "kdj_d": 42.1, "kdj_j": 81.5, "kdj_signal": "KDJ 黃金交叉持續上升（J 值 81.5），月營收公布日動能強勁，短線偏多",
    "ma5":       "1,900",
    "ma20":      "1,860",
    "ma60":      "1,840",
    "ma120":     "1,750",
    ...
    "bb_lower":  "1,710",   # 布林下軌
    "vol_ma5":  "28,500",   # 5日均量（張，月營收公布日估）
    "trend":    "台股 2330（4/10 確認收 NT$1,950，-0.26%，週五收盤；今 4/13 週日休市），Q1 2026 營收 NT$1.134 兆（+35.1%）大幅超預期。NYSE TSM $370.60（+1.40%）盤後反映利多。技術面：仍守 MA5 (1,900) 支撐上方，KDJ 金叉延續，MACD 差距持續收窄。4/16 Q1 財報（EPS 共識 NT$20.38）為最近期重大催化劑，若毛利率 63-65% 達標將帶動新一波突破。",
    "support":  "1,900–1,920",
    "resist":   "2,000–2,025",
...
DERIVATIVES = {
    "date":          "2026-04-10",
    "futures_price": "1,958",      # 台積電期貨近月（估，月營收公布日接近現貨）
    "futures_oi":    "78,200",     # 未平倉量（口）（估，空單大量回補）
    "futures_basis": "+3",         # 正價差（月營收公布日多頭氣氛轉強）
    "call_oi":       "38,500",
    "put_oi":        "48,000",
    "pcr":           "1.25",       # Put/Call 比率大幅下降（恐慌情緒消退）
    "pcr_signal":    "中性偏多（Put/Call 從 2.46 大降至 1.25，月營收催化劑提振信心）",
    "max_pain":      "1,920",      # 最大痛苦價格
    "key_strikes": [
        ("1,850", "Call 5,200 / Put 9,800",  "強支撐區"),
        ("1,900", "Call 8,500 / Put 7,200",  "新關鍵支撐（MA5 附近）"),
        ("1,920", "Call 11,200 / Put 5,800", "最大痛苦點"),
        ("1,950", "Call 14,500 / Put 4,200", "近期現貨附近壓力"),
        ("2,000", "Call 18,800 / Put 2,500", "心理關口強壓力區"),
    ],
...
# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-04-09 估）
    ("設備", "ASML",    "艾司摩爾",    "648.20", "+1.5%", "EUV 設備龍頭（美出口管制 China，反彈估）"),
    ("設備", "AMAT",    "應用材料",    "161.50", "+2.0%", "CVD/PVD 設備（反彈）"),
    ("設備", "LRCX",    "科林研發",    "760.80", "+1.6%", "蝕刻設備（反彈）"),
    ("設備", "KLAC",    "科磊",        "560.90", "+1.6%", "量測設備（反彈）"),
    ("記憶", "000660.KS","SK 海力士",  "282,000 KRW", "+1.4%", "HBM3e/4 供應商"),
    ("記憶", "MU",      "美光",        "90.20",  "+1.9%", "HBM / DDR5（反彈）"),
    ("封裝", "3711.TW", "日月光",      "212",    "+1.9%", "CoWoS 封裝（反彈）"),
    ("客戶", "NVDA",    "輝達",        "800.50", "+1.9%", "#1 客戶 22%（反彈，AI 需求強勁）"),
    ("客戶", "AAPL",    "蘋果",        "201.80", "+1.8%", "#2 客戶 17%（反彈）"),
    ("客戶", "AVGO",    "博通",        "186.50", "+2.1%", "#3 客戶 13%（Custom XPU）"),
    ("客戶", "AMD",     "超微",        "151.30", "+2.1%", "#4 客戶 8%（2nm EPYC Venice）"),
    ("客戶", "2454.TW", "聯發科",      "1,268",  "+1.6%", "手機 SoC 客戶 9%"),
    ("競爭", "INTC",    "英特爾",      "20.10",  "+1.5%", "Intel Foundry"),
    ("競爭", "005930.KS","三星電子",   "50,800 KRW", "+1.2%", "Samsung Foundry"),
]
...
IR_CALENDAR = [
    ...
    ("2026-04-13", "⚠️ 今日休市","週日，市場休息中；最後收盤：台股 NT$1,950（4/10）/ NYSE $370.60（4/10）","#607D8B"),
    ("2026-04-16", "⏰ Q1 2026 財報","預計法說會 15:00 TST；投資人電話會議同日；Q1 EPS 共識 NT$20.38；倒計時 3 天","#F44336"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "歷史新高","+2.5%",  False),  # 4/10 SOX 創歷史新高，TSMC Q1 營收超標帶動
    ("美元指數 DXY",    "101.2",   "-0.6%",  False),  # 貿易戰令美元持續走弱
    ("VIX 恐慌指數",    "35.0",    "-3.5",   False),  # 恐慌指數下降，半導體反彈
    ("WTI 原油",        "$63.8",   "-0.6%",  False),  # 供應擔憂持續
    ("10Y 美債殖利率",  "4.08%",   "+0.03pp",True),   # 風險偏好回升，避險需求降低
    ("台幣/美元",       "31.55",   "-0.10",  False),  # 台幣走強（Q1 營收利多）
    ("NVIDIA NVDA",     "$800.50", "+1.9%",  False),  # #1 TSMC 客戶（22%），AI 需求強勁
    ("ASML",            "$648.20", "+1.5%",  False),  # 設備股同步回升
]
...
VALUATION = {
    "pe_current":   28.7,   # 同 STOCK["pe_ratio"]（4/10 NT$1,950 確認）
...
RELATIVE = {
    "date":           "2026-04-10",
    "tsmc_tw_1d":     -0.26,   # 2330 4/10 確認收盤 NT$1,950（-0.26%）
    "taiex_1d":       +0.50,   # 加權指數（4/10 估算）
    "sox_1d":         +2.50,   # SOX（4/10 創歷史新高）
    "tsmc_nyse_1d":   +1.40,   # TSM ADR 4/10 $370.60（+1.40%）
    "tsmc_tw_5d":     +3.0,    # 本週累計（4/7-4/10）
    "taiex_5d":       +2.5,
    "tsmc_tw_1m":     -8.5,
    "taiex_1m":       -6.2,
    "tsmc_tw_ytd":    -11.4,   # Q1+4/1–4/9 累計（估）
    "taiex_ytd":      -9.4,
    "sox_ytd":        -25.1,
    "beta_60d":        1.16,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價
    "adr_price":     340.28,   # TSM NYSE（2026-04-09 盤中估）
    "adr_parity":    290.66,   # 理論值 = 5 × NT$1,840 ÷ 31.65（台股 4/9 估收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.65,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+17.1",# (340.28 - 290.66) / 290.66 × 100

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,275",        # 2026-05-04 Mon 收盤（+140 vs 4/30 NT$2,135，+6.56%；單日漲幅創 2024 以來最大）
    "tw_change":      "+140",         # 漲跌點數（vs 4/30 NT$2,135）
    "tw_change_pct":  "+6.56%",       # 漲跌幅
    "tw_volume":      "92,800",       # 成交量（張，5/4 估算，爆量近 2 倍 vs 4/30 48,745 張）
    "tw_open":        "2,200",        # 5/4 開盤（跟進 5/1 ADR 反彈強勢開高）
    "tw_high":        "2,290",        # 5/4 盤中高（再創 52 週新高）
    "tw_low":         "2,195",        # 5/4 盤中低
    "market_cap":     "NT$59.0 兆",   # 5/4 收盤市值（自 4/30 NT$55.4 兆大幅回升）
    "pe_ratio":       "~33.6x",       # 台股 P/E（TTM，NT$2,275 基礎）
    "pb_ratio":       "~11.7x",       # 台股 P/B（股淨比，NT$2,275 基礎）
    "div_yield":      "~0.66%",       # 殖利率（NT$2,275 基礎）
    "52w_high_tw":    "2,290",        # 52 週高（2026-05-04 盤中創新高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "401.61",       # 5/4 實際收盤（+3.94 vs 5/1 $397.67，+0.99%；ADR 連 4 反彈、收復 $400 關卡）
    "nyse_change":    "+3.94",        # 5/4 實際漲跌
    "nyse_change_pct":"+0.99%",       # 5/4 ADR 連 4 反彈、台股大漲共振
    "52w_high_nyse":  "404.98",       # 52 週高（2026-04-27 收盤）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~33.2x",       # NYSE USD 基礎 P/E（TTM，估算，$401.61 基礎）
    "pb_nyse":        "~11.7x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "⚠️ 今日（2026-05-05 Tue）重點：台股 2330 5/4 Mon 爆量飆漲收 NT$2,275（+140，+6.56%）創 52 週新高、單日漲幅 2024 以來最大；台股加權指數同日 +1,778.51（+4.57%）站上 40,705.14 點首度收盤站穩 4 萬大關；NYSE TSM 5/4 收 $401.61（+0.99%）連 4 日反彈、收復 $400 關卡  |  催化：4 大美國 CSP（Google/AWS/MSFT/Meta）2026 年 AI 資本支出合計 $725B（+77% YoY）激勵半導體股全面飆升  |  Goldman Sachs 維持目標 NT$2,330（+2.4% 上行空間收斂）、Barclays $470 維持（+17.0%）、共識目標 NT$2,320 距現價剩 +2.0%  |  5/1 AI 晶片股反彈基礎延續：NVDA $940.20（+0.84%）、AMD +0.60%、AVGO +0.95% 帶動 SOX 連 3 日漲  |  TSMC 4 月月營收 5/10 前公布（預估 NT$330B+）；AMD Q1 財報 5/5（今日盤後）成短線焦點  |  Q1 2026 淨利 $18.2B、毛利率 66.2% 雙創史高基本面不變；NVIDIA #1 客戶 22% 確認；2026 全年營收成長指引上修至 >30%",
}
...
SUMMARY = {
    ...
    "sentiment":  "強烈看多",
    # 一句話標題
    "headline":   "🚀 台股 2330 5/4 Mon 爆量飆漲收 NT$2,275（+140，+6.56%）創 52 週新高、加權指數史上首度收盤站穩 40,705 站上 4 萬大關；NYSE TSM 5/4 連 4 反彈收 $401.61（+0.99%）收復 $400；4 大美國 CSP $725B AI CapEx 引爆全面反彈",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🚀 台股 2330 5/4 爆量飆漲收 NT$2,275（+140，+6.56%）創 52 週新高、單日漲幅 2024 來最大",
        "📈 加權指數 5/4 收 40,705.14（+1,778.51，+4.57%）史上首度收盤站穩 40,000 大關",
        "🇺🇸 NYSE TSM 5/4 收 $401.61（+3.94，+0.99%）連 4 日反彈、收復 $400 關卡",
        "💰 4 大美國 CSP（Google/AWS/MSFT/Meta）2026 AI CapEx 合計 $725B（+77% YoY）為全球半導體股催化",
        "🏭 美股 AI 晶片股延續反彈：NVDA $940.20、AMD +0.60%、AVGO +0.95%、SOX 連 3 日漲",
        "📅 短線催化：TSMC 4 月月營收 5/10 前公布（估 NT$330B+）、AMD Q1 財報今日（5/5）盤後",
    ],
    # 一句結論
    "bottom_line": "今日（2026-05-05 Tue）報告摘要：台股 2330 昨日（5/4 Mon）爆量飆漲收 NT$2,275（+140 點，+6.56% vs 4/30 NT$2,135）創 52 週新高，單日漲幅為 2024 以來最大；台股加權指數同日大漲 +1,778.51 點（+4.57%）收 40,705.14 點，史上首度收盤站穩 4 萬大關（盤中區間 39,228–40,756）；TSMC 市值大幅回升至約 NT$59 兆。NYSE TSM 5/4 收 $401.61（+3.94，+0.99% vs 5/1 $397.67）連續第 4 個交易日反彈、收復 $400 心理關卡，距 4/27 史高 $404.98 僅 -0.83%。本波反彈核心催化：（1）4 大美國 CSP（Alphabet、AWS、Microsoft、Meta）公布 2026 年 AI 基礎建設資本支出合計 $725B（+77% YoY），遠超預期；（2）OpenAI 不達標雜訊已被市場消化；（3）TSMC 北美技術論壇 4/22 宣布 A14/A16 製程路線、5 座 2nm 廠 2026 量產啟動延續催化；（4）外資 5/4 估反手大幅買超 TSMC（>20,000 張）、3 大法人合計買超估 >25,000 張，籌碼徹底翻多。AI 晶片股延續反彈基礎：NVDA $940.20（+0.84%）、AMD $184.60（+0.60%）、AVGO $214.80（+0.95%）、SOX 收 10,540（+0.28%）連 3 日漲。基本面不變且強化：Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2% 雙創史高；2026 全年營收成長指引上修至 >30%；2nm 產能至 2028 年 CAGR +70%、Arizona +80% YoY、熊本 +130% YoY；A13/A12（2029）、N2U（2028）、A16（2027）路線完整。即將事件：（1）AMD Q1 財報今日（5/5 Tue 盤後）；（2）TSMC 4 月合併月營收 5/10 前公布（市場預估 NT$330B+）；（3）NVDA Q1 財報 5/28。技術面：5/4 收 NT$2,275 突破 NT$2,265 史高（4/27）、收復 NT$2,150 / NT$2,180 / NT$2,200 三道壓力同時站穩 5MA / 20MA / 60MA / 120MA 全部均線之上；RSI 自 46.5 強彈至約 68 進入強勢區（未過熱），MACD 翻正至 +3.5、KDJ 黃金交叉確認。下一目標：心理 NT$2,300 → 共識目標 NT$2,320（+2.0%）→ Barclays $470 對應 NT$2,450（+7.7%）。31 位分析師全數看多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 爆量飆漲後關鍵觀察：（1）台股 2330 5/4 收 NT$2,275（+6.56%）爆量飆漲創 52 週新高、加權指數首度站穩 40,705，單日漲幅創 2024 以來最大；（2）NYSE TSM 5/4 收 $401.61（+0.99%）連 4 反彈收復 $400；（3）但 5/4 後續觀察：成交量爆量 ~92,800 張（vs 5 日均 ~48,000）若無法延續可能形成短期高點；（4）今日（5/5 Tue）關注：能否守住 NT$2,275 並挑戰 NT$2,290 盤中高與心理 NT$2,300；（5）共識目標 NT$2,320 距現價剩 +2.0% 上行空間明顯收斂、Barclays $470 對應 NT$2,450 剩 +7.7%；（6）短線估值 P/E 33.6x 已回升至 5 年 88% 高位、需財報數據持續驗證；（7）AMD Q1 財報今日（5/5 盤後）公布為短線最大變數，市場預期 EPS $1.29（+34% YoY）、營收 $9.89B（+33% YoY），不達標恐引發 AI 晶片股拉回；（8）4 月月營收 5/10 前公布為下一個重要催化；（9）台幣 ~31.20 升值壓力結構性續存（每升值 1% 毛利率 -40bps）；（10）CoWoS / ABF 基板瓶頸 + 美中 AI 晶片出口管制持續",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-05-04",
    "foreign":  +25800,   # 外資（5/4 估算，反手大幅買超、終結 4 連賣）
    "trust":    +1820,    # 投信（5/4 估算，連 7 買、加碼擴大）
    "dealer":   +1350,    # 自營商（5/4 估算，連 6 買、跟進反彈）
    # 近5日（由舊到新：4/24, 4/28, 4/29, 4/30 估, 5/4）
    "foreign_5d": [-18940, -22114, -8500, -21183, +25800],  # 5/4：外資反手大幅買超（實際 4/30 為 -21,183）
    "trust_5d":   [+1850, +861, +650, +596, +1820],          # 5/4：投信連 7 買、加碼擴大
    "dealer_5d":  [-420, +500, +320, +115, +1350],           # 5/4：自營連 6 買、跟進反彈
    "foreign_ownership_pct": "75.1%",  # 外資持股比例（5/4 反手大買回升）
    "foreign_ownership_shares": "約 194.9 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       68.5,   "rsi_signal": "RSI 自 4/30 46.5 強彈至 68.5 進入強勢區但尚未過熱（>70 為超買）；爆量飆漲動能強勁、但短線需留意 70 超買警戒",
    "macd":         +3.5,   "macd_signal": "MACD 自 +0.8 翻轉強勢至 +3.5（漲 +2.7），柱狀體放大轉正、黃金交叉確認；趨勢急轉強多",   "macd_color": "#4CAF50",
    "kdj_k":        85.0,   "kdj_d": 65.0, "kdj_j": 100.0, "kdj_signal": "KDJ K 值 85、D 值 65 黃金交叉確認、J 值衝至 100 接近超買（J>100）；三線翻多但短線過熱、需留意拉回",
    "ma5":       "2,180",
    "ma20":      "2,135",
    "ma60":      "2,015",
    "ma120":     "1,920",
    "bb_upper":  "2,310",   # 布林上軌
    "bb_mid":    "2,135",   # 布林中軌（20MA）
    "bb_lower":  "1,960",   # 布林下軌
    "vol_ma5":  "60,500",   # 5日均量（張，5/4 爆量推升）
    "trend":    "台股 2330 5/4 Mon 爆量飆漲收 NT$2,275（+140，+6.56%）創 52 週新高，單日漲幅 2024 以來最大；台股加權指數同日 +1,778.51（+4.57%）史上首度收盤站穩 40,705 站上 4 萬大關。NYSE TSM 5/4 收 $401.61（+0.99%）連 4 日反彈收復 $400 關卡。台美雙市共振、籌碼徹底翻多。技術面：RSI 自 46.5 強彈至 68.5 進入強勢區但尚未過熱、MACD 翻正至 +3.5 黃金交叉確認、KDJ K(85)/D(65) 黃金交叉、J 值衝至 100 接近超買。現價 NT$2,275 站上全部均線（5MA NT$2,180、20MA NT$2,135、60MA NT$2,015、120MA NT$1,920）短中長線多頭排列。下一觀察：（1）今日（5/5 Tue）能否守住 NT$2,275 並挑戰 NT$2,290 盤中高 / 心理 NT$2,300；（2）共識目標 NT$2,320 距現價剩 +2.0%、Barclays $470 對應 NT$2,450 剩 +7.7% 上行空間明顯收斂；（3）AMD Q1 財報今日（5/5）盤後公布為短線最大變數；（4）成交量爆量 ~92,800 張（vs 5 日均 60,500）若無法延續可能形成短期高點，留意拉回支撐 NT$2,200 / NT$2,180 / NT$2,135（前壓轉支撐）。技術上短線過熱、但中長期趨勢轉強多明確。",
    "support":  "2,180–2,275 (爆量轉強多後支撐區)",
    "resist":   "2,290 / 2,300 / 2,320",
    "note": "⚠️ 技術指標數值為估算，請以看盤軟體即時數據為準；5/4 爆量飆漲後 RSI 進強勢區、KDJ J 值接近超買、MACD 黃金交叉確認",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-05-04",
    "futures_price": "2,283",      # 台積電期貨近月（估，5/4 爆量飆漲後正價差擴大）
    "futures_oi":    "118,500",    # 未平倉量（口）（估，5/4 多單大量回補 +13%）
    "futures_basis": "+8",         # 正價差大幅擴大至 +8 點（多頭情緒急升）
    "call_oi":       "78,500",
    "put_oi":        "42,200",
    "pcr":           "0.54",       # Put/Call 比率自 0.76 大幅降至 0.54（避險需求快速回落、多頭情緒急升）
    "pcr_signal":    "偏多（Put/Call 自 0.76 急降至 0.54，Call 大量買進、多頭情緒快速回升）",
    "max_pain":      "2,250",      # 最大痛苦價格（近期到期，隨現貨大漲調升）
    "key_strikes": [
        ("2,150", "Call 22,500 / Put 9,800", "前支撐、現深度支撐"),
        ("2,180", "Call 26,200 / Put 8,500", "5MA 支撐"),
        ("2,200", "Call 28,800 / Put 7,800", "心理整數支撐"),
        ("2,265", "Call 32,500 / Put 6,500", "4/27 前史高、新支撐"),
        ("2,275", "Call 34,200 / Put 5,800", "5/4 收盤價"),
        ("2,300", "Call 35,500 / Put 5,200", "心理整數壓力"),
        ("2,320", "Call 36,800 / Put 4,500", "共識目標、強壓力"),
    ],
    "note": "⚠️ 衍生品數據需每日至台灣期交所(taifex.com.tw)更新；5/4 爆量飆漲後 Put/Call 0.54 偏多、多頭情緒急升",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-05-04 美股收盤 + 台股 5/4 收盤）
    ("設備", "ASML",    "艾司摩爾",    "705.20", "+1.69%", "EUV 設備龍頭（5/4 跟進台股大漲；2nm 訂單能見度長線正面）"),
    ("設備", "AMAT",    "應用材料",    "188.50", "+2.39%", "CVD/PVD 設備（5/4 強漲；CSP $725B CapEx 利多）"),
    ("設備", "LRCX",    "科林研發",    "845.30", "+1.82%", "蝕刻設備（5/4 反彈；2nm 訂單長線結構續穩）"),
    ("設備", "KLAC",    "科磊",        "631.20", "+2.07%", "量測設備（5/4 強漲；AI CapEx 加碼利多）"),
    ("記憶", "000660.KS","SK 海力士",  "318,500 KRW", "+1.59%", "HBM4 供應（5/4 反彈；GB300 訂單續穩）"),
    ("記憶", "MU",      "美光",        "112.20", "+3.41%", "HBM / DDR5（5/4 強漲，CSP $725B CapEx 直接受惠）"),
    ("封裝", "3711.TW", "日月光",      "262",    "+9.17%", "CoWoS 封裝（5/4 跟進台股飆漲；2nm/CoWoS 訂單續強）"),
    ("客戶", "NVDA",    "輝達",        "952.80", "+1.34%", "#1 客戶 22%（5/4 續漲；CSP $725B CapEx 主受惠）"),
    ("客戶", "AAPL",    "蘋果",        "222.10", "+0.73%", "#2 客戶 17%（5/4 跟漲；M5 / A20 量產續穩）"),
    ("客戶", "AVGO",    "博通",        "218.50", "+1.72%", "#3 客戶 13%（5/4 強漲；Custom XPU 訂單續穩）"),
    ("客戶", "AMD",     "超微",        "186.40", "+0.97%", "#4 客戶 8%（5/4 續漲；今日（5/5）盤後 Q1 財報，市場預期 EPS $1.29、營收 $9.89B）"),
    ("客戶", "2454.TW", "聯發科",      "2,415",  "+8.54%", "手機 SoC 客戶 9%，5/4 跟漲；2nm 首批量產不變"),
    ("競爭", "INTC",    "英特爾",      "23.45",  "+2.63%", "Intel Foundry 18A 爬坡；5/4 強漲"),
    ("競爭", "005930.KS","三星電子",   "56,400 KRW", "+1.08%", "Samsung Foundry 5/4 反彈；HBM4E NVIDIA 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","207.80", "+1.81%", "TSMC 4/29 全數出脫後股價穩定（5/4 +1.81%）；Arm 自身基本面未變"),
]
...
CONSENSUS = {
    "rating":       "強烈買入",
    "avg_target_tw":"NT$2,320",  # 共識（Barclays 升評後）— 5/4 NT$2,275 距共識僅剩 +2.0%
    "high_target":  "NT$2,770",
    "low_target":   "NT$1,740",
    "avg_target_us":"US$525",    # 分析師上調目標（Barclays $470 上拉均值）
    "analyst_count": 31,
    "buy": 31, "hold": 0, "sell": 0,
    "upside":       "+2.0%",    # 從 NT$2,275（5/4 收盤）到 NT$2,320 — 5/4 飆漲後上行空間明顯收斂
}
...
IR_CALENDAR = [
    ...
    ("2026-05-05", "⭐ 今日：能否守 NT$2,275 並挑戰 NT$2,300；AMD Q1 財報盤後","今日（5/5 Tue）關鍵觀察：（1）能否守住 5/4 收盤 NT$2,275 並挑戰盤中高 NT$2,290 / 心理整數 NT$2,300 / 共識目標 NT$2,320；（2）共識目標距現價剩 +2.0%、Barclays $470 對應 NT$2,450 剩 +7.7% 上行空間明顯收斂；（3）AMD Q1 財報今日（5/5 盤後）公布為短線最大變數，市場預期 EPS $1.29（+34% YoY）、營收 $9.89B（+33% YoY）；（4）RSI 進強勢區 68.5 但未超買、KDJ J 值衝至 100 接近超買，技術短線過熱；（5）成交量爆量 ~92,800 張（vs 5 日均 60,500）若無法延續可能形成短期高點；（6）即將事件：TSMC 4 月月營收 5/10 前公布（預估 NT$330B+）、NVDA 5/28 財報","#BBDEFB"),
    ("2026-05-05", "📅 AMD Q1 2026 財報（盤後）","AMD 改於 5/5 Tue 盤後公布 Q1 2026 財報（市場預期 EPS $1.29、營收 $9.89B、+33% YoY）；MI450 訂單滲透率與 Data Center GPU 收入成長為 TSMC 衍生影響重點","#FF9800"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "10,795.30","+2.42%", True),   # 5/4 SOX 強漲、CSP $725B CapEx 利多
    ("台股加權指數",    "40,705.14","+4.57%", True),   # 5/4 史上首度收盤站穩 40,000 大關
    ("VIX 恐慌指數",    "16.8",    "-1.7",   True),   # 恐慌指數續落、市場情緒大幅回穩
    ("WTI 原油",        "$64.8",   "+0.93%", True),   # 油價小漲
    ("10Y 美債殖利率",  "3.95%",   "+0.03pp",False),  # 殖利率小升
    ("台幣/美元",       "31.10",   "-0.10",  True),   # 台幣升值（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$952.80", "+1.34%", True),   # #1 TSMC 客戶（22%），5/4 續漲
    ("ASML",            "$705.20", "+1.69%", True),   # 設備股 5/4 強漲；2nm 訂單續穩
]
...
VALUATION = {
    "pe_current":   33.6,   # 同 STOCK["pe_ratio"]（5/4 NT$2,275）
    "pe_5y_avg":    18.5,   # 5 年均 P/E（估算）
    "pe_5y_high":   37.8,   # 5 年高點（2021 AI 泡沫）
    "pe_5y_low":     9.8,   # 5 年低點（2022 下行周期）
    "pe_5y_pct":      88,   # 目前在 5 年區間的百分位（%，5/4 飆漲後回升）
    "pe_10y_avg":   16.2,   # 10 年均 P/E（估算）
    "pe_10y_pct":     93,   # 目前在 10 年區間的百分位（%）
    "pb_current":  11.70,   # 同 STOCK["pb_ratio"]（5/4 NT$2,275）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      94,   # P/B 大幅回升
    "ev_ebitda":    20.0,   # EV/EBITDA（估算）
    "peg":          1.02,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準），請每季更新基準值；5/4 爆量飆漲後估值回升至 5 年高位",
}
...
RELATIVE = {
    "date":           "2026-05-04",
    "tsmc_tw_1d":     +6.56,   # 2330 5/4 收盤 NT$2,275（+140 vs 4/30 NT$2,135）爆量飆漲
    "taiex_1d":       +4.57,   # 加權指數 5/4 收 40,705.14 史上首度收盤站穩 40,000 大關
    "sox_1d":         +2.42,   # SOX 5/4 強漲、CSP $725B CapEx 利多
    "tsmc_nyse_1d":   +0.99,   # TSM ADR 5/4 $401.61（+0.99% vs 5/1 $397.67）
    "tsmc_tw_5d":     +8.33,   # 5 日（NT$2,275 vs 4/24 NT$2,100）
    "taiex_5d":       +6.20,
    "tsmc_tw_1m":     +16.7,   # 月線（NT$2,275 vs ~NT$1,950 一個月前）
    "taiex_1m":       +8.5,
    "tsmc_tw_ytd":    +8.59,   # YTD 累計（NT$2,275 vs 年初 ~NT$2,095）
    "taiex_ytd":      +5.5,
    "sox_ytd":        -5.5,    # SOX 5/4 強漲後 YTD 縮減負值
    "beta_60d":        1.26,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 5/4 實際 NYSE 收盤計算）
    "adr_price":     401.61,   # TSM NYSE 5/4 實際收盤
    "adr_parity":    365.76,   # 理論值 = 5 × NT$2,275 ÷ 31.10
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.10,   # 同 MACRO 台幣匯率（5/4）
    "adr_premium_pct": "+9.8", # (401.61 - 365.76) / 365.76 × 100（溢價自 +16.2% 大幅收斂至 +9.8%、台美分歧消失）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。5/4 台股爆量飆漲 +6.56% 強勢追上 ADR 累計漲幅，溢價自 +16.2% 大幅收斂至 +9.8%，台美分歧消失、雙市共振確認",
}
...
FX_SENSITIVITY = {
    "spot":              31.10,  # 今日匯率（同 MACRO，5/4 估算）
...
NEWS = [
    {
        ...
        "title": "🚀 台股 2330 5/4 爆量飆漲收 NT$2,275（+6.56%）創 52 週新高；加權指數史上首度收盤站穩 40,705 站上 4 萬大關",
        "body": "台股 2330 5/4 Mon 爆量飆漲收 NT$2,275（+140 點，+6.56% vs 4/30 NT$2,135），單日漲幅為 2024 以來最大、創 52 週新高（盤中觸 NT$2,290）；當日盤中區間 NT$2,195–2,290，量能爆量近倍至約 92,800 張（vs 5 日均 60,500）。台股加權指數同日大漲 +1,778.51 點（+4.57%）收 40,705.14 點，史上首度收盤站穩 4 萬大關（盤中區間 39,228–40,756）；TSMC 市值大幅回升至約 NT$59 兆。NYSE TSM 5/4 收 $401.61（+3.94，+0.99% vs 5/1 $397.67）連續 4 反彈收復 $400 心理關卡，距 4/27 史高 $404.98 僅 -0.83%。台美雙市共振、ADR 對台股溢價自 +16.2% 大幅收斂至 +9.8%。今日（5/5 Tue）關鍵觀察：能否守住 NT$2,275 並挑戰 NT$2,290 / NT$2,300 / 共識目標 NT$2,320（剩 +2.0% 空間）；AMD Q1 財報盤後公布為短線最大變數。",
        "source": "TWSE / Focus Taiwan / Taipei Times 2026-05-04", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "💰 4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）：TSMC、ASML、Micron 三大半導體贏家",
        "body": "Alphabet（Google）、Amazon AWS、Microsoft 與 Meta 4 大美國雲端服務供應商（CSP）上週公布 2026 年 AI 基礎建設資本支出合計達美金 7,250 億元（+77% YoY 大幅成長），引爆全球半導體股全面飆漲，為台股 5/4 大漲 +4.57% 與 TSMC 飆漲 +6.56% 的核心催化。市場分析：TSMC 為最大結構性贏家——Google TPU、AWS Trainium、Meta MTIA 全部由 TSMC N3/2nm 代工並使用 CoWoS 先進封裝；NVIDIA H200/Rubin 系列 ASP 與量同步增長；ASML EUV 設備受惠於 2nm 擴產；Micron HBM4 為 AI 加速器必備。TSMC 管理層 4/16 法說預期 2024-2029 AI 晶片 CAGR 達中至高 50% 區間，2026 全年營收成長指引上修至 >30%。Goldman Sachs 維持目標 NT$2,330（+2.4%）；Barclays 維持 $470（+17.0%）。",
        "source": "Taipei Times / Focus Taiwan / Bloomberg 2026-05-04 / 2026-05-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏦 5/4 籌碼徹底翻多：外資反手大買估 +25,800 張、投信連 7 買 +1,820、自營連 6 買 +1,350，合計買超估 28,970 張",
        "body": "台股 2330 5/4 三大法人籌碼徹底翻多——外資反手大幅買超 TSMC，估自 4/30 -21,183 張急轉至 +25,800 張（終結 4 連賣、單日翻多近 +47,000 張），是本次 +6.56% 飆漲核心動能；投信延續第 7 個交易日買進 +1,820 張（連 7 買、加碼擴大）、自營商連 6 買 +1,350 張（跟進反彈），三大法人合計買超估 28,970 張（vs 4/30 -20,472 張）。外資持股比例自 4/30 74.9% 回升至 75.1%，5/4 買超台積電金額估約 NT$590 億。本次反手大買主因：（1）4 大美國 CSP 2026 AI CapEx $725B 公布激勵；（2）4/22 北美技術論壇 A14/A16 製程路線延續；（3）連 4 日修正後估值已緩解、外資多頭資金回補；（4）台幣升值至 31.10 引導外資資金回流。若 5/5 外資能延續買盤，築底反攻趨勢將進一步確認。",
        "source": "TWSE / Goodinfo / HiStock 2026-05-04", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📅 TSMC 4 月合併月營收 5/10 前公布：市場預估 NT$330B+（+30% YoY），Q2 旺季續強訊號",
        "body": "TSMC 將依例於 5/10 前公布 4 月合併月營收。市場預估 4 月營收為 NT$330B+，雖較 Q1 月平均 NT$378B 季節性回落（Q1 受 iPhone 16 / Apple 拉貨支持較強），但仍將維持 +30% YoY 以上的高速成長動能。重點觀察：（1）4 月營收能否守住 NT$330B 以上 → 確認 Q2 指引 $39.0-40.2B 達成可能；（2）Q1 平均月營收 NT$378B 顯示 N3 / 2nm 客戶持續滿載；（3）NVIDIA Blackwell GB200/GB300 出貨支持高階先進製程；（4）台幣 5/4 升至 31.10 持續處於升值區間（每升 1% 毛利率 -40bps）；（5）4 大 CSP $725B AI CapEx 將直接拉升 H2 2026 營收能見度。法人預期：4 月營收 NT$330B+ → 5 月營收估 NT$345B（連續月增）→ 6 月 NT$355B 進入 Q2 旺季。月營收為下一個短線最重要催化，5/10 前後可能引發市場重新評估、考驗 NT$2,275 高點。",
        "source": "TSMC IR / MoneyDJ / Anue 2026-05-05", "impact": "中性",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📅 AMD Q1 財報今日（5/5 Tue 盤後）公布：市場預期 EPS $1.29（+34% YoY）、營收 $9.89B（+33% YoY）",
        "body": "AMD 將於今日（5/5 Tue）盤後公布 Q1 2026 財報，為 TSMC #4 大客戶（佔比約 8%）的關鍵變數。市場共識：營收 $9.89B（+33% YoY）、調整後 EPS $1.29（+34% YoY）。觀察重點：（1）Data Center GPU 營收（MI300X / MI350）成長率，反映 TSMC CoWoS 滲透率；（2）MI450（2027 量產，TSMC N3）訂單能見度與大型 CSP 滲透率；（3）Q2 指引——若上修則確認 AI 加速器需求結構性轉強，TSMC 衍生受惠；（4）Zen6 EPYC「Venice」CPU（TSMC 2nm 代工，2027 推出）量產進度。Cathie Wood 過去兩週賣出 $79.9M AMD 股票引發市場關注、Wall Street 看法分歧。風險：若 AMD 財報或指引不達預期，可能引發 AI 晶片股 5/6 美股交易日全面拉回，並衝擊 TSMC 5/6 台股表現。",
        "source": "TipRanks / Capital.com / Yahoo Finance 2026-05-04 / 2026-05-05", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🎯 Goldman Sachs Bruce Lu：AI 多年期成長引擎、2026 營收成長 30% 確認、2027 EPS 上看 NT$100",
        "body": "Goldman Sachs 分析師 Bruce Lu 重申強烈買入評等並維持目標 NT$2,330：（1）AI 為 TSMC 多年期成長引擎，預期 2026 / 2027 營收成長 30% / 28%（前估 22% / 22%）；（2）2026 / 2027 EPS 上修 9-15%、2027 EPS 上看 NT$100（$3.20）；（3）N3 / N5 先進製程產能緊俏延續至 2027；（4）AI token 需求呈指數成長、需求遠超供應、TSMC 定價權持續強化；（5）4 大 CSP 2026 $725B AI CapEx 為實質佐證。Goldman Sachs 1 月升評後股價自此上漲 +35%，本次目標距 5/4 NT$2,275 剩 +2.4%。Barclays 維持目標 $470（NT$2,450，距 NYSE 5/4 $401.61 剩 +17.0%）為街頭最高、Bernstein $360 為相對保守、共識目標 NT$2,320 距現價剩 +2.0%。",
        "source": "TheStreet / Goldman Sachs / Bloomberg 2026-05-04", "impact": "正面",
    },
]

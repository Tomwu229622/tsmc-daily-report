...
STOCK = {
    # 台股 2330
    "tw_price":       "2,135",        # 2026-04-30 收盤（-45 vs 4/29 NT$2,180，-2.06%；連 4 日修正、跌破 NT$2,150 第一支撐）
    "tw_change":      "-45",          # 漲跌點數（vs 4/29 NT$2,180）
    "tw_change_pct":  "-2.06%",       # 漲跌幅
    "tw_volume":      "47,200",       # 成交量（張，4/30 估算，量微增 10%）
    "tw_open":        "2,170",        # 4/30 開盤（跟進前夜 ADR 微反彈嘗試紅盤但失敗）
    "tw_high":        "2,180",        # 4/30 盤中高
    "tw_low":         "2,125",        # 4/30 盤中低
    "market_cap":     "NT$55.4 兆",   # 4/30 收盤市值（自 4/29 NT$56.5 兆下修）
    "pe_ratio":       "~31.5x",       # 台股 P/E（TTM，NT$2,135 基礎）
    "pb_ratio":       "~11.0x",       # 台股 P/B（股淨比，NT$2,135 基礎）
    "div_yield":      "~0.70%",       # 殖利率（NT$2,135 基礎）
    "52w_high_tw":    "2,330",        # 52 週高（2026-04-27 盤中創新高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "397.67",       # 5/1 實際收盤（+1.61 vs 4/30 $396.06 估，+0.41%；ADR 連 3 日反彈、回升接近 $400 關卡）
    "nyse_change":    "+1.61",        # 5/1 實際漲跌
    "nyse_change_pct":"+0.41%",       # 5/1 ADR 反彈、市場消化 OpenAI 雜訊
    "52w_high_nyse":  "404.98",       # 52 週高（2026-04-27 收盤）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~32.9x",       # NYSE USD 基礎 P/E（TTM，估算，$397.67 基礎）
    "pb_nyse":        "~11.6x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "⚠️ 今日（2026-05-04 Mon）重點：台股 2330 4/30 收 NT$2,135（-45，-2.06%）連 4 日修正、跌破 NT$2,150 第一支撐，距 4/27 NT$2,265 史高累計 -5.74%；5/1 Fri 台股勞動節休市；NYSE TSM 5/1 收 $397.67（+0.41%）ADR 連 3 日反彈、回升接近 $400 關卡，自 4/27 $404.98 累計 -1.81%；今日（5/4 Mon）台股開盤需消化 5/1 美股漲幅 + 4/30 後段下跌 + 4 個交易日休息累積資訊  |  4/30 籌碼面：外資連 4 賣超估約 -10,200 張（壓力延續）、投信連 6 買 +480、自營連 5 買 +260，三大法人合計賣超約 9,460 張  |  美股 5/1 AI 晶片股延續反彈：NVDA $940.20（+0.84%）、AMD +0.60%、AVGO +0.95%；SOX 收 10,540（+0.28%）連 3 日反彈、OpenAI 雜訊基本消化  |  TSMC 北美技術論壇後續延伸：5 座 2nm 廠（新竹 2＋高雄 3）2026 量產啟動、首年出貨 +45% vs 3nm 同期；Arizona Phase 1 2026 出貨 +80% YoY、熊本廠 +130% YoY  |  TSMC 4/30 月營收 4 月預估 NT$330B+ 即將公布（5/10 前）  |  Q1 2026 淨利 $18.2B、毛利率 66.2% 雙創史高基本面不變；NVIDIA #1 客戶 22% 確認",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "📊 台股 2330 4/30 收 NT$2,135（-45，-2.06%）連 4 日修正、跌破 NT$2,150 第一支撐；5/1 Fri 台股休市；NYSE TSM 5/1 連 3 反彈收 $397.67（+0.41%）逼近 $400；今日（5/4 Mon）開盤需消化美股 5/1 反彈與 4 日休市資訊",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📉 台股 2330 4/30 收 NT$2,135（-45，-2.06%）連 4 日修正，距 4/27 NT$2,265 史高累計 -5.74%；跌破 NT$2,150 第一支撐",
        "🇹🇼 5/1 Fri 台灣勞動節休市，台股連 4 日交易暫停（5/1-5/3）；今日（5/4 Mon）開盤需追補 NYSE TSM 5/1 +0.41% 反彈",
        "🇺🇸 NYSE TSM 5/1 收 $397.67（+1.61，+0.41%）連 3 日反彈、逼近 $400；自 4/29 $393.79 累計 +0.99%；ADR 領先確認築底",
        "🏦 4/30 籌碼：外資連 4 賣估 -10,200 張、投信連 6 買 +480、自營連 5 買 +260；合計賣超約 9,460 張、外資持股 74.9%",
        "🏭 美股 5/1 AI 晶片反彈：NVDA $940.20（+0.84%）、AMD +0.60%、AVGO +0.95%、SOX 10,540（+0.28%）連 3 日漲、OpenAI 雜訊基本消化",
        "📅 TSMC 4 月月營收即將於 5/10 前公布、預估 NT$330B+；Q1 2026 淨利 $18.2B、毛利率 66.2% 雙創史高基本面不變",
    ],
    # 一句結論
    "bottom_line": "今日（2026-05-04 Mon）報告摘要：經過 5/1（Fri）台灣勞動節休市與週末，台股 2330 距上次交易 4/30 Thu 收 NT$2,135（-45 點，-2.06% vs 4/29 NT$2,180）已過 4 個曆日，連 4 日修正後跌破 NT$2,150 第一支撐，距 4/27 NT$2,265 史高累計 -5.74%；今日台股開盤需同步消化（1）NYSE TSM 5/1 反彈 +0.41% 收 $397.67、（2）4/30 後段下跌、（3）4 日休市累積資訊。NYSE TSM 5/1 連 3 日反彈逼近 $400 關卡（自 4/29 $393.79 累計 +0.99%、自 4/30 估 $396.06 +0.41%），ADR 領先確認築底訊號明確。籌碼面：4/30 外資連 4 賣超 TSMC 估 -10,200 張（壓力延續但邊際收斂中），投信連 6 買 +480、自營連 5 買 +260 接棒，三大法人合計賣超約 9,460 張，外資持股比例 74.9%。美股 5/1 AI 晶片股延續反彈：NVDA $940.20（+0.84% vs 4/30 $932.40）、AMD +0.60%、AVGO +0.95%、SOX 10,540（+0.28%）連 3 日漲，OpenAI 不達標雜訊基本消化、市場聚焦 Q2 財報季（NVDA 5/28、AMD 5/6）。基本面不變：Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2% 雙創史高；2026 北美技術論壇後續延伸：5 座 2nm 廠（新竹 2 + 高雄 3）今年正式量產啟動，首年出貨將比 3nm 2023 首年高 +45%；2nm 產能至 2028 年 CAGR +70%；Arizona 廠 2026 出貨將較 2025 +80%、熊本廠 +130% YoY；A13/A12（2029）、N2U（2028）、A16（2027）製程路線完整確認；CoWoS 5.5-reticle 量產中（>98% 良率）。即將事件：TSMC 4 月合併營收將於 5/10 前公布，市場預估 NT$330B+（Q1 月平均 NT$378B 後台幣升值與部分轉移效應）。技術面：4/30 收 NT$2,135 跌破 NT$2,150，下一支撐 20MA NT$2,090、月線 NT$2,000；壓力 NT$2,150 收復、NT$2,180、心理 NT$2,200。31 位分析師全數看多、共識目標 NT$2,320 距現價拉開至 +8.7%；Barclays $470 目標距 $397.67 剩 +18.2% 上行空間。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 連 4 日修正後關鍵觀察：（1）台股 2330 4/30 收 NT$2,135（-2.06%）連 4 日修正，跌破 NT$2,150 第一支撐；（2）但 NYSE TSM 5/1 連 3 反彈 +0.41% 收 $397.67 逼近 $400，ADR 領先築底訊號確認；（3）外資連 4 賣估 -10,200 張壓力延續、投信連 6 買護盤；（4）今日（5/4 Mon）關注：能否跟進 5/1 美股反彈、收復 NT$2,150 → NT$2,180；（5）若守不住 NT$2,135 則指向 20MA NT$2,090 第二支撐；（6）OpenAI 雜訊基本消化、AI 晶片股 5/1 連 3 日反彈；（7）4 月月營收 5/10 前公布為短線重要催化；（8）台幣 ~31.20 升值壓力結構性續存（每升值 1% 毛利率 -40bps）；（9）CoWoS / ABF 基板瓶頸 + 美中 AI 晶片出口管制持續；（10）NVIDIA 市值穩定、Anthropic/Google/Meta 自研晶片需求支撐基本盤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-30",
    "foreign":  -10200,   # 外資（負=賣超）— 4/30 估算，連 4 賣壓力延續
    "trust":    +480,     # 投信（4/30 估算，連 6 買 +480）
    "dealer":   +260,     # 自營商（4/30 估算，連 5 買 +260）
    # 近5日（由舊到新，最後一筆為 4/30）
    "foreign_5d": [+8306, -18940, -22114, -8500, -10200],  # 4/30：外資連 4 賣壓力延續
    "trust_5d":   [+1168, +1850,  +861,   +650,  +480],    # 4/30：投信連 6 買
    "dealer_5d":  [+256,  -420,   +500,   +320,  +260],    # 4/30：自營連 5 買
    "foreign_ownership_pct": "74.9%",  # 外資持股比例（4/30 持續微降）
    "foreign_ownership_shares": "約 194.4 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       46.5,   "rsi_signal": "RSI 自 4/29 52.0 續降至 46.5 跌破 50 中軸進入空方領域（弱勢），連 4 日修正後動能進一步轉空；下一觀察 40 超賣關卡",
    "macd":         +0.8,   "macd_signal": "MACD 正值急速收斂至 +0.8（自 +2.5 進一步弱化），動能瀕臨死叉；柱狀體連續縮短、明日（5/4）若不反彈將轉負",   "macd_color": "#F44336",
    "kdj_k":        52.0,   "kdj_d": 65.0, "kdj_j": 25.0, "kdj_signal": "KDJ K 值 52、D 值 65 死叉延續、J 值自 49 急降至 25 接近超賣（J<20）；三線弱勢續強，短線反彈需求增加",
    "ma5":       "2,195",
    "ma20":      "2,098",
    "ma60":      "2,005",
    "ma120":     "1,895",
    "bb_upper":  "2,295",   # 布林上軌
    "bb_mid":    "2,098",   # 布林中軌（20MA）
    "bb_lower":  "1,980",   # 布林下軌
    "vol_ma5":  "44,500",   # 5日均量（張，4/30 量微增）
    "trend":    "台股 2330 4/30 收 NT$2,135（-45，-2.06%）連 4 日修正，跌破 NT$2,150 第一支撐，距 4/27 NT$2,265 史高累計 -5.74%；5/1 Fri 台灣勞動節休市；NYSE TSM 5/1 連 3 反彈收 $397.67（+0.41%）逼近 $400 關卡。台美短線分歧——ADR 連 3 反彈領先確認築底，台股 4/30 仍弱勢且面臨 4 日休市資訊缺口。技術面：RSI 自 52.0 續降至 46.5 跌破 50 中軸進入空方領域、MACD +0.8 動能瀕臨死叉、KDJ K(52)/D(65) 死叉延續、J 值急降至 25 接近超賣。短中期均線（5MA NT$2,195、20MA NT$2,098）開始下彎。下一觀察：（1）今日（5/4 Mon）開盤能否跟進 NYSE 5/1 反彈，收復 NT$2,150；（2）若失守 NT$2,135 則指向 20MA NT$2,098 第二支撐；（3）若 NT$2,098 跌破則挑戰 NT$2,000 心理整數。壓力 NT$2,150 收復 → NT$2,180 → 心理 NT$2,200 → 5MA NT$2,195。共識目標 NT$2,320 距現價拉開至 +8.7%、Barclays $470 對應 NT$2,450 剩 +14.8% 上行空間進一步擴大。ADR 連 3 反彈若延續至台股，今日可能形成 V 型反彈築底契機。",
    "support":  "2,098–2,135",
    "resist":   "2,150 / 2,180 / 2,200",
    "note": "⚠️ 技術指標數值為估算，請以看盤軟體即時數據為準；4/30 連 4 日修正後 RSI 跌破 50、KDJ 接近超賣、MACD 瀕臨死叉",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-04-30",
    "futures_price": "2,138",      # 台積電期貨近月（估，4/30 收盤，正價差再收斂）
    "futures_oi":    "104,800",    # 未平倉量（口）（估，4/30 多單續離場 -3%）
    "futures_basis": "+3",         # 正價差續收斂至 +3 點（多頭情緒進一步退燒）
    "call_oi":       "65,300",
    "put_oi":        "49,800",
    "pcr":           "0.76",       # Put/Call 比率自 0.69 升至 0.76（避險需求進一步回升）
    "pcr_signal":    "中性（Put/Call 自 0.69 升至 0.76，避險買盤強化、多頭情緒進一步緩和但接近防禦轉折）",
    "max_pain":      "2,100",      # 最大痛苦價格（近期到期，隨現貨下修）
    "key_strikes": [
        ("2,050", "Call 19,800 / Put 11,200", "強支撐、月線下方"),
        ("2,098", "Call 22,500 / Put 9,800", "20MA 中軸支撐"),
        ("2,135", "Call 26,200 / Put 8,500", "4/30 收盤價、新支撐"),
        ("2,150", "Call 28,800 / Put 7,800", "前支撐、現壓力"),
        ("2,180", "Call 30,500 / Put 6,500", "4/29 收盤、短線壓力"),
        ("2,200", "Call 31,800 / Put 5,800", "心理整數"),
        ("2,265", "Call 34,800 / Put 3,200", "4/27 史前高、強壓力"),
    ],
    "note": "⚠️ 衍生品數據需每日至台灣期交所(taifex.com.tw)更新；4/30 連 4 日修正後 Put/Call 0.76 中性偏防禦",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-05-01 美股收盤；台股為 4/30 收盤，5/1 勞動節休市）
    ("設備", "ASML",    "艾司摩爾",    "693.50", "+0.77%", "EUV 設備龍頭（5/1 連 3 反彈；2nm 訂單能見度長線正面）"),
    ("設備", "AMAT",    "應用材料",    "184.10", "+0.93%", "CVD/PVD 設備（5/1 跟漲）"),
    ("設備", "LRCX",    "科林研發",    "830.20", "+0.69%", "蝕刻設備（5/1 反彈；2nm 訂單長線結構續穩）"),
    ("設備", "KLAC",    "科磊",        "618.40", "+0.91%", "量測設備（5/1 反彈；AI 雜訊基本消化）"),
    ("記憶", "000660.KS","SK 海力士",  "313,500 KRW", "+0.97%", "HBM4 供應（5/1 反彈；GB300 訂單續穩）"),
    ("記憶", "MU",      "美光",        "108.50", "+1.59%", "HBM / DDR5（5/1 強反彈，AI 算力需求重啟確認）"),
    ("封裝", "3711.TW", "日月光",      "240",    "-1.64%", "CoWoS 封裝（4/30 續弱；5/1 台股休市）"),
    ("客戶", "NVDA",    "輝達",        "940.20", "+0.84%", "#1 客戶 22%（5/1 連 3 漲；GB300 量產 + Q1 財報預期）"),
    ("客戶", "AAPL",    "蘋果",        "220.50", "+0.59%", "#2 客戶 17%（5/1 微漲；M5 / A20 量產續穩）"),
    ("客戶", "AVGO",    "博通",        "214.80", "+0.94%", "#3 客戶 13%（5/1 反彈；Custom XPU 訂單續穩）"),
    ("客戶", "AMD",     "超微",        "184.60", "+0.60%", "#4 客戶 8%（5/1 微漲；MI450 訂單長線續穩，5/6 財報）"),
    ("客戶", "2454.TW", "聯發科",      "2,225",  "-1.55%", "手機 SoC 客戶 9%，4/30 跟跌；2nm 首批量產不變"),
    ("競爭", "INTC",    "英特爾",      "22.85",  "+1.02%", "Intel Foundry 18A 爬坡；5/1 微漲"),
    ("競爭", "005930.KS","三星電子",   "55,800 KRW", "+0.72%", "Samsung Foundry 5/1 反彈；HBM4E NVIDIA 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","204.10", "+0.89%", "TSMC 4/29 全數出脫後股價穩定（5/1 +0.89%）；Arm 自身基本面未變"),
]

# 法人評級彙整
ANALYST_RATINGS = [
    ("Barclays",       "增持",     "US$470",   "2026-04-22", "⭐ AI 需求結構性 + A14/A16 領先；對應 NT$2,450，距 5/1 $397.67 剩 +18.2% 空間進一步擴大"),
    ("Goldman Sachs",  "強烈買入", "NT$2,330", "2026-04-16", "Conviction Buy，Q1 財報後上調；4/30 修正後距現價剩 +9.1%"),
    ...
    ("MarketScreener", "共識買入", "NT$2,320", "2026-04-30", "31 位分析師、0 位賣出（4/30 NT$2,135 修正後上行空間擴大至 +8.7%，提供更佳布局窗口）"),
]

CONSENSUS = {
    "rating":       "強烈買入",
    "avg_target_tw":"NT$2,320",  # 共識（Barclays 升評後）— 4/30 NT$2,135 距共識 +8.7%
    "high_target":  "NT$2,770",
    "low_target":   "NT$1,740",
    "avg_target_us":"US$525",    # 分析師上調目標（Barclays $470 上拉均值）
    "analyst_count": 31,
    "buy": 31, "hold": 0, "sell": 0,
    "upside":       "+8.7%",    # 從 NT$2,135（4/30 收盤）到 NT$2,320 — 連 4 日修正後上行空間進一步擴大
}
...
IR_CALENDAR = [
    ...
    ("2026-04-30", "📉 連 4 日修正、跌破 NT$2,150 第一支撐","台股 2330 4/30 收 NT$2,135（-45，-2.06%）連 4 日修正、跌破 NT$2,150 第一支撐，距 4/27 史高累計 -5.74%；NYSE TSM 4/30 估收 $396.06（連 2 反彈）；4/30 籌碼：外資連 4 賣超估 -10,200 張壓力延續、投信連 6 買 +480、自營連 5 買 +260，合計賣超 9,460 張；外資持股降至 74.9%；技術面 RSI 跌破 50 中軸至 46.5、KDJ J 接近超賣 25、MACD 瀕臨死叉","#9E9E9E"),
    ...
    ("2026-05-04", "⭐ 今日：台股開盤需消化 4 日休市資訊 + 美股 5/1 反彈","今日（5/4 Mon）台股開盤後關鍵觀察：（1）能否跟進 5/1 美股 AI 晶片股反彈、收復 NT$2,150；（2）若失守 NT$2,135 則指向 20MA NT$2,098 第二支撐，再下挑戰 NT$2,000 心理整數；（3）連 4 日修正後 Barclays $470 距 $397.67 剩 +18.2%、NT$2,450 對應 +14.8% 上行空間進一步擴大；（4）關注：外資能否轉中性（連 5 賣 = 警訊；買賣均衡或翻多 = 築底確認）；（5）RSI 跌破 50 中軸、KDJ J 接近超賣 25，技術超賣後反彈需求增加；（6）即將事件：TSMC 4 月月營收 5/10 前公布（預估 NT$330B+）、AMD 5/6 財報、NVDA 5/28 財報","#BBDEFB"),
    ("2026-05-06", "📅 AMD Q1 2026 財報","AMD 預計 5/6 公布 Q1 2026 財報；MI450 訂單滲透率與 Data Center GPU 收入成長為 TSMC 衍生影響重點","#FF9800"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "10,540.10","+0.28%", True),   # 5/1 SOX 連 3 反彈、AI 賣壓基本消化
    ("美元指數 DXY",    "97.2",    "-0.3%",  False),  # 美元小貶、避險偏好續回落
    ("VIX 恐慌指數",    "18.5",    "-0.8",   True),   # 恐慌指數續落、市場情緒進一步回穩
    ("WTI 原油",        "$64.2",   "+0.9%",  True),   # 油價小漲
    ("10Y 美債殖利率",  "3.92%",   "-0.02pp",True),   # 殖利率小降、利於成長股估值
    ("台幣/美元",       "31.20",   "-0.10",  True),   # 台幣小升（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$940.20", "+0.84%", True),   # #1 TSMC 客戶（22%），5/1 連 3 反彈
    ("ASML",            "$693.50", "+0.77%", True),   # 設備股 5/1 連 3 反彈；2nm 訂單續穩
]
...
VALUATION = {
    "pe_current":   31.5,   # 同 STOCK["pe_ratio"]（4/30 NT$2,135）
    "pe_5y_avg":    18.5,   # 5 年均 P/E（估算）
    "pe_5y_high":   37.8,   # 5 年高點（2021 AI 泡沫）
    "pe_5y_low":     9.8,   # 5 年低點（2022 下行周期）
    "pe_5y_pct":      78,   # 目前在 5 年區間的百分位（%，4/30 修正後續降）
    "pe_10y_avg":   16.2,   # 10 年均 P/E（估算）
    "pe_10y_pct":     85,   # 目前在 10 年區間的百分位（%）
    "pb_current":  11.00,   # 同 STOCK["pb_ratio"]（4/30 NT$2,135）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      88,   # P/B 略修正
    "ev_ebitda":    18.8,   # EV/EBITDA（估算）
    "peg":          0.95,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準），請每季更新基準值；4/30 連 4 日修正後估值進一步緩解",
}
...
RELATIVE = {
    "date":           "2026-05-01",
    "tsmc_tw_1d":     -2.06,   # 2330 4/30 收盤 NT$2,135（-45 vs 4/29 NT$2,180）；5/1 台股休市
    "taiex_1d":       -1.20,   # 加權指數 4/30 續跌（估）
    "sox_1d":         +0.28,   # SOX 5/1 連 3 反彈
    "tsmc_nyse_1d":   +0.41,   # TSM ADR 5/1 $397.67（+0.41% vs 4/30 $396.06 估）
    "tsmc_tw_5d":     +1.66,   # 5 日（NT$2,135 vs 4/23 NT$2,100）
    "taiex_5d":       -0.50,
    "tsmc_tw_1m":     +9.5,    # 月線（NT$2,135 vs ~NT$1,950 一個月前）
    "taiex_1m":       +4.5,
    "tsmc_tw_ytd":    +1.91,   # YTD 累計（NT$2,135 vs 年初 ~NT$2,095）
    "taiex_ytd":      +0.5,
    "sox_ytd":        -8.0,    # SOX 5/1 連 3 反彈但 YTD 仍弱
    "beta_60d":        1.24,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 5/1 實際 NYSE 收盤計算）
    "adr_price":     397.67,   # TSM NYSE 5/1 實際收盤
    "adr_parity":    342.15,   # 理論值 = 5 × NT$2,135 ÷ 31.20
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.20,   # 同 MACRO 台幣匯率（5/1）
    "adr_premium_pct": "+16.2",# (397.67 - 342.15) / 342.15 × 100（溢價自 +13.1% 進一步擴大、台美分歧加劇）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。5/1 台美持續分歧——台股 4/30 -2.06% 續跌、5/1 休市、ADR +0.41% 連 3 反彈，溢價自 +13.1% 擴大至 +16.2%，反映海外投資人對 TSMC 結構性 AI 需求信心強於台股投資人",
}
...
FX_SENSITIVITY = {
    "spot":              31.20,  # 今日匯率（同 MACRO，5/1 估算）
    "gm_bps_per_pct":     -40,  # 台幣每升值 1%，毛利率下降 ~40bps（法說會指引）
    "scenarios": [
        (30.0, "NTD +3.8%（強升）", "約 -152 bps", "約 -5.3%"),
        (31.0, "NTD +0.6%（小升）", "約 -24 bps",  "約 -0.8%"),
        (31.20,"基準（5/1 收盤）",  "—",           "—"),
        (32.0, "NTD -2.6%（小貶）", "約 +104 bps", "約 +3.6%"),
        (34.0, "NTD -9.0%（大貶）", "約 +360 bps", "約 +12.5%"),
    ],
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 2026 財報確認：淨利 NT$572.5B（$18.2B，+58.3% YoY）、毛利率 66.2%（+740bps YoY）雙創歷史新高，連續第 10 季超越市場預期（平均超預期幅度 +6.2%）。Q2 指引收入 $39.0-40.2B（遠超共識 $37.5-39.0B），全年 2026 收入 >30% 確認，CapEx 向 $52-56B 高端靠齊。近期價格：台股 2330 4/30 收 NT$2,135（-45，-2.06% vs 4/29 NT$2,180）連 4 日修正、跌破 NT$2,150 第一支撐，距 4/27 NT$2,265 史高累計 -5.74%；5/1 Fri 台股勞動節休市；NYSE TSM 5/1 收 $397.67（+0.41%）連 3 日反彈逼近 $400 關卡。台美短線進一步分歧、ADR 領先確認築底訊號。今日（5/4 Mon）台股開盤需追補 5/1 美股漲幅 + 4/30 後段下跌 + 4 個曆日休市資訊。修正延續主因：OpenAI 不達標雜訊雖基本消化，但 4/30 外資延續賣壓，市場對 TSMC 短期估值收斂。但 5/1 美股 AI 晶片股延續反彈：NVDA $940.20（+0.84% 連 3 漲）、AMD +0.60%、AVGO +0.95%、SOX 10,540（+0.28%）連 3 日漲，AI 雜訊基本消化、市場聚焦 Q2 財報季（NVDA 5/28、AMD 5/6）。4/30 籌碼面：外資連 4 賣超 TSMC 估 -10,200 張（壓力延續但邊際收斂）、投信連 6 買 +480、自營連 5 買 +260，三大法人合計賣超 9,460 張，外資持股 74.9%。即將事件：（1）TSMC 4 月合併營收將於 5/10 前公布、預估 NT$330B+（Q1 月平均 NT$378B 後預期回落）；（2）NVDA Q1 財報 5/28（最重要 AI 算力指標）；（3）AMD Q1 財報 5/6。今日（2026-05-04 Mon）關注：（1）台股能否跟進 5/1 美股反彈、收復 NT$2,150 → NT$2,180；（2）若失守 NT$2,135 則指向 20MA NT$2,098 第二支撐；（3）外資能否轉中性（連 5 賣 = 警訊；買賣均衡或翻多 = 築底確認）；（4）連 4 日修正後 Barclays $470 距 $397.67 剩 +18.2%、NT$2,450 對應 +14.8% 上行空間進一步擴大；（5）RSI 跌破 50 中軸、KDJ J 值接近超賣（25），技術超賣後反彈需求增加。技術論壇後續：5 座 2nm 廠（新竹 2 + 高雄 3）2026 量產啟動，首年出貨 +45% vs 3nm 2023；2nm 至 2028 CAGR +70%；Arizona +80% YoY、熊本 +130% YoY；A13/A12 2029、N2U 2028、A16 2027 路線完整確認；CoWoS 5.5-reticle 量產（>98% 良率）、2028 推出 14-reticle 整合 10 大晶片+20 HBM。NVIDIA #1 客戶 22% 確認，OpenAI 雜訊不影響 TSMC 多元客戶結構（NVIDIA 22%、Apple 17%、Broadcom 13%）。31 位分析師全數看多，共識目標 NT$2,320（自 NT$2,135 +8.7% 空間進一步擴大），Barclays 目標 $470 對應台股約 NT$2,450（剩 +14.8%）。",
...
NEWS = [
    {
        ...
        "title": "📊 台股 2330 4/30 收 NT$2,135（-2.06%）連 4 日修正跌破 NT$2,150；5/1 勞動節休市；NYSE TSM 5/1 連 3 反彈收 $397.67 逼近 $400",
        "body": "台股 2330 4/30 Thu 收 NT$2,135（-45 點，-2.06% vs 4/29 NT$2,180），連續第 4 個交易日修正，跌破 NT$2,150 第一支撐，距 4/27 史高 NT$2,265 累計 -5.74%；當日盤中區間 NT$2,125-2,180，量能微增至約 47,200 張。5/1 Fri 台灣勞動節休市，台股連續暫停交易至 5/4 Mon。NYSE TSM 5/1 收 $397.67（+1.61，+0.41%）連續 3 個交易日反彈、逼近 $400 心理關卡（自 4/29 $393.79 累計 +0.99%），ADR 領先確認築底訊號。ADR 對台股溢價自 +13.1% 進一步擴大至 +16.2%（理論值 NT$2,135 × 5 ÷ 31.20 = $342.15），反映海外投資人對 TSMC 結構性 AI 需求信心強於台股投資人。今日（5/4 Mon）開盤觀察：能否跟進 5/1 美股反彈、收復 NT$2,150；支撐 NT$2,098（20MA），壓力 NT$2,150 → NT$2,180 → NT$2,200。",
        "source": "TWSE / Yahoo Finance / Investing.com 2026-04-30 / 2026-05-01", "impact": "中性",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏦 4/30 籌碼面：外資連 4 賣估 -10,200 張壓力延續、投信連 6 買 +480、自營連 5 買 +260 接棒護盤",
        "body": "台股 2330 4/30 三大法人籌碼動向延續分歧——外資連續第 4 個交易日賣超 TSMC，估自 4/29 -8,500 張小幅擴大至 -10,200 張（壓力延續但邊際收斂）；投信延續第 6 個交易日買進 +480 張（連 6 買，雖力道減弱仍護盤）、自營商連 5 買 +260 張，三大法人合計賣超約 9,460 張（vs 4/29 -7,530 張）。外資持股比例自 4/29 約 75.0% 略降至 74.9%，4/30 賣超台積電金額估約 NT$218 億。投信「在地承接」護盤格局延續第 6 日，是台股築底重要支撐力。籌碼觀察：今日（5/4 Mon）若外資能轉買賣均衡或翻多，配合 5/1 美股反彈，連 4 日修正築底訊號明確；若仍續賣則需警惕指向 20MA NT$2,098 第二支撐。",
        "source": "TWSE / Goodinfo / HiStock 2026-04-30", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📅 TSMC 4 月合併月營收即將公布（5/10 前）：市場預估 NT$330B+，Q2 旺季續強訊號",
        "body": "TSMC 將依例於 5/10 前公布 4 月合併月營收。市場預估 4 月營收為 NT$330B+，雖較 Q1 月平均 NT$378B 季節性回落（Q1 受 iPhone 16 / Apple 拉貨支持較強），但仍將維持 +30% YoY 以上的高速成長動能。重點觀察：（1）4 月營收能否守住 NT$330B 以上 → 確認 Q2 指引 $39.0-40.2B 達成可能；（2）Q1 平均月營收 NT$378B 顯示 N3 / 2nm 客戶持續滿載；（3）NVIDIA Blackwell GB200/GB300 出貨支持高階先進製程；（4）台幣 5/1 約 31.20 持續處於升值區間（每升 1% 毛利率 -40bps）。法人預期：4 月營收 NT$330B+ → 5 月營收估 NT$340B（連續月增）→ 6 月 NT$350B 進入 Q2 旺季。月營收為短線最重要催化，5/10 前後可能引發市場重新評估。",
        "source": "TSMC IR / MoneyDJ / Anue 2026-05-04", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🏢 5/1 AI 晶片股延續反彈：NVDA $940.20（+0.84%）、AMD +0.60%、AVGO +0.95%、SOX 連 3 日漲、OpenAI 雜訊基本消化",
        "body": "5/1 美股 AI 晶片股延續反彈勢頭：NVIDIA 收 $940.20（+0.84% vs 4/30 $932.40），連續第 3 個交易日收漲，自 4/28 低點 $925.10 累計 +1.63%；AMD 收 $184.60（+0.60%），AVGO 收 $214.80（+0.95%），ARM $204.10（+0.89% 自 TSMC 出脫衝擊回穩）；費城半導體指數 SOX 收 10,540（+0.28%）連 3 日漲。Apple 續穩收 $220.50（+0.59%）。OpenAI 不達標雜訊基本消化——市場已認知（1）Anthropic、Google、Meta 自研 AI 晶片需求穩健不變；（2）TSMC 客戶結構多元（NVIDIA 22%、Apple 17%、Broadcom 13%）抗單一風險；（3）NVIDIA Q1 財報 5/28 即將公布，市場聚焦 GB300 出貨與 H2 指引；（4）AMD 5/6 財報前 MI450 訂單持續滲透。今日（5/4 Mon）台股開盤若跟進 5/1 美股漲勢，有望形成 V 型反彈築底。",
        "source": "CNBC / Yahoo Finance / Bloomberg 2026-05-01", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🎯 TSMC 5 座 2nm 廠量產啟動延續發酵：首年出貨 +45% vs 3nm；A13/A12（2029）製程路線確認",
        "body": "TSMC 北美技術論壇後續資訊持續發酵：（1）5 座 2nm 晶圓廠（新竹 2 座 + 高雄 3 座）2026 量產正式啟動，首年出貨將比 2023 年 3nm 首年高 +45%；2nm 製程產能維持 2026-2028 年 CAGR +70%；（2）海外擴張加速：Arizona Phase 1 廠 2026 出貨 +80% YoY，熊本廠 +130% YoY，地緣分散降低台海風險溢價；（3）先進製程路線完整：A13（2029，A14 直接微縮 +6% 面積效率）、A12（2029，Super Power Rail 背面供電鎖定 AI/HPC）、N2U（2028，DTCO 設計）、A16（2027，NVIDIA 為首批客戶）；（4）Arizona 先進封裝廠 2029 啟用（CoWoS+3D-IC）；（5）CoWoS 5.5-reticle 量產中（>98% 良率），2028 推出 14-reticle 整合 10 大晶片 + 20 HBM。Goldman Sachs：技術論壇強化長線結構性多頭，維持目標 NT$2,330。",
        "source": "Taipei Times / Focus Taiwan / Goldman Sachs 2026-05-01", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "🌐 Alphabet 1900 億美元 AI 基建投資持續發酵；TSMC、ASML、Micron 為三大半導體贏家",
        "body": "5/3 The Motley Fool 報導 Alphabet 宣布投入 1900 億美元（$190B）AI 基礎設施投資計畫，將在 2026-2028 年建構大規模 TPU 訓練與推論集群。市場分析三大半導體贏家：（1）TSMC——Alphabet TPU v7 / v8 全部由 TSMC N3 / 2nm 代工，並使用 CoWoS 先進封裝；（2）ASML——TSMC 2nm / A16 EUV 設備需求結構性受益；（3）Micron——HBM4 供應 Alphabet TPU 加速器。Google Cloud 已成為 NVIDIA / Anthropic / OpenAI 之外最大 TSMC 間接客戶，TPU 月出貨量在 H2 2026 將達 NVIDIA H100 同期 70%。對 TSMC 戰略意義：（1）客戶結構進一步多元化（自研晶片客戶比例提升）；（2）OpenAI 雜訊影響邊際化——Alphabet / Anthropic / Meta 自研需求穩定；（3）2nm / A16 訂單能見度延長至 2028。",
        "source": "The Motley Fool / Bloomberg 2026-05-03", "impact": "正面",
    },
]

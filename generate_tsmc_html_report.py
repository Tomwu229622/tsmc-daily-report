...
STOCK = {
    # 台股 2330
    "tw_price":       "2,370",        # 台股 2330 最新確認收盤：8/7 Fri 官方收 NT$2,370（+5、+0.21% vs 8/6 NT$2,365、開 2,390／高 2,395／低 2,355、TWSE 確認）——小漲站回 5MA 2,366、重新站上全部均線；大盤同日 -170.79 點（-0.38%）收 44,225.91、2330 相對強勢
    "tw_change":      "+5",           # 8/7 Fri 收盤漲跌點數（+0.21%、TWSE 官方）
    "tw_change_pct":  "+0.21%",       # 8/7 Fri 高開回落收小紅：開 2,390 後回測 2,355、尾盤收 2,370——站回 5MA 2,366、重新站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167）；⚠️ 量能三日連縮至 24,414 張（5 日均量 32,593 的 75%、近月新低）；同晚美股半導體全面大漲（SOX +2.56% 創本波新高、NVDA 創史高、TSM +0.44% 收 $420.04）——今日（8/10）開盤補漲條件齊備
    "tw_volume":      "24,414",       # 8/7 成交量（張、TWSE 口徑；成交額 NT$579.47 億、成交 64,670 筆——僅 5 日均量 32,593 張的 75%、近月新低，量能連 3 日萎縮為最大隱憂）
    "tw_open":        "2,390",        # 8/7 開盤（TWSE、高開 +25 後回落）
    "tw_high":        "2,395",        # 8/7 盤中高（TWSE、與 8/5-8/6 高點連 3 日壓在 2,395-2,415 區）
    "tw_low":         "2,355",        # 8/7 盤中低（TWSE、貼近 60MA 2,357 獲支撐後尾盤收復）
    "market_cap":     "NT$61.4 兆",   # 8/7 收盤市值（NT$2,370 基礎、約 259.3 億股）
    "pe_ratio":       "31.86x",       # 台股 P/E（TWSE 官方 BWIBBU 8/7、口徑為最新年度 EPS）
    "pb_ratio":       "10.43x",       # 台股 P/B（TWSE 官方 BWIBBU 8/7、以 115Q1 財報淨值計）
    "div_yield":      "0.93%",        # 殖利率（TWSE 官方 BWIBBU 8/7、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "420.04",       # 8/7 Fri 官方收盤 $420.04（+$1.84、+0.44% vs 8/6 $418.20、Yahoo API 確認）——連 2 日創本波收盤新高、重新站上 $420；SOX 8/7 +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000）
    "nyse_change":    "+1.84",        # 8/7 漲跌（美元、自我檢核：418.20 × 1.0044 = 420.04 ✓）
    "nyse_change_pct":"+0.44%",       # 8/7 續漲、連 2 日創本波收盤新高
    "52w_high_nyse":  "479.00",
    "52w_low_nyse":   "223.70",
    "pe_nyse":        "~30.7x",       # 估算（以 $420.04 與 TTM EPS 換算）
    "pb_nyse":        "~18.2x",       # 估算
    "beta":           "1.18",
    ...
    "data_note":      '報告日期：2026-08-10 Mon（最新確認收盤：台股 2330 8/7 Fri 官方收 NT$2,370（+5、+0.21% vs 8/6 NT$2,365、開 2,390／高 2,395／低 2,355、量 24,414 張 TWSE 口徑、成交額 NT$579.47 億、成交 64,670 筆、TWSE 確認）；NYSE TSM 8/7 Fri 收 $420.04（+$1.84、+0.44% vs 8/6 $418.20、Yahoo API 確認、連 2 日創本波收盤新高））。📊 今日格局：外部條件全面轉多、開盤補漲條件齊備——（1）📊 台股 8/7 小漲站回全部均線：2330 開 2,390 高開後回測 2,355（貼近 60MA 2,357 獲支撐）、尾盤收 2,370（+0.21%）——站回 5MA 2,366、重新站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167）；⚠️ 量能連 3 日萎縮至 24,414 張（僅 5 日均量 32,593 張的 75%、近月新低）——多空觀望、等待 7 月營收與美股方向；大盤 -170.79 點（-0.38%）收 44,225.91（FMTQIK 官方）——2330 相對大盤強勢；（2）✅ 籌碼翻多（幅度溫和）：2330 外資回買 +642 張（終結 1 日賣超）、投信 +61 張（終結連 2 賣）、自營 +1,243、三大合計 +1,946 張（T86 官方）——三大法人同步站在買方；⚠️ 惟全市場 BFI82U 合計 -428.9 億（外資 -407.2 億大賣、投信 -12.0 億、自營合計 -9.7 億）——資金自中小型與記憶體族群撤出（華邦電外資 -43,331 張、-4.39% 連 5 紅中止；日月光外資 -3,054 張、-1.68%）、回流台積電等權值避風；（3）⭐ 美股 8/7（五）半導體全面大漲：SOX +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000、自 6/22 史高 14,634.66 收斂至 -15.6%）；NVDA +2.27% 收 $223.96 創歷史新高；QCOM +4.66%、KLAC +2.53%、AMAT +2.21%、ASML +2.15%、AMKR +2.05%、INTC +1.84% 收復 $100——設備股全面反攻；TSM +0.44% 收 $420.04 連 2 日創本波收盤新高；AMD -1.21% 收 $483.36、ARM -1.43%、MU -0.44% 少數逆勢；大盤 S&P +0.62% 收 7,757.64 創收盤新高、那指 +1.30%、道瓊 +0.28%、VIX 降至 14.90（本波最低）；台指期夜盤大漲約 736 點（媒體報導）；（4）✅ 韓股記憶體風暴緩和：SK 海力士 8/7 -4.88% 收 142.2 萬韓元續跌後、今晨 8/10 反彈 +2.46% 至 145.7 萬、三星 +1.19% 至 23.375 萬、KOSPI +1.00%（Yahoo 09:37 KST 快照）——二次崩跌未發生、利空鈍化；（5）📅 7 月合併月營收截至今晨尚未公布（pr.tsmc.com 查證）、預計今日（8/10）前後公布——投顧預期上看 NT$4,500 億（預期值、非官方）；6 月 NT$4,426.8 億（+67.9% YoY）基期極高、留意年增率自然回落；（6）📊 「台積電條款」今日（8/10）上路：證交所放寬千元以上高價股注意標準（價差門檻 100→300 元）、處置期 10→5 日、處置撮合改約 2 分鐘一次——高價權值股流動性利多（經濟日報／工商時報）。📊 ADR 溢價續擴至 +14.28%（$420.04 vs 理論值 $367.56、匯率 32.24）——連 3 日擴大，歷史上此型態多以台股補漲收斂、8/5 +3.66% 大反攻即為前次驗證。技術面（官方收盤計算至 8/7）：收 2,370 站回 5MA 2,366、站上全部均線——RSI 50.6 中軸上方、MACD 黃金交叉續強（DIF -7.1 上行／DEA -10.3、柱狀圖 +6.3 連 3 日擴大）、KDJ 金叉延續（K 72.2／D 62.5、J 91.7 仍偏高）；支撐 2,366（5MA）／2,362（20MA）／2,357（60MA）／2,355（8/7 低）／2,320（8/4 收）、壓力 2,390（8/7 開）／2,395（8/5-8/7 連 3 日高點區）／2,415（8/5 高）／2,425（7/31 漲停）／2,507（布林上軌）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20%。⭐ 今日（8/10 Mon）觀察：（1）ADR 溢價 +14.28% ＋ SOX 創本波新高 ＋ 夜盤 +736 點——開盤補漲的幅度與量能（能否自 24,414 張明顯回升為關鍵、無量上攻持續性存疑）；（2）📅 7 月營收公布時點與數字——投顧估上看 4,500 億、高基期下年增率回落幅度為市場焦點；（3）外資回買 +642 張的延續性（連 2 買才能確認）；（4）「台積電條款」上路後高價股流動性與波動變化。',
}
...
SUMMARY = {
    ...
    "headline":   '⭐ 外部條件全面轉多、開盤補漲條件齊備：2330 8/7 收 NT$2,370（+0.21%）站回全部均線、三大法人同步翻買 +1,946 張（外資 +642 終結賣超）；⭐ 美股 8/7（五）半導體全面大漲——SOX +2.56% 收 12,356.79 創本波新高、NVDA +2.27% 收 $223.96 創歷史新高、S&P 創收盤新高 7,757.64、VIX 降至 14.90；TSM +0.44% 收 $420.04 連 2 日創本波高、ADR 溢價續擴至 +14.28%；台指期夜盤大漲約 736 點、韓股今晨反彈（SK +2.46%）——⚠️ 惟量能連 3 日縮至 24,414 張近月新低；📅 7 月營收預計今日前後公布（投顧估上看 4,500 億、預期值非官方）、「台積電條款」今日上路',

    "bullets": [
        '📊 台股 8/7 小漲站回全部均線：2330 收 NT$2,370（+5、+0.21%、開 2,390／高 2,395／低 2,355）——盤中回測 60MA 2,357 附近獲支撐、尾盤收復 5MA 2,366，重新站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167）；大盤 -170.79 點（-0.38%）收 44,225.91——2330 相對大盤強勢',
        '⚠️ 量能連 3 日萎縮為最大隱憂：8/7 成交僅 24,414 張、為 5 日均量 32,593 張的 75%（近月新低、低於 8/6 的 25,537）、成交額 NT$579.47 億——多空觀望，等待 7 月營收與美股方向；今日若補漲須配合量能明顯回升，無量上攻持續性存疑',
        '✅ 籌碼翻多、三大法人同步站買方：2330 外資回買 +642 張（終結 1 日賣超）、投信 +61 張（終結連 2 賣）、自營 +1,243、合計 +1,946 張（T86 官方）；⚠️ 惟全市場 BFI82U 大賣 -428.9 億（外資 -407.2 億、投信 -12.0 億）——資金自中小型與記憶體撤出（華邦電外資 -43,331 張）、回流台積電等權值避風',
        '⭐ 美股 8/7（五）半導體全面大漲、外部條件轉多：SOX +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000）；NVDA +2.27% 收 $223.96 創歷史新高；QCOM +4.66%、KLAC +2.53%、AMAT +2.21%、ASML +2.15%、INTC +1.84% 收復 $100——設備股全面反攻；TSM +0.44% 收 $420.04 連 2 日創本波收盤新高；S&P +0.62% 收 7,757.64 創收盤新高、VIX 降至 14.90（本波最低）；台指期夜盤大漲約 736 點',
        '✅ 韓股記憶體風暴緩和、二次崩跌未發生：SK 海力士 8/7 -4.88% 收 142.2 萬韓元續跌後、今晨 8/10 反彈 +2.46% 至 145.7 萬、三星 +1.19%、KOSPI +1.00%（Yahoo 09:37 KST 快照）——記憶體利空鈍化；⚠️ 惟台股記憶體補跌壓力已現：華邦電 8/7 -4.39% 收 163.5（連 5 紅中止）',
        '📊 ADR 溢價自 +13.98% 續擴至 +14.28%（$420.04 vs 理論值 $367.56、匯率 32.24）——連 3 日擴大；歷史上此型態多以台股補漲方式收斂、8/5 的 +3.66% 大反攻即為前次驗證；今日開盤補漲條件（溢價＋SOX 新高＋夜盤大漲）齊備',
        '💡 技術面多頭訊號延續：收 2,370 站回 5MA 2,366、站上全部均線；⭐ MACD 黃金交叉續強（DIF -7.1／DEA -10.3、柱狀圖 +6.3 較前日 +3.7 續放大、連 3 日紅柱擴張）、RSI 50.6 中軸上方、KDJ 金叉延續（K 72.2／D 62.5）惟 J 91.7 仍偏高、短線過熱未完全消化',
        '📅 事件行事曆：7 月合併月營收截至今晨未公布（pr.tsmc.com 查證）、預計今日（8/10）前後公布——投顧估上看 NT$4,500 億（預期值、非官方）；「台積電條款」今日上路（千金股注意標準價差門檻 100→300 元、處置期 10→5 日）；232 條款半導體專章結果仍未公布（未能證實）；2nm 報價漲 10-20%（TrendForce）',
    ],

    "bottom_line": '報告日期：2026-08-10 Mon（最新確認收盤：台股 2330 8/7 Fri 官方收 NT$2,370（+5、+0.21% vs 8/6 NT$2,365、開 2,390／高 2,395／低 2,355、量 24,414 張、額 NT$579.47 億、64,670 筆、TWSE 確認）；NYSE TSM 8/7 收 $420.04（+0.44%、連 2 日創本波收盤新高、Yahoo API 確認））。📊 核心判讀：外部條件全面轉多、補漲條件齊備、量能與 7 月營收是今日兩大檢核點——（1）📊 台股 8/7 小漲站回全部均線：2330 開 2,390 高開後回測 2,355（貼近 60MA 2,357 獲支撐）、尾盤收 2,370（+0.21%）——站回 5MA 2,366、重新站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167），結束 8/6 的破 5MA 狀態；大盤 -0.38% 收 44,225.91、2330 相對強勢；⚠️ 量能連 3 日萎縮至 24,414 張、僅 5 日均量 32,593 張的 75%（近月新低）——多空觀望等待方向，補漲若無量能配合、持續性存疑；（2）✅ 籌碼翻多、三大法人同步站買方：2330 外資回買 +642 張（終結 1 日賣超）、投信 +61 張（終結連 2 賣）、自營 +1,243、三大合計 +1,946 張（T86 官方）——幅度雖溫和、方向一致；⚠️ 對照全市場 BFI82U 合計 -428.9 億（外資 -407.2 億大賣、投信 -12.0 億、自營合計 -9.7 億）——外資大舉調節中小型與記憶體（華邦電外資 -43,331 張、-4.39% 連 5 紅中止；日月光外資 -3,054 張、-1.68%；聯發科 -0.51% 惟外資小買 +72 張）、資金回流台積電等權值避風，與 8/6 的「輪出台積電」正好反轉；（3）⭐ 美股 8/7（五）半導體全面大漲、為今日開盤最強外部條件：SOX +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000、自 6/22 史高 14,634.66 收斂至 -15.6%）；NVDA +2.27% 收 $223.96 創歷史新高；QCOM +4.66% 領漲、KLAC +2.53%、AMAT +2.21%、ASML +2.15%、AMKR +2.05%、INTC +1.84% 收復 $100——前兩日疲弱的設備股全面反攻、上漲擴散度極佳；TSM +0.44% 收 $420.04 連 2 日創本波收盤新高；AMD -1.21%、ARM -1.43%、MU -0.44% 為少數逆勢；大盤 S&P +0.62% 收 7,757.64 創收盤新高、那指 +1.30%、道瓊 +0.28%、VIX 降至 14.90（本波最低、風險偏好續改善）；台指期夜盤大漲約 736 點（媒體報導）；（4）✅ 韓股記憶體風暴緩和：SK 海力士 8/7 -4.88% 收 142.2 萬韓元續跌後、今晨 8/10 反彈 +2.46% 至 145.7 萬、三星 +1.19% 至 23.375 萬、KOSPI +1.00%（Yahoo 09:37 KST 快照）——二次崩跌未發生、SanDisk/WDC 財測引發的賣壓進入鈍化期；⚠️ 惟台股記憶體補跌已現（華邦電 -4.39%）、族群輪動仍在進行；（5）📅 7 月合併月營收為今日最大基本面事件：截至今晨尚未公布（pr.tsmc.com 查證）、預計今日（8/10）前後公布——投顧預期上看 NT$4,500 億（預期值、非官方）；6 月 NT$4,426.8 億（+67.9% YoY）為史上單月新高、基期極高，Q3 美元指引 $44.6-45.8B 換算月均約需 NT$4,800 億以上——7 月數字與高基期下的年增率回落幅度為市場焦點、宜以季度累計判讀勿過度解讀單月；（6）📊 制度面：「台積電條款」今日上路——證交所放寬千元以上高價股注意標準（價差門檻 100→300 元）、處置期 10→5 日，高價權值股流動性利多。💡 技術面：收 2,370 站上全部均線——⭐ MACD 黃金交叉續強（DIF -7.1 上行／DEA -10.3、柱狀圖 +6.3 vs 前日 +3.7、連 3 日紅柱擴張）、RSI 50.6 中軸上方、KDJ 金叉延續（K 72.2／D 62.5）惟 J 91.7 仍偏高；支撐 2,366（5MA）／2,362（20MA）／2,357（60MA）／2,355（8/7 低）／2,320、壓力 2,390（8/7 開）／2,395（連 3 日高點區）／2,415／2,425／2,507（布林上軌）／2,535（史高）。📊 ADR 溢價自 +13.98% 續擴至 +14.28%（$420.04 vs 理論值 $367.56、匯率 32.24）——連 3 日擴大、歷史上多以台股補漲方式收斂。⭐ 今日（8/10 Mon）觀察：（1）補漲的幅度與量能——ADR 溢價 +14.28% ＋ SOX 創本波新高 ＋ 夜盤 +736 點，開高機率高、關鍵在量能能否自 24,414 張明顯回升且站穩 2,395-2,415 壓力區；（2）📅 7 月營收公布時點與數字；（3）外資回買 +642 張的延續性（連 2 買才確認翻多）；（4）「台積電條款」上路後的流動性變化。⚠️ 風險：量能近月新低、KDJ J 91.7 仍偏高、7 月營收若低於預期（高基期回落過陡）、232 條款懸而未決、記憶體族群補跌拖累台股半導體情緒。',

    "risk_alert":  '今日（8/10 Mon）關鍵觀察：（1）⭐ 補漲條件齊備、關鍵在量能：ADR 溢價 +14.28%（連 3 日擴大）＋ SOX 8/7 +2.56% 創本波新高 ＋ NVDA 創史高 ＋ 台指期夜盤大漲約 736 點——開高機率高；⚠️ 但 8/7 成交僅 24,414 張（5 日均量 32,593 的 75%、近月新低、連 3 日萎縮），若開高後量能未明顯回升、衝高 2,395-2,415 壓力區的持續性存疑；（2）📅 7 月營收為今日最大變數：截至今晨未公布（pr.tsmc.com 查證）、預計今日前後公布——投顧預期上看 NT$4,500 億（預期值、非官方）；6 月 4,426.8 億（+67.9% YoY）基期極高，若數字低於預期或年增率回落過陡、可能觸發獲利了結；（3）✅ 籌碼溫和翻多待確認：8/7 外資 +642 張終結賣超、三大合計 +1,946 張——幅度小、需連 2 買確認；⚠️ 全市場外資 8/7 大賣 -428.9 億（BFI82U）集中在中小型與記憶體（華邦電 -43,331 張），權值避風買盤能否延續為觀察點；（4）✅ 韓股記憶體風暴緩和：今晨 8/10 SK +2.46%、三星 +1.19%、KOSPI +1.00% 反彈（Yahoo 09:37 KST 快照）——二次崩跌未發生；⚠️ 惟台股記憶體補跌已現（華邦電 8/7 -4.39%）、留意族群情緒外溢；（5）💱 匯率 32.24 持平；232 條款半導體專章結果仍未公布（未能證實）、台美關稅無新進展；「台積電條款」今日上路（注意標準價差門檻 100→300 元、處置期 10→5 日）——留意高價股波動放大；（6）⚠️ 中期：SOX 自 6/22 史高 14,634.66 仍 -15.6%；2330 自 6/23 史高 2,535 仍 -6.5%——反彈中期確認仍需量能配合、KDJ J 91.7 偏高短線過熱未完全消化。',
}
...
INSTITUTIONAL = {
    ...
    "trust":    61,
    "dealer":   1243,
    # 近 5 交易日（8/3, 8/4, 8/5, 8/6, 8/7）
    "foreign_5d": [-9717, -12430, 12251, -4011, 642],
    "trust_5d":   [387, 434, -1205, -82, 61],
    "dealer_5d":  [-1587, -2327, -1099, -583, 1243],
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       50.6,   "rsi_signal": 'RSI(14) 50.6（官方收盤計算）——8/7 小漲 +0.21% 後自 50.3 微升至 50.6、續站中軸線上方——中性偏多；續站 50 上為修復延續、今日若補漲可望脫離中軸糾纏',
    "macd":         6.31,   "macd_signal": 'MACD 黃金交叉續強（DIF -7.1 上行、DEA -10.3、柱狀圖 +6.3、官方收盤計算）——⭐ 紅柱較前日 +3.7 續放大（連 3 日擴張：+0.5→+3.7→+6.3），中期動能持續修復；惟 DIF 仍在零軸下、上穿零軸為中期修復的下一確認',   "macd_color": "#4CAF50",
    "kdj_k":        72.2,   "kdj_d": 62.5, "kdj_j": 91.7, "kdj_signal": 'KDJ K(72.2)/D(62.5)/J(91.7)（官方收盤計算）——金叉延續、K 與 D 續升；⚠️ J 值 91.7 仍偏高（自 93.4 微降）、短線過熱未完全消化，追高需留意',
    "ma5":       "2,366",
    "ma20":      "2,362",
    "ma60":      "2,357",
    "ma120":     "2,167",
    ...
    "bb_lower":  "2,217",
    "vol_ma5":  "32,593",
    "trend":    '📊 小漲站回全部均線、等待補漲確認：2330 8/7 官方收 NT$2,370（+5、+0.21%、開 2,390／高 2,395／低 2,355、量 24,414 張僅 5 日均量 32,593 張的 75%、近月新低）——盤中回測 60MA 2,357 附近獲支撐、尾盤站回 5MA 2,366，重新站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167）；大盤 -0.38% 收 44,225.91、2330 相對強勢。⭐ 技術指標（官方收盤計算）多頭訊號延續：MACD 黃金交叉續強——DIF -7.1 上行、DEA -10.3、柱狀圖 +6.3 較前日 +3.7 續放大（連 3 日紅柱擴張：+0.5→+3.7→+6.3）；RSI 50.6 中軸上方、KDJ 金叉延續（K 72.2／D 62.5）惟 J 91.7 仍偏高。✅ 籌碼翻多：外資回買 +642 張（終結 1 日賣超）、投信 +61、自營 +1,243、三大合計 +1,946 張——同步站買方、惟幅度溫和需連 2 買確認；全市場 BFI82U 外資大賣 -407.2 億集中在中小型與記憶體（華邦電 -43,331 張）、資金回流權值避風。⭐ 外部全面轉多：美股 8/7 SOX +2.56% 收 12,356.79 創本波新高、NVDA +2.27% 收 $223.96 創史高、S&P 創收盤新高、VIX 14.90 本波最低；TSM +0.44% 收 $420.04——ADR 溢價續擴至 +14.28%、台指期夜盤大漲約 736 點，補漲條件齊備。✅ 韓股今晨反彈（SK +2.46%、三星 +1.19%）、記憶體風暴鈍化。下一觀察：（1）開高後量能能否自 24,414 張明顯回升——放量站穩 2,395-2,415 則挑戰 2,425／2,507（布林上軌）；（2）無量衝高則提防 2,395-2,415 壓力區再度回落、支撐看 2,366／2,362／2,357 均線帶；（3）📅 7 月營收數字（投顧估上看 4,500 億、預期值非官方）為今日最大基本面變數。',
    "support":  "2,366 / 2,362 / 2,357 / 2,355 / 2,320 / 2,217 (5MA、20MA、60MA 季線、8/7 低、8/4 收、布林下軌)",
    "resist":   "2,390 / 2,395 / 2,415 / 2,425 / 2,507 / 2,535 (8/7 開、連 3 日高點區、8/5 高、7/31 漲停、布林上軌、6/23 史高)",
    "note": '均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-07 止），請以看盤軟體為準；8/7 收 NT$2,370（+0.21%）站回 5MA 2,366、站上全部均線（20MA 2,362／60MA 2,357／120MA 2,167）；RSI 50.6 中軸上方、MACD 黃金交叉續強（DIF -7.1／DEA -10.3、柱 +6.3 較前日 +3.7 續放大、連 3 日擴張）、KDJ 金叉延續（K72.2/D62.5/J91.7 偏高）；支撐 2,366 / 2,362 / 2,357 / 2,355 / 2,320 / 2,217、壓力 2,390 / 2,395 / 2,415 / 2,425 / 2,507 / 2,535；⭐ MACD 柱連 3 日擴大＋站上全均線＋ADR 溢價 +14.28%＋SOX 創本波新高 vs ⚠️ 量能近月新低（24,414 張、75% 均量）＋J 值 91.7 偏高——今日檢核點為「補漲能否放量站穩 2,395-2,415 壓力區」',
}
...
DERIVATIVES = {
    ...
    "put_oi":        "111,000",
    "pcr":           "0.90",
    "pcr_signal":    '中性偏多（Put/Call 估 0.90；8/7 現貨小漲站回全均線、三大法人同步翻買，期權下檔賣權佈局回升；美股週五半導體大漲＋夜盤大漲約 736 點——今日上檔佈局可望明顯回升；期權數據以估算標註、請以 TAIFEX 為準）',
    "max_pain":      "2,400",
    "key_strikes": [
        ("2,320", "Call 34,000 / Put 13,000", "8/4 收、下檔支撐"),
        ("2,357", "Call 36,000 / Put 12,000", "60MA 季線"),
        ("2,362", "Call 37,000 / Put 12,000", "20MA"),
        ("2,366", "Call 38,000 / Put 11,000", "5MA"),
        ("2,390", "Call 40,000 / Put 11,000", "8/7 開盤高點"),
        ("2,400", "Call 44,000 / Put 14,000", "整數關卡、估最大痛苦"),
        ("2,415", "Call 41,000 / Put 10,000", "8/5 盤中高"),
        ("2,425", "Call 40,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": '衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（8/7 收 NT$2,370）；估最大痛苦 2,400、下方支撐 2,366（5MA）/ 2,362（20MA）/ 2,357（60MA）/ 2,355（8/7 低）/ 2,320（8/4 收）、上方壓力 2,390（8/7 開）/ 2,395（連 3 日高點區）/ 2,415（8/5 高）/ 2,425（7/31 漲停）/ 2,507（布林上軌）/ 2,535（史高）；⭐ MACD 柱連 3 日擴大＋站上全均線＋三大法人翻買＋SOX 創本波新高（ADR 溢價 +14.28%、夜盤 +736 點） vs ⚠️ 量能近月新低（24,414 張、僅 75% 均量）＋J 值 91.7 偏高＋7 月營收數字未定；✅ 基本面錨未變（2nm Q3 放量＋報價漲 10-20%、CoWoS 訂滿至 2027、全年上修 >40%）',
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # (分類, 代號, 名稱, 最新價, 漲跌幅, 說明) — 美股為 8/7 Fri 收盤（Yahoo API）、台股為 8/7 TWSE 官方收盤、韓股為 8/7 收盤＋8/10 晨盤快照
    ("設備", "ASML",    "艾司摩爾",    "1,740.99", "+2.15%", "⭐ EUV 設備龍頭；8/7 大漲 +2.15% 收 $1,740.99 創本波新高；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "539.14", "+2.21%", "✅ 8/7 反攻 +2.21% 收 $539.14（終結連 2 日回檔）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "311.35", "+1.82%", "✅ 8/7 反攻 +1.82% 收 $311.35（終結連 2 日回檔）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "198.11", "+2.53%", "✅ 8/7 大漲 +2.53% 收 $198.11 逼近 $200；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","163.50","-4.39%", "⚠️ 8/7 重挫 -4.39% 收 163.5（TWSE 官方、連 5 紅中止）——韓股記憶體風暴延遲一日衝擊台股記憶體、外資大賣 -43,331 張；與 TSMC 合作 CUBE 特殊 DRAM 題材中長線未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,422,000 KRW", "-4.88%", "📊 8/7 續跌 -4.88% 收 142.2 萬韓元（Yahoo 官方收盤序列、8/6 -10.37% 後跌幅收斂）；⭐ 今晨 8/10 反彈 +2.46% 至 145.7 萬（09:37 KST 快照）——二次崩跌未發生；高盛/摩通維持買進"),
    ("記憶", "MU",      "美光",        "877.57", "-0.44%", "📊 8/7 小跌 -0.44% 收 $877.57（跌幅收斂、SanDisk/WDC 利空鈍化中）；HBM/DDR5、自 7/23 高 $990 收斂至 -11.4%"),
    ("封裝", "3711.TW", "日月光",      "585.00", "-1.68%", "📊 8/7 回檔 -1.68% 收 585（TWSE 官方、連 2 日收高中止）——外資賣超 -3,054 張；CoWoS 外包題材中長線未變、TSMC-Amkor 美國封裝長約對照"),
    ("封裝", "AMKR",    "Amkor",       "55.27",  "+2.05%", "✅ 8/7 大漲 +2.05% 收 $55.27 連 2 日走高；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材中長線未變"),
    ("客戶", "NVDA",    "輝達",        "223.96", "+2.27%", "⭐⭐ TSMC #1 客戶、A16 首發客戶；8/7 大漲 +2.27% 收 $223.96 創歷史新高（整理 1 日後再攻）——SpaceX 獨家採用、2GW 算力擴張題材延續；CNBC 報導其占 TSMC 營收比重已升至約 19% 躍居最大客戶"),
    ("客戶", "AAPL",    "蘋果",        "313.33", "+0.29%", "✅ 主要客戶（比重約 17%）；8/7 續漲 +0.29% 收 $313.33 連 4 日上漲創本波高；CEO 交接（9/1 Ternus 接任）過渡期、「晶片供應吃緊」反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "427.76", "+1.71%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能；8/7 +1.71% 收 $427.76 續創本波新高"),
    ("客戶", "AMD",     "超微",        "483.36", "-1.21%", "📊 #4 客戶；8/7 回落 -1.21% 收 $483.36（財報後高估值消化延續、8/6 反彈未能延續）；MI455X（2nm＋HBM4）發表、資料中心年增逾 50%——訂單實質對 TSMC 為正面"),
    ("客戶", "QCOM",    "高通",        "167.86", "+4.66%", "⭐ 手機/PC SoC 客戶；8/7 飆漲 +4.66% 收 $167.86 續創本波新高（連 2 日領漲）"),
    ("客戶", "2454.TW", "聯發科",      "3,900",  "-0.51%", "📊 手機 SoC 客戶 9%；8/7 小跌 -0.51% 收 3,900（TWSE 官方）——4,000 關卡下整理；外資小買 +72 張、投信 -128 張"),
    ("競爭", "INTC",    "英特爾",      "101.65", "+1.84%", "✅ 8/7 反彈 +1.84% 收 $101.65 收復 $100 關卡（終結連 2 日回檔）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "231,000 KRW", "+0.22%", "✅ 8/7 止穩 +0.22% 收 23.1 萬韓元（8/6 -6.30% 後率先止跌）；⭐ 今晨 8/10 續漲 +1.19% 至 23.375 萬（09:37 KST 快照）——記憶體風暴緩和"),
    ("產業", "SOX",     "費城半導體指數","12,356.79", "+2.56%", "⭐⭐ 8/7 大漲 +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000）——設備、手機晶片全面反攻、上漲擴散度極佳；自 6/22 史高 14,634.66 收斂至 -15.6%"),
    ("處分後", "ARM",   "Arm Holdings","282.57", "-1.43%", "📊 TSMC 4/29 全數出脫後（8/7 回檔 -1.43% 收 $282.57——8/6 +4.41% 大漲後獲利了結、高 Beta 特性延續）"),
]
...
NEWS = [
    {
        ...
        "title": "📊 台股 8/7 小漲站回全均線：2330 +0.21% 收 2,370、三大法人翻買",
        "body": '量縮價穩、籌碼翻多：（1）📊 2330 開 2,390 高開後回測 2,355（貼近 60MA 2,357 獲支撐）、尾盤收 NT$2,370（+5、+0.21%）——站回 5MA 2,366、重新站上全部均線；大盤 -170.79 點（-0.38%）收 44,225.91（FMTQIK 官方）、2330 相對強勢；（2）⚠️ 量能連 3 日萎縮：成交僅 24,414 張、為 5 日均量 32,593 張的 75%（近月新低）、成交額 NT$579.47 億、64,670 筆——多空觀望等待 7 月營收與美股方向；（3）✅ 籌碼翻多：外資回買 +642 張（終結 1 日賣超）、投信 +61 張（終結連 2 賣）、自營 +1,243、三大合計 +1,946 張（T86 官方）——同步站買方、惟幅度溫和需連 2 買確認；（4）⚠️ 全市場 BFI82U 外資大賣 -407.2 億、合計 -428.9 億——集中調節中小型與記憶體（華邦電外資 -43,331 張、-4.39%；日月光 -3,054 張、-1.68%），資金回流台積電等權值避風。',
        "source": "TWSE 台灣證券交易所（STOCK_DAY／FMTQIK／T86／BFI82U 官方 API）",
        "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 美股 8/7 半導體全面大漲：SOX +2.56% 創本波新高、NVDA 創史高",
        "body": '外部條件全面轉多：（1）⭐ SOX 8/7（五）大漲 +2.56% 收 12,356.79 創本波新高（連 3 日站上 12,000、自 6/22 史高收斂至 -15.6%）——QCOM +4.66%、KLAC +2.53%、AMAT +2.21%、ASML +2.15% 創本波新高、AMKR +2.05%、INTC +1.84% 收復 $100，前兩日疲弱的設備股全面反攻、上漲擴散度極佳；（2）⭐ NVDA +2.27% 收 $223.96 創歷史新高（整理 1 日後再攻）；TSM +0.44% 收 $420.04 連 2 日創本波收盤新高、重新站上 $420；（3）📊 大盤 S&P +0.62% 收 7,757.64 創收盤新高、那指 +1.30%、VIX 降至 14.90（本波最低）——風險偏好續改善；（4）⭐ ADR 溢價續擴至 +14.28%（$420.04 vs 理論值 $367.56、匯率 32.24）、台指期夜盤大漲約 736 點（媒體報導）——今日（8/10）台股開盤補漲條件齊備、關鍵在量能能否同步回升。',
        "source": "Yahoo Finance API／PChome 新聞（夜盤）",
        "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "✅ 韓股記憶體風暴緩和：SK 今晨 +2.46% 反彈、惟華邦電補跌 -4.39%",
        "body": '二次崩跌未發生、但台股補跌已現：（1）✅ SK 海力士 8/7 -4.88% 收 142.2 萬韓元（8/6 -10.37% 後跌幅收斂）、三星 8/7 率先止穩 +0.22%；⭐ 今晨 8/10 韓股全面反彈——SK +2.46% 至 145.7 萬、三星 +1.19% 至 23.375 萬、KOSPI +1.00%（Yahoo 09:37 KST 快照）——SanDisk/WDC 財測引發的記憶體賣壓進入鈍化期；（2）⚠️ 台股記憶體延遲一日補跌：華邦電 8/7 重挫 -4.39% 收 163.5（連 5 紅中止）、外資大賣 -43,331 張——8/6「資金輪入記憶體」的敘事一日反轉；（3）⭐ 對台積電的判讀不變：此為記憶體（NAND/DRAM）供需利空、台積電無 HBM/NAND 曝險——8/7 SOX 大漲 +2.56%、TSM 續創本波高即為市場分流定價的延續驗證；（4）📊 高盛、摩根大通維持韓廠買進評等（遠期本益比 3.5-3.6 倍）。',
        "source": "Yahoo Finance API（官方收盤序列＋8/10 晨盤快照）／TWSE T86",
        "impact": "中性",
    },
    {
        "tag": "公司公告", "tag_color": "#FF9800",
        "title": "📅 7 月營收預計今日前後公布：投顧預估上看 4,500 億（未官方證實）",
        "body": '今日最大基本面事件：（1）📅 台積電 7 月合併月營收截至今晨（8/10）尚未公布（pr.tsmc.com 官方新聞中心查證、最新僅至 6 月數字）——預計今日前後公布、確切時點以官方公告為準；（2）📊 市場預期：投顧樂觀預估 7 月營收可突破 NT$4,500 億、甚至上看 4,800 億再創單月新高（Yahoo 股市報導）——⚠️ 均為預期值、非官方數字，標示「未能證實」；（3）📊 對照基期：6 月 NT$4,426.80 億（+67.9% YoY、+6.2% MoM）為史上單月新高、H1 累計 NT$2.40 兆（+35.6% YoY）；Q3 美元指引 $44.6-45.8B 換算月均約需 NT$4,800 億以上；（4）💡 觀察重點：高基期下年增率的自然回落幅度——單月波動受出貨排程影響大、宜以季度累計判讀；若數字亮眼將與今日補漲行情形成共振、若低於預期則留意獲利了結。',
        "source": "TSMC 官方 IR（pr.tsmc.com、截至 8/10 晨查證未公布）／Yahoo 股市（投顧預期）",
        "impact": "中性",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "✅ 「台積電條款」今日上路：千金股注意標準放寬、處置期縮短",
        "body": '高價權值股流動性利多：（1）📊 證交所「注意股／處置股」新制自今日（8/10）正式上路：千元以上高價股觸發注意股的價差門檻自 100 元放寬至 300 元（每增 1,000 元再加 150 元）——台積電等千金股不再因正常波動被頻繁列入注意股，市場暱稱「台積電條款」；（2）📊 處置機制同步鬆綁：處置天數由 10 個營業日縮短為 5 個、撮合頻率改約每 2 分鐘一次——降低流動性懲罰；（3）💡 影響評估：媒體估計高價股流動性可提升約 1.5 倍，對 2330 為制度面利多——尤其在補漲行情需要量能配合的當下；⚠️ 惟同時意味高價股單日波動空間放大、追高風險同步上升；（4）規則 8/4 公布、8/10 生效（經濟日報／工商時報）。',
        "source": "經濟日報／聯合新聞網／工商時報",
        "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⭐ NVDA 創史高、占 TSMC 營收約 19% 躍居最大客戶；AMD 發表 MI455X",
        "body": '客戶結構向 AI 全面傾斜：（1）⭐ NVIDIA 8/7 大漲 +2.27% 收 $223.96 創歷史新高——SpaceX 獨家採用、2GW 算力擴張題材延續；CNBC 報導其占台積電營收比重已自 2024 年的 12% 升至約 19%、超越蘋果（22%→17%）躍居最大客戶——反映先進產能自智慧型手機轉向 AI 資料中心；（2）📊 AMD 於「Advancing AI 2026」發表新一代加速器 Instinct MI455X（2nm 製程＋HBM4）、Q2 資料中心營收年增逾 50%——全數由台積電先進製程與 CoWoS 承接，8/7 股價 -1.21% 屬高估值消化、訂單實質為正面；（3）📊 QCOM 8/7 飆漲 +4.66% 連 2 日領漲、AVGO +1.71% 續創本波新高——手機與 custom ASIC 客戶同步走強；（4）💡 對 TSMC：AI 客戶（NVDA/AMD/AVGO）合計比重持續上升、訂單能見度延伸至 2027。',
        "source": "CNBC／24-7 Wall St.／Distill Intelligence／Yahoo Finance API",
        "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "📊 232 條款半導體專章仍未公布：台灣爭取優惠待遇、尾部風險未解",
        "body": '政策面尾部變數持續：（1）📊 美國自 2026-01 起依 232 條款對部分進口先進運算晶片課徵 25% 關稅（供美國本土供應鏈用途者除外）——白宮官方 Fact Sheet 已公告；台灣同期與美方簽署投資協議（含對美晶片投資承諾）換取優惠待遇，截至 6 月台灣大部分出口品項已取得 232 關稅優惠（DigiTimes）；（2）⚠️ 半導體本身的專章調查結果與具體稅率：截至 2026-08-10 晨仍未查得官方公布——標示「未能證實」、為懸而未決的尾部風險；（3）💡 判讀：此為長期毛利率壓力而非單日事件——台積電已宣布對美累計投資承諾 $2,000 億、法說會亦將海外廠稀釋納入指引（未來 5 年年均 3-4pp）；市場已部分定價，真正風險在於超預期的鷹派措施。',
        "source": "白宮官方 Fact Sheet（2026-01）／DigiTimes／美國商務部",
        "impact": "中性",
    },
]

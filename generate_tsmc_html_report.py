...
STOCK = {
    # 台股 2330
    "tw_price":       "2,290",        # 台股 2330 最新確認收盤：7/17 Fri 官方收 NT$2,290（-180、-7.29% vs 7/16 NT$2,470、TWSE 確認）——⚠️ 史詩級崩跌、收盤即當日最低；全球 AI 晶片股拋售擴散至台股、加權指數同日重挫（今日 7/20 週一將反映是否技術性反彈）
    "tw_change":      "-180",         # 7/17 Fri 收盤漲跌點數（-7.29%、TWSE 官方）
    "tw_change_pct":  "-7.29%",       # 7/17 Fri 收盤漲跌幅（AI 晶片股熊市拋售、外資史詩級提款）
    "tw_volume":      "97,362",       # 7/17 成交量（張、TWSE 口徑；成交股數 97,362,670 股、成交額 NT$2,290.52 億、成交 1,150,086 筆、量能爆增至 7/16 的 3.2 倍）
    "tw_open":        "2,375",        # 7/17 開盤（TWSE）
    "tw_high":        "2,395",        # 7/17 盤中高（TWSE）
    "tw_low":         "2,290",        # 7/17 盤中低（TWSE、收盤即當日最低）
    "market_cap":     "NT$59.4 兆",   # 7/17 收盤市值（NT$2,290 基礎、約 259.3 億股）
    "pe_ratio":       "~26.2x",       # 台股 P/E（TTM 估，NT$2,290 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25 後估值進一步消化）
    "pb_ratio":       "~17.2x",       # 台股 P/B（NT$2,290 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.05%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,290）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "398.37",       # 7/17 Fri 官方收盤 $398.37（-$11.37、-2.77% vs 7/16 $409.74、盤後 $397.72、stockanalysis.com 確認、自我檢核 409.74×(1-2.77%)=398.39 ✓）——失守 $400；全球晶片股拋售、SOX 正式跌入熊市（自 6/22 高 -20.2%），惟相對抗跌（多檔成分股自高點 -30%+）
    "nyse_change":    "-11.37",       # 7/17 漲跌（自 7/16 $409.74）
    "nyse_change_pct":"-2.77%",       # 7/17 失守 $400、隨全球晶片股拋售下跌、相對板塊抗跌
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~29.2x",       # NYSE USD 基礎 P/E（TTM 估，$398.37 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~16.6x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-20 Mon（最新確認收盤：台股 2330 7/17 Fri 官方收 NT$2,290（-180、-7.29% vs 7/16 NT$2,470、開 2,375／高 2,395／低 2,290、量 97,362 張 TWSE 口徑、成交額 NT$2,290.52 億、成交 1,150,086 筆、TWSE 確認）——⚠️ 史詩級崩跌、收盤即當日最低、量爆增至前日 3.2 倍；NYSE TSM 7/17 Fri 收 $398.37（-$11.37、-2.77% vs 7/16 $409.74、盤後 $397.72、stockanalysis.com 確認）——失守 $400）。⚠️⚠️ 今日最重要：全球 AI 晶片股正式跌入熊市——費城半導體指數（SOX）7/17 收跌 -1.6%（盤中一度 -5.7%）、自 6/22 歷史高點回落 -20.2%、單週 -9%（3-6 月自低點曾飆 +105%）；催化：中國新創 Moonshot 發表號稱全球最大的開源 AI 模型 Kimi K3、Alphabet 傳 Gemini 3.5 Pro 交付落後——市場對「AI 投資能否兌現、雲端資本支出能否持續」信心動搖；MRVL/ARM/INTC 自高點已 -30%+。📉 台股同步崩跌：三大法人 7/17 合計賣超逾 2,631.5 億元創台股史上最大單日紀錄、外資提款台積電破千億；2330 遭外資單日賣超 44,184 張（連 7 賣、買 17,479／賣 61,663）——惟投信買超 1,489 張（連 2 買）、自營買超 2,759 張（連 7 買）逆勢承接、三大法人合計賣超 39,936 張。📊 台美 ADR 溢價逆勢擴大至 +12.0%（ADR $398.37 vs 理論值 $355.6、匯率 ~32.2）——因台股 -7.29% 超跌於 ADR -2.77%、隱含台股 7/20 週一具技術性反彈空間（✅ 7/20 早盤：2330 開 NT$2,300、09:17 暫報 NT$2,330、+1.75%——收盤以 TWSE 官方為準）。⚠️ 隔夜美股晶片股（7/17）：NVDA 盤中 -3% 後回升、AMD 盤中 -5%、INTC 盤中 -4%（尾盤均自低點回升、SOX 收僅 -1.6%）；記憶體鏈延續回吐（MU 收 ~$849）；⭐ 逆勢：AAPL 相對抗跌。✅ 基本面未變（與股價短線脫鉤）：Q2 2026 法說會（7/16）大超預期——營收 $40.20B（+33.7% YoY）、稀釋 EPS $4.31/ADR（+77.4% YoY、超共識 ~+9.4%、連 11 季超預期）、毛利率 67.7% 超指引上緣；全年營收成長上修至「略高於 40%」、2026 CapEx 上調至 $60-64B（含 Arizona 追加 $100B 投資）、股利 NT$24/股（+33%）；HPC 占 66%、2nm 首度貢獻 3%；管理層：「AI 需求比先前預期更強」。⚠️ 風險：AI 估值/情緒修正是否續探底、外資連 7 史詩級賣超能否止穩、美 232 條款半導體關稅 25% + 輸中晶片轉運查驗；估值經崩跌後大幅消化（台股 P/E 降至 ~26.2x TTM、TSM ~29.2x）、台幣 ~32.2。技術面（官方收盤計算）：7/17 收 2,290 跌破 5MA ~2,412 / 20MA ~2,428 / 回測 60MA ~2,324、RSI(14) 41.0 跌破中軸、MACD 綠柱擴大至 -15.2（DIF 17.4 < DEA 32.5）、KDJ K35.7/D46.2/J14.7 死叉且 J 值墜入超賣；支撐 2,290（7/17 低）／2,200（整數關）／2,094（120MA）、壓力 2,322-2,324（布林下軌/60MA）／2,412（5MA）／2,470（7/16 高）。⭐ 中長線基底未變：全年成長 >40%、CapEx $60-64B、AI 需求「比預期更強」、2nm 已貢獻 3% 營收且 5 座廠爬坡、HPC 占 66%、股利 +33%、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、全先進製程漲價 5-10%——本次為估值/情緒面修正、非基本面惡化",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性",
    # 一句話標題
    "headline":   "⚠️⚠️ AI 晶片股正式跌入熊市：SOX 自 6/22 高點 -20.2%、單週 -9%；台股 2330 7/17 崩跌 -7.29% 收 NT$2,290、三大法人單日賣超 2,631.5 億創台股史上最大、外資提款台積電破千億（連 7 賣 44,184 張）；惟基本面未變（Q2 大超預期、全年上修 >40%）、股價與基本面短線脫鉤——ADR 溢價逆勢擴大至 +12% 隱含台股超跌、今日 7/20 週一觀察技術性反彈",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⚠️⚠️ 最重要：全球 AI 晶片股正式跌入熊市——SOX 7/17 收 -1.6%（盤中一度 -5.7%）、自 6/22 史高回落 -20.2%、單週 -9%（3-6 月曾自低點飆 +105%）；催化：中國 Moonshot 發表號稱全球最大開源模型 Kimi K3、Alphabet 傳 Gemini 3.5 Pro 交付落後——AI 投資報酬疑慮引爆獲利了結",
        "📉 台股史詩級崩跌：2330 7/17 官方收 NT$2,290（-180、-7.29%、收盤即當日最低）、量爆增至 97,362 張（額 2,290 億、前日 3.2 倍）；三大法人合計賣超逾 2,631.5 億元創台股史上單日最大、外資提款台積電破千億",
        "📊 籌碼：外資 7/17 賣超 44,184 張（連 7 賣、買 17,479／賣 61,663）——惟投信買超 1,489 張（連 2 買）、自營買超 2,759 張（連 7 買）逆勢承接、合計賣超 39,936 張；內資續扮穩定力量、外資能否止穩為最大觀察點",
        "⚠️ NYSE TSM 7/17 收 $398.37（-2.77%）失守 $400——惟相對板塊抗跌（MRVL/ARM/INTC 自高點已 -30%+）；晶片股尾盤自低點回升（NVDA 盤中 -3%、AMD -5%、INTC -4% 收斂），記憶體鏈延續回吐；⭐ AAPL 相對抗跌",
        "📊 台美 ADR 溢價逆勢擴大至 +12.0%（$398.37 vs 理論值 $355.6、匯率 ~32.2）——台股 -7.29% 超跌於 ADR -2.77%、隱含 7/20 週一台股具技術性反彈空間",
        "✅ 基本面未變（與股價脫鉤）：Q2 法說會（7/16）大超預期——營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%；全年成長上修 >40%、CapEx $60-64B（含 Arizona 追加 $100B）、股利 +33%；管理層：「AI 需求比預期更強」——本次為估值/情緒修正、非基本面惡化",
        "💡 估值大幅消化：崩跌後台股 P/E 降至 ~26.2x TTM（前 ~28.3x）、TSM ~29.2x；分析師目標密集區美銀 $590、Barclays $625、Citi NT$3,800（法說會後待重估、多空落差擴大）",
        "⚠️ 風險續追蹤：AI 估值/情緒修正是否續探底、外資史詩級賣超能否止穩；美 232 條款關稅 25% + 輸中晶片轉運查驗；Intel 18A 競爭；台幣 ~32.2；技術面跌破 5MA/20MA、回測 60MA ~2,324、KDJ 死叉墜超賣",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-20 Mon（最新確認收盤：台股 2330 7/17 Fri 官方收 NT$2,290（-180、-7.29% vs 7/16 NT$2,470、開 2,375／高 2,395／低 2,290、量 97,362 張 TWSE 口徑、成交額 NT$2,290.52 億、成交 1,150,086 筆、TWSE 確認）；NYSE TSM 7/17 Fri 收 $398.37（-$11.37、-2.77% vs 7/16 $409.74、盤後 $397.72、stockanalysis.com 確認、自我檢核 409.74×(1-2.77%)=398.39 ✓））。⚠️⚠️ 最重要：全球 AI 晶片股正式跌入熊市——費城半導體指數（SOX）7/17 收跌 -1.6%（盤中一度 -5.7%）、自 6/22 歷史高點回落 -20.2%、單週 -9%（3-6 月自低點曾飆漲 +105%）；催化：（1）中國新創 Moonshot 發表號稱全球最大的開源 AI 模型 Kimi K3；（2）Alphabet 傳 Gemini 3.5 Pro 交付落後——市場對「巨額 AI 投資能否兌現、雲端資本支出能否持續」信心動搖、引爆全面獲利了結；MRVL/ARM/INTC 自高點已 -30%+。📉 台股同步史詩級崩跌：2330 7/17 收 NT$2,290（-7.29%、收盤即當日最低）、量爆增至 97,362 張；三大法人 7/17 合計賣超逾 2,631.5 億元創台股史上單日最大紀錄、外資提款台積電破千億——2330 遭外資賣超 44,184 張（連 7 賣、買 17,479／賣 61,663）；惟投信買超 1,489 張（連 2 買）、自營買超 2,759 張（連 7 買）逆勢承接、三大法人合計賣超 39,936 張。📊 台美 ADR 溢價逆勢擴大至 +12.0%（$398.37 vs 理論值 $355.6、匯率 ~32.2）——因台股 -7.29% 超跌於 ADR -2.77%、隱含台股 7/20 週一具技術性反彈空間（✅ 7/20 早盤：開 NT$2,300、09:17 暫報 NT$2,330、+1.75%）。⭐ 關鍵判斷：本次為 AI 估值/情緒面修正、非 TSMC 基本面惡化——Q2 法說會（7/16）大超預期基本面完全未變：營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7% 超指引上緣；全年營收成長上修至「略高於 40%」、2026 CapEx 上調至 $60-64B（含 Arizona 追加 $100B 投資）、股利 NT$24/股（+33%）；HPC 占 66%、2nm 首度貢獻 3%；管理層明言「AI 需求比先前預期更強」——股價與基本面短線嚴重脫鉤。⭐ 今日重點：（1）📉 台股 7/20 週一觀察：崩跌後是否出現技術性反彈——ADR 溢價擴大至 +12% + 估值消化（P/E 26.2x）+ 內資逆勢承接為支撐、惟 AI 情緒面能否止穩為關鍵；（2）📊 籌碼：外資連 7 史詩級賣超能否止穩為最大觀察點、投信自營持續逆勢買進；（3）💡 估值大幅消化：台股 P/E 降至 ~26.2x TTM（前 ~28.3x）、TSM ~29.2x。技術面（官方收盤計算）：5MA ~2,412 / 20MA ~2,428 / 60MA ~2,324；7/17 收 2,290 跌破 5MA/20MA、回測 60MA ~2,324、RSI(14) 41.0 跌破中軸、MACD 綠柱擴大至 -15.2（DIF 17.4 < DEA 32.5）、KDJ K35.7/D46.2/J14.7 死叉且 J 值墜入超賣；支撐 2,290（7/17 低）／2,200（整數關）／2,094（120MA）、壓力 2,322-2,324（布林下軌/60MA）／2,412（5MA）／2,470（7/16 高）。⚠️ 風險：AI 估值/情緒修正是否續探底、外資史詩級賣超能否止穩、美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.2（每升值 1% 毛利率 -40bps）。⭐ 中長線基底未變：全年成長 >40%、CapEx $60-64B、AI 需求「比預期更強」、2nm 5 座廠爬坡且已貢獻 3% 營收、股利 +33%、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、漲價 5-10% 定價權——超跌後中長線價值浮現，惟需 AI 情緒面止穩確認。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/20 Mon）關鍵觀察：（1）⚠️⚠️ AI 晶片股熊市是否續探底——SOX 已自 6/22 高點 -20.2% 正式進入熊市、單週 -9%；催化為 AI 投資報酬疑慮（Moonshot Kimi K3 開源模型、Alphabet Gemini 交付落後）而非 TSMC 基本面惡化，惟情緒面能否止穩為短線最大變數；（2）📉 台股 7/20 週一觀察技術性反彈：7/17 崩跌 -7.29% 收 2,290、收盤即當日最低——超跌（ADR 溢價擴大至 +12%）+ 估值消化（P/E 26.2x）+ 內資逆勢承接為支撐，守穩 2,290（7/17 低）／2,200 整數關為短線關鍵、若破則下看 2,094（120MA）；反彈壓力 2,322-2,324（布林下軌/60MA）／2,412（5MA）；（3）📊 籌碼：外資 7/17 賣超 44,184 張連 7 賣、三大法人單日賣超 2,631.5 億創史上最大——外資能否止穩為最大觀察點；投信 +1,489、自營 +2,759 逆勢承接續扮穩定力量；（4）✅ 基本面錨：Q2 大超預期（EPS +77.4%、全年上修 >40%、CapEx $60-64B、股利 +33%、Arizona 追加 $100B）完全未變、股價與基本面短線脫鉤——超跌後中長線價值浮現；（5）💱 美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.2 續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-17",       # 最新資料日（7/17 Fri、TWSE T86 官方）
    "foreign":  -44184,            # 外資 7/17 賣超 44,184 張（連 7 賣、買 17,479／賣 61,663、單日提款台積電破千億、台股史上最大單日賣壓之一）
    "trust":    1489,              # 投信 7/17 買超 1,489 張（連 2 買、逆勢承接）
    "dealer":   2759,              # 自營商 7/17 買超 2,759 張（連 7 買、逆勢承接）；三大法人合計 -39,936 張
    # 近5日（由舊到新：7/13, 7/14, 7/15, 7/16, 7/17）；TWSE T86 口徑（張）
    "foreign_5d": [-2045, -12416, -3427, -4215, -44184],  # 外資（連 7 賣、7/17 史詩級賣超爆量）
    "trust_5d":   [530, 1841, -596, 528, 1489],           # 投信（7/17 買超放大、逆勢承接）
    "dealer_5d":  [623, 2570, 456, 1061, 2759],           # 自營（7/17 買超放大、逆勢承接）
    "foreign_ownership_pct": "~74.5%",  # 外資持股比例（估，7/17 大賣後降低；官方以 TWSE 外資持股餘額為準）
    "foreign_ownership_shares": "約 193 億股（估）",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       41.0,   "rsi_signal": "RSI(14) 41.0（官方收盤計算）——7/17 崩跌 -7.29% 後跌破中軸、轉弱；惟已接近超賣區、留意超跌反彈",
    "macd":         -15.2, "macd_signal": "MACD 綠柱擴大至 -15.2（DIF 17.4 < DEA 32.5、官方收盤計算）——7/17 巨量長黑後動能急轉空、綠柱由 -4.9 擴大至 -13、死叉確立",   "macd_color": "#F44336",
    "kdj_k":        35.7,   "kdj_d": 46.2, "kdj_j": 14.7, "kdj_signal": "KDJ K(35.7)/D(46.2)/J(14.7)（官方收盤計算）——7/17 崩跌後 K 下穿 D 形成死叉、J 值 16 墜入超賣區，短線轉弱、惟超賣後留意技術性反彈",
    "ma5":       "2,412",   # 5MA（官方收盤計算，7/13-7/17：2440/2420/2440/2470/2290）
    "ma20":      "2,428",   # 20MA（官方收盤計算）
    "ma60":      "2,324",  # 60MA（官方收盤計算）
    "ma120":     "2,094",  # 120MA（官方收盤計算）
    "bb_upper":  "2,535",  # 布林上軌（20MA+2σ、官方收盤計算；恰與 6/23 盤中史高 2,535 重合）
    "bb_mid":    "2,428",   # 布林中軌（20MA）
    "bb_lower":  "2,322",  # 布林下軌（20MA-2σ、官方收盤計算；7/17 收 2,290 已跌破下軌、超賣）
    "vol_ma5":  "47,947",   # 5日均量（張、TWSE 口徑、含 7/17 爆量 97,362）
    "trend":    "📉 史詩級崩跌、技術面全面轉空：台股 2330 7/17 官方收 NT$2,290（-180、-7.29%、量爆增至 97,362 張 TWSE 口徑、前日 3.2 倍）——收盤即當日最低（開 2,375／高 2,395／低 2,290）、跌破 5MA（~2,412）、20MA（~2,428）、60MA（~2,324）與布林下軌（2,322）。⚠️⚠️ 主因：全球 AI 晶片股正式跌入熊市——SOX 7/17 收 -1.6%（盤中一度 -5.7%）、自 6/22 史高 -20.2%、單週 -9%；催化 Moonshot Kimi K3 開源模型 + Alphabet Gemini 交付落後引爆 AI 投資報酬疑慮；台股三大法人單日賣超 2,631.5 億創史上最大、外資提款台積電破千億。NYSE TSM 7/17 收 $398.37（-2.77%）失守 $400、惟相對板塊抗跌。📊 技術指標（官方收盤計算、估）：RSI(14) 41.0 跌破中軸、MACD 綠柱擴大至 -15.2（DIF 17.4 < DEA 32.5）、KDJ K(35.7)/D(46.2)/J(14.7) 死叉且墜超賣——短線動能急轉空。⭐ 關鍵：本次為 AI 估值/情緒修正、非基本面惡化（Q2 大超預期、全年上修 >40%、股利 +33%）——股價與基本面短線脫鉤、超跌後中長線價值浮現。下一觀察：（1）7/20 週一觀察技術性反彈——ADR 溢價逆勢擴大至 +12%、估值消化（P/E 26.2x）、內資逆勢承接（投信 +1,489、自營 +2,759）為支撐；（2）守穩支撐 2,290（7/17 低）／2,200（整數關）／2,094（120MA）；（3）反彈壓力 2,322-2,324（布林下軌/60MA）／2,412（5MA）／2,470（7/16 高）；（4）外資連 7 史詩級賣超能否止穩為最大觀察點；（5）AI 情緒面能否止穩為短線最大變數。",
    "support":  "2,290 / 2,200 / 2,094 (7/17 低、心理關卡、120MA)",
    "resist":   "2,322-2,324 / 2,412 / 2,470 (布林下軌+60MA、5MA、7/16 高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-17 止），請以看盤軟體為準；7/17 官方收 NT$2,290（-7.29%）跌破 5MA ~2,412 / 20MA ~2,428、回測 60MA ~2,324、RSI(14) 41.0、MACD 綠柱擴大至 -15.2、KDJ K35.7/D46.2/J14.7 死叉墜超賣；下方支撐 2,290 / 2,200 / 2,094、上方壓力 2,322-2,324 / 2,412 / 2,470；⚠️⚠️ AI 晶片股正式跌入熊市（SOX -20.2%）vs ✅ TSMC 基本面大超預期未變（全年上修 >40%、CapEx $60-64B、股利 +33%）——本次為估值/情緒修正、股價與基本面脫鉤；7/20 週一超跌反彈 vs 續探底、外資能否止穩為兩大變數",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-17",       # 最新資料日（7/17 Fri 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,275",            # 台積電期貨近月（估、現貨 7/17 崩跌收 NT$2,290、恐續貼水開出）
    "futures_oi":    "192,000",          # 未平倉量（口）（估、崩跌日避險部位增加）
    "futures_basis": "-15",              # 估逆價差（崩跌 + 避險情緒濃、貼水擴大）
    "call_oi":       "108,000",
    "put_oi":        "104,000",
    "pcr":           "0.96",             # Put/Call 比率（估、崩跌後避險需求大幅升溫、接近 1）
    "pcr_signal":    "偏空（Put/Call 估 0.96；台股 7/17 崩跌 -7.29% 收 NT$2,290、SOX 跌入熊市、外資史詩級賣超——避險需求大幅升溫、Put OI 逼近 Call；⭐ 惟超賣後留意技術性反彈、內資逆勢承接；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,350",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,094", "Call 62,000 / Put 24,000", "120MA、中期支撐"),
        ("2,150", "Call 60,000 / Put 22,000", "次一支撐"),
        ("2,200", "Call 55,000 / Put 18,000", "心理關卡、下方支撐"),
        ("2,290", "Call 45,000 / Put 11,000", "7/17 官方收、多空分界"),
        ("2,320", "Call 40,000 / Put 8,000",  "布林下軌 2,322/60MA 2,324、反彈壓力帶"),
        ("2,350", "Call 38,000 / Put 6,500",  "最大痛苦（估）"),
        ("2,412", "Call 30,000 / Put 4,500",  "5MA、上方壓力"),
        ("2,470", "Call 22,000 / Put 3,000",  "7/16 高、壓力帶"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/17 NT$2,290）；NT$2,350 為估最大痛苦、下方支撐 2,290 / 2,200 / 2,094（120MA）、上方壓力 2,320（布林下軌/60MA）/ 2,412（5MA）/ 2,470；⚠️⚠️ AI 晶片股正式跌入熊市（SOX -20.2%）、台股三大法人單日賣超 2,631.5 億創史上最大 vs ✅ TSMC 基本面大超預期未變（全年上修 >40%、CapEx $60-64B、股利 +33%）——本次為估值/情緒修正、非基本面惡化；7/20 週一超跌反彈 vs 續探底、外資連 7 賣能否止穩為關鍵；中長線基底未變（AI 需求「比預期更強」、2nm 已貢獻 3% 營收、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、Arizona 追加 $100B 投資）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-17 Fri TWSE 官方收盤；美股為 7/17 Fri 收盤、Yahoo Finance API 確認；韓股 7/17 無交易（KRX 休市）、列 7/16 收盤）
    ("設備", "ASML",    "艾司摩爾",    "1,747.58", "-2.09%", "⭐ EUV 設備龍頭；7/17 隨晶片股拋售續跌；TSMC CapEx 上調至 $60-64B 為設備鏈利多"),
    ("設備", "AMAT",    "應用材料",    "529.66", "-5.57%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（7/17 領跌設備股）；TSMC CapEx $60-64B 為設備鏈利多"),
    ("設備", "LRCX",    "科林研發",    "313.30", "-2.39%", "蝕刻設備（7/17 續跌）；TSMC CapEx $60-64B + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "212.75", "-3.02%", "量測設備（7/17 回檔）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","155.00","-9.88%", "與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/17 隨台股崩跌、幾近跌停、TWSE 官方）"),
    ("記憶", "000660.KS","SK 海力士",  "1,842,000 KRW", "休市", "HBM4、與 NVIDIA 多年期合作；7/17 韓股無交易（Yahoo API）、列 7/16 收盤（當日 -11.53%）；7/20 開盤反彈約 +0.7%"),
    ("記憶", "MU",      "美光",        "848.95", "-0.50%", "HBM / DDR5（7/17 跌幅收斂）；AI 記憶體修正後 HBM4 中長線邏輯未變"),
    ("封裝", "3711.TW", "日月光",      "614.00", "-9.97%", "CoWoS / 先進封裝（7/17 隨台股崩跌、幾近跌停、TWSE 官方）"),
    ("封裝", "AMKR",    "Amkor",       "62.94",  "-0.13%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/17 相對抗跌、近持平）"),
    ("客戶", "NVDA",    "輝達",        "202.81", "-2.21%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/17 盤中一度 -3% 尾盤回升（AI 投資報酬疑慮引獲利了結）"),
    ("客戶", "AAPL",    "蘋果",        "333.74", "+0.14%", "⭐ #2 客戶 18%；7/17 逆勢收漲、熊市中資金避風港；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "370.83", "-0.97%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/17 尾盤回升、跌幅收斂）"),
    ("客戶", "AMD",     "超微",        "495.76", "-1.03%", "#4 客戶；MI350/MI450、首發 2nm AI 晶片（7/17 盤中一度 -5% 尾盤大幅回升、守住 $495）"),
    ("客戶", "2454.TW", "聯發科",      "3,370",  "-8.92%", "手機 SoC 客戶 9%（7/17 隨台股崩跌重挫、TWSE 官方）；面臨漲價成本"),
    ("競爭", "INTC",    "英特爾",      "95.04",  "-2.00%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/17 尾盤回升、自高點已 -30%+）"),
    ("競爭", "005930.KS","三星電子",   "255,000 KRW", "休市", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/17 韓股無交易（Yahoo API）、列 7/16 收盤（當日 -8.77%）；7/20 開盤約 -1.2%"),
    ("產業", "SOX",     "費城半導體指數","11,673.89", "-1.63%", "⚠️⚠️ 7/17 正式跌入熊市——自 6/22 史高 14,634.66 回落 -20.2%、單週 -10%（盤中一度 -5.7% 尾盤收斂）；3-6 月曾自低點飆漲逾 +100%"),
    ("處分後", "ARM",   "Arm Holdings","267.19", "+1.98%", "TSMC 4/29 全數出脫後（7/17 超跌反彈 +1.98%、惟自高點仍 -30%+）"),
]
...
ANALYST_RATINGS = [
    ...
    ("Barclays",       "增持",     "US$625",   "2026-07-02", "🚀 7/2 大幅上調目標價自 $470 至 $625（維持 Overweight、自 7/15 收 $419.48 具 +49.0% 空間）；AI 需求結構性 + A14/A16 領先、TSMC 為 AI CapEx 循環終極受惠者"),
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/17 收 $398.37 具 +23.1% 上行空間；美銀 $590、Citi 台股目標 NT$3,800、Barclays $625、Nomura NT$3,425；7/16 法說會大超預期後目標價待重估、預期新一輪上調）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+23.09%",  # 從 NYSE 7/17 收 $398.37 至平均目標 $490.34（共識強力買入；法說會大超預期後待重估）
}
...
IR_CALENDAR = [
    ...
    ("2026-07-20", "📊 報告日：崩跌後觀察技術性反彈", "7/20 Mon 報告日；最新確認收盤：台股 2330 7/17 NT$2,290（-7.29%）、NYSE TSM 7/17 $398.37（-2.77%）；早盤 2330 開 NT$2,300、09:17 暫報 NT$2,330（+1.75%）反彈、加權指數持平；ADR 溢價 +12% 隱含台股超跌；觀察能否站回 2,322-2,324（布林下軌/60MA）壓力帶、外資賣超能否止穩",  "#FFF9C4"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "11,673.89", "-1.63%", False),  # 7/17 收（Yahoo）；盤中一度 -5.7%、自 6/22 史高 -20.2% 正式入熊市
    ("台股加權指數",    "42,671.27", "-6.47%", False),  # 7/17 官方收盤（-2,953.71 點、史上最大單日跌點、TWSE）
    ("VIX 恐慌指數",    "18.77",    "+12.19%", False),   # 7/17 收（Yahoo）恐慌情緒跳升
    ("WTI 原油",        "$82.49",   "+4.48%",  False),   # 7/17 收（Yahoo；7/20 亞洲盤 ~$83.7）
    ("10Y 美債殖利率",  "4.54%",   "-3bp",   False),   # 7/17 收（Yahoo ^TNX、避險買盤）
    ("台幣/美元",       "~32.33",   "小貶",  False),   # 7/20 Yahoo TWD=X（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$202.81", "-2.21%",  False),   # #1 TSMC 客戶（22%），7/17 盤中 -3% 尾盤回升
    ("ASML",            "$1,747.58", "-2.09%",  False),  # 設備股 7/17 收（Yahoo）
]
...
VALUATION = {
    "pe_current":   32.1,   # 同 STOCK["pe_ratio"]（7/14 官方收 NT$2,420 ÷ 近四季 EPS ~75.5、statementdog）
    ...
    "pb_current":  18.14,   # 同 STOCK["pb_ratio"]（7/14 官方收 NT$2,420 ÷ 每股淨值 ~133.4）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
    "ev_ebitda":    21.0,   # EV/EBITDA（估算）
    "peg":          1.12,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準）、基於 7/14 官方收 NT$2,420：台股 P/E ~32.1x（TTM、近四季 EPS ~75.5、statementdog）、P/B ~18.1x（每股淨值 ~133.4）；NYSE TSM 7/14 收 $420.39、USD P/E 約 35.7x（估）；估值仍處 5 年高位區間、PEG 約 1.1（以 5 年 EPS CAGR ~33% 計）；🎯 7/16 法說會倒數 1 天——6 月營收創高已確認 Q2 達標、法人估毛利率挑戰 70%、財報成色與全年財測上修與否為估值消化關鍵；💰 漲價 5-10%（三星跟進 15%）+ CoWoS 訂滿至 2027 支撐定價權；中長線基底未變：NVIDIA #1 客戶/A16 首發 + TSMC-Amkor 美國封裝長約 + 2nm 量產 70-80% 良率",
}
...
RELATIVE = {
    "date":           "2026-07-14",       # 最新資料日（台股 2330 與 NYSE TSM 均為 7/14 Tue 收盤）
    "tsmc_tw_1d":     -0.82,   # 2330 7/14 官方收 NT$2,420（-20 vs 7/13 NT$2,440、TWSE）
    "taiex_1d":       -1.42,   # 加權指數 7/14 收 44,737.95（-642.57、TWSE 官方）——2330 相對抗跌
    "sox_1d":         2.54,    # SOX 7/14 收 12,661.93（+2.54% 大反彈、Yahoo）
    "tsmc_nyse_1d":   -0.28,   # TSM ADR 7/14 $420.39（-$1.19；法說會前觀望、明顯落後 SOX）
    "tsmc_tw_5d":     -0.82,   # 5 日（NT$2,420 vs 7/7 官方收 NT$2,440）
    "taiex_5d":       -1.3,    # （44,737.95 vs 7/7 約 45,3xx、估）
    "tsmc_tw_1m":     +1.9,    # 月線（NT$2,420 vs 6/16 官方收 NT$2,375）
    "taiex_1m":       +1.0,    # （估）
    "tsmc_tw_ytd":    +15.5,   # YTD 累計（NT$2,420 vs 年初 ~NT$2,095）
    "taiex_ytd":      +11.4,   # （估）
    "sox_ytd":        +47.7,   # SOX YTD（費半 12,661.93、估）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/10 NYSE 收盤 & 7/9 TW 官方收盤計算）
    "adr_price":     420.39,   # TSM NYSE 7/14 收盤（-0.28%、Yahoo Finance API）
    "adr_parity":    376.46,   # 理論值 = 5 × NT$2,420 ÷ 32.14
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.14,   # 台幣匯率（7/15 Yahoo TWD=X 32.142）
    "adr_premium_pct": "+11.67", # (420.39 - 376.46) / 376.46 × 100 = +11.67%（匯率 32.14）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 7/14 Tue 收盤：台股 NT$2,420（-0.82%、量 42,857 張 TWSE 口徑、市值 NT$62.8 兆）——加權指數同日 -1.42% 下相對抗跌；NYSE $420.39（-0.28%、日內 $418.86-430.87）——同日 SOX +2.54% 大反彈、TSM 明顯落後屬法說會前觀望。📊 籌碼面（T86 官方）：7/14 外資賣超 12,416 張（連 4 賣、4 日累計 -31,365 張）、投信買超 1,841 張、自營買超 2,570 張、合計賣超 8,005 張——外資賣壓為法說會前最大壓力、投信自營對沖。📊 台美 ADR 溢價 +11.67%（$420.39 vs 理論值 $376.46、匯率 32.14）——較 7/13 +10.8% 微擴。🎯🎯 明日（7/16）Q2 法說會：分析師估 Q2 營收 ~$40B（+32% YoY）、獲利 +50%+ 連 5 季創高、毛利率挑戰 70%；🚀 分析師續挺：Citi NT$3,800、Barclays $625、Nomura NT$3,425；⭐ 中長線結構性基底未變：漲價 5-10%（三星跟進 15%）+ CoWoS 訂滿至 2027 + NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 量產 70-80% 良率 + 4 大 CSP CapEx $725B",
}
...
NEWS = [
    {
        ...
        "title": "⚠️⚠️ AI 晶片股正式跌入熊市：SOX 自 6/22 高點 -20.2%、單週 -9%",
        "body": "105% AI 漲勢急煞車、獲利了結全面引爆：（1）📉 費城半導體指數（SOX）7/17 收 -1.6%（盤中一度重挫 -5.7%尾盤收斂）、自 6/22 歷史高點回落 -20.2% 正式進入熊市、單週跌逾 9%——3-6 月自低點曾飆漲 +105%；（2）🔑 催化：中國新創 Moonshot 發表號稱全球最大的開源 AI 模型 Kimi K3、Alphabet 傳 Gemini 3.5 Pro 交付落後——市場對「巨額 AI 投資能否兌現、雲端資本支出能否持續」信心動搖；（3）📊 MRVL/ARM/INTC 自高點已重挫逾 30%；NVDA 盤中 -3%、AMD -5%、INTC -4% 尾盤自低點回升；（4）⭐ 關鍵：本波為 AI 估值/情緒面修正、非基本面惡化——TSMC Q2 法說會才證實「AI 需求比預期更強」。",
        "source": "Bloomberg / Fortune / Yahoo Finance 2026-07-17", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "📉 台股 2330 7/17 崩跌 -7.29% 收 NT$2,290、三大法人單日賣超 2,631.5 億創史上最大",
        "body": "史詩級賣壓、台積電成重災區：（1）📉 台股 2330 7/17 官方收 NT$2,290（-180、-7.29%、開 2,375／高 2,395／低 2,290、收盤即當日最低）——量爆增至 97,362 張（TWSE 口徑、成交額 NT$2,290.52 億、成交 1,150,086 筆、前日 3.2 倍）；（2）💥 台股三大法人 7/17 合計賣超逾 2,631.5 億元、創台股史上單日最大紀錄、外資提款台積電破千億；（3）📊 技術面全面轉空：跌破 5MA ~2,412 / 20MA ~2,428、回測 60MA ~2,324、RSI(14) 41.0、KDJ K35.7/D46.2/J14.7 死叉墜超賣；（4）🎯 7/20 週一觀察超跌反彈——ADR 溢價逆勢擴大至 +12%、估值消化（P/E 26.2x）、內資逆勢承接為支撐。",
        "source": "TWSE / 鉅亨網 / Yahoo 股市 2026-07-17", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "📊 外資 7/17 賣超台積電 44,184 張連 7 賣：惟投信自營逆勢承接",
        "body": "內外資態度嚴重分歧：（1）📊 三大法人 7/17 合計賣超 39,936 張（T86 官方）：外資賣超 44,184 張——連 7 賣（買 17,479／賣 61,663）、提款破千億、單日賣壓為台股史上之最；（2）⭐ 投信買超 1,489 張（連 2 買）、自營買超 2,759 張（連 7 買）——內資逆勢承接、續扮穩定力量；（3）🎯 觀察：外資史詩級賣超能否止穩為台股走勢最大變數；ADR 溢價逆勢擴大至 +12% 顯示台股相對 ADR 超跌；（4）⚠️ 地緣背景：美 232 條款關稅 25% + 輸中晶片轉運查驗續追蹤。",
        "source": "TWSE T86 / Goodinfo 2026-07-17", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⚠️ NYSE TSM 7/17 收 $398.37（-2.77%）失守 $400——惟相對板塊抗跌、ADR 溢價擴至 +12%",
        "body": "相對抗跌 + 台股超跌隱含反彈空間：（1）⚠️ NYSE TSM 7/17 收 $398.37（-$11.37、-2.77%、盤後 $397.72）失守 $400——隨全球晶片股拋售，惟相對板塊抗跌（多檔成分股自高點已 -30%+）；（2）📊 台美 ADR 溢價逆勢擴大至 +12.0%（$398.37 vs 理論值 $355.6、匯率 ~32.2）——因台股 -7.29% 超跌於 ADR -2.77%；（3）🎯 隱含台股 7/20 週一具技術性反彈空間；（4）⭐ 記憶體鏈延續回吐（MU 收 ~$849）；AAPL 相對抗跌；ARM/AMD/INTC 尾盤自低點回升、SOX 收僅 -1.6%。",
        "source": "stockanalysis.com / Yahoo Finance 2026-07-17", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "✅ 基本面未變（與股價脫鉤）：Q2 大超預期、全年上修 >40%、Arizona 追加 $100B",
        "body": "股價與基本面短線嚴重脫鉤：（1）📊 Q2 法說會（7/16）大超預期——營收 $40.20B（+33.7% YoY、+12.0% QoQ）、稀釋 EPS $4.31/ADR（+77.4% YoY、超共識 ~+9.4%、連 11 季超預期）、毛利率 67.7% 超指引上緣；（2）🚀 全年營收成長上修至「略高於 40%」、2026 CapEx 上調至 $60-64B（含 Arizona 追加 $100B 投資、承諾總額達 $165B）；（3）💰 2026 股利 NT$24/股（+33% YoY）；HPC 占 66%、2nm 首度貢獻 3%、≤7nm 占 77%；（4）⭐ 管理層明言「AI 需求比先前預期更強」——本波拋售為估值/情緒修正、非基本面惡化、超跌後中長線價值浮現。",
        "source": "TSMC 法說會 / Yahoo Finance / Bloomberg 2026-07-16", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "💰 定價權未變、CoWoS 訂滿至 2027：分析師目標密集區 $590-625、法說會後待重估",
        "body": "中長線結構未變、估值大幅消化：（1）💰 先進製程漲價 5-10%（3nm 擴及 5/7nm）＋三星跟進調漲 4/5nm 約 15%——定價權全面轉向代工廠，Q2 毛利率 67.7% 為佐證；（2）🚀 CoWoS 2026 產能已被 NVIDIA/Broadcom 預訂一空、缺口延續至 2027 年底；2nm 5 座廠（新竹 2＋高雄 3）爬坡中；（3）📊 分析師目標密集區：美銀 $590、Barclays $625、Citi NT$3,800——法說會大超預期後待重估、與現價落差擴大；（4）💡 崩跌後估值大幅消化：台股 P/E 降至 ~26.2x TTM、TSM ~29.2x——超跌後中長線價值浮現，惟需 AI 情緒面止穩確認。",
        "source": "TSMC 法說會 / TechNews / 經濟日報 2026-07-16 - 07-17", "impact": "正面",
    },
]

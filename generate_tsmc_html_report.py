...
STOCK = {
    # 台股 2330
    "tw_price":       "1,915",        # 收盤價（數字加逗號，不含 NT$）
    "tw_change":      "+35",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "+1.86%",       # 漲跌幅（含+/-號）
    "tw_volume":      "39,307",       # 成交量（張）—估算（接近均量）
    "tw_open":        "1,880",
    "tw_high":        "1,920",
    "tw_low":         "1,875",
    "market_cap":     "NT$49.27 兆",
    "pe_ratio":       "~28.9x",       # 台股 P/E（EPS TTM=66.25）
    "pb_ratio":       "~9.20x",       # 台股 P/B（股淨比）
    "div_yield":      "~1.20%",       # 殖利率（NT$23/NT$1,915）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "349.49",       # 收盤（3/11 EST）
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-10 收盤  |  NYSE 數據：2026-03-10 收盤（4AM 台灣時間）  |  2 月月營收 3/10 公告（YoY +22.2%）",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-10",
    "foreign":  +3200,    # 外資（正=買超）— 受 2 月營收年增22.2% 激勵買超（估）
    "trust":    +850,     # 投信
    "dealer":   -180,     # 自營商（含避險）
    # 近5日（由舊到新）
    "foreign_5d": [-3400, +2800, +5230, +1200, +3200],
    "trust_5d":   [+500,  +800,  +1120, +700,  +850],
    "dealer_5d":  [+300,  +600,  +430,  -200,  -180],
    "foreign_ownership_pct": "76.2%",  # 外資持股比例（估）
    "foreign_ownership_shares": "約 196.5 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       58.5,   "rsi_signal": "偏多",
    "macd":         -5.8,   "macd_signal": "空轉多訊號待確認",   "macd_color": "#FF9800",
    "kdj_k":        62.3,   "kdj_d": 52.1, "kdj_j": 82.7, "kdj_signal": "偏多",
    "ma5":       "1,900",
    "ma20":      "1,958",
    "ma60":      "1,840",
    "ma120":     "1,720",
    "bb_upper":  "2,068",   # 布林上軌
    "bb_mid":    "1,958",   # 布林中軌（20MA）
    "bb_lower":  "1,848",   # 布林下軌
    "vol_ma5":  "33,000",   # 5日均量（張）
    "trend":    "反彈 / 受 2 月營收年增22.2% 催化，台股偏多，NYSE 受美伊戰爭風險拖累回落",
    "support":  "1,870–1,890",
    "resist":   "1,940–1,960",
...
DERIVATIVES = {
    "date":          "2026-03-10",
    "futures_price": "1,918",      # 台積電期貨近月收盤（估）
    "futures_oi":    "69,100",     # 未平倉量（口）（估）
    "futures_basis": "+3",         # 正價差/逆價差
    "call_oi":       "32,100",
    "put_oi":        "41,500",
    "pcr":           "1.29",       # Put/Call 比率（>1 偏空）
    "pcr_signal":    "偏空",
...
CONSENSUS = {
    ...
    "upside":       "+19.6%",   # 從 NT$1,915 到 NT$2,290
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,382",   "-0.6%",  False),
    ("美元指數 DXY",    "103.5",   "-0.3%",  False),
    ("VIX 恐慌指數",    "21.2",    "+1.7",   False),  # 美伊風險升溫
    ("WTI 原油",        "$68.5",   "+1.8%",  False),  # 地緣政治推升油價
    ("10Y 美債殖利率",  "4.29%",   "+0.02pp",False),
    ("台幣/美元",       "31.68",   "-0.04",  False),
    ("NVIDIA NVDA",     "$872",    "-3.1%",  False),
    ("ASML",            "$681",    "-0.9%",  False),
]
...
NEWS = [
    {
        ...
        "title": "TSMC 2 月月營收年增 22.2%，前 2 月累計成長 29.9%",
        "body": "台積電 3/10 公告 2026 年 2 月合併月營收 NT$3,176.6 億，雖月減 20.8%（工作天數較少），但年增 22.2%。前兩月合計 NT$7,189.1 億（約 US$229 億），年增 29.9%，仍略低於市場預期 33% 增速，主因記憶體晶片需求偏弱。受惠 AI 晶片持續強勁，N3/N5 滿載支撐，台股 2330 今日上漲 1.86% 反映利多，NYSE 盤中受地緣政治風險影響另行回落。",
        "source": "Bloomberg / Yahoo Finance 2026-03-10", "impact": "正面",
    },
    {
        "tag": "擴產計畫", "tag_color": "#9C27B0",
        "title": "TSMC 台南 Mega Fab 取得環評許可，加速興建搶 AI 需求",
        "body": "台積電已完成台南科學園區新巨型廠區（15.46 公頃）環評程序，並進入正式建設階段，目標 2028 年完工。新廠主要承接 AI 硬體與高效能運算晶片訂單，同步加速 Arizona Phase 2（3nm）量產時程，由 2028 年提前至 2027 年。鑑於五大超大規模雲端業者 2026 年 AI 資本支出逾 $7,000 億美元，台積電先行擴產策略符合市場預期。",
        "source": "Simply Wall St / Digitimes 2026-03-11", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "AMD 獲 Meta $600 億 AI 晶片大單，TSMC 為製造端最大受益者",
        "body": "AMD 與 Meta 簽訂為期五年、規模約 $600 億美元的 AI 晶片協議，部署客製 Instinct MI450 GPU 與第六代 EPYC 'Venice' CPU（皆由 TSMC N3/N2 製造）。AMD AI 加速器市佔率預計從 2025 年約 9% 提升至 2026 年底逾 15%。此大單意味 TSMC N2 產能需求將進一步拉緊，N3/N5 滿載格局延續，預計客戶收入多元化有助降低單一客戶集中風險。",
        "source": "CNBC / Investing.com 2026-02-24", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "美國起草規則，全球 AI 晶片出口須獲美方逐案許可",
        "body": "美國商務部起草法規，擬對全球任何目的地的 AI 晶片出口實施美方審批控制，賦予華盛頓對各國 AI 資料中心建設的廣泛否決權。若規則正式生效，NVIDIA、AMD 等 TSMC 主要客戶的全球銷售計畫將受影響，可能延緩海外 AI 基礎設施投資，間接降低 TSMC 中長期訂單能見度。TSMC 目前約 15% 收入來自中國市場，為最高風險暴露區域。",
        "source": "Bloomberg 2026-03-05", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "NVIDIA 投資 40 億美元光子學公司，強化數據中心互連供應鏈",
        "body": "NVIDIA 宣布投資 Lumentum 與 Coherent 兩家光子學公司共計 $40 億美元，藉光子傳輸技術（以光代電）提升數據中心 AI 晶片間互連效能，並在美國建立 R&D 與製造設施。此舉確保 NVIDIA 新一代 Rubin 架構晶片（預計 2026 H2 量產，由 TSMC N2 代工）的配套供應鏈完整，有利 TSMC 高階封裝（CoWoS-S/L）需求持續成長。",
        "source": "Tom's Hardware 2026-03-11", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "美伊戰爭風險升溫，NYSE TSM 3/10（美東）收跌 2.22% 至 $349.49",
        "body": "美伊衝突風險令中東數據中心供應鏈前景蒙陰，加上市場擔憂全球 AI 晶片出口管制收緊，NYSE TSM 於美東 3/10 交易日下跌 $7.95（-2.22%）至 $349.49（台灣時間 3/11 凌晨 4 時收盤），較 2/25 高點 $390.21 回落逾 10%。相比之下，台灣本地 2330 於 3/10 受惠 2 月月營收年增 22.2% 利多，逆勢上漲 1.86%。川普總統表示伊戰「接近尾聲」，短期波動或趨緩，惟需持續追蹤中東局勢發展。",
        "source": "StockTwits / Yahoo Finance 2026-03-10（美東）", "impact": "負面",
    },
]

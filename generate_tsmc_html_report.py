...
STOCK = {
    # 台股 2330
    "tw_price":       "2,080",        # 2026-04-23 收盤（+30 vs 4/22 NT$2,050，+1.46%；跟進 4/22 NYSE ADR 大漲）
    "tw_change":      "+30",          # 漲跌點數（vs 4/22 NT$2,050）
    "tw_change_pct":  "+1.46%",       # 漲跌幅
    "tw_volume":      "~22,500",      # 成交量（張，4/23 估算，ADR 大漲帶動量能回補）
    "tw_open":        "2,060",
    "tw_high":        "2,085",        # 4/23 日中高
    "tw_low":         "2,055",        # 4/23 日中低
    "market_cap":     "NT$53.9 兆",
    "pe_ratio":       "~30.7x",       # 台股 P/E（TTM，NT$2,080 基礎，EPS FY25 實績 NT$67.8）
    "pb_ratio":       "~10.9x",       # 台股 P/B（股淨比）
    "div_yield":      "~0.72%",       # 殖利率（已除息後調整，USD $2.65/ADR 年息）
    "52w_high_tw":    "2,110",        # 52 週高（2026-04-16 日中高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "387.53",       # 4/23 實際收盤（-0.05% vs 4/22 $387.72；大漲後小幅整理）
    "nyse_change":    "-0.19",        # 4/23 實際（vs 4/22 $387.72）
    "nyse_change_pct":"-0.05%",       # 4/23 小幅整理，技術性消化前日 +5.3% 大漲
    "52w_high_nyse":  "392.00",       # 52 週高（2026-04-16 after-hours）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~32.1x",       # NYSE USD 基礎 P/E（TTM，估算，$387.53 基礎）
    "pb_nyse":        "~11.3x",
    "beta":           "1.16",
    ...
    # 資料日期備註
    "data_note":      "⭐ 今日（2026-04-24 Fri）重點：台股 2330 4/23 收盤 NT$2,080（+30，+1.46%，vs 4/22 NT$2,050），跟進 4/22 NYSE 大漲但幅度收斂；NYSE TSM 4/23 收 $387.53（-0.05%，技術性消化前日 +5.3% 大漲）  |  ASML 高 NA EUV 遞延利空：TSMC 4/22 宣布暫不採用 ASML 最新高 NA EUV 設備（€350M/台過貴），Bloomberg 報導後 ASML 股價承壓；反之 TSMC A13/N2U 路線突顯「以既有 EUV 達到更小更快晶片」能力  |  Siemens ⟷ TSMC 合作深化 AI 晶片設計平台  |  Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2%（+740bps）雙創歷史新高  |  A14（2028 量產）/ A16（H2 2026 量產）技術論壇利多延續；2028 先進封裝可整合 10 顆大晶片 + 20 HBM 堆疊  |  NVIDIA 超越 Apple 成 TSMC #1 客戶（22%）確認；AI 將占 TSMC 1/3 營收",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "📊 台股 2330 4/23 收 NT$2,080（+1.46%）跟進 ADR 大漲；NYSE TSM 4/23 $387.53 小幅整理（-0.05%）；ASML 高 NA EUV 遞延、TSMC A13/N2U 路線突顯技術彈性",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 2330 4/23 收盤 NT$2,080（+30，+1.46% vs 4/22 NT$2,050），跟進 ADR 大漲但漲幅收斂；52 週高 NT$2,110 僅差 30 點",
        "💵 NYSE TSM 4/23 小幅整理收 $387.53（-0.05%），消化 4/22 爆量大漲 +5.3% 後技術性整理；盤中仍呈現強勢格局",
        "🛠 ASML 高 NA EUV 遞延利空：TSMC 4/22 宣布暫不採用新設備（€350M/台過貴），A13/N2U 路線改以設計協同突破、ASML 股價承壓",
        "🤝 Siemens ⟷ TSMC 擴大 AI 晶片設計合作；2028 先進封裝可整合 10 顆大晶片 + 20 HBM 堆疊，CoWoS 護城河加深",
        "⭐ 共識目標 NT$2,320（+11.5% 上行空間），Barclays $470 目標對應 NT$2,450；Q1 2026 淨利 +58% 創高、全年 >30% 成長確認",
    ],
    # 一句結論
    "bottom_line": "今日（2026-04-24 Fri）報告摘要：台股 2330 4/23 收盤 NT$2,080（+30 點，+1.46% vs 4/22 NT$2,050）順利跟進 ADR 前日漲勢，雖較 NYSE TSM 4/22 單日 +5.30% 漲幅收斂，但收盤創近週新高、距 52 週高 NT$2,110 僅 30 點空間。NYSE TSM 4/23 收 $387.53（-0.05%，小幅整理），屬前日 +5.3% 大漲後的技術性消化，並未改變多頭結構。本週關鍵變數：（1）ASML 高 NA EUV 遞延：TSMC 4/22 宣布暫不採用 ASML 最新高 NA EUV（單機 €350M 過貴），改以 A13/N2U 設計協同突破，Bloomberg/Yahoo Finance 報導後 ASML 股價承壓——對 TSMC 是「資本支出效率」正面故事，對設備股（ASML/AMAT）是情緒利空；（2）Siemens 宣布與 TSMC 擴大 AI 晶片設計平台合作，EDA + 晶圓代工綁定更緊密；（3）TSMC 預告 2028 先進封裝可整合 10 顆大晶片 + 20 HBM 堆疊（目前 Vera Rubin 僅 2+8），CoWoS 護城河延伸至下下世代；（4）NVIDIA #1 客戶（22%）確認，黃仁勳稱 AI 晶片製造瓶頸是「2-3 年問題」，暗示 TSMC 長期供需缺口。基本面：Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2%（+740bps）歷史新高，全年 >30% 成長、AI 將占營收 1/3。技術面：RSI 65、MACD +3.5、KDJ 向上發散、均線多頭排列，短線支撐 NT$2,050-2,060、壓力 NT$2,100-2,110（突破後中線看 NT$2,320-2,450）。31 位分析師全數看多，共識目標 NT$2,320（上行 +11.5%）、Barclays $470 對應 NT$2,450。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 短線漲幅已消化 Barclays 升評利多，ADR 4/23 開始技術整理，台股上攻 NT$2,110 壓力需量能配合；台幣升值壓力（每升值 1% 毛利率 -40bps）未解；ASML 高 NA EUV 遞延雖對 TSMC 資本效率正面，但設備股（ASML/AMAT/LRCX）連動短期承壓；Arizona 先進封裝廠 2029 啟用時程較市場預期略晚；CoWoS ABF 基板供應瓶頸未解、亞利桑那廠爬坡成本拖累 Q2-Q3 毛利率；美中 AI 晶片出口管制（BIS 逐案審查）仍為結構性雜音",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-23",
    "foreign":  +4200,    # 外資（正=買超）— 4/23 估算，跟進 ADR 大漲持續買超
    "trust":    +620,     # 投信（4/23 估算，延續加碼）
    "dealer":   +80,      # 自營商（4/23 估算，小幅買超）
    # 近5日（由舊到新，最後一筆為 4/23）
    "foreign_5d": [-3200, +1200, +2210, +3500, +4200],  # 4/23 估算：外資連 4 日買超
    "trust_5d":   [+250,  +300,  +696,  +820,  +620],   # 4/23 估算：投信續買超
    "dealer_5d":  [-180,  +80,   -77,   +150,  +80],    # 4/23 估算：自營商小幅買超
    "foreign_ownership_pct": "75.1%",  # 外資持股比例（4/23 略增）
    "foreign_ownership_shares": "約 194.8 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       65.2,   "rsi_signal": "RSI 升至 65.2（自 62.8 升），多頭動能續強；距 70 超買區僅 5 點差",
    "macd":         +3.5,   "macd_signal": "MACD 正值再擴大（+3.1→+3.5），多頭動能強勁、黃金交叉維持；ADR 大漲結構延續",   "macd_color": "#4CAF50",
    "kdj_k":        72.0,   "kdj_d": 68.5, "kdj_j": 75.5, "kdj_signal": "KDJ 三線向上發散進入強勢區（K>D），J 值 75.5 逼近超買區需留意",
    "ma5":       "2,060",
    "ma20":      "2,015",
    "ma60":      "1,930",
    "ma120":     "1,830",
    "bb_upper":  "2,130",   # 布林上軌
    "bb_mid":    "2,015",   # 布林中軌（20MA）
    "bb_lower":  "1,900",   # 布林下軌
    "vol_ma5":  "21,200",   # 5日均量（張）
    "trend":    "台股 2330 4/23 收盤 NT$2,080（+30，+1.46% vs 4/22 NT$2,050），跟進 NYSE TSM 4/22 爆量大漲 +5.30%；NYSE TSM 4/23 $387.53（-0.05% 技術整理）。技術面全面強勢：RSI 65.2 多頭區、MACD +3.5 動能擴大、KDJ 三線向上發散。均線多頭排列（MA5>MA20>MA60>MA120），現價站穩所有均線。短線支撐 NT$2,050-2,060，壓力 NT$2,100-2,110（52 週高，突破後看 NT$2,200-2,330）。中線目標 NT$2,320-2,450（分析師共識 + Barclays 新高目標）。",
    "support":  "2,050–2,060",
    "resist":   "2,100–2,110",
...
DERIVATIVES = {
    "date":          "2026-04-23",
    "futures_price": "2,085",      # 台積電期貨近月（估，4/23 收盤，正價差）
    "futures_oi":    "93,800",     # 未平倉量（口）（估，跟進現貨留倉增加）
    "futures_basis": "+5",         # 正價差（多頭情緒延續）
    "call_oi":       "54,200",
    "put_oi":        "38,800",
    "pcr":           "0.72",       # Put/Call 比率續降至 0.72（多頭動能續強）
    "pcr_signal":    "偏多（Put/Call 0.72，較 4/22 的 0.76 再下降，現貨突破 NT$2,080 帶動 Call 需求擴大）",
    "max_pain":      "2,060",      # 最大痛苦價格（近期到期，估）
    "key_strikes": [
        ("2,000", "Call 28,500 / Put 7,800", "心理整數關口，強支撐區"),
        ("2,050", "Call 25,200 / Put 5,500", "4/22 收盤價支撐，5MA 附近"),
        ("2,080", "Call 23,500 / Put 3,500", "4/23 收盤價，短線多空分界"),
        ("2,110", "Call 18,200 / Put 2,100", "52 週高壓力（突破後看 NT$2,200-2,330）"),
        ("2,200", "Call 10,500 / Put 1,100", "中線樂觀目標"),
    ],
...
# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-04-23 估算；ASML 高 NA EUV 遞延利空施壓設備股）
    ("設備", "ASML",    "艾司摩爾",    "662.40", "-2.6%", "EUV 設備龍頭（4/23 下跌；TSMC 宣布暫不採用高 NA EUV，€350M/台過貴利空）"),
    ("設備", "AMAT",    "應用材料",    "172.10", "-2.1%", "CVD/PVD 設備（4/23 連帶承壓；設備投資減速隱憂）"),
    ("設備", "LRCX",    "科林研發",    "786.30", "-1.5%", "蝕刻設備（4/23 整理；但 2nm 訂單基本面仍正面）"),
    ("設備", "KLAC",    "科磊",        "582.90", "-0.9%", "量測設備（4/23 小跌；2nm/A16 驗證需求仍強）"),
    ("記憶", "000660.KS","SK 海力士",  "300,500 KRW", "+0.8%", "HBM4 供應（NVIDIA GB300 主力，Samsung HBM4E 競爭中）"),
    ("記憶", "MU",      "美光",        "99.20",  "+0.7%", "HBM / DDR5（4/23 小漲；DRAM 漲價 50% YTD）"),
    ("封裝", "3711.TW", "日月光",      "230",    "+1.3%", "CoWoS 封裝（4/23 跟進；Arizona 2029 封裝廠利多）"),
    ("客戶", "NVDA",    "輝達",        "870.20", "+0.2%", "#1 客戶 22%（4/23 盤整；黃仁勳稱 2-3 年瓶頸問題）"),
    ("客戶", "AAPL",    "蘋果",        "211.50", "+0.6%", "#2 客戶 17%（A20 採 2nm，iPhone 18，鎖定 A14）"),
    ("客戶", "AVGO",    "博通",        "199.80", "+0.6%", "#3 客戶 13%（Custom XPU，AI ASIC 強勢）"),
    ("客戶", "AMD",     "超微",        "171.60", "+1.7%", "#4 客戶 8%（MI450 新大單、法國政府 AI 戰略合作）"),
    ("客戶", "2454.TW", "聯發科",      "2,180",  "+0.7%", "手機 SoC 客戶 9%，2nm 首批量產，連 2 日創高"),
    ("競爭", "INTC",    "英特爾",      "21.35",  "+0.7%", "Intel Foundry 18A 爬坡，仍落後 TSMC 2nm"),
    ("競爭", "005930.KS","三星電子",   "53,800 KRW", "+1.1%", "Samsung Foundry 良率改善，HBM4E 送樣 NVIDIA 提速"),
]
...
ANALYST_RATINGS = [
    ...
    ("MarketScreener", "共識買入", "NT$2,320", "2026-04-24", "31 位分析師、0 位賣出（Barclays 升評後上修）"),
]
...
CONSENSUS = {
    ...
    "upside":       "+11.5%",   # 從 NT$2,080（4/23 收盤）到 NT$2,320
}
...
IR_CALENDAR = [
    ...
    ("2026-04-24", "⭐ 今日：多頭延續觀察壓力 NT$2,110","台股 2330 距 52 週高 NT$2,110 僅 30 點，RSI 65.2/MACD +3.5/KDJ 向上發散、均線多頭排列；短期關注 ASML 高 NA EUV 遞延對設備股情緒面影響、台幣匯率走勢（~31.35）、Q2 2026 財報 7 月展望；分析師共識目標上修至 NT$2,320（+11.5% 上行），Barclays $470 對應 NT$2,450","#BBDEFB"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "小幅整理","-0.85%", False),  # 4/23 SOX 整理（設備股承壓，科技大廠盤整）
    ("美元指數 DXY",    "98.1",    "-0.2%",  False),  # 美元續弱，利好 TSMC ADR 估值
    ("VIX 恐慌指數",    "19.8",    "-0.7",   False),  # 恐慌指數再降，風險偏好延續
    ("WTI 原油",        "$62.8",   "+0.5%",  False),  # 油價小升
    ("10Y 美債殖利率",  "3.95%",   "-0.03pp",False),  # 殖利率回落，風險資產受惠
    ("台幣/美元",       "31.35",   "持平",   False),  # 台幣持平（每升值 1% 毛利率 -40bps，需追蹤）
    ("NVIDIA NVDA",     "$870.20", "+0.2%",  False),  # #1 TSMC 客戶（22%），黃仁勳稱瓶頸「2-3 年問題」
    ("ASML",            "$662.40", "-2.6%",  False),  # 設備股下跌（TSMC 暫不採用高 NA EUV 遞延利空）
]
...
RELATIVE = {
    "date":           "2026-04-23",
    "tsmc_tw_1d":     +1.46,   # 2330 4/23 收盤 NT$2,080（+30 vs 4/22 NT$2,050）
    "taiex_1d":       +0.45,   # 加權指數 4/23 續漲（估）
    "sox_1d":         -0.85,   # SOX 4/23 小幅整理（設備股承壓）
    "tsmc_nyse_1d":   -0.05,   # TSM ADR 4/23 $387.53（-0.05% vs 4/22 $387.72）
    "tsmc_tw_5d":     +2.46,   # 本週至今（4/23 NT$2,080，vs 4/17 收 NT$2,030）
    "taiex_5d":       +1.50,
    "tsmc_tw_1m":     +7.2,    # 月線（NT$2,080 vs ~NT$1,940 一個月前）
    "taiex_1m":       +4.1,
    "tsmc_tw_ytd":    -0.7,    # Q1+4/1–4/23 累計（NT$2,080 vs 年初 ~NT$2,095）
    "taiex_ytd":      -2.8,
    "sox_ytd":        -17.5,
    "beta_60d":        1.16,   # 60 日滾動 Beta vs TAIEX（估算）
    # ADR 折溢價（以 4/23 實際 NYSE 收盤計算）
    "adr_price":     387.53,   # TSM NYSE 4/23 實際收盤
    "adr_parity":    331.74,   # 理論值 = 5 × NT$2,080 ÷ 31.35
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.35,   # 同 MACRO 台幣匯率（估算）
    "adr_premium_pct": "+16.8",# (387.53 - 331.74) / 331.74 × 100（估算，溢價收斂）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。ADR 溢價由 4/22 +18% 收斂至 +16.8%，反映台股跟進補漲、美股小幅整理",
}
...
FX_SENSITIVITY = {
    "spot":              31.35,  # 今日匯率（同 MACRO，4/23 估算）
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 2026 財報確認：淨利 NT$572.5B（$18.2B，+58.3% YoY）、毛利率 66.2%（+740bps YoY）雙創歷史新高，連續第 10 季超越市場預期（平均超預期幅度 +6.2%）。Q2 指引收入 $39.0-40.2B（遠超共識 $37.5-39.0B），全年 2026 收入 >30% 確認，CapEx 向 $52-56B 高端靠齊。近期價格：台股 2330 4/23 收盤 NT$2,080（+30，+1.46% vs 4/22 NT$2,050），成功跟進 ADR 大漲；NYSE TSM 4/23 收 $387.53（-0.05%，技術性整理消化 4/22 +5.30% 爆漲）。今日（4/24）多頭動能持續：Barclays 升評 $470、Arizona CoWoS 2029 啟用、A14/A16 技術路線圖明確。新增事件：TSMC 4/22 宣布暫不採用 ASML 最新高 NA EUV 設備（單機 €350M 過貴），改以 A13/N2U 設計協同突破——對 TSMC 是資本效率正面故事，但 ASML 股價 4/23 受壓 -2.6%；Siemens 宣布擴大 TSMC AI 晶片設計合作；2028 先進封裝將整合 10 大晶片 +20 HBM 堆疊（目前 Vera Rubin 2+8）。NVIDIA #1 客戶 22% 確認，黃仁勳稱 AI 晶片製造瓶頸為「2-3 年問題」暗示長期供需缺口。31 位分析師全數看多，共識目標上修至 NT$2,320（自 NT$2,080 有 +11.5% 上行空間），Barclays 目標 $470 對應台股約 NT$2,450。",
...
NEWS = [
    {
        ...
        "title": "🛠 TSMC 暫不採用 ASML 高 NA EUV 設備（€350M/台過貴）；A13/N2U 以設計協同突破",
        "body": "TSMC 副營運長 Kevin Zhang 4/22 於北美技術論壇明確表示：公司目前無計劃採用 ASML 最新高 NA EUV 微影機，描述該設備為「非常、非常昂貴」（每台價格達 €350M／約 $410M USD）。TSMC 將持續使用現有 EUV 技術，並透過 A13（2029 量產，AI 晶片專用）與 N2U（較實惠版本，手機/筆電/AI 兼用）等設計協同製程路線取得「更小、更快」晶片。N2U 較 N2P 提供 3–4% 高速、8–10% 低功耗、2–3% 密度改善。Bloomberg 報導後 ASML 股價 4/23 下跌 -2.6% 至 $662.40，設備股（AMAT/LRCX/KLAC）同步承壓。對 TSMC 是資本效率正面故事、對設備股情緒短期利空。",
        "source": "Manila Times / Bloomberg / Yahoo Finance 2026-04-22/23", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📦 TSMC 技術藍圖延伸至 2028：先進封裝將整合 10 大晶片 +20 HBM 堆疊",
        "body": "4/22 北美技術論壇揭示 TSMC 先進封裝能力將在 2028 年達到新里程碑：可將 10 顆大型運算晶片與 20 座高頻寬記憶體（HBM）堆疊縫合整合——對比目前 NVIDIA Vera Rubin（2026 量產，由 TSMC 製造）為 2 顆運算晶片 + 8 座 HBM 堆疊，規模擴大 5–2.5 倍。搭配 A13（2029 量產）、A12、N2U 三條並行路線，AI/HPC/行動 全方位覆蓋。TSMC 對 High-NA EUV 的遞延決策反映設計協同優化（DTCO）已足以取代昂貴曝光設備，突顯其製程工程能力領先 Samsung / Intel Foundry。此為 AI 超級晶片長期供需缺口提供產能擴張路徑。",
        "source": "SemiWiki / Manila Times / TSPA Semiconductor 2026-04-23", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🤝 Siemens 宣布與 TSMC 擴大合作：EDA + AI 晶片設計平台更深度綁定",
        "body": "Siemens Digital Industries Software 4/22 宣布與 TSMC 在 AI 半導體設計合作上取得重大進展，雙方將共同推進適用於 TSMC N2、A16、A14 等先進製程的 AI 輔助晶片設計流程與 EDA 工具鏈整合。此舉強化 TSMC 在「製程 + 設計工具 + AI 賦能」完整生態系的領先地位，進一步提高客戶切換成本（switching cost）。Siemens 與 Synopsys、Cadence 並列 TSMC 三大 EDA 合作夥伴，此類深度合作長期鞏固 TSMC 護城河。對 Samsung Foundry / Intel Foundry 構成結構性壁壘。",
        "source": "SemiWiki / Siemens Press Release 2026-04-22", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏢 NVIDIA 確認成 TSMC #1 客戶 22%；黃仁勳：AI 晶片瓶頸是「2-3 年問題」",
        "body": "NVIDIA 已超越 Apple 成為 TSMC 最大客戶（2026 預估貢獻約 $330 億收入，佔 TSMC 總營收 22%），NVDA 4/23 持平於 $870.20（+0.2%）消化前日 +2.6% 漲勢。NVIDIA CEO 黃仁勳近日表示「AI 晶片製造瓶頸是 2-3 年問題」，意味供需缺口將長期持續——所有瓶頸本質上都指向 TSMC 先進製程與 CoWoS 封裝產能。Apple（#2，17%）鎖定 A14 用於 iPhone 18 A20 晶片（2028）；AMD（#4，8%）MI450 新大單並獲法國政府 AI 戰略合作（數十億美元規模）；Broadcom（#3，13%）Custom XPU 強勢；MediaTek（#5，9%）4/23 收 NT$2,180（+0.7%）連 2 日創高。",
        "source": "CNBC / 24/7 Wall St / Tweaktown 2026-04-21/23", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🌏 全球半導體 2026 目標 $975B–$1 兆；AI 資本支出 $660B 壓制供給",
        "body": "產業觀察：2026 年 AI 基礎設施資本支出預估達 $660B，全球半導體銷售額可望達 $975B–$1 兆新高（Deloitte）。TSMC 已預告 Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2% 雙創歷史新高，全年 >30% 成長確認，AI 將占營收 1/3（Next Platform 分析）。關鍵瓶頸已從「技術」轉為「資源約束」：電力、銅、關鍵氣體（helium 短缺後 Taiwan/Korea fabs 限量配給）、DRAM 漲價 50% YTD。NVIDIA 保留 TSMC 絕大多數 CoWoS 產能，ASE 預計 2026 先進封裝營收翻倍。Amazon Trainium、Meta / Anthropic 自研 AI 晶片、OpenAI-Cerebras $200 億協議——全部依賴 TSMC 先進製程。",
        "source": "Manufacturing Dive / Deloitte / Next Platform 2026-04-20/23", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 短線 KDJ 逼近超買（J=75.5），台幣升值 + ABF 基板瓶頸未解",
        "body": "技術面提醒：台股 2330 4/23 收 NT$2,080 創近期新高，RSI 65.2、KDJ J 值 75.5 逐步逼近超買區；距 52 週高 NT$2,110 僅 30 點，量能需配合方能突破。ADR 4/23 小幅整理（-0.05%）顯示短期消化 4/22 +5.3% 大漲。結構性壓力：台幣走強（~31.35）持續施壓毛利率（每升值 1% -40bps）；ASML 高 NA EUV 遞延雖對 TSMC 資本效率正面，但設備股（ASML/AMAT/LRCX）連動短期承壓；Arizona CoWoS 封裝廠 2029 啟用時程較市場期待的 2028 略晚；ABF 基板供應瓶頸未解，2026 CoWoS 目標 ~35,000 wspm（+59% YoY）實現仍受限；亞利桑那廠爬坡成本 + 台灣電費調漲拖累 Q2-Q3 毛利率；美中 AI 晶片出口管制（BIS 2026-01-15 逐案審查）仍為結構性雜音。",

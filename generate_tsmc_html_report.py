...
STOCK = {
    # 台股 2330
    "tw_price":       "2,455",        # 2026-06-30 Tue 官方收盤（+85、+3.59% vs 6/29 NT$2,370；跳空補漲跟進 NYSE 6/29 +5.26%；7/1 Wed 台股盤中待 TWSE）
    "tw_change":      "+85",          # 6/30 官方收盤漲跌點數（+3.59%）
    "tw_change_pct":  "+3.59%",       # 6/30 官方收盤漲跌幅（自 6/29 NT$2,370 跳空大漲）
    "tw_volume":      "15,514",       # 6/30 官方全日成交量（張，成交金額 NT$380.9 億、振幅 1.69%）
    "tw_open":        "2,440",        # 6/30 開盤（跳空高開）
    "tw_high":        "2,475",        # 6/30 盤中高
    "tw_low":         "2,435",        # 6/30 盤中低
    "market_cap":     "NT$63.7 兆",   # 6/30 收盤市值（NT$2,455 基礎）
    "pe_ratio":       "~36.2x",       # 台股 P/E（TTM，NT$2,455 基礎）
    "pb_ratio":       "~12.7x",       # 台股 P/B（股淨比，NT$2,455 基礎）
    "div_yield":      "~0.61%",       # 殖利率（NT$2,455 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "468.80",       # 6/30 Tue 收盤（+$13.70，+3.02%；Morgan Stanley + UBS 上調目標、MS 估 2026 營收 +40% YoY 帶動續攻、創波段新高、市值約 $2.15 兆、P/E ~33.5；前收 $455.10）
    "nyse_change":    "+13.70",       # 6/30 漲跌（6/30 收盤確認、4:00 PM EDT）
    "nyse_change_pct":"+3.02%",       # 6/30 NYSE 續漲（Morgan Stanley + UBS 上調目標、漲價定價力 + Amkor 美國封裝合作 + AI 資本支出循環）
    "52w_high_nyse":  "476.79",       # 52 週/歷史高（區間）
    "52w_low_nyse":   "221.18",       # 52 週低（區間）
    "pe_nyse":        "~33.5x",       # NYSE USD 基礎 P/E（TTM，$468.80 基礎）
    "pb_nyse":        "~13.6x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-01 Wed（台股 2330 6/30 Tue 官方收盤 NT$2,455（+85、+3.59%）跳空補漲確認、7/1 Wed 台股盤中請以 TWSE 為準；NYSE TSM 6/30 Tue 收盤 $468.80（+3.02%）為最新確認收盤、7/1 美股盤待台灣時間今晚開盤）。⭐ 今日頭條——台美雙市連袂走高、法人上調目標接力：（1）🚀 NYSE TSM 6/30 續漲 +3.02%（+$13.70）收 $468.80、逼近 52 週高 $476.79、市值約 $2.15 兆（P/E ~33.5、前收 $455.10），催化——⭐ Morgan Stanley 上調目標並估 2026 營收 +40% YoY、⭐ UBS 上調目標並上修長期銷售成長預期、⭐ 漲價定價力（全先進製程調漲 5-10%、佔晶圓營收 ~74%）、⭐ TSMC-Amkor 美國封裝長約降低地緣風險；（2）🚀 台股 2330 6/30 官方收盤 NT$2,455（+85、+3.59%、開 2,440 / 高 2,475 / 低 2,435、量 15,514 張、成交額 NT$380.9 億、振幅 1.69%），跳空跟進 NYSE 前波大漲、突破 5MA/20MA；（3）📊 分析師共識目標上修——彙整平均 $476.24、最高 $625.00、最低 $351.00（自 6/30 收 $468.80 仍具上行空間）；（4）📊 台美 ADR 溢價（以 6/30 ADR $468.80 + 台股 6/30 NT$2,455、匯率 31.05 計）：理論值 5×2,455÷31.05 = $395.33、溢價 (468.80-395.33)/395.33 = +18.59%（NYSE 6/30 續漲、溢價走闊、台股 7/1 具補漲空間）。⚠️ 漲價雙面刃——利多毛利率（估全年 +2pp 以上），惟客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、恐促部分客戶調整下單量或轉向替代；Benzinga 指 NVIDIA 因 AI 需求剛性與轉嫁能力強、長期反為利多。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。⚠️ ITC 專利調查初判預計近期（6 月底/7 月初）出爐、為短線關鍵雜訊。⚠️ 半導體監理雜訊——超微電腦（Super Micro）遭台灣當局搜索（涉 NVIDIA AI 晶片走私中國調查）股價重挫 -8%，惟晶片設備股 AMAT/LRCX 因 AI 記憶體需求勁揚、SOXX 單日反彈 +4.1%。⚠️ 南韓公布逾 $1 兆美元 AI/晶片投資計畫（三星+SK 海力士 800 兆韓元），中長期競爭加劇。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、Winbond 特殊 DRAM 自主供應、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、近期預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：ITC 初判近期（6 月底/7 月初）；6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +26% YoY 達 ~$975B、Gartner 估全年營收超 $1.3 兆；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆（佔 AI 伺服器機櫃價值 95%）  |  ⚠️ 本日（7/1）關鍵觀察：（1）台股 2330 能否跟進 NYSE 6/30 續漲、站穩 NT$2,455 並挑戰 NT$2,490 / NT$2,535 史高；（2）ADR 溢價走闊至 +18.59%、台股具補漲空間；（3）下方支撐 NT$2,400 / NT$2,370 / NT$2,340；上方壓力 NT$2,490 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~33.5x、台股 P/E ~36.2x，留意 ITC 初判、6 月營收 miss 風險與漲價客戶反彈",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "🚀 台美雙市連袂走高、法人上調目標接力：NYSE TSM 6/30 續漲 +3.02% 至 $468.80（Morgan Stanley/UBS 上調、MS 估 2026 營收 +40% YoY）；台股 2330 6/30 收 NT$2,455（+3.59%）跳空跟進，漲價定價力（全先進製程 +5-10%、佔晶圓營收 ~74%）續發酵",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🚀 NYSE TSM 6/30 Tue 續漲 +3.02%（+$13.70）收 $468.80、逼近 52 週高 $476.79、市值約 $2.15 兆——Morgan Stanley + UBS 上調目標、MS 估 2026 營收 +40% YoY",
        "🚀 台股 2330 6/30 官方收盤 NT$2,455（+85、+3.59%、開 2,440/高 2,475/低 2,435、量 15,514 張、成交額 NT$380.9 億）跳空跟進 NYSE 前波大漲、突破 5MA/20MA",
        "📈 分析師共識目標上修：彙整平均 $476.24、最高 $625.00、最低 $351.00（自 6/30 收 $468.80 仍具上行空間）；漲價印證 AI 需求下定價力",
        "⭐ 漲價題材續發酵：擴及全先進製程（含 N7/N5/N3、佔晶圓營收 ~74%）、幅度 5-10%、估貢獻全年毛利率 +2pp 以上；TSMC-Amkor 美國封裝長約降地緣風險",
        "📊 ADR 溢價走闊至 +18.59%（以 6/30 ADR $468.80 + 台股 6/30 NT$2,455、匯率 31.05 計）——NYSE 6/30 續漲、台股 7/1 具補漲空間",
        "🏆 中長線基底未變：NVIDIA #1 客戶/A16 首發 2027 + NVIDIA-TSMC AI 製程合作 + TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + Winbond 自主 DRAM 供應",
        "⚠️ 風險續追蹤：ITC 專利調查近期待初判；4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）關鍵；Super Micro 遭台灣搜索 -8%、南韓 $1 兆晶片投資；估值偏高（TSM P/E ~33.5x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-01 Wed（台股 2330 6/30 Tue 官方收盤 NT$2,455（+85、+3.59%）跳空補漲確認、7/1 Wed 台股盤中請以 TWSE 為準；NYSE TSM 6/30 Tue 收盤 $468.80（+3.02%）為最新確認收盤、7/1 美股盤待今晚開盤）。⭐ 今日頭條——台美雙市連袂走高、法人上調目標接力：（1）🚀 NYSE TSM 6/30 續漲 +3.02%（+$13.70）收 $468.80、逼近 52 週高 $476.79、市值約 $2.15 兆（P/E ~33.5、前收 $455.10），催化——⭐ Morgan Stanley 上調目標並估 2026 營收 +40% YoY、⭐ UBS 上調目標並上修長期銷售成長預期、⭐ 漲價定價力（全先進製程調漲 5-10%、佔晶圓營收 ~74%）、⭐ TSMC-Amkor 美國封裝長約降低地緣風險；（2）🚀 台股 2330 6/30 官方收盤 NT$2,455（+85、+3.59%、開 2,440/高 2,475/低 2,435、量 15,514 張、成交額 NT$380.9 億、振幅 1.69%）跳空跟進 NYSE 前波大漲、突破 5MA/20MA；（3）📊 分析師共識目標上修——彙整平均 $476.24、最高 $625.00、最低 $351.00（自 6/30 收 $468.80 仍具上行空間）。📊 台美 ADR 溢價（以 6/30 ADR $468.80 + 台股 6/30 NT$2,455、匯率 31.05 計）：理論值 5×2,455÷31.05 = $395.33、溢價 +18.59%（NYSE 6/30 續漲、溢價走闊、台股 7/1 具補漲空間）。⚠️ 漲價雙面刃：利多毛利率（估全年 +2pp 以上），惟客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、恐促部分調整下單或轉向替代；Benzinga 指 NVIDIA 因 AI 需求剛性與轉嫁能力強、長期反為利多。6 月營收（7 月上旬公布）為關鍵 catalyst——4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、惟即使小幅月增亦可能使 Q2 營收逼近指引（$39.0-40.2B）中值。⚠️ ITC 專利調查初判預計近期（6 月底/7 月初）出爐、為短線關鍵雜訊。⚠️ 半導體監理雜訊——Super Micro 遭台灣當局搜索（涉 NVIDIA AI 晶片走私中國調查）股價 -8%，惟晶片設備股（AMAT/LRCX）因 AI 記憶體需求勁揚、SOXX 單日反彈 +4.1%；南韓公布逾 $1 兆美元 AI/晶片投資計畫（三星+SK 海力士 800 兆韓元）、中長期競爭加劇。⭐ 中長線結構性利多基底未變：（1）🤝 TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）；（2）⭐ TSMC-Winbond 特殊 DRAM 自主供應、降低 HBM 對韓系依賴；（3）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產，Apple 跳過 A16 直上 A14；（4）⭐ NVIDIA-TSMC 深化 AI 導入晶圓廠（cuLitho/缺陷檢測/工廠排程）；（5）⭐ 2nm 已量產、初期良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（6）4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（7）⭐ 5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆；（8）2026 全球半導體市場估 +26% YoY 達 ~$975B、Gartner 估全年營收超 $1.3 兆、SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆。⚠️ 下行風險：（1）漲價恐引發客戶反彈與訂單調整；（2）4-5 月營收 +24% YoY 落後 Q2 ~35% 預期、6 月營收為關鍵驗證；（3）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、近期初判恐對含 AI 加速器晶片發布美國進口禁令；（4）Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung；（5）估值警示——TSM P/E ~33.5x、台股 P/E ~36.2x；（6）台幣升值（每升 1% 毛利率 -40bps）。基本面未變：Q1 2026 淨利 NT$572.5B（$18.2B、+58.3% YoY）、毛利率 66.2% 雙創史高。技術面（基於 6/30 收 NT$2,455）：5MA NT$2,417 / 20MA NT$2,405 / 60MA NT$2,375 / 120MA NT$2,275；RSI 升至 63 進偏多區、KDJ 黃金交叉續強、MACD 紅柱放大；下方支撐 NT$2,400 / NT$2,370 / NT$2,340；上方壓力 NT$2,490 / NT$2,535（史高）。分析師全面看多、Morgan Stanley/UBS 等上調目標（彙整平均 $476.24、最高 $625）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/1 Wed）關鍵觀察：（1）🚀 NYSE TSM 6/30 續漲 +3.02% 至 $468.80、台股 2330 6/30 收 NT$2,455（+3.59%）跳空跟進、台美雙市連袂走高；留意台股 7/1 能否跟進 NYSE 續漲並站穩 NT$2,455、挑戰 NT$2,490；（2）📈 Morgan Stanley + UBS 上調目標、MS 估 2026 營收 +40% YoY——本波續攻核心動能；（3）📊 共識目標上修（彙整平均 $476.24、最高 $625、最低 $351）；（4）⭐ 漲價升級為定價力、擴及全先進製程（佔晶圓營收 ~74%）、估全年毛利率 +2pp 以上；（5）📊 ADR 溢價走闊至 +18.59%、台股具補漲空間；（6）⚠️ 漲價雙面刃——客戶成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多；（7）⚠️ 4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst；（8）⚠️ ITC 專利調查初判預計近期（6 月底/7 月初）出爐——若不利恐對含 AI 加速器晶片發布美國進口禁令；（9）⚠️ 半導體監理雜訊——Super Micro 遭台灣搜索 -8%、南韓 $1 兆晶片投資中長期競爭加劇；（10）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027；（11）⚠️ Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung；（12）⚠️ 估值警示：TSM P/E ~33.5x、台股 P/E ~36.2x；本日關鍵：台股能否跟進站穩 NT$2,455 並挑戰 NT$2,490 / NT$2,535 史高；下方支撐 NT$2,400 / NT$2,370 / NT$2,340",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-06-30",       # 最新確認資料日（6/30 Tue 官方收盤；三大法人淨額為估算，官方明細以 TWSE 為準；7/1 待公布）
    "foreign":  9000,              # 外資 6/30 估買超約 9,000 張（台股跳空大漲 +3.59%、外盤成交占 62.84%、外資回補）
    "trust":    1300,              # 投信 6/30 估買超約 1,300 張
    "dealer":   700,               # 自營商 6/30 估買超約 700 張
    # 近5日（由舊到新：6/24, 6/25, 6/26, 6/29, 6/30）；6/30 為官方收盤日、三大法人淨額為估算
    "foreign_5d": [-15000, 1500, -6104, 2500, 9000],     # 外資 6/30 估買超約 9,000 張
    "trust_5d":   [800, 600, 179, 400, 1300],            # 投信 6/30 估買超約 1,300 張
    "dealer_5d":  [-500, 300, -310, 150, 700],           # 自營 6/30 估買超約 700 張
    "foreign_ownership_pct": "74.9%",  # 外資持股比例（6/30 估升至 74.9%）
    "foreign_ownership_shares": "約 193.7 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       64.0,   "rsi_signal": "RSI 估升至 64（台股 6/30 跳空 +3.59% 收 NT$2,455），動能轉強、進入偏多區、尚未超買；留意 7/1 台股能否跟進 NYSE 6/30 續漲與量能確認",
    "macd":         0.45,  "macd_signal": "MACD 估升至 +0.45、紅柱放大、動能改善、死叉風險解除、跳空帶量轉強",   "macd_color": "#4CAF50",
    "kdj_k":        72.0,   "kdj_d": 60.0, "kdj_j": 90.0, "kdj_signal": "KDJ K(72)/D(60)/J(90) 黃金交叉續強、6/30 跳空帶動、短線轉強、J 進高檔留意過熱",
    "ma5":       "2,417",   # 5MA（6/24 2390, 6/25 2390, 6/26 2340, 6/29 2370, 6/30 2455）— 基於 6/30 官方收盤
    "ma20":      "2,405",   # 20MA
    "ma60":      "2,375",   # 60MA 緩升
    "ma120":     "2,275",
    "bb_upper":  "2,545",   # 布林上軌
    "bb_mid":    "2,405",   # 布林中軌（20MA）
    "bb_lower":  "2,265",   # 布林下軌
    "vol_ma5":  "38,000",   # 5日均量（張，估、基於 6/24-6/30 五日均）
    "trend":    "🚀 台美雙市連袂走高：NYSE TSM 6/30 續漲 +3.02% 至 $468.80（Morgan Stanley/UBS 上調目標）、台股 2330 6/30 官方收盤 NT$2,455（+3.59%、量 15,514 張）跳空跟進、突破 5MA/20MA；漲價定價力（全先進製程 +5-10%、佔晶圓營收 ~74%）續發酵。📊 技術面（基於 6/30 官方收盤 NT$2,455）：跳空一舉突破 5MA NT$2,417 / 20MA NT$2,405、RSI 升至 64 進偏多區、MACD +0.45 紅柱放大、KDJ 黃金交叉續強，中期 60MA/120MA 趨勢偏多。下一觀察：（1）台股 7/1 能否跟進 NYSE 6/30 續漲、站穩 NT$2,455 並挑戰 NT$2,490 / NT$2,535 史高；（2）下方支撐 NT$2,400 / NT$2,370 / NT$2,340；（3）上方壓力 NT$2,490 / NT$2,535 史高；（4）⭐ 漲價定價力+法人上調利多、惟須留意 6 月營收（7 月上旬）與 ITC 初判（近期）；（5）⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發 + 2nm 70-80% 良率 + MS/UBS 上調目標。",
    "support":  "2,400 / 2,370 / 2,340 / 2,310 (整數、6/29 收、6/26 收、整數)",
    "resist":   "2,475 / 2,490 / 2,535 (6/30 高、6/23 收、史高)",
    "note": "技術指標數值為估算（基於 6/30 官方收盤 NT$2,455），請以看盤軟體即時數據為準；台股 6/30 跳空 +3.59% 跟進 NYSE、6/30 NYSE 續漲 +3.02%、突破 5MA/20MA、RSI 升至 64、KDJ 黃金交叉續強、MACD 紅柱放大；上方壓力 NT$2,490 / NT$2,535 史高、下方支撐 NT$2,400；⭐ 漲價定價力+法人上調利多 vs ⚠️ 6 月營收與 ITC 初判雜訊拉鋸；7/1 台股盤中待 TWSE",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-06-30",       # 最新資料日（6/30 Tue 盤後估、官方待 TAIFEX 公布；7/1 待更新）
    "futures_price": "2,460",            # 台積電期貨近月（6/30 收盤附近、小幅正價差）
    "futures_oi":    "173,000",          # 未平倉量（口）（6/30 估）
    "futures_basis": "+5",               # 6/30 估小幅正價差（雙市連袂走高、情緒偏樂觀）
    "call_oi":       "114,000",
    "put_oi":        "75,000",
    "pcr":           "0.66",             # Put/Call 比率（6/30 估 0.66、台股跳空大漲、避險買盤降、情緒偏樂觀）
    "pcr_signal":    "中性偏多（Put/Call 6/30 估 0.66；台股 6/30 跳空 +3.59%、NYSE 6/30 續漲 +3.02%、雙市連袂走高、情緒偏樂觀、期貨小幅正價差；⭐ 漲價定價力+法人上調為中長線支撐）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、6/30 估）
    "key_strikes": [
        ("2,250", "Call 56,000 / Put 18,000", "整數支撐"),
        ("2,310", "Call 58,000 / Put 14,000", "整數支撐"),
        ("2,370", "Call 56,000 / Put 10,000", "6/29 收"),
        ("2,400", "Call 50,000 / Put 8,000",  "整數、最大痛苦"),
        ("2,450", "Call 44,000 / Put 6,000",  "整數壓力"),
        ("2,490", "Call 36,000 / Put 4,500",  "6/23 收"),
        ("2,535", "Call 28,000 / Put 3,000",  "盤中史高"),
    ],
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；6/30 PCR 估自 0.68 降至 0.66 反映台股跳空 +3.59% 及 NYSE 6/30 續漲 +3.02% 帶動情緒轉樂觀、期貨轉小幅正價差；NT$2,400 為最大痛苦、下方支撐 NT$2,370 / NT$2,340、上方壓力 NT$2,490；⭐ 漲價定價力+法人上調結構性利多、⚠️ 留意 ITC 初判、6 月營收 miss 風險與客戶反彈；中長線基底未變（NVIDIA #1 客戶/A16 首發、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約、MS/UBS 等上調目標）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-06-30 Tue 收盤估、台股為 6/30 Tue 收盤；價格為估算，半導體隨 TSM/設備股反彈）
    ("設備", "ASML",    "艾司摩爾",    "885.00", "+1.10%", "EUV 設備龍頭（6/30 隨半導體走高；高 NA EUV 需求結構性多頭基底未變）"),
    ("設備", "AMAT",    "應用材料",    "560.00", "+2.00%", "🛠️ 6/29 單日飆 +10.8% 領軍設備股反彈（AI 記憶體需求）；新一代設備加速 DRAM 與 AI 先進封裝；6/30 續強"),
    ("設備", "LRCX",    "科林研發",    "1,090.00", "+1.80%", "蝕刻設備（6/29 大漲 +8.4%、目標價上調至 $500；6/30 續強；WFE 全年展望上修）"),
    ("設備", "KLAC",    "科磊",        "808.00", "+1.60%", "量測設備（6/30 續強；2nm 量產驅動）"),
    ("記憶", "2344.TW", "華邦電 Winbond","33.00","+1.50%", "⭐ 與 TSMC 合作將 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC、降低台灣對 Samsung/SK HBM 依賴（價格為估算）"),
    ("記憶", "000660.KS","SK 海力士",  "390,000 KRW", "+0.50%", "南韓公布逾 $1 兆美元 AI/晶片投資（三星+SK 800 兆韓元）；HBM4 需求仍強、HBM4E 送樣"),
    ("記憶", "MU",      "美光",        "154.00", "+2.60%", "HBM / DDR5（6/30 反彈；AI 記憶體需求勁揚、HBM4 中長線強勁）"),
    ("封裝", "3711.TW", "日月光",      "328",    "+1.24%", "CoWoS / 先進封裝（6/30 隨大盤走高；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "53.00",  "+2.90%", "⭐ TSMC-Amkor 簽 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（降地緣風險、本波利多核心）；6/30 續強"),
    ("客戶", "NVDA",    "輝達",        "208.00", "+1.50%", "🏆 確認 TSMC #1 客戶 22%、A16 首發客戶 2027 量產；市值逾 $5 兆；Benzinga 指漲價長期反利多"),
    ("客戶", "AAPL",    "蘋果",        "302.00", "+0.70%", "⚠️ #2 客戶 18%；面臨 TSMC 漲價成本；Apple 同意與 Intel 合作在美生產晶片；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "476.00", "+1.30%", "⚠️ #3 客戶（6/30 走高；面臨漲價成本；custom XPU 中長線強勁）"),
    ("客戶", "AMD",     "超微",        "525.00", "+1.40%", "#4 客戶（6/30 走高；面臨漲價成本；瞄準 $1,200 億資料中心市場、MI350/MI450；傳接觸 Samsung）"),
    ("客戶", "2454.TW", "聯發科",      "4,760",  "+0.85%", "手機 SoC 客戶 9%；6/30 走高；面臨漲價成本；RTX Spark 由聯發科協同 NVIDIA 設計"),
    ("競爭", "INTC",    "英特爾",      "51.30",  "+1.00%", "Apple 將與 Intel 合作在美設計製造晶片；18A-P 進入風險量產（效能 +9%）；與 UMC 結盟挑戰 TSMC"),
    ("競爭", "005930.KS","三星電子",   "67,100 KRW", "+0.45%", "⚠️ 南韓 $1 兆美元晶片投資領頭者；傳 Google/AMD/Tesla 接觸先進製程；HBM4E 送樣"),
    ("警示", "SMCI",    "美超微 Super Micro","38.50","-8.10%", "⚠️ 遭台灣當局搜索（涉 NVIDIA AI 晶片走私中國調查）、6/29 重挫 -8%；供應鏈監理雜訊"),
    ("處分後", "ARM",   "Arm Holdings","260.00", "+0.80%", "TSMC 4/29 全數出脫後（6/30 走高；RTX Spark ARM 架構中長線受惠）"),
]
...
NEWS = [
    {
        ...
        "title": "🚀 台美雙市連袂走高：NYSE TSM 6/30 續漲 +3.02% 收 $468.80、逼近 52 週高；台股 2330 6/30 官方收盤 NT$2,455（+3.59%、量 15,514 張）跳空跟進、突破 5MA/20MA",
        "body": "今日頭條——台美雙市連袂走高：（1）🚀 NYSE TSM 6/30 Tue 續漲 +3.02%（+$13.70）收 $468.80、逼近 52 週高 $476.79、市值約 $2.15 兆（P/E ~33.5、前收 $455.10），表現優於同業（半導體類股 +2.77%）；（2）🚀 台股 2330 6/30 官方收盤 NT$2,455（+85、+3.59% vs 6/29 NT$2,370、開 2,440/高 2,475/低 2,435、量 15,514 張、成交額 NT$380.9 億、振幅 1.69%），跳空跟進 NYSE 前波大漲、突破 5MA/20MA；（3）📊 台美 ADR 溢價走闊至 +18.59%（以 6/30 ADR $468.80 + 台股 6/30 NT$2,455、匯率 31.05 計、理論值 $395.33）——NYSE 6/30 續漲、台股 7/1 具補漲空間；（4）🎯 7/1 台股盤中請以 TWSE 為準。",
        "source": "TradingKey / Yahoo奇摩股市 / CNBC / Investing.com 2026-06-30", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📈 Morgan Stanley + UBS 上調 TSM 目標價：MS 估 2026 營收 +40% YoY、UBS 上修長期銷售成長；共識平均目標升至 $476.24、最高 $625",
        "body": "法人評級接力上調、為 6/30 ADR 續漲主要動能：（1）📈 Morgan Stanley 上調 TSMC 目標價，並提高營收與定價展望、預估公司 2026 營收年增約 40%；（2）📈 UBS 同步上調目標並上修長期銷售成長預期，看好持續性長線需求；（3）🎯 市場彙整分析師共識目標升至平均 $476.24、最高 $625.00、最低 $351.00，自 6/30 收 $468.80 仍具上行空間；（4）⭐ 支撐基本面：管理層指引「全年美元營收成長逾 30%」、AI 加速器需求強勁，加上 TSMC-Amkor 美國封裝新合作降低地緣風險；（5）🏆 NVIDIA 仍為 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產。",
        "source": "TradingKey / Morgan Stanley / UBS 2026-06-30", "impact": "正面",
    },
    {
        ...
        "source": "Tom's Hardware / Yahoo Finance / Benzinga 2026-06-29 - 06-30", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🛠️ 晶片設備股領軍反彈：AMAT +10.8%、LRCX +8.4%、SOXX +4.1%——AI 記憶體需求勁揚；Gartner 估 2026 半導體營收超 $1.3 兆",
        "body": "半導體設備與產業景氣同步轉強：（1）🛠️ Applied Materials（AMAT）單日飆漲 10.8%、Lam Research（LRCX）大漲 8.4%，帶動 VanEck 半導體 ETF（SOXX）+4.14%——反映 AI 記憶體（HBM/3D 堆疊）需求勁揚；（2）📈 Lam Research 目標價上調至 $500（自 $425），Bank of America 與 Wells Fargo 分別上調至 $480 / $450，指向創紀錄季度獲利與 2026 WFE 全年展望上修；（3）📊 Gartner 預估 2026 全年半導體營收超過 $1.3 兆；生成式 AI 晶片營收逼近 $5,000 億、約佔全球市場一半，惟高價元件單位量仍不到 2,000 萬顆（僅佔產業總量 0.2%）；（4）⭐ TSMC 為 AI 晶片與先進封裝核心供應者、直接受惠此結構性需求。",
        "source": "StartupHub.ai / Gartner / Bank of America 2026-06-29 - 06-30", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 近期營收疑慮續存 + ITC 專利調查近期待初判：4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst",
        "body": "短線兩大雜訊須留意：（1）⚠️ 營收動能——TSMC 4-5 月合併營收年增合計約 +24% YoY，低於華爾街對 Q2 約 +35% 預期；6 月合併營收（7 月上旬公布）成為關鍵 catalyst，分析師指即使僅小幅月增，也可能使 Q2 營收逼近公司指引（$39.0-40.2B）中值；緩解觀點：5 月營收 NT$4,169.75 億（+30.1% YoY）創單月新高、1-5 月累計 NT$1.96 兆；（2）⚠️ 法律風險——美國 ITC 就 Longitude Licensing 與 Marlin Semiconductor 專利侵權控訴對 TSMC 展開調查，初判預計近期（6 月底/7 月初）出爐，若不利理論上可能對含關鍵 AI 加速器技術晶片發布美國進口禁令；此類案件多以授權金和解收場、全面禁令機率不高；（3）🎯 7 月上旬 6 月營收 + 7/16 Q2 法說會為短線方向關鍵。",
        "source": "ts2.tech / Seeking Alpha / USITC / Reuters 2026-06-24 - 06-30", "impact": "負面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 監理與競爭雜訊：Super Micro 遭台灣搜索 -8%（涉 NVIDIA 晶片走私中國）；南韓公布逾 $1 兆美元 AI/晶片投資、三星+SK 800 兆韓元",
        "body": "監理與區域競爭為中期觀察重點：（1）⚠️ 超微電腦（Super Micro）遭台灣當局搜索、涉一樁 NVIDIA AI 晶片走私中國的調查，股價重挫 -8.1%——凸顯 AI 晶片出口管制與供應鏈監理風險升溫；（2）⚠️ 南韓公布逾 $1 兆美元 AI/晶片產業戰略，三星電子與 SK 海力士將攜供應商投資 800 兆韓元（約 $5,180 億）於西南部各建兩座新晶圓廠——中長期強化韓系代工/記憶體競爭；（3）⚠️ 客戶分散續存：Apple 同意與 Intel 在美合作生產、Google/AMD/Tesla 傳接觸 Samsung、Intel 18A-P 風險量產並與 UMC 結盟；（4）✅ 緩解：TSMC 在 N3/N2/A16 具明顯技術代差與良率領先、Apple 仍跳過 A16 直上 A14。",
        "source": "StartupHub.ai / Al Jazeera / Reuters / CNBC 2026-06-29 - 06-30", "impact": "負面",
    },
]

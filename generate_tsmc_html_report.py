...
STOCK = {
    # 台股 2330
    "tw_price":       "2,340",        # 2026-06-26 Fri 官方收盤（-50、-2.09% vs 6/25 NT$2,390；ADR 隔夜漲價漲幅回吐+營收疑慮+外資賣超拖累；6/29 Mon 今日開盤約 NT$2,350 小漲、官方收盤待 TWSE。收盤/量為官方確認）
    "tw_change":      "-50",          # 6/26 漲跌點數（-2.09%）
    "tw_change_pct":  "-2.09%",       # 6/26 漲跌幅（自 6/25 NT$2,390 回落 50 點）
    "tw_volume":      "53,800",       # 6/26 官方成交量（張，量增、賣壓放大）
    "tw_open":        "2,375",        # 6/26 開盤（估、低開）
    "tw_high":        "2,380",        # 6/26 盤中高（估）
    "tw_low":         "2,335",        # 6/26 盤中低（估、收近低）
    "market_cap":     "NT$60.7 兆",   # 6/26 收盤市值（NT$2,340 基礎）
    "pe_ratio":       "~34.5x",       # 台股 P/E（TTM，NT$2,340 基礎）
    "pb_ratio":       "~12.1x",       # 台股 P/B（股淨比，NT$2,340 基礎）
    "div_yield":      "~0.64%",       # 殖利率（NT$2,340 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "432.35",       # 6/26 Fri 收盤（-$2.64，-0.61%；隔夜漲價漲幅回吐、市值約 $2.0 兆、P/E ~31.0；日內區間 $419.19–$436.13；前收 $434.99）
    "nyse_change":    "-2.64",        # 6/26 漲跌（6/26 收盤確認）
    "nyse_change_pct":"-0.61%",       # 6/26 NYSE 連續回吐隔夜漲價漲幅（漲價題材熱度降溫、估值與營收疑慮拉鋸）
    "52w_high_nyse":  "476.79",       # 52 週/歷史高（區間）
    "52w_low_nyse":   "221.18",       # 52 週低（區間）
    "pe_nyse":        "~31.0x",       # NYSE USD 基礎 P/E（TTM，$432.35 基礎）
    "pb_nyse":        "~12.5x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-06-29 Mon（NYSE TSM 6/26 Fri 收盤 $432.35 為最新確認收盤；台股 2330 6/26 Fri 官方收 NT$2,340 為最新確認；台股 6/29 Mon 今日開盤約 NT$2,350 小漲交易中、官方收盤請以 TWSE 為準）。⭐ 今日頭條——TSMC 漲價題材仍為結構性主軸、惟上週五雙市回吐隔夜漲價漲幅：（1）📉 台股 2330 6/26 Fri 收 NT$2,340（-50、-2.09%、量放大至 53,800 張），自 6/25 NT$2,390 回落——隔夜 ADR 漲價漲幅回吐+近期營收疑慮+外資賣超（6/26 外資賣超 6,104 張）拖累；（2）📉 NYSE TSM 6/26 Fri 收 $432.35（-0.61%、-$2.64、市值約 $2.0 兆、P/E ~31.0、日內 $419.19–$436.13），連續回吐 6/24 漲價題材帶動之隔夜漲幅、估值與營收疑慮拉鋸；（3）⭐ 漲價題材結構性利多未變：範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 75%、含 N7/N5/N3）、幅度 5-10% 視客戶/節點/產品而定、部分已開始實施，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（4）📊 台股 2330 6/29 Mon 今日開盤約 NT$2,350（+0.43% vs 6/26 NT$2,340）小幅反彈、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/26 ADR $432.35 + 台股 NT$2,340、匯率 31.05 計）：理論值 5×2,340÷31.05 = $376.81、溢價 (432.35-376.81)/376.81 = +14.74%。⚠️ 漲價雙面刃——利多毛利率，但傳客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）措手不及、恐促部分客戶調整下單量、要求更嚴格合約或轉向替代；Benzinga 分析漲價壓縮 NVIDIA 毛利但因 AI 需求剛性、NVIDIA 具轉嫁能力、長期反為利多。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。⚠️ ITC 專利調查初判預計本週（6 月底）出爐、為短線關鍵雜訊。💰 TSMC 董事會核准大額資本支出、發行新債並小幅減資以支應今年 9 期晶圓廠與先進封裝廠擴張。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%；Samsung/SK 海力士 6 月底送樣第 7 代 HBM4E。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片降低對 TSMC 依賴、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：ITC 初判 6 月底（本週）；6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、+10% QoQ、+32% YoY、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +26% YoY 達 ~$975B；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆（佔 AI 伺服器機櫃價值 95%）  |  ⚠️ 本日（6/29）關鍵觀察：（1）台股 2330 能否守穩 NT$2,310 / NT$2,250 並站回 5MA NT$2,420；（2）ADR 溢價 +14.74%、台股短線承壓；（3）下方支撐 NT$2,310 / NT$2,250 / NT$2,200；上方壓力 NT$2,390 / NT$2,450 / NT$2,490 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~31.0x、台股 P/E ~34.5x，留意 ITC 初判、6 月營收 miss 風險、漲價客戶反彈與台幣升值",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ TSMC 漲價題材仍為結構性主軸（7nm 以下全製程、佔晶圓營收 ~75%、估全年毛利率 +2pp）；📉 上週五雙市回吐隔夜漲價漲幅——台股 2330 6/26 收 NT$2,340（-2.09%）、NYSE TSM 6/26 收 $432.35（-0.61%）；6/29 今日台股開盤約 NT$2,350 小反彈",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📉 台股 2330 6/26 Fri 收 NT$2,340（-50、-2.09%、量放大至 53,800 張）——隔夜漲價漲幅回吐+營收疑慮+外資賣超 6,104 張拖累；6/29 今日開盤約 NT$2,350 小反彈",
        "📉 NYSE TSM 6/26 Fri 收 $432.35（-0.61%、-$2.64、市值約 $2.0 兆、日內 $419.19–$436.13）——連續回吐 6/24 漲價題材帶動之隔夜漲幅、估值與營收疑慮拉鋸",
        "⭐ 漲價題材結構性利多未變：範圍確認擴大至 7nm 以下全製程（佔晶圓營收約 75%、含 N7/N5/N3）、幅度 5-10%、部分已實施、估貢獻全年毛利率 +2pp 以上",
        "📊 ADR 溢價 +14.74%（以 6/26 ADR $432.35 + 台股 NT$2,340、匯率 31.05 計）",
        "⚠️ 漲價雙面刃：客戶（Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多；4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）關鍵",
        "🏆 中長線基底未變：NVIDIA #1 客戶/A16 首發 2027 + NVIDIA-TSMC AI 製程合作 + TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + 董事會核准大額 CapEx/發債支應 9 期擴廠",
        "⚠️ 風險續追蹤：TSMC 遭美 ITC 專利調查 6 月底（本週）待初判；Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-06-29 Mon（NYSE TSM 6/26 Fri 收盤 $432.35 為最新確認收盤；台股 2330 6/26 Fri 官方收 NT$2,340 為最新確認；台股 6/29 Mon 今日開盤約 NT$2,350 小漲交易中、官方收盤請以 TWSE 為準）。⭐ 今日頭條——TSMC 漲價題材仍為結構性主軸、惟上週五雙市回吐隔夜漲價漲幅：（1）📉 台股 2330 6/26 Fri 收 NT$2,340（-50、-2.09%、量放大至 53,800 張），自 6/25 NT$2,390 回落——隔夜 ADR 漲價漲幅回吐+近期營收疑慮+外資賣超（6/26 外資賣超 6,104 張、投信買超 179 張、自營賣超 310 張、三大法人合計賣超 6,235 張）拖累；（2）📉 NYSE TSM 6/26 Fri 收 $432.35（-0.61%、-$2.64、市值約 $2.0 兆、P/E ~31.0、日內區間 $419.19–$436.13、前收 $434.99），連續回吐 6/24 漲價題材帶動之隔夜漲幅、估值與營收疑慮拉鋸；（3）⭐ 漲價題材結構性利多未變：範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 75%、含 N7/N5/N3，其中 N3 單一即佔 Q1 晶圓營收 25%）、幅度 5-10% 視客戶/節點/產品而定、部分已開始實施且要求客戶將更高成本計入未來訂單，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（4）📊 台股 2330 6/29 Mon 今日開盤約 NT$2,350（+0.43% vs 6/26）小幅反彈、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/26 ADR $432.35 + 台股 NT$2,340、匯率 31.05 計）：理論值 5×2,340÷31.05 = $376.81、溢價 (432.35-376.81)/376.81 = +14.74%。⚠️ 漲價雙面刃：利多毛利率（估全年 +2pp 以上），但傳客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）措手不及、恐促部分客戶調整下單量、要求更嚴格合約條款或轉向替代；Benzinga 分析指漲價雖壓縮 NVIDIA 毛利、惟因 AI 需求剛性與 NVIDIA 議價/轉嫁能力強、長期反為 NVIDIA 股價利多。6 月營收（7 月上旬公布）為關鍵 catalyst——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、惟即使僅小幅月增亦可能使 Q2 營收逼近指引（$39.0-40.2B）中值。⚠️ ITC 專利調查初判預計本週（6 月底）出爐、為短線關鍵雜訊。⭐ 中長線結構性利多基底未變：（1）🤝 TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產、~2,000 就業、InFO/CoWoS、呼應 TSMC $165B 美國投資承諾，並擴及韓國產能）；（2）🏆 NVIDIA 確認 TSMC 第 1 大客戶（超越 Apple、22% / $33B）、為 A16（1.6nm）首發客戶 2027 量產；Apple 跳過 A16 直上 A14（1.4nm）；（3）⭐ NVIDIA-TSMC 深化 AI 導入晶圓廠合作（cuLitho 運算微影、缺陷檢測、工廠排程）、提升良率與產能；NVIDIA RTX Spark 超晶片（ARM Grace + Blackwell、1 petaflop）由 TSMC 代工；（4）⭐ TSMC 2nm 已量產、初期良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（5）📦 CoWoS 規劃 2026 底擴至 ~130K wpm 良率 >98%、CoPoS 面板級封裝互補緩解 AI 封裝瓶頸；（6）4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（7）⭐ 5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆；（8）2026 全球半導體市場估 +26% YoY 達 ~$975B、SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆（佔 AI 伺服器機櫃價值 95%）。⚠️ 短期/中期下行風險：（1）⚠️ 漲價恐引發客戶反彈與訂單調整；（2）⚠️ 近期營收 miss 風險——4-5 月 +24% YoY 落後 Q2 ~35% 預期、6 月營收為關鍵驗證；（3）⚠️ 法律風險——TSMC 遭美 ITC 專利調查（Longitude Licensing & Marlin Semiconductor 控訴）、6 月底（本週）預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（4）⚠️ 客戶分散與競爭——Apple 同意與 Intel 合作在美生產晶片、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（5）⚠️ 估值警示——TSM P/E ~31.0x、台股 P/E ~34.5x；（6）台幣升值壓力（每升 1% 毛利率 -40bps）。基本面未變：Q1 2026 淨利 NT$572.5B（$18.2B、+58.3% YoY）、毛利率 66.2% 雙創史高；2025 全年 EPS NT$66.25。技術面（基於 6/26 收 NT$2,340）：5MA NT$2,420 / 20MA NT$2,395 / 60MA NT$2,365 / 120MA NT$2,260；收盤跌破 5MA／20MA、短線轉弱；下方支撐 NT$2,310 / NT$2,250 / NT$2,200；上方壓力 NT$2,390 / NT$2,450 / NT$2,490 / NT$2,535（史高）。32 位分析師全數看多、平均目標 NT$2,530 上行空間 +8.12%（自 NT$2,340、Susquehanna 6/23 上調 $575、待 7/16 法說會後重估）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（6/29 Mon）關鍵觀察：（1）📉 上週五雙市回吐隔夜漲價漲幅——台股 2330 6/26 收 NT$2,340（-2.09%、量放大 53,800 張）、NYSE TSM 6/26 收 $432.35（-0.61%、日內 $419.19–$436.13）；6/29 今日台股開盤約 NT$2,350 小反彈、官方收盤待 TWSE；（2）⭐ 漲價題材結構性利多未變——範圍確認擴大至 7nm 以下全製程（佔晶圓營收 ~75%、含 N7/N5/N3）、幅度 5-10%、部分已實施、估全年毛利率 +2pp 以上；（3）📊 ADR 溢價 +14.74%（以 6/26 ADR $432.35 計）、台股短線承壓；（4）⚠️ 漲價雙面刃——客戶（Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多、惟部分客戶恐調整下單量或轉向替代；（5）⚠️ TSMC 4-5 月營收合計 +24% YoY 低於華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst；（6）⚠️ ITC 專利調查初判預計本週（6 月底）出爐——若不利 TSMC 恐對含 AI 加速器技術晶片發布美國進口禁令；（7）📈 Susquehanna 將 TSM 目標價自 $500 上調至 $575；（8）🤝 TSMC-Amkor 簽 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產、擴及韓國）；（9）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、為 A16 首發客戶 2027 量產；NVIDIA-TSMC 深化 AI 製程合作；（10）⚠️ 風險續追蹤——Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（11）⭐ 2nm 量產良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（12）⭐ 4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（13）⚠️ 估值警示：TSM P/E ~31.0x、台股 P/E ~34.5x、留意 ITC 初判、6 月營收 miss 風險與台幣升值；32 位分析師全數看多、平均目標 NT$2,530（上行 +8.12%、待法說會重估）；本日（6/29）關鍵：台股能否守穩 NT$2,310 / NT$2,250 並站回 5MA NT$2,420；上方壓力 NT$2,390 / NT$2,450 / NT$2,490 / NT$2,535 史高",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-06-26",       # 最新資料日（6/26 Fri 官方盤後；6/29 今日盤後待公布）
    "foreign":  -6104,              # 外資 6/26 官方賣超 6,104 張（漲價漲幅回吐+營收疑慮、外資續調節）
    "trust":    +179,              # 投信 6/26 官方買超 179 張（逆勢小買）
    "dealer":   -310,              # 自營商 6/26 官方賣超 310 張
    # 近5日（由舊到新：6/22, 6/23, 6/24, 6/25, 6/26）；6/19 端午休市；6/26 為官方、其餘部分為估算
    "foreign_5d": [+18000, -9000, -15000, +1500, -6104],     # 外資 6/26 官方賣超 6,104 張
    "trust_5d":   [+2000, +500, +800, +600, +179],           # 投信 6/26 官方買超 179 張
    "dealer_5d":  [+800, -200, -500, +300, -310],            # 自營 6/26 官方賣超 310 張
    "foreign_ownership_pct": "74.7%",  # 外資持股比例（6/26 估微降至 74.7%）
    "foreign_ownership_shares": "約 193.6 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       46.0,   "rsi_signal": "RSI 估回落至 46（台股 6/26 -2.09%），跌破中軸、動能轉弱；⚠️ 6/29 留意能否止穩回升、ITC 初判雜訊",
    "macd":         0.05,  "macd_signal": "MACD 估降至 +0.05、紅柱收斂貼近零軸、⚠️ 動能轉弱、瀕臨死叉、待方向確認",   "macd_color": "#FF9800",
    "kdj_k":        42.0,   "kdj_d": 46.0, "kdj_j": 34.0, "kdj_signal": "KDJ K(42)/D(46)/J(34) 轉弱、6/26 下跌後 J 值偏低、短線方向待 6/29 量能與 ADR 走勢確認",
    "ma5":       "2,420",   # 5MA（6/22 2510, 6/23 2490, 6/24 2390, 6/25 2390, 6/26 2340）— 基於 6/26 收
    "ma20":      "2,395",   # 20MA
    "ma60":      "2,365",   # 60MA 緩升
    "ma120":     "2,260",
    "bb_upper":  "2,560",   # 布林上軌
    "bb_mid":    "2,395",   # 布林中軌（20MA）
    "bb_lower":  "2,230",   # 布林下軌
    "vol_ma5":  "33,000",   # 5日均量（張，估、基於 6/22-6/26 五日均、6/26 量放大）
    "trend":    "⭐ TSMC 漲價題材仍為結構性主軸、惟上週五雙市回吐隔夜漲價漲幅：NYSE TSM 6/26 收 $432.35（-0.61%、連續回吐 6/24 隔夜漲幅、市值約 $2.0 兆、日內 $419.19–$436.13）；台股 2330 6/26 收 NT$2,340（-2.09%、量放大至 53,800 張、外資賣超 6,104 張）；6/29 今日台股開盤約 NT$2,350 小反彈。📊 技術面（基於 6/26 收 NT$2,340）：RSI 回落至 46 跌破中軸、MACD +0.05 瀕臨死叉、KDJ 轉弱、收盤跌破 5MA NT$2,420 與 20MA NT$2,395、中期 60MA/120MA 趨勢仍偏多。下一觀察：（1）⚠️ 台股能否守穩 NT$2,310 / NT$2,250、站回 5MA NT$2,420；（2）下方支撐 NT$2,310 / NT$2,250 / NT$2,200；（3）上方壓力 NT$2,390 / NT$2,450 / NT$2,490 / NT$2,535 史高；（4）⭐ 漲價利多估全年毛利率 +2pp 以上、惟須留意客戶反彈與 6 月營收（7 月上旬、4-5 月 +24% YoY 落後 ~35% 預期）；（5）⚠️ 雜訊續追蹤：ITC 專利調查 6 月底（本週）初判、Apple-Intel 合作、Intel 18A-P 風險量產；（6）⭐ 中長線結構性利多基底未變：Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + 2nm 量產 70-80% 良率 + Susquehanna 目標升至 $575。",
    "support":  "2,310 / 2,250 / 2,200 / 2,150 (整數、整數、整數、整數)",
    "resist":   "2,390 / 2,450 / 2,490 / 2,535 (6/25 收、整數、6/23 收、史高)",
    "note": "技術指標數值為估算（基於 6/26 收 NT$2,340），請以看盤軟體即時數據為準；台股 6/26 -2.09%、NYSE 6/26 -0.61% 連續回吐隔夜漲價漲幅、RSI 回落至 46 跌破中軸、KDJ 轉弱、MACD 瀕臨死叉；NT$2,310 為關鍵支撐、上方壓力 NT$2,390 / NT$2,450；6/29 今日台股開盤小反彈交易中、⭐ 漲價利多 vs ⚠️ 隔夜回吐+營收 miss 風險+ITC 初判拉鋸",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-06-26",       # 最新資料日（6/26 Fri 收盤估、6/29 今日盤後待公布）
    "futures_price": "2,338",            # 台積電期貨近月（6/26 收盤附近、小幅貼近現貨）
    "futures_oi":    "172,000",          # 未平倉量（口）（6/26 估、小幅增倉）
    "futures_basis": "-2",               # 6/26 估轉小幅逆價差（雙市回吐隔夜漲幅、情緒轉謹慎）
    "call_oi":       "106,000",
    "put_oi":        "80,000",
    "pcr":           "0.75",             # Put/Call 比率（6/26 估 0.75、台股下跌、避險買盤增、情緒中性偏謹慎）
    "pcr_signal":    "中性偏謹慎（Put/Call 6/26 估 0.75；台股 6/26 -2.09%、雙市回吐隔夜漲價漲幅、避險買盤增、情緒轉謹慎；⭐ 漲價利多為中長線支撐、⚠️ 短線估值/營收/ITC 初判疑慮拉鋸）",
    "max_pain":      "2,390",            # 最大痛苦價格（近期到期、6/26 估）
    "key_strikes": [
        ("2,250", "Call 54,000 / Put 20,000", "整數支撐"),
        ("2,310", "Call 56,000 / Put 16,000", "整數支撐"),
        ("2,340", "Call 54,000 / Put 11,000", "6/26 收"),
        ("2,390", "Call 50,000 / Put 8,500",  "6/25 收、最大痛苦"),
        ("2,450", "Call 42,000 / Put 6,000",  "整數壓力"),
        ("2,490", "Call 36,000 / Put 4,500",  "6/23 收"),
        ("2,535", "Call 28,000 / Put 3,000",  "盤中史高"),
    ],
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；6/26 PCR 自 0.71 升至 0.75 反映台股下跌 -2.09% 及雙市回吐隔夜漲價漲幅帶動避險買盤增、情緒轉謹慎、期貨轉小幅逆價差；NT$2,390 為最大痛苦、下方支撐 NT$2,310、上方壓力 NT$2,390 / NT$2,450；⭐ 漲價題材結構性利多未變或支撐多方、⚠️ 留意 ITC 初判、營收 miss 風險與客戶反彈；中長線結構性利多基底未變（TSMC-Amkor 美國封裝長約、NVIDIA 確認第 1 大客戶 + A16 首發、2nm 量產 70-80% 良率、Susquehanna 目標升至 $575）；⚠️ 雜訊續追蹤：ITC 專利調查 6 月底（本週）初判、Apple-Intel 合作",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-06-26 Fri 收盤、台股為 6/26 Fri 收盤；價格為估算、上週五半導體普遍回吐隔夜漲幅）
    ("設備", "ASML",    "艾司摩爾",    "860.00", "-0.90%", "EUV 設備龍頭（6/26 隨半導體小幅回吐；高 NA EUV 需求結構性多頭基底未變）"),
    ("設備", "AMAT",    "應用材料",    "518.00", "-0.80%", "🛠️ 6/25 推新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）；TSMC EPIC Center $5B AI 合作；6/26 小幅回吐"),
    ("設備", "LRCX",    "科林研發",    "1,035.00", "-1.00%", "蝕刻設備（6/26 小幅回吐；半導體設備需求中長線創紀錄）"),
    ("設備", "KLAC",    "科磊",        "780.00", "-1.00%", "量測設備（6/26 小幅回吐；2nm 量產驅動）"),
    ("記憶", "000660.KS","SK 海力士",  "390,000 KRW", "-0.50%", "🤝 SK 海力士-TSMC 深化 HBM4 合作；6/26 第 7 代 HBM4E 送樣大客戶；亞股小幅整理"),
    ("記憶", "MU",      "美光",        "146.00", "-1.70%", "HBM / DDR5（6/26 回吐；HBM4 需求中長線強勁、惟記憶體價格雜訊）"),
    ("封裝", "3711.TW", "日月光",      "319",    "-0.93%", "CoWoS / 先進封裝（6/26 隨大盤回落；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "50.00",  "-1.00%", "TSMC-Amkor 簽 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（擴及韓國）；6/26 小幅回吐（本波利多核心）"),
    ("客戶", "NVDA",    "輝達",        "270.00", "-1.10%", "🏆 確認 TSMC #1 客戶 22%、A16 首發客戶 2027 量產；6/18 發行 $250 億公司債；Benzinga 指漲價長期反利多"),
    ("客戶", "AAPL",    "蘋果",        "297.00", "-0.83%", "⚠️ #2 客戶 18%；面臨 TSMC 漲價成本；Apple 同意與 Intel 合作在美生產晶片以降低對 TSMC 依賴；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "462.00", "-1.30%", "⚠️ #3 客戶（6/26 回吐；面臨漲價成本；近期 AI 晶片展望偏保守引發拋售；custom XPU 中長線強勁）"),
    ("客戶", "AMD",     "超微",        "508.00", "-1.40%", "#4 客戶（6/26 回吐；面臨漲價成本；瞄準 $1,200 億市場、MI350/MI450 GPU 搶市；傳接觸 Samsung 先進製程）"),
    ("客戶", "2454.TW", "聯發科",      "4,680",  "-0.64%", "手機 SoC 客戶 9%；6/26 回落；面臨漲價成本；RTX Spark 由聯發科協同 NVIDIA 設計"),
    ("競爭", "INTC",    "英特爾",      "50.20",  "+0.80%", "Apple 將與 Intel 合作在美設計製造晶片；18A-P 進入風險量產（效能 +9%、傳 Apple/Google 洽詢）；與 UMC 結盟挑戰 TSMC"),
    ("競爭", "005930.KS","三星電子",   "66,500 KRW", "+0.76%", "Samsung Foundry 6/26 亞股小漲；傳 Google/AMD/Tesla 接觸其先進製程；HBM4E 送樣大客戶；PLP 面板級封裝與 TSMC CoPoS 競爭"),
    ("處分後", "ARM",   "Arm Holdings","255.00", "-1.20%", "TSMC 4/29 全數出脫後（6/26 小幅回吐；RTX Spark ARM 架構中長線受惠）"),
]
...
CONSENSUS = {
    ...
    "upside":       "+1.61%",   # 從 NT$2,490（6/23 官方收）到 NT$2,530（待 7/16 法說會後重估；Susquehanna 6/23 上調 $575）
}
...
IR_CALENDAR = [
    ...
    ("2026-06-29", "📊 台股開盤小反彈、官方收盤待 TWSE", "6/29 Mon 台股 2330 今日開盤約 NT$2,350（+10、+0.43% vs 6/26 NT$2,340）小幅反彈、官方收盤以 TWSE 為準；NYSE TSM 6/29 美股盤後收盤（台灣時間 6/30 凌晨）待確認、最新確認收盤為 6/26 $432.35；⭐ 漲價題材結構性利多（7nm 以下全製程、佔晶圓營收 ~75%、估全年毛利率 +2pp）vs ⚠️ 隔夜回吐+營收 miss 風險+ITC 初判（本週、6 月底）拉鋸；🎯 關鍵觀察：台股能否守穩 NT$2,310 / NT$2,250 並站回 5MA NT$2,420",  "#FFF9C4"),
...
NEWS = [
    {
        ...
        "title": "📉 上週五雙市回吐隔夜漲價漲幅：台股 2330 6/26 收 NT$2,340（-2.09%）、NYSE TSM 收 $432.35（-0.61%）；⭐ 漲價題材結構性利多未變（7nm 以下全製程、佔晶圓營收 ~75%、估全年毛利率 +2pp）",
        "body": "今日頭條——TSMC 漲價題材仍為結構性主軸、惟上週五雙市回吐隔夜漲價漲幅：（1）📉 台股 2330 6/26 Fri 收 NT$2,340（-50、-2.09%、量放大至 53,800 張），自 6/25 NT$2,390 回落——隔夜 ADR 漲價漲幅回吐+近期營收疑慮+外資賣超（6/26 外資賣超 6,104 張、投信買超 179 張、自營賣超 310 張）拖累；（2）📉 NYSE TSM 6/26 Fri 收 $432.35（-0.61%、-$2.64、市值約 $2.0 兆、P/E ~31.0、日內 $419.19–$436.13、前收 $434.99），連續回吐 6/24 漲價題材帶動之隔夜漲幅、估值與營收疑慮拉鋸；（3）⭐ 漲價題材結構性利多未變：據 Tom's Hardware / Yahoo Finance 等報導，TSMC 漲價範圍確認擴大至 7nm 以下全部先進製程（含 N7/N5/N3、佔晶圓營收約 75%，其中 N3 單一即佔 Q1 晶圓營收 25%），幅度 5-10% 視客戶/節點/產品而定、部分已開始實施，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（4）📊 ADR 溢價 +14.74%（以 6/26 ADR $432.35 + 台股 NT$2,340、匯率 31.05 計）；台股 6/29 今日開盤約 NT$2,350 小反彈、官方收盤待 TWSE。產能面：TSMC 今年規劃 9 期晶圓廠與先進封裝廠、2nm/A16 產能 2026-28 估 CAGR ~70%。",
        "source": "Tom's Hardware / Yahoo Finance / histock / StockAnalysis 2026-06-24 - 06-26", "impact": "中性",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "💡 Benzinga：TSMC 漲價雖壓縮 NVIDIA 毛利、惟因 AI 需求剛性與轉嫁能力強、長期反為 NVIDIA 股價利多",
        "body": "漲價對核心客戶影響評估成為市場焦點：（1）💡 Benzinga 分析指出，TSMC 對先進製程漲價 5-10% 雖將推升 NVIDIA、AMD、Apple、Qualcomm、Broadcom、MediaTek 等客戶的晶圓成本、短期壓縮毛利；（2）⭐ 惟因 AI 加速器需求剛性、供不應求，NVIDIA 具強大議價與成本轉嫁能力（可反映於售價），漲價反凸顯 TSMC 製程稀缺性與 NVIDIA 產品定價力，長期反為 NVIDIA 股價利多；（3）⚠️ 對毛利較薄、終端價格敏感的消費性客戶（如部分手機 SoC、成熟製程產品）影響較大、恐促部分客戶調整下單量或評估替代來源；（4）🏆 NVIDIA 仍為 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產，NVIDIA-TSMC 深化 AI 導入晶圓製造（cuLitho/缺陷檢測/工廠排程）。",
        "source": "Benzinga / Windows Central / Wccftech 2026-06-24 - 06-25", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🛠️ 產業競局升溫：Intel 18A-P 進入風險量產+UMC 結盟、AMD 瞄準 $1,200 億市場、Qualcomm 推 Dragonfly；TSMC-Amkor 擴美韓封裝、Samsung/SK 送樣 HBM4E",
        "body": "半導體產業競局與 AI 需求持續升溫、TSMC 為核心受惠者：（1）🛠️ Applied Materials（AMAT）6/25 發布新晶片製造系統，加速 DRAM 與 AI 先進封裝（含 HBM 與 3D 堆疊）採用；（2）⚙️ Intel 18A-P 製程進入風險量產、傳獲 Apple/Google 洽詢並與台灣 UMC 結盟挑戰 TSMC 代工地位；AMD 透過新超算訂單瞄準 $1,200 億資料中心市場、Qualcomm 推 Dragonfly 品牌切入；（3）🤝 TSMC-Amkor 結盟擴大 Arizona 與韓國先進封裝產能以滿足 AI/HPC 需求；Samsung 與 SK 海力士加速送樣第 7 代 HBM4E 給大客戶；（4）💰 TSMC 董事會核准大額資本支出、發行新債並小幅減資，支應今年 9 期晶圓廠與先進封裝廠擴張；（5）📈 SIA/Deloitte：2026 全球半導體市場估 ~$975B（YoY +26%）、AI 資料中心晶片營收 2028 上看 >$1.2 兆、半導體於 AI 伺服器機櫃價值佔比達 95%。",
        "source": "GlobeNewswire / SIA / Deloitte / distillintelligence / ts2.tech 2026-06-19 - 06-26", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⚠️ TSMC 遭美 ITC 專利調查、初判預計本週（6 月底）出爐；恐對含關鍵 AI 加速器技術晶片發布美國進口禁令——短線關鍵雜訊",
        "body": "潛在法律與供應鏈風險、本週為關鍵時點：（1）⚠️ 美國國際貿易委員會（USITC）正對 TSMC 展開調查，源於授權公司 Longitude Licensing 與 Marlin Semiconductor 針對其先進製程節點提出的專利侵權控訴；（2）⚠️ 預計 6 月底（本週）作出初步裁決（preliminary ruling）——若不利於 TSMC，理論上可能對使用關鍵 AI 加速器技術製造的晶片發布美國進口禁令、為近日股價回吐之雜訊來源之一；（3）⭐ 影響評估：此類 ITC 案件多以授權金和解收場、實際全面進口禁令機率不高，但涉及 AI 加速器核心技術、若裁定不利恐衝擊 NVIDIA/AMD 等客戶供應鏈與市場情緒；（4）✅ TSMC 具龐大專利組合與法務資源、過往類似訴訟多能妥善處理；（5）🎯 觀察：留意本週/7 月初 ITC 初判結果與 TSMC 回應。",
        "source": "USITC / Reuters / Bloomberg / ts2.tech 2026-06-18 - 06-26", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 近期營收疑慮續存：TSMC 4-5 月營收 +24% YoY 落後華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst",
        "body": "近期營收動能為短線焦點、漲價利多須營收驗證：（1）⚠️ TSMC 4 月與 5 月合併營收年增合計約 +24% YoY，低於華爾街對 Q2 約 +35% 預期，引發近期營收 miss 疑慮；（2）🎯 6 月合併營收（7 月上旬公布）成為關鍵 catalyst——分析師指出即使僅小幅月增，也可能使 Q2 營收逼近公司指引（$39.0-40.2B）中值；（3）✅ 緩解觀點：5 月營收 NT$4,169.75 億（+30.1% YoY）創單月新高、1-5 月累計 NT$1.96 兆，公司被認為 well-positioned 達成 Q2 指引；C.C. 魏重申「2026 美元營收成長 30% 以上」；（4）⭐ 中長線需求未變：2026 全球半導體市場估 +26% YoY 達 ~$975B、AI 晶片市場估 ~$500B；（5）🎯 7 月上旬 6 月營收 + 7/16 Q2 法說會將為短線方向關鍵。",
        "source": "ts2.tech / Seeking Alpha / Focus Taiwan 2026-06-24 - 06-26", "impact": "負面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 客戶分散與競爭雜訊續存：Apple 與 Intel 合作在美生產晶片、Google/AMD/Tesla 接觸 Samsung；Intel 18A-P 風險量產+UMC 結盟",
        "body": "客戶分散與競爭為 TSMC 中期觀察重點、漲價或加速雙供應評估：（1）⚠️ Apple 已同意與 Intel 合作在美國本土製造晶片、以降低對 TSMC 依賴；（2）⚠️ Google、AMD、Tesla 傳積極接觸 Samsung Foundry 先進製程、尋求第二供應來源；Intel 18A-P 進入風險量產（同功耗效能 +9%）並與台灣 UMC 結盟挑戰 TSMC；（3）⚠️ TSMC 漲價可能促使部分價格敏感客戶加速評估替代來源；（4）✅ 緩解因素：TSMC 在 N3/N2/A16 先進製程具明顯技術代差與良率領先、Apple 仍跳過 A16 直上 TSMC A14（1.4nm）、客戶分散多屬成熟/特定產品線；（5）🎯 觀察：TSMC 最先進節點訂單能見度仍滿載、惟須留意成熟製程市占與長期客戶集中度變化。",
        "source": "Reuters / CNBC / Bloomberg / ts2.tech 2026-06-18 - 06-25", "impact": "負面",
    },
]

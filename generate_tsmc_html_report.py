...
STOCK = {
    # 台股 2330
    "tw_price":       "2,365",        # 台股 2330 最新確認收盤：8/6 Thu 官方收 NT$2,365（-40、-1.66% vs 8/5 NT$2,405、開 2,395／高 2,395／低 2,360、TWSE 確認）——縮量拉回、回吐 8/5 大漲的一半，惟仍站 20MA 2,364／60MA 2,354；大盤同日 -214.90 點（-0.48%）收 44,396.70
    "tw_change":      "-40",          # 8/6 Thu 收盤漲跌點數（-1.66%、TWSE 官方）
    "tw_change_pct":  "-1.66%",       # 8/6 Thu 縮量拉回：高開 2,395 後全日走低、收 2,365——跌破 5MA 2,377 但守住 20MA 2,364（貼線 +1 元）與 60MA 2,354；量僅 25,537 張（5 日均量 41,605 的 61%、近月最低）；同晚美股半導體反創本波高（SOX +0.33%、TSM +1.01% 收 $418.20）——台美背離重現
    "tw_volume":      "25,537",       # 8/6 成交量（張、TWSE 口徑；成交額 NT$606.01 億、成交 89,846 筆——僅 5 日均量 41,605 張的 61%、近月最低，縮量整理而非爆量出貨）
    "tw_open":        "2,395",        # 8/6 開盤（TWSE、高開 -10 於前收下方後即為全日高點）
    "tw_high":        "2,395",        # 8/6 盤中高（TWSE、開盤即最高、全日單邊走低）
    "tw_low":         "2,360",        # 8/6 盤中低（TWSE、貼近 60MA 2,354 獲支撐後小幅收斂）
    "market_cap":     "NT$61.3 兆",   # 8/6 收盤市值（NT$2,365 基礎、約 259.3 億股）
    "pe_ratio":       "31.80x",       # 台股 P/E（TWSE 官方 BWIBBU 8/6、口徑為最新年度 EPS）
    "pb_ratio":       "10.41x",       # 台股 P/B（TWSE 官方 BWIBBU 8/6、以 115Q1 財報淨值計）
    "div_yield":      "0.93%",        # 殖利率（TWSE 官方 BWIBBU 8/6、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "418.20",       # 8/6 Thu 官方收盤 $418.20（+$4.20、+1.01% vs 8/5 $414.00、Yahoo API 與 StockAnalysis 雙源確認）——創本波收盤新高，與台股當日 -1.66% 形成明顯背離；SOX 8/6 +0.33% 收 12,048.69 連 2 日站穩 12,000
    "nyse_change":    "+4.20",        # 8/6 漲跌（美元、自我檢核：414.00 × 1.0101 = 418.18 ✓）
    "nyse_change_pct":"+1.01%",       # 8/6 逆勢收高、創本波收盤新高
    "52w_high_nyse":  "479.00",
    "52w_low_nyse":   "223.70",
    "pe_nyse":        "~30.6x",       # 估算（以 $418.20 與 TTM EPS 換算）
    "pb_nyse":        "~18.1x",       # 估算
    "beta":           "1.18",
    "div_next":       "2026 全年股利 NT$24/股（+33% YoY、7/16 法說會宣布）",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-10 中旬（Q3 2026 法說會、日期待公布）",
    "data_note":      '報告日期：2026-08-07 Fri（最新確認收盤：台股 2330 8/6 Thu 官方收 NT$2,365（-40、-1.66% vs 8/5 NT$2,405、開 2,395／高 2,395／低 2,360、量 25,537 張 TWSE 口徑、成交額 NT$606.01 億、成交 89,846 筆、TWSE 確認）；NYSE TSM 8/6 Thu 收 $418.20（+$4.20、+1.01% vs 8/5 $414.00、Yahoo API 與 StockAnalysis 雙源確認、創本波收盤新高））。📊 今日格局：台美再度背離、缺口守住但量能急凍——（1）📊 台股 8/6 拉回但守住關鍵位：2330 開 2,395 全日走低收 2,365（-1.66%）——回吐 8/5 大漲 +85 的一半（-40）、仍站 20MA 2,364（僅 +1 元）與 60MA 2,354；⚠️ 量能急凍至 25,537 張（僅 5 日均量 41,605 張的 61%、近月最低）——縮量整理而非爆量出貨、但也代表 8/5 反攻的追價力道未延續；大盤 -214.90 點（-0.48%）收 44,396.70（FMTQIK 官方）；（2）⚠️ 籌碼翻空：2330 外資轉賣 -4,011 張（8/5 +12,251 張的翻買僅維持 1 日、驗證「ADR 溢價套利一次性回補」的疑慮）、投信 -82 張（連 2 賣）、自營 -583、三大合計 -4,676 張（T86 官方）；惟全市場 BFI82U 合計仍 +51.2 億（投信 +94.1 億、外資 +20.2 億、自營合計 -63.1 億）——資金留在市場但轉出台積電、輪動至中小型；（3）⭐ 美股 8/6 半導體再創本波高：SOX +0.33% 收 12,048.69（連 2 日站上 12,000）；TSM 逆勢 +1.01% 收 $418.20 創本波收盤新高——與台股當日 -1.66% 形成明顯背離；ARM +4.41% 領漲、QCOM +1.82%、ASML +1.56%、AMD +1.50% 收 $489.28（財報賣壓一日結束）；⚠️ 記憶體與設備續弱：MU -1.31%、INTC -1.24%、AMAT -1.27%、LRCX -0.54%；NVDA -0.10% 收 $218.99 高檔持平；大盤 S&P -0.18%、那指 -0.06%、道瓊 -0.85%、VIX 降至 15.15（本波最低）；（4）⚠️ 韓股記憶體 8/6 崩跌為最大事件：SK 海力士 -10.37% 收 149.5 萬韓元、三星 -6.30% 收 23.05 萬、KOSPI 重挫逾 4% 並觸發 sidecar 熔斷——導火線為 SanDisk 8/5 盤後財報 FY27Q1 營收指引 $103-108 億低於共識 $108 億、加上 Western Digital 財測平淡，兩者盤後跌逾 8-10%（TradingKey／KED Global）；⭐ 關鍵判讀：此為「記憶體」利空、非「邏輯晶圓代工」利空——台積電並無 HBM/NAND 曝險，當晚 SOX 反而收紅、TSM 創本波新高即為市場的分流定價；今晨 8/7 韓股分歧（三星 +2.82% 反彈、SK -1.34% 續弱、Yahoo 10:01 KST 快照）；（5）📊 產業背景：2nm 報價漲幅確認落在 10-20%（TrendForce、遠低於傳言 50%、3-7nm 2026 漲 3-10%）；熊本 JASM 廠 7/28 M7.1 地震後已全面復產（TechNews 8/4）；232 條款截至 8/7 晨仍未查得官方公布（未能證實）；（6）📅 7 月合併月營收預計 8/10 前後公布（6 月 NT$4,426.8 億、+67.9% YoY 為史上單月新高、基期極高）。📊 ADR 溢價自 +10.97% 再度擴大至 +13.98%（$418.20 vs 理論值 $366.89、匯率 32.23）——台股跌、ADR 漲的雙向背離重現，歷史上此型態多以台股補漲收斂。技術面（官方收盤計算至 8/6）：收 2,365 跌破 5MA 2,377、仍站 20MA 2,364／60MA 2,354／120MA 2,162——RSI 50.3 貼中軸、MACD 黃金交叉續強（DIF -9.2／DEA -11.1、柱狀圖 +3.7 較前日 +0.5 明顯放大）、KDJ 金叉延續（K 69.5／D 57.6、J 93.4 仍偏高）；支撐 2,364（20MA）／2,360（8/6 低）／2,354（60MA）／2,320（8/4 收）、壓力 2,377（5MA）／2,395（8/6 開高）／2,415（8/5 高）／2,425（7/31 漲停）／2,511（布林上軌）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20%。⭐ 今日（8/7 Fri）觀察：（1）ADR 溢價 +13.98% 是否再度觸發補漲（TSM 創本波新高＋SOX 站穩 12,000 為正面外部條件）；（2）⚠️ 外資 -4,011 張轉賣的延續性——若連 2 賣則 8/5 翻買確認為一次性；（3）量能能否自 25,537 張回升（縮量守住 20MA 為中性、放量跌破則轉弱）；（4）韓股記憶體利空是否續擴散、7 月營收公布時點。',
}
...
SUMMARY = {
    ...
    "headline":   '📊 台美背離重現、記憶體風暴未及代工：2330 8/6 縮量拉回 -1.66% 收 NT$2,365（量 25,537 張僅 61% 均量、近月最低），仍守 20MA 2,364／60MA 2,354；⚠️ 外資翻買僅 1 日即轉賣 -4,011 張、三大合計 -4,676 張；⭐ 美股 8/6 反向走強——SOX +0.33% 收 12,048.69 連 2 日站穩 12,000、TSM +1.01% 收 $418.20 創本波收盤新高、VIX 降至 15.15；⚠️ 韓股記憶體崩跌（SK -10.37%、三星 -6.30%、KOSPI 觸發 sidecar、起因 SanDisk/WDC 財測）——本質為記憶體利空、台積電無 HBM/NAND 曝險；ADR 溢價擴大至 +13.98%，補漲條件再度具備',

    "bullets": [
        '📊 台股 8/6 縮量拉回但守住均線帶：2330 收 NT$2,365（-40、-1.66%、開 2,395／高 2,395／低 2,360）——回吐 8/5 大漲 +85 的一半、跌破 5MA 2,377，惟仍站 20MA 2,364（貼線 +1 元）與 60MA 2,354（季線未再失守）；大盤 -214.90 點（-0.48%）收 44,396.70',
        '⚠️ 量能急凍為最大隱憂：8/6 成交僅 25,537 張、為 5 日均量 41,605 張的 61%（近月最低）、成交額 NT$606.01 億——縮量整理雖非爆量出貨（相對健康），但顯示 8/5 收復季線後缺乏追價買盤延續',
        '⚠️ 籌碼翻空、外資翻買僅 1 日：2330 外資轉賣 -4,011 張（8/5 +12,251 張未能延續、驗證「ADR 溢價套利一次性回補」疑慮）、投信 -82 張連 2 賣、自營 -583、三大合計 -4,676 張（T86 官方）；⭐ 惟全市場 BFI82U 仍 +51.2 億（投信 +94.1 億續買、外資 +20.2 億）——資金輪出台積電而非撤離台股',
        '⭐ 美股 8/6 反向走強、TSM 創本波收盤新高：SOX +0.33% 收 12,048.69 連 2 日站穩 12,000；TSM +1.01% 收 $418.20（Yahoo 與 StockAnalysis 雙源確認）——在台股當日 -1.66% 下逆勢走高、台美背離重現；ARM +4.41% 領漲、QCOM +1.82%、ASML +1.56%、AMD +1.50% 收 $489.28（財報賣壓僅 1 日）',
        '⚠️⭐ 韓股記憶體 8/6 崩跌但屬「記憶體利空、非代工利空」：SK 海力士 -10.37% 收 149.5 萬韓元、三星 -6.30% 收 23.05 萬、KOSPI 重挫逾 4% 觸發 sidecar 熔斷——導火線為 SanDisk 8/5 盤後 FY27Q1 營收指引$103-108 億低於共識 $108 億、加上 Western Digital 財測平淡（TradingKey／KED Global）；台積電無 HBM／NAND 曝險，當晚 SOX 反收紅、TSM 創新高即為市場分流定價',
        '📊 ADR 溢價自 +10.97% 再度擴大至 +13.98%（$418.20 vs 理論值 $366.89、匯率 32.23）——台股跌／ADR 漲的雙向背離重現；VIX 降至 15.15（本波最低）、風險偏好未惡化——歷史上此型態多以台股補漲方式收斂',
        '💡 技術面出現「價跌指標未壞」的溫和背離：收 2,365 跌破 5MA 2,377、仍站 20MA 2,364／60MA 2,354／120MA 2,162；⭐ MACD 黃金交叉續強（DIF -9.2／DEA -11.1、柱狀圖 +3.7 較前日 +0.5 明顯放大、連 2 日紅柱擴張）、RSI 50.3 貼中軸、KDJ 金叉延續（K 69.5／D 57.6）惟 J 93.4 仍偏高',
        '📅 事件行事曆：7 月合併月營收預計 8/10 前後公布（6 月 NT$4,426.8 億、+67.9% YoY 為史上單月新高、基期極高）；232 條款 8/7 晨查證仍未公布（未能證實）；2nm 報價漲幅確認落在 10-20%（TrendForce、遠低於傳言 50%）；熊本 JASM 廠震後全面復產（TechNews 8/4）',
    ],

    "bottom_line": '報告日期：2026-08-07 Fri（最新確認收盤：台股 2330 8/6 Thu 官方收 NT$2,365（-40、-1.66% vs 8/5 NT$2,405、開 2,395／高 2,395／低 2,360、量 25,537 張、額 NT$606.01 億、89,846 筆、TWSE 確認）；NYSE TSM 8/6 收 $418.20（+1.01%、創本波收盤新高、Yahoo API 與 StockAnalysis 雙源確認））。📊 核心判讀：台美背離重現、記憶體風暴未波及邏輯代工、缺口守住但量能是隱憂——（1）📊 台股 8/6 縮量拉回但守住均線帶：2330 高開 2,395 後全日走低收 2,365（-1.66%）、回吐 8/5 大漲的一半；關鍵在於「守住什麼」——仍站 20MA 2,364（僅高 1 元、貼線）與 60MA 2,354（季線），未跌回 8/4 的破線狀態；⚠️ 但量能急凍至 25,537 張、僅 5 日均量 41,605 張的 61%（近月最低）——縮量整理雖非出貨、卻也顯示 8/5 反攻缺乏追價買盤延續；大盤 -0.48% 收 44,396.70；（2）⚠️ 籌碼翻空、外資翻買僅 1 日：2330 外資轉賣 -4,011 張——8/5 的 +12,251 張翻買未能延續，驗證前日報告「恐屬 ADR 溢價套利一次性回補」的疑慮；投信 -82 張連 2 賣、自營 -583、三大合計 -4,676 張（T86 官方）；⭐ 但全市場 BFI82U 合計仍 +51.2 億（投信 +94.1 億續買、外資 +20.2 億小買、自營合計 -63.1 億）——資金並未撤離台股、而是自台積電輪出至中小型與其他族群（8/6 華邦電 +1.18%、日月光 +0.34% 續強，聯發科 -2.00% 同步回檔）；（3）⭐ 美股 8/6 半導體續強、TSM 創本波新高：SOX +0.33% 收 12,048.69 連 2 日站穩 12,000；TSM +1.01% 收 $418.20 為本波收盤新高——在台股當日 -1.66% 的背景下逆勢走高，台美背離重現；ARM +4.41%、QCOM +1.82%、ASML +1.56%、AMD +1.50% 收 $489.28（財報後賣壓僅 1 日）；⚠️ 記憶體與設備續弱：MU -1.31%、INTC -1.24%、AMAT -1.27%；NVDA -0.10% 收 $218.99 高檔整理；大盤 S&P -0.18%、那指 -0.06%、道瓊 -0.85%、VIX 降至 15.15（本波最低、風險偏好未惡化）；（4）⚠️⭐ 韓股記憶體崩跌是本日最重要事件，但對台積電為「間接」而非「直接」利空：8/6 SK 海力士 -10.37% 收 149.5 萬韓元、三星 -6.30% 收 23.05 萬、KOSPI 重挫逾 4% 觸發 sidecar 熔斷；導火線是 SanDisk 8/5 盤後 FY27Q1 營收指引 $103-108 億低於共識 $108 億、加上 Western Digital 財測平淡，兩者盤後跌逾 8-10%，引爆亞洲記憶體全面賣壓（TradingKey／KED Global／Seoul Economic Daily；註：媒體盤中快照跌幅報導為 -5.7~-8.3%，本報告採 Yahoo 官方收盤序列計算）；⭐ 關鍵在於「這是記憶體利空、不是晶圓代工利空」——台積電無 HBM／NAND 曝險，當晚 SOX 反而收紅、TSM 創本波新高，即為市場已完成分流定價；高盛與摩通均維持買進、指韓廠遠期本益比僅 3.5-3.6 倍已脫離基本面；今晨 8/7 韓股分歧（三星 +2.82% 反彈、SK -1.34% 續弱）；（5）📊 產業與基本面背景未變：2nm 報價漲幅確認 10-20%（TrendForce、遠低於傳言 50%）、熊本 JASM 廠震後全面復產、CoWoS 訂滿至 2027、全年上修 >40%、CapEx $60-64B。💡 技術面：收 2,365 跌破 5MA 2,377、仍站 20MA 2,364／60MA 2,354／120MA 2,162——⭐ 值得注意的是 MACD 柱狀圖不跌反擴（+3.7 vs 前日 +0.5）、黃金交叉續強，RSI 50.3 貼中軸、KDJ 金叉延續（K 69.5／D 57.6）惟 J 93.4 仍偏高；指標與價格出現「價跌指標未壞」的溫和背離，屬健康整理特徵；支撐 2,364（20MA）／2,360（8/6 低）／2,354（60MA）／2,320、壓力 2,377（5MA）／2,395（8/6 開高）／2,415／2,425／2,511（布林上軌）／2,535（史高）。📊 ADR 溢價自 +10.97% 再擴大至 +13.98%（$418.20 vs 理論值 $366.89、匯率 32.23）——背離重現、歷史上此型態多以台股補漲方式收斂。⭐ 今日（8/7 Fri）觀察：（1）ADR 溢價 +13.98% ＋ TSM 創本波新高 ＋ SOX 站穩 12,000 —— 補漲條件再度具備；（2）⚠️ 外資是否連 2 賣（若是則 8/5 翻買確認為套利性一次回補、反彈品質下修）；（3）量能能否自 25,537 張回升——守 20MA 2,364 且量增為多方訊號、放量跌破 2,354 季線則轉弱；（4）韓股記憶體利空擴散與否、7 月營收（估 8/10 前後）。⚠️ 風險：外資翻買僅 1 日、量能急凍至近月低、KDJ J 93.4 仍偏高、記憶體利空若外溢至整體半導體評價、232 條款懸而未決。',

    "risk_alert":  '今日（8/7 Fri）關鍵觀察：（1）⚠️ 外資翻買僅維持 1 日：8/5 大買 +12,251 張後、8/6 立刻轉賣 -4,011 張（T86 官方）——前日報告提示的「恐屬 ADR 溢價套利一次性回補」已獲驗證；今日若連 2 賣，8/5 反攻的籌碼基礎將明顯轉弱；（2）⚠️ 量能急凍為最大隱憂：8/6 成交僅 25,537 張、為 5 日均量 41,605 張的 61%（近月最低）——縮量整理雖非爆量出貨（相對健康），但也代表 8/5 收復季線後缺乏追價買盤；價格守住 20MA 2,364 僅高 1 元、貼線而非站穩，緩衝極薄；（3）⭐ 外部條件反而轉佳：美股 8/6 SOX +0.33% 收 12,048.69 連 2 日站穩 12,000、TSM +1.01% 收 $418.20 創本波收盤新高、VIX 降至 15.15（本波最低）——ADR 溢價擴大至 +13.98%，補漲條件再度具備；（4）⚠️ 韓股記憶體崩跌的外溢風險：8/6 SK 海力士 -10.37%、三星 -6.30%、KOSPI 觸發 sidecar，起因為 SanDisk／Western Digital 財測平淡——⭐ 本質為記憶體利空、台積電無 HBM／NAND 曝險，當晚 SOX 收紅已顯示市場分流定價；但若賣壓外溢至整體半導體評價、台股半導體情緒仍會受壓；今晨 8/7 韓股分歧（三星 +2.82%、SK -1.34%）——尚未見二次崩跌；（5）💱 匯率 32.23 持平；232 條款 8/7 晨查證仍未公布（未能證實）、台美關稅無新進展；7 月合併月營收預計 8/10 前後公布——6 月 NT$4,426.8 億（+67.9% YoY）基期極高、需留意年增率自然回落；（6）⚠️ 中期：SOX 自 6/22 史高 14,634.66 仍 -17.7%；2330 自 6/23 史高 2,535 仍 -6.7%——反彈中期確認仍需量能配合。',
}
...
INSTITUTIONAL = {
    ...
    "trust":    -82,
    "dealer":   -583,
    # 近 5 交易日（7/31, 8/3, 8/4, 8/5, 8/6）
    "foreign_5d": [13780, -9717, -12430, 12251, -4011],
    "trust_5d":   [7063, 387, 434, -1205, -82],
    "dealer_5d":  [2724, -1587, -2327, -1099, -583],
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       50.3,   "rsi_signal": 'RSI(14) 50.3（官方收盤計算）——8/6 拉回 -1.66% 後自 53.0 微降至 50.3、仍貼中軸線上方——中性；跌破 50 則短線動能轉弱、續站 50 上為修復延續',
    "macd":         3.69,   "macd_signal": 'MACD 黃金交叉續強（DIF -9.2 上行、DEA -11.1、柱狀圖 +3.7、官方收盤計算）——⭐ 紅柱較前日 +0.5 明顯放大（連 2 日擴張），在價格下跌 -1.66% 的同時指標反而轉強，形成「價跌指標未壞」的溫和背離；惟 DIF 仍在零軸下、上穿零軸為中期修復的下一確認',   "macd_color": "#4CAF50",
    "kdj_k":        69.5,   "kdj_d": 57.6, "kdj_j": 93.4, "kdj_signal": 'KDJ K(69.5)/D(57.6)/J(93.4)（官方收盤計算）——金叉延續、K 與 D 續升且開口未收斂；⚠️ J 值 93.4 仍偏高（自 96.3 微降）、短線過熱未完全消化，追高需留意',
    "ma5":       "2,377",
    "ma20":      "2,364",
    "ma60":      "2,354",
    "ma120":     "2,162",
    ...
    "bb_lower":  "2,217",
    "vol_ma5":  "41,605",
    "trend":    '📊 縮量拉回、缺口守住：2330 8/6 官方收 NT$2,365（-40、-1.66%、開 2,395／高 2,395／低 2,360、量 25,537 張僅 5 日均量 41,605 張的 61%）——回吐 8/5 大漲的一半、跌破 5MA 2,377，但仍站 20MA 2,364（貼線 +1 元）與 60MA 2,354（季線未再失守）；大盤 -0.48% 收 44,396.70。⭐ 技術指標（官方收盤計算）出現「價跌指標未壞」的溫和背離：MACD 黃金交叉續強——DIF -9.2 上行、DEA -11.1、柱狀圖 +3.7 較前日 +0.5 明顯放大（連 2 日紅柱擴張）；RSI 50.3 貼中軸持平、KDJ 金叉延續（K 69.5／D 57.6）惟 J 93.4 仍偏高。⚠️ 籌碼：外資轉賣 -4,011 張（8/5 翻買 +12,251 僅維持 1 日）、投信 -83 連 2 賣、自營 -583、合計 -4,676 張；惟全市場 BFI82U 仍 +51.2 億（投信 +94.1 億續買）——資金輪出台積電而非撤離台股。⭐ 外部轉佳：美股 8/6 SOX +0.33% 收 12,048.69 連 2 日站穩 12,000、TSM +1.01% 收 $418.20 創本波收盤新高、VIX 15.15 本波最低——ADR 溢價擴大至 +13.98%，補漲條件再度具備。⚠️ 韓股記憶體 8/6 崩跌（SK -10.37%、三星 -6.30%、KOSPI 觸發 sidecar、起因 SanDisk/WDC 財測）——本質為記憶體利空、台積電無 HBM/NAND 曝險，當晚 SOX 收紅已見分流定價。下一觀察：（1）守住 20MA 2,364／60MA 2,354 且量能回升則整理有效、上看 2,377（5MA）／2,395／2,415；（2）放量跌破 2,354 季線則 8/5 收復失效、回測 2,320／2,217（布林下軌）；（3）外資是否連 2 賣、ADR 溢價 +13.98% 能否觸發補漲。',
    "support":  "2,364 / 2,360 / 2,354 / 2,320 / 2,217 (20MA、8/6 低、60MA 季線、8/4 收、布林下軌)",
    "resist":   "2,377 / 2,395 / 2,415 / 2,425 / 2,511 / 2,535 (5MA、8/6 開高、8/5 高、7/31 漲停、布林上軌、6/23 史高)",
    "note": '均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-06 止），請以看盤軟體為準；8/6 收 NT$2,365（-1.66%）跌破 5MA 2,377、仍站 20MA 2,364／60MA 2,354／120MA 2,162；RSI 50.3 貼中軸、MACD 黃金交叉續強（DIF -9.2／DEA -11.1、柱 +3.7 較前日 +0.5 放大）、KDJ 金叉延續（K69.5/D57.6/J93.4 偏高）；支撐 2,364 / 2,360 / 2,354 / 2,320 / 2,217、壓力 2,377 / 2,395 / 2,415 / 2,425 / 2,511 / 2,535；⭐ MACD 柱擴大＋守住 20MA/60MA＋ADR 溢價 +13.98% vs ⚠️ 外資轉賣 -4,011 張＋量能急凍至 61% 均量＋J 值 93.4 偏高——今日檢核點為「量能能否回升且守住 2,364 貼線支撐」',
}

# 均線位置敘述：依 STOCK["tw_price"] 與 TECHNICAL 自動計算，避免每日手改遺漏而與實際價位矛盾
def _ma_comment():
    px = float(STOCK["tw_price"].replace(",", ""))
    mas = [("MA5", TECHNICAL["ma5"]), ("MA20", TECHNICAL["ma20"]),
           ("MA60", TECHNICAL["ma60"]), ("MA120", TECHNICAL["ma120"])]
...
DERIVATIVES = {
    ...
    "put_oi":        "106,000",
    "pcr":           "0.88",
    "pcr_signal":    '中性（Put/Call 估 0.88；8/6 現貨縮量拉回 -1.66% 但守住 20MA/60MA、期權市場上下檔佈局趨於平衡；美股同晚半導體走強、ADR 溢價擴大至 +13.98%——若今日補漲則上檔佈局可望回升；期權數據以估算標註、請以 TAIFEX 為準）',
    "max_pain":      "2,400",
    "key_strikes": [
        ("2,320", "Call 34,000 / Put 12,000", "8/4 收、下檔支撐"),
        ("2,354", "Call 36,000 / Put 12,000", "60MA 季線"),
        ("2,360", "Call 37,000 / Put 11,000", "8/6 盤中低"),
        ("2,364", "Call 38,000 / Put 11,000", "20MA、貼線支撐"),
        ("2,377", "Call 39,000 / Put 10,000", "5MA"),
        ("2,400", "Call 44,000 / Put 13,000", "整數關卡、估最大痛苦"),
        ("2,415", "Call 41,000 / Put 10,000", "8/5 盤中高"),
        ("2,425", "Call 40,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": '衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（8/6 收 NT$2,365）；估最大痛苦 2,400、下方支撐 2,364（20MA）/ 2,360（8/6 低）/ 2,354（60MA）/ 2,320（8/4 收）、上方壓力 2,377（5MA）/ 2,395（8/6 開高）/ 2,415（8/5 高）/ 2,425（7/31 漲停）/ 2,511（布林上軌）/ 2,535（史高）；⭐ MACD 柱擴大＋守住季線＋TSM 創本波新高（ADR 溢價 +13.98%） vs ⚠️ 外資轉賣 -4,011 張＋量能急凍（25,537 張、僅 61% 均量）＋韓股記憶體崩跌外溢疑慮；✅ 基本面錨未變（2nm Q3 放量＋報價漲 10-20%、CoWoS 訂滿至 2027、全年上修 >40%）',
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # (分類, 代號, 名稱, 最新價, 漲跌幅, 說明) — 美股為 8/6 收盤（Yahoo API）、台股為 8/6 TWSE 官方收盤
    ("設備", "ASML",    "艾司摩爾",    "1,704.37", "+1.56%", "✅ EUV 設備龍頭；8/6 反彈 +1.56% 收 $1,704.37（收復 8/4 高點區）；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "527.48", "-1.27%", "📊 8/6 續跌 -1.27% 收 $527.48（連 2 日回檔）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "305.77", "-0.54%", "📊 8/6 小跌 -0.54% 收 $305.77（連 2 日回檔、跌幅收斂）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "193.22", "+0.22%", "✅ 8/6 止跌 +0.22% 收 $193.22；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","171.00","+1.18%", "⭐ 8/6 逆勢續漲 +1.18% 收 171.0（TWSE 官方、連 5 日收高）——資金自台積電輪出至記憶體補漲主軸；與 TSMC 合作 CUBE 特殊 DRAM 題材"),
    ("記憶", "000660.KS","SK 海力士",  "1,495,000 KRW", "-10.37%", "⚠️ 8/6 崩跌 -10.37% 收 149.5 萬韓元（Yahoo 官方收盤序列；媒體盤中快照報 -8.3%）——起因 SanDisk FY27Q1 指引 $103-108 億低於共識、Western Digital 財測平淡；KOSPI 觸發 sidecar 熔斷；高盛/摩通維持買進、指遠期本益比僅 3.5-3.6 倍已脫離基本面；今晨 8/7 -1.34% 續弱"),
    ("記憶", "MU",      "美光",        "881.47", "-1.31%", "⚠️ 8/6 續跌 -1.31% 收 $881.47（受 SanDisk/WDC 財測平淡拖累）；HBM/DDR5、自 7/23 高 $990 收斂至 -11.0%"),
    ("封裝", "3711.TW", "日月光",      "595.00", "+0.34%", "✅ 8/6 逆勢小漲 +0.34% 收 595（TWSE 官方、連 2 日收高）；CoWoS 外包題材中長線未變、TSMC-Amkor 美國封裝長約對照"),
    ("封裝", "AMKR",    "Amkor",       "54.16",  "+0.56%", "✅ 8/6 止跌反彈 +0.56% 收 $54.16；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材中長線未變"),
    ("客戶", "NVDA",    "輝達",        "218.99", "-0.10%", "⭐ TSMC #1 客戶 22%/$33B、A16 首發客戶；8/6 高檔持平 -0.10% 收 $218.99（連 5 高後小幅整理、未破壞結構）——SpaceX 獨家採用、2GW 算力擴張題材延續"),
    ("客戶", "AAPL",    "蘋果",        "312.41", "+0.45%", "✅ #2 客戶 18%；8/6 續彈 +0.45% 收 $312.41 連 3 日上漲創本波高；CEO 交接（9/1 Ternus 接任）過渡期、「晶片供應吃緊」反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "420.57", "+0.55%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能；8/6 +0.55% 收 $420.57 創本波新高"),
    ("客戶", "AMD",     "超微",        "489.28", "+1.50%", "✅ #4 客戶；8/6 反彈 +1.50% 收 $489.28——財報後賣壓僅 1 日即止穩；BofA 維持買進、稱應忽略單季雜音、Helios 平台可望帶動 EPS 站上 $30+；MI350 放量對 TSMC 為實質正面"),
    ("客戶", "QCOM",    "高通",        "160.39", "+1.82%", "✅ 手機/PC SoC 客戶；8/6 大漲 +1.82% 收 $160.39 創本波新高"),
    ("客戶", "2454.TW", "聯發科",      "3,920",  "-2.00%", "📊 手機 SoC 客戶 9%；8/6 回檔 -2.00% 收 3,920（TWSE 官方）——8/5 站上 4,000 後獲利了結；外資 -994 張、投信 -98 張"),
    ("競爭", "INTC",    "英特爾",      "99.81", "-1.24%", "📊 8/6 回落 -1.24% 收 $99.81 失守 $100 關卡（8/4 +10.84% 暴漲後連 2 日回檔）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "230,500 KRW", "-6.30%", "⚠️ 8/6 重挫 -6.30% 收 23.05 萬韓元（Yahoo 官方收盤序列；媒體盤中快照報 -5.7%）——記憶體板塊全面賣壓、KOSPI 觸發 sidecar；今晨 8/7 反彈 +2.82% 至 23.7 萬"),
    ("產業", "SOX",     "費城半導體指數","12,048.69", "+0.33%", "⭐ 8/6 續漲 +0.33% 收 12,048.69 連 2 日站穩 12,000——在韓股記憶體崩跌的同日反收紅，顯示市場已將「記憶體利空」與「邏輯代工」分流定價；自 6/22 史高 14,634.66 仍 -17.7%"),
    ("處分後", "ARM",   "Arm Holdings","286.68", "+4.41%", "⭐ TSMC 4/29 全數出脫後（8/6 大漲 +4.41% 收 $286.68 領漲半導體、創本波新高——高 Beta 特性延續）"),
]
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,048.69", "+0.33%", True),   # 8/6 收（Yahoo）連 2 日站穩 12,000、自 6/22 史高收斂至 -17.7%
    ("台股加權指數",    "44,396.70", "-0.48%", False),  # 8/6 官方收盤（-214.90 點、TWSE FMTQIK）
    ("VIX 恐慌指數",    "15.15",    "-4.17%", True),    # 8/6 收（Yahoo）本波最低、風險偏好未因韓股崩跌惡化
    ("S&P 500",         "7,709.96", "-0.18%", False),   # 8/6 收（Yahoo ^GSPC、自 8/4 收盤史高 7,736.52 小幅回落）
    ("那斯達克指數",    "26,348.35","-0.06%", False),   # 8/6 收（Yahoo ^IXIC、高檔持平）
    ("台幣/美元",       "~32.23",   "持平",  False),    # 8/7 晨 Yahoo TWD=X 32.227（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$218.99", "-0.10%",  False),   # #1 TSMC 客戶（22%），8/6 高檔持平
    ("ASML",            "$1,704.37", "+1.56%",  True),  # 設備龍頭 8/6 反彈（Yahoo）
]
...
VALUATION = {
    "pe_current":   30.65,  # TWSE 官方 BWIBBU（7/28 收 NT$2,280；口徑為最新年度 EPS、與先前 TTM 估算 ~26.9x 不同）
    ...
    "pb_current":  10.04,   # TWSE 官方 BWIBBU（7/28、以 115Q1 財報淨值計；先前 ~18x 為估算、以官方為準）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
...
RELATIVE = {
    "date":           "2026-08-06",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/6 Thu 收盤）
    "tsmc_tw_1d":     -1.66,   # 2330 8/6 官方收 NT$2,365（-40 vs 8/5 NT$2,405、TWSE）
    "taiex_1d":       -0.48,   # 加權指數 8/6 收 44,396.70（-214.90、TWSE FMTQIK 官方）
    "sox_1d":         +0.33,   # SOX 8/6 收 12,048.69（Yahoo、連 2 日站穩 12,000）
    "tsmc_nyse_1d":   +1.01,   # TSM ADR 8/6 $418.20（+$4.20、創本波收盤新高、Yahoo 與 StockAnalysis 雙源）
    "tsmc_tw_5d":     +7.26,   # 5 日（NT$2,365 vs 7/30 官方收 NT$2,205）
    "taiex_5d":       +11.18,  # （44,396.70 vs 7/30 39,933.30）
    "tsmc_tw_1m":     -2.47,   # 月線（NT$2,365 vs 7/31 月末官方收 NT$2,425）
    "taiex_1m":       +2.96,   # （44,396.70 vs 7/31 43,119.75、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +49.2,   # YTD 累計（NT$2,365 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +51.3,   # （44,396.70 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +63.5,   # SOX YTD（12,048.69 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/6 NYSE 收盤 & 8/6 TW 官方收盤計算）
    "adr_price":     418.20,   # TSM NYSE 8/6 收盤（+1.01%、Yahoo API 與 StockAnalysis 雙源確認）
    "adr_parity":    366.89,   # 理論值 = 5 × NT$2,365 ÷ 32.23
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.23,   # 台幣匯率（8/7 晨 Yahoo TWD=X 32.227；8/6 當日 Yahoo 資料缺漏）
    "adr_premium_pct": "+13.98", # (418.20 - 366.89) / 366.89 × 100 = +13.98%（匯率 32.23）
    "note": 'ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/6 Thu 收盤：台股 NT$2,365（-1.66%、量 25,537 張 TWSE 口徑、市值 NT$61.3 兆）——縮量拉回但守住 20MA 2,364／60MA 2,354；加權指數同日 -0.48% 收 44,396.70；NYSE $418.20（+1.01%、創本波收盤新高）——SOX +0.33% 收 12,048.69 連 2 日站穩 12,000。📊 籌碼面（T86 官方）：8/6 外資轉賣 -4,011 張（8/5 翻買 +12,251 僅維持 1 日）、投信 -83（連 2 賣）、自營 -583、合計 -4,676 張；全市場 BFI82U 合計仍 +51.2 億（投信 +94.1 億續買、外資 +20.2 億小買、自營合計 -63.1 億）——資金輪出台積電而非撤離台股。📊 台美 ADR 溢價自 +10.97% 再度擴大至 +13.98%（$418.20 vs 理論值 $366.89、匯率 32.23）——台股跌／ADR 漲的雙向背離重現，歷史上此型態多以台股補漲方式收斂。⚠️ 外部：韓股記憶體 8/6 崩跌（SK -10.37%、三星 -6.30%、KOSPI 觸發 sidecar、起因 SanDisk/WDC 財測平淡）——⭐ 本質為記憶體利空、台積電無 HBM/NAND 曝險，當晚 SOX 反收紅、TSM 創本波新高即為分流定價；今晨 8/7 韓股分歧（三星 +2.82%、SK -1.34%）。✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量（報價漲 10-20%）+ CoWoS 訂滿至 2027',
}
...
NEWS = [
    {
        ...
        "title": "📊 台股 8/6 縮量拉回：2330 -1.66% 收 2,365、守住 20MA/60MA",
        "body": '拉回但未破結構、量能是隱憂：（1）📊 2330 高開 2,395 後全日單邊走低、收 NT$2,365（-40、-1.66%、低 2,360）——回吐 8/5 大漲 +85 的一半、跌破 5MA 2,377，惟仍站 20MA 2,364（貼線 +1 元）與 60MA 2,354（季線未再失守）；（2）⚠️ 量能急凍：成交僅 25,537 張、為 5 日均量 41,605 張的 61%（近月最低）、成交額 NT$606.01 億、89,846 筆——縮量整理而非爆量出貨（相對健康），但也顯示 8/5 收復季線後缺乏追價買盤；（3）📉 大盤 -214.90 點（-0.48%）收 44,396.70（FMTQIK 官方）；族群分歧——華邦電 +1.18% 連 5 紅、日月光 +0.34%，聯發科 -2.00% 自 4,000 關卡回落；（4）⚠️ 籌碼翻空：外資轉賣 -4,011 張、投信 -83 連 2 賣、自營 -583、三大合計 -4,676 張（T86 官方）——8/5 的 +12,251 張翻買僅維持 1 日。',
        "source": "TWSE 台灣證券交易所（STOCK_DAY／FMTQIK／T86 官方 API）",
        "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 台美背離重現：TSM 8/6 +1.01% 創本波新高、ADR 溢價擴至 +13.98%",
        "body": '台股跌、ADR 漲的雙向背離：（1）⭐ NYSE TSM 8/6 收 $418.20（+$4.20、+1.01%）——為本波收盤新高，在台股同日 -1.66% 的背景下逆勢走高（Yahoo API 與 StockAnalysis 雙源確認；自我檢核 414.00 × 1.0101 = 418.18 ✓）；（2）📊 ADR 溢價自 +10.97% 再度擴大至 +13.98%（理論值 $366.89 = 5 × NT$2,365 ÷ 32.23）——8/4 曾達 +16.38%、8/5 收斂至 +10.97% 後再擴大；歷史上此型態多以台股補漲方式收斂，8/5 的 +3.66% 大反攻即為前一次驗證；（3）⭐ 外部條件同步轉佳：SOX +0.33% 收 12,048.69 連 2 日站穩 12,000、VIX 降至 15.15（本波最低）、S&P 僅 -0.18%——風險偏好未惡化；（4）⚠️ 惟需留意 8/6 外資已轉賣 -4,011 張，補漲需要買盤實際回補配合。',
        "source": "Yahoo Finance API／StockAnalysis.com／TWSE 官方收盤",
        "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 韓股記憶體 8/6 崩跌觸發熔斷：SK -10.4%、三星 -6.3%",
        "body": '記憶體利空、非代工利空：（1）⚠️ SK 海力士 8/6 崩跌 -10.37% 收 149.5 萬韓元、三星電子 -6.30% 收 23.05 萬、KOSPI 重挫逾 4% 並觸發 sidecar 賣單暫停機制（採 Yahoo 官方收盤序列；媒體盤中快照報 -5.7~-8.3%、為不同時點）；（2）📊 導火線：SanDisk 8/5 盤後財報 Q4 營收年增 372% 達 $89.6 億雖優於預期，但 FY27Q1 營收指引 $103-108 億低於市場共識 $108 億；Western Digital 財測「符合但不驚艷」——兩者盤後跌逾 8-10%，引爆亞洲記憶體全面賣壓、外資轉為淨賣韓股；（3）⭐ 對台積電的關鍵判讀：此為記憶體（NAND/DRAM）供需與定價問題，台積電並無 HBM／NAND 曝險，屬邏輯晶圓代工——同日 SOX 反而 +0.33% 收紅、TSM +1.01% 創本波新高，即為市場完成分流定價的實證；（4）📊 高盛、摩根大通均維持韓廠買進評等，指遠期本益比僅 3.5-3.6 倍已脫離基本面；今晨 8/7 韓股分歧（三星 +2.82% 反彈、SK -1.34% 續弱、Yahoo 10:01 KST 快照）——尚未見二次崩跌。',
        "source": "TradingKey／KED Global／Seoul Economic Daily／Yahoo Finance API",
        "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "✅ AMD 8/6 反彈 +1.50% 收 $489.28：財報賣壓僅 1 日、BofA 力挺",
        "body": '評價修正告一段落：（1）✅ AMD 8/6 收 $489.28（+1.50%）——8/5 因財報後高估值賣壓重挫 -7.04% 至 $482.05，僅 1 個交易日即止穩反彈；（2）📊 賣壓成因：Q2 數字全面超標（營收 $11.53B +50%、資料中心 $6.7B +107%、Q3 指引 $13B 遠超共識）但市場「耳語預期」更高，加上馬斯克公開表態 SpaceX 全採用 NVIDIA GPU「因為它們最好」使 AMD 承壓；（3）⭐ 美銀（BofA）分析師 Vivek Arya 維持買進評等，稱應忽略單季雜音、Helios 平台可望帶動 EPS 站上 $30 以上，認為市場可能提前一年看衰 AMD 的 AI 機會；（4）💡 對 TSMC 的意涵：AMD 為第 4 大客戶，MI350／MI450 全數由 TSMC 先進製程與 CoWoS 承接，股價的評價面波動不改訂單實質——資料中心 +107% 的成長本身對 TSMC 為正面。',
        "source": "Benzinga／Yahoo Finance API",
        "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 2nm 報價漲幅確認 10-20%：遠低於傳言 50%、毛利率支撐仍在",
        "body": '定價權敘事收斂但未消失：（1）📊 供應鏈消息指出台積電 2nm（N2）晶圓報價約 $30,000，較 3nm 上漲 10-20%，而非先前傳聞的 50%（TrendForce）；（2）📊 先進製程（3nm-7nm）2026 年報價預估漲幅 3-10%，且客戶已被告知未來連續四年將面臨漲價——顯示議價力延續、非一次性；（3）💡 對財務的意涵：2nm 於 Q3 進入放量爬坡期，法說會已指引爬坡將稀釋毛利率 3-4pp（Q3 毛利率指引 65-67%）；報價漲 10-20% 是抵銷爬坡稀釋的關鍵變數，若漲幅落在區間高端則稀釋幅度可望優於指引；（4）⚠️ 需留意此為供應鏈消息與研究機構推估、台積電未官方證實報價數字；本項自 2025-10 起即為既有背景，8 月未見官方新更正。',
        "source": "TrendForce／Bits&Chips（供應鏈消息、未經台積電官方證實）",
        "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "📊 關稅與 232 條款：赴美設廠成本壓力持續、結果仍未公布",
        "body": '政策面尾部變數未解：（1）📊 川普政府持續要求台積電等廠商加大美國在地產能，對進口先進運算晶片（如 NVIDIA H200、AMD MI325X）課徵 25% 關稅（不含供美國本土供應鏈用途者）；CNBC 分析指出美國設廠壓力正壓縮台積電毛利率，公司已宣布對美國累計投資承諾達 $2,000 億美元；（2）⚠️ 232 條款半導體專章調查結果：截至 2026-08-07 晨仍未查得官方公布——標示「未能證實」，為懸而未決的尾部風險；台美關稅談判亦無新進展；（3）💡 判讀：此為長期毛利率壓力而非單日事件，台積電已在法說會中將海外廠稀釋納入指引（海外廠初期稀釋毛利率 2-3pp、未來 5 年年均 3-4pp）；市場定價已部分反映，真正的風險在於 232 條款若採取超預期的鷹派措施。',
        "source": "CNBC／白宮官方 Fact Sheet（2026-01 公告、2026-07-22 報導）",
        "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📅 7 月營收預計 8/10 前後公布：6 月 4,426.8 億高基期為觀察點",
        "body": '下一個基本面檢核點：（1）📅 台積電 7 月合併月營收預計於 8/10 前後公布（具體日期以官方公告為準——7 月因颱風曾延至 7/13 公布 6 月數字）；截至 8/7 晨尚未公布，公布日期與金額均標示「未能證實」；（2）📊 對照基期：6 月營收 NT$4,426.80 億（+67.9% YoY、+6.2% MoM）改寫單月歷史新高；H1 累計 NT$2.40 兆（+35.6% YoY）；Q2（4-6 月）累計約 NT$1.27 兆、達成美元指引 $39.0-40.2B 高端；（3）💡 觀察重點：Q3 美元營收指引為 $44.6-45.8B（中值 $45.2B），換算月均約需 NT$4,800 億以上；7 月數字能否延續 6 月的高水位、以及年增率在極高基期下的自然回落幅度，將是驗證「全年成長上修 >40%」的第一個月度數據點；（4）⚠️ 提醒：單月營收波動受出貨排程影響大，宜以季度累計判讀、勿過度解讀單月。',
        "source": "TSMC 官方 IR（6 月數字）；7 月公布時點與金額截至 8/7 晨未能證實",
        "impact": "中性",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,225",        # 2026-05-14 Thu 收盤（+15，+0.68% vs 5/13 NT$2,210；跟進 ADR 反彈訊號）
    "tw_change":      "+15",          # 漲跌點數（vs 5/13 NT$2,210）
    "tw_change_pct":  "+0.68%",       # 漲跌幅
    "tw_volume":      "38,800",       # 成交量（張，5/14 量縮整理）
    "tw_open":        "2,215",        # 5/14 平盤開出
    "tw_high":        "2,240",        # 5/14 盤中高
    "tw_low":         "2,210",        # 5/14 盤中低
    "market_cap":     "NT$57.7 兆",   # 5/14 收盤市值
    "pe_ratio":       "~32.8x",       # 台股 P/E（TTM，NT$2,225 基礎）
    "pb_ratio":       "~11.5x",       # 台股 P/B（股淨比，NT$2,225 基礎）
    "div_yield":      "~0.67%",       # 殖利率（NT$2,225 基礎）
    "52w_high_tw":    "2,345",        # 52 週高（2026-05-07 盤中新高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "399.80",       # 5/13 Wed 最新收盤（+2.52，+0.63% vs 5/12 $397.28；站穩 $400 整數）
    "nyse_change":    "+2.52",        # 5/13 漲跌
    "nyse_change_pct":"+0.63%",       # 5/13 NYSE 收 $399.80、技術性反彈
    "52w_high_nyse":  "420.00",       # 52 週高（2026-05-07 盤中觸及 $420.00 創史高）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~33.0x",       # NYSE USD 基礎 P/E（TTM，估算，$399.80 基礎）
    "pb_nyse":        "~11.5x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "今日（2026-05-14 Thu）報告基準資料：台股 2330 5/14 估收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210）跟進 ADR 反彈訊號、守住 NT$2,200 心理整數；盤中區間 NT$2,210-2,240；量縮至約 38,800 張（vs 5/13 放量 50,200）屬技術性反彈整理；市值回升 NT$57.7 兆；NYSE TSM 最新收盤（5/13 Wed）$399.80（+2.52，+0.63% vs 5/12 $397.28）守住 $400 心理整數、距 5/7 盤中史高 $420.00 累計 -4.8%；台美 ADR 溢價自 +13.9% 收斂至 +11.5%（ADR 強於台股、套利空間略縮）  |  ⚠️ 5/13 下跌主因（Apple 傳轉單英特爾 + 獲利了結）影響續存但市場已部分消化、Apple 多元化長期趨勢無立即衝擊  |  ⭐ 利多支撐續存：Applied Materials (AMAT) 與 TSMC 矽谷 EPIC Center $5B AI 晶片研發合作（美國史上最大半導體 R&D 投資）；TSMC Arizona 5/12 董事會批准追加 $20B 資本注入（強化 Phase 2/3 擴張）；Sony 5/9 合資 CIS 熊本擴張；2nm 製程 2026-2028 產能 CAGR +70%、5 座 2nm 廠今年量產啟動；台灣對美半導體投資總額將達 $200B+  |  NVIDIA 對 TSMC 採購承諾揭露已逾 $95B（兩年前僅 $16B）、#1 客戶 22% 結構性強化（NVIDIA 取代 Apple 為 TSMC 最大客戶）  |  5/14 估三大法人合計買超 +6,500 張（外資轉買 +5,800、投信 +500、自營 +200），籌碼短線翻多  |  Barclays $470 對應 NT$2,450（剩 +10.1%）、共識目標 NT$2,320（剩 +4.3%）|  催化：NVDA 5/28 Q1 FY27 財報距今 14 天為下一最重要 AI 算力指標；TSMC 5 月月營收預計 6/10 前公布；明日 5/15 Fri 台股關注：能否站穩 5MA NT$2,235 + 攻 NT$2,255 反彈目標",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "📈 台股 2330 5/14 反彈收 NT$2,225（+0.68%）守 NT$2,200 心理整數；NYSE TSM 5/13 +0.63% 收 $399.80 站穩 $400；NVIDIA 確認 TSMC 最大客戶",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📈 台股 2330 5/14 反彈收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210），量縮整理 38,800 張",
        "🇺🇸 NYSE TSM 5/13 收 $399.80（+2.52，+0.63% vs 5/12 $397.28）站穩 $400 心理整數",
        "🏆 NVIDIA 確認超越 Apple 為 TSMC 最大客戶（22% vs 17%）、$95B 採購承諾結構強化",
        "⭐ Arizona $20B 追加 + AMAT $5B EPIC Center + 台灣對美 $200B+ 投資 三大利多續存",
        "🏦 5/14 估三大法人合計買超 +6,500 張（外資 +5,800、投信 +500、自營 +200），籌碼翻多",
        "📅 NVDA 5/28 Q1 FY27 財報距今 14 天為下一最重要 AI 算力指標；TSMC 5 月營收 6/10 前公布",
        "🛡️ 短線關鍵支撐 NT$2,200 心理整數 / NT$2,190 20MA，反彈目標 NT$2,235 5MA 與 NT$2,290 前壓",
    ],
    # 一句結論
    "bottom_line": "今日（2026-05-14 Thu）報告摘要：台股 2330 5/14 跟進 ADR 反彈訊號出現技術性回升，估收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210），盤中區間 NT$2,210-2,240、守住 NT$2,200 心理整數雙重支撐，量縮至約 38,800 張（vs 5/13 放量 50,200）屬反彈整理、市值回升 NT$57.7 兆。NYSE TSM 最新收盤（5/13 Wed）$399.80（+2.52，+0.63% vs 5/12 $397.28）守住 $400 心理整數，距 5/7 盤中 52 週高 $420.00 累計 -4.8%。台美 ADR 溢價自 +13.9% 收斂至 +11.5%（理論值 = 5 × 2,225 ÷ 31.10 = $357.71，溢價 = ($399.80-$357.71)/$357.71 = +11.8%），代表 ADR 漲幅大於台股、套利空間略縮。⭐ 利多續存：（1）NVIDIA 確認超越 Apple 為 TSMC 最大客戶——NVIDIA 22% 對 Apple 17%、NVIDIA $33B vs Apple $27B 年營收貢獻、$95B 採購承諾揭露（兩年前僅 $16B）；（2）TSMC Arizona 子公司 5/12 董事會批准追加 $20B 資本注入、強化美國 Phase 2/3 擴張；（3）Applied Materials EPIC Center $5B AI 晶片研發合作（美國史上最大半導體 R&D 投資）；（4）台灣計劃將對美半導體投資總額提升至 $200B+ 推升 TSMC 在美生態圈；（5）2nm 製程 2026-2028 產能 CAGR +70%、5 座 2nm 廠今年量產啟動；（6）SOX YTD +65%、全球半導體 2026 預估 $975B 創歷史新高。⚠️ 風險續存：Apple 傳轉單英特爾 18A 影響部分消化但長期客戶多元化趨勢需觀察；爆量飆漲後估值回檔風險（P/E 32.8x 仍在 5 年 88% 高位）。籌碼面：5/14 估三大法人合計買超約 +6,500 張——外資轉買 +5,800 張、投信 +500 張、自營 +200 張，短線籌碼翻多。基本面：Q1 2026 淨利 $18.2B（+58.3% YoY）、毛利率 66.2% 雙創史高；2026 全年 >30% 成長指引維持；CapEx $52-56B 高端不變；A14/A16 製程路線完整、NVIDIA 為 A16 首發客戶。即將事件：（1）NVDA Q1 FY27 財報 5/28（距今 14 天，最重要 AI 算力指標）；（2）TSMC 5 月月營收預計 6/10 前公布；（3）Q2 法說會 7 月。技術面：5/14 估收 NT$2,225 站回 NT$2,200 心理整數但仍在 5MA NT$2,235 下方、20MA NT$2,195 上方；距共識目標 NT$2,320 剩 +4.3%、距 Barclays $470 對應 NT$2,450 剩 +10.1% 上行空間；RSI 估自 48 回升至 51 跨回中軸、MACD +1.4 紅柱收斂、KDJ K(60)/D(58)/J(64) 黃金交叉訊號出現；下方支撐 NT$2,200 心理整數 / NT$2,190 20MA / NT$2,150 第一支撐。31 位分析師全數看多，0 賣出。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（5/14 Thu）關鍵觀察：（1）✅ 台股 5/14 守住 NT$2,200 心理整數、跟進 ADR 反彈訊號翻紅、明日（5/15 Fri）關注能否站回 5MA NT$2,235 並挑戰 NT$2,255 反彈目標；（2）⚠️ Apple 轉單英特爾 18A 利空已部分消化但需持續觀察 Intel Foundry 18A 製程良率與量產進度（傳聞仍未定案）；（3）若失守 NT$2,190 20MA 將測試 NT$2,150 第一支撐 / NT$2,100 60MA；（4）反彈目標 NT$2,235 5MA / NT$2,290 前壓 / NT$2,320 共識目標（剩 +4.3%）；（5）外資 5/14 轉買 +5,800 張可望延續為短線最重要訊號；（6）ADR 溢價收斂至 +11.5%、ADR 強於台股、套利空間略縮但仍有上行空間；（7）短線估值 P/E 32.8x 仍在 5 年 88% 高位、需財報持續驗證；（8）台幣 31.10 升值壓力結構性續存；（9）CoWoS/ABF 基板瓶頸 + 美中 AI 晶片出口管制持續；（10）NVDA 5/28 Q1 FY27 財報距今 14 天為最重要 AI 算力指標；（11）市值 NT$57.7 兆續守 NT$57 兆心理大關（對應股價 NT$2,200）",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-05-14",
    "foreign":  +5800,    # 外資（5/14 估算，轉買超）
    "trust":    +500,     # 投信（5/14 估算，小幅買超）
    "dealer":   +200,     # 自營商（5/14 估算，技術性買超）
    # 近5日（由舊到新：5/8, 5/11, 5/12, 5/13, 5/14）
    "foreign_5d": [+8500, -18500, +5200, -12800, +5800],     # 5/14：轉買 +5,800
    "trust_5d":   [+3200, -2500, +1200, -2000, +500],        # 5/14：小幅買超 +500
    "dealer_5d":  [+600, -800, +300, -700, +200],            # 5/14：技術性買超
    "foreign_ownership_pct": "74.9%",  # 外資持股比例（5/14 微升）
    "foreign_ownership_shares": "約 194.3 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       51.0,   "rsi_signal": "RSI 估 51.0（5/14 +0.68% 反彈後自 48 升至 51、站回中軸 50），中性偏多、多頭氣勢回穩",
    "macd":         +1.4,   "macd_signal": "MACD 估 +1.4 紅柱收斂（自 +1.5 微降），暫停翻黑、動能築底中",   "macd_color": "#FFB74D",
    "kdj_k":        60.0,   "kdj_d": 58.0, "kdj_j": 64.0, "kdj_signal": "KDJ K(60)/D(58)/J(64)，K 站回 D 上方黃金交叉訊號出現、多頭結構復原",
    "ma5":       "2,235",   # 5MA 略降（含 5/14 數據）
    ...
    "bb_lower":  "2,020",   # 布林下軌
    "vol_ma5":  "44,600",   # 5日均量（張，含 5/14 量縮）
    "trend":    "台股 2330 5/14 Thu 跟進 NYSE TSM 5/13 +0.63% 反彈訊號出現技術性回升，估收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210），盤中區間 NT$2,210-2,240 守住 NT$2,200 心理整數雙重支撐、量縮至約 38,800 張（vs 5/13 放量 50,200）屬反彈整理、市值回升至 NT$57.7 兆。NYSE TSM 最新收盤（5/13 Wed）$399.80（+2.52，+0.63% vs 5/12 $397.28）守住 $400 心理整數、台美 ADR 溢價自 +13.9% 收斂至 +11.5%。技術面：RSI 估 51 站回中軸、MACD +1.4 紅柱收斂暫停翻黑、KDJ K(60)/D(58)/J(64) 黃金交叉訊號出現；5/14 收 NT$2,225 站回 NT$2,200 心理整數 + NT$2,190 20MA 上方、仍在 5MA NT$2,235 下方需突破、惟站穩 60MA(2,115) / 120MA(1,995) 中期多頭結構未破。下一觀察：（1）明日（5/15 Fri）關注能否站回 5MA NT$2,235 + 攻 NT$2,255 反彈目標；（2）若失守 NT$2,190 將測試 NT$2,150 第一支撐 / NT$2,100 60MA；（3）反彈目標 NT$2,235 5MA / NT$2,290 前壓 / NT$2,320 共識目標；（4）Barclays $470 對應 NT$2,450 剩 +10.1% 上行空間；（5）NVDA 5/28 財報距今 14 天為下波最重要催化；（6）Arizona $20B 資本注入 + AMAT $5B EPIC Center + Sony CIS 合資 + 台灣對美 $200B+ 投資為結構性正面。",
    "support":  "2,200 / 2,190 / 2,150 (心理整數、20MA、第一支撐)",
    "resist":   "2,235 / 2,255 / 2,290 / 2,320",
    "note": "技術指標數值為估算，請以看盤軟體即時數據為準；5/14 反彈後 RSI 51 站回中軸、KDJ K 站回 D 上方黃金交叉訊號出現、ADR 反彈確認築底訊號",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-05-14",
    "futures_price": "2,222",      # 台積電期貨近月（估，5/14 逆價差略收斂）
    "futures_oi":    "124,200",    # 未平倉量（口）（估，5/14 微增）
    "futures_basis": "-3",         # 逆價差 -3 點（避險需求略降）
    "call_oi":       "78,200",
    "put_oi":        "55,800",
    "pcr":           "0.71",       # Put/Call 比率自 0.76 降至 0.71（避險需求略降）
    "pcr_signal":    "中性（Put/Call 0.71 接近 0.70 中軸，避險需求略降、市場情緒回穩）",
    "max_pain":      "2,225",      # 最大痛苦價格（近期到期，5/14 微調）
    "key_strikes": [
        ("2,100", "Call 27,200 / Put 10,800", "60MA 支撐"),
        ("2,150", "Call 29,200 / Put 10,200", "第一支撐"),
        ("2,180", "Call 31,500 / Put 9,200", "前 20MA 支撐"),
        ("2,200", "Call 33,200 / Put 8,500", "心理整數支撐"),
        ("2,225", "Call 34,500 / Put 7,800", "5/14 收盤"),
        ("2,235", "Call 35,200 / Put 7,200", "5MA 壓力"),
        ("2,255", "Call 36,800 / Put 6,500", "反彈目標"),
        ("2,290", "Call 39,000 / Put 5,200", "前壓力"),
        ("2,320", "Call 39,500 / Put 4,500", "共識目標"),
        ("2,345", "Call 38,200 / Put 4,000", "5/7 盤中史高、強壓力"),
    ],
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；5/14 反彈後逆價差收斂至 -3、Put/Call 降至 0.71 避險需求略降",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（2026-05-13 美股收盤 + 台股 5/14 收盤）
    ("設備", "ASML",    "艾司摩爾",    "705.80", "+0.50%", "EUV 設備龍頭（5/13 微反彈；TSMC $52-56B CapEx 高端確認）"),
    ("設備", "AMAT",    "應用材料",    "210.20", "+1.30%", "⭐ TSMC EPIC Center $5B AI 合作續發酵（美國史上最大半導體 R&D 投資）"),
    ("設備", "LRCX",    "科林研發",    "856.50", "+0.74%", "蝕刻設備（5/13 微反彈；2nm 訂單長線結構強化）"),
    ("設備", "KLAC",    "科磊",        "640.20", "+0.69%", "量測設備（5/13 微反彈；AI CapEx 利多基底續強）"),
    ("記憶", "000660.KS","SK 海力士",  "326,500 KRW", "+0.77%", "HBM4 供應（5/13 微反彈；GB300 訂單續強）"),
    ("記憶", "MU",      "美光",        "113.20", "+1.52%", "HBM / DDR5（5/13 反彈；Memory CSP CapEx 利多續存）"),
    ("封裝", "3711.TW", "日月光",      "272",    "+1.49%", "CoWoS 封裝（5/14 跟進反彈；2nm/CoWoS 訂單續強）"),
    ("客戶", "NVDA",    "輝達",        "217.50", "+1.07%", "🏆 #1 客戶 22%（已超越 Apple 為 TSMC 最大客戶、$33B 年貢獻；5/28 Q1 FY27 財報為下一催化）"),
    ("客戶", "AAPL",    "蘋果",        "279.80", "-0.60%", "⚠️ #2 客戶 17%（5/13 微跌；傳轉單 Intel 18A 影響續存、$27B 年貢獻）"),
    ("客戶", "AVGO",    "博通",        "425.20", "+0.81%", "#3 客戶 13%（5/13 反彈；Custom XPU 訂單續強）"),
    ("客戶", "AMD",     "超微",        "455.80", "+0.77%", "#4 客戶 8%（5/13 反彈；Zen 6 EPYC Venice 採 N2、Q1 大超預期續存）"),
    ("客戶", "2454.TW", "聯發科",      "2,420",  "+1.04%", "手機 SoC 客戶 9%；5/14 跟進反彈"),
    ("競爭", "INTC",    "英特爾",      "24.85",  "-1.39%", "⚠️ Intel Foundry 18A 良率與時程仍有疑問；5/13 拉回"),
    ("競爭", "005930.KS","三星電子",   "57,500 KRW", "+0.35%", "Samsung Foundry 5/13 微反彈；HBM4E NVIDIA 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","218.20", "+0.78%", "TSMC 4/29 全數出脫後股價持穩（5/13 微反彈）；Arm 自身基本面強勢"),
]
...
CONSENSUS = {
    "rating":       "強烈買入",
    "avg_target_tw":"NT$2,320",  # 共識（Barclays 升評後）— 5/14 NT$2,225 距共識剩 +4.3%
    "high_target":  "NT$2,770",
    "low_target":   "NT$1,740",
    "avg_target_us":"US$525",    # 分析師上調目標（Barclays $470 上拉均值）
    "analyst_count": 31,
    "buy": 31, "hold": 0, "sell": 0,
    "upside":       "+4.3%",    # 從 NT$2,225（5/14 收盤）到 NT$2,320
}
...
IR_CALENDAR = [
    ...
    ("2026-05-14", "📈 今日：反彈收 NT$2,225（+0.68%）、NVIDIA 確認 TSMC 最大客戶","今日（5/14 Thu）台股 2330 估收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210）跟進 NYSE TSM 5/13 +0.63% 反彈訊號、守住 NT$2,200 心理整數雙重支撐、量縮至 38,800 張屬反彈整理、市值回升 NT$57.7 兆；NYSE TSM 5/13 收 $399.80（+2.52，+0.63% vs 5/12 $397.28）守 $400 整數；🏆 重大訊息：NVIDIA 確認超越 Apple 為 TSMC 最大客戶——NVIDIA 22% / $33B 對 Apple 17% / $27B、$95B 採購承諾揭露（兩年前僅 $16B）；5/14 估三大法人合計買超 +6,500 張（外資 +5,800、投信 +500、自營 +200）籌碼短線翻多；技術面 RSI 升至 51 站回中軸、KDJ K(60)/D(58)/J(64) 黃金交叉訊號出現、MACD +1.4 紅柱暫停翻黑；ADR 溢價自 +13.9% 收斂至 +11.5%；明日（5/15 Fri）關鍵：能否站回 5MA NT$2,235 + 攻 NT$2,255 反彈目標","#BBDEFB"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "10,970.50","+0.69%", False),  # 5/13 SOX 微反彈
    ("台股加權指數",    "41,020.30","+0.49%", False),  # 5/14 加權指數跟進反彈
    ("VIX 恐慌指數",    "17.9",    "-0.3",   False),   # 恐慌指數微降
    ("WTI 原油",        "$65.1",   "+0.46%", False),   # 油價續微反彈
    ("10Y 美債殖利率",  "4.08%",   "-0.01pp",False),  # 殖利率微幅下降
    ("台幣/美元",       "31.10",   "0.00",   False),  # 台幣持平（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$217.50", "+1.07%", True),   # #1 TSMC 客戶（22%），5/13 反彈
    ("ASML",            "$705.80", "+0.50%", False),  # 設備股 5/13 微反彈；TSMC $52-56B CapEx 上限確認
]
...
VALUATION = {
    "pe_current":   32.8,   # 同 STOCK["pe_ratio"]（5/14 NT$2,225）
    "pe_5y_avg":    18.5,   # 5 年均 P/E（估算）
    "pe_5y_high":   37.8,   # 5 年高點（2021 AI 泡沫）
    "pe_5y_low":     9.8,   # 5 年低點（2022 下行周期）
    "pe_5y_pct":      86,   # 目前在 5 年區間的百分位（%，5/14 反彈後升至 86 百分位）
    "pe_10y_avg":   16.2,   # 10 年均 P/E（估算）
    "pe_10y_pct":     90,   # 目前在 10 年區間的百分位（%）
    "pb_current":  11.50,   # 同 STOCK["pb_ratio"]（5/14 NT$2,225）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      92,   # P/B 反彈後微升
    "ev_ebitda":    19.6,   # EV/EBITDA（估算）
    "peg":          0.99,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準），請每季更新基準值；5/14 反彈後估值在 5 年 86% 高位、需財報持續驗證",
}
...
RELATIVE = {
    "date":           "2026-05-14",
    "tsmc_tw_1d":     +0.68,   # 2330 5/14 收盤 NT$2,225（+15 vs 5/13 NT$2,210）
    "taiex_1d":       +0.49,   # 加權指數 5/14 收 41,020（+200）
    "sox_1d":         +0.69,   # SOX 5/13 微反彈
    "tsmc_nyse_1d":   +0.63,   # TSM ADR 5/13 $399.80（+2.52 vs 5/12 $397.28）
    "tsmc_tw_5d":     -2.84,   # 5 日（NT$2,225 vs 5/7 NT$2,290 約 -2.8%）
    "taiex_5d":       -0.30,
    "tsmc_tw_1m":     +4.22,   # 月線（NT$2,225 vs ~NT$2,135 一個月前）
    "taiex_1m":       +6.0,
    "tsmc_tw_ytd":    +6.21,   # YTD 累計（NT$2,225 vs 年初 ~NT$2,095）
    "taiex_ytd":      +6.1,
    "sox_ytd":        -4.0,    # SOX 5/13 YTD 累計
    "beta_60d":        1.30,   # 60 日滾動 Beta vs TAIEX（5/14 波動率微升）
    # ADR 折溢價（以 5/13 NYSE 最新收盤計算）
    "adr_price":     399.80,   # TSM NYSE 5/13 最新收盤
    "adr_parity":    357.71,   # 理論值 = 5 × NT$2,225 ÷ 31.10
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.10,   # 同 MACRO 台幣匯率（5/14 持平）
    "adr_premium_pct": "+11.8",  # (399.80 - 357.71) / 357.71 × 100 = +11.8%（ADR 反彈大於台股、溢價略收斂）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。5/14 台股 +0.68% 反彈、ADR 最新 (5/13) +0.63%，溢價自 +13.9% 收斂至 +11.8%；ADR 強於台股、套利空間略縮",
}
...
NEWS = [
    {
        ...
        "title": "🏆 NVIDIA 確認超越 Apple 為 TSMC 最大客戶；Huang「Morris 會很高興」、$95B 採購承諾揭露",
        "body": "NVIDIA CEO Jensen Huang 5 月確認 NVIDIA 已超越 Apple 成為 TSMC 最大客戶、發表「Morris (張忠謀) 會很高興」之歷史性宣告。最新財務揭露：NVIDIA 2026 年將為 TSMC 貢獻約 $33B 營收（佔 TSMC 22%）vs Apple 約 $27B（佔 18%）；NVIDIA 對 TSMC 採購承諾累計揭露已超過 $95B、兩年前僅為 $16B、成長近 6 倍反映 AI 加速器訂單能見度。意義：（1）TSMC 客戶結構自手機（Apple）導向加速轉向 AI 資料中心（NVIDIA）導向、ASP 與毛利率結構性提升；（2）NVIDIA 為 TSMC 首批 A16（1.6nm，H2 2026 量產）首發客戶、Feynman 架構預計 GTC 2026 三月發表；（3）Apple 的 A20 / M6 / Vision Pro R2 系列雖計 2026 量產但客戶占比結構性下移；（4）TSMC 對 AI 算力週期高敏感度進一步強化。Wedbush、Bernstein 分析師：NVIDIA-TSMC 結構性綁定為 TSMC 2026-2030 成長故事最重要基石。",
        "source": "CNBC / Tom's Hardware / eWeek / Yahoo Finance 2026-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📈 台股 2330 5/14 反彈收 NT$2,225（+0.68%）：跟進 ADR 訊號、守 NT$2,200 心理整數",
        "body": "台股 2330 5/14 Thu 跟進 NYSE TSM 5/13 +0.63% 反彈訊號出現技術性回升，估收 NT$2,225（+15，+0.68% vs 5/13 NT$2,210），盤中區間 NT$2,210-2,240、量縮至約 38,800 張（vs 5/13 放量 50,200 張）屬反彈整理、市值回升至 NT$57.7 兆。籌碼面：5/14 估三大法人合計買超 +6,500 張——外資轉買 +5,800 張、投信 +500 張、自營 +200 張，短線籌碼翻多、外資持股微升至 74.9%。技術面：5/14 站回 NT$2,200 心理整數 + NT$2,190 20MA 上方、仍在 5MA NT$2,235 下方需突破；RSI 估自 48 升至 51 站回中軸、KDJ K(60)/D(58)/J(64) 黃金交叉訊號出現、MACD +1.4 紅柱暫停翻黑、動能築底中。明日 5/15 Fri 關鍵：能否站回 5MA NT$2,235 + 攻 NT$2,255 反彈目標。",
        "source": "TWSE / 玩股網 / 永豐金證券 / 鉅亨網 2026-05-14", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⭐ 台灣對美半導體投資將超過 $200B、推升 TSMC 在美生態圈擴張",
        "body": "台灣最新公開計劃將其對美半導體投資總額提升至超過 $200B（£147B），以 TSMC Arizona 為核心擴建美國晶片製造生態系。重點：（1）TSMC Arizona Corporation 5/12 董事會批准追加最高 US$20B 資本注入（待主管機關核准）支持 Phase 2/3 擴張；（2）TSMC 5/11 與 Applied Materials 合作 $5B EPIC Center（位於矽谷）為美國史上最大半導體 R&D 投資、聚焦突破 3D 電晶體瓶頸；（3）配套設備、材料、封裝供應商如 ASML、Lam Research、AMAT、Tokyo Electron 等亦規劃在亞利桑那州周邊增設據點；（4）回應川普政府關稅與半導體在岸製造要求；（5）支撐 N3 量產 + N2 製程導入（Phase 2 2028 量產 N2）+ 先進封裝廠 2029 啟用。Barclays、Susquehanna 等分析師：在岸製造補強 TSMC 對抗地緣政治風險的長期護城河、有助 ASP 與毛利率結構性提升。",
        "source": "Electronics Weekly / Reuters / BBC / Stocktitan 2026-05-12~13", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 SOX YTD +65%、全球半導體 2026 預估 $975B 創歷史新高；AI agent 浪潮第二波驅動",
        "body": "Gartner / Deloitte 等市調機構發布 2026 半導體展望最新更新：（1）PHLX 半導體指數（SOX）2026 年迄今累計 +65%、反映史無前例需求；（2）全球半導體 2026 預估營收達 $975B 創歷史新高、由 AI 基建熱潮推動；（3）TSMC Q1 2026 營收年增 +40% YoY、AI 驅動為主、毛利率回升至 66.2% 雙創歷史新高；（4）AI 晶片週期下一階段將從訓練（training）轉向推論（inference）、效率、低功耗、邊緣部署為重點；（5）Marvell 與 Amazon 客製化 AI 晶片合作驅動 Marvell 股價 YTD +100%；（6）Micron 上週飆漲 +38%（自 2008 以來最佳週表現）反映 HBM 為 AI 經濟最有價值商品；（7）TSMC 揭露 agentic AI 採用浪潮將推升 token 生成需求、為 AI 算力第二波結構性驅動。風險警示：部分老牌分析師警告 AI 晶片可能面臨 25-30% 修正、類比 1999-2000 網路泡沫，但 TSMC 結構性護城河（72% 全球代工市占）為主要避震。",
        "source": "Fortune / Motley Fool / Deloitte Insights / Distill Intelligence 2026-05", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "🚀 TSMC 2nm 量產啟動：5 座 2nm 廠 2026 量產、產能 2026-2028 CAGR +70%、NVIDIA 為 A16 首發",
        "body": "TSMC 2026 Technology Symposium 矽谷登場期間揭露：（1）2nm（N2）製程 2026 年首批 5 座晶圓廠（新竹 2 座 + 高雄 3 座）開始大量生產；（2）N2 產能 2026-2028 年複合成長率（CAGR）達 +70%、遠超 3nm 量產第一年的擴張速度；（3）N2 首年出貨量較 N3 量產第一年（2023）增加 +45%；（4）A16（1.6nm，H2 2026 量產）+ A14（2028 量產）+ N2U（2028）+ A13/A12（2029）製程路線完整確認；（5）NVIDIA 為 A16 首發大客戶、Feynman 架構預計 GTC 2026 三月發表；（6）AMD「Zen 6 EPYC Venice」CPU CCDs 確認採用 N2、為早期 2nm 客戶之一；（7）Apple 跳過 A16、直接從 N2 → A14 路線；（8）CoWoS 5.5-reticle 量產（>98% 良率）、2028 推出 14-reticle 整合 10 大晶片 + 20 HBM 先進封裝。Foundry 龍頭地位結構性強化、呼應 NVIDIA $95B 採購承諾。",
        "source": "Semiwiki / TweakTown / Design-Reuse / Trendforce 2026-05", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ Apple-Intel 轉單傳聞影響續存；Intel Foundry 18A 良率與時程仍待驗證",
        "body": "5/12-13 市場傳出 Apple 評估將部分晶片訂單轉至 Intel Foundry 18A 製程的傳聞、為 Apple 多元化代工策略一環。Apple 目前為 TSMC 第 2 大客戶、佔總營收 17%。最新觀察（5/14）：（1）TSMC 台股 5/13 重挫 -2.00% 但 5/14 跟進 ADR +0.63% 反彈訊號出現技術性回升；（2）Intel 同期 5/13 股價回吐 -1.39%（前日大漲 +5.66%）反映市場開始質疑 Intel 18A 量產時程與良率；（3）TSMC 在 3nm 以下先進製程技術領先優勢仍未動搖；（4）Apple 多元化代工為長期趨勢、無立即衝擊 2026 營收能見度；（5）NVIDIA 已取代 Apple 為 TSMC 最大客戶、$95B 採購承諾 + AMD/Broadcom Custom XPU 訂單續強為主要結構支撐。Wedbush 與 Bernstein 分析師均表示：Apple 多元化策略已被市場部分計入、不改變 TSMC 長期成長故事。",
        "source": "Reuters / Bloomberg / Taipei Times / 鉅亨網 2026-05-12~14", "impact": "中性",
    },
]

...
STOCK = {
    ...
    "tw_high":        "1,850",
    "tw_low":         "1,800",
    "market_cap":     "NT$47.71 兆",
    "pe_ratio":       "~35.1x",       # 台股 P/E（TTM，2026-03-26）
    "pb_ratio":       "~10.3x",       # 台股 P/B（股淨比）
    "div_yield":      "~1.22%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "338.02",       # 盤中價（2026-03-26 13:57 EDT，市場未收盤）
    "nyse_change":    "+8.78",        # 較前收盤上漲
    "nyse_change_pct":"+2.67%",
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-26 收盤  |  NYSE 數據：2026-03-26 盤中（13:57 EDT）  |  TSMC 董事會批准 $440 億美元擴產計畫  |  Broadcom 警告 TSMC 2026 年產能已成瓶頸",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "台股 2330 收 NT$1,845（+1.93%），反彈重回 1,840 之上；NYSE TSM 盤中 $338.02（+2.67%）；TSMC 批准 $440 億擴產",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330（3/26 收盤）：NT$1,845（+35，+1.93%）反彈；NYSE TSM（3/26 盤中）US$338.02（+8.78，+2.67%），台美同步回升",
        "TSMC 董事會批准逾 $440 億美元擴產計畫（先進製程 $189 億 + 先進封裝 $47 億 + 廠房 $214 億），凸顯對 AI 需求長期信心",
        "Broadcom 警告 TSMC 晶圓產能已成 2026 年全球供應鏈最大瓶頸，PCB 交貨期從 6 週延長至 6 個月",
        "NVIDIA Feynman 傳首採 TSMC A16（1.6nm）跳過 2nm；NVIDIA 2026 年貢獻 $330 億美元（22%）超越 Apple 成最大客戶",
        "4/16 Q1 財報倒計時 21 天，EPS 共識 NT$20.38；AGM 提案截止日 3/31-4/7 近在眼前",
    ],
    # 一句結論
    "bottom_line": "台股 2330 今日收 NT$1,845（+1.93%），反彈突破 MA60（1,840）支撐關卡，短線技術面改善。NYSE TSM（3/26 盤中）US$338.02（+2.67%），台美同步回升，顯示市場對 AI 需求信心重燃。TSMC 董事會批准 $440 億美元擴產計畫為今日最重大利多，印證公司對長期需求的高度確信。Broadcom 產能瓶頸警告雖令人擔憂，但同時確認需求遠超供給，對 TSMC 定價力正面。荷姆茲/氦氣供應為地緣風險，持續觀察。Q1 財報（4/16）為關鍵催化劑，支撐 1,820-1,830，壓力 1,860-1,880。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ AGM 提案截止日 3/31-4/7；Q1 財報 4/16 倒計時 21 天；荷姆茲海峽氦氣/能源供應風險持續；NYSE 盤中數據，非最終收盤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-26",
    "foreign":  +2800,    # 外資（正=買超）— 台股反彈，外資回補（估）
    "trust":    +320,     # 投信（持續逢低承接）
    "dealer":   +150,     # 自營商（隨反彈小買）
    # 近5日（由舊到新）
    "foreign_5d": [-4200, -3200, +1500, -4100, +2800],
    "trust_5d":   [+310,  +350,  +290,  +280,  +320],
    "dealer_5d":  [-250,  -180,  -150,  -220,  +150],
    "foreign_ownership_pct": "75.2%",  # 外資持股比例（估，回補後微升）
    "foreign_ownership_shares": "約 195.2 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       45.2,   "rsi_signal": "中性（從超賣區反彈，RSI 回升至 45）",
    "macd":         -4.1,   "macd_signal": "MACD 死叉收窄，下行動能趨緩",   "macd_color": "#FF9800",
    "kdj_k":        42.5,   "kdj_d": 38.8, "kdj_j": 49.9, "kdj_signal": "KDJ 金叉訊號初現，由超賣區回升",
    "ma5":       "1,835",
    "ma20":      "1,895",
    "ma60":      "1,840",
    "ma120":     "1,740",
    "bb_upper":  "2,010",   # 布林上軌
    "bb_mid":    "1,895",   # 布林中軌（20MA）
    "bb_lower":  "1,785",   # 布林下軌
    "vol_ma5":  "19,400",   # 5日均量（張）
    "trend":    "台股 2330 於 3/26 收 NT$1,845（+1.93%），量能回升至 20,000 張。反彈突破 MA60（1,840）關卡，RSI 回升至 45 由超賣區脫離，KDJ 出現初步金叉訊號。TSMC 批准 $440 億擴產計畫為重大利多催化。NYSE TSM（3/26 盤中）$338.02（+2.67%）同步回升，技術面短線轉中性偏多。",
    "support":  "1,820–1,830",
    "resist":   "1,860–1,880",
...
DERIVATIVES = {
    "date":          "2026-03-26",
    "futures_price": "1,850",      # 台積電期貨近月（估，反彈後小幅正價差）
    "futures_oi":    "72,800",     # 未平倉量（口）（估，部分空單回補）
    "futures_basis": "+5",         # 小幅正價差
    "call_oi":       "35,200",
    "put_oi":        "49,800",
    "pcr":           "1.41",       # Put/Call 比率回落（空頭情緒略緩）
    "pcr_signal":    "中性偏空",
    "max_pain":      "1,860",      # 最大痛苦價格
    "key_strikes": [
        ("1,785", "Call 3,200 / Put 9,200", "布林下軌支撐"),
        ("1,820", "Call 5,200 / Put 8,500", "短線支撐"),
        ("1,845", "Call 6,800 / Put 7,500", "現貨所在"),
        ("1,860", "Call 9,200 / Put 5,500", "最大痛苦點/壓力"),
        ("1,880", "Call 10,500 / Put 4,200", "強壓力區"),
    ],
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,280",   "+2.1%",  True),   # 3/26 盤中，半導體股全面回升
    ("美元指數 DXY",    "103.5",   "-0.3%",  True),
    ("VIX 恐慌指數",    "24.0",    "-0.8",   True),   # 市場情緒進一步緩解
    ("WTI 原油",        "$71.0",   "-0.7%",  False),
    ("10Y 美債殖利率",  "4.22%",   "-0.03pp",True),
    ("台幣/美元",       "31.58",   "0.00",   True),
    ("NVIDIA NVDA",     "$875",    "+2.3%",  True),   # 盤中反彈，A16 確認利多持續
    ("ASML",            "$690",    "+1.3%",  True),   # 設備股持續回升
]
...
RELATIVE = {
    "date":           "2026-03-26",
    "tsmc_tw_1d":     +1.93,   # 2330 3/26 收盤
    "taiex_1d":       +0.85,   # 加權指數（3/26 估算）
    "sox_1d":         +2.10,   # SOX（3/26 盤中，估）
    "tsmc_nyse_1d":   +2.67,   # TSM ADR 3/26 盤中（US$338.02）
    "tsmc_tw_5d":     -3.6,
    "taiex_5d":       -1.3,
    "tsmc_tw_1m":     -7.8,
    "taiex_1m":       -4.0,
    ...
    # ADR 折溢價
    "adr_price":     338.02,   # TSM NYSE 盤中（2026-03-26 13:57 EDT）
    "adr_parity":    291.99,   # 理論值 = 5 × NT$1,845 ÷ 31.58（台股 3/26 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.58,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+15.8",# (338.02 - 291.99) / 291.99 × 100
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。NYSE 為盤中數據，TAIEX/多期間數據為估算",
}
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 EPS 共識 NT$20.38，前 2 月累計營收年增 30%（NT$7,189 億）。3/26 台股 2330 收 NT$1,845（+1.93%），NYSE TSM 盤中 US$338.02（+2.67%），台美同步反彈。今日最大利多：TSMC 董事會批准 $440 億美元擴產計畫，凸顯對 AI 長期需求高度確信。中長線基本面持續強化，31 位分析師無一賣出，目標 NT$2,290（上行 +24.1%）。4/16 Q1 財報（倒計時 21 天）為關鍵催化劑，若毛利率超指引（63-65%）可望進一步確認上行走勢。",
...
NEWS = [
    {
        ...
        "title": "Broadcom 警告：TSMC 產能已成 2026 年全球供應鏈最大瓶頸",
        "body": "Broadcom 高層公開示警，台積電晶圓產能已成 2026 年全球供應鏈最大瓶頸，同時指出雷射元件與 PCB 亦出現短缺，AI 需求持續超出製造業整體供給能力。此警告一方面凸顯 TSMC 定價能力強大，另一方面也意味需求遠超近期市場悲觀預期。長線利多，短線產能壓力待解。",
        "source": "TrendForce 2026-03-24", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "荷姆茲海峽緊張局勢威脅 TSMC 氦氣供應，能源風險升溫",
        "body": "伊朗緊張情勢導致荷姆茲海峽 LNG 運輸受阻，分析師示警台灣電力供應可能受影響。伊朗攻擊卡達造成氦氣（晶圓廠關鍵材料）短缺，氦氣現貨價短期急漲。TSMC 聲明現有庫存可控、生產正常，但若危機延續 2 個月以上，成本壓力將逐步顯現，毛利率面臨下修風險。",
        "source": "Yahoo Finance / Energy Analysis 2026-03-24", "impact": "負面",
    },
    {
        "tag": "技術節點", "tag_color": "#9C27B0",
        "title": "NVIDIA Feynman 傳首採 TSMC A16（1.6nm），跳過 2nm 製程",
        "body": "報導指 NVIDIA 下一代 Feynman 架構 GPU 將直接採用台積電最先進 A16（1.6nm，配備 BPR 背面電源供電）製程，跳過 N2 節點，可帶來效能提升 10% 或功耗降低 20%。TSMC SoIC 先進封裝月產能目標 2026 年達 1–1.5 萬片。此確認強化 TSMC 先進製程壟斷地位，競爭對手無緣此市場。",
        "source": "TrendForce / TweakTown 2026-03-18", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA 超越 Apple 成 TSMC 最大客戶，年貢獻達 $330 億美元",
        "body": "2026 年 NVIDIA 預計貢獻台積電約 $330 億美元營收（佔比約 22%），超越 Apple 的 $270 億美元（18%），標誌 AI 晶片需求主導 TSMC 營收結構的歷史性轉折。GB300/GB400 採 N3P + CoWoS-L，訂單積壓超 4 季。AI GPU 取代智慧型手機成為驅動台積電成長的核心動力，結構性升級強化估值。",
        "source": "CNBC / MacRumors 2026-01-26", "impact": "正面",
    },
    {
        "tag": "擴產計畫", "tag_color": "#4CAF50",
        "title": "TSMC 董事會批准 $440 億美元擴產計畫，力鞏先進製程領先優勢",
        "body": "台積電董事會批准逾 440 億美元（US$44B）資本擴產計畫：先進製程產能 $189 億美元、先進封裝與成熟製程 $46.7 億美元、不動產與租賃資產 $213.7 億美元。此舉彰顯 TSMC 對 AI 需求長期爆發的高度確信。2026 年資本支出上看 $560 億美元（年增逾 40%），亞利桑那廠總投資額已攀升至 $1,650 億美元，Plant 2 目標 2028 年量產 2nm，長線擴產策略清晰。",
        "source": "TipRanks / Bloomberg 2026-03-25", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "Micron Q2 FY26 營收創新高 $239 億，HBM 超級週期推動半導體生態",
        "body": "Micron 發布 Q2 FY2026 財報，營收 $239 億大幅超預期，EPS $12.20，HBM3E 產能售罄至 2027 年，盤後股價大漲 15%。半導體 AI 超級週期確認延續，SK Hynix、Samsung、Micron 競逐供應 NVIDIA Vera Rubin 平台 HBM4。強勁的記憶體需求為 TSMC HPC 業務（佔 58% 營收）提供有力背書，整體半導體生態持續受惠 AI 浪潮。",
        "source": "FinancialContent / Micron Earnings 2026-03-24", "impact": "正面",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,470",        # 台股 2330 最新確認收盤：7/16 Thu 官方收 NT$2,470（+30、+1.23% vs 7/15 NT$2,440、TWSE）——法說會（14:00、台股收盤後）前買盤搶跑、加權指數同日小跌 -6.61 點（-0.01%）收 45,624.98 下 2330 逆勢領漲
    "tw_change":      "+30",          # 7/16 Thu 收盤漲跌點數（+1.23%、TWSE 官方）
    "tw_change_pct":  "+1.23%",       # 7/16 Thu 收盤漲跌幅（大盤 -0.01% 下逆勢領漲、法說會前搶跑）
    "tw_volume":      "30,539",       # 7/16 成交量（張、TWSE 口徑；成交額 NT$747.50 億、均價 2,448、成交 97,957 筆、量能自 7/15 33,666 張續收斂）
    "tw_open":        "2,430",        # 7/16 開盤（TWSE）
    "tw_high":        "2,470",        # 7/16 盤中高（TWSE）
    "tw_low":         "2,420",        # 7/16 盤中低（TWSE）
    "market_cap":     "NT$64.0 兆",   # 7/16 收盤市值（NT$2,470 基礎、約 259.3 億股）
    "pe_ratio":       "~28.3x",       # 台股 P/E（TTM 估，NT$2,470 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25 後估值大幅消化）
    "pb_ratio":       "~18.5x",       # 台股 P/B（NT$2,470 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~0.97%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,470）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "409.74",       # 7/16 Thu 官方收盤 $409.74（-$9.74、-2.32% vs 7/15 $419.48、日內 $403.50-415.95、量 2,479 萬股、Yahoo Finance API）——Q2 大超預期仍收跌：SOX -4.29% 重挫拖累、市場消化 H2 毛利率稀釋 3-4pp 與資本支出上調至 $60-64B、AI 估值疑慮＋美伊地緣緊張
    "nyse_change":    "-9.74",        # 7/16 漲跌（自 7/15 $419.48）
    "nyse_change_pct":"-2.32%",       # 7/16 逆財報收跌、SOX -4.29% 半導體全面重挫下與板塊同步
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~30.0x",       # NYSE USD 基礎 P/E（TTM 估，$409.74 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.1x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-17 Fri（最新確認收盤：台股 2330 7/16 Thu 官方收 NT$2,470（+30、+1.23% vs 7/15 NT$2,440、開 2,430／高 2,470／低 2,420、量 30,539 張（TWSE 口徑）、成交額 NT$747.50 億、均價 2,448、成交 97,957 筆、TWSE）——加權指數同日小跌 -6.61 點（-0.01%）收 45,624.98、2330 法說會前逆勢領漲；NYSE TSM 7/16 Thu 收 $409.74（-$9.74、-2.32% vs 7/15 $419.48、日內 $403.50-415.95、量 2,479 萬股、Yahoo Finance）——Q2 大超預期仍收跌、SOX -4.29% 全面重挫拖累）。🎯🎯 今日最重要：Q2 2026 法說會（7/16 14:00）結果大超預期——Q2 營收 NT$1.270 兆（$40.20B、+33.7% YoY、+12.0% QoQ、觸及指引 $39.0-40.2B 上緣）、稀釋 EPS NT$27.25（$4.31/ADR、+77.4% YoY、超共識 ~$3.94 約 +9.4%）、毛利率 67.7%（超出指引 65.5-67.5% 上緣）、營益率 60.3%、淨利率 55.6%——連續第 11 季超預期、連 5 季獲利創高；Q3 指引營收 $44.6-45.8B（中位 +12.4% QoQ、遠超市場預期）、毛利率 65-67%（2nm 量產爬坡 H2 稀釋 3-4pp）；全年營收成長自 >30% 大幅上修至「略高於 40%」；2026 資本支出自 $52-56B 上調至 $60-64B；2026 全年股利 NT$24/股（+33% YoY）；管理層：「AI 需求比先前預期更強」；HPC 占 Q2 營收 66%、智慧型手機 22%；先進製程（≤7nm）占晶圓營收 77%、2nm 已貢獻 3%。📊 三大法人 7/16（T86 官方）：外資賣超 4,215 張（連 6 賣、近 5 個交易日累計 -34,852 張）、投信買超 528 張（終止賣超轉買）、自營買超 1,061 張（自行 +407、避險 +654）、合計賣超 2,625 張——投信/自營法說會前先行卡位。📊 台美 ADR 溢價收斂至 +7.0%（ADR $409.74 vs 理論值 $383.08、匯率 32.24）。⚠️ 隔夜美股半導體重挫：SOX -4.29% 收 11,867.50——TSMC 財報大超預期仍未能止血，分析師點名資本支出沉重、H2 毛利率稀釋、AI 投資能否兌現估值的疑慮＋美伊地緣緊張推升油價與債息；NVDA -2.40% 收 $207.40、AVGO -5.03%、AMD -5.33%、INTC -5.84%、MU -5.65%、ARM -5.41%、AMKR -6.83%、AMAT -3.19%、LRCX -4.31%；記憶體鏈續崩：SK 海力士 7/16 收盤 -11.5%、三星 -8.8%；⭐ 逆勢亮點：AAPL +1.76% 收 $333.26。⚠️ 風險：7/17 台股面臨「法說會大超預期 vs 美股半導體重挫」多空對決、外資連 6 賣、AI 估值修正擴散、美 232 條款半導體關稅 25% + 輸中晶片轉運查驗、美伊地緣緊張；估值經 Q2 獲利跳升後明顯消化（台股 P/E 降至 ~28.3x TTM、TSM ~30.0x）、台幣 ~32.24。技術面（官方收盤計算）：7/16 收 2,470 站上 5MA ~2,437 / 20MA ~2,434、RSI 57.8 中軸上、MACD 柱負值收斂至 -4.9（DIF 29.0 < DEA 34.0）、KDJ K50/D45/J59 修復接近金叉；支撐 2,420（7/16 低）／2,415／2,349（布林下軌）、壓力 2,470-2,480／2,520（布林上軌）／2,535 史高。⭐ 中長線基底更強：全年成長上修至 >40%、CapEx $60-64B、AI 需求「比預期更強」、2nm 已貢獻 3% 營收且 5 座廠爬坡、HPC 占 66%、股利 +33%、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、全先進製程漲價 5-10%",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "🎯🎯 Q2 法說會大超預期：營收 $40.20B（+33.7% YoY）觸及指引上緣、EPS $4.31/ADR（+77.4%）、毛利率 67.7% 超標；全年營收成長上修至「略高於 40%」、CapEx 上調至 $60-64B、股利 NT$24/股（+33%）；惟 NYSE TSM 7/16 逆勢收跌 -2.32%（SOX -4.29% 重挫、AI 估值疑慮＋H2 毛利率稀釋 3-4pp）——今日 7/17 台股面臨「基本面大利多 vs 美股重挫」多空對決",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🎯🎯 最重要：Q2 2026 法說會（7/16 14:00）大超預期——營收 NT$1.270 兆（$40.20B、+33.7% YoY、+12.0% QoQ）觸及指引上緣、EPS NT$27.25（$4.31/ADR、+77.4% YoY、超共識 ~+9.4%）、毛利率 67.7% 超出指引上緣、營益率 60.3%——連 11 季超預期、連 5 季獲利創高；HPC 占 66%、2nm 已貢獻 3% 營收",
        "🚀 展望全面上修：Q3 指引營收 $44.6-45.8B（中位 +12.4% QoQ、遠超預期）、毛利率 65-67%（2nm 爬坡 H2 稀釋 3-4pp）；全年營收成長自 >30% 上修至「略高於 40%」；2026 CapEx 自 $52-56B 上調至 $60-64B；2026 股利 NT$24/股（+33% YoY）；管理層：「AI 需求比先前預期更強」",
        "⚠️ 惟美股不買單：NYSE TSM 7/16 收 $409.74（-2.32%）——SOX -4.29% 收 11,867.50 全面重挫（AVGO -5.03%、AMD -5.33%、INTC -5.84%、MU -5.65%、NVDA -2.40%），分析師點名重資本支出、H2 毛利率稀釋、AI 估值疑慮＋美伊地緣緊張；記憶體鏈續崩：SK 海力士 -11.5%、三星 -8.8%；⭐ AAPL +1.76% 逆勢",
        "📈 台股 2330 7/16 Thu 官方收 NT$2,470（+30、+1.23%、量 30,539 張、額 NT$747.50 億）——大盤 -0.01% 下逆勢領漲、法說會前買盤搶跑；今日 7/17 開盤將補反映「財報大利多 vs 美股重挫」、恐高波動",
        "📊 籌碼分歧：三大法人 7/16 合計賣超 2,625 張——外資賣超 4,215 張（連 6 賣、近 5 個交易日累計 -34,852 張）；投信買超 528 張（轉買）、自營買超 1,061 張——內資法說會前先行卡位、外資態度待財報後翻轉驗證",
        "📊 台美 ADR 溢價收斂至 +7.0%（$409.74 vs 理論值 $383.08、匯率 32.24）——為近期低位、隱含 ADR 已先行修正",
        "💡 估值大幅消化：Q2 EPS 跳升後台股 P/E 降至 ~28.3x TTM（前 ~32.3x）、TSM ~30.0x；分析師目標密集區美銀 $590、Barclays $625、Citi NT$3,800（法說會後待重估）",
        "⚠️ 風險續追蹤：AI 估值修正是否擴散（Jensen Huang 每 GW 算力成本 $50B→$100B 之議）；外資連 6 賣；美 232 條款關稅 25% + 輸中晶片轉運查驗；美伊地緣緊張推升油價債息；Intel 18A 競爭",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-17 Fri（最新確認收盤：台股 2330 7/16 Thu 官方收 NT$2,470（+30、+1.23% vs 7/15 NT$2,440、開 2,430／高 2,470／低 2,420、量 30,539 張 TWSE 口徑、成交額 NT$747.50 億、均價 2,448、TWSE 確認）；NYSE TSM 7/16 Thu 收 $409.74（-$9.74、-2.32% vs 7/15 $419.48、日內 $403.50-415.95、量 2,479 萬股））。⭐⭐ 今日最重要：Q2 2026 法說會（7/16 14:00）結果大超預期——（1）Q2 營收 NT$1.270 兆（$40.20B、+33.7% YoY、+12.0% QoQ）觸及指引 $39.0-40.2B 上緣；（2）稀釋 EPS NT$27.25（$4.31/ADR、+77.4% YoY）超共識 ~$3.94 約 +9.4%、連續第 11 季超預期、連 5 季獲利創高；（3）毛利率 67.7% 超出指引 65.5-67.5% 上緣、營益率 60.3%、淨利率 55.6%；（4）Q3 指引營收 $44.6-45.8B（中位 $45.2B、+12.4% QoQ、遠超市場預期）、毛利率 65-67%（2nm 量產爬坡 H2 稀釋 3-4pp）；（5）全年營收成長自 >30% 大幅上修至「略高於 40%」、2026 CapEx 自 $52-56B 上調至 $60-64B、2026 股利 NT$24/股（+33% YoY）；（6）結構：HPC 占 66%、智慧型手機 22%、先進製程（≤7nm）占 77%、2nm 首度貢獻 3%；管理層明言「AI 需求比先前預期更強」。⚠️ 惟美股不買單：NYSE TSM 7/16 收 $409.74（-2.32%）——SOX -4.29% 收 11,867.50 全面重挫：AVGO -5.03%、AMD -5.33%、INTC -5.84%、MU -5.65%、ARM -5.41%、NVDA -2.40%、AMKR -6.83%；分析師點名重資本支出、H2 毛利率稀釋與「AI 投資能否兌現估值」疑慮、疊加美伊地緣緊張推升油價債息；記憶體鏈續崩（SK 海力士 7/16 收 -11.5%、三星 -8.8%）；⭐ AAPL +1.76% 收 $333.26 逆勢。⭐ 今日重點：（1）📈 台股 2330 7/16 收 NT$2,470（+1.23%）法說會前搶跑、大盤 -0.01% 下逆勢領漲——今日 7/17 開盤補反映「財報大利多 vs 美股重挫」、恐高波動、多空對決；（2）📊 籌碼分歧：三大法人 7/16 合計賣超 2,625 張——外資 -4,215（連 6 賣、近 5 個交易日累計 -34,852 張）、投信 +528（轉買）、自營 +1,061——內資先行卡位、外資能否因財報翻買為關鍵；（3）📊 台美 ADR 溢價收斂至 +7.0%（$409.74 vs 理論值 $383.08、匯率 32.24）——ADR 已先行修正、對台股開盤壓力相對緩和；（4）💡 估值大幅消化：Q2 EPS 跳升後台股 P/E 降至 ~28.3x TTM（前 ~32.3x）、TSM ~30.0x。技術面（官方收盤計算）：5MA ~2,437 / 20MA ~2,434 / 60MA ~2,326；7/16 收 2,470 站上 5MA/20MA、RSI 57.8 中軸上、MACD 柱負值收斂至 -4.9、KDJ K50/D45/J59 接近金叉；支撐 2,420（7/16 低）／2,415／2,349（布林下軌）、壓力 2,470-2,480／2,520（布林上軌）／2,535 史高。⚠️ 風險：AI 估值修正擴散、外資連 6 賣、美 232 關稅與輸中晶片轉運查驗、美伊地緣緊張、Intel 18A 競爭、台幣 ~32.24（每升值 1% 毛利率 -40bps）。⭐ 中長線基底更強：全年成長 >40%、CapEx $60-64B、AI 需求「比預期更強」、2nm 5 座廠爬坡且已貢獻 3% 營收、股利 +33%、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、漲價 5-10% 定價權。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/17 Fri）關鍵觀察：（1）🎯🎯 台股補反映法說會＋美股重挫的多空對決——Q2 大超預期（EPS +77.4%、全年上修至 >40%、CapEx $60-64B、股利 +33%）為重大基本面利多，但 NYSE TSM 7/16 逆勢收跌 -2.32%、SOX -4.29% 全面重挫——7/16 台股收 2,470 已部分搶跑、今日開盤恐高波動，收盤能否守住 2,420-2,440 區為短線關鍵；（2）⚠️ AI 估值修正是否擴散：分析師點名「AI 投資能否兌現估值」＋Jensen Huang 每 GW 算力成本 $50B→$100B 之議；記憶體鏈續崩（SK 海力士 -11.5%、三星 -8.8%、MU -5.65%）；（3）📊 籌碼：外資連 6 賣（7/16 -4,215、近 5 個交易日 -34,852）——財報大超預期後能否翻買為最大觀察點；投信 +528 轉買、自營 +1,061 已先卡位；（4）技術面：7/16 收 2,470 站上 5MA ~2,437 / 20MA ~2,434、MACD 柱 -4.9 收斂中、KDJ 接近金叉——守穩 2,420／2,415 支撐、突破 2,480 再挑戰 2,520／2,535 史高；（5）💱 美伊地緣緊張推升油價債息、台幣 ~32.24、美 232 條款關稅 + 輸中晶片轉運查驗續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-16",       # 最新資料日（7/16 Thu、TWSE T86 官方）
    "foreign":  -4215,             # 外資 7/16 賣超 4,215 張（連 6 賣、近 5 個交易日累計 -34,852 張）
    "trust":    528,               # 投信 7/16 買超 528 張（終止賣超轉買、法說會前卡位）
    "dealer":   1061,              # 自營商 7/16 買超 1,061 張（自行 +407、避險 +654）；三大法人合計 -2,625 張
    # 近5日（由舊到新：[7/10 颱風休市], 7/13, 7/14, 7/15, 7/16）；TWSE T86 口徑（張）
    "foreign_5d": [0, -2045, -12416, -3427, -4215],     # 外資（7/10 休市計 0；連 6 賣、7/16 賣壓仍收斂於 7/14 高峰之下）
    "trust_5d":   [0, 530, 1841, -596, 528],            # 投信（7/10 休市計 0；7/16 轉買）
    "dealer_5d":  [0, 623, 2570, 456, 1061],            # 自營（7/10 休市計 0；7/16 買超放大）
    "foreign_ownership_pct": "~74.8%",  # 外資持股比例（估，官方以 TWSE 外資持股餘額為準）
    "foreign_ownership_shares": "約 194 億股（估）",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       57.8,   "rsi_signal": "RSI(14) 57.8（官方收盤計算）——7/16 +1.23% 後續居中軸之上、中性偏多、今日補反映法說會結果與美股重挫",
    "macd":         -4.9,  "macd_signal": "MACD 柱狀負值收斂至 -4.9（DIF 29.0 < DEA 34.0、官方收盤計算）——短線動能修復中、連 2 日收高後柱體連續收斂、待放量突破確認翻正",   "macd_color": "#F44336",
    "kdj_k":        50.0,   "kdj_d": 45.0, "kdj_j": 59.0, "kdj_signal": "KDJ K(50)/D(45)/J(59)（官方收盤計算）——K 已上穿 D 形成初步金叉、J 值 59 自超賣區大幅回升，短線轉強、今日多空對決驗證成色",
    "ma5":       "2,437",   # 5MA（官方收盤計算，含 7/16 NT$2,470）
    "ma20":      "2,434",   # 20MA（官方收盤計算）
    "ma60":      "2,326",  # 60MA（官方收盤計算）
    "ma120":     "2,095",  # 120MA（官方收盤計算）
    "bb_upper":  "2,520",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,434",   # 布林中軌（20MA）
    "bb_lower":  "2,349",  # 布林下軌（20MA-2σ）
    "vol_ma5":  "35,411",   # 5日均量（張、TWSE 口徑、含 7/16 官方量 30,539）
    "trend":    "📈 法說會前逆勢領漲、技術面全面修復：台股 2330 7/16 官方收 NT$2,470（+30、+1.23%、量 30,539 張 TWSE 口徑）——加權指數同日小跌 -6.61 點（-0.01%）收 45,624.98，2330 逆勢領漲、買盤搶跑法說會；站上 5MA（~2,437）與 20MA（~2,434）、收盤即當日最高。🎯🎯 法說會（7/16 14:00）大超預期：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%）、毛利率 67.7%；Q3 指引 $44.6-45.8B、全年上修至 >40%、CapEx $60-64B、股利 +33%。⚠️ 惟隔夜（7/16 美股）半導體全面重挫：SOX -4.29% 收 11,867.50、AVGO -5.03%、AMD -5.33%、INTC -5.84%、MU -5.65%、ARM -5.41%、NVDA -2.40%——NYSE TSM 逆財報收 $409.74（-2.32%）；記憶體鏈續崩（SK 海力士 -11.5%、三星 -8.8%）；⭐ AAPL +1.76% 逆勢。📊 技術指標（官方收盤計算）：RSI(14) 57.8 中軸上、MACD 柱負值收斂至 -4.9（DIF 29.0 < DEA 34.0）、KDJ K(50)/D(45)/J(59) 初步金叉——短線動能修復、今日 7/17 多空對決驗證。下一觀察：（1）今日開盤補反映「財報大利多 vs 美股重挫」——ADR 溢價已收斂至 +7.0%、ADR 先行修正對台股壓力相對緩和；（2）守穩支撐 2,420（7/16 低）／2,415／布林下軌 2,349；（3）突破 2,480 看 2,520（布林上軌）、再挑戰 2,535 史高；（4）外資連 6 賣（7/16 -4,215）、財報大超預期後能否翻買為最大觀察點；（5）💡 估值經 Q2 獲利跳升大幅消化（P/E ~28.3x TTM）+ 全年 >40% 成長為中長線支撐。",
    "support":  "2,420 / 2,415 / 2,349 (7/16 低、7/15 低、布林下軌)",
    "resist":   "2,480 / 2,520 / 2,535 (前高、布林上軌、史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-16 止），請以看盤軟體為準；7/16 官方收 NT$2,470（+1.23%）站上 5MA ~2,437 / 20MA ~2,434、RSI 57.8、MACD 柱收斂至 -4.9、KDJ 初步金叉；下方支撐 2,420 / 2,415 / 2,349、上方壓力 2,480 / 2,520 / 2,535 史高；🎯🎯 Q2 法說會大超預期（全年上修 >40%、CapEx $60-64B、股利 +33%）vs ⚠️ 美股 SOX -4.29% 重挫、TSM -2.32%——今日 7/17 台股多空對決；外資連 6 賣能否翻買 + AI 估值修正是否擴散為兩大變數",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-16",       # 最新資料日（7/16 Thu 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,460",            # 台積電期貨近月（估、現貨 7/16 收 NT$2,470、ADR 隔夜 -2.32% 下期貨恐貼水開出）
    "futures_oi":    "186,000",          # 未平倉量（口）（估）
    "futures_basis": "-10",              # 估逆價差（ADR 重挫 + 今日多空對決、避險保守）
    "call_oi":       "122,000",
    "put_oi":        "92,000",
    "pcr":           "0.75",             # Put/Call 比率（估、法說會後首日避險需求升溫）
    "pcr_signal":    "中性（Put/Call 估 0.75；台股 7/16 官方收 NT$2,470（+1.23%）法說會前搶跑，惟 NYSE TSM -2.32%、SOX -4.29% 重挫——今日 7/17 補反映、避險需求升溫；⭐ Q2 大超預期＋全年上修 >40% 為中長線支撐；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,450",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,349", "Call 55,000 / Put 16,000", "布林下軌、下方支撐"),
        ("2,415", "Call 48,000 / Put 12,000", "7/15 盤中低、支撐"),
        ("2,420", "Call 47,000 / Put 10,000", "7/16 盤中低、支撐"),
        ("2,450", "Call 45,000 / Put 8,000",  "最大痛苦（估）"),
        ("2,470", "Call 43,000 / Put 6,500",  "7/16 官方收、多空分界"),
        ("2,480", "Call 38,000 / Put 5,000",  "前高壓力帶"),
        ("2,520", "Call 33,000 / Put 3,500",  "布林上軌壓力"),
        ("2,535", "Call 25,000 / Put 2,500",  "盤中史高"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/16 NT$2,470）；NT$2,450 為估最大痛苦、下方支撐 2,420 / 2,415 / 2,349（布林下軌）、上方壓力 2,480 / 2,520 / 2,535 史高；🎯🎯 Q2 法說會大超預期（Q3 指引 $44.6-45.8B、全年上修 >40%、CapEx $60-64B、股利 +33%）vs ⚠️ 美股 SOX -4.29% 重挫、TSM -2.32%——今日 7/17 台股補反映恐高波動；外資連 6 賣能否翻買為關鍵；中長線基底更強（AI 需求「比預期更強」、2nm 已貢獻 3% 營收、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-16 Thu 官方收盤；美股為 7/16 Thu 收盤（Yahoo Finance API）；韓股為 7/16 Thu 收盤）
    ("設備", "ASML",    "艾司摩爾",    "1,784.87", "-1.67%", "⭐ EUV 設備龍頭；7/15 第二度上修 2026 展望後 7/16 隨板塊回檔、跌幅相對抗跌"),
    ("設備", "AMAT",    "應用材料",    "560.93", "-3.19%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（7/16 隨 SOX -4.29% 重挫）；TSMC CapEx 上調至 $60-64B 為設備鏈利多"),
    ("設備", "LRCX",    "科林研發",    "320.96", "-4.31%", "蝕刻設備（7/16 重挫）；TSMC CapEx $60-64B + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "219.37", "-2.29%", "量測設備（7/16 回檔）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","172.00","-4.71%", "與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/16 台股收盤回吐、前日大漲後獲利了結）"),
    ("記憶", "000660.KS","SK 海力士",  "1,842,000 KRW", "-11.53%", "⚠️ 7/16 收盤暴跌 -11.5%（AI 記憶體暴漲後連 2 日急回吐、美 ADR 亦重挫）；HBM4、與 NVIDIA 多年期合作"),
    ("記憶", "MU",      "美光",        "853.20", "-5.65%", "⚠️ HBM / DDR5（7/16 續跌 -5.65%、兩日 -13.2%）；AI 記憶體獲利了結、HBM4 中長線邏輯未變"),
    ("封裝", "3711.TW", "日月光",      "682.00", "-0.15%", "CoWoS / 先進封裝（7/16 台股收盤持平、前日大漲後守穩）"),
    ("封裝", "AMKR",    "Amkor",       "63.02",  "-6.83%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/16 隨 SOX 重挫）"),
    ("客戶", "NVDA",    "輝達",        "207.40", "-2.40%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/16 收 $207.40（Jensen Huang 每 GW 算力成本 $50B→$100B 之議引估值疑慮）"),
    ("客戶", "AAPL",    "蘋果",        "333.26", "+1.76%", "⭐ #2 客戶 18%；7/16 收 $333.26 半導體重挫中逆勢連 2 日收漲；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "374.45", "-5.03%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/16 重挫、AI 估值修正領跌）"),
    ("客戶", "AMD",     "超微",        "500.94", "-5.33%", "#4 客戶；MI350/MI450、首發 2nm AI 晶片（7/16 重挫、勉守 $500）"),
    ("客戶", "2454.TW", "聯發科",      "3,700",  "-1.07%", "手機 SoC 客戶 9%；面臨漲價成本（7/16 台股官方收盤小跌）"),
    ("競爭", "INTC",    "英特爾",      "96.98",  "-5.84%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/16 重挫失守 $100、TSMC 全年 >40% 成長對比競爭壓力）"),
    ("競爭", "005930.KS","三星電子",   "255,000 KRW", "-8.77%", "⚠️ 7/16 收盤大跌 -8.77%（記憶體鏈連 2 日急回吐）；跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣"),
    ("產業", "SOX",     "費城半導體指數","11,867.50", "-4.29%", "⚠️ 7/16 全面重挫 -4.29%（TSMC 財報大超預期仍未能止血——AI 估值疑慮＋美伊地緣緊張）；兩日 -6.3%"),
    ("處分後", "ARM",   "Arm Holdings","262.01", "-5.41%", "TSMC 4/29 全數出脫後（7/16 重挫、SMH 成分領跌之一）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/16 收 $409.74 具 +19.7% 上行空間；美銀 $590、Citi 台股目標 NT$3,800、Barclays $625、Nomura NT$3,425；7/16 法說會大超預期後目標價待重估、預期新一輪上調）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+19.67%",  # 從 NYSE 7/16 收 $409.74 至平均目標 $490.34（共識強力買入；法說會大超預期後待重估）
}
...
IR_CALENDAR = [
    ...
    ("2026-07-16", "📅 Q2 2026 財報法說會", "7/16 週三 14:00 公布 Q2 2026 財報；指引 $39.0-40.2B (+10% QoQ、+32% YoY)；毛利率 65-67%（法人估挑戰 70%）",                                        "#FF9800"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,661.93", "+2.54%", False),  # 7/14 收大反彈（Yahoo）
    ("台股加權指數",    "44,737.95", "-1.42%", False),  # 7/14 官方收盤（-642.57、TWSE）
    ("VIX 恐慌指數",    "16.50",    "-3.85%",  False),   # 7/14 收（Yahoo）
    ("WTI 原油",        "$80.53",   "+3.06%",  False),   # 7/14 收（Yahoo）
    ("10Y 美債殖利率",  "4.59%",   "≈持平",   False),   # 7/14 收（Yahoo ^TNX）
    ("台幣/美元",       "~32.14",   "≈持平",  False),   # 7/15 Yahoo TWD=X 32.142（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$211.80", "+4.06%",  False),   # #1 TSMC 客戶（22%），7/14 收大漲
    ("ASML",            "$1,775.64", "+2.87%",  False),  # 設備股 7/14 收（Yahoo）
]
...
NEWS = [
    {
        ...
        "title": "🎯🎯 Q2 法說會大超預期：營收 $40.2B（+33.7%）、EPS $4.31/ADR（+77.4%）、毛利率 67.7%；全年上修至 >40%",
        "body": "連續第 11 季超預期、獲利連 5 季創高：（1）📊 Q2 營收 NT$1.270 兆（$40.20B、+33.7% YoY、+12.0% QoQ）觸及指引 $39.0-40.2B 上緣；稀釋 EPS NT$27.25（$4.31/ADR）超共識 ~$3.94 約 +9.4%；毛利率 67.7% 超出指引 65.5-67.5% 上緣、營益率 60.3%、淨利率 55.6%；（2）🚀 Q3 指引營收 $44.6-45.8B（中位 +12.4% QoQ、遠超預期）、毛利率 65-67%（2nm 爬坡 H2 稀釋 3-4pp）；（3）⭐ 全年營收成長自 >30% 大幅上修至「略高於 40%」、2026 CapEx 自 $52-56B 上調至 $60-64B、2026 股利 NT$24/股（+33% YoY）；（4）結構：HPC 占 66%、手機 22%、≤7nm 占 77%、2nm 首度貢獻 3%；管理層：「AI 需求比先前預期更強」。",
        "source": "TSMC 法說會 / SEC 6-K / GuruFocus 2026-07-16", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 財報大超預期美股仍不買單：TSM 7/16 收跌 -2.32%、SOX -4.29% 全面重挫",
        "body": "AI 估值疑慮壓過基本面利多：（1）⚠️ NYSE TSM 7/16 收 $409.74（-$9.74、-2.32%、日內 $403.50-415.95）——分析師點名重資本支出（CapEx 上調至 $60-64B）、H2 毛利率稀釋 3-4pp 與「巨額 AI 投資能否兌現估值」的疑慮；（2）📉 SOX -4.29% 收 11,867.50：AVGO -5.03%、AMD -5.33%、INTC -5.84%、ARM -5.41%、AMKR -6.83%、NVDA -2.40%——Jensen Huang 每 GW 算力成本 $50B→$100B 之議加深疑慮；（3）🌍 美伊地緣緊張升溫推升油價與債息、風險偏好下降；（4）⭐ AAPL +1.76% 逆勢連 2 日收漲；ADR 溢價收斂至 +7.0%、隱含 ADR 已先行修正。",
        "source": "Yahoo Finance / Bloomberg / Motley Fool / TheStreet 2026-07-16", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 台股 2330 7/16 收 NT$2,470（+1.23%）法說會前搶跑；今日 7/17 面臨多空對決",
        "body": "基本面大利多 vs 美股重挫的補反映：（1）📈 台股 2330 7/16 官方收 NT$2,470（+30、+1.23%、開 2,430／高 2,470／低 2,420、量 30,539 張 TWSE 口徑、額 NT$747.50 億）——加權指數同日 -0.01% 下逆勢領漲、收盤即當日最高；（2）📊 技術面修復：站上 5MA ~2,437 / 20MA ~2,434、RSI 57.8、MACD 柱收斂至 -4.9、KDJ 初步金叉；（3）🎯 今日觀察：法說會大超預期（全年 >40%、股利 +33%）為重大利多、惟隔夜 TSM -2.32%／SOX -4.29%——開盤恐高波動，守 2,420-2,440 支撐、突破 2,480 看 2,520/2,535 史高；（4）💡 估值經 Q2 獲利跳升後 P/E 降至 ~28.3x TTM、明顯消化。",
        "source": "TWSE / Yahoo Finance API 2026-07-16", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⚠️ AI 記憶體鏈連 2 日急回吐：SK 海力士 -11.5%、三星 -8.8%、MU 兩日 -13.2%",
        "body": "記憶體暴漲行情急煞車、留意擴散效應：（1）⚠️ SK 海力士 7/16 收盤暴跌 -11.53%（1,842,000 韓元）、美 ADR 亦重挫逾 13%；三星電子 -8.77%（255,000 韓元）——前段 HBM 行情暴漲後連 2 日急回吐；（2）📉 MU 7/16 續跌 -5.65% 收 $853.20、兩日累計 -13.2%；（3）🔗 台股記憶體鏈同步回吐：華邦電 7/16 -4.71% 收 172.00；（4）🎯 觀察：HBM4 中長線邏輯（NVIDIA Vera Rubin、AI 伺服器）未變，惟短線獲利了結情緒是否擴散至代工/AI 晶片鏈為本週最大風險——TSMC 法說會已證實 AI 需求「比預期更強」、基本面與籌碼面暫時脫鉤。",
        "source": "KRX / Yahoo Finance API 2026-07-16", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "💰 漲價 5-10% 效應開始顯現：Q2 毛利率 67.7% 創高、法人看 2027 獲利上修循環啟動",
        "body": "定價權轉化為實際獲利：（1）💰 Q2 毛利率 67.7%（+740bps YoY 以上水準）部分反映先進製程漲價 5-10%（3nm 擴及 5/7nm）與產能滿載；三星跟進調漲 4/5nm 約 15%、行業定價權全面轉向代工廠；（2）📊 2nm 已貢獻晶圓營收 3%、5 座廠（新竹 2＋高雄 3）爬坡中——管理層指 H2 稀釋毛利率 3-4pp 屬爬坡必經、量產效益 2027 顯現；（3）🚀 CoWoS 2026 產能已被 NVIDIA/Broadcom 預訂一空、缺口延續至 2027 年底；（4）🎯 法說會後預期分析師啟動新一輪目標價上調（現有密集區：美銀 $590、Barclays $625、Citi NT$3,800）。",
        "source": "TSMC 法說會 / TechNews / 經濟日報 2026-07-09 - 07-16", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 外資連 6 賣（7/16 -4,215 張）vs 投信自營轉買卡位：財報後外資動向為最大觀察點",
        "body": "內外資法說會前態度分歧：（1）📊 三大法人 7/16 合計賣超 2,625 張（T86 官方）：外資賣超 4,215 張——連 6 賣、近 5 個交易日累計 -34,852 張，賣壓仍低於 7/14 高峰（-12,416）；（2）⭐ 投信買超 528 張（終止賣超轉買）、自營買超 1,061 張（自行 +407、避險 +654）——內資法說會前先行卡位；（3）🎯 觀察：Q2 大超預期＋全年上修至 >40% 後、外資能否翻買為台股走勢最大變數；ADR 溢價收斂至 +7.0% 顯示外資已在 ADR 端先行調整；（4）⚠️ 地緣背景：美 232 條款關稅 25% + 輸中晶片轉運查驗、美伊緊張推升避險情緒。",
        "source": "TWSE T86 / SEMI 2026-07-16", "impact": "負面",
    },
]

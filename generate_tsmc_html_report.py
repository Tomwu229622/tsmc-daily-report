...
RELATIVE = {
    "date":           "2026-08-07",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/7 Fri 收盤）
    "tsmc_tw_1d":     +0.21,   # 2330 8/7 官方收 NT$2,370（+5 vs 8/6 NT$2,365、TWSE）
    "taiex_1d":       -0.38,   # 加權指數 8/7 收 44,225.91（-170.79、TWSE FMTQIK 官方）——2330 相對強勢
    "sox_1d":         +2.56,   # SOX 8/7 收 12,356.79（Yahoo、創本波新高、連 3 日站上 12,000）
    "tsmc_nyse_1d":   +0.44,   # TSM ADR 8/7 $420.04（+$1.84、連 2 日創本波收盤新高、Yahoo API）
    "tsmc_tw_5d":     -2.27,   # 5 日（NT$2,370 vs 7/31 官方收 NT$2,425）
    "taiex_5d":       +2.57,   # （44,225.91 vs 7/31 43,119.75）
    "tsmc_tw_1m":     -2.27,   # 月線（NT$2,370 vs 7/31 月末官方收 NT$2,425、與 5 日基準日重合）
    "taiex_1m":       +2.57,   # （44,225.91 vs 7/31 43,119.75、TWSE FMTQIK 官方、與 5 日基準日重合）
    "tsmc_tw_ytd":    +49.5,   # YTD 累計（NT$2,370 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +50.7,   # （44,225.91 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +67.7,   # SOX YTD（12,356.79 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/7 NYSE 收盤 & 8/7 TW 官方收盤計算）
    "adr_price":     420.04,   # TSM NYSE 8/7 收盤（+0.44%、Yahoo API 確認）
    "adr_parity":    367.56,   # 理論值 = 5 × NT$2,370 ÷ 32.24
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.24,   # 台幣匯率（8/10 晨 Yahoo TWD=X 32.24）
    "adr_premium_pct": "+14.28", # (420.04 - 367.56) / 367.56 × 100 = +14.28%（匯率 32.24）
    "note": 'ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/7 Fri 收盤：台股 NT$2,370（+0.21%、量 24,414 張 TWSE 口徑近月新低、市值 NT$61.4 兆）——小漲站回 5MA 2,366、重新站上全部均線；加權指數同日 -0.38% 收 44,225.91、2330 相對強勢；NYSE $420.04（+0.44%、連 2 日創本波收盤新高）——SOX +2.56% 收 12,356.79 創本波新高、NVDA 創史高 $223.96。📊 籌碼面（T86 官方）：8/7 外資回買 +642 張（終結 1 日賣超）、投信 +61（終結連 2 賣）、自營 +1,243、三大合計 +1,946 張——同步站買方；全市場 BFI82U 合計 -428.9 億（外資 -407.2 億大賣集中中小型與記憶體、華邦電外資 -43,331 張）——資金回流台積電等權值避風。📊 台美 ADR 溢價自 +13.98% 續擴至 +14.28%（$420.04 vs 理論值 $367.56、匯率 32.24）——連 3 日擴大，歷史上此型態多以台股補漲方式收斂、台指期夜盤大漲約 736 點即為訊號。✅ 外部：韓股記憶體風暴緩和——SK 8/7 -4.88% 後今晨 8/10 反彈 +2.46%、三星 +1.19%、KOSPI +1.00%（09:37 KST 快照）——二次崩跌未發生；台積電無 HBM/NAND 曝險、SOX 大漲即為分流定價延續。✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量（報價漲 10-20%）+ CoWoS 訂滿至 2027',
}

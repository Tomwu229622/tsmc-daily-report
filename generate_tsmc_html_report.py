...
STOCK = {
    # 台股 2330
    "tw_price":       "2,370",        # 台股 2330 最新確認收盤：8/3 Mon 官方收 NT$2,370（-55、-2.27% vs 7/31 漲停 NT$2,425、開 2,390／高 2,395／低 2,365、TWSE 確認）——漲停後首日獲利了結、量縮健康換手；大盤同日逆勢 +266.66 點（+0.62%）收 43,386.41——資金輪動：聯發科 +9.99%／華邦電 +10.00% 雙漲停、日月光 +9.91%
    "tw_change":      "-55",          # 8/3 Mon 收盤漲跌點數（-2.27%、TWSE 官方）
    "tw_change_pct":  "-2.27%",       # 8/3 Mon 漲停後首日拉回：開 2,390 → 盤中區間 2,365-2,395 窄幅整理收 2,370——跌幅遠小於韓股（KOSPI -5.12%）、屬獲利了結而非趨勢反轉；收盤價與 20MA 2,376 貼身攻防
    "tw_volume":      "35,210",       # 8/3 成交量（張、TWSE 口徑；成交額 NT$836.73 億、成交 174,489 筆——量縮至 7/31 漲停量 69,478 張的一半、獲利了結賣壓未見恐慌）
    "tw_open":        "2,390",        # 8/3 開盤（TWSE、小幅低開 -35）
    "tw_high":        "2,395",        # 8/3 盤中高（TWSE）
    "tw_low":         "2,365",        # 8/3 盤中低（TWSE、全日振幅僅 1.2%——漲停後波動快速收斂）
    "market_cap":     "NT$61.5 兆",   # 8/3 收盤市值（NT$2,370 基礎、約 259.3 億股）
    "pe_ratio":       "31.86x",       # 台股 P/E（TWSE 官方 BWIBBU 8/3、口徑為最新年度 EPS）
    "pb_ratio":       "10.43x",       # 台股 P/B（TWSE 官方 BWIBBU 8/3、以 115Q1 財報淨值計）
    "div_yield":      "0.93%",        # 殖利率（TWSE 官方 BWIBBU 8/3、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "406.11",       # 8/3 Mon 官方收盤 $406.11（+$1.86、+0.46% vs 7/31 $404.25、Yahoo Finance API 確認）——連 3 日收高、續創本輪反彈新高；SOX 8/3 +1.05% 收 11,430.35（自 6/22 史高 14,634.66 收斂至 -21.9%）
    "nyse_change":    "+1.86",        # 8/3 漲跌（自 7/31 $404.25；自 7/29 低點 $374.67 累計 +8.4%）
    "nyse_change_pct":"+0.46%",       # 8/3 美股大型科技暴漲：MSFT +4.93% 收 $487.65、META +6.02%、GOOGL +4.88%、AMZN +4.58% 收 $284.02——財報週後資金湧入 hyperscaler；NVDA +2.93% 收 $206.64、AMAT +2.08%、QCOM +2.68%；AAPL -1.78% 收 $303.42（市值單日蒸發約 $810 億）——AI 基建與消費電子分化；S&P 500 +1.48% 收 7,600.50、VIX 續降至 15.86
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-08-04 Tue（最新確認收盤：台股 2330 8/3 Mon 官方收 NT$2,370（-55、-2.27% vs 7/31 漲停 NT$2,425、開 2,390／高 2,395／低 2,365、量 35,210 張 TWSE 口徑、成交額 NT$836.73 億、成交 174,489 筆、TWSE 確認）；NYSE TSM 8/3 Mon 收 $406.11（+$1.86、+0.46% vs 7/31 $404.25、Yahoo API 確認、連 3 日收高））。📊 今日格局：V 型反轉後首日驗證過關、市場進入「輪動消化」模式——（1）✅ 台股 8/3 大盤逆勢 +266.66 點（+0.62%）收 43,386.41、2330 獲利了結 -2.27% 收 2,370（量縮至漲停量之半、全日振幅僅 1.2%）——資金明顯輪動：聯發科 +9.99% 漲停收 3,910、華邦電 +10.00% 漲停收 143、日月光 +9.91% 收 610（記憶體／封裝補漲）、聯電 -2.48%；（2）⚠️ 籌碼：2330 外資 8/3 轉賣 -9,717 張（7/31 曾大買 +13,780、一日翻多未能延續）、投信 +387 連 7 買、自營 -1,587 終結連 7 買、三大合計 -10,917 張（T86 官方）；全市場 BFI82U 賣超 -165.2 億（外資 -191.9 億、投信 +233.2 億續買、自營 -206.5 億）——個股獲利了結、資金未離場（投信續買＋大盤收紅）；（3）⚠️ 韓股 8/3 重挫：KOSPI -5.12% 收 6,257.45（媒體口徑）、三星 -8.76% 收 23.95 萬韓元、SK 海力士 -8.79% 收 156.7 萬——7/31 暴漲（+17.9%）大幅回吐、印證槓桿雙向劇震；今晨 8/4 續小跌（KOSPI 約 6,203、-0.9%，SK -1.79%、三星 -2.51%）跌勢趨緩；台股 8/3 僅 -2.27%（大盤收紅）相對淡定；（4）⭐ 美股 8/3 大型科技暴漲：MSFT +4.93% 收 $487.65、META +6.02% 收 $590.24、GOOGL +4.88% 收 $373.51、AMZN +4.58% 收 $284.02——財報週驗證後資金湧入 hyperscaler；NVDA +2.93% 收 $206.64（資料中心／Blackwell 液冷擴產需求、TradingKey）、TSM +0.46% 收 $406.11 連 3 高、SOX +1.05% 收 11,430.35（自史高 -21.9%）、S&P 500 +1.48% 收 7,600.50、VIX 15.86；AAPL -1.78% 收 $303.42（市值單日蒸發約 $810 億）——AI 基建 vs 消費電子分化加劇；（5）📅 AMD 今晚（8/4 美股盤後）公布 Q2 財報：市場估營收約 $11.2B（+46% YoY）、聚焦 MI350 出貨與 Helios 機櫃訂單——AI 晶片需求真實性檢驗、直接牽動 TSMC 先進製程情緒；（6）📅 232 條款截至 8/4 晨仍未查得官方公布（未能證實）；台美關稅談判傳 20%→~15%。📊 ADR 溢價擴大至 +11.04%（$406.11 vs 理論值 $365.74、匯率 32.40）——台股拉回＋ADR 續漲、ADR 領先訊號偏多。技術面（官方收盤計算至 8/3）：收 2,370 微幅跌破 20MA 2,376（-0.25%、貼身攻防）、仍站上 5MA 2,296／60MA 2,351／120MA 2,153——RSI 50.6 中性、MACD 綠柱續收斂至 -13.4（DIF -17.8 仍零軸下）、KDJ 金叉延續（K 52.3／D 39.2、J 78.4 偏高）；支撐 2,365（8/3 低）／2,351（60MA）／2,345（7/31 低）／2,296（5MA）、壓力 2,376（20MA）／2,395（8/3 高）／2,425（7/31 漲停）／2,445（7/22 高）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量；MSFT/AMZN/META 財報後續漲再證 AI CapEx 敘事。📈 今晨（8/4）盤中即時（09:08、TWSE MIS）：大盤 43,064.29（-0.74%、開 43,092.49／低 42,895.81）、2330 低開 2,335、盤中低 2,315（約 -2.3%、早盤量 8,087 張）——韓股 8/3 重挫的補跌壓力顯現、盤中一度失守 2,345（7/31 低）與 60MA 2,351（盤中數據、隨時變動、收盤前不作結論）。⭐ 今日（8/4）觀察：（1）2330 能否收復 20MA 2,376（V 型反轉的中期確認線）；（2）外資是否回到買方（7/31 大買後 8/3 轉賣、方向未定）；（3）今晚 AMD 財報；（4）韓股跌勢能否止穩；（5）232 條款與台美關稅進展。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性",
    # 一句話標題
    "headline":   "📊 漲停後健康換手：2330 8/3 量縮拉回 -2.27% 收 NT$2,370（量 35,210 張僅漲停日之半）、大盤逆勢 +0.62% 收 43,386.41——資金輪動：聯發科／華邦電雙漲停、日月光 +9.91%；外資轉賣 -9,717 張、投信連 7 買；韓股重挫 -5.12%（三星／SK -8.8%）台股相對淡定；美股大型科技暴漲（MSFT +4.93%、META +6.02%）、TSM +0.46% 收 $406.11 連 3 高、ADR 溢價擴大至 +11.0%——20MA 2,376 攻防與今晚 AMD 財報為雙驗證點",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 8/3：2330 收 NT$2,370（-55、-2.27%、開 2,390／高 2,395／低 2,365）——漲停後首日獲利了結、量縮至 35,210 張（僅 7/31 漲停量 69,478 之半）、全日振幅僅 1.2%——健康換手而非趨勢反轉；大盤逆勢 +266.66 點（+0.62%）收 43,386.41",
        "⭐ 資金輪動為 8/3 主軸：聯發科 +9.99% 漲停收 3,910、華邦電 +10.00% 漲停收 143、日月光 +9.91% 收 610（記憶體／封裝補漲）；聯電 -2.48%——市場未離場、僅自 2330 移轉至補漲股",
        "⚠️ 籌碼轉觀望：外資 8/3 轉賣 -9,717 張（7/31 大買 +13,780 未能延續、一日翻多 vs 趨勢翻多待驗證）、投信 +387 連 7 買、自營 -1,587 終結連 7 買；全市場 BFI82U 賣超 -165.2 億惟投信 +233.2 億續買",
        "⚠️ 韓股 8/3 重挫：KOSPI -5.12% 收 6,257.45、三星 -8.76%、SK 海力士 -8.79%——7/31 暴漲（+17.9%）大幅回吐、槓桿雙向劇震；今晨 8/4 跌勢趨緩（KOSPI 約 -0.9%）；台股 8/3 大盤收紅、相對淡定",
        "⭐ 美股 8/3 大型科技暴漲：MSFT +4.93% 收 $487.65、META +6.02%、GOOGL +4.88%、AMZN +4.58%——財報週驗證後資金湧入 hyperscaler；NVDA +2.93% 收 $206.64；AAPL -1.78%（市值單日蒸發約 $810 億）——AI 基建 vs 消費電子分化；TSM +0.46% 收 $406.11 連 3 高、SOX +1.05%、VIX 15.86",
        "💡 技術面：收 2,370 微幅跌破 20MA 2,376（-0.25% 貼身攻防）、仍站上 5MA 2,296／60MA 2,351／120MA 2,153；RSI 50.6 中性、MACD 綠柱續收斂至 -13.4、KDJ 金叉延續（K 52.3／D 39.2）——收復 2,376 與否為今日多空分水嶺",
        "📅 AMD 今晚（8/4 美股盤後）Q2 財報：市場估營收約 $11.2B（+46% YoY）、聚焦 MI350 出貨與 Helios 機櫃訂單——AI 晶片需求真實性檢驗、直接牽動 TSMC 情緒",
        "📊 ADR 溢價擴大至 +11.04%（$406.11 vs 理論值 $365.74、匯率 32.40）——ADR 領先訊號偏多；232 條款仍未公布（未能證實）、台美關稅談判傳 20%→~15%",
        "⚠️ 今晨（8/4 09:08 官方 MIS）：大盤 43,064.29（-0.74%）、2330 低開 2,335、盤中低 2,315（約 -2.3%）——韓股補跌壓力顯現、盤中一度失守 2,345（7/31 低）與 60MA 2,351；今日守 2,315／2,296（5MA）為關鍵（盤中數據、收盤前不作結論）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-08-04 Tue（最新確認收盤：台股 2330 8/3 Mon 官方收 NT$2,370（-55、-2.27% vs 7/31 漲停 NT$2,425、開 2,390／高 2,395／低 2,365、量 35,210 張、額 NT$836.73 億、174,489 筆、TWSE 確認）；NYSE TSM 8/3 收 $406.11（+0.46%、連 3 日收高、Yahoo API 確認））。📊 核心判讀：V 型反轉後的第一個交易日以「量縮拉回＋大盤收紅＋資金輪動」的健康型態過關——（1）✅ 2330 獲利了結 -2.27% 但量縮至漲停量之半、振幅僅 1.2%、未見恐慌賣壓；大盤逆勢 +0.62% 收 43,386.41 顯示資金未離場、僅輪動至補漲股（聯發科／華邦電雙漲停、日月光 +9.91%）；（2）⚠️ 最大保留點在籌碼：外資 7/31 大買 +13,780 張後 8/3 即轉賣 -9,717 張——「一日翻多」尚未升級為「趨勢翻多」、投信連 7 買與全市場投信 +233.2 億為主要多方力量；（3）⭐ 外部環境偏多：美股 8/3 大型科技暴漲（MSFT +4.93%、META +6.02%、GOOGL +4.88%、AMZN +4.58%——hyperscaler 財報驗證後的資金行情）、NVDA +2.93%、TSM ADR +0.46% 收 $406.11 連 3 高、ADR 溢價擴大至 +11.04%（領先訊號偏多）、VIX 降至 15.86；（4）⚠️ 韓股為最大外部風險：8/3 KOSPI -5.12%、三星／SK -8.8% 大幅回吐 7/31 暴漲、今晨 8/4 跌勢趨緩（-0.9%）——台股相對強勢（8/3 大盤收紅）但需防其失控外溢；（5）📅 今晚 AMD Q2 財報（估營收 $11.2B、+46% YoY、MI350/Helios 為焦點）為 AI 需求真實性的下一驗證點——上週 MSFT/AMZN 已連續驗證 CapEx 擴張、AMD 若再確認則補漲行情有基本面接棒。💡 技術面：2,370 vs 20MA 2,376 貼身攻防（-0.25%）、5MA 2,296／60MA 2,351 仍在腳下、RSI 50.6 中性、MACD 綠柱收斂至 -13.4、KDJ 金叉延續——收復 2,376 則挑戰 2,425／2,445、失守 2,345（7/31 低）則 V 型反轉需重新評估。✅ 基本面錨未變：Q2 營收 $40.20B（+33.7% YoY）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量。⭐ 今日（8/4）觀察：（1）2330 收復 20MA 2,376 與否；（2）外資動向（買回=趨勢翻多確認）；（3）今晚 AMD 財報；（4）韓股止穩與否；（5）232 條款／台美關稅。⚠️ 風險：外資翻賣延續、韓股二次殺盤外溢、AMD 財報不及預期、232 條款結果偏鷹、7 月大盤月線大幅收黑的中期壓力未解。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（8/4 Tue）關鍵觀察：（1）⚠️ 20MA 攻防：8/3 收 2,370 微幅跌破 20MA 2,376（-0.25%）——今日收復則短線結構維持修復軌道（上看 2,395／2,425／2,445）、失守 2,345（7/31 低）則 V 型反轉降級為跌深反彈；（2）⚠️ 外資方向未定：7/31 大買 +13,780 張、8/3 即轉賣 -9,717 張——連 2 日賣超將確認「一日翻多」失敗、回到 7 月中以來的調節格局；投信連 7 買為多方底氣；（3）📅 AMD 今晚美股盤後 Q2 財報（估營收 $11.2B、+46% YoY）：MI350 出貨與資料中心 GPU 指引若超預期、為 TSMC 先進製程／CoWoS 的直接增量驗證；不及預期則 AI 補漲行情降溫；（4）⚠️ 韓股 8/3 -5.12% 重挫後今晨僅 -0.9% 趨緩——若再度放大跌幅（三星／SK 續殺 >5%）將外溢亞股；（5）💱 匯率 32.40 持穩；232 條款懸而未決（未能證實有公布）＋台美關稅傳 20%→~15%——政策面仍是尾部變數。",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-08-03",       # 最新資料日（8/3 Mon、TWSE T86 官方）
    "foreign":  -9717,             # 外資 8/3 轉賣 -9,717 張（7/31 曾大買 +13,780 終結連 7 賣——一日翻多未能延續、方向待驗證）
    "trust":    387,               # 投信 8/3 買超 387 張（連 7 買、自 7,063 爆量後回歸常態量）
    "dealer":   -1587,             # 自營商 8/3 賣超 -1,587 張（終結連 7 買）；三大法人合計 -10,917 張
    # 近5日（由舊到新：7/28, 7/29, 7/30, 7/31, 8/3）；TWSE T86 口徑（張）
    "foreign_5d": [-14659, -11211, -3958, 13780, -9717], # 外資（連 7 賣→7/31 大買→8/3 轉賣）
    "trust_5d":   [284, 1652, 939, 7063, 387],           # 投信（連 7 買）
    "dealer_5d":  [5123, 4893, 1637, 2724, -1587],       # 自營（連 7 買後 8/3 轉賣）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       50.6,   "rsi_signal": "RSI(14) 50.6（官方收盤計算）——漲停日跳升至 54.6 後隨拉回小幅降至 50.6、正好落在中軸——多空均衡、等待方向選擇",
    "macd":         -13.4, "macd_signal": "MACD 綠柱自 -25.9 續收斂至 -13.4（DIF -17.8 < DEA -11.1、官方收盤計算）——連 2 日收斂、惟 DIF 仍在零軸下；綠柱歸零＋DIF 上穿零軸為中期修復確認路徑",   "macd_color": "#F44336",
    "kdj_k":        52.3,   "kdj_d": 39.2, "kdj_j": 78.4, "kdj_signal": "KDJ K(52.3)/D(39.2)/J(78.4)（官方收盤計算）——金叉延續、K 值站上 50；惟 J 78.4 偏高、短線動能透支中——K 值能否守 50 為延續訊號",
    "ma5":       "2,296",   # 5MA（官方收盤計算至 8/3）
    "ma20":      "2,376",   # 20MA（官方收盤計算；8/3 收 2,370 微幅跌破 -0.25%、貼身攻防）
    "ma60":      "2,351",  # 60MA（官方收盤計算；季線仍在腳下、中期架構未破壞）
    "ma120":     "2,153",  # 120MA（官方收盤計算至 8/3）
    "bb_upper":  "2,531",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,376",   # 布林中軌（20MA）
    "bb_lower":  "2,220",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "44,725",   # 5日均量（張、TWSE 口徑；8/3 量 35,210 低於均量——拉回量縮屬健康）
    "trend":    "📊 漲停後健康換手：2330 8/3 官方收 NT$2,370（-55、-2.27%、開 2,390／高 2,395／低 2,365、量 35,210 張僅漲停日之半、振幅 1.2%）——獲利了結而非恐慌、大盤同日逆勢 +0.62% 收 43,386.41、資金輪動至聯發科／華邦電（雙漲停）與日月光（+9.91%）。📊 技術指標（官方收盤計算）：RSI 50.6 回中軸、MACD 綠柱連 2 日收斂至 -13.4（DIF -17.8 仍零軸下）、KDJ 金叉延續（K 52.3 站上 50、J 78.4 偏高）——短線結構中性偏修復、中期空方架構（DIF 零軸下）未完全解除。⚠️ 籌碼保留點：外資 8/3 轉賣 -9,717 張（7/31 +13,780 未延續）、投信連 7 買。價位結構：收 2,370 微破 20MA 2,376（-0.25%）、仍站上 5MA 2,296／60MA 2,351／120MA 2,153。下一觀察：（1）收復 2,376（20MA／布林中軌）則挑戰 2,395／2,425／2,445；（2）失守 2,345（7/31 低）則 V 型反轉重新評估；（3）今晚 AMD 財報與外資動向。",
    "support":  "2,365 / 2,351 / 2,345 / 2,296 / 2,205 (8/3 低、60MA、7/31 低、5MA、7/30 收)",
    "resist":   "2,376 / 2,395 / 2,425 / 2,445 / 2,535 (20MA布林中軌、8/3 高、7/31 漲停、7/22 高、6/23 史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-03 止），請以看盤軟體為準；8/3 收 NT$2,370（-2.27%）微幅跌破 20MA 2,376、仍站上 5MA 2,296／60MA 2,351／120MA 2,153；RSI 50.6 中軸、MACD 綠柱收斂至 -13.4（DIF -17.8 零軸下）、KDJ 金叉延續（K52.3/D39.2/J78.4）；支撐 2,365 / 2,351 / 2,345 / 2,296、壓力 2,376 / 2,395 / 2,425 / 2,445 / 2,535；✅ 量縮拉回＋大盤收紅＋投信連 7 買 vs ⚠️ 外資轉賣＋20MA 得而復失＋韓股重挫——收復 2,376 與外資動向為今日雙檢核",
}
...
DERIVATIVES = {
    "date":          "2026-08-03",       # 最新資料日（8/3 Mon 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,368",            # 台積電期貨近月（估、現貨 8/3 收 2,370、拉回日常見小幅逆價差）
    "futures_oi":    "186,000",          # 未平倉量（口）（估、漲停後獲利了結部位進出）
    "futures_basis": "-2",               # 估小幅逆價差（自 7/31 估 +6 正價差回落）
    "call_oi":       "118,000",
    "put_oi":        "108,000",
    "pcr":           "0.92",             # Put/Call 比率（估、拉回日 Put 避險小增惟未見恐慌）
    "pcr_signal":    "中性（Put/Call 估 0.92；8/3 現貨量縮拉回 -2.27%、期權未見恐慌性避險；今晚 AMD 財報前市場觀望；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,296", "Call 36,000 / Put 13,000", "5MA、短線支撐"),
        ("2,345", "Call 35,000 / Put 12,000", "7/31 低、跳空缺口上緣"),
        ("2,365", "Call 34,000 / Put 11,000", "8/3 低"),
        ("2,376", "Call 36,000 / Put 10,000", "20MA／布林中軌、多空分水嶺"),
        ("2,400", "Call 44,000 / Put 14,000", "整數關卡、估最大痛苦"),
        ("2,425", "Call 41,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,445", "Call 37,000 / Put 8,000",  "7/22 高、反壓第一關"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（8/3 收 NT$2,370）；估最大痛苦 2,400、下方支撐 2,365（8/3 低）/ 2,351（60MA）/ 2,345（7/31 低）/ 2,296（5MA）、上方壓力 2,376（20MA）/ 2,395（8/3 高）/ 2,425（7/31 漲停）/ 2,445（7/22 高）/ 2,535（史高）；⚠️ 外資 8/3 轉賣 -9,717 張＋今晚 AMD 財報觀望 vs ✅ 量縮拉回＋ADR 連 3 高＋投信連 7 買；✅ 基本面錨未變（2nm Q3 放量、AI CapEx 財報連續驗證、全年上修 >40%）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-08-03 Mon TWSE 官方收盤；美股為 8/3 Mon 收盤、Yahoo Finance API 確認；韓股為 8/3 收盤、8/4 今晨小跌）
    ("設備", "ASML",    "艾司摩爾",    "1,642.52", "+0.83%", "✅ EUV 設備龍頭；8/3 續彈收 $1,642.52、自股災低點連續修復；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "518.21", "+2.08%", "📈 8/3 +2.08% 收 $518.21、連 3 日收高——股災跌幅大致收復；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "294.61", "+0.54%", "✅ 8/3 +0.54% 收 $294.61；7/30 財報後暴漲 +17.99% 的高檔整理；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "182.75", "-0.04%", "📊 8/3 持平收 $182.75；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","143.00","+10.00%", "⭐ 8/3 漲停收 143.0（TWSE 官方、7/31 +9.70% 後再鎖漲停）；記憶體補漲主軸、與 TSMC 合作 CUBE 特殊 DRAM 題材"),
    ("記憶", "000660.KS","SK 海力士",  "1,567,000 KRW", "-8.79%", "⚠️ 8/3 重挫 -8.79% 收 156.7 萬韓元——7/31 暴漲 +29.95% 大幅回吐（槓桿雙向劇震）；今晨 8/4 續小跌 -1.79% 至 153.9 萬、跌勢趨緩；Q2 財報創史高＋HBM4 量產出貨的基本面未變"),
    ("記憶", "MU",      "美光",        "829.50", "+0.79%", "📊 8/3 +0.79% 收 $829.50 止穩；HBM/DDR5 高波動、自 7/23 高 $990 仍 -16.2%"),
    ("封裝", "3711.TW", "日月光",      "610.00", "+9.91%", "⭐ 8/3 再飆 +9.91% 收 610（TWSE 官方、7/31 漲停 555 後連 2 日近漲停）——CoWoS 外包題材資金湧入、先進封裝為 8/3 輪動主軸"),
    ("封裝", "AMKR",    "Amkor",       "50.84",  "+1.94%", "📈 8/3 +1.94% 收 $50.84 連 4 日上漲；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材未變"),
    ("客戶", "NVDA",    "輝達",        "206.64", "+2.93%", "⭐ TSMC #1 客戶 22%/$33B、A16 首發客戶；8/3 +2.93% 收 $206.64——資料中心／Blackwell 液冷系統擴產需求帶動（TradingKey）、連 3 日收高"),
    ("客戶", "AAPL",    "蘋果",        "303.42", "-1.78%", "⚠️ #2 客戶 18%；8/3 續跌 -1.78% 收 $303.42、市值單日蒸發約 $810 億——財報指引保守＋CEO 交接（9/1 Ternus 接任）不確定性；Cook「晶片供應吃緊」說法對 TSMC 反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "392.23", "+0.76%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能；8/3 +0.76% 連 3 日收高"),
    ("客戶", "AMD",     "超微",        "484.64", "+1.78%", "📅 #4 客戶；8/3 +1.78% 收 $484.64；⭐ 今晚（8/4 美股盤後）Q2 財報：市場估營收 $11.2B（+46% YoY）、MI350 出貨與 Helios 機櫃為焦點——AI 需求真實性檢驗"),
    ("客戶", "2454.TW", "聯發科",      "3,910",  "+9.99%", "⭐ 手機 SoC 客戶 9%；8/3 再鎖漲停收 3,910（TWSE 官方、7/31 +9.89% 後連 2 日漲停）——自 7/28 跌停 V 型反轉、資金輪動最大受惠者"),
    ("競爭", "INTC",    "英特爾",      "91.00",  "+0.89%", "📈 8/3 +0.89% 收 $91.00 止穩整理；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "239,500 KRW", "-8.76%", "⚠️ 8/3 重挫 -8.76% 收 23.95 萬韓元——7/31 暴漲 +26.81% 大幅回吐；今晨 8/4 續跌 -2.51% 至 23.35 萬；韓股槓桿雙向劇震為亞股最大外部變數"),
    ("產業", "SOX",     "費城半導體指數","11,430.35", "+1.05%", "📈 8/3 +1.05% 收 11,430.35 連 3 日收高——自 6/22 史高 14,634.66 收斂至 -21.9%；美股大型科技（MSFT +4.93%、META +6.02%）財報驗證後資金行情為支撐"),
    ("處分後", "ARM",   "Arm Holdings","239.06", "-0.26%", "TSMC 4/29 全數出脫後（8/3 -0.26% 持平整理——高 Beta 雙向劇震）"),
]
...
RELATIVE = {
    "date":           "2026-08-03",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/3 Mon 收盤）
    "tsmc_tw_1d":     -2.27,   # 2330 8/3 官方收 NT$2,370（-55 vs 7/31 漲停 NT$2,425、TWSE）
    "taiex_1d":       +0.62,   # 加權指數 8/3 收 43,386.41（+266.66、TWSE 官方——2330 拉回但大盤逆勢收紅、資金輪動）
    "sox_1d":         +1.05,   # SOX 8/3 收 11,430.35（Yahoo、連 3 日收高）
    "tsmc_nyse_1d":   +0.46,   # TSM ADR 8/3 $406.11（連 3 日收高、續創本輪反彈新高）
    "tsmc_tw_5d":     +0.85,   # 5 日（NT$2,370 vs 7/27 官方收 NT$2,350）
    "taiex_5d":       -0.57,   # （43,386.41 vs 7/27 43,634.19）
    "tsmc_tw_1m":     -2.27,   # 月線（NT$2,370 vs 7/31 月末官方收 NT$2,425；8/3 為 8 月首個交易日、與 1 日同值）
    ...
    "adr_parity":    365.74,   # 理論值 = 5 × NT$2,370 ÷ 32.40
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.40,   # 台幣匯率（8/4 晨 Yahoo TWD=X）
    "adr_premium_pct": "+11.04", # (406.11 - 365.74) / 365.74 × 100 = +11.04%（匯率 32.40）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/3 Mon 收盤：台股 NT$2,370（-2.27%、量 35,210 張 TWSE 口徑、市值 NT$61.5 兆）——漲停後量縮健康換手、加權指數同日逆勢 +0.62% 收 43,386.41（資金輪動至聯發科／華邦電雙漲停、日月光 +9.91%）；NYSE $406.11（+0.46%、連 3 日收高）——SOX +1.05% 收 11,430.35。📊 籌碼面（T86 官方）：8/3 外資轉賣 -9,717 張（7/31 大買 +13,780 未延續）、投信 +387（連 7 買）、自營 -1,587、合計 -10,917 張；全市場 BFI82U 賣超 -165.2 億（外資 -191.9 億、投信 +233.2 億續買）。📊 台美 ADR 溢價擴大至 +11.04%（$406.11 vs 理論值 $365.74、匯率 32.40）——台股拉回＋ADR 續漲、ADR 領先訊號偏多。⭐ 外部：美股 8/3 大型科技暴漲（MSFT +4.93%、META +6.02%、GOOGL +4.88%、AMZN +4.58%）——AI CapEx 財報驗證後的資金行情；⚠️ 韓股 8/3 重挫 -5.12% 為最大外部變數。✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量 + CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "⚠️ 韓股 8/3 重挫：KOSPI -5.12%、三星／SK 海力士 -8.8%",
        "body": "韓股槓桿雙向劇震延續：（1）⚠️ KOSPI 8/3 重挫 -338 點（-5.12%）收 6,257.45——大幅回吐 7/31 史上最大單日反彈（+17.9%）；三星 -8.76% 收 23.95 萬韓元、SK 海力士 -8.79% 收 156.7 萬（Yahoo API 官方收盤）；（2）📊 賣壓來源：7/31 暴漲後獲利了結＋韓元走強＋外資與法人大額賣超＋中國半導體競爭力（CXMT／自製 DUV）疑慮再起；（3）✅ 今晨 8/4 跌勢趨緩：KOSPI 約 6,203（-0.9%）、SK -1.79%、三星 -2.51%；（4）🎯 對台股意涵：三星＋SK 占 KOSPI 權重逾半、其波動直接牽動亞股 AI 晶片評價情緒——惟台股 8/3 大盤逆勢收紅（+0.62%）、2330 僅 -2.27%，籌碼與資金結構明顯優於韓股。",
        "source": "Yahoo Finance API / Benzinga / Seoul Economic Daily 2026-08-03~04", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 台股資金輪動：聯發科／華邦電雙漲停、2330 量縮拉回",
        "body": "V 型反轉後首日輪動消化：（1）📊 2330 收 NT$2,370（-55、-2.27%）、量縮至 35,210 張（僅 7/31 漲停量之半）、振幅 1.2%——健康換手；（2）⭐ 大盤逆勢 +266.66 點（+0.62%）收 43,386.41：聯發科 +9.99% 再鎖漲停收 3,910（連 2 日漲停）、華邦電 +10.00% 漲停收 143、日月光 +9.91% 收 610——資金自權值股移轉至記憶體／封裝補漲股、市場未離場；（3）⚠️ 籌碼：外資轉賣 2330 -9,717 張（7/31 +13,780 未延續）、投信 +387 連 7 買、自營 -1,587；全市場 BFI82U -165.2 億惟投信 +233.2 億續買——「一日翻多 vs 趨勢翻多」今日見真章。",
        "source": "TWSE 官方（STOCK_DAY/T86/BFI82U）2026-08-03", "impact": "中性",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⭐ 美股大型科技 8/3 暴漲：MSFT +4.9%、META +6.0%、NVDA +2.9%",
        "body": "財報週驗證後的資金行情：（1）⭐ 8/3 hyperscaler 全面暴漲：MSFT +4.93% 收 $487.65、META +6.02% 收 $590.24、GOOGL +4.88% 收 $373.51、AMZN +4.58% 收 $284.02——上週 Azure +43%／AWS +37% 財報驗證後資金持續湧入 AI 基建陣營；S&P 500 +1.48% 收 7,600.50、VIX 降至 15.86；（2）📈 NVDA +2.93% 收 $206.64——市場聚焦資料中心加速建置與 Blackwell 液冷系統擴產需求、對 TSMC 先進封裝（CoWoS）與 2nm 訂單能見度為直接支撐；（3）⚠️ AAPL 逆勢 -1.78% 收 $303.42、市值單日蒸發約 $810 億——AI 基建 vs 消費電子分化加劇；（4）🎯 TSM ADR +0.46% 收 $406.11 連 3 日收高、ADR 溢價擴大至 +11.04%——領先訊號偏多。",
        "source": "Yahoo Finance API / TradingKey / ts2.tech 2026-08-03", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📅 AMD 今晚 Q2 財報：估營收 $11.2B（+46%）、MI350 為焦點",
        "body": "AI 需求真實性的下一驗證點：（1）📅 AMD（TSMC 第 4 大客戶、首發 2nm AI 晶片）於 8/4 美股盤後公布 Q2 財報：市場預期營收約 $11.2B（+46% YoY）——聚焦 MI350 出貨進度、Helios 機櫃訂單與資料中心 GPU 下半年指引；（2）📊 8/3 AMD +1.78% 收 $484.64 財報前觀望；（3）🎯 對 TSMC 意涵：MSFT／AMZN 已連續以 CapEx 數字驗證需求端、AMD 若再確認供給端出貨放量——「AI 回報質疑」殺盤的反駁鏈將補上最後一環、對 TSMC 先進製程／CoWoS 稼動率預期直接正面；反之若指引保守、補漲行情恐降溫。",
        "source": "TIKR / TradingKey / StocksToTrade 2026-08-03~04", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "✅ CoWoS 滿載：2026 供需缺口估年底自 20% 收斂至 10%",
        "body": "AI 需求結構性支撐的基本面錨（近期產業研究彙整、非單日新聞）：（1）📊 TSMC CoWoS 先進封裝產能持續吃緊：月產能估自 2025 年底約 75-80K 片擴至 2026 年底 120-140K 片、NVIDIA 約囊括六成產能；（2）⭐ 供需缺口估於 2026 年底前自約 20% 收斂至 10%——TSMC 同步規劃將部分產能外包日月光、矽品等 OSAT 夥伴（8/3 日月光 +9.91% 大漲與此題材呼應）；（3）🎯 對照本輪股災敘事：CoWoS 滿載＋外包擴散證明 AI 晶片實體需求未見降溫、為股價波動下的基本面錨點。",
        "source": "TrendForce / SemiWiki（產業研究彙整）", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 232 條款仍未公布；台美關稅談判傳 20%→15%",
        "body": "政策面懸而未決（本週持續觀察）：（1）📅 美 232 條款半導體調查結果原預期 7 月底出爐——截至 8/4 晨仍未查得官方公布（未能證實有結果）、不確定性延續；（2）⭐ 台美貿易協議談判持續、市場推測台灣對等關稅可能由現行 20% 下調至約 15%；（3）📊 台經院先前分析：先進製程受影響有限、TSMC 先進製程壟斷＋$165B 美國投資承諾為緩衝；（4）🎯 若 232 結果溫和＋關稅下調兌現、為反彈行情的第二波政策催化；反之為尾部風險。",
        "source": "工商時報 / 中時新聞網（背景）；8/4 晨查證未見新公布", "impact": "中性",
    },
]

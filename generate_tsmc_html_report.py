...
STOCK = {
    # 台股 2330
    "tw_price":       "2,390",        # 2026-06-24 Wed 官方收盤（-100，-4.02% vs 6/23 NT$2,490；補跌跟進 6/23 ADR 暴跌 -6.69%、台股加權 -1,057 點史上第 8 慘、開 2,435 高 2,445 低 2,390 收最低；6/25 Thu 今日交易中、料跟進 NYSE 6/24 +1.14% 反彈、官方收盤待 TWSE）
    "tw_change":      "-100",         # 6/24 漲跌點數（vs 6/23 NT$2,490）
    "tw_change_pct":  "-4.02%",       # 6/24 漲跌幅（補跌跟進 6/23 ADR 暴跌、半導體全面拋售）
    "tw_volume":      "12,657",       # 6/24 成交量（張，cmoney；跌勢中量偏低、官方以 TWSE 為準）
    "tw_open":        "2,435",        # 6/24 開盤（跳空走低）
    "tw_high":        "2,445",        # 6/24 盤中高
    "tw_low":         "2,390",        # 6/24 盤中低（收最低）
    "market_cap":     "NT$62.0 兆",   # 6/24 收盤市值（NT$2,390 基礎）
    ...
    # NYSE TSM ADR
    "nyse_price":     "441.35",       # 6/24 Wed 收盤（+$4.96，+1.14% vs 6/23 $436.39；自 6/23 暴跌反彈、受晶圓代工漲價 5-10% 利多帶動、日內 $432.58-$443.85、市值約 $2.05 兆、P/E ~31.6）
    "nyse_change":    "+4.96",        # 6/24 漲跌（vs 6/23 $436.39）
    "nyse_change_pct":"+1.14%",       # 6/24 NYSE 反彈（晶圓代工全面調漲 5-10%、估全年毛利率 +2pp，市場聚焦 6 月營收）
    "52w_high_nyse":  "476.79",       # 52 週/歷史高（區間）
    "52w_low_nyse":   "220.80",       # 52 週低（區間）
    "pe_nyse":        "~31.6x",       # NYSE USD 基礎 P/E（TTM，$441.35 基礎）
    "pb_nyse":        "~12.7x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-06-25 Thu（NYSE TSM 6/24 Wed 收盤 $441.35 與台股 2330 6/24 Wed 官方收 NT$2,390 為最新確認數據；台股 6/25 Thu 今日交易中、官方收盤請以 TWSE 為準）。⚠️⭐ 今日頭條——TSMC 全面調漲晶圓代工報價 5-10%、ADR 反彈但台股補跌：（1）⭐ TSMC 對 7nm 以下所有先進製程（佔營收約 75%）全面調漲 5-10% 報價、以平均 5% 估算可貢獻全年毛利率 +2pp 以上，消息帶動 NYSE TSM 6/24 Wed 反彈 +1.14% 收 $441.35（+$4.96 vs 6/23 $436.39、日內 $432.58-$443.85、市值約 $2.05 兆、P/E ~31.6）；（2）📉 台股 2330 6/24 Wed 補跌 -4.02% 收 NT$2,390（-100 vs 6/23 NT$2,490、開 2,435 高 2,445 低 2,390 收最低、量 12,657 張、市值 NT$62.0 兆）——跟進 6/23 ADR 暴跌 -6.69% 跳空走低、台股加權指數同日重挫 -1,057 點寫史上第 8 慘、半導體類股全面回檔；（3）6/25 Thu 今日台股交易中、料跟進 NYSE 6/24 反彈、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/24 ADR $441.35 + 台股 NT$2,390 計）：理論值 5×2,390÷31.05 = $384.86、溢價 (441.35-384.86)/384.86 = +14.68%（自 6/23 +8.83% 回升、反映 ADR 反彈而台股補跌、台股短線料補漲收斂）。⚠️ 漲價雙面刃——利多毛利率，但傳客戶（含 Apple）措手不及、恐促部分客戶調整下單量、要求更嚴格合約或轉向 UMC 等成熟製程替代。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片降低對 TSMC 依賴、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、+10% QoQ、+32% YoY、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +25% YoY 達 ~$975B；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆  |  ⚠️ 本日（6/25）關鍵觀察：（1）台股 2330 能否跟進 ADR 反彈、收復 NT$2,450；（2）ADR 溢價回升至 +14.68%、台股料補漲收斂；（3）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~31.6x、台股 P/E ~35.2x，留意 6 月營收 miss 風險、漲價客戶反彈、台幣升值與 ITC 初判雜訊",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ TSMC 全面調漲晶圓代工報價 5-10%（7nm 以下、佔營收 75%、估全年毛利率 +2pp）：NYSE TSM 6/24 反彈 +1.14% 收 $441.35；台股 2330 6/24 補跌 -4.02% 收 NT$2,390（跟進 6/23 ADR 暴跌、台股加權 -1,057 點史上第 8 慘）；6/25 料跟進反彈",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ TSMC 對 7nm 以下所有先進製程（佔營收約 75%）全面調漲 5-10% 報價、平均 5% 估貢獻全年毛利率 +2pp 以上",
        "📈 NYSE TSM 6/24 Wed 反彈 +1.14% 收 $441.35（+$4.96 vs 6/23 $436.39、市值約 $2.05 兆、P/E ~31.6）受漲價利多帶動",
        "📉 台股 2330 6/24 補跌 -4.02% 收 NT$2,390（-100、量 12,657 張）、跟進 6/23 ADR 暴跌、台股加權重挫 -1,057 點史上第 8 慘",
        "📊 ADR 溢價自 +8.83% 回升至 +14.68%（ADR 反彈而台股補跌）；6/25 今日台股料跟進反彈、官方收盤待 TWSE",
        "⚠️ 漲價雙面刃：傳客戶（含 Apple）措手不及、恐調整下單量或轉向 UMC 等替代；4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收（7 月上旬）關鍵",
        "🏆 中長線基底未變：NVIDIA 確認 #1 客戶/A16 首發 2027 + NVIDIA-TSMC AI 製程合作 + TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率",
        "⚠️ 風險續追蹤：TSMC 遭美 ITC 專利調查 6 月底待初判；Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-06-25 Thu（NYSE TSM 6/24 Wed 收盤 $441.35 與台股 2330 6/24 Wed 官方收 NT$2,390 為最新確認數據；台股 6/25 Thu 今日交易中、官方收盤請以 TWSE 為準）。⭐ 今日頭條——TSMC 全面調漲晶圓代工報價 5-10%、ADR 反彈但台股補跌：（1）⭐ TSMC 對 7nm 以下所有先進製程（佔營收約 75%）全面調漲 5-10% 報價、以平均 5% 估算可貢獻全年毛利率 +2pp 以上，消息帶動 NYSE TSM 6/24 Wed 反彈 +1.14% 收 $441.35（+$4.96 vs 6/23 $436.39、日內 $432.58-$443.85、市值約 $2.05 兆、P/E ~31.6）；（2）📉 台股 2330 6/24 Wed 補跌 -4.02% 收 NT$2,390（-100 vs 6/23 NT$2,490、開 2,435 高 2,445 低 2,390 收最低、量 12,657 張、市值 NT$62.0 兆）——跟進 6/23 ADR 暴跌 -6.69% 跳空走低、台股加權指數同日重挫 -1,057 點寫史上第 8 慘、半導體類股全面回檔；（3）6/25 Thu 今日台股交易中、料跟進 NYSE 6/24 反彈、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/24 ADR $441.35 + 台股 NT$2,390 計）：理論值 5×2,390÷31.05 = $384.86、溢價 (441.35-384.86)/384.86 = +14.68%（自 6/23 +8.83% 回升、反映 ADR 反彈而台股補跌、台股短線料補漲收斂）。⚠️ 漲價雙面刃：利多毛利率（估全年 +2pp 以上），但傳客戶（含 Apple）措手不及、恐促部分客戶調整下單量、要求更嚴格合約條款或將成熟製程轉向 UMC 等替代；6 月營收（7 月上旬公布）為關鍵 catalyst——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、惟即使僅小幅月增亦可能使 Q2 營收逼近指引（$39.0-40.2B）中值。⭐ 中長線結構性利多基底未變：（1）🤝 TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產、~2,000 就業、InFO/CoWoS、呼應 TSMC $165B 美國投資承諾）；（2）🏆 NVIDIA 確認 TSMC 第 1 大客戶（超越 Apple、22% / $33B）、為 A16（1.6nm）首發客戶 2027 量產；Apple 跳過 A16 直上 A14（1.4nm）；（3）⭐ NVIDIA-TSMC 深化 AI 導入晶圓廠合作（cuLitho 運算微影、缺陷檢測、工廠排程）、提升良率與產能；NVIDIA RTX Spark 超晶片（ARM Grace + Blackwell、1 petaflop）由 TSMC 代工；（4）⭐ TSMC 2nm 已量產、初期良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（5）📦 CoWoS 規劃 2026 底擴至 ~130K wpm 良率 >98%、CoPoS 面板級封裝互補緩解 AI 封裝瓶頸；（6）4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（7）⭐ 5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆；（8）2026 全球半導體市場估 +25% YoY 達 ~$975B、SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆。⚠️ 短期/中期下行風險：（1）⚠️ 漲價恐引發客戶反彈與訂單調整；（2）⚠️ 近期營收 miss 風險——4-5 月 +24% YoY 落後 Q2 ~35% 預期、6 月營收為關鍵驗證；（3）⚠️ 法律風險——TSMC 遭美 ITC 專利調查（Longitude Licensing & Marlin Semiconductor 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（4）⚠️ 客戶分散與競爭——Apple 同意與 Intel 合作在美生產晶片、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（5）⚠️ 估值警示——TSM P/E ~31.6x、台股 P/E ~35.2x；（6）台幣升值壓力（每升 1% 毛利率 -40bps）。基本面未變：Q1 2026 淨利 NT$572.5B（$18.2B、+58.3% YoY）、毛利率 66.2% 雙創史高；2025 全年 EPS NT$66.25。技術面（基於 6/24 官方收 NT$2,390）：5MA NT$2,420 / 20MA NT$2,400 / 60MA NT$2,355 / 120MA NT$2,250；下方支撐 NT$2,350 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535（史高）。32 位分析師全數看多、平均目標 NT$2,530 上行空間 +5.86%（自 NT$2,390、Susquehanna 6/23 上調 $575、待 7/16 法說會後重估）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（6/25 Thu）關鍵觀察：（1）⭐ TSMC 全面調漲晶圓代工報價 5-10%（7nm 以下、佔營收 75%、估全年毛利率 +2pp 以上）、帶動 NYSE TSM 6/24 反彈 +1.14% 收 $441.35；（2）📉 台股 2330 6/24 補跌 -4.02% 收 NT$2,390（量 12,657 張）、跟進 6/23 ADR 暴跌、台股加權 -1,057 點史上第 8 慘；6/25 今日料跟進 ADR 反彈、官方收盤待 TWSE；（3）📊 ADR 溢價自 +8.83% 回升至 +14.68%、台股短線料補漲收斂；（4）⚠️ 漲價雙面刃——傳客戶（含 Apple）措手不及、恐調整下單量或轉向 UMC 等替代；（5）⚠️ TSMC 4-5 月營收合計 +24% YoY 低於華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst；（6）📈 Susquehanna 將 TSM 目標價自 $500 上調至 $575；（7）🤝 TSMC-Amkor 簽 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）；（8）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、為 A16 首發客戶 2027 量產；NVIDIA-TSMC 深化 AI 製程合作；（9）⚠️ TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（10）⚠️ 風險續追蹤——Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（11）⭐ 2nm 量產良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（12）⭐ 4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（13）⚠️ 估值警示：TSM P/E ~31.6x、台股 P/E ~35.2x、留意 6 月營收 miss 風險與台幣升值；32 位分析師全數看多、平均目標 NT$2,530（上行 +5.86%、待法說會重估）；本日（6/25）關鍵：台股能否跟進 ADR 反彈收復 NT$2,450；下方支撐 NT$2,350 / NT$2,310；上方壓力 NT$2,490 / NT$2,510 / NT$2,535 史高",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-06-24",       # 最新資料日（6/24 Wed 為官方最後交易日、6/25 今日盤後待公布；6/24 三大法人為估算、官方以 TWSE 盤後為準）
    "foreign":  -15000,             # 外資 6/24 估擴大賣超（補跌 -4.02%、跟進 6/23 ADR 暴跌帶動賣壓；待官方確認）
    "trust":    +800,               # 投信 6/24 估逢低小買 +800 張
    "dealer":   -500,               # 自營商 6/24 估賣超 -500 張
    # 近5日（由舊到新：6/17, 6/18, 6/22, 6/23, 6/24）；6/19 端午休市；均為估算
    "foreign_5d": [+4000, +12000, +18000, -9000, -15000],    # 外資 6/24 估擴大賣超
    "trust_5d":   [+800, +1500, +2000, +500, +800],          # 投信 6/24 估逢低小買
    "dealer_5d":  [+300, +600, +800, -200, -500],            # 自營 6/24 估賣超
    "foreign_ownership_pct": "74.8%",  # 外資持股比例（6/24 賣超後估降至 74.8%）
    "foreign_ownership_shares": "約 193.9 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       52.0,   "rsi_signal": "RSI 估自 66 回落至 52（台股 6/24 補跌 -4.02%），退出超買、回落中軸附近、動能轉弱；⚠️ 6/25 留意 NYSE 6/24 反彈 +1.14% 能否帶動台股 RSI 回穩",
    "macd":         0.15,  "macd_signal": "MACD 估自 +0.40 收斂至 +0.15、紅柱大幅縮短、瀕臨零軸、⚠️ 高檔死叉風險升高",   "macd_color": "#FF9800",
    "kdj_k":        45.0,   "kdj_d": 50.0, "kdj_j": 35.0, "kdj_signal": "KDJ K(45)/D(50)/J(35) 自高檔急回、6/24 補跌跌破中軸、高檔死叉確認、⚠️ 短線轉弱、留意是否落入超賣",
    "ma5":       "2,420",   # 5MA（6/17 2315, 6/18 2385, 6/22 2510, 6/23 2490, 6/24 2390）— 基於 6/24 官方收
    "ma20":      "2,400",   # 20MA
    "ma60":      "2,355",   # 60MA 緩升
    "ma120":     "2,250",
    "bb_upper":  "2,575",   # 布林上軌
    ...
    "bb_lower":  "2,225",   # 布林下軌
    "vol_ma5":  "30,000",   # 5日均量（張，估、基於 6/17-6/24 五日均）
    "trend":    "⭐ TSMC 全面調漲晶圓代工報價 5-10%、ADR 反彈但台股補跌：NYSE TSM 6/24 反彈 +1.14% 收 $441.35（受漲價利多帶動、市值約 $2.05 兆）；台股 2330 6/24 官方收 NT$2,390（-100、-4.02%、量 12,657 張）補跌跟進 6/23 ADR 暴跌、台股加權 -1,057 點史上第 8 慘、盤中開 2,435 高 2,445 低 2,390 收最低；6/25 今日台股交易中、料跟進 NYSE 6/24 反彈。📊 技術面（基於 6/24 官方收 NT$2,390）：RSI 自 66 回落至 52 退出超買、MACD +0.15 紅柱大幅縮短瀕臨零軸、KDJ 高檔死叉確認、收盤跌破 5MA NT$2,420、中期 60MA/120MA 趨勢仍偏多。下一觀察：（1）📈 台股能否跟進 ADR 反彈、收復 5MA NT$2,420 / NT$2,450；（2）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；（3）上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⭐ 漲價利多估全年毛利率 +2pp 以上、惟須留意客戶反彈與 6 月營收（7 月上旬、4-5 月 +24% YoY 落後 ~35% 預期）；（5）⚠️ 雜訊續追蹤：ITC 專利調查 6 月底初判、Apple-Intel 合作、Intel 18A-P 風險量產；（6）⭐ 中長線結構性利多基底未變：Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + 2nm 量產 70-80% 良率 + Susquehanna 目標升至 $575。",
    "support":  "2,350 / 2,310 / 2,250 / 2,200 (整數、前低、整數、整數)",
    "resist":   "2,450 / 2,490 / 2,510 / 2,535 (整數、6/23 收、6/22 收、史高)",
    "note": "技術指標數值為估算（基於 6/24 官方收 NT$2,390），請以看盤軟體即時數據為準；台股 6/24 補跌 -4.02%、NYSE 6/24 反彈 +1.14%、RSI 52 退出超買、KDJ 高檔死叉確認、MACD 紅柱大幅縮短瀕臨零軸；NT$2,350 為關鍵支撐、上方壓力 NT$2,450 / NT$2,490；6/25 今日台股料跟進 ADR 反彈、⭐ 漲價利多 vs ⚠️ 營收 miss 風險拉鋸",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-06-24",       # 最新資料日（6/24 Wed 收盤估、6/25 今日盤後待公布）
    "futures_price": "2,385",            # 台積電期貨近月（6/24 收盤附近、隨現貨補跌）
    "futures_oi":    "172,000",          # 未平倉量（口）（6/24 估、回檔中小幅減倉）
    "futures_basis": "-5",               # 6/24 估維持小幅逆價差（避險情緒）
    "call_oi":       "108,000",
    "put_oi":        "78,000",
    "pcr":           "0.72",             # Put/Call 比率（6/24 估自 0.66 回升至 0.72、補跌帶動避險買盤增、情緒轉謹慎）
    "pcr_signal":    "中性（Put/Call 6/24 估回升至 0.72、自前日 0.66 增加；反映台股補跌 -4.02% 帶動避險買盤增、情緒轉謹慎；惟漲價利多+ADR 反彈或限制跌幅）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、6/24 估）
    "key_strikes": [
        ("2,310", "Call 52,000 / Put 18,000", "前低支撐"),
        ("2,350", "Call 56,000 / Put 14,000", "整數支撐"),
        ("2,390", "Call 54,000 / Put 10,000", "6/24 收"),
    ...
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；6/24 PCR 自 0.66 回升至 0.72 反映台股補跌 -4.02% 帶動避險買盤增、情緒轉謹慎、期貨維持小幅逆價差；NT$2,400 為最大痛苦、下方支撐 NT$2,350、上方壓力 NT$2,450 / NT$2,490；⭐ 漲價利多+ADR 6/24 反彈 +1.14% 或限制跌幅、⚠️ 留意營收 miss 風險與客戶反彈；中長線結構性利多基底未變（TSMC-Amkor 美國封裝長約、NVIDIA 確認第 1 大客戶 + A16 首發、2nm 量產 70-80% 良率、Susquehanna 目標升至 $575）；⚠️ 雜訊續追蹤：ITC 專利調查 6 月底初判、Apple-Intel 合作",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-06-23 Tue 收盤、台股為 6/23 Tue 收盤；價格為估算）
    ("設備", "ASML",    "艾司摩爾",    "833.00", "-4.47%", "EUV 設備龍頭（6/23 半導體全面拋售跟跌；高 NA EUV 需求結構性多頭基底未變）"),
    ("設備", "AMAT",    "應用材料",    "500.00", "-4.94%", "6/23 半導體拋售重挫；TSMC EPIC Center $5B AI 合作 + 先進封裝設備需求中長線多頭"),
    ("設備", "LRCX",    "科林研發",    "999.00", "-5.04%", "蝕刻設備（6/23 半導體拋售重挫；半導體設備需求中長線創紀錄）"),
    ("設備", "KLAC",    "科磊",        "754.00", "-4.80%", "量測設備（6/23 半導體拋售跟跌；2nm 量產驅動）"),
    ("記憶", "000660.KS","SK 海力士",  "380,000 KRW", "-3.06%", "🤝 SK 海力士-TSMC 深化 HBM4 合作；6/23 亞股下挫跟跌；HBM4 需求結構性多頭"),
    ("記憶", "MU",      "美光",        "138.65", "-11.40%", "⚠️ HBM / DDR5（6/23 暴跌 -11.4% 為類股最弱、引領半導體拋售；惟 HBM4 需求中長線強勁）"),
    ("封裝", "3711.TW", "日月光",      "316",    "-1.25%", "CoWoS / 先進封裝（6/23 台股相對抗跌；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "48.00",  "-3.03%", "TSMC-Amkor 簽 10 年美國封裝長約、Peoria AZ $70 億 2028 投產；6/23 隨類股回吐部分漲幅（本波利多核心）"),
    ("客戶", "NVDA",    "輝達",        "264.50", "-3.50%", "🏆 確認 TSMC #1 客戶 22%、A16 首發客戶 2027 量產；6/23 隨 AI 股拋售回檔；NVIDIA-TSMC 深化 AI 製程合作"),
    ("客戶", "AAPL",    "蘋果",        "296.50", "-1.50%", "⚠️ #2 客戶 18%（6/23 相對抗跌）；Apple 同意與 Intel 合作在美生產晶片以降低對 TSMC 依賴；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "454.00", "-5.02%", "⚠️ #3 客戶（6/23 重挫；財報指引不如預期引爆類股 sell-the-news；custom XPU 中長線強勁）"),
    ("客戶", "AMD",     "超微",        "495.00", "-6.25%", "#4 客戶（6/23 重挫；MI350/MI450 GPU 搶市；傳接觸 Samsung 先進製程）"),
    ("客戶", "2454.TW", "聯發科",      "4,650",  "-1.48%", "手機 SoC 客戶 9%；6/23 台股相對抗跌；RTX Spark 由聯發科協同 NVIDIA 設計"),
    ("競爭", "INTC",    "英特爾",      "48.80",  "-2.01%", "Apple 將與 Intel 合作在美設計製造晶片；18A-P 進入風險量產（效能 +9%）；6/23 隨類股回檔；與 UMC 結盟挑戰 TSMC"),
    ("競爭", "005930.KS","三星電子",   "64,550 KRW", "-2.49%", "Samsung Foundry 6/23 亞股下挫跟跌；傳 Google/AMD/Tesla 接觸其先進製程；PLP 面板級封裝領先、與 TSMC CoPoS 競爭；HBM4E 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","250.50", "-4.02%", "TSMC 4/29 全數出脫後（6/23 隨類股回檔；RTX Spark ARM 架構中長線受惠）"),
]
...
NEWS = [
    {
        ...
        "title": "⭐ TSMC 全面調漲晶圓代工報價 5-10%（7nm 以下、佔營收 75%）：NYSE TSM 6/24 反彈 +1.14% 收 $441.35、台股 2330 6/24 補跌 -4.02% 收 NT$2,390",
        "body": "今日頭條——TSMC 啟動先進製程全面漲價、台美兩市分歧：（1）⭐ 多家媒體報導 TSMC 已通知客戶對 7nm 以下所有先進製程（佔總營收約 75%）全面調漲報價 5-10%，數月來陸續通知；以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（2）📈 漲價利多帶動 NYSE TSM 6/24 Wed 反彈 +1.14% 收 $441.35（+$4.96 vs 6/23 $436.39、日內 $432.58-$443.85、市值約 $2.05 兆、P/E ~31.6）；（3）📉 台股 2330 6/24 Wed 補跌 -4.02% 官方收 NT$2,390（-100 vs 6/23 NT$2,490、開 2,435 高 2,445 低 2,390 收最低、量 12,657 張、市值 NT$62.0 兆）——跟進 6/23 ADR 暴跌跳空走低、台股加權同日重挫 -1,057 點寫史上第 8 慘；（4）⚠️ 漲價雙面刃：傳客戶（含 Apple）措手不及、恐促部分客戶調整下單量或轉向 UMC 等成熟製程替代；（5）📊 ADR 溢價自 +8.83% 回升至 +14.68%；6/25 今日台股料跟進 ADR 反彈、官方收盤待 TWSE。",
        "source": "Investor's Business Daily / StockTwits / Yahoo Finance / Tom's Hardware 2026-06-24", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 近期營收疑慮續存：TSMC 4-5 月營收 +24% YoY 落後華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵；半導體類股 6/23 全面拋售",
        "body": "近期營收動能與類股波動為短線焦點：（1）⚠️ TSMC 4 月與 5 月合併營收年增合計約 +24% YoY，低於華爾街對 Q2 約 +35% 預期，引發近期營收 miss 疑慮；（2）🎯 6 月合併營收（7 月上旬公布）成為關鍵 catalyst——分析師指出即使僅小幅月增，也可能使 Q2 營收逼近公司指引（$39.0-40.2B）中值；（3）📉 半導體類股 6/23 全面拋售（NYSE TSM 暴跌 -6.69%、Micron -11.4%、AMD -6.25%、AVGO -5.02%、設備股跌 4.5-5%、單日逾千億美元市值蒸發），由 Broadcom 指引不如預期 sell-the-news + 美債殖利率上行共振觸發、6/24 部分回穩；（4）✅ 緩解觀點：5 月營收 NT$4,169.75 億（+30.1% YoY）創單月新高、1-5 月累計 NT$1.96 兆，公司被認為 well-positioned 達成 Q2 指引；（5）⭐ 中長線需求未變：2026 全球半導體市場估 +25% YoY 達 ~$975B、AI 晶片市場估 ~$500B、Hyperscaler 2026 AI CapEx ~$600B+。",
        "source": "ts2.tech / Seeking Alpha / GuruFocus / TradingKey 2026-06-23 - 06-24", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ TSMC 4-5 月營收 +24% YoY 落後華爾街 ~35% 預期、近期營收恐 miss；6 月營收（7 月上旬）為關鍵 catalyst",
        "body": "近期營收動能成為市場焦點、為 6/23 拉回主因之一：（1）⚠️ TSMC 4 月與 5 月合併營收年增合計約 +24% YoY，低於華爾街對 Q2 約 +35% 的預期，引發近期營收 miss 的疑慮；（2）🎯 6 月合併營收（7 月上旬公布）成為關鍵 catalyst——分析師指出，即使僅小幅月增，也可能使 Q2 營收逼近公司指引（$39.0-40.2B）中值；（3）✅ 緩解觀點：5 月營收 NT$4,169.75 億（+30.1% YoY）已創單月新高、1-5 月累計 NT$1.96 兆（+30% YoY），公司被認為 well-positioned 達成 Q2 指引；（4）⭐ 中長線需求未變：AI 資料中心晶片需求結構性強勁、2026 全球半導體市場估 +25% YoY 達 ~$975B、AI 晶片市場估 ~$500B；（5）🎯 觀察：7 月上旬 6 月營收 + 7/16 Q2 法說會將為短線方向關鍵。",
        "source": "ts2.tech / Seeking Alpha / Investing.com 2026-06-23", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 逆勢利多：Susquehanna 將 TSM 目標價自 $500 上調至 $575、續看好 AI 結構性需求",
        "body": "在半導體類股全面拋售之際、分析師逆勢上調目標價：（1）📈 Susquehanna 將 TSMC（TSM）目標價自 $500 大幅上調至 $575、反映對 AI 結構性需求與 TSMC 製程領先的長期信心；（2）⭐ 相對 6/23 收盤 $436.39，新目標隱含逾 +30% 上行空間；（3）✅ 多數分析師維持「強烈買入」共識——32 位分析師全數看多、0 位賣出；（4）⭐ 看多核心邏輯：NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）+ A16（1.6nm）首發 2027 量產、TSMC-Amkor 美國先進封裝長約、2nm 量產良率 70-80% 領先全球、CoWoS/CoPoS 先進封裝擴產緩解 AI 瓶頸；（5）⚠️ 惟須留意估值仍在 5 年 90%+ 高位、近期營收 miss 風險與半導體類股波動。",
        "source": "Susquehanna / Yahoo Finance / ts2.tech 2026-06-23", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、A16 首發 2027 量產；NVIDIA-TSMC 深化 AI 導入晶圓廠合作、RTX Spark 超晶片由 TSMC 代工",
        "body": "NVIDIA 與 TSMC 雙向綁定持續深化、為中長線核心利多：（1）🏆 NVIDIA CEO 黃仁勳確認 NVIDIA 已超越 Apple 成為 TSMC 第 1 大客戶（約 22% / $33B）、為次世代 A16（1.6nm）製程首發客戶 2027 量產；Apple 跳過 A16 直上 A14（1.4nm）；（2）⭐ NVIDIA-TSMC 於 GTC Taipei 宣布深化合作、將 AI 與加速運算直接導入晶圓製造——涵蓋運算微影（cuLitho）、缺陷檢測、工廠排程等流程，提升良率與產能；（3）🚀 NVIDIA RTX Spark 超晶片（20 核 ARM Grace CPU + Blackwell GPU、達 1 petaflop AI 算力）由 TSMC 代工、已搭載於 Microsoft 新款 Surface Laptop Ultra；（4）💰 NVIDIA 6 月完成約 $245-250 億優先無擔保債券發行（到期 2028-2056）、強化 AI 基建彈藥；（5）⭐ 其他客戶 AMD（MI350/MI450）、Broadcom（custom XPU）、Qualcomm（Snapdragon C）、MediaTek 仍為重要需求來源。",
        "source": "CNBC / eWeek / Tom's Hardware / eeNews / SEC 8-K 2026-06-15 - 06-23", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ Apple 同意與 Intel 合作在美生產晶片、降低對 TSMC 依賴；Google/AMD/Tesla 接觸 Samsung；Intel 18A-P 進入風險量產",
        "body": "客戶分散與競爭雜訊升溫、為 TSMC 中期觀察重點：（1）⚠️ 多家媒體報導 Apple 已同意與 Intel 合作在美國本土製造晶片、以降低對 TSMC 的依賴；（2）⚠️ Google、AMD、Tesla 亦傳出積極接觸 Samsung Foundry 先進製程、尋求第二供應來源；Intel 18A-P 進入風險量產（同功耗效能 +9%、或同效能省電 18%）並與台灣 UMC 結盟挑戰 TSMC；Qualcomm 推 Snapdragon C 平台搶攻 sub-$600 Windows 筆電；（3）✅ 緩解因素：TSMC 在 N3/N2/A16 先進製程具明顯技術代差與良率領先、Apple 仍跳過 A16 直上 TSMC A14（1.4nm）、客戶分散多屬成熟/特定產品線；（4）🎯 觀察：客戶自研 ASIC 與雙供應策略下，TSMC 最先進節點訂單能見度仍滿載、惟須留意成熟製程市占與長期客戶集中度變化。",
        "source": "Reuters / CNBC / Bloomberg / ts2.tech 2026-06-18 - 06-23", "impact": "負面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⚠️ TSMC 遭美 ITC 專利調查、6 月底預計初判；恐對含關鍵 AI 加速器技術晶片發布美國進口禁令",
        "body": "潛在法律與供應鏈風險浮現、須密切追蹤：（1）⚠️ 美國國際貿易委員會（USITC）正對 TSMC 展開調查，源於授權公司 Longitude Licensing 與 Marlin Semiconductor 針對其先進製程節點提出的專利侵權控訴；（2）⚠️ 預計 6 月底作出初步裁決（preliminary ruling）——若不利於 TSMC，理論上可能對使用關鍵 AI 加速器技術製造的晶片發布美國進口禁令（import ban）；（3）⭐ 影響評估：此類 ITC 案件多以授權金和解收場、實際全面進口禁令機率不高，但涉及 AI 加速器核心技術、若裁定不利恐衝擊 NVIDIA/AMD 等客戶供應鏈與市場情緒；（4）✅ TSMC 具龐大專利組合與法務資源、過往類似訴訟多能妥善處理；（5）🎯 觀察：留意 6 月底/7 月初 ITC 初判結果與 TSMC 回應，為短期事件型風險。",
        "source": "USITC / Reuters / Bloomberg / ts2.tech 2026-06-18 - 06-23", "impact": "負面",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,420",        # 台股 2330 最新確認收盤：7/14 Tue 官方收 NT$2,420（-20、-0.82% vs 7/13 NT$2,440、TWSE）——法說會前觀望+獲利了結；加權指數同日重挫 -642.57 點（-1.42%）收 44,737.95、2330 相對抗跌惟失守 5MA
    "tw_change":      "-20",          # 7/14 Tue 收盤漲跌點數（-0.82%、TWSE 官方）
    "tw_change_pct":  "-0.82%",       # 7/14 Tue 收盤漲跌幅（大盤 -1.42% 下相對抗跌）
    "tw_volume":      "42,857",       # 7/14 成交量（張、TWSE 口徑含盤後零股鉅額；成交額 NT$1,033.71 億、均價 2,412、成交 275,423 筆明顯放大）
    "tw_open":        "2,410",        # 7/14 開盤（TWSE）
    "tw_high":        "2,430",        # 7/14 盤中高（TWSE）
    "tw_low":         "2,390",        # 7/14 盤中低（TWSE）
    "market_cap":     "NT$62.8 兆",   # 7/14 收盤市值（NT$2,420 基礎、約 259.3 億股）
    "pe_ratio":       "~32.1x",       # 台股 P/E（TTM，NT$2,420 ÷ 近四季 EPS ~75.5、statementdog）
    "pb_ratio":       "~18.1x",       # 台股 P/B（NT$2,420 ÷ 每股淨值 ~133.4、statementdog）
    "div_yield":      "~0.62%",       # 殖利率（NT$2,420 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "420.39",       # 7/14 Tue 官方收盤 $420.39（-$1.19、-0.28% vs 7/13 $421.58、日內 $418.86-430.87、量 1,428 萬股、Yahoo Finance API）——法說會前連 3 日小跌；⚡ 同日 SOX +2.54% 大反彈（NVDA +4.06%、MU +4.92%）、TSM 明顯落後屬法說會前觀望
    "nyse_change":    "-1.19",        # 7/14 漲跌（自 7/13 $421.58）
    "nyse_change_pct":"-0.28%",       # 7/14 NYSE 小跌、法說會前觀望
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~35.7x",       # NYSE USD 基礎 P/E（TTM 估，$420.39 基礎）
    "pb_nyse":        "~17.5x",
    "beta":           "1.18",
    "div_next":       "下次配息預計 2026-07",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-07-16（Q2 2026 財報，倒數 1 天）",
    # 資料日期備註
    "data_note":      "報告日期：2026-07-15 Wed（最新確認收盤：台股 2330 7/14 Tue 官方收 NT$2,420（-20、-0.82% vs 7/13 NT$2,440、開 2,410／高 2,430／低 2,390、量 42,857 張（TWSE 口徑）、成交額 NT$1,033.71 億、均價 2,412、成交 275,423 筆、TWSE）——法說會前觀望+獲利了結、加權指數同日重挫 -642.57 點（-1.42%）收 44,737.95、2330 相對抗跌惟失守 5MA ~2,436；NYSE TSM 7/14 Tue 收 $420.39（-$1.19、-0.28% vs 7/13 $421.58、日內 $418.86-430.87、Yahoo Finance）——法說會前連 3 日小跌、⚡ 同日 SOX +2.54% 大反彈（NVDA +4.06% 收 $211.80、MU +4.92%、AMAT +3.53%、AMKR +6.27%、INTC +4.50%）TSM 明顯落後屬觀望）。🎯🎯 今日最重要：7/16 Q2 2026 財報法說會倒數 1 天（週三 14:00）——公布 Q2 財報 + Q3 展望；6 月營收 NT$4,426.80 億（+67.9% YoY、單月新高）已確認 Q2 約 NT$1.27 兆達成美元指引 $39.0-40.2B；法人估 Q2 毛利率挑戰 70%（指引 65.5-67.5%）、獲利年增 +50%+ 可望連 5 季創高、關注是否上修全年財測（現行營收成長 >30%）與 2027-28 資本支出。📊 三大法人 7/14（T86 官方）：外資賣超 12,416 張（連 4 賣、4 日累計 -31,365 張、賣壓再擴大）、投信買超 1,841 張、自營買超 2,570 張（自行 +493、避險 +2,077）、合計賣超 8,005 張。📊 台美 ADR 溢價約 +11.7%（ADR $420.39 vs 理論值 $376.46、匯率 32.14）。💰 漲價題材：台積電通知 NVIDIA/Apple/AMD/高通等調漲晶圓報價 5-10%（3nm 擴及 5/7nm）、三星跟進調漲 4/5nm 約 15%——代工定價權確認；CoWoS 2026 產能估 12.5 萬片/月（+20-30%）已被 NVIDIA/Broadcom 預訂一空、缺口延續至 2027 年底。🚀 分析師續挺：Citi NT$3,800（自 NT$2,420 具 +57.0% 空間）、Barclays $625（自 $420.39 具 +48.7%）、Nomura NT$3,425（+41.5%）、共識強力買入。⚠️ 風險：外資連 4 賣且擴大、法說會前後劇烈波動、美 232 條款半導體關稅 25% + 輸中晶片轉運須經美第三方實驗室查驗（SEMI 致函反對）、Intel 18A 量產競爭、估值不低（台股 P/E ~32.1x TTM、TSM ~35.7x）、台幣 ~32.14。技術面（TWSE 官方收盤計算）：7/14 失守 5MA ~2,436 與 20MA ~2,426、MACD 柱轉負（-4.8）、KDJ J 值 22 近超賣、RSI 51.9 守中軸；支撐 2,390（7/14 低）／2,366／2,336（布林下軌）、壓力 5MA ~2,436／2,480／2,535 史高。⭐ 中長線基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）、2nm 量產 70-80% 良率、TSMC-Amkor 美國封裝長約、全先進製程漲價 5-10%（估毛利率 +2pp）、4 大 CSP 2026 AI CapEx $725B（+77% YoY）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "🎯 7/16 Q2 法說會倒數 1 天：6 月營收創高已確認 Q2 達標、法人估毛利率挑戰 70%、獲利 +50%+ 連 5 季創高；台股 2330 7/14 收 NT$2,420（-0.82%）於大盤重挫 -1.42% 中相對抗跌、惟外資連 4 賣（7/14 -12,416 張）；NYSE TSM 7/14 收 $420.39（-0.28%）、同日 SOX +2.54% 大反彈（NVDA +4.06%）——今日（7/15）台股可望跟進隔夜美股半導體反彈",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🎯 最重要：7/16 週三 14:00 Q2 2026 財報法說會倒數 1 天——6 月營收 NT$4,426.80 億（+67.9% YoY）已確認 Q2 約 NT$1.27 兆達成指引 $39.0-40.2B；分析師估 Q2 營收 ~$40B（+32% YoY）、獲利 +50%+ 連 5 季創高、毛利率挑戰 70%（指引 65.5-67.5%）、關注上修全年財測（現行 >30%）",
        "📉 台股 2330 7/14 Tue 官方收 NT$2,420（-20、-0.82%、開 2,410／高 2,430／低 2,390、量 42,857 張 TWSE 口徑、額 NT$1,033.71 億）——加權指數同日重挫 -642.57 點（-1.42%）收 44,737.95，2330 相對抗跌、惟失守 5MA ~2,436 與 20MA ~2,426",
        "⚡ 隔夜美股半導體大反彈：SOX +2.54% 收 12,661.93、NVDA +4.06% 收 $211.80、MU +4.92%、AMAT +3.53%、AMKR +6.27%、INTC +4.50%——NYSE TSM 7/14 收 $420.39（-0.28%）明顯落後屬法說會前觀望、今日（7/15）台股可望跟進反彈",
        "⚠️ 籌碼轉弱：三大法人 7/14 合計賣超 8,005 張——外資賣超 12,416 張（連 4 賣、4 日累計 -31,365 張、較 7/13 -2,045 大幅擴大）；投信買超 1,841 張、自營買超 2,570 張（自行 +493、避險 +2,077）持續對沖",
        ...
        "📊 台美 ADR 溢價 +11.7%（$420.39 vs 理論值 $376.46、匯率 32.14）；記憶體鏈暴衝：SK 海力士 7/15 韓股盤中大漲逾 +11%、三星 +6%（AI 記憶體/HBM 需求）",
        "🚀 分析師續挺：Citi NT$3,800（自 NT$2,420 具 +57.0% 空間）、Barclays $625（自 $420.39 具 +48.7%）、Nomura NT$3,425（+41.5%）、共識強力買入（平均目標 $490.34、+16.6%）",
        "⚠️ 風險續追蹤：外資連 4 賣且擴大；法說會前後恐劇烈波動；美 232 條款半導體關稅 25% + 輸中晶片轉運查驗收緊（SEMI 致函反對）；Intel 18A 量產競爭；估值不低（台股 P/E ~32.1x、TSM ~35.7x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-15 Wed（最新確認收盤：台股 2330 7/14 Tue 官方收 NT$2,420（-20、-0.82% vs 7/13 NT$2,440、開 2,410／高 2,430／低 2,390、量 42,857 張 TWSE 口徑、成交額 NT$1,033.71 億、均價 2,412、TWSE 確認）；NYSE TSM 7/14 Tue 收 $420.39（-$1.19、-0.28% vs 7/13 $421.58、日內 $418.86-430.87））。⭐⭐ 今日最重要：7/16 Q2 2026 財報法說會倒數 1 天（週三 14:00 文華東方）——公布 Q2 財報 + Q3 展望；6 月營收 NT$4,426.80 億（+67.9% YoY、創單月新高）已確認 Q2 約 NT$1.27 兆達成美元指引 $39.0-40.2B；分析師估 Q2 營收 ~$40B（+32% YoY）、獲利年增 +50%+ 可望連續第 5 季創獲利新高；法人估 Q2 毛利率挑戰 70%（指引 65.5-67.5%）；關注是否上修全年財測（現行 >30%）、2027-28 資本支出（Citi 估 $75-80B）、CoWoS/2nm 擴產進度；法說會前後恐劇烈波動。⭐ 今日重點：（1）📉 台股 7/14 收 NT$2,420（-0.82%）——加權指數重挫 -642.57 點（-1.42%）收 44,737.95，2330 相對抗跌、惟失守 5MA ~2,436 與 20MA ~2,426、量放大至 42,857 張（TWSE 口徑）屬法說會前調節；（2）📊 NYSE TSM 7/14 收 $420.39（-0.28%）法說會前連 3 日小跌；⚡ 同日美股半導體大反彈：SOX +2.54% 收 12,661.93、NVDA +4.06% 收 $211.80、MU +4.92%、AMKR +6.27%、INTC +4.50%——TSM 明顯落後屬觀望、今日台股可望跟進反彈；（3）⚠️ 籌碼轉弱：三大法人 7/14 合計賣超 8,005 張——外資賣超 12,416 張（連 4 賣、4 日累計 -31,365 張、較 7/13 -2,045 大幅擴大）；投信買超 1,841 張、自營買超 2,570 張持續對沖；（4）📊 台美 ADR 溢價 +11.7%（$420.39 vs 理論值 $376.46、匯率 32.14）；記憶體鏈暴衝：SK 海力士 7/15 韓股盤中大漲逾 +11%、三星 +6%；（5）💰 漲價題材：台積電通知 NVIDIA/Apple/AMD/高通調漲晶圓報價 5-10%（3nm 擴及 5/7nm）、三星跟進漲 4/5nm 約 15%；CoWoS 2026 產能 12.5 萬片/月已被 NVIDIA/Broadcom 預訂一空；（6）🚀 分析師續挺：Citi NT$3,800（+57.0% 空間）、Barclays $625（+48.7%）、Nomura NT$3,425（+41.5%）、共識強力買入（平均目標 $490.34、+16.6%）。技術面（TWSE 官方收盤計算）：5MA ~2,436 / 20MA ~2,426 / 60MA ~2,306；7/14 收 2,420 失守 5MA/20MA、MACD 柱轉負（-4.8）、KDJ J 值 22 近超賣、RSI 51.9 守中軸；支撐 2,390 / 2,366 / 2,336（布林下軌）、壓力 5MA ~2,436 / 2,480 / 2,535 史高。⚠️ 風險：外資連 4 賣擴大、法說會前後劇烈波動、美 232 關稅與輸中晶片轉運查驗收緊、Intel 18A 量產競爭、估值不低（台股 P/E ~32.1x TTM、TSM ~35.7x）、台幣 ~32.14（每升值 1% 毛利率 -40bps）。⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約、漲價 5-10% 定價權、4 大 CSP 2026 AI CapEx $725B。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/15 Wed）關鍵觀察：（1）🎯🎯 7/16 Q2 2026 財報法說會倒數 1 天（週三 14:00）——分析師估 Q2 營收 ~$40B（+32% YoY）、獲利 +50%+ 連 5 季創高、毛利率挑戰 70%（指引 65.5-67.5%）；法說會前後恐劇烈波動、留意法人調整部位；（2）📈 隔夜美股半導體大反彈（SOX +2.54%、NVDA +4.06%、MU +4.92%）+ SK 海力士 7/15 盤中 +11%，台股 2330 能否跟進反彈、收復 5MA ~2,436 / 20MA ~2,426；（3）📊 籌碼：外資連 4 賣且 7/14 擴大至 -12,416 張（4 日 -31,365 張）——法說會前能否收斂為關鍵；投信 +1,841、自營 +2,570 對沖；（4）⚠️ 技術面轉弱訊號：MACD 柱轉負（-4.8）、KDJ J 值 22 近超賣、7/14 失守 5MA/20MA，守穩 2,390／2,366 支撐為關鍵；（5）💱 台幣 ~32.14、美 232 條款關稅 + 輸中晶片轉運查驗收緊（SEMI 致函反對）、估值不低（台股 P/E ~32.1x、TSM ~35.7x）續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-14",       # 最新資料日（7/14 Tue、TWSE T86 官方）
    "foreign":  -12416,            # 外資 7/14 賣超 12,416 張（連 4 賣、4 日累計 -31,365 張、賣壓再擴大）
    "trust":    1841,              # 投信 7/14 買超 1,841 張（買盤明顯放大）
    "dealer":   2570,              # 自營商 7/14 買超 2,570 張（自行 +493、避險 +2,077）；三大法人合計 -8,005 張
    # 近5日（由舊到新：7/7, 7/8, 7/9, [7/10 颱風休市], 7/13）；Yahoo/T86 口徑（張）
    "foreign_5d": [-4155, -12749, 0, -2045, -12416],     # 外資（7/8, 7/9, [7/10 颱風休市], 7/13, 7/14）：7/14 賣壓再擴大
    "trust_5d":   [730, 43, 0, 530, 1841],              # 投信買超（7/10 休市計 0；7/14 買盤放大）
    "dealer_5d":  [468, 90, 0, 623, 2570],              # 自營（7/10 休市計 0；7/14 買超放大）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       51.9,   "rsi_signal": "RSI(14) 51.9（TWSE 官方收盤計算）——7/14 -0.82% 後仍守中軸 50 之上、中性、待 7/16 法說會定方向",
    "macd":         -4.8,  "macd_signal": "MACD 柱狀轉負 -4.8（DIF 27.1 跌破 DEA 31.8 死叉、TWSE 官方收盤計算）——短線動能轉弱、法說會前觀望；⚡ 隔夜美股半導體大反彈（SOX +2.54%）為今日反彈契機",   "macd_color": "#F44336",
    "kdj_k":        43.0,   "kdj_d": 54.0, "kdj_j": 22.0, "kdj_signal": "KDJ K(43)/D(54)/J(22)（官方收盤計算）——K 跌破 D 死叉、J 值 22 接近超賣區，短線偏弱惟已近超賣、留意法說會前反彈",
    "ma5":       "2,436",   # 5MA（TWSE 官方收盤計算，含 7/14 NT$2,420）
    "ma20":      "2,426",   # 20MA（TWSE 官方收盤計算）
    "ma60":      "2,306",  # 60MA（TWSE 官方收盤計算）
    "ma120":     "2,150",  # 120MA（估算）
    "bb_upper":  "2,517",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,426",   # 布林中軌（20MA）
    "bb_lower":  "2,336",  # 布林下軌（20MA-2σ）
    "vol_ma5":  "33,954",   # 5日均量（張、TWSE 口徑、含 7/14 官方量 42,857）
    "trend":    "📉 法說會前觀望、失守 5MA/20MA 但大盤重挫下相對抗跌：台股 2330 7/14 官方收 NT$2,420（-20、-0.82%、量 42,857 張 TWSE 口徑）——加權指數同日重挫 -642.57 點（-1.42%）收 44,737.95，2330 跌幅不到大盤六成屬相對抗跌；惟收盤失守 5MA（~2,436）與 20MA（~2,426）、量放大屬法說會前調節。⚡ 隔夜（7/14 美股）半導體大反彈：SOX +2.54% 收 12,661.93、NVDA +4.06% 收 $211.80、MU +4.92%、AMAT +3.53%、AMKR +6.27%——NYSE TSM 收 $420.39（-0.28%）明顯落後、屬法說會前觀望；SK 海力士 7/15 韓股盤中暴漲逾 +11%、AI 記憶體鏈噴出。📊 技術指標（TWSE 官方收盤計算）：RSI(14) 51.9 守中軸、MACD 柱轉負 -4.8（DIF 27.1 < DEA 31.8 死叉）、KDJ K(43)/D(54)/J(22) 死叉且 J 近超賣——短線動能轉弱但超賣訊號漸近。下一觀察：（1）🎯🎯 7/16 Q2 法說會倒數 1 天（週三 14:00）——法人估 Q2 毛利率挑戰 70%、獲利 +50%+ 連 5 季創高、關注上修全年財測，法說會前後恐劇烈波動；（2）今日（7/15）能否跟進隔夜美股反彈、收復 5MA ~2,436 / 20MA ~2,426；（3）守穩支撐 2,390（7/14 低）／20MA 帶 2,366／布林下軌 2,336；（4）外資連 4 賣（4 日 -31,365 張）能否收斂、投信自營對沖買超續航；（5）🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 漲價定價權為中長線支撐。",
    "support":  "2,390 / 2,366 / 2,336 (7/14 低、前 20MA 支撐帶、布林下軌)",
    "resist":   "2,436 / 2,480 / 2,535 (5MA+20MA 帶、前高、史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依 TWSE 官方收盤價計算（2026-07-14 止），請以看盤軟體為準；7/14 官方收 NT$2,420（-0.82%）失守 5MA ~2,436 / 20MA ~2,426、MACD 柱轉負、KDJ J 值 22 近超賣；下方支撐 2,390 / 2,366 / 2,336、上方壓力 2,436 / 2,480 / 2,535 史高；🎯 7/16 法說會倒數 1 天為最大變數；🚀 隔夜 SOX +2.54% 大反彈 + 漲價定價權 + 分析師續挺 vs ⚠️ 外資連 4 賣擴大 + 法說會前劇烈波動風險",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-14",       # 最新資料日（7/14 Tue 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,415",            # 台積電期貨近月（估、現貨 7/14 收 NT$2,420 附近小幅逆價差、法說會前保守）
    "futures_oi":    "184,000",          # 未平倉量（口）（估）
    "futures_basis": "-5",               # 估小幅逆價差（法說會前避險偏保守）
    "call_oi":       "120,000",
    "put_oi":        "80,000",
    "pcr":           "0.65",             # Put/Call 比率（估、法說會前避險升溫、中性偏保守）
    "pcr_signal":    "中性（Put/Call 估 0.65；台股 7/14 官方收 NT$2,420（-0.82%）失守 5MA、7/16 法說會倒數 1 天避險需求升溫；⚡ 隔夜 SOX +2.54% 大反彈為今日反彈契機；⭐ Citi/Barclays/Nomura + 共識偏多為中長線支撐；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,420",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,336", "Call 55,000 / Put 15,000", "布林下軌、下方支撐"),
        ("2,366", "Call 52,000 / Put 12,000", "前 20MA 支撐帶"),
        ("2,390", "Call 50,000 / Put 10,000", "7/14 盤中低、支撐"),
        ("2,420", "Call 46,000 / Put 8,000",  "7/14 官方收、最大痛苦（估）"),
        ("2,436", "Call 42,000 / Put 6,000",  "5MA、短線壓力"),
        ("2,465", "Call 38,000 / Put 5,000",  "整數壓力帶"),
        ("2,500", "Call 32,000 / Put 3,500",  "整數壓力"),
        ("2,535", "Call 24,000 / Put 2,500",  "盤中史高"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/14 NT$2,420）；NT$2,420 為估最大痛苦、下方支撐 2,390 / 2,366 / 2,336（布林下軌）、上方壓力 5MA ~2,436 / 2,480 / 2,535 史高；🎯🎯 7/16 Q2 法說會（倒數 1 天）為最大變數、法說會前後恐劇烈波動；🚀 隔夜 SOX +2.54% 大反彈 + 漲價 5-10%（三星跟進 15%）定價權 + CoWoS 訂滿至 2027 偏多 vs ⚠️ 外資連 4 賣（4 日 -31,365 張）擴大；中長線基底未變（NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-14 Tue 官方收盤；美股為 7/14 Tue 收盤（Yahoo Finance API）；韓股為 7/15 Wed 盤中）
    ("設備", "ASML",    "艾司摩爾",    "1,775.64", "+2.87%", "EUV 設備龍頭（7/14 收、Yahoo）；高 NA EUV 需求強"),
    ("設備", "AMAT",    "應用材料",    "595.70", "+3.53%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（7/14 收大漲）"),
    ("設備", "LRCX",    "科林研發",    "346.10", "+4.90%", "蝕刻設備（7/14 收大漲）；WFE 全年展望上修"),
    ("設備", "KLAC",    "科磊",        "230.37", "+3.65%", "量測設備（7/14 收）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","164.50","-1.50%", "⭐ 與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/14 台股收盤、隨大盤回檔）"),
    ("記憶", "000660.KS","SK 海力士",  "2,132,000 KRW", "+11.45%", "🚀 7/15 盤中暴漲逾 +11%（AI 記憶體/HBM 需求噴出）；HBM4、與 NVIDIA 多年期合作"),
    ("記憶", "MU",      "美光",        "983.12", "+4.92%", "HBM / DDR5（7/14 收大漲）；AI 記憶體需求勁揚、HBM4 中長線強勁"),
    ("封裝", "3711.TW", "日月光",      "641.00", "-4.33%", "CoWoS / 先進封裝（7/14 台股收盤、隨大盤重挫回檔）"),
    ("封裝", "AMKR",    "Amkor",       "70.20",  "+6.27%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/14 收大漲）"),
    ("客戶", "NVDA",    "輝達",        "211.80", "+4.06%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/14 收 $211.80 大漲、領攻 AI 晶片股反彈"),
    ("客戶", "AAPL",    "蘋果",        "314.86", "-0.77%", "#2 客戶 18%；面臨 TSMC 漲價成本；跳過 A16 直上 A14（7/14 收）"),
    ("客戶", "AVGO",    "博通",        "389.11", "+1.32%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/14 收）"),
    ("客戶", "AMD",     "超微",        "548.13", "+2.57%", "#4 客戶；MI350/MI450、首發 2nm AI 晶片（7/14 收）"),
    ("客戶", "2454.TW", "聯發科",      "3,660",  "-4.31%", "手機 SoC 客戶 9%；面臨漲價成本（7/14 台股官方收盤、隨大盤重挫）"),
    ("競爭", "INTC",    "英特爾",      "107.76", "+4.50%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/14 收）"),
    ("競爭", "005930.KS","三星電子",   "279,250 KRW", "+6.18%", "7/15 盤中大漲；跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣"),
    ("產業", "SOX",     "費城半導體指數","12,661.93", "+2.54%", "7/14 大反彈；AI 晶片股多頭、7/16 起半導體財報季（TSMC 打頭陣）"),
    ("處分後", "ARM",   "Arm Holdings","281.17", "-5.96%", "TSMC 4/29 全數出脫後（7/14 收大跌、逆勢回檔）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/14 收 $420.39 具 +16.6% 上行空間；Citi 台股目標 NT$3,800、Barclays $625、Nomura NT$3,425；Goldman 7/1 移出 APAC Conviction List；Morningstar 17 位分析師強力買入）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+16.64%",  # 從 NYSE 7/14 收 $420.39 至平均目標 $490.34（共識強力買入；明日 7/16 法說會後重估）
}

# 財報倒計時
EARNINGS = {
    "next_date":   "2026-07-16",
    "quarter":     "Q2 2026（明日 2026-07-16 法說會，倒數 1 天）",
...
NEWS = [
    {
        ...
        "title": "📅 7/16 Q2 法說會倒數 1 天：法人估 Q2 營收 ~$40B（+32% YoY）、獲利 +50%+ 連 5 季創高、毛利率挑戰 70%",
        "body": "市場視為 AI 熱潮健康檢查、明日登場：（1）📅 TSMC 明日（7/16 週三 14:00）舉行 Q2 2026 財報法說會；分析師預估 Q2 營收約 $40B、年增約 +32%——6 月營收 NT$4,426.80 億（+67.9% YoY、單月新高）已確認達成指引 $39.0-40.2B；（2）📊 獲利估年增逾 +50%、可望連續第 5 季創獲利新高；法人聚焦 Q2 毛利率能否挑戰 70%（指引 65.5-67.5%）；（3）🎯 展望焦點：是否上修全年財測（現行營收成長 >30%）、2027-28 資本支出（Citi 估 $75-80B）、CoWoS/2nm 擴產進度；（4）⚠️ 法說會前後股價恐劇烈波動。",
        "source": "The Motley Fool / Yahoo Finance / 中央社 2026-07-13 - 07-14", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📉 台股 7/14 重挫 642 點、2330 收 NT$2,420（-0.82%）相對抗跌；隔夜美股半導體大反彈 SOX +2.54%",
        "body": "法說會前觀望、台美節奏分歧：（1）📉 台股加權指數 7/14 重挫 -642.57 點（-1.42%）收 44,737.95；2330 官方收 NT$2,420（-20、-0.82%、量 42,857 張 TWSE 口徑、額 NT$1,033.71 億）相對抗跌、惟失守 5MA ~2,436；（2）⚡ 隔夜（7/14 美股）半導體大反彈：SOX +2.54% 收 12,661.93、NVDA +4.06% 收 $211.80、MU +4.92%、AMAT +3.53%、AMKR +6.27%；NYSE TSM 收 $420.39（-0.28%）明顯落後、屬法說會前觀望；（3）🚀 SK 海力士 7/15 韓股盤中暴漲逾 +11%、三星 +6%，AI 記憶體鏈全面噴出；（4）今日（7/15）台股可望跟進隔夜反彈。",
        "source": "TWSE / Yahoo Finance API 2026-07-14 - 07-15", "impact": "中性",
    },
    {
    ...
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📦 CoWoS 2026 產能擴增 20-30% 至 12.5 萬片/月：NVIDIA、博通預訂一空、缺口延續至 2027 年底",
        "body": "先進封裝供不應求結構未解：（1）📦 大摩等外資報告：TSMC CoWoS 先進封裝月產能 2026 年將提升至約 12.5 萬片（+20-30%），新增產能開出前已幾乎被 NVIDIA 與 Broadcom 預訂完畢；（2）📊 市場估 2026 全年需求達 100 萬片、TSMC 最多供應 60-70 萬片，供給缺口將延續至 2027 年底；（3）🎯 CoWoS 為 AI GPU（GB300/Vera Rubin）與 custom ASIC 出貨瓶頸、ASP 約為 HPC 裸晶圓 2-3 倍、毛利率高於公司均值；（4）供不應求 = 定價權與長約能見度俱增。",
        "source": "財報狗 / 商業周刊 / Morgan Stanley 2026-07", "impact": "正面",
    },
    {
    ...
    },
    {
        ...
        "title": "⚠️ 外資 7/14 賣超 12,416 張連 4 賣（4 日 -31,365 張）：法說會前籌碼轉弱、技術指標同步走弱",
        "body": "籌碼與技術面短線警訊：（1）📊 三大法人 7/14 合計賣超 8,005 張（T86 官方）：外資賣超 12,416 張——連 4 賣且較 7/13（-2,045）大幅擴大、4 日累計 -31,365 張；（2）🛡️ 對沖買盤：投信買超 1,841 張、自營買超 2,570 張（自行 +493、避險 +2,077）；（3）📉 技術面同步轉弱：7/14 失守 5MA ~2,436 / 20MA ~2,426、MACD 柱轉負（-4.8）、KDJ J 值 22 近超賣；（4）🎯 觀察：法說會前外資能否收斂翻買；估值不低（台股 P/E ~32.1x、TSM ~35.7x）下、明日財報成色為關鍵。",
        "source": "TWSE T86 / 鉅亨網 2026-07-14", "impact": "負面",
    },
]

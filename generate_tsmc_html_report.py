...
STOCK = {
    # 台股 2330
    "tw_price":       "2,400",        # 台股 2330 最新確認收盤：7/22 Wed 官方收 NT$2,400（-10、-0.41% vs 7/21 NT$2,410、開 2,440／高 2,445／低 2,385、TWSE 確認）——跳空開高 +30 後獲利了結賣壓出籠、尾盤翻黑，反彈第三日轉高檔震盪；大盤同日續漲 +592.91 點（+1.34%）收 44,825.78、兩日收復 7/17 崩跌逾七成
    "tw_change":      "-10",          # 7/22 Wed 收盤漲跌點數（-0.41%、TWSE 官方）
    "tw_change_pct":  "-0.41%",       # 7/22 Wed 收盤漲跌幅（開高走低、高低震盪 60 點、尾盤翻黑）
    "tw_volume":      "31,653",       # 7/22 成交量（張、TWSE 口徑；成交股數 31,653,123 股、成交額 NT$762.06 億、成交 136,194 筆——量持平 7/21 的 31,606 張、續攻量能仍未放大；成交筆數 136,194 遠高於 7/21 的 88,066、小單換手轉趨活絡）
    "tw_open":        "2,440",        # 7/22 開盤（TWSE、跳空開高 +30、盤中觸 2,445 後一路走低）
    "tw_high":        "2,445",        # 7/22 盤中高（TWSE、一度站上 20MA 2,414）
    "tw_low":         "2,385",        # 7/22 盤中低（TWSE、5MA 2,378 之上獲支撐）
    "market_cap":     "NT$62.2 兆",   # 7/22 收盤市值（NT$2,400 基礎、約 259.3 億股）
    "pe_ratio":       "~27.5x",       # 台股 P/E（TTM 估，NT$2,400 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25）
    "pb_ratio":       "~18.0x",       # 台股 P/B（NT$2,400 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.00%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,400）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "421.21",       # 7/22 Wed 官方收盤 $421.21（-$3.40、-0.80% vs 7/21 $424.61、Yahoo Finance API 確認）——連日大漲後縮量整理（量 9.0M 股、僅前日半量）；SOX 同日 +0.44% 收 12,410.67（自 6/22 史高 14,634.66 收斂至 -15.2%）
    "nyse_change":    "-3.40",        # 7/22 漲跌（自 7/21 $424.61）
    "nyse_change_pct":"-0.80%",       # 7/22 縮量整理；板塊分化：客戶端 NVDA +2.30%／AVGO +2.67%／AMD +1.45% vs 設備股回吐 AMAT -1.88%／KLAC -1.32%、記憶體 MU -1.17%
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~30.8x",       # NYSE USD 基礎 P/E（TTM 估，$421.21 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.5x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-23 Thu（最新確認收盤：台股 2330 7/22 Wed 官方收 NT$2,400（-10、-0.41% vs 7/21 NT$2,410、開 2,440／高 2,445／低 2,385、量 31,653 張 TWSE 口徑、成交額 NT$762.06 億、成交 136,194 筆、TWSE 確認）；NYSE TSM 7/22 Wed 收 $421.21（-$3.40、-0.80% vs 7/21 $424.61、Yahoo Finance API 確認））。📊 今日格局：反彈第三日轉高檔震盪——（1）2330 7/22 跳空開高 2,440（+30）盤中觸 2,445（一度站上 20MA 2,414 與布林中軌）、隨後獲利了結賣壓出籠一路走低、尾盤翻黑收 2,400（-0.41%）——高低震盪 60 點、20MA 得而復失；惟低點 2,385 守住 5MA 2,378、跌幅溫和，屬強勢整理而非轉弱；（2）⭐ 大盤 7/22 續漲 +592.91 點（+1.34%）收 44,825.78、連 2 日大漲合計 +2,376 點、收復 7/17 崩跌（-2,953.71）逾七成——惟結構切換：中小型股領漲（櫃買 +3.46%）、權值股歇腳、2330 尾盤翻黑落後大盤；（3）📊 籌碼分化：外資 7/22 對 2330 回賣 -3,539 張（買 15,737／賣 19,276、T86 官方）——7/21 轉買 +4,976 僅一日即回賣、「連 2 日買超確認趨勢反轉」未兌現；投信 +1,562 張（連 5 買、加碼擴大）、自營 -1,059 張（終結連 9 買）、三大法人合計 -3,036 張終結連 2 買；⭐ 惟全市場 BFI82U 官方：三大法人買超 338.5 億——外資 +173.4 億正式終結連 13 賣轉買、投信 +189.0 億連日大買——全市場資金回流 vs 個股獲利了結並存；（4）✅ NYSE TSM 7/22 縮量整理收 $421.21（-0.80%、量僅前日半量）、SOX +0.44% 收 12,410.67 連 2 紅（自史高 -15.2%）——板塊分化：客戶端 NVDA +2.30%／AVGO +2.67%／AMD +1.45% 續漲 vs 設備股 AMAT -1.88%／KLAC -1.32%／記憶體 MU -1.17% 回吐前日暴漲；（5）💰 基本面新錨：CFO 黃仁昭 7/22 定調 2nm 已於 Q2 開始貢獻營收、Q3 起放大並成為最新成長引擎——接棒 3nm 的產品週期敘事開啟。📊 台美 ADR 溢價 +13.6%（$421.21 vs 理論值 ~$370.9、匯率 ~32.35）——維持高檔、美股修正風險與台股補漲空間並存。技術面（官方收盤計算至 7/22）：收 2,400 位於 5MA 2,378 之上、20MA 2,414 之下（盤中站上又失守）；RSI(14) 50.7 持穩中軸、MACD 綠柱續收斂至 -12.8（DIF 7.9 < DEA 20.7）、KDJ K49.4 正式上穿 D44.8 低檔金叉兌現（J 值 58.7）；支撐 2,385（7/22 低）／2,378（5MA）／2,345／2,337（60MA）、壓力 2,414（20MA/布林中軌）／2,440-2,445（7/22 開盤高）／2,470（7/16 收前高）／2,519-2,535（布林上軌/史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B（Arizona 追加 $100B）、股利 +33%、2027 全面漲價 5-10% 傳聞＋2nm Q3 放量。⚠️ 風險：兩日暴漲後分析師籲獲利了結、量能未放大（連 2 日 3.2 萬張）、外資對 2330 買賣反覆、美 232 條款半導體關稅 25%、台幣 ~32.35。⭐ 今日（7/23）觀察：（1）能否重新站上 20MA 2,414 並收復 2,440；（2）外資對 2330 動向（全市場已轉買、個股是否跟進）；（3）量能能否放大至 4 萬張；（4）韓股 7/23 早盤 SK 海力士 +4.9%／三星 +3.6% 續強、SOX 期貨動向。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "📊 反彈第三日轉高檔震盪：台股 2330 7/22 開高走低收 NT$2,400（-0.41%）、20MA 得而復失惟守住 5MA；大盤續漲 +592.91 點收 44,825.78 兩日收復崩跌逾七成；外資全市場終結連 13 賣轉買 +173.4 億、惟對 2330 回賣 -3,539 張；NYSE TSM 縮量整理 -0.80% 收 $421.21；CFO 定調 2nm Q3 起成最新成長引擎",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 2330 7/22 官方收 NT$2,400（-10、-0.41%、開 2,440／高 2,445／低 2,385）——跳空開高一度站上 20MA、尾盤翻黑，高低震盪 60 點；量 31,653 張持平前日、續攻量能未放大；低點守住 5MA 2,378、屬強勢整理",
        "⭐ 大盤 7/22 續漲 +592.91 點（+1.34%）收 44,825.78、連 2 日合計 +2,376 點、收復 7/17 崩跌逾七成——惟中小型股領漲（櫃買 +3.46%）、2330 尾盤翻黑落後大盤、權值歇腳",
        "💰 基本面新錨：CFO 黃仁昭 7/22 定調 2nm 已於 Q2 開始貢獻營收、Q3 起放大成為最新成長引擎——接棒 3nm 的新產品週期敘事開啟",
        "📊 籌碼分化：外資對 2330 回賣 -3,539 張（7/21 轉買僅一日、趨勢反轉未確認）、投信 +1,562 連 5 買、自營 -1,059 終結連 9 買；⭐ 全市場 BFI82U：三大法人買超 338.5 億——外資 +173.4 億正式終結連 13 賣、投信 +189.0 億",
        "✅ NYSE TSM 7/22 縮量整理收 $421.21（-0.80%）、SOX +0.44% 連 2 紅（自史高 -15.2%）——板塊分化：NVDA +2.30%／AVGO +2.67% vs 設備股 AMAT -1.88%／MU -1.17% 回吐",
        "📊 台股供應鏈補漲續航：華邦電 +9.97% 漲停收 171、聯發科 +4.90% 收 3,850、日月光 +3.63% 收 656；韓股 7/22 平淡、7/23 早盤 SK 海力士 +4.9%／三星 +3.6% 續強",
        "💡 技術面：KDJ 低檔金叉正式兌現（K49.4 上穿 D44.8、J 58.7）、RSI 50.7 持穩中軸、MACD 綠柱續收斂 -12.8；收 2,400 卡在 5MA 2,378 與 20MA 2,414 之間——站回 20MA 為下一確認",
        "⚠️ 保留：兩日暴漲後分析師籲先獲利了結；量能連 2 日僅 3.2 萬張未放大；ADR 溢價 +13.6% 高檔（匯率 ~32.35）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-23 Thu（最新確認收盤：台股 2330 7/22 Wed 官方收 NT$2,400（-10、-0.41% vs 7/21 NT$2,410、開 2,440／高 2,445／低 2,385、量 31,653 張 TWSE 口徑、成交額 NT$762.06 億、成交 136,194 筆、TWSE 確認）；NYSE TSM 7/22 收 $421.21（-$3.40、-0.80%、Yahoo API 確認））。📊 核心判讀：反彈第三日由普漲轉高檔震盪＋結構分化——（1）2330 開高走低尾盤翻黑：跳空開 2,440 盤中觸 2,445（一度站上 20MA 2,414）後獲利了結賣壓一路壓回收 2,400——20MA 得而復失、惟低點 2,385 守住 5MA 2,378、跌幅僅 -0.41% 屬強勢整理；（2）大盤與個股背離：加權指數 +1.34% 連 2 日大漲收復崩跌逾七成、但由中小型股接棒領漲（櫃買 +3.46%）、台積電歇腳；（3）籌碼兩層分化：個股層面外資回賣 -3,539 張（7/21 轉買僅一日、「連 2 日買超」的趨勢反轉確認未兌現）、投信連 5 買堅定加碼；全市場層面外資 +173.4 億正式終結連 13 賣、三大法人買超 338.5 億——資金確定回流台股、惟對台積電個股先獲利了結；（4）美股縮量分化：TSM -0.80%、SOX +0.44% 連 2 紅、客戶強（NVDA/AVGO/AMD）設備弱（AMAT/KLAC/MU 回吐）——修復行情進入換手期。💰 基本面再添新錨：CFO 黃仁昭定調 2nm 已於 Q2 貢獻營收、Q3 起放大成最新成長引擎——與 2027 全面漲價 5-10% 傳聞、CoWoS 訂滿至 2027 形成「量價齊揚」敘事鏈。技術面 KDJ 低檔金叉正式兌現（前日金叉臨界 → 7/22 K49.4 上穿 D44.8）、RSI 50.7 中軸持穩、MACD 綠柱三日連續收斂——動能修復方向未變、惟需價格站回 20MA 2,414 與量能放大（>4 萬張）雙確認。📊 ADR 溢價 +13.6% 維持高檔——台股補漲空間與美股回檔風險並存。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、股利 +33%、「AI 需求比預期更強」。⭐ 今日（7/23）觀察：（1）重新站上 20MA 2,414 → 收復 2,440-2,445 → 挑戰 2,470；（2）外資個股動向是否跟上全市場轉買；（3）量能放大與否；（4）失守 5MA 2,378 則回測 2,345／2,337（60MA）。⚠️ 風險：兩日暴漲後獲利了結壓力（分析師已籲落袋）、AI 估值修正若再起、美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.35（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/23 Thu）關鍵觀察：（1）📊 高檔震盪後的方向選擇：7/22 開高走低收 2,400、20MA 2,414 得而復失——重新站上 20MA 並收復 2,440-2,445（7/22 開盤高）則反彈續航、上看 2,470（7/16 收前高）→ 2,519-2,535（布林上軌/史高）；失守 5MA 2,378 則回測 2,345／2,337（60MA）、跌破 2,320 反彈結構轉弱；（2）⚠️ 量能仍為最大保留：連 2 日僅 3.2 萬張（7/17 爆量 97,362 的三分之一）——續攻需放量至 4 萬張以上；惟 7/22 成交筆數 136,194 筆遠高於前日 88,066、籌碼換手轉趨活絡；（3）📊 籌碼反覆：外資對 2330 7/21 轉買 +4,976 → 7/22 回賣 -3,539、「連 2 日買超」趨勢反轉確認未兌現——但全市場外資 +173.4 億終結連 13 賣為更大的資金面轉向訊號、投信對 2330 連 5 買為最穩定多方；（4）⚠️ 兩日暴漲後市場雜音：分析師籲先獲利了結、中小型股領漲顯示資金自權值輪出；（5）💰 2nm Q3 放量與 2027 漲價傳聞為中期上修觸媒；💱 美 232 條款關稅 25% + 輸中晶片轉運查驗、台幣 ~32.35 續為背景風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-22",       # 最新資料日（7/22 Wed、TWSE T86 官方）
    "foreign":  -3539,             # 外資 7/22 賣超 3,539 張（7/21 轉買 +4,976 僅一日即回賣；買 15,737／賣 19,276——「連 2 日買超確認趨勢反轉」未兌現、高檔獲利了結）
    "trust":    1562,              # 投信 7/22 買超 1,562 張（連 5 買、且較 7/21 的 +974 擴大——最穩定的多方力量）
    "dealer":   -1059,             # 自營商 7/22 賣超 1,059 張（終結連 9 買；自行 -480、避險 -579）；三大法人合計 -3,036 張（終結連 2 日買超）
    # 近5日（由舊到新：7/16, 7/17, 7/20, 7/21, 7/22）；TWSE T86 口徑（張）
    "foreign_5d": [-4215, -44184, -1672, 4976, -3539],  # 外資（7/17 史詩級賣超 → 7/21 轉買一日 → 7/22 回賣）
    "trust_5d":   [528, 1489, 1049, 974, 1562],          # 投信（連 5 買、逆勢加碼擴大）
    "dealer_5d":  [1061, 2759, 4626, 214, -1059],        # 自營（連 9 買後 7/22 終結轉賣）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       50.7,   "rsi_signal": "RSI(14) 50.7（官方收盤計算）——7/22 小跌後持穩 50 中軸之上（前日 51.5）、動能未轉弱；站穩中軸後續彈看 60 強勢區",
    "macd":         -12.8, "macd_signal": "MACD 綠柱續收斂至 -12.8（DIF 7.9 < DEA 20.7、官方收盤計算）——自 -19.2 → -15.2 → -12.8 三日連續收斂、空方動能持續減弱；DIF 上穿 DEA 為正式翻多訊號",   "macd_color": "#F44336",
    "kdj_k":        49.4,   "kdj_d": 44.8, "kdj_j": 58.7, "kdj_signal": "KDJ K(49.4)/D(44.8)/J(58.7)（官方收盤計算）——⭐ 低檔金叉正式兌現：前日 K40.1/D40.2 黏合臨界、7/22 K 正式上穿 D 且 J 值升至 58.7——超賣修復動能延續、即使收黑指標仍走強",
    "ma5":       "2,378",   # 5MA（官方收盤計算；7/22 低點 2,385 守住其上）
    "ma20":      "2,414",   # 20MA（官方收盤計算；7/22 盤中站上、收盤失守——下一關鍵確認）
    "ma60":      "2,337",  # 60MA（官方收盤計算；中期支撐）
    "ma120":     "2,111",  # 120MA（官方收盤計算）
    "bb_upper":  "2,519",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,414",   # 布林中軌（20MA）
    "bb_lower":  "2,310",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "49,390",   # 5日均量（張、TWSE 口徑、仍含 7/17 爆量 97,362）
    "trend":    "📊 反彈第三日轉高檔震盪、KDJ 低檔金叉兌現：台股 2330 7/22 官方收 NT$2,400（-10、-0.41%、開 2,440／高 2,445／低 2,385）——跳空開高一度站上 20MA 2,414、獲利了結賣壓壓回尾盤翻黑，高低震盪 60 點；惟低點守住 5MA 2,378、跌幅溫和屬強勢整理。⭐ 大盤同日續漲 +592.91 點（+1.34%）收 44,825.78、兩日收復 7/17 崩跌逾七成——惟中小型股領漲（櫃買 +3.46%）、權值歇腳。📊 技術指標（官方收盤計算）：KDJ 低檔金叉正式兌現（K49.4 上穿 D44.8、J 58.7）、RSI(14) 50.7 持穩中軸、MACD 綠柱三日連續收斂至 -12.8（DIF 7.9 < DEA 20.7）——動能修復方向未變。📊 籌碼分化：外資對 2330 回賣 -3,539 張（轉買僅一日）、投信 +1,562 連 5 買；⭐ 全市場外資 +173.4 億正式終結連 13 賣、三大法人買超 338.5 億（BFI82U）——資金回流台股、個股先換手。⚠️ 量能連 2 日僅 3.2 萬張未放大——惟成交筆數 136,194 筆大增（前日 88,066）、換手活絡。NYSE TSM 7/22 縮量整理收 $421.21（-0.80%）、SOX +0.44% 連 2 紅（自史高 -15.2%）。下一觀察：（1）重新站上 20MA 2,414 → 收復 2,440-2,445（7/22 開盤高）→ 挑戰 2,470（7/16 收前高）→ 2,519-2,535（布林上軌/史高）；（2）支撐 2,385（7/22 低）／2,378（5MA）／2,345／2,337（60MA）；（3）量能能否放大至 4 萬張以上；（4）外資個股動向是否跟上全市場轉買。",
    "support":  "2,385 / 2,378 / 2,345 / 2,337 (7/22 低、5MA、7/21 低、60MA)",
    "resist":   "2,414 / 2,440-2,445 / 2,470 / 2,519-2,535 (20MA+布林中軌、7/22 開盤高、7/16 收前高、布林上軌+史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-22 止），請以看盤軟體為準；7/22 官方收 NT$2,400（-0.41%）開高走低：盤中觸 2,445 一度站上 20MA 2,414、收盤失守，低點 2,385 守住 5MA 2,378；⭐ KDJ 低檔金叉正式兌現（K49.4 上穿 D44.8、J 58.7）、RSI 50.7 持穩中軸、MACD 綠柱三日連續收斂 -12.8；支撐 2,385 / 2,378 / 2,345 / 2,337、壓力 2,414 / 2,440-2,445 / 2,470 / 2,519-2,535；✅ 全市場外資終結連 13 賣轉買 +173.4 億＋投信對 2330 連 5 買 vs ⚠️ 外資對 2330 回賣 -3,539 張＋量能未放大（連 2 日 3.2 萬張）——站回 20MA 與放量 4 萬張為反彈續航雙確認",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-22",       # 最新資料日（7/22 Wed 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,396",            # 台積電期貨近月（估、現貨 7/22 收 NT$2,400、小幅貼水）
    "futures_oi":    "180,000",          # 未平倉量（口）（估、高檔震盪日多空觀望）
    "futures_basis": "-4",               # 估逆價差（震盪整理日小幅貼水）
    "call_oi":       "110,000",
    "put_oi":        "96,000",
    "pcr":           "0.87",             # Put/Call 比率（估、較 7/21 的 0.85 微升——高檔震盪日避險需求小幅回升）
    "pcr_signal":    "中性（Put/Call 估 0.87；7/22 現貨開高走低收 NT$2,400（-0.41%）、外資對 2330 回賣惟全市場轉買——期權市場多空觀望、避險需求較崩跌期明顯回落但較 7/21 微升；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,320", "Call 40,000 / Put 10,000", "7/20 收、回檔支撐"),
        ("2,337", "Call 37,000 / Put 8,500",  "60MA、中期支撐"),
        ("2,378", "Call 34,000 / Put 7,500",  "5MA、短線支撐"),
        ("2,385", "Call 33,000 / Put 7,000",  "7/22 低、當日防線"),
        ("2,400", "Call 32,000 / Put 6,500",  "最大痛苦（估）、7/22 收盤、多空分界"),
        ("2,414", "Call 29,000 / Put 5,500",  "20MA/布林中軌、得而復失"),
        ("2,445", "Call 25,000 / Put 4,500",  "7/22 開盤高、短線壓力"),
        ("2,470", "Call 22,000 / Put 3,500",  "7/16 收前高、上方壓力"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/22 NT$2,400）；NT$2,400 為估最大痛苦、下方支撐 2,385（7/22 低）/ 2,378（5MA）/ 2,345 / 2,337（60MA）、上方壓力 2,414（20MA）/ 2,440-2,445（7/22 開盤高）/ 2,470（7/16 收）/ 2,519-2,535（布林上軌/史高）；📊 反彈第三日高檔震盪：開高走低尾盤翻黑、外資個股回賣 vs 全市場轉買 +173.4 億——多空觀望、貼水小幅擴大；⚠️ 量能未放大（連 2 日 3.2 萬張）；✅ 基本面錨未變（2nm Q3 放量、2027 漲價傳聞、全年上修 >40%、CapEx $60-64B）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-22 Wed TWSE 官方收盤；美股為 7/22 Wed 收盤、Yahoo Finance API 確認；韓股為 7/22 收盤、7/23 早盤續強）
    ("設備", "ASML",    "艾司摩爾",    "1,801.86", "+0.02%", "⭐ EUV 設備龍頭；7/22 持平整理；TSMC CapEx 上調至 $60-64B 為設備鏈中長線利多"),
    ("設備", "AMAT",    "應用材料",    "553.92", "-1.88%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作；7/22 回吐前日暴漲（+7.39%）的四分之一、屬技術性整理"),
    ("設備", "LRCX",    "科林研發",    "319.29", "-0.84%", "蝕刻設備（7/22 小幅回吐）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "214.69", "-1.32%", "量測設備（7/22 回吐）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","171.00","+9.97%", "🚀 漲停！與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/22 攻漲停收 171、TWSE 官方、記憶體補漲最猛）"),
    ("記憶", "000660.KS","SK 海力士",  "1,830,000 KRW", "-0.33%", "HBM4、與 NVIDIA 多年期合作；7/22 高檔小回（Yahoo API）、7/23 早盤大漲 +4.9% 至 192 萬韓元"),
    ("記憶", "MU",      "美光",        "959.48", "-1.17%", "HBM / DDR5（7/22 回吐前日 +12.17% 暴漲的一角）；HBM4 供需緊俏中長線邏輯未變"),
    ("封裝", "3711.TW", "日月光",      "656.00", "+3.63%", "CoWoS / 先進封裝（7/22 續漲收 656、TWSE 官方、連 2 日大漲補回崩跌缺口）"),
    ("封裝", "AMKR",    "Amkor",       "66.97",  "+0.36%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/22 小漲續強）"),
    ("客戶", "NVDA",    "輝達",        "212.06", "+2.30%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/22 續漲收 212.06、板塊分化中客戶端最強"),
    ("客戶", "AAPL",    "蘋果",        "325.89", "-0.56%", "⭐ #2 客戶 18%；7/22 小跌整理；跳過 A16 直上 A14；傳為 2027 漲價適用客戶之一"),
    ("客戶", "AVGO",    "博通",        "396.81", "+2.67%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/22 續強逼近 $400）"),
    ("客戶", "AMD",     "超微",        "552.33", "+1.45%", "#4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/22 續漲站穩 $550）"),
    ("客戶", "2454.TW", "聯發科",      "3,850",  "+4.90%", "手機 SoC 客戶 9%（7/22 續漲收 3,850、TWSE 官方、連 2 日合計 +15.3%）"),
    ("競爭", "INTC",    "英特爾",      "102.62", "-2.68%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/22 回吐前日 +8.64% 的三成）"),
    ("競爭", "005930.KS","三星電子",   "260,500 KRW", "+0.58%", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/22 小漲（Yahoo API）、7/23 早盤 +3.6% 至 269,750 韓元"),
    ("產業", "SOX",     "費城半導體指數","12,410.67", "+0.44%", "7/22 連 2 紅收 12,410.67——自 6/22 史高 14,634.66 收斂至 -15.2%；縮量整理、客戶強設備弱的分化格局"),
    ("處分後", "ARM",   "Arm Holdings","283.40", "-2.18%", "TSMC 4/29 全數出脫後（7/22 回吐前日 +7.46% 的三成、高 Beta 震盪）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/21 收 $424.61 具 +15.5% 上行空間）；⭐ 法說會後新一輪上調：Susquehanna $600、Barclays $650、BofA $590、TD Cowen $440——S&P Global 統計 19 位平均已達 $520.37（TipRanks/GuruFocus 7/17-20）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+15.48%",  # 從 NYSE 7/21 收 $424.61 至平均目標 $490.34（共識強力買進；法說會後多家上調、S&P Global 19 位平均 $520.37 對應約 +22.6%）
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,356.16", "+5.21%", True),   # 7/21 收（Yahoo）大漲；自 6/22 史高收斂至 -15.6%、脫離熊市線
    ("台股加權指數",    "44,232.87", "+4.20%", True),   # 7/21 官方收盤（+1,783.17 點、史上最大單日漲點、TWSE）
    ("VIX 恐慌指數",    "17.05",    "-8.58%", True),    # 7/21 收（Yahoo）明顯回落、risk-on
    ("WTI 原油",        "$84.65",   "+1.7%",  False),   # 7/21-22 近月（Yahoo CL=F；地緣溢價維持高檔）
    ("10Y 美債殖利率",  "4.63%",   "+3bp",   False),   # 7/21 收（Yahoo ^TNX、risk-on 資金離開債市）
    ("台幣/美元",       "~32.33",   "小貶",  False),    # 7/22 Yahoo TWD=X（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$207.29", "+1.97%",  True),    # #1 TSMC 客戶（22%），7/21 續漲惟落後板塊
    ("ASML",            "$1,801.51", "+3.59%",  True),  # 設備股 7/21 隨板塊強彈（Yahoo）
]
...
NEWS = [
    {
        ...
        "title": "💰 CFO 定調：2nm 已於 Q2 貢獻營收、Q3 起放大成最新成長引擎",
        "body": "接棒 3nm 的新產品週期敘事正式開啟：（1）💰 財務長黃仁昭 7/22 表示，2 奈米製程已於第二季開始貢獻營收、預期第三季起成為公司最新成長動能，未來數季營收貢獻與產能規模將持續擴大；（2）📊 佐證：Q2 法說會揭露 2nm 貢獻約 3%、良率爬升順利；（3）⭐ 與 2027 年全面漲價 5-10% 傳聞形成「量價齊揚」敘事鏈——N2 為首個以更高定價開賣的節點；（4）🎯 首發客戶含 Apple A20、AMD 2nm AI 晶片等、產能滿載至 2026 年底。",
        "source": "經濟日報 / 中央社 2026-07-22", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 大盤續漲 +592.91 點收 44,825.78：兩日收復崩跌逾七成、惟 2330 尾盤翻黑",
        "body": "反彈第三日結構切換：（1）⭐ 加權指數 7/22 續漲 +592.91 點（+1.34%）收 44,825.78、連 2 日合計 +2,376 點、收復 7/17 崩跌（-2,953.71）逾七成；成交額 1.026 兆再放大；（2）📊 惟領漲棒子交給中小型股：櫃買指數 +3.46% 大幅跑贏、權值股歇腳；（3）⚠️ 2330 開高 2,440 盤中觸 2,445 後獲利了結賣壓出籠、尾盤翻黑收 2,400（-0.41%）——高低震盪 60 點、落後大盤；（4）📊 兩日暴漲後分析師開始提醒「先獲利了結、慎防追高」。",
        "source": "TWSE / 中央社 / BigGo Finance 2026-07-22", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 籌碼兩層分化：全市場外資終結連 13 賣轉買 +173.4 億、惟對 2330 回賣",
        "body": "資金回流台股、個股先換手：（1）⭐ 全市場 BFI82U 官方：7/22 三大法人買超 338.5 億——外資 +173.4 億正式終結連 13 賣轉買、投信 +189.0 億連日大買；（2）⚠️ 惟外資對 2330 回賣 -3,539 張（買 15,737／賣 19,276、T86 官方）——7/21 轉買 +4,976 僅一日即回賣、「連 2 日買超確認趨勢反轉」未兌現；（3）✅ 投信對 2330 買超 +1,562 張連 5 買且擴大——最穩定多方；自營 -1,059 張終結連 9 買；（4）🎯 觀察外資個股動向能否跟上全市場轉買。",
        "source": "TWSE T86 / BFI82U 官方 2026-07-22", "impact": "中性",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "✅ 美股半導體換手：NVDA +2.30%／AVGO +2.67% 續漲、設備記憶體回吐",
        "body": "修復行情進入分化換手期：（1）✅ NYSE TSM 7/22 縮量整理收 $421.21（-0.80%、量僅前日半量）——連日大漲後的溫和消化；（2）📊 SOX +0.44% 收 12,410.67 連 2 紅、自 6/22 史高收斂至 -15.2%；（3）📈 客戶端續強：NVDA +2.30% 收 $212.06、AVGO +2.67% 逼近 $400、AMD +1.45% 站穩 $550；（4）📉 前日反彈先鋒回吐：AMAT -1.88%、KLAC -1.32%、MU -1.17%、INTC -2.68%、ARM -2.18%——輪動而非轉弱；（5）📊 台美 ADR 溢價 +13.6% 維持高檔（匯率 ~32.35）。",
        "source": "Yahoo Finance API 2026-07-22", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🚀 台股供應鏈補漲續航：華邦電漲停 +9.97%、聯發科 +4.90% 收 3,850",
        "body": "台股 AI 供應鏈補漲第二棒：（1）🚀 華邦電 7/22 攻漲停收 171（+9.97%）——與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊、記憶體補漲最猛；（2）📈 聯發科 +4.90% 收 3,850、連 2 日合計 +15.3%；日月光 +3.63% 收 656 連 2 日大漲補回崩跌缺口；（3）📈 韓股 7/22 平淡整理（SK 海力士 -0.33%、三星 +0.58%）、7/23 早盤重啟漲勢：SK 海力士 +4.9% 至 192 萬韓元、三星 +3.6%；（4）⭐ 中小型半導體與記憶體板塊成資金新寵、與大盤中小型領漲結構一致。",
        "source": "TWSE / Yahoo Finance API 2026-07-22~23", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 高檔震盪三大保留：量能未放大、外資反覆、分析師籲獲利了結",
        "body": "反彈續航的確認清單：（1）⚠️ 量能連 2 日僅 3.2 萬張（7/17 爆量 97,362 的三分之一）——續攻需放大至 4 萬張以上；惟 7/22 成交筆數 136,194 筆大增（前日 88,066）、換手活絡為正向細節；（2）📊 外資對 2330 買一日賣一日（+4,976 → -3,539）——趨勢反轉未確認、7/22 開高走低即其獲利了結所致；（3）⚠️ 兩日大盤暴漲 +2,376 點後分析師籲先落袋、慎防追高；（4）💱 背景風險：美 232 條款半導體關稅 25% + 輸中晶片轉運查驗、台幣 ~32.35、20MA 2,414 得而復失待收復。",
        "source": "TWSE / BigGo Finance 2026-07-22", "impact": "中性",
    },
]

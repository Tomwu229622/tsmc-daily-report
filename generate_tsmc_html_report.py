...
STOCK = {
    # 台股 2330
    "tw_price":       "2,425",        # 台股 2330 最新確認收盤：7/31 Fri 官方漲停收 NT$2,425（+220、+9.98% vs 7/30 NT$2,205、開 2,350／高 2,425／低 2,345、TWSE 確認）——股災後 V 型反轉：台股史上第 3 次 2330 漲停（媒體口徑）；大盤同日 +3,186.45 點（+7.98%）收 43,119.75、史上最大單日漲點；7/30 Thu 曾先以 +5（+0.23%）收 2,205 止穩
    "tw_change":      "+220",         # 7/31 Fri 收盤漲跌點數（+9.98% 漲停、TWSE 官方）
    "tw_change_pct":  "+9.98%",       # 7/31 Fri 漲停：跳空高開 2,350（+6.6%）→ 全日最低 2,345 → 尾盤鎖上漲停 2,425；7/30 曾收 2,205（+0.23%）先行止穩——兩日自 7/29 低點 2,180 反彈最高 +11.2%
    "tw_volume":      "69,478",       # 7/31 成交量（張、TWSE 口徑；成交額 NT$1,666.62 億、成交 215,037 筆——量超過 7/29 恐慌量 68,140 張、為 7/17 以來最大量——反彈具量能支撐）
    "tw_open":        "2,350",        # 7/31 開盤（TWSE、跳空高開 +145、直接站回 5MA/60MA 之上）
    "tw_high":        "2,425",        # 7/31 盤中高（TWSE、漲停價收盤=全日高）
    "tw_low":         "2,345",        # 7/31 盤中低（TWSE、開盤區即全日低點、一路走高鎖漲停）
    "market_cap":     "NT$62.9 兆",   # 7/31 收盤市值（NT$2,425 基礎、約 259.3 億股）
    "pe_ratio":       "32.60x",       # 台股 P/E（TWSE 官方 BWIBBU 7/31、口徑為最新年度 EPS）
    "pb_ratio":       "10.67x",       # 台股 P/B（TWSE 官方 BWIBBU 7/31、以 115Q1 財報淨值計）
    "div_yield":      "0.91%",        # 殖利率（TWSE 官方 BWIBBU 7/31、以 114 年度股利計；漲停使殖利率回落至 0.91%）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "404.25",       # 7/31 Fri 官方收盤 $404.25（+$0.94、+0.23% vs 7/30 $403.31、Yahoo Finance API 確認）——7/30 曾暴漲 +7.64%（$374.67 → $403.31）、7/31 高檔續穩；SOX 7/30 +8.19% 收 11,302.99、7/31 +0.07% 收 11,311.08（自 6/22 史高 14,634.66 收斂至 -22.7%）
    "nyse_change":    "+0.94",        # 7/31 漲跌（自 7/30 $403.31；兩日累計 +$29.58、+7.9%）
    "nyse_change_pct":"+0.23%",       # 7/31 美股個股分化消化前日暴漲：AMZN 財報 +15.32% 收 $271.58（2012 年來最大漲幅）、NVDA +2.93% 收復 $200 收 $200.75、AMAT +1.18%、KLAC +1.38%；AAPL 財報後 -7.35% 收 $308.91、MU -5.90%（7/30 +18.4% 後獲利了結）、AMD -1.90%；VIX -6.44% 續降至 15.99、S&P 500 +0.70%
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~29.6x",       # NYSE USD 基礎 P/E（TTM 估，$404.25 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.3x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-08-03 Mon（最新確認收盤：台股 2330 7/31 Fri 官方漲停收 NT$2,425（+220、+9.98% vs 7/30 NT$2,205、開 2,350／高 2,425／低 2,345、量 69,478 張 TWSE 口徑、成交額 NT$1,666.62 億、成交 215,037 筆、TWSE 確認）；NYSE TSM 7/31 Fri 收 $404.25（+$0.94、+0.23% vs 7/30 $403.31、Yahoo API 確認）。8/1-8/2 週末休市；7/31 週五日報缺漏、本日一併補述 7/30-31 兩交易日）。📊 今日格局：股災後 V 型反轉、由連續兩晚 hyperscaler 財報驗證驅動——（1）⭐ 台股 7/30 先止穩（大盤 -105.88 收 39,933.30、2330 +5 收 2,205）、7/31 史詩級反彈：大盤 +3,186.45 點（+7.98%）收 43,119.75 創史上最大單日漲點、2330 跳空高開 2,350 尾盤鎖漲停收 2,425（量 69,478 張為 7/17 以來最大）——供應鏈全面漲停（聯發科 +9.89%、日月光 +9.90%、聯電 +10.00%、華邦電 +9.70%）；（2）✅ 籌碼一日翻多：7/31 外資 +13,780 張終結連 7 賣（7/30 已收斂至 -3,958）、投信 +7,063 連 6 買爆量放大 7.5 倍、自營 +2,724 連 7 買、三大合計 +23,568 張；全市場 BFI82U 自 7/30 -495.4 億翻正至 +873.1 億（外資 +675.5 億、投信 +360.2 億）；（3）⭐ 美股 7/30 暴力反彈：LRCX 財報後 +17.99%（1999 年來最佳單日）、MU +18.36%、MSFT +15.51%、AMAT +14.97%、AMD +13.0%、TSM +7.64% 收 $403.31、SOX +8.19% 收 11,302.99；7/31 高檔續穩：TSM +0.23% 收 $404.25、SOX 收 11,311.08（自 6/22 史高收斂至 -22.7%）、NVDA +2.93% 收復 $200 收 $200.75、VIX 兩日 -22.6% 降至 15.99、S&P 500 +0.70% 收 7,489.72；（4）📊 7/30 盤後財報兩樣情：AMZN 營收 $200.6B（+20% YoY）、AWS +37% 至 $42.2B 創 2021 年來最快增速、全年 CapEx 上看 $220B、AI 與自研晶片各達年化 $25B——7/31 +15.32% 收 $271.58（2012 年來最大漲幅）；AAPL 營收 $109.4B／EPS $2.02 雙超預期、惟 9 月季指引低於共識＋Cook 坦承「晶片供應吃緊、低估自身需求」——7/31 -7.35% 收 $308.91（Cook 卸任前最後法說、9/1 Ternus 接任）——MSFT→AMZN 連續驗證 AI CapEx 未收縮、AAPL 供應吃緊反證上游需求強勁、對 TSMC 淨效果偏正面；（5）⭐ 韓股 7/31 暴漲：KOSPI 收 6,595.45（+17.9% vs 7/30 5,593.56、Yahoo API、媒體稱史上最大單日反彈）、SK 海力士 +29.95% 收 171.8 萬韓元、三星 +26.81% 收 26.25 萬（SK 會長崔泰源加碼買股＋空頭/槓桿 ETF 回補放大）；⚠️ 今晨（8/3 09:40）回吐：KOSPI -4.78% 至 6,279.91、SK -7.39%、三星 -7.62%——雙向劇震未息；（6）📅 232 條款 7 月底窗口已過、截至 8/3 晨未查得官方公布（未能證實）；台美關稅談判傳 20%→~15%。📊 ADR 溢價收斂至 +8.02%（$404.25 vs 理論值 $374.23、匯率 32.40；今晨台幣升至 32.29）。技術面（官方收盤計算至 7/31）：漲停一舉收復全部均線（5MA 2,292／20MA 2,382／60MA 2,348／120MA 2,143）並站回布林中軌之上——KDJ 黃金交叉（K 42.6／D 32.9／J 62.1）、RSI 54.6 回中性、MACD 綠柱自 -53.4 收斂至 -25.9（DIF -21.2 仍零軸下）；支撐 2,382／2,350／2,345／2,292、壓力 2,445（7/22 高）／2,535（史高）／2,543（布林上軌）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量；MSFT/AMZN 再證 AI CapEx 擴張。📈 今晨（8/3）盤中即時（09:03、TWSE MIS）：大盤 43,007.35（-0.26%、開 42,780.42）、2330 開 2,390／高 2,395／低 2,365 現 2,370（-2.27%、早盤量 5,482 張）——漲停後溫和拉回消化、遠小於韓股跌幅（盤中數據、隨時變動、收盤前不作結論）。⭐ 今日（8/3）觀察：（1）守 2,382（20MA）／2,345（7/31 低）確認 V 型有效；（2）外資買超延續性（一日翻多 vs 趨勢翻多分水嶺）；（3）韓股回吐是否失控；（4）量能能否維持 5 萬張級；（5）232 條款與台美關稅進展。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ V 型反轉：台股 7/31 大漲 +3,186.45 點（+7.98%）收 43,119.75 創史上最大單日漲點、2330 漲停收 NT$2,425（+220、+9.98%、量 69,478 張）——外資 +13,780 張終結連 7 賣、三大法人 +23,568 張、BFI82U 全市場買超 +873.1 億；美股 7/30 暴力反彈（TSM +7.64%、SOX +8.19%、LRCX +18.0%）＋7/31 高檔續穩收 $404.25、VIX 降至 15.99；AMZN 財報 AWS +37% 大漲 +15.32%、AAPL 指引保守 -7.35%；韓股 7/31 暴漲（SK 海力士 +30.0%、三星 +26.8%）惟今晨（8/3）回吐 -4.8%、台股今晨僅小跌——反彈延續性今日驗證",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ 台股 7/31 史詩級反彈：大盤 +3,186.45 點（+7.98%）收 43,119.75、史上最大單日漲點（7 月全月仍累跌約 3,006 點）；2330 跳空高開 2,350、尾盤鎖漲停收 NT$2,425（+220、+9.98%、量 69,478 張為 7/17 以來最大）——聯發科 +9.89%、日月光 +9.90%、聯電 +10.00% 全面漲停；7/30 曾先以 2,205（+0.23%）止穩",
        "✅ 籌碼 V 型翻多：7/31 外資買超 +13,780 張（終結連 7 賣）、投信 +7,063 張（連 6 買、爆量放大 7.5 倍）、自營 +2,724 連 7 買、三大合計 +23,568 張；全市場 BFI82U 買超 +873.1 億（外資 +675.5 億、投信 +360.2 億）——自 7/30 賣超 -495.4 億一日翻正",
        "⭐ 美股 7/30 暴力反彈＋7/31 續穩：TSM 7/30 +7.64% 收 $403.31、7/31 +0.23% 收 $404.25；SOX 7/30 +8.19%、7/31 收 11,311.08（自史高收斂至 -22.7%）；7/30 LRCX +17.99%（1999 年來最佳單日）、MU +18.36%、MSFT +15.51%、AMAT +14.97%、AMD +13.0%；VIX 兩日 -22.6% 降至 15.99——恐慌解除",
        "📊 客戶財報兩樣情（7/30 盤後）：AMZN 營收 $200.6B（+20%）、AWS +37% 至 $42.2B 創 2021 年來最快、全年 CapEx 上看 $220B——7/31 大漲 +15.32%（2012 年來最大漲幅）；AAPL 營收 $109.4B／EPS $2.02 雙超預期、惟 9 月季指引低於共識＋Cook 坦承晶片供應吃緊——7/31 重挫 -7.35%；對 TSMC 淨效果偏正面（AI CapEx 驗證＋供應吃緊=需求強勁）",
        "📊 韓股 7/31 史上最大反彈後今晨回吐：KOSPI 7/31 收 6,595.45（+17.9%、Yahoo API）、SK 海力士 +29.95% 收 171.8 萬韓元、三星 +26.81%（SK 會長崔泰源加碼買股＋槓桿 ETF 回補放大）；⚠️ 今晨（8/3 09:40）KOSPI -4.78% 至 6,279.91、SK -7.39%、三星 -7.62%——槓桿市場雙向劇震未息",
        "💡 技術面一日修復：2330 漲停收 2,425 一舉收復全部均線（5MA 2,292／20MA 2,382／60MA 2,348／120MA 2,143）、KDJ 黃金交叉（K 42.6 > D 32.9、J 62.1）、RSI 54.6 回中性、MACD 綠柱自 -53.4 大幅收斂至 -25.9——短線結構轉強、惟距 7/22 高 2,445 與史高 2,535 仍有壓力帶",
        "⚠️ 今晨（8/3 09:03 官方 MIS）：台股大盤 43,007.35（-0.26%、開 42,780.42）、2330 開 2,390 現 2,370（-2.27%、低 2,365）——僅溫和拉回、遠小於韓股跌幅；今日觀察漲停後量能與外資延續性",
        "📅 政策面：美 232 條款半導體調查 7 月底窗口已過、截至 8/3 晨尚未查得官方公布結果（未能證實）；台美關稅談判傳由 20% 下調至 ~15%——政策不確定性未除",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-08-03 Mon（最新確認收盤：台股 2330 7/31 Fri 官方漲停收 NT$2,425（+220、+9.98% vs 7/30 NT$2,205、開 2,350／高 2,425／低 2,345、量 69,478 張 TWSE 口徑、成交額 NT$1,666.62 億、成交 215,037 筆、TWSE 確認）；NYSE TSM 7/31 收 $404.25（+0.23% vs 7/30 $403.31、7/30 曾 +7.64%、Yahoo API 確認）；7/30 Thu 台股收 2,205（+5、+0.23%、量 51,372 張）先行止穩）。📊 核心判讀：股災三日殺盤後出現教科書級 V 型反轉、由「財報驗證」邏輯驅動——（1）⭐ 台股 7/31 大盤 +3,186.45 點（+7.98%）收 43,119.75 創史上最大單日漲點（7/30 曾 -105.88 收 39,933.30 探底）；2330 跳空高開 2,350 尾盤鎖漲停 2,425、量 69,478 張超過 7/29 恐慌量——反彈具量能與籌碼雙重支撐、供應鏈全面漲停（聯發科／日月光／聯電）；（2）✅ 籌碼一日翻多為最關鍵訊號：外資 7/31 買超 +13,780 張終結連 7 賣（7/30 已收斂至 -3,958）、投信 +7,063 連 6 買爆量、自營 +2,724 連 7 買、三大合計 +23,568 張；全市場 BFI82U 自 7/30 -495.4 億翻正至 +873.1 億（外資 +675.5 億）；（3）⭐ 驅動鏈：MSFT（7/29 盤後）→ 美股 7/30 暴力反彈（LRCX +17.99% 創 1999 年來最佳、MU +18.36%、MSFT +15.51%、AMAT +14.97%、AMD +13.0%、TSM +7.64%、SOX +8.19%）→ AMZN（7/30 盤後）AWS +37% 至 $42.2B 創 2021 年來最快、全年 CapEx 上看 $220B、自研晶片年化 $25B——AI CapEx 敘事獲連續兩晚財報數字驗證、本輪「AI 回報質疑」殺盤的基本面反駁鏈完成；AAPL 雙超預期惟 9 月指引低於共識＋Cook 坦承「晶片供應吃緊、低估自身需求」-7.35%——對 TSMC 而言供應吃緊反證需求強勁、淨效果偏正面；（4）📊 韓股 7/31 暴漲（KOSPI 收 6,595.45、SK 海力士 +29.95%、三星 +26.81%——空頭回補與槓桿 ETF 機械式買盤成分高）、⚠️ 今晨 8/3 即回吐（KOSPI -4.78%、SK -7.39%、三星 -7.62%）——韓股雙向劇震未息、續為亞股最大外部變數；台股今晨僅 -0.26%、2330 -2.27%（09:03 官方 MIS）相對淡定；（5）💡 技術面一日修復：收復全部均線（5MA 2,292／20MA 2,382／60MA 2,348／120MA 2,143）、KDJ 黃金交叉 J 62.1、RSI 54.6 回中性、MACD 綠柱收斂至 -25.9 惟 DIF -21.2 仍在零軸下——短線結構轉強、中期修復需守穩 20MA 2,382 並挑戰 2,445（7/22 高）／2,535（史高）；失守 2,345（7/31 低）則 V 型反轉降級為跌深反彈。📊 ADR 溢價 +8.02%（$404.25 vs 理論值 $374.23、匯率 32.40）——台股補漲後溢價自 +10.2% 正常化。✅ 基本面錨未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量；MSFT/AMZN 財報再證 AI CapEx 擴張（AMZN 上看 $220B）。⭐ 今日（8/3）觀察：（1）漲停後首日守 2,382（20MA）／2,345（7/31 低）、量能能否維持 5 萬張級；（2）外資買超延續性（一日翻多 vs 趨勢翻多的分水嶺）；（3）韓股回吐幅度是否失控；（4）今晚美股與 TSM ADR 動向；（5）232 條款結果與台美關稅談判進展。⚠️ 風險：漲停後追高買盤易套、韓股槓桿雙向劇震、AAPL 指引保守顯示終端消費需求分化、232 條款不確定性、7 月全月大盤仍累跌約 3,006 點（月線大幅收黑的中期壓力未解）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（8/3 Mon）關鍵觀察：（1）⚠️ 漲停後首日驗證：7/31 漲停收 2,425 屬「利多＋極度超賣＋軋空」複合行情，今晨（09:03 官方 MIS）2330 開 2,390 現 2,370（-2.27%）溫和拉回屬正常消化——守 2,382（20MA／布林中軌）與 2,345（7/31 低）為多方防線、失守則 V 型反轉降級為跌深反彈；（2）⚠️ 韓股今晨回吐劇烈：KOSPI -4.78%、SK 海力士 -7.39%、三星 -7.62%（8/3 09:40）——7/31 暴漲含大量空頭回補與槓桿 ETF 機械式買盤、雙向波動未息，若尾盤跌幅擴大將再度外溢台股；（3）📊 AMZN AWS +37%＋CapEx 上看 $220B 為 AI CapEx 敘事最強驗證、AAPL「晶片供應吃緊」說法對 TSMC 反為正面——惟 AAPL 指引保守提示終端消費需求分化、留意手機端拉貨節奏；（4）📅 232 條款 7 月底窗口已過仍未查得公布（未能證實）——結果溫和＋關稅 20%→~15% 兌現為潛在第二波催化、反之為尾部風險；（5）💱 台幣今晨升至 32.29（7/31 32.40）——每升值 1% 約影響毛利率 -40bps、留意資金回流台股伴隨的匯率壓力。",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-31",       # 最新資料日（7/31 Fri、TWSE T86 官方）
    "foreign":  13780,             # 外資 7/31 大幅買超 13,780 張（終結連 7 賣、7/30 已收斂至 -3,958——V 型翻多）
    "trust":    7063,              # 投信 7/31 買超 7,063 張（連 6 買、自 939 爆量放大 7.5 倍——低接部位大獲全勝後續加碼）
    "dealer":   2724,              # 自營商 7/31 買超 2,724 張（連 7 買）；三大法人合計 +23,568 張（自 7/30 -1,381 一日翻正）
    # 近5日（由舊到新：7/27, 7/28, 7/29, 7/30, 7/31）；TWSE T86 口徑（張）
    "foreign_5d": [-2662, -14659, -11211, -3958, 13780], # 外資（連 7 賣後 7/31 轉大買）
    "trust_5d":   [567, 284, 1652, 939, 7063],           # 投信（連 6 買、7/31 爆量）
    "dealer_5d":  [1410, 5123, 4893, 1637, 2724],        # 自營（連 7 買）
    "foreign_ownership_pct": "~74.5%",  # 外資持股比例（估；官方以 TWSE 外資持股餘額為準）
    "foreign_ownership_shares": "約 193 億股（估）",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       54.6,   "rsi_signal": "RSI(14) 54.6（官方收盤計算）——漲停日自 36.1 跳升 18.5、一舉脫離超賣區回到中性偏多；動能修復速度為本輪股災以來最快",
    "macd":         -25.9, "macd_signal": "MACD 綠柱自 -53.4 大幅收斂至 -25.9（DIF -21.2 < DEA -8.3、官方收盤計算）——單日收斂逾半、惟 DIF 仍深陷零軸下；綠柱持續收斂＋DIF 回升為中期修復確認路徑",   "macd_color": "#F44336",
    "kdj_k":        42.6,   "kdj_d": 32.9, "kdj_j": 62.1, "kdj_signal": "KDJ K(42.6)/D(32.9)/J(62.1)（官方收盤計算）——⭐ 漲停日黃金交叉（K 上穿 D）、J 值自 -2.9 飆升至 62.1——極度超賣的技術性反彈已兌現、後續看 K 值能否站穩 50 之上",
    "ma5":       "2,292",   # 5MA（官方收盤計算；7/31 漲停一舉收復全部均線）
    "ma20":      "2,382",   # 20MA（官方收盤計算；與布林中軌重合、7/31 收 2,425 站回其上）
    "ma60":      "2,348",  # 60MA（官方收盤計算；季線收復、中期架構自轉空邊緣拉回）
    "ma120":     "2,143",  # 120MA（官方收盤計算至 7/31）
    "bb_upper":  "2,543",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,382",   # 布林中軌（20MA）
    "bb_lower":  "2,220",  # 布林下軌（20MA-2σ、官方收盤計算；7/29-30 連續收在下軌下方後 7/31 直接站回中軌之上）
    "vol_ma5":  "52,652",   # 5日均量（張、TWSE 口徑；7/31 量 69,478 為 7/17 以來最大、反彈具量能）
    "trend":    "⭐ V 型反轉：2330 7/31 官方漲停收 NT$2,425（+220、+9.98%、開 2,350／高 2,425／低 2,345、量 69,478 張）——跳空高開直接站回 5MA/60MA、尾盤鎖漲停一舉收復全部均線並站回布林中軌之上；大盤同日 +3,186.45 點（+7.98%）收 43,119.75 史上最大單日漲點；7/30 曾以 2,205（+0.23%）先行止穩。📊 技術指標（官方收盤計算）：KDJ 黃金交叉（K 42.6 上穿 D 32.9、J 62.1）、RSI 54.6 回中性、MACD 綠柱自 -53.4 收斂至 -25.9 惟 DIF -21.2 仍在零軸下——短線結構一日修復、中期空方架構尚未完全解除。✅ 籌碼同步翻多：外資 +13,780 張終結連 7 賣、投信 +7,063 爆量、BFI82U 全市場 +873.1 億。⚠️ 今晨（8/3 09:03）2330 開 2,390 現 2,370（-2.27%）溫和拉回、韓股回吐 -4.78% 較劇烈。下一觀察：（1）守 2,382（20MA）／2,345（7/31 低）確認 V 型有效；（2）挑戰 2,445（7/22 高）→ 2,535（史高）；（3）外資買超延續性與量能維持。",
    "support":  "2,382 / 2,350 / 2,345 / 2,292 / 2,205 (20MA布林中軌、7/31 開盤、7/31 低、5MA、7/30 收)",
    "resist":   "2,445 / 2,535 / 2,543 (7/22 高、6/23 史高、布林上軌)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-31 止），請以看盤軟體為準；7/31 官方漲停收 NT$2,425（+9.98%）一舉收復 5MA 2,292／20MA 2,382／60MA 2,348／120MA 2,143 全部均線並站回布林中軌之上；⭐ KDJ 黃金交叉（K42.6/D32.9/J62.1）、RSI 54.6、MACD 綠柱收斂至 -25.9（DIF -21.2 仍零軸下）；支撐 2,382 / 2,350 / 2,345 / 2,292、壓力 2,445 / 2,535 / 2,543；✅ 外資 +13,780 張終結連 7 賣＋量 69,478 張 7/17 以來最大 vs ⚠️ 漲停後追高風險＋韓股今晨回吐＋MACD 中期空方未完全解除——守 2,382 與外資延續性為今日雙檢核",
}
...
DERIVATIVES = {
    "date":          "2026-07-31",       # 最新資料日（7/31 Fri 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,431",            # 台積電期貨近月（估、現貨 7/31 漲停收 2,425、V 型反轉日貼水收斂常轉小幅正價差）
    "futures_oi":    "188,000",          # 未平倉量（口）（估、漲停日空單回補、OI 自高檔回落）
    "futures_basis": "+6",               # 估小幅正價差（自連日 -8 逆價差翻正）
    "call_oi":       "121,000",
    "put_oi":        "112,000",
    "pcr":           "0.93",             # Put/Call 比率（估、自 0.98 回落——暴力反彈後避險 Put 需求驟降、Call 追價）
    "pcr_signal":    "中性偏多（Put/Call 估 0.93；7/31 現貨漲停＋外資期現貨同步翻多、避險需求解除中；惟漲停後隔日常見獲利了結賣壓、今晨現貨 -2.27% 溫和拉回；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,205", "Call 40,000 / Put 14,000", "7/30 收盤、V 型起點"),
        ("2,292", "Call 38,000 / Put 12,000", "5MA、短線支撐"),
        ("2,345", "Call 36,000 / Put 11,000", "7/31 低、跳空缺口上緣"),
        ("2,382", "Call 34,000 / Put 10,000", "20MA／布林中軌、多方防線"),
        ("2,400", "Call 45,000 / Put 15,000", "整數關卡、估最大痛苦"),
        ("2,425", "Call 42,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,445", "Call 38,000 / Put 8,000",  "7/22 高、反壓第一關"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/31 漲停 NT$2,425）；估最大痛苦 2,400、下方支撐 2,382（20MA）/ 2,345（7/31 低）/ 2,292（5MA）、上方壓力 2,445（7/22 高）/ 2,535（史高）/ 2,543（布林上軌）；⭐ 外資 +13,780 張終結連 7 賣＋KDJ 黃金交叉 vs ⚠️ 漲停後獲利了結＋韓股今晨回吐；✅ 基本面錨未變（2nm Q3 放量、AMZN CapEx 上看 $220B、全年上修 >40%）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-31 Fri TWSE 官方收盤；美股為 7/31 Fri 收盤、Yahoo Finance API 確認；韓股為 7/31 收盤、8/3 今晨回吐）
    ("設備", "ASML",    "艾司摩爾",    "1,629.00", "-1.36%", "✅ EUV 設備龍頭；7/30 +6.50% 反彈後 7/31 小幅整理；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "507.67", "+1.18%", "⭐ 7/30 暴漲 +14.97% 後 7/31 續漲——兩日收復股災大半跌幅；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "293.02", "-1.58%", "⭐ 7/30 財報超預期暴漲 +17.99%（1999 年來最佳單日）後 7/31 小幅獲利了結；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "182.82", "+1.38%", "📈 7/30 +5.96%、7/31 續漲——自 7/29 重挫 -10.80% 低點回升；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","130.00","+9.70%", "⭐ 7/31 反彈近漲停收 130.0（TWSE 官方；7/30 曾再挫 -8.85% 收 118.50）；與 TSMC 合作 CUBE 特殊 DRAM 題材未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,718,000 KRW", "+29.95%", "⭐ 7/31 暴漲 +29.95% 收 171.8 萬韓元（SK 會長崔泰源加碼買股＋空頭/槓桿 ETF 回補放大）；⚠️ 今晨（8/3 09:40）回吐 -7.39% 至 159.1 萬——雙向劇震未息；Q2 財報創史高＋HBM4 量產出貨的基本面未變"),
    ("記憶", "MU",      "美光",        "823.03", "-5.90%", "📊 7/30 暴漲 +18.36% 收 $874.66 後 7/31 獲利了結 -5.90% 收 $823.03；HBM/DDR5 高波動、自 7/23 高 $990 仍 -16.9%"),
    ("封裝", "3711.TW", "日月光",      "555.00", "+9.90%", "⭐ 7/31 漲停收 555（TWSE 官方、7/30 +1.20% 收 505 先止穩）；CoWoS / 先進封裝、與 2330 同步 V 型反轉"),
    ("封裝", "AMKR",    "Amkor",       "49.87",  "+3.34%", "📈 7/30 +12.94%、7/31 續漲 +3.34% 收 $49.87——自 7/28 崩跌 -24.74% 低點連 3 日回升；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材未變"),
    ("客戶", "NVDA",    "輝達",        "200.75", "+2.93%", "⭐ TSMC #1 客戶 22%/$33B、A16 首發客戶；7/31 +2.93% 收復 $200 關卡收 $200.75（兩日 +5.7%）——AMZN AWS +37% 財報再證 AI 算力需求"),
    ("客戶", "AAPL",    "蘋果",        "308.91", "-7.35%", "⚠️ #2 客戶 18%；7/30 盤後財報營收 $109.4B／EPS $2.02 雙超預期、惟 9 月季指引低於共識＋Cook 坦承晶片供應吃緊——7/31 重挫 -7.35% 收 $308.91；Cook 卸任前最後法說（9/1 Ternus 接任）；供應吃緊對 TSMC 反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "389.28", "+0.37%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/30 +4.73% 反彈後 7/31 續穩）"),
    ("客戶", "AMD",     "超微",        "476.15", "-1.90%", "📈 #4 客戶；7/30 暴漲 +13.0% 收 $485.39 後 7/31 小幅整理收 $476.15；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片"),
    ("客戶", "2454.TW", "聯發科",      "3,555",  "+9.89%", "⭐ 手機 SoC 客戶 9%；7/31 漲停收 3,555（TWSE 官方、7/30 +2.70% 收 3,235 先反彈）——自 7/28 跌停 V 型反轉"),
    ("競爭", "INTC",    "英特爾",      "90.20",  "-1.02%", "📈 7/30 大漲 +11.30% 收 $91.13 後 7/31 小幅整理；市場並傳 TSMC「類 EMIB」先進封裝技術與 Intel 對標（媒體報導）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "262,500 KRW", "+26.81%", "⭐ 7/31 暴漲 +26.81% 收 26.25 萬韓元（Yahoo API）；⚠️ 今晨（8/3 09:40）回吐 -7.62% 至 24.25 萬——韓股槓桿雙向劇震；7/30 韓媒報導 Q2 營益創新高"),
    ("產業", "SOX",     "費城半導體指數","11,311.08", "+0.07%", "⭐ 7/30 暴漲 +8.19% 收 11,302.99 終結連跌後、7/31 高檔續穩收 11,311.08——自 6/22 史高 14,634.66 收斂至 -22.7%（自 -28.6% 修復）；MSFT/AMZN 財報驗證 AI CapEx 為反轉主因"),
    ("處分後", "ARM",   "Arm Holdings","239.69", "-0.77%", "TSMC 4/29 全數出脫後（7/30 +7.40% 反彈、7/31 -0.77% 持平整理——高 Beta 雙向劇震）"),
]
...
RELATIVE = {
    "date":           "2026-07-31",       # 最新資料日（台股 2330 與 NYSE TSM 均為 7/31 Fri 收盤）
    "tsmc_tw_1d":     +9.98,   # 2330 7/31 官方漲停收 NT$2,425（+220 vs 7/30 NT$2,205、TWSE）
    "taiex_1d":       +7.98,   # 加權指數 7/31 收 43,119.75（+3,186.45、史上最大單日漲點、TWSE 官方）
    "sox_1d":         +0.07,   # SOX 7/31 收 11,311.08（7/30 曾 +8.19%、Yahoo）
    "tsmc_nyse_1d":   +0.23,   # TSM ADR 7/31 $404.25（7/30 曾 +7.64% 收 $403.31）
    "tsmc_tw_5d":     +3.19,   # 5 日（NT$2,425 vs 7/24 官方收 NT$2,350）
    "taiex_5d":       -1.23,   # （43,119.75 vs 7/24 43,654.84）
    "tsmc_tw_1m":     +0.62,   # 月線（NT$2,425 vs 6/30 官方收 NT$2,410）
    "taiex_1m":       -6.52,   # （43,119.75 vs 6/30 46,125.91、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +53.0,   # YTD 累計（NT$2,425 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY；本欄過往為估值、已依官方數據重算更正）
    "taiex_ytd":      +46.9,   # （43,119.75 vs 1/2 官方收 29,349.81、FMTQIK；過往估值已更正）
    "sox_ytd":        +53.5,   # SOX YTD（11,311.08 vs 1/2 收 7,367.47、Yahoo；過往估值已更正）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/31 NYSE 收盤 & 7/31 TW 官方收盤計算）
    "adr_price":     404.25,   # TSM NYSE 7/31 收盤（+0.23%、Yahoo Finance API）
    "adr_parity":    374.23,   # 理論值 = 5 × NT$2,425 ÷ 32.40
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.40,   # 台幣匯率（7/31 Yahoo TWD=X；今晨 8/3 升至 32.29）
    "adr_premium_pct": "+8.02", # (404.25 - 374.23) / 374.23 × 100 = +8.02%（匯率 32.40）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 7/31 Fri 收盤：台股漲停 NT$2,425（+9.98%、量 69,478 張 TWSE 口徑、市值 NT$62.9 兆）——加權指數同日 +7.98%（+3,186.45 點、史上最大單日漲點）；NYSE $404.25（+0.23%、7/30 曾 +7.64%）——SOX 兩日 +8.3% 收 11,311.08。📊 籌碼面（T86 官方）：7/31 外資買超 +13,780 張（終結連 7 賣），投信 +7,063（連 6 買、爆量）、自營 +2,724（連 7 買）、合計 +23,568 張；全市場 BFI82U 三大法人買超 +873.1 億（外資 +675.5 億、投信 +360.2 億）——自 7/30 -495.4 億一日翻正。📊 台美 ADR 溢價收斂至 +8.02%（$404.25 vs 理論值 $374.23、匯率 32.40）——台股漲停補漲後溢價自 +10.2% 正常化。⭐ V 型反轉驅動：MSFT/AMZN 財報連續驗證 AI CapEx（Azure +43%、AWS +37%、AMZN CapEx 上看 $220B）；✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量 + CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "⭐ 史上最大漲點：台股 +3,186 點、2330 漲停收 2,425",
        "body": "股災後 V 型反轉：（1）⭐ 台股 7/31 大漲 +3,186.45 點（+7.98%）收 43,119.75——創史上最大單日漲點（7/30 曾 -105.88 收 39,933.30 探底）；惟 7 月全月仍累跌約 3,006 點；（2）📊 2330 跳空高開 2,350、尾盤鎖漲停收 NT$2,425（+220、+9.98%、量 69,478 張為 7/17 以來最大、成交額 NT$1,666.62 億）——媒體稱台股史上第 3 次 2330 漲停；供應鏈全面漲停：聯發科 +9.89%、日月光 +9.90%、聯電 +10.00%、華邦電 +9.70%；（3）✅ 籌碼一日翻多：外資 +13,780 張終結連 7 賣、投信 +7,063 爆量、自營 +2,724、三大合計 +23,568 張；全市場 BFI82U 買超 +873.1 億（外資 +675.5 億）——自 7/30 -495.4 億翻正。",
        "source": "TWSE 官方（STOCK_DAY/T86/BFI82U）2026-07-31", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⭐ AMZN AWS +37% 創 2021 來最快、CapEx 上看 $220B",
        "body": "7/30 盤後財報為 AI CapEx 驗證鏈最後一棒：（1）✅ AMZN Q2 營收 $200.6B（+20% YoY）、調整後 EPS $1.97 超預期（est $1.82）；AWS 成長 +37% 至 $42.2B——大幅超預期（est +31%）、創 2021 年以來最快增速；AI 與自研晶片（Trainium）業務各達年化 $25B、三位數成長；全年 CapEx 上看 $220B——7/31 大漲 +15.32% 收 $271.58（2012 年以來最大單日漲幅）；（2）🎯 對 TSMC 意涵：MSFT（Azure +43%）→ AMZN（AWS +37%）連續兩晚以數字反駁「AI 回報質疑」——hyperscaler CapEx 不僅未收縮且持續擴張、自研晶片放量對 TSMC 先進製程／CoWoS 為直接增量訂單、AI 訂單能見度進一步上修。",
        "source": "CNBC / Bloomberg / aboutamazon.com 2026-07-30~31", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⚠️ AAPL 雙 beat 惟指引保守 -7.35%；Cook：晶片供應吃緊",
        "body": "TSMC 第 2 大客戶財報兩面解讀：（1）📊 AAPL FYQ3 營收 $109.4B（+16% YoY）、EPS $2.02 均超預期（est $1.89）、毛利率 50.1%；iPhone $54.3B（+22%、6 月季新高）、Mac +29%、惟服務 $30.7B（+12%）未達預期、iPad -6%；（2）⚠️ 9 月季指引低於市場共識＋Tim Cook 坦承「晶片供應吃緊、低估自身需求」——7/31 重挫 -7.35% 收 $308.91；本次為 Cook 卸任前最後一次法說（9/1 起由 John Ternus 接任 CEO）；（3）🎯 對 TSMC 意涵：「供應吃緊、低估需求」反證上游投片需求強勁而非疲弱——利空在 AAPL 自身（指引與交接不確定性）、對 TSMC 淨效果偏正面；惟指引保守提示終端消費電子需求分化、留意 N3／N4 手機端拉貨節奏。",
        "source": "Apple Newsroom / CNBC / digitalapplied 2026-07-30~31", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 美股半導體 V 型反彈：LRCX +18% 創 1999 來最佳、SOX 兩日 +8.3%",
        "body": "美股 7/30 暴力反彈、7/31 高檔續穩：（1）⭐ 7/30：LRCX 財報超預期暴漲 +17.99%（1999 年以來最佳單日）、MU +18.36%、MSFT +15.51%（Azure +43% 財報）、AMAT +14.97%、AMD +13.0%、INTC +11.30%、TSM +7.64% 收 $403.31、SOX +8.19% 收 11,302.99 終結連跌；（2）📊 7/31 分化消化：TSM +0.23% 收 $404.25、NVDA +2.93% 收復 $200 關卡收 $200.75、SOX +0.07% 收 11,311.08（自 6/22 史高收斂至 -22.7%、自 -28.6% 修復）；MU -5.90%、AMD -1.90% 獲利了結；（3）💡 VIX 兩日 -22.6% 降至 15.99、S&P 500 +0.70% 收 7,489.72——恐慌情緒解除；另有 TSMC「類 EMIB」先進封裝對標 Intel 的報導助攻 ADR（媒體報導、未經官方證實）。",
        "source": "Yahoo Finance API / CNBC 2026-07-30~31", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 韓股 7/31 暴漲（SK +30%）後、今晨 8/3 回吐近 5%",
        "body": "韓股槓桿市場雙向劇震未息：（1）⭐ KOSPI 7/31 收 6,595.45（+17.9% vs 7/30 5,593.56、Yahoo API；媒體稱史上最大單日反彈）——SK 海力士 +29.95% 收 171.8 萬韓元、三星 +26.81% 收 26.25 萬；催化：MSFT/AMZN 財報＋SK 會長崔泰源加碼買進 SK 海力士釋出信心＋空頭與槓桿 ETF 機械式回補放大漲幅；（2）⚠️ 今晨（8/3 09:40）即大幅回吐：KOSPI -4.78% 至 6,279.91、SK 海力士 -7.39%、三星 -7.62%——印證 7/31 暴漲含高比例技術性買盤、雙向波動放大；（3）🎯 對台股意涵：韓股為本輪股災震央、其回吐幅度為今日亞股最大變數；台股今晨（09:03）大盤僅 -0.26%、2330 -2.27% 相對淡定——籌碼結構（外資翻買）優於韓股。",
        "source": "Yahoo Finance API / CNBC / Invezz 2026-07-31~08-03", "impact": "負面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 232 條款 7 月底窗口已過仍未公布；台美關稅談判傳 20%→15%",
        "body": "政策面懸而未決：（1）📅 市場原預期美 232 條款半導體調查結果 7 月底出爐——窗口（7/30-31）已過、截至 8/3 晨尚未查得官方公布（未能證實有結果）——不確定性延長至本週；（2）📊 台經院先前分析：先進製程受影響有限、衝擊較大為產能過剩的成熟製程——TSMC 先進製程壟斷＋$165B 美國投資承諾為緩衝；（3）⭐ 台美貿易協議談判持續、市場推測台灣關稅可能由現行 20% 下調至約 15%；（4）🎯 若 232 結果溫和＋關稅下調兌現、為 V 型反轉後的第二波政策催化；反之為尾部風險——本週為關鍵觀察窗口。",
        "source": "工商時報 / 中時新聞網（背景 2026-07-24）；8/3 查證未見新公布", "impact": "中性",
    },
]

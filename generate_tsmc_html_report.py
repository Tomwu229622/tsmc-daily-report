...
STOCK = {
    # 台股 2330
    "tw_price":       "2,390",        # 2026-06-25 Thu 官方收盤（0.00% 平盤 vs 6/24 NT$2,390；尾盤賣壓自 2,410 殺回 2,390；漲價利多 vs 估值/營收疑慮拉鋸；6/26 Fri 今日交易中、官方收盤待 TWSE。OHLC/量為官方確認）
    "tw_change":      "0",            # 6/25 漲跌點數（平盤，0.00%）
    "tw_change_pct":  "0.00%",        # 6/25 漲跌幅（平盤、尾盤賣壓抹去日內漲幅）
    "tw_volume":      "34,271",       # 6/25 官方成交量（張）、成交值 NT$822.97 億
    "tw_open":        "2,410",        # 6/25 開盤（官方）
    "tw_high":        "2,420",        # 6/25 盤中高（官方）
    "tw_low":         "2,390",        # 6/25 盤中低（官方、收最低）
    "market_cap":     "NT$62.0 兆",   # 6/25 收盤市值（NT$2,390 基礎）
    "pe_ratio":       "~35.2x",       # 台股 P/E（TTM，NT$2,390 基礎）
    "pb_ratio":       "~12.4x",       # 台股 P/B（股淨比，NT$2,390 基礎）
    "div_yield":      "~0.63%",       # 殖利率（NT$2,390 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "440.83",       # 6/25 Thu 收盤（+$4.44，+1.02%；漲價題材、市值約 $2.05 兆、P/E ~31.6）；⚠️ 盤後一度 +2.48% 至 $451.76、惟 6/26 盤中漲幅回吐、最新約 $435.42（日內區間 $432.50–$452.56、回吐隔夜漲幅）
    "nyse_change":    "+4.44",        # 6/25 漲跌（6/25 收盤確認）
    "nyse_change_pct":"+1.02%",       # 6/25 NYSE 續漲（晶圓代工漲價範圍擴大至 7nm 以下全製程／佔晶圓營收 ~75%）；⚠️ 6/26 盤中回吐隔夜漲幅、最新約 $435.42（-1.2% vs $440.83）
    "52w_high_nyse":  "476.79",       # 52 週/歷史高（區間）
    "52w_low_nyse":   "220.80",       # 52 週低（區間）
    "pe_nyse":        "~31.6x",       # NYSE USD 基礎 P/E（TTM，$440.83 基礎）
    "pb_nyse":        "~12.7x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-06-26 Fri（NYSE TSM 6/25 Thu 收盤 $440.83 為最新確認收盤；台股 2330 6/25 Thu 官方收 NT$2,390 為最新確認；台股 6/26 Fri 今日交易中、官方收盤請以 TWSE 為準）。⭐ 今日頭條——TSMC 晶圓代工漲價題材為結構性主軸、惟隔夜漲幅回吐：（1）⭐ 漲價範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 75%、含 N7/N5/N3）、幅度 5-10% 視客戶/節點/產品而定、部分已開始實施，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（2）📈 NYSE TSM 6/25 Thu 收 $440.83（+1.02%、+$4.44、市值約 $2.05 兆、P/E ~31.6），⚠️ 盤後一度 +2.48% 至 $451.76（日內高 $452.56），惟 6/26 盤中漲幅回吐、最新約 $435.42（-1.2% vs $440.83、日內區間 $432.50–$452.56）——隔夜漲價題材熱度降溫、估值與營收疑慮拉鋸；（3）📊 台股 2330 6/25 Thu 收 NT$2,390 平盤（0.00%、量 34,271 張、成交值 NT$822.97 億、開 2,410／高 2,420／低 2,390、尾盤賣壓自 2,410 殺回 2,390）；6/26 Fri 今日台股交易中、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/25 ADR $440.83 + 台股 NT$2,390 計）：理論值 5×2,390÷31.05 = $384.86、溢價 (440.83-384.86)/384.86 = +14.54%（若以 6/26 最新 $435.42 計則收斂至 +13.14%）。⚠️ 漲價雙面刃——利多毛利率，但傳客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）措手不及、恐促部分客戶調整下單量、要求更嚴格合約或轉向替代；Benzinga 分析漲價壓縮 NVIDIA 毛利但因 AI 需求剛性、NVIDIA 具轉嫁能力、長期反為利多。⚠️ 近期營收疑慮續存——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、6 月營收（7 月上旬公布）為關鍵 catalyst、僅需小幅月增即可使 Q2 營收逼近指引中值。📈 24/7 Wall St 6/24 專文探討「TSM 年底前能否站上 $500」、認為 AI 需求+漲價+2nm 量產為支撐。💰 TSMC 董事會核准大額資本支出、發行新債並小幅減資以支應今年 9 期晶圓廠與先進封裝廠擴張。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2027 量產、NVIDIA-TSMC AI 製程合作（cuLitho/缺陷檢測/工廠排程）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%；Applied Materials 6/25 推新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）；Tensordyne 預期其 TSMC 代工的 Napier AI 推論晶片獲逾 $200M 訂單。⚠️ 風險續追蹤——（1）TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（2）Apple 同意與 Intel 合作在美生產晶片降低對 TSMC 依賴、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung。  |  🎯 即將事件：ITC 初判 6 月底；6 月營收 7 月上旬公布（關鍵 catalyst、攸關 Q2 指引達成）；Q2 2026 財報法說會 7/16（指引 $39.0-40.2B、+10% QoQ、+32% YoY、毛利率 65-67%、漲價後上修空間）  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；2026 全球半導體市場估 +26% YoY 達 ~$975B；SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆  |  ⚠️ 本日（6/26）關鍵觀察：（1）台股 2330 在 ADR 隔夜漲幅回吐下能否守穩 NT$2,350 / NT$2,310；（2）ADR 溢價 +14.54%（最新 $435.42 計 +13.14%）、台股短線承壓；（3）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⚠️ 估值警示：TSM P/E ~31.6x、台股 P/E ~35.2x，留意 6 月營收 miss 風險、漲價客戶反彈、台幣升值與 ITC 初判雜訊",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ TSMC 漲價題材為結構性主軸（7nm 以下全製程、佔晶圓營收 ~75%、估全年毛利率 +2pp）；⚠️ 隔夜漲幅回吐——NYSE TSM 6/25 收 $440.83、盤後一度 +2.48% 至 $451.76 但 6/26 盤中回吐至約 $435；台股 2330 6/25 平盤收 NT$2,390",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ 漲價範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 75%、含 N7/N5/N3）、幅度 5-10%、部分已實施、估貢獻全年毛利率 +2pp 以上",
        "⚠️ NYSE TSM 6/25 收 $440.83（+1.02%）、盤後一度 +2.48% 至 $451.76（日內高 $452.56），惟 6/26 盤中漲幅回吐、最新約 $435.42（-1.2%）——隔夜熱度降溫",
        "📊 台股 2330 6/25 平盤收 NT$2,390（0.00%、量 34,271 張、尾盤賣壓自 2,410 殺回 2,390）；6/26 今日交易中、官方收盤待 TWSE",
        "📊 ADR 溢價 +14.54%（以 6/25 ADR $440.83 計；最新 $435.42 計收斂至 +13.14%）",
        "⚠️ 漲價雙面刃：客戶（Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多；4-5 月營收 +24% YoY 落後 ~35% 預期、6 月營收關鍵",
        "🏆 中長線基底未變：NVIDIA #1 客戶/A16 首發 2027 + NVIDIA-TSMC AI 製程合作 + TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + 董事會核准大額 CapEx/發債支應 9 期擴廠",
        "⚠️ 風險續追蹤：TSMC 遭美 ITC 專利調查 6 月底待初判；Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-06-26 Fri（NYSE TSM 6/25 Thu 收盤 $440.83 為最新確認收盤；台股 2330 6/25 Thu 官方收 NT$2,390 為最新確認；台股 6/26 Fri 今日交易中、官方收盤請以 TWSE 為準）。⭐ 今日頭條——TSMC 晶圓代工漲價題材為結構性主軸、惟隔夜漲幅回吐：（1）⭐ 漲價範圍確認擴大至 7nm 以下全部先進製程（佔晶圓營收約 75%、含 N7/N5/N3，其中 N3 單一即佔 Q1 晶圓營收 25%）、幅度 5-10% 視客戶/節點/產品而定、部分已開始實施且要求客戶將更高成本計入未來訂單，以平均 5% 估算可貢獻全年毛利率 +2pp 以上；消息帶動 NYSE TSM 6/25 Thu 收 $440.83（+1.02%、+$4.44、市值約 $2.05 兆、P/E ~31.6），⚠️ 盤後一度 +2.48% 至 $451.76（日內高 $452.56），惟 6/26 盤中漲幅回吐、最新約 $435.42（-1.2% vs $440.83、日內區間 $432.50–$452.56）——隔夜漲價題材熱度降溫、估值與營收疑慮拉鋸；（2）📊 台股 2330 6/25 Thu 收 NT$2,390 平盤（0.00%、量 34,271 張、成交值 NT$822.97 億、開 2,410／高 2,420／低 2,390、尾盤賣壓自 2,410 殺回 2,390）；6/26 Fri 今日台股交易中、官方收盤待 TWSE。📊 台美 ADR 溢價（以 6/25 ADR $440.83 + 台股 NT$2,390 計）：理論值 5×2,390÷31.05 = $384.86、溢價 (440.83-384.86)/384.86 = +14.54%（若以 6/26 最新 $435.42 計則收斂至 +13.14%）。⚠️ 漲價雙面刃：利多毛利率（估全年 +2pp 以上），但傳客戶（含 Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）措手不及、恐促部分客戶調整下單量、要求更嚴格合約條款或轉向替代；Benzinga 分析指漲價雖壓縮 NVIDIA 毛利、惟因 AI 需求剛性與 NVIDIA 議價/轉嫁能力強、長期反為 NVIDIA 股價利多。6 月營收（7 月上旬公布）為關鍵 catalyst——TSMC 4-5 月營收合計 +24% YoY 低於華爾街 Q2 約 +35% 預期、惟即使僅小幅月增亦可能使 Q2 營收逼近指引（$39.0-40.2B）中值。⭐ 中長線結構性利多基底未變：（1）🤝 TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產、~2,000 就業、InFO/CoWoS、呼應 TSMC $165B 美國投資承諾）；（2）🏆 NVIDIA 確認 TSMC 第 1 大客戶（超越 Apple、22% / $33B）、為 A16（1.6nm）首發客戶 2027 量產；Apple 跳過 A16 直上 A14（1.4nm）；（3）⭐ NVIDIA-TSMC 深化 AI 導入晶圓廠合作（cuLitho 運算微影、缺陷檢測、工廠排程）、提升良率與產能；NVIDIA RTX Spark 超晶片（ARM Grace + Blackwell、1 petaflop）由 TSMC 代工；（4）⭐ TSMC 2nm 已量產、初期良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（5）📦 CoWoS 規劃 2026 底擴至 ~130K wpm 良率 >98%、CoPoS 面板級封裝互補緩解 AI 封裝瓶頸；（6）4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（7）⭐ 5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆；（8）2026 全球半導體市場估 +25% YoY 達 ~$975B、SIA/Deloitte 估 AI 資料中心晶片營收 2028 達 ~$1.2 兆。⚠️ 短期/中期下行風險：（1）⚠️ 漲價恐引發客戶反彈與訂單調整；（2）⚠️ 近期營收 miss 風險——4-5 月 +24% YoY 落後 Q2 ~35% 預期、6 月營收為關鍵驗證；（3）⚠️ 法律風險——TSMC 遭美 ITC 專利調查（Longitude Licensing & Marlin Semiconductor 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（4）⚠️ 客戶分散與競爭——Apple 同意與 Intel 合作在美生產晶片、Intel 18A-P 進入風險量產並與 UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（5）⚠️ 估值警示——TSM P/E ~31.6x、台股 P/E ~35.2x；（6）台幣升值壓力（每升 1% 毛利率 -40bps）。基本面未變：Q1 2026 淨利 NT$572.5B（$18.2B、+58.3% YoY）、毛利率 66.2% 雙創史高；2025 全年 EPS NT$66.25。技術面（基於 6/25 收 NT$2,390）：5MA NT$2,415 / 20MA NT$2,400 / 60MA NT$2,360 / 120MA NT$2,255；下方支撐 NT$2,350 / NT$2,310 / NT$2,250；上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535（史高）。32 位分析師全數看多、平均目標 NT$2,530 上行空間 +5.86%（自 NT$2,390、Susquehanna 6/23 上調 $575、待 7/16 法說會後重估）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（6/26 Fri）關鍵觀察：（1）⭐ TSMC 漲價題材為結構性主軸——範圍確認擴大至 7nm 以下全製程（佔晶圓營收 ~75%、含 N7/N5/N3）、幅度 5-10%、部分已實施、估全年毛利率 +2pp 以上；NYSE TSM 6/25 收 $440.83（+1.02%）、盤後一度 +2.48% 至 $451.76，⚠️ 惟 6/26 盤中漲幅回吐至約 $435.42（-1.2%、日內區間 $432.50–$452.56）——隔夜熱度降溫；（2）📊 台股 2330 6/25 平盤收 NT$2,390（0.00%、量 34,271 張、尾盤賣壓自 2,410 殺回）；6/26 今日交易中、官方收盤待 TWSE；（3）📊 ADR 溢價 +14.54%（最新 $435.42 計收斂至 +13.14%）、台股短線承壓；（4）⚠️ 漲價雙面刃——客戶（Apple/NVIDIA/AMD/Qualcomm/Broadcom/MediaTek）成本升、Benzinga 指 NVIDIA 可轉嫁長期反利多、惟部分客戶恐調整下單量或轉向替代；（5）⚠️ TSMC 4-5 月營收合計 +24% YoY 低於華爾街 ~35% 預期、6 月營收（7 月上旬）為關鍵 catalyst；（6）📈 Susquehanna 將 TSM 目標價自 $500 上調至 $575；（7）🛠️ Applied Materials 6/25 推新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）；（8）🤝 TSMC-Amkor 簽 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）；（9）🏆 NVIDIA 確認 TSMC 第 1 大客戶（22%/$33B）、為 A16 首發客戶 2027 量產；NVIDIA-TSMC 深化 AI 製程合作；（10）⚠️ TSMC 遭美 ITC 專利調查（Longitude/Marlin 控訴）、6 月底預計初判、恐對含 AI 加速器技術晶片發布美國進口禁令；（11）⚠️ 風險續追蹤——Apple-Intel 合作、Intel 18A-P 風險量產+UMC 結盟、Google/AMD/Tesla 接觸 Samsung（中期觀察、技術代差仍領先）；（12）⭐ 2nm 量產良率 70-80% 領先全球、2nm/A16 產能 2026-28 估 CAGR ~70%；（13）⭐ 4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；（14）⚠️ 估值警示：TSM P/E ~31.6x、台股 P/E ~35.2x、留意 6 月營收 miss 風險與台幣升值；32 位分析師全數看多、平均目標 NT$2,530（上行 +5.86%、待法說會重估）；本日（6/26）關鍵：台股能否跟進 ADR 大漲收復 NT$2,450 / NT$2,490；下方支撐 NT$2,350 / NT$2,310；上方壓力 NT$2,490 / NT$2,510 / NT$2,535 史高",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-06-25",       # 最新資料日（6/25 Thu 三大法人為估算、官方以 TWSE 盤後為準；6/26 今日盤後待公布）
    "foreign":  +1500,              # 外資 6/25 估賣壓收斂、漲價利多下小幅回補（台股平盤）；待官方確認
    "trust":    +500,               # 投信 6/25 估逢低小買 +500 張
    "dealer":   +200,               # 自營商 6/25 估小買 +200 張
    # 近5日（由舊到新：6/18, 6/22, 6/23, 6/24, 6/25）；6/19 端午休市；均為估算
    "foreign_5d": [+12000, +18000, -9000, -15000, +1500],    # 外資 6/25 估賣壓收斂、小幅回補
    "trust_5d":   [+1500, +2000, +500, +800, +600],          # 投信 6/25 估逢低小買
    "dealer_5d":  [+600, +800, -200, -500, +300],            # 自營 6/25 估翻多小買
    "foreign_ownership_pct": "74.8%",  # 外資持股比例（6/25 估持穩 74.8%）
    "foreign_ownership_shares": "約 193.9 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       52.0,   "rsi_signal": "RSI 估持平於 52（台股 6/25 平盤），位於中軸附近、動能中性；⚠️ 6/26 留意 NYSE 隔夜漲幅回吐（最新約 $435）對台股 RSI 之壓抑",
    "macd":         0.15,  "macd_signal": "MACD 估持平於 +0.15、紅柱動能持平、貼近零軸、⚠️ 多空於零軸附近拉鋸、待方向確認",   "macd_color": "#FF9800",
    "kdj_k":        48.0,   "kdj_d": 49.0, "kdj_j": 46.0, "kdj_signal": "KDJ K(48)/D(49)/J(46) 中性偏弱、6/25 平盤、短線方向待 6/26 量能與 ADR 走勢確認",
    "ma5":       "2,400",   # 5MA（6/18 2385, 6/22 2510, 6/23 2490, 6/24 2390, 6/25 2390）— 基於 6/25 收
    "ma20":      "2,400",   # 20MA
    "ma60":      "2,360",   # 60MA 緩升
    "ma120":     "2,255",
    "bb_upper":  "2,570",   # 布林上軌
    "bb_mid":    "2,400",   # 布林中軌（20MA）
    "bb_lower":  "2,230",   # 布林下軌
    "vol_ma5":  "26,000",   # 5日均量（張，估、基於 6/18-6/25 五日均）
    "trend":    "⭐ TSMC 漲價題材為結構性主軸、惟 ADR 隔夜漲幅回吐：NYSE TSM 6/25 收 $440.83（+1.02%、漲價題材、市值約 $2.05 兆）、盤後一度 +2.48% 至 $451.76，惟 6/26 盤中回吐至約 $435.42（-1.2%、日內區間 $432.50–$452.56）；台股 2330 6/25 收 NT$2,390（平盤、量 34,271 張、尾盤賣壓自 2,410 殺回 2,390）；6/26 今日台股交易中、ADR 隔夜漲幅回吐恐壓抑台股。📊 技術面（基於 6/25 收 NT$2,390）：RSI 持平 52 位於中軸附近、MACD +0.15 貼近零軸、KDJ 中性、收盤略低於 5MA NT$2,400、中期 60MA/120MA 趨勢仍偏多。下一觀察：（1）⚠️ 台股能否守穩 NT$2,350 / NT$2,310、站回 5MA NT$2,400；（2）下方支撐 NT$2,350 / NT$2,310 / NT$2,250；（3）上方壓力 NT$2,450 / NT$2,490 / NT$2,510 / NT$2,535 史高；（4）⭐ 漲價利多估全年毛利率 +2pp 以上、惟須留意客戶反彈與 6 月營收（7 月上旬、4-5 月 +24% YoY 落後 ~35% 預期）；（5）⚠️ 雜訊續追蹤：ITC 專利調查 6 月底初判、Apple-Intel 合作、Intel 18A-P 風險量產；（6）⭐ 中長線結構性利多基底未變：Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + 2nm 量產 70-80% 良率 + Susquehanna 目標升至 $575。",
    "support":  "2,350 / 2,310 / 2,250 / 2,200 (整數、前低、整數、整數)",
    "resist":   "2,450 / 2,490 / 2,510 / 2,535 (整數、6/23 收、6/22 收、史高)",
    "note": "技術指標數值為估算（基於 6/25 收 NT$2,390），請以看盤軟體即時數據為準；台股 6/25 平盤 0.00%、NYSE 6/25 +1.02% 但盤後漲幅於 6/26 盤中回吐至約 $435、RSI 持平 52、KDJ 中性偏弱、MACD 貼近零軸；NT$2,350 為關鍵支撐、上方壓力 NT$2,450 / NT$2,490；6/26 今日台股交易中、⭐ 漲價利多 vs ⚠️ 隔夜漲幅回吐+營收 miss 風險拉鋸",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-06-25",       # 最新資料日（6/25 Thu 收盤估、6/26 今日盤後待公布）
    "futures_price": "2,392",            # 台積電期貨近月（6/25 收盤附近、小幅貼近現貨）
    "futures_oi":    "170,000",          # 未平倉量（口）（6/25 估、小幅減倉）
    "futures_basis": "+2",               # 6/25 估轉小幅正價差（漲價利多+ADR 盤後大漲、情緒回穩）
    "call_oi":       "110,000",
    "put_oi":        "76,000",
    "pcr":           "0.71",             # Put/Call 比率（6/25 估 0.71、台股平盤+ADR 隔夜漲幅回吐、避險買盤略增、情緒中性）
    "pcr_signal":    "中性（Put/Call 6/25 估 0.71；台股 6/25 平盤、ADR 隔夜漲幅於 6/26 回吐、避險買盤略增、情緒中性；⭐ 漲價利多為中長線支撐、⚠️ 短線估值/營收疑慮拉鋸）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、6/25 估）
    "key_strikes": [
        ("2,310", "Call 52,000 / Put 18,000", "前低支撐"),
        ("2,350", "Call 56,000 / Put 14,000", "整數支撐"),
        ("2,390", "Call 54,000 / Put 10,000", "6/25 收"),
        ("2,400", "Call 50,000 / Put 8,500",  "整數、最大痛苦"),
        ("2,450", "Call 44,000 / Put 6,000",  "整數壓力"),
        ("2,490", "Call 38,000 / Put 4,500",  "6/23 收"),
        ("2,535", "Call 30,000 / Put 3,000",  "盤中史高"),
    ],
    "note": "衍生品數據需每日至台灣期交所(taifex.com.tw)更新；6/25 PCR 自 0.72 回落至 0.69 反映台股小漲 +0.63% 及 ADR 反彈帶動避險買盤略減、情緒回穩、期貨轉小幅正價差；NT$2,400 為最大痛苦、下方支撐 NT$2,350、上方壓力 NT$2,450 / NT$2,490；⭐ 漲價題材延燒+ADR 6/25 +1.02% 且盤後 +2.48% 大漲或支撐多方、⚠️ 留意營收 miss 風險與客戶反彈；中長線結構性利多基底未變（TSMC-Amkor 美國封裝長約、NVIDIA 確認第 1 大客戶 + A16 首發、2nm 量產 70-80% 良率、Susquehanna 目標升至 $575）；⚠️ 雜訊續追蹤：ITC 專利調查 6 月底初判、Apple-Intel 合作",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股為 2026-06-25 Thu 收盤、台股為 6/25 Thu 收盤；價格為估算、自 6/23 拋售反彈）
    ("設備", "ASML",    "艾司摩爾",    "868.00", "+2.10%", "EUV 設備龍頭（6/24-25 自 6/23 拋售反彈；高 NA EUV 需求結構性多頭基底未變）"),
    ("設備", "AMAT",    "應用材料",    "522.00", "+2.40%", "🛠️ 6/25 推新一代設備加速 DRAM 與 AI 先進封裝（HBM/3D 堆疊）；TSMC EPIC Center $5B AI 合作；自 6/23 拋售反彈"),
    ("設備", "LRCX",    "科林研發",    "1,045.00", "+2.30%", "蝕刻設備（自 6/23 拋售反彈；半導體設備需求中長線創紀錄）"),
    ("設備", "KLAC",    "科磊",        "788.00", "+2.20%", "量測設備（自 6/23 拋售反彈；2nm 量產驅動）"),
    ("記憶", "000660.KS","SK 海力士",  "392,000 KRW", "+1.60%", "🤝 SK 海力士-TSMC 深化 HBM4 合作；6/24-25 亞股反彈；HBM4 需求結構性多頭"),
    ("記憶", "MU",      "美光",        "148.50", "+3.50%", "HBM / DDR5（自 6/23 暴跌 -11.4% 強勁反彈；HBM4 需求中長線強勁、記憶體漲價題材）"),
    ("封裝", "3711.TW", "日月光",      "322",    "+0.90%", "CoWoS / 先進封裝（自 6/23 反彈；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "50.50",  "+2.80%", "TSMC-Amkor 簽 10 年美國封裝長約、Peoria AZ $70 億 2028 投產；自 6/23 反彈（本波利多核心）"),
    ("客戶", "NVDA",    "輝達",        "273.00", "+1.90%", "🏆 確認 TSMC #1 客戶 22%、A16 首發客戶 2027 量產；Benzinga 指漲價長期反利多；NVIDIA-TSMC 深化 AI 製程合作"),
    ("客戶", "AAPL",    "蘋果",        "299.50", "+0.80%", "⚠️ #2 客戶 18%；面臨 TSMC 漲價成本；Apple 同意與 Intel 合作在美生產晶片以降低對 TSMC 依賴；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "468.00", "+2.10%", "⚠️ #3 客戶（自 6/23 反彈；面臨漲價成本；custom XPU 中長線強勁）"),
    ("客戶", "AMD",     "超微",        "515.00", "+2.50%", "#4 客戶（自 6/23 反彈；面臨漲價成本；MI350/MI450 GPU 搶市；傳接觸 Samsung 先進製程）"),
    ("客戶", "2454.TW", "聯發科",      "4,710",  "+0.86%", "手機 SoC 客戶 9%；自 6/23 反彈；面臨漲價成本；RTX Spark 由聯發科協同 NVIDIA 設計"),
    ("競爭", "INTC",    "英特爾",      "49.80",  "+1.20%", "Apple 將與 Intel 合作在美設計製造晶片；18A-P 進入風險量產（效能 +9%）；自 6/23 反彈；與 UMC 結盟挑戰 TSMC"),
    ("競爭", "005930.KS","三星電子",   "66,000 KRW", "+1.50%", "Samsung Foundry 6/24-25 亞股反彈；傳 Google/AMD/Tesla 接觸其先進製程；PLP 面板級封裝領先、與 TSMC CoPoS 競爭；HBM4E 送樣中"),
    ("處分後", "ARM",   "Arm Holdings","258.00", "+1.50%", "TSMC 4/29 全數出脫後（自 6/23 反彈；RTX Spark ARM 架構中長線受惠）"),
]
...
NEWS = [
    {
        ...
        "title": "⭐ TSMC 漲價範圍確認擴大至 7nm 以下全製程（佔晶圓營收 ~75%）、幅度 5-10%；NYSE TSM 6/25 收 $440.83、盤後一度 +2.48% 至 $451.76、惟 6/26 盤中回吐至約 $435",
        "body": "今日頭條——TSMC 晶圓代工漲價題材為結構性主軸、惟隔夜漲幅回吐：（1）⭐ 據 Tom's Hardware / Yahoo Finance 等報導，TSMC 已通知客戶漲價範圍擴大至 7nm 以下全部先進製程（含 N7/N5/N3、佔晶圓營收約 75%，其中 N3 單一即佔 Q1 晶圓營收 25%），幅度視客戶/節點/產品在 5-10% 區間，部分已開始實施、其餘要求客戶將更高成本計入未來訂單；以平均 5% 估算可貢獻全年毛利率 +2pp 以上；（2）📈 消息帶動 NYSE TSM 6/25 Thu 收 $440.83（+1.02%、+$4.44、市值約 $2.05 兆、P/E ~31.6），盤後一度 +2.48% 至 $451.76（日內高 $452.56）；⚠️ 惟 6/26 盤中漲幅回吐、最新約 $435.42（-1.2% vs $440.83、日內區間 $432.50–$452.56）——隔夜題材熱度降溫、估值與營收疑慮拉鋸；（3）📊 台股 2330 6/25 Thu 收 NT$2,390 平盤（0.00%、量 34,271 張、尾盤賣壓自 2,410 殺回 2,390）；6/26 今日交易中、官方收盤待 TWSE；（4）📊 ADR 溢價 +14.54%（最新 $435.42 計收斂至 +13.14%）。產能面：TSMC 今年規劃 9 期晶圓廠與先進封裝廠、2nm/A16 產能 2026-28 估 CAGR ~70%。",
        "source": "Tom's Hardware / Yahoo Finance / StockTwits 2026-06-24 - 06-26", "impact": "正面",
    },
    {
    ...
    },
    {
        ...
        "title": "🛠️ Applied Materials 推新一代設備加速 AI 先進封裝；TSMC 董事會核准大額 CapEx／發債／小幅減資支應 9 期擴廠；Tensordyne TSMC 代工晶片獲逾 $200M 訂單",
        "body": "半導體設備與產業需求持續擴張、TSMC 大舉擴產：（1）🛠️ Applied Materials（AMAT）6/25 發布新晶片製造系統，專為次世代 AI 先進 3D 晶片架構、加速 DRAM 與先進封裝（含 HBM 與 3D 堆疊）採用，緩解 AI 運算記憶體瓶頸；（2）💰 TSMC 董事會核准大額資本支出、發行新債並小幅減資，以支應今年 9 期晶圓廠與先進封裝廠擴張（5 月相關申報文件揭露）；（3）🔧 Tensordyne 表示其新 AI 推論系統獲逾 $200M 訂單、核心 Napier 晶片由 TSMC 代工、與 Broadcom／Juniper 共同開發——印證 TSMC 客戶基礎持續擴大；（4）📈 SIA/Deloitte：2026 全球半導體市場估 ~$975B（YoY +26%）、AI 資料中心晶片營收 2028 上看 >$1.2 兆、半導體於 AI 伺服器機櫃價值佔比達 95%；（5）⭐ TSMC 為先進製程與 CoWoS/CoPoS 先進封裝核心受惠者、與 AMAT EPIC Center $5B AI 合作深化。",
        "source": "GlobeNewswire / SIA / Deloitte / TipRanks / ts2.tech 2026-06-25 - 06-26", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "⚠️ TSMC 遭美 ITC 專利調查、6 月底預計初判；恐對含關鍵 AI 加速器技術晶片發布美國進口禁令",
        "body": "潛在法律與供應鏈風險、本週為關鍵時點：（1）⚠️ 美國國際貿易委員會（USITC）正對 TSMC 展開調查，源於授權公司 Longitude Licensing 與 Marlin Semiconductor 針對其先進製程節點提出的專利侵權控訴；（2）⚠️ 預計 6 月底作出初步裁決（preliminary ruling）——若不利於 TSMC，理論上可能對使用關鍵 AI 加速器技術製造的晶片發布美國進口禁令；（3）⭐ 影響評估：此類 ITC 案件多以授權金和解收場、實際全面進口禁令機率不高，但涉及 AI 加速器核心技術、若裁定不利恐衝擊 NVIDIA/AMD 等客戶供應鏈與市場情緒；（4）✅ TSMC 具龐大專利組合與法務資源、過往類似訴訟多能妥善處理；（5）🎯 觀察：留意 6 月底/7 月初 ITC 初判結果與 TSMC 回應。",
        "source": "USITC / Reuters / Bloomberg / ts2.tech 2026-06-18 - 06-25", "impact": "負面",
    },
    {
        ...
        "source": "ts2.tech / Seeking Alpha / Focus Taiwan 2026-06-24 - 06-25", "impact": "負面",
    },
    {

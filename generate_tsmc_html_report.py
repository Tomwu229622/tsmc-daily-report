...
STOCK = {
    # 台股 2330
    "tw_price":       "1,955",        # 收盤價（數字加逗號，不含 NT$）（2026-04-10 收盤，估；4/9 確認 NT$1,955）
    "tw_change":      "+115",         # 漲跌點數（含+/-號，vs 4/8 收盤估）
    "tw_change_pct":  "+6.25%",       # 漲跌幅（含+/-號）
    "tw_volume":      "28,000",       # 成交量（張，估，3 月營收公布日量能放大）
    "tw_open":        "1,960",
    "tw_high":        "2,000",
    "tw_low":         "1,940",
    "market_cap":     "NT$50.8 兆",
    "pe_ratio":       "~28.8x",       # 台股 P/E（TTM，2026-04-10 估）
    "pb_ratio":       "~10.5x",       # 台股 P/B（股淨比）
    "div_yield":      "~0.77%",       # 殖利率（已除息後調整，USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "366.57",       # 最新報價（2026-04-09/10，+6.15%）
    "nyse_change":    "+21.25",       # 與 4/8 收盤 $345.32 相比
    "nyse_change_pct":"+6.15%",
    "52w_high_nyse":  "390.21",       # 52 週高（2026-02-25）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~32.5x",       # NYSE USD 基礎 P/E（TTM）
    "pb_nyse":        "~10.4x",
    "beta":           "1.16",
    "div_next":       "Q2 除息已完成（4/9，US$0.968/ADR）；下次配息 2026-06",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-04-16（Q1 2026 財報倒計時 6 天）",
    # 資料日期備註
    "data_note":      "台股數據：2026-04-10 估（NT$1,955，+6.25%，3 月月營收公布日行情）  |  NYSE 數據：最新報價 $366.57（+6.15%）  |  ⭐ 今日（4/10）公布 3 月合併月營收（共識年增 ~30%）  |  Q2 除息（4/9）已完成（US$0.968/ADR）  |  4/16 Q1 財報倒計時 6 天  |  Goldman Sachs 上調 TSM 目標價  |  NVIDIA 超越 Apple 成 TSMC 最大客戶（22%，~$330B）",
}
...
SUMMARY = {
    ...
    "sentiment":  "看多",
    # 一句話標題
    "headline":   "台股 2330 估收 NT$1,955（+6.25%），今日公布 3 月月營收（共識年增 ~30%）為最大催化劑；NYSE TSM $366.57（+6.15%）強勢領漲",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330 估收 NT$1,955（+115，+6.25%），4/9 確認實際收盤 NT$1,955，顯著優於前日估算",
        "NYSE TSM 最新報價 $366.57（+6.15%），全球半導體板塊在關稅恐慌後強力反彈",
        "⭐ 今日（4/10）TSMC 公布 3 月合併月營收，共識年增 ~30%；1-2 月已分別達 +37%、+22%",
        "Goldman Sachs 上調 TSM 目標價；NVIDIA 超越 Apple 成 TSMC 最大客戶（22%，預估年貢獻 $330 億美元）",
        "Q2 除息（4/9，US$0.968/ADR）已完成；4/16 Q1 財報（EPS 共識 NT$20.38）倒計時 6 天",
    ],
    # 一句結論
    "bottom_line": "台積電（4/10）台股 2330 估收 NT$1,955（+6.25%），NYSE TSM $366.57（+6.15%）。今日為 3 月月營收公布日（共識年增 ~30%），若達標或超預期將強化上升動能。Q2 除息（4/9）已完成，短線除息補漲效應持續。4/16 Q1 財報（EPS 共識 NT$20.38）為最近期重大催化劑。Goldman Sachs 上調目標價，AI 需求驅動基本面持續強健，分析師 31 人全數買入，中長線看多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⭐ 今日（4/10）公布 3 月月營收，市場共識年增 ~30%；4/16 Q1 財報（EPS 共識 NT$20.38）；⚠️ 關稅政策不確定性持續，VIX 仍偏高；中國地緣政治風險存在；3nm 產能瓶頸至 2028 年，先進封裝成新約束",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-10",
    "foreign":  +2500,    # 外資（正=買超）— 4/10 月營收公布日加碼買進（估）
    "trust":    +420,     # 投信（持續逢低承接，月營收利多，估）
    "dealer":   +280,     # 自營商（估，月營收公布日跟多）
    # 近5日（由舊到新，最後一筆為今日估算）
    "foreign_5d": [-1200, -800, -3500, +1200, +2500],  # 4/10 月營收公布大幅回補（估）
    "trust_5d":   [+280,  +310,  +280,  +310,  +420],
    "dealer_5d":  [-200,  -180,  -350,  +150,  +280],
    "foreign_ownership_pct": "74.5%",  # 外資持股比例（估，月營收公布日大幅回補）
...
TECHNICAL = {
    ...
    "trend":    "台股 2330（4/10 估收 NT$1,955，+6.25%）持續強勁反彈，3 月月營收公布為主要催化劑，NYSE TSM $366.57（+6.15%）同步走強。RSI 回升至 48.5 接近中性水位，KDJ 黃金交叉確認，MACD 差距大幅收窄。突破 MA5 (1,900) 及 MA20 (1,860)，下一壓力在布林上軌 2,010。關鍵支撐上移至 1,900，4/16 Q1 財報為最近催化劑。",
    "support":  "1,900–1,920",
    "resist":   "2,000–2,025",
...
IR_CALENDAR = [
    ...
    ("2026-04-10", "⭐ 今日：月營收","TSMC 今日（4/10）公布 3 月合併月營收，市場共識年增 ~30%（1-2 月分別 +37%、+22%）","#E91E63"),
    ("2026-04-16", "Q1 2026 財報","預計法說會 15:00 TST；投資人電話會議同日；Q1 EPS 共識 NT$20.38；倒計時 6 天","#F44336"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "3,750",   "+1.8%",  False),  # 4/10 估，TSMC 月營收公布帶動上漲
    ("美元指數 DXY",    "101.2",   "-0.6%",  False),  # 貿易戰令美元持續走弱
    ("VIX 恐慌指數",    "38.5",    "-4.0",   False),  # 恐慌指數下降，TSMC 強勢帶動
    ("WTI 原油",        "$63.8",   "-0.6%",  False),  # 供應擔憂持續
    ("10Y 美債殖利率",  "4.08%",   "+0.03pp",True),   # 風險偏好回升，避險需求降低
    ("台幣/美元",       "31.55",   "-0.10",  False),  # 台幣走強（估，月營收利多）
    ("NVIDIA NVDA",     "$800.50", "+1.9%",  False),  # #1 TSMC 客戶，AI 需求強勁
...
VALUATION = {
    "pe_current":   28.8,   # 同 STOCK["pe_ratio"]（4/10 NT$1,955 估）
...
RELATIVE = {
    "date":           "2026-04-09",
    "tsmc_tw_1d":     +1.66,   # 2330 4/9 收盤估 1,840（+1.66%）
    "taiex_1d":       +1.50,   # 加權指數（4/9 估算，反彈）
    "sox_1d":         +1.80,   # SOX（4/9 估，反彈）
    "tsmc_nyse_1d":   +1.93,   # TSM ADR 4/9 $340.28（+1.93% 估）
    "tsmc_tw_5d":     -5.0,    # 本週累計（含今日反彈，週累計仍負）
    "taiex_5d":       -3.8,
    "tsmc_tw_1m":     -10.5,
    "taiex_1m":       -6.2,

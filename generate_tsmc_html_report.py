...
STOCK = {
    # 台股 2330
    "tw_price":       "2,440",        # 台股 2330 最新確認收盤：7/15 Wed 官方收 NT$2,440（+20、+0.83% vs 7/14 NT$2,420、TWSE）——加權指數同日大漲 +893.64 點（+2.00%）收 45,631.59、2330 漲幅明顯落後大盤屬法說會前觀望、收盤重新站回 5MA/20MA 之上
    "tw_change":      "+20",          # 7/15 Wed 收盤漲跌點數（+0.83%、TWSE 官方）
    "tw_change_pct":  "+0.83%",       # 7/15 Wed 收盤漲跌幅（大盤 +2.00% 下明顯落後、法說會前觀望）
    "tw_volume":      "33,666",       # 7/15 成交量（張、TWSE 口徑含盤後零股鉅額；成交額 NT$822.14 億、均價 2,442、成交 79,059 筆、量能自 7/14 42,857 張收斂）
    "tw_open":        "2,425",        # 7/15 開盤（TWSE）
    "tw_high":        "2,460",        # 7/15 盤中高（TWSE）
    "tw_low":         "2,415",        # 7/15 盤中低（TWSE）
    "market_cap":     "NT$63.3 兆",   # 7/15 收盤市值（NT$2,440 基礎、約 259.3 億股）
    "pe_ratio":       "~32.3x",       # 台股 P/E（TTM，NT$2,440 ÷ 近四季 EPS ~75.5、statementdog；TWSE 官方口徑 32.80）
    "pb_ratio":       "~18.3x",       # 台股 P/B（NT$2,440 ÷ 每股淨值 ~133.4、statementdog；TWSE 官方口徑 10.74 為最新季淨值基礎）
    "div_yield":      "~0.61%",       # 殖利率（NT$2,440 基礎；TWSE 官方口徑 0.90%）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "419.48",       # 7/15 Wed 官方收盤 $419.48（-$0.91、-0.22% vs 7/14 $420.39、日內 $410.78-428.88、量 1,683 萬股、Yahoo Finance API）——法說會前連 4 日小跌、盤中一度殺至 $410.78 尾盤收復；同日 SOX -2.08% 回檔（MU -8.02% 拖累）、TSM 相對抗跌
    "nyse_change":    "-0.91",        # 7/15 漲跌（自 7/14 $420.39）
    "nyse_change_pct":"-0.22%",       # 7/15 NYSE 小跌、SOX -2.08% 下相對抗跌、法說會前觀望
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~35.6x",       # NYSE USD 基礎 P/E（TTM 估，$419.48 基礎）
    "pb_nyse":        "~17.5x",
    "beta":           "1.18",
    "div_next":       "下次配息預計 2026-07",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-07-16（Q2 2026 財報法說會今日 14:00 登場）",
    # 資料日期備註
    "data_note":      "報告日期：2026-07-16 Thu（最新確認收盤：台股 2330 7/15 Wed 官方收 NT$2,440（+20、+0.83% vs 7/14 NT$2,420、開 2,425／高 2,460／低 2,415、量 33,666 張（TWSE 口徑）、成交額 NT$822.14 億、均價 2,442、成交 79,059 筆、TWSE）——加權指數同日大漲 +893.64 點（+2.00%）收 45,631.59、2330 漲幅明顯落後大盤屬法說會前觀望、收盤重新站回 5MA ~2,436 / 20MA ~2,430；NYSE TSM 7/15 Wed 收 $419.48（-$0.91、-0.22% vs 7/14 $420.39、日內 $410.78-428.88、量 1,683 萬股、Yahoo Finance）——法說會前連 4 日小跌、盤中一度殺至 $410.78 尾盤收復、同日 SOX -2.08% 回檔下相對抗跌）。🎯🎯 今日最重要：Q2 2026 財報法說會今日（7/16 Thu）14:00 登場——公布 Q2 財報 + Q3 展望；Q2 初步合併營收約 NT$1.27 兆（~$39.2B、季度新高）已確認達成美元指引 $39.0-40.2B；共識估營收 ~$40B（+33% YoY）、每 ADR EPS $3.81-3.87（去年同期 $2.47）、毛利率挑戰 70%（指引 65.5-67.5%）、獲利年增 +50%+ 可望連 5 季創高；關注是否上修全年財測（現行營收成長 >30%）與 2027-28 資本支出（Citi 估 $75-80B）。📊 三大法人 7/15（T86 官方）：外資賣超 3,427 張（連 5 賣、5 日累計 -34,792 張、惟賣壓較 7/14 -12,416 大幅收斂）、投信賣超 596 張（終止連買轉賣）、自營買超 456 張（自行 +547、避險 -91）、合計賣超 3,568 張。📊 台美 ADR 溢價約 +10.5%（ADR $419.48 vs 理論值 $379.47、匯率 32.15）。⚠️ 隔夜美股半導體回檔：SOX -2.08% 收 12,398.89、MU -8.02% 收 $904.28（HBM 暴漲後獲利了結）、AMAT -2.73%、LRCX -3.08%、INTC -4.43%、AMD -3.46%；記憶體重挫延續至 7/16 韓股盤中：SK 海力士 -10%、三星 -7.7%；⭐ 逆勢亮點：ASML 7/15 第二度上修 2026 展望（收 $1,815.27、+2.23%）、NVDA +0.33% 收 $212.50、AAPL +4.01% 收 $327.50。💰 漲價題材：3nm 擴及 5/7nm 調漲 5-10%、三星跟進 4/5nm 漲 15%——代工定價權確認；CoWoS 2026 產能 12.5 萬片/月（+20-30%）已被 NVIDIA/Broadcom 預訂一空、缺口延續至 2027 年底。🚀 分析師法說會前密集上調：美銀 $490→$590（7/14）、Barclays $625、Citi NT$3,800、Nomura NT$3,425、共識強力買入。⚠️ 風險：法說會後劇烈波動（台股收盤後 14:00 登場、效應 7/17 反映）、外資連 5 賣（惟收斂）、AI 記憶體鏈急回吐、美 232 條款半導體關稅 25% + 輸中晶片轉運查驗（SEMI 致函反對）、Intel 18A 量產競爭、估值不低（台股 P/E ~32.3x TTM、TSM ~35.6x）、台幣 ~32.15。技術面（TWSE 官方收盤計算）：7/15 收 2,440 重新站回 5MA ~2,436 / 20MA ~2,430、RSI 54.6 回中軸上、MACD 柱仍負（-7.0）惟 KDJ J 值 26 自超賣回升；支撐 2,415（7/15 低）／2,390／2,342（布林下軌）、壓力 2,460（7/15 高）／2,480／2,535 史高。⭐ 中長線基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）、2nm 量產 70-80% 良率、TSMC-Amkor 美國封裝長約、全先進製程漲價 5-10%（估毛利率 +2pp）、4 大 CSP 2026 AI CapEx $725B（+77% YoY）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "🎯🎯 Q2 法說會今日 14:00 登場：Q2 初步營收 NT$1.27 兆已確認達標、共識估 EPS $3.81-3.87/ADR（+55%）、毛利率挑戰 70%；台股 2330 7/15 收 NT$2,440（+0.83%）站回 5MA/20MA、惟大盤大漲 +2.00% 下明顯落後屬觀望；NYSE TSM 7/15 收 $419.48（-0.22%）於 SOX -2.08% 回檔中相對抗跌——法說會結果為今明兩日行情最大變數",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🎯🎯 最重要：Q2 2026 財報法說會今日（7/16 Thu）14:00 登場——Q2 初步合併營收約 NT$1.27 兆（~$39.2B、季度新高）已確認達成指引 $39.0-40.2B；共識估營收 ~$40B（+33% YoY）、每 ADR EPS $3.81-3.87（+55%+）、毛利率挑戰 70%（指引 65.5-67.5%）、獲利可望連 5 季創高；關注上修全年財測（現行 >30%）與 2027-28 資本支出——因台股 13:30 收盤，法說會效應主要 7/17 反映",
        "📈 台股 2330 7/15 Wed 官方收 NT$2,440（+20、+0.83%、開 2,425／高 2,460／低 2,415、量 33,666 張 TWSE 口徑、額 NT$822.14 億）——加權指數同日大漲 +893.64 點（+2.00%）收 45,631.59，2330 漲幅明顯落後大盤屬法說會前觀望、惟收盤重新站回 5MA ~2,436 / 20MA ~2,430",
        "⚠️ 隔夜美股半導體回檔：SOX -2.08% 收 12,398.89、MU -8.02%（HBM 暴漲後獲利了結）、AMAT -2.73%、INTC -4.43%、AMD -3.46%；記憶體重挫延續至 7/16 韓股盤中：SK 海力士 -10%、三星 -7.7%——NYSE TSM 收 $419.48（-0.22%）相對抗跌；⭐ ASML 第二度上修 2026 展望（+2.23%）、AAPL +4.01%、NVDA +0.33% 撐盤",
        "📊 籌碼收斂：三大法人 7/15 合計賣超 3,568 張——外資賣超 3,427 張（連 5 賣、5 日累計 -34,792 張，惟較 7/14 -12,416 大幅收斂）；投信賣超 596 張（終止連買）、自營買超 456 張——法說會前觀望性調節、賣壓明顯降溫",
        "💰 漲價定價權確認：台積電通知 NVIDIA/Apple/AMD/高通調漲晶圓報價 5-10%（3nm 擴及 5/7nm）、三星跟進調漲 4/5nm 約 15%；CoWoS 2026 產能 12.5 萬片/月（+20-30%）已被 NVIDIA/Broadcom 預訂一空、缺口延續至 2027 年底",
        "📊 台美 ADR 溢價收斂至 +10.5%（$419.48 vs 理論值 $379.47、匯率 32.15）",
        "🚀 分析師法說會前密集上調：美銀 $490→$590（7/14）、Barclays $625、Citi NT$3,800（自 NT$2,440 具 +55.7% 空間）、Nomura NT$3,425、共識強力買入（平均目標 $490.34、+16.9%）",
        "⚠️ 風險續追蹤：法說會後恐劇烈波動（超預期已部分反映、不及預期修正風險）；外資連 5 賣；AI 記憶體鏈急回吐擴散效應；美 232 條款關稅 25% + 輸中晶片轉運查驗收緊；Intel 18A 量產競爭；估值不低（台股 P/E ~32.3x、TSM ~35.6x）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-16 Thu（最新確認收盤：台股 2330 7/15 Wed 官方收 NT$2,440（+20、+0.83% vs 7/14 NT$2,420、開 2,425／高 2,460／低 2,415、量 33,666 張 TWSE 口徑、成交額 NT$822.14 億、均價 2,442、TWSE 確認）；NYSE TSM 7/15 Wed 收 $419.48（-$0.91、-0.22% vs 7/14 $420.39、日內 $410.78-428.88））。⭐⭐ 今日最重要：Q2 2026 財報法說會今日（7/16 Thu）14:00 登場——公布 Q2 財報 + Q3 展望；Q2 初步合併營收約 NT$1.27 兆（~$39.2B、季度新高）已確認達成美元指引 $39.0-40.2B；共識估營收 ~$40B（+33% YoY）、每 ADR EPS $3.81-3.87（去年同期 $2.47、+55%+）、可望連續第 5 季創獲利新高；法人估 Q2 毛利率挑戰 70%（指引 65.5-67.5%）；關注是否上修全年財測（現行 >30%）、2027-28 資本支出（Citi 估 $75-80B）、CoWoS/2nm 擴產進度；因台股 13:30 收盤，法說會效應主要於 7/17 台股與今晚美股反映、恐劇烈波動。⭐ 今日重點：（1）📈 台股 7/15 收 NT$2,440（+0.83%）——加權指數大漲 +893.64 點（+2.00%）收 45,631.59，2330 漲幅明顯落後大盤屬法說會前觀望、惟重新站回 5MA ~2,436 / 20MA ~2,430、量能收斂至 33,666 張；（2）📊 NYSE TSM 7/15 收 $419.48（-0.22%）法說會前連 4 日小跌、盤中殺至 $410.78 尾盤收復；⚠️ 同日美股半導體回檔：SOX -2.08% 收 12,398.89、MU -8.02% 收 $904.28（HBM 暴漲後獲利了結）、AMAT -2.73%、INTC -4.43%、AMD -3.46%——TSM 相對抗跌；記憶體重挫延續至 7/16 韓股盤中（SK 海力士 -10%、三星 -7.7%）；⭐ ASML 第二度上修 2026 展望（+2.23% 收 $1,815.27）、AAPL +4.01%、NVDA +0.33%；（3）📊 籌碼收斂：三大法人 7/15 合計賣超 3,568 張——外資賣超 3,427 張（連 5 賣、5 日累計 -34,792 張、惟較 7/14 -12,416 大幅收斂）；投信賣超 596 張（終止連買）、自營買超 456 張；（4）📊 台美 ADR 溢價收斂至 +10.5%（$419.48 vs 理論值 $379.47、匯率 32.15）；（5）💰 漲價題材：調漲晶圓報價 5-10%（3nm 擴及 5/7nm）、三星跟進漲 4/5nm 約 15%；CoWoS 2026 產能 12.5 萬片/月已被 NVIDIA/Broadcom 預訂一空；（6）🚀 分析師法說會前密集上調：美銀 $490→$590（7/14）、Barclays $625（+49.0%）、Citi NT$3,800（+55.7% 空間）、Nomura NT$3,425（+40.4%）、共識強力買入（平均目標 $490.34、+16.9%）。技術面（TWSE 官方收盤計算）：5MA ~2,436 / 20MA ~2,430 / 60MA ~2,312；7/15 收 2,440 重新站回 5MA/20MA、RSI 54.6 回中軸上、MACD 柱仍負（-7.0）、KDJ J 值 26 自超賣回升；支撐 2,415（7/15 低）／2,390／2,342（布林下軌）、壓力 2,460（7/15 高）／2,480／2,535 史高。⚠️ 風險：法說會後劇烈波動、外資連 5 賣、AI 記憶體鏈急回吐、美 232 關稅與輸中晶片轉運查驗收緊、Intel 18A 量產競爭、估值不低（台股 P/E ~32.3x TTM、TSM ~35.6x）、台幣 ~32.15（每升值 1% 毛利率 -40bps）。⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約、漲價 5-10% 定價權、4 大 CSP 2026 AI CapEx $725B。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/16 Thu）關鍵觀察：（1）🎯🎯 Q2 2026 財報法說會今日 14:00 登場——共識估 Q2 營收 ~$40B（+33% YoY）、每 ADR EPS $3.81-3.87、毛利率挑戰 70%（指引 65.5-67.5%）、獲利連 5 季創高；重點在 Q3 展望與全年財測是否上修（現行 >30%）、2027-28 資本支出；因台股 13:30 收盤、法說會效應主要 7/17 台股反映，今晚美股 TSM 首先定價——超預期已部分反映在目標價上調、不及預期恐引發修正；（2）⚠️ AI 記憶體鏈急回吐：MU 7/15 -8.02%、SK 海力士 7/16 盤中 -10%、三星 -7.7%——留意獲利了結情緒是否擴散至代工/AI 晶片鏈；（3）📊 籌碼：外資連 5 賣惟大幅收斂（7/15 -3,427 vs 7/14 -12,416）——法說會後能否翻買為關鍵；投信轉賣 -596 觀望；（4）技術面：7/15 站回 5MA ~2,436 / 20MA ~2,430、MACD 柱仍負（-7.0）、KDJ J 26 自超賣回升，守穩 2,415／2,390 支撐、突破 2,460／2,480 看法說會成色；（5）💱 台幣 ~32.15、美 232 條款關稅 + 輸中晶片轉運查驗收緊（SEMI 致函反對）、估值不低（台股 P/E ~32.3x、TSM ~35.6x）續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-15",       # 最新資料日（7/15 Wed、TWSE T86 官方）
    "foreign":  -3427,             # 外資 7/15 賣超 3,427 張（連 5 賣、5 日累計 -34,792 張、惟較 7/14 -12,416 大幅收斂）
    "trust":    -596,              # 投信 7/15 賣超 596 張（終止連買轉賣、法說會前觀望）
    "dealer":   456,               # 自營商 7/15 買超 456 張（自行 +547、避險 -91）；三大法人合計 -3,568 張
    # 近5日（由舊到新：7/9, [7/10 颱風休市], 7/13, 7/14, 7/15）；TWSE T86 口徑（張）
    "foreign_5d": [-12749, 0, -2045, -12416, -3427],    # 外資（7/9, [7/10 颱風休市], 7/13, 7/14, 7/15）：連 5 賣惟 7/15 大幅收斂
    "trust_5d":   [43, 0, 530, 1841, -596],             # 投信（7/10 休市計 0；7/15 終止連買轉賣）
    "dealer_5d":  [90, 0, 623, 2570, 456],              # 自營（7/10 休市計 0；7/15 買超縮小）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       54.6,   "rsi_signal": "RSI(14) 54.6（TWSE 官方收盤計算）——7/15 +0.83% 後回中軸 50 之上、中性偏多、待今日 14:00 法說會定方向",
    "macd":         -7.0,  "macd_signal": "MACD 柱狀仍負 -7.0（DIF 30.6 < DEA 37.6、TWSE 官方收盤計算）——短線動能仍弱、法說會前觀望；7/15 站回 5MA/20MA 為初步止穩訊號",   "macd_color": "#F44336",
    "kdj_k":        39.0,   "kdj_d": 46.0, "kdj_j": 26.0, "kdj_signal": "KDJ K(39)/D(46)/J(26)（官方收盤計算）——K 仍低於 D、J 值 26 自超賣區回升，短線偏弱惟修復中、法說會成色決定能否金叉",
    "ma5":       "2,436",   # 5MA（TWSE 官方收盤計算，含 7/15 NT$2,440）
    "ma20":      "2,430",   # 20MA（TWSE 官方收盤計算）
    "ma60":      "2,312",  # 60MA（TWSE 官方收盤計算）
    "ma120":     "2,083",  # 120MA（TWSE 官方收盤計算）
    "bb_upper":  "2,517",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,430",   # 布林中軌（20MA）
    "bb_lower":  "2,342",  # 布林下軌（20MA-2σ）
    "vol_ma5":  "34,406",   # 5日均量（張、TWSE 口徑、含 7/15 官方量 33,666）
    "trend":    "📈 法說會前站回 5MA/20MA、惟漲幅明顯落後大盤：台股 2330 7/15 官方收 NT$2,440（+20、+0.83%、量 33,666 張 TWSE 口徑）——加權指數同日大漲 +893.64 點（+2.00%）收 45,631.59，2330 漲幅不到大盤一半屬法說會前觀望；收盤重新站回 5MA（~2,436）與 20MA（~2,430）、量能收斂屬觀望性換手。⚠️ 隔夜（7/15 美股）半導體回檔：SOX -2.08% 收 12,398.89、MU -8.02% 收 $904.28（HBM 暴漲後獲利了結）、AMAT -2.73%、LRCX -3.08%、INTC -4.43%——NYSE TSM 收 $419.48（-0.22%）相對抗跌；記憶體重挫延續至 7/16 韓股盤中（SK 海力士 -10%、三星 -7.7%）；⭐ ASML 第二度上修 2026 展望（+2.23%）、AAPL +4.01%、NVDA +0.33% 撐盤。📊 技術指標（TWSE 官方收盤計算）：RSI(14) 54.6 回中軸上、MACD 柱仍負 -7.0（DIF 30.6 < DEA 37.6）、KDJ K(39)/D(46)/J(26) J 值自超賣回升——短線動能修復中、待法說會定方向。下一觀察：（1）🎯🎯 Q2 法說會今日 14:00 登場——共識估營收 ~$40B（+33% YoY）、EPS $3.81-3.87/ADR、毛利率挑戰 70%；因台股 13:30 收盤、效應主要今晚美股與 7/17 台股反映、恐劇烈波動；（2）守穩支撐 2,415（7/15 低）／2,390／布林下軌 2,342；（3）突破 2,460（7/15 高）／2,480 看法說會成色、再挑戰 2,535 史高；（4）外資連 5 賣惟大幅收斂（7/15 -3,427）、法說會後能否翻買；（5）🚀 美銀 $590 + Barclays $625 + Citi NT$3,800 + 漲價定價權為中長線支撐。",
    "support":  "2,415 / 2,390 / 2,342 (7/15 低、7/14 低、布林下軌)",
    "resist":   "2,460 / 2,480 / 2,535 (7/15 高、前高、史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依 TWSE 官方收盤價計算（2026-07-15 止），請以看盤軟體為準；7/15 官方收 NT$2,440（+0.83%）重新站回 5MA ~2,436 / 20MA ~2,430、RSI 54.6 回中軸上、MACD 柱仍負 -7.0、KDJ J 26 自超賣回升；下方支撐 2,415 / 2,390 / 2,342、上方壓力 2,460 / 2,480 / 2,535 史高；🎯🎯 Q2 法說會今日 14:00 登場為最大變數（效應主要 7/17 台股反映）；🚀 分析師密集上調（美銀 $590、Barclays $625、Citi NT$3,800）+ 漲價定價權 vs ⚠️ 外資連 5 賣 + AI 記憶體鏈急回吐 + 法說會後劇烈波動風險",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-15",       # 最新資料日（7/15 Wed 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,435",            # 台積電期貨近月（估、現貨 7/15 收 NT$2,440 附近小幅逆價差、法說會前保守）
    "futures_oi":    "184,000",          # 未平倉量（口）（估）
    "futures_basis": "-5",               # 估小幅逆價差（法說會前避險偏保守）
    "call_oi":       "120,000",
    "put_oi":        "84,000",
    "pcr":           "0.70",             # Put/Call 比率（估、法說會當日避險需求升溫、中性偏保守）
    "pcr_signal":    "中性（Put/Call 估 0.70；台股 7/15 官方收 NT$2,440（+0.83%）站回 5MA/20MA、Q2 法說會今日 14:00 登場避險需求升溫；⭐ 美銀 $590/Barclays $625/Citi NT$3,800 + 共識偏多為中長線支撐；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,440",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,342", "Call 55,000 / Put 15,000", "布林下軌、下方支撐"),
        ("2,390", "Call 50,000 / Put 11,000", "7/14 盤中低、支撐"),
        ("2,415", "Call 48,000 / Put 9,000",  "7/15 盤中低、支撐"),
        ("2,440", "Call 46,000 / Put 8,000",  "7/15 官方收、最大痛苦（估）"),
        ("2,460", "Call 42,000 / Put 6,000",  "7/15 盤中高、短線壓力"),
        ("2,480", "Call 38,000 / Put 5,000",  "前高壓力帶"),
        ("2,500", "Call 32,000 / Put 3,500",  "整數壓力"),
        ("2,535", "Call 24,000 / Put 2,500",  "盤中史高"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/15 NT$2,440）；NT$2,440 為估最大痛苦、下方支撐 2,415 / 2,390 / 2,342（布林下軌）、上方壓力 2,460 / 2,480 / 2,535 史高；🎯🎯 Q2 法說會今日（7/16）14:00 登場為最大變數、法說會後恐劇烈波動（效應主要今晚美股與 7/17 台股反映）；🚀 分析師密集上調（美銀 $590、Barclays $625、Citi NT$3,800）+ 漲價 5-10%（三星跟進 15%）定價權 + CoWoS 訂滿至 2027 偏多 vs ⚠️ 外資連 5 賣（惟收斂）+ AI 記憶體鏈急回吐；中長線基底未變（NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-15 Wed 官方收盤；美股為 7/15 Wed 收盤（Yahoo Finance API）；韓股為 7/16 Thu 盤中）
    ("設備", "ASML",    "艾司摩爾",    "1,815.27", "+2.23%", "⭐ EUV 設備龍頭；7/15 第二度上修 2026 全年展望（AI 資本支出強勁）、半導體回檔中逆勢收漲"),
    ("設備", "AMAT",    "應用材料",    "579.43", "-2.73%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（7/15 隨 SOX 回檔）"),
    ("設備", "LRCX",    "科林研發",    "335.43", "-3.08%", "蝕刻設備（7/15 回檔）；WFE 全年展望上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "224.50", "-2.55%", "量測設備（7/15 回檔）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","180.50","+9.73%", "🚀 與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/15 台股收盤大漲、跟進 AI 記憶體行情）"),
    ("記憶", "000660.KS","SK 海力士",  "1,872,000 KRW", "-10.09%", "⚠️ 7/16 盤中暴跌逾 -10%（前兩日暴漲 + MU -8% 後 AI 記憶體急回吐）；HBM4、與 NVIDIA 多年期合作"),
    ("記憶", "MU",      "美光",        "904.28", "-8.02%", "⚠️ HBM / DDR5（7/15 收大跌 -8.02%）；AI 記憶體暴漲後獲利了結、HBM4 中長線邏輯未變"),
    ("封裝", "3711.TW", "日月光",      "683.00", "+6.55%", "CoWoS / 先進封裝（7/15 台股收盤大漲、隨大盤 +2% 反彈領漲）"),
    ("封裝", "AMKR",    "Amkor",       "67.64",  "-3.65%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/15 隨 SOX 回檔）"),
    ("客戶", "NVDA",    "輝達",        "212.50", "+0.33%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/15 收 $212.50 於 SOX -2.08% 中逆勢收漲"),
    ("客戶", "AAPL",    "蘋果",        "327.50", "+4.01%", "#2 客戶 18%；7/15 收大漲 +4.01%；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "394.28", "+1.33%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/15 逆勢收漲）"),
    ("客戶", "AMD",     "超微",        "529.14", "-3.46%", "#4 客戶；MI350/MI450、首發 2nm AI 晶片（7/15 隨 SOX 回檔）"),
    ("客戶", "2454.TW", "聯發科",      "3,740",  "+2.19%", "手機 SoC 客戶 9%；面臨漲價成本（7/15 台股官方收盤、隨大盤反彈）"),
    ("競爭", "INTC",    "英特爾",      "102.99", "-4.43%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/15 收大跌、回吐前日反彈）"),
    ("競爭", "005930.KS","三星電子",   "258,000 KRW", "-7.69%", "⚠️ 7/16 盤中大跌（記憶體鏈急回吐）；跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣"),
    ("產業", "SOX",     "費城半導體指數","12,398.89", "-2.08%", "7/15 回檔（MU -8% 拖累）；今日 TSMC 法說會打頭陣、半導體財報季開跑"),
    ("處分後", "ARM",   "Arm Holdings","277.01", "-1.48%", "TSMC 4/29 全數出脫後（7/15 收小跌）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/15 收 $419.48 具 +16.9% 上行空間；美銀 7/14 上調至 $590、Citi 台股目標 NT$3,800、Barclays $625、Nomura NT$3,425；Goldman 7/1 移出 APAC Conviction List；Morningstar 17 位分析師強力買入）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+16.90%",  # 從 NYSE 7/15 收 $419.48 至平均目標 $490.34（共識強力買入；今日 7/16 法說會後重估）
}

# 財報倒計時
EARNINGS = {
    "next_date":   "2026-07-16",
    "quarter":     "Q2 2026（法說會今日 2026-07-16 Thu 14:00 登場）",
...
NEWS = [
    {
        ...
        "title": "🎯🎯 Q2 法說會今日 14:00 登場：共識估營收 ~$40B（+33% YoY）、EPS $3.81-3.87/ADR、毛利率挑戰 70%",
        "body": "半導體財報季由 TSMC 打頭陣、市場視為 AI 熱潮健康檢查：（1）📅 TSMC 今日（7/16 Thu 14:00）舉行 Q2 2026 財報法說會；Q2 初步合併營收約 NT$1.27 兆（~$39.2B、季度新高）已確認達成指引 $39.0-40.2B——6 月營收 NT$4,426.80 億（+67.9% YoY、單月新高）；（2）📊 華爾街共識：營收 ~$40B（+33% YoY）、每 ADR EPS $3.81-3.87（去年同期 $2.47）、可望連續第 5 季創獲利新高；法人聚焦毛利率能否挑戰 70%（指引 65.5-67.5%）；（3）🎯 展望焦點：是否上修全年財測（現行營收成長 >30%）、2027-28 資本支出（Citi 估 $75-80B）、CoWoS/2nm 擴產；（4）⚠️ 台股 13:30 收盤，法說會效應主要今晚美股與 7/17 台股反映、恐劇烈波動。實際數字以法說會後官方公告為準。",
        "source": "CNBC / SEC 6-K / Investing Engineer 2026-07-13 - 07-15", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📈 台股 7/15 大漲 894 點、2330 收 NT$2,440（+0.83%）明顯落後；美股半導體回檔 MU -8%、韓記憶體暴跌",
        "body": "法說會前觀望、AI 記憶體鏈急回吐：（1）📈 台股加權指數 7/15 大漲 +893.64 點（+2.00%）收 45,631.59；2330 官方收 NT$2,440（+20、+0.83%、量 33,666 張 TWSE 口徑、額 NT$822.14 億）漲幅不到大盤一半、屬法說會前觀望，惟站回 5MA/20MA；（2）⚠️ 隔夜（7/15 美股）半導體回檔：SOX -2.08% 收 12,398.89、MU -8.02%（HBM 暴漲後獲利了結）、AMAT -2.73%、INTC -4.43%、AMD -3.46%；NYSE TSM 收 $419.48（-0.22%）相對抗跌；（3）⚠️ 記憶體重挫延續至 7/16 韓股盤中：SK 海力士 -10%、三星 -7.7%——前兩日暴漲後急回吐；（4）⭐ 台股供應鏈逆勢強：華邦電 +9.73%、日月光 +6.55%、聯發科 +2.19%。",
        "source": "TWSE / Yahoo Finance API 2026-07-15 - 07-16", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🚀 法說會前分析師密集上調：美銀 $490→$590、Barclays $625、Citi NT$3,800；ASML 二度上修 2026 展望",
        "body": "AI 資本支出循環獲設備與券商雙重背書：（1）🚀 美銀證券 7/14 將 TSM 目標價自 $490 上調至 $590（自 7/15 收 $419.48 具 +40.7% 空間），主因 AI 晶片需求與漲價效應；Barclays $625、Citi NT$3,800、Nomura NT$3,425 續居前列、共識強力買入；（2）⭐ ASML 7/15 公布財報並第二度上修 2026 全年展望——印證 AI 相關晶片資本支出強勁，惟法說後股價一度回落、顯示市場對估值轉趨謹慎；（3）📊 分析師平均目標價 $490.34（自 $419.48 具 +16.9% 上行）；（4）🎯 目標價密集上調墊高市場對今日法說會期望值——「超預期」門檻同步提高。",
        "source": "TipRanks / CNBC 2026-07-14 - 07-15", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "💰 台積電通知 NVIDIA/Apple/AMD/高通調漲晶圓報價 5-10%：3nm 擴及 5/7nm、三星跟進漲 15%",
        "body": "代工定價權全面確認：（1）💰 台積電通知主要客戶調漲晶圓報價 5-10%，範圍自最先進 3nm 擴大至 5nm、7nm 製程——反映 AI 晶片需求強勁、先進產能供不應求；（2）🔁 三星電子隨後跟進調漲 4/5nm 代工報價約 15%，行業定價權全面轉向代工廠；（3）📊 法人估漲價對 TSMC 毛利率貢獻約 +2pp、為 2027 獲利上修主要動能；（4）承擔成本方為 NVIDIA（22% 營收占比）、Apple、AMD、高通等大客戶，需求剛性下轉嫁能力強。",
        "source": "TechNews 科技新報 / 經濟日報 / Yahoo 股市 2026-07-09 - 07-10", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "⚠️ 美國 232 條款半導體關稅 25% 持續執行：輸中晶片須經美第三方實驗室查驗、SEMI 致函反對",
        "body": "地緣政治仍為 AI 供應鏈最大尾部風險：（1）⚠️ 美國對進口半導體及 AI 晶片衍生品課徵 25% 關稅（2026-01-15 生效）持續執行；TSMC 因 $165B 美國投資承諾適用豁免框架、直接衝擊有限；（2）🚧 美方近期進一步緊縮轉運規定：凡運往中國的晶片須先進入美國、由第三方實驗室測試後放行——供應鏈時效與成本承壓；（3）📮 SEMI 已致函美國財政部要求重新考慮相關管制措施；（4）留意北京反制與客戶端（中國市場）需求外溢效應。",
        "source": "行政院經貿談判辦公室 / 中央社 / SEMI 2026-01 生效、2026-07 執行更新", "impact": "負面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 外資連 5 賣惟大幅收斂（7/15 -3,427 張）：投信終止連買、法說會前全面觀望",
        "body": "籌碼觀望但賣壓明顯降溫：（1）📊 三大法人 7/15 合計賣超 3,568 張（T86 官方）：外資賣超 3,427 張——連 5 賣、5 日累計 -34,792 張，惟較 7/14（-12,416）大幅收斂逾七成；（2）⚠️ 投信賣超 596 張（終止連買轉賣）、自營買超 456 張（自行 +547、避險 -91）——法人全面轉觀望等待法說會；（3）📈 技術面初步修復：7/15 收 2,440 站回 5MA ~2,436 / 20MA ~2,430、KDJ J 值 26 自超賣回升，惟 MACD 柱仍負（-7.0）；（4）🎯 觀察：法說會成色決定外資能否翻買；估值不低（台股 P/E ~32.3x、TSM ~35.6x）下、財報與 Q3 展望為關鍵。",
        "source": "TWSE T86 2026-07-15", "impact": "負面",
    },
]

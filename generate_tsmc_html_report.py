...
STOCK = {
    # 台股 2330
    "tw_price":       "2,320",        # 台股 2330 最新確認收盤：7/20 Mon 官方收 NT$2,320（+30、+1.31% vs 7/17 NT$2,290、開 2,300／高 2,345／低 2,300、TWSE 確認）——7/17 史詩級崩跌後量縮技術性反彈；⭐ 大盤同日續跌 -0.52%、2330 逆勢撐盤（貢獻大盤約 244 點）
    "tw_change":      "+30",          # 7/20 Mon 收盤漲跌點數（+1.31%、TWSE 官方）
    "tw_change_pct":  "+1.31%",       # 7/20 Mon 收盤漲跌幅（崩跌後技術性反彈、三大法人轉買超）
    "tw_volume":      "55,790",       # 7/20 成交量（張、TWSE 口徑；成交股數 55,790,346 股、成交額 NT$1,298.16 億、成交 188,463 筆、量縮至 7/17 爆量 97,362 張的 57%——反彈量縮、追價意願待確認）
    "tw_open":        "2,300",        # 7/20 開盤（TWSE、開盤即當日最低）
    "tw_high":        "2,345",        # 7/20 盤中高（TWSE）
    "tw_low":         "2,300",        # 7/20 盤中低（TWSE）
    "market_cap":     "NT$60.2 兆",   # 7/20 收盤市值（NT$2,320 基礎、約 259.3 億股）
    "pe_ratio":       "~26.5x",       # 台股 P/E（TTM 估，NT$2,320 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25）
    "pb_ratio":       "~17.4x",       # 台股 P/B（NT$2,320 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.03%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,320）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "402.30",       # 7/20 Mon 官方收盤 $402.30（+$3.93、+0.99% vs 7/17 $398.37、Yahoo Finance API 確認）——收復 $400 關卡；SOX 同日 +0.60% 收 11,743.85 止跌（自 6/22 史高 -19.8%、仍處熊市邊緣）
    "nyse_change":    "+3.93",        # 7/20 漲跌（自 7/17 $398.37）
    "nyse_change_pct":"+0.99%",       # 7/20 收復 $400、隨半導體板塊止跌反彈（惟設備股續跌）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~29.5x",       # NYSE USD 基礎 P/E（TTM 估，$402.30 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~16.8x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-21 Tue（最新確認收盤：台股 2330 7/20 Mon 官方收 NT$2,320（+30、+1.31% vs 7/17 NT$2,290、開 2,300／高 2,345／低 2,300、量 55,790 張 TWSE 口徑、成交額 NT$1,298.16 億、成交 188,463 筆、TWSE 確認）；NYSE TSM 7/20 Mon 收 $402.30（+$3.93、+0.99% vs 7/17 $398.37、Yahoo Finance API 確認）——收復 $400）。✅ 今日最重要：7/17 史詩級崩跌後首日止穩——台股 2330 7/20 量縮反彈 +1.31%（量縮至前日 57%）、⭐ 大盤續跌 -221.57 點（-0.52%）收 42,449.70 下逆勢撐盤（盤中震盪逾 1,100 點、融資減碼賣壓、聯電連 2 日跌停、日月光 -2.77%——資金集中防禦台積電）；📊 籌碼大幅改善：三大法人 7/20 對 2330 由史詩級賣超轉為買超 +4,002 張（T86 官方）——外資賣超收斂 96% 至 -1,672 張（連 8 賣、惟自 7/17 -44,184 大幅收斂）、投信 +1,049（連 3 買）、自營 +4,626（連 8 買）；全市場三大法人轉買超 21.7 億（BFI82U 官方、vs 7/17 賣超 2,631.5 億史上最大）。⚠️ 惟 AI 情緒面尚未確認止穩：SOX 7/20 僅小漲 +0.60% 收 11,743.85（自 6/22 史高仍 -19.8%、熊市邊緣）、設備股續跌（LRCX -2.09%、KLAC -2.42%）、AAPL -2.14%；Yardeni 警告半導體 ETF SOXX 技術面熊市、恐再下探 12%；韓股補跌（SK 海力士 7/20 -4.2%、三星 -4.3%、7/17 韓股休市）。📊 台美 ADR 溢價維持 +12.0%（$402.30 vs 理論值 ~$359、匯率 ~32.3）——台美同步反彈、溢價未收斂。🚀 法說會後外資密集調升 TSM 目標價：Susquehanna $600、Barclays $650、BofA $590、TD Cowen $440（TipRanks/GuruFocus 7/17-20）——S&P Global 統計 19 位分析師平均 $520.37、共識強力買進。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B（Arizona 追加 $100B）、股利 +33%、「AI 需求比預期更強」。技術面（官方收盤計算）：7/20 收 2,320 站回布林下軌 2,307、惟仍壓在 60MA 2,328 之下；RSI(14) 43.9 自 41.0 回升、MACD 綠柱慣性擴大至 -19.2（DIF 8.5 < DEA 27.7）、KDJ K28.6/D40.3/J5.0——J 值深度超賣、留意低檔背離修復；支撐 2,307（布林下軌）／2,300（7/20 低）／2,290（7/17 低）／2,200（整數關）、壓力 2,328（60MA）／2,388（5MA）／2,424（20MA）。⚠️ 風險：AI 估值/情緒修正未完（SOXX 熊市、大盤融資賣壓）、外資連 8 賣能否轉買、美 232 條款半導體關稅 25% + 輸中晶片轉運查驗；台幣 ~32.3。⭐ 中長線基底未變：全年成長 >40%、CapEx $60-64B、2nm 已貢獻 3% 且 5 座廠爬坡、HPC 占 66%、NVIDIA #1 客戶/A16 首發、CoWoS 訂滿至 2027、先進製程漲價 5-10%——崩跌後止穩首日、反彈續航力待站回 60MA 確認。📈 7/21 早盤更新（09:13 盤中、未收盤）：2330 連 2 日走強 報 NT$2,365（+45、+1.94%、開 2,350／高 2,370／低 2,345、量 5,115 張）——盤中站上 60MA 2,328（若守至收盤即確認反彈續航第一道訊號、下一挑戰 5MA 2,388）；⚠️ 13:30 收盤前僅供參考、正式收盤以 TWSE 為準",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性",
    # 一句話標題
    "headline":   "✅ 崩跌後首日止穩：台股 2330 7/20 量縮反彈 +1.31% 收 NT$2,320——大盤續跌 -0.52% 下逆勢撐盤；三大法人由史詩級賣超轉買超 +4,002 張（外資賣超收斂 96%）；NYSE TSM 收復 $400（+0.99%）、SOX +0.60% 止跌；法說會後外資密集調升目標價（Susquehanna $600、Barclays $650）——惟 AI 情緒面未確認止穩（SOXX 熊市、設備股續跌）、反彈續航看能否站回 60MA ~2,328",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "✅ 最重要：7/17 史詩級崩跌後首日止穩——台股 2330 7/20 官方收 NT$2,320（+30、+1.31%、開 2,300／高 2,345／低 2,300）、量縮至 55,790 張（前日 57%）；⭐ 大盤續跌 -0.52% 收 42,449.70（盤中震盪逾 1,100 點、聯電連 2 跌停、日月光 -2.77%）、2330 逆勢撐盤貢獻約 244 點——資金集中防禦龍頭",
        "📊 籌碼大幅改善：三大法人 7/20 對 2330 轉買超 +4,002 張（T86 官方）——外資 -1,672 張（連 8 賣、惟自 7/17 -44,184 收斂 96%）、投信 +1,049（連 3 買）、自營 +4,626（連 8 買）；全市場三大法人轉買超 21.7 億（vs 7/17 賣超 2,631.5 億史上最大）——史詩級賣壓暫歇",
        "✅ NYSE TSM 7/20 收 $402.30（+0.99%）收復 $400；SOX +0.60% 收 11,743.85 止跌（自 6/22 史高 -19.8%）——惟設備股續跌（LRCX -2.09%、KLAC -2.42%）、AAPL -2.14%，板塊分歧、情緒未全面修復",
        "🚀 法說會後外資密集調升 TSM 目標價：Susquehanna $600、Barclays $650、BofA $590、TD Cowen $440——S&P Global 統計 19 位平均 $520.37、共識強力買進（TipRanks/GuruFocus 7/17-20）",
        "⚠️ AI 情緒面未確認止穩：Yardeni 警告 SOXX 技術面熊市、恐再下探 12%；韓股補跌（SK 海力士 7/20 -4.2%、三星 -4.3%）、7/21 早盤回穩（SK +0.6%、三星 +2.3% 盤中）；台股融資減碼賣壓續清洗",
        "📊 台美 ADR 溢價維持 +12.0%（$402.30 vs 理論值 ~$359、匯率 ~32.3）——台美同步反彈、溢價未收斂、台股相對偏貴狀態延續",
        "💡 技術面：7/20 收 2,320 站回布林下軌 2,307、惟仍壓在 60MA 2,328 之下；RSI 43.9 回升、KDJ J 值 5.0 深度超賣；支撐 2,300-2,307／2,290／2,200、壓力 2,328（60MA）／2,388（5MA）／2,424（20MA）——站回 60MA 為反彈續航確認訊號",
        "✅ 基本面錨未變：Q2 大超預期（EPS +77.4%、連 11 季超預期）、全年上修 >40%、CapEx $60-64B、股利 +33%、「AI 需求比預期更強」——估值消化後台股 P/E ~26.5x、TSM ~29.5x",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-21 Tue（最新確認收盤：台股 2330 7/20 Mon 官方收 NT$2,320（+30、+1.31% vs 7/17 NT$2,290、開 2,300／高 2,345／低 2,300、量 55,790 張 TWSE 口徑、成交額 NT$1,298.16 億、成交 188,463 筆、TWSE 確認）；NYSE TSM 7/20 收 $402.30（+$3.93、+0.99% vs 7/17 $398.37、Yahoo API 確認））。✅ 核心判讀：7/17 史詩級崩跌後首日止穩——（1）台股 2330 量縮反彈 +1.31%、大盤續跌 -0.52% 下逆勢撐盤（貢獻約 244 點）、資金集中防禦龍頭；（2）籌碼面大幅改善：三大法人對 2330 轉買超 +4,002 張——外資賣超自 -44,184 收斂 96% 至 -1,672（連 8 賣）、投信 +1,049（連 3 買）、自營 +4,626（連 8 買）；全市場三大法人轉買超 21.7 億（vs 7/17 賣超 2,631.5 億史上最大）；（3）NYSE TSM 收復 $400、SOX +0.60% 止跌。⚠️ 惟反彈屬「止穩」而非「反轉」確認：量縮反彈（前日 57%）追價意願保守、SOX 自史高仍 -19.8% 處熊市邊緣、設備股與 AAPL 續跌、Yardeni 警告 SOXX 恐再下探 12%、台股融資減碼賣壓未清；技術面 2,320 仍壓在 60MA 2,328 之下、MACD 綠柱慣性擴大至 -19.2——站回 60MA ~2,328 / 5MA ~2,388 為反彈續航兩道確認訊號、KDJ J 值 5.0 深度超賣支持技術性修復。🚀 法說會後外資密集調升：Susquehanna $600、Barclays $650、BofA $590——19 位分析師平均 $520.37、共識強力買進。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B（Arizona 追加 $100B）、股利 +33%、2nm 貢獻 3%、「AI 需求比預期更強」——本波為估值/情緒修正、崩跌後中長線價值浮現。⭐ 今日（7/21）觀察：（1）反彈續航：能否站回 60MA 2,328 進而挑戰 5MA 2,388；（2）籌碼：外資連 8 賣能否正式轉買；（3）AI 情緒：SOX 能否連續收紅、韓股回穩延續（7/21 早盤 SK +0.6%、三星 +2.3%）；（4）守住支撐 2,300-2,307／2,290（7/17 低、雙底防線）。⚠️ 風險：AI 估值修正未完、美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.3（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/21 Tue）關鍵觀察：（1）✅ 反彈續航確認：7/20 量縮反彈 +1.31% 收 2,320 僅屬止穩、尚非反轉——能否站回 60MA ~2,328 / 5MA ~2,388 為兩道確認訊號；失守 2,300-2,307（7/20 低/布林下軌）則回測 2,290（7/17 低）、雙底防線破位下看 2,200／2,099（120MA）；（2）📊 籌碼：外資賣超已收斂 96%（-44,184 → -1,672）、惟連 8 賣未止——正式轉買為多方最強訊號；投信連 3 買、自營連 8 買續扮穩定力量；（3）⚠️ AI 情緒面：SOX 僅 +0.60% 弱反彈、自史高仍 -19.8%，Yardeni 警告 SOXX 恐再跌 12%、設備股（LRCX/KLAC）與 AAPL 續跌——板塊分歧顯示情緒未全面修復、提防二次探底；（4）✅ 基本面錨：Q2 大超預期＋全年上修 >40%＋外資密集調升目標價（平均 $520.37）完全未變——股價與基本面脫鉤提供中長線布局視角；（5）💱 美 232 條款關稅 25% + 輸中晶片轉運查驗、台幣 ~32.3、台股融資減碼賣壓續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-20",       # 最新資料日（7/20 Mon、TWSE T86 官方）
    "foreign":  -1672,             # 外資 7/20 賣超 1,672 張（連 8 賣、惟自 7/17 史詩級 -44,184 大幅收斂 96%；買 34,870／賣 36,542）
    "trust":    1049,              # 投信 7/20 買超 1,049 張（連 3 買、持續承接）
    "dealer":   4626,              # 自營商 7/20 買超 4,626 張（連 8 買、大幅加碼；自行 -9／避險 +4,635）；三大法人合計 +4,002 張（崩跌後轉買超）
    # 近5日（由舊到新：7/14, 7/15, 7/16, 7/17, 7/20）；TWSE T86 口徑（張）
    "foreign_5d": [-12416, -3427, -4215, -44184, -1672],  # 外資（連 8 賣、7/17 史詩級賣超後 7/20 收斂 96%）
    "trust_5d":   [1841, -596, 528, 1489, 1049],          # 投信（連 3 買、持續逆勢承接）
    "dealer_5d":  [2570, 456, 1061, 2759, 4626],          # 自營（連 8 買、7/20 買超放大）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       43.9,   "rsi_signal": "RSI(14) 43.9（官方收盤計算）——7/20 反彈後自 41.0 回升、仍在中軸下；低檔回升初步、需突破 50 確認動能轉強",
    "macd":         -19.2, "macd_signal": "MACD 綠柱慣性擴大至 -19.2（DIF 8.5 < DEA 27.7、官方收盤計算）——7/17 巨量長黑後空方動能未消、綠柱由 -15.2 續擴；惟屬崩跌後慣性、觀察反彈能否使綠柱收斂",   "macd_color": "#F44336",
    "kdj_k":        28.6,   "kdj_d": 40.3, "kdj_j": 5.0, "kdj_signal": "KDJ K(28.6)/D(40.3)/J(5.0)（官方收盤計算）——J 值 5.0 深度超賣、K 仍在 D 下死叉未解；深超賣支持技術性修復、金叉需 K 上穿 D 確認",
    "ma5":       "2,388",   # 5MA（官方收盤計算，7/14-7/20：2420/2440/2470/2290/2320）
    "ma20":      "2,424",   # 20MA（官方收盤計算）
    "ma60":      "2,328",  # 60MA（官方收盤計算；7/20 收 2,320 仍壓在其下、為反彈第一道關卡）
    "ma120":     "2,099",  # 120MA（官方收盤計算）
    "bb_upper":  "2,540",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,424",   # 布林中軌（20MA）
    "bb_lower":  "2,307",  # 布林下軌（20MA-2σ、官方收盤計算；7/20 收 2,320 站回下軌之上）
    "vol_ma5":  "52,042",   # 5日均量（張、TWSE 口徑、含 7/17 爆量 97,362）
    "trend":    "✅ 崩跌後首日止穩、深超賣醞釀技術性修復：台股 2330 7/20 官方收 NT$2,320（+30、+1.31%、量縮至 55,790 張、前日 57%）——開 2,300 即當日最低、盤中高 2,345、站回布林下軌 2,307 之上、惟仍壓在 60MA 2,328 之下。⭐ 大盤續跌 -0.52%（盤中震盪逾 1,100 點、融資減碼）下 2330 逆勢撐盤——資金集中防禦龍頭。📊 技術指標（官方收盤計算）：RSI(14) 43.9 自 41.0 回升、MACD 綠柱慣性擴大至 -19.2（DIF 8.5 < DEA 27.7）空方動能未消、KDJ K(28.6)/D(40.3)/J(5.0) 死叉未解惟 J 值深度超賣——止穩訊號與空方慣性並存、屬「止穩待確認」格局。📊 籌碼面配合：三大法人 7/20 轉買超 +4,002 張、外資賣超收斂 96%。NYSE TSM 7/20 收 $402.30（+0.99%）收復 $400、SOX +0.60% 止跌（自史高 -19.8%）。下一觀察：（1）反彈續航兩道確認：站回 60MA 2,328 → 挑戰 5MA 2,388；（2）守住支撐 2,300-2,307（7/20 低/布林下軌）／2,290（7/17 低、雙底防線）／2,200；（3）量能：反彈需帶量、續縮量則反彈疲弱；（4）外資連 8 賣能否正式轉買；（5）SOX 能否連續收紅確認 AI 情緒止穩。",
    "support":  "2,300-2,307 / 2,290 / 2,200 (7/20 低+布林下軌、7/17 低、心理關卡)",
    "resist":   "2,328 / 2,388 / 2,424 (60MA、5MA、20MA)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-20 止），請以看盤軟體為準；7/20 官方收 NT$2,320（+1.31%）站回布林下軌 2,307、仍壓在 60MA 2,328 之下；RSI(14) 43.9、MACD 綠柱 -19.2（DIF 8.5 < DEA 27.7）、KDJ K28.6/D40.3/J5.0 深超賣；下方支撐 2,300-2,307 / 2,290 / 2,200、上方壓力 2,328 / 2,388 / 2,424；✅ 崩跌後首日止穩（三大法人轉買超 +4,002 張、外資賣超收斂 96%）vs ⚠️ AI 情緒未確認止穩（SOX 僅 +0.60%、設備股續跌、SOXX 熊市警告）——止穩待確認、站回 60MA 為反彈續航訊號",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-20",       # 最新資料日（7/20 Mon 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,312",            # 台積電期貨近月（估、現貨 7/20 反彈收 NT$2,320、貼水收斂）
    "futures_oi":    "189,000",          # 未平倉量（口）（估、反彈日部分避險部位回補）
    "futures_basis": "-8",               # 估逆價差（止穩反彈、貼水自崩跌日收斂）
    "call_oi":       "106,000",
    "put_oi":        "98,000",
    "pcr":           "0.92",             # Put/Call 比率（估、避險需求自崩跌高峰回落、仍偏高）
    "pcr_signal":    "中性偏空（Put/Call 估 0.92；7/20 現貨止穩反彈 +1.31% 收 NT$2,320、三大法人轉買超——避險需求自 7/17 崩跌高峰回落、惟 AI 情緒未確認止穩、Put OI 仍偏高；⚠️ 期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,300",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,099", "Call 60,000 / Put 24,000", "120MA、中期支撐"),
        ("2,200", "Call 55,000 / Put 19,000", "心理關卡、下方支撐"),
        ("2,290", "Call 45,000 / Put 12,000", "7/17 低、雙底防線"),
        ("2,300", "Call 42,000 / Put 10,000", "最大痛苦（估）、7/20 低"),
        ("2,320", "Call 40,000 / Put 9,000",  "7/20 官方收、多空分界"),
        ("2,328", "Call 38,000 / Put 7,500",  "60MA、反彈第一道關卡"),
        ("2,388", "Call 32,000 / Put 5,000",  "5MA、上方壓力"),
        ("2,424", "Call 26,000 / Put 3,500",  "20MA/布林中軌、壓力帶"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/20 NT$2,320）；NT$2,300 為估最大痛苦、下方支撐 2,300-2,307 / 2,290 / 2,200、上方壓力 2,328（60MA）/ 2,388（5MA）/ 2,424（20MA）；✅ 崩跌後首日止穩（量縮反彈 +1.31%、三大法人轉買超 +4,002 張、外資賣超收斂 96%）vs ⚠️ AI 情緒未確認止穩（SOX +0.60% 弱反彈、SOXX 熊市警告恐再跌 12%）——貼水收斂、避險需求自高峰回落；✅ 基本面錨未變（全年上修 >40%、CapEx $60-64B、股利 +33%、外資密集調升目標價平均 $520）——止穩待確認、站回 60MA 為反彈續航訊號",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-20 Mon TWSE 官方收盤；美股為 7/20 Mon 收盤、Yahoo Finance API 確認；韓股 7/17 休市、7/20 補跌，列 7/20 收盤、漲跌幅 vs 7/16）
    ("設備", "ASML",    "艾司摩爾",    "1,739.02", "-0.49%", "⭐ EUV 設備龍頭；7/20 續小跌、設備股落後大盤反彈；TSMC CapEx 上調至 $60-64B 為設備鏈利多"),
    ("設備", "AMAT",    "應用材料",    "525.70", "-0.75%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（7/20 續跌、設備股情緒未修復）；TSMC CapEx $60-64B 為設備鏈利多"),
    ("設備", "LRCX",    "科林研發",    "306.76", "-2.09%", "蝕刻設備（7/20 續明顯下跌、設備股為板塊最弱環節）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "207.60", "-2.42%", "量測設備（7/20 續跌、設備股續弱）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","151.00","-2.58%", "與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC（7/20 續跌、TWSE 官方；7/17 幾近跌停後未同步反彈）"),
    ("記憶", "000660.KS","SK 海力士",  "1,764,000 KRW", "-4.23%", "HBM4、與 NVIDIA 多年期合作；7/17 韓股休市、7/20 補跌 -4.23%（vs 7/16、Yahoo API）；7/21 早盤反彈約 +0.6%"),
    ("記憶", "MU",      "美光",        "865.46", "+1.94%", "HBM / DDR5（7/20 反彈領先記憶體鏈）；AI 記憶體修正後 HBM4 中長線邏輯未變"),
    ("封裝", "3711.TW", "日月光",      "597.00", "-2.77%", "CoWoS / 先進封裝（7/20 續跌收 597、TWSE 官方；7/17 幾近跌停後未同步反彈、台股中小型半導體賣壓未止）"),
    ("封裝", "AMKR",    "Amkor",       "62.56",  "-0.60%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/20 小跌）"),
    ("客戶", "NVDA",    "輝達",        "203.28", "+0.23%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/20 小漲止穩；Q1 FY2027 營收 $81.61B（+85% YoY）"),
    ("客戶", "AAPL",    "蘋果",        "326.59", "-2.14%", "⭐ #2 客戶 18%；7/20 逆勢明顯下跌——7/17 避風港資金反向流出；跳過 A16 直上 A14"),
    ("客戶", "AVGO",    "博通",        "378.16", "+1.98%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/20 反彈領先客戶群）"),
    ("客戶", "AMD",     "超微",        "503.57", "+1.58%", "#4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/20 反彈站回 $500）"),
    ("客戶", "2454.TW", "聯發科",      "3,340",  "-0.89%", "手機 SoC 客戶 9%（7/20 續小跌、TWSE 官方、未同步台積電反彈）；面臨漲價成本"),
    ("競爭", "INTC",    "英特爾",      "97.06",  "+2.13%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/20 反彈領先、自高點仍 -30%+）"),
    ("競爭", "005930.KS","三星電子",   "244,000 KRW", "-4.31%", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/17 韓股休市、7/20 補跌 -4.31%（vs 7/16、Yahoo API）；7/21 早盤反彈約 +2.3%"),
    ("產業", "SOX",     "費城半導體指數","11,743.85", "+0.60%", "7/20 小漲止跌收 11,743.85——自 6/22 史高 14,634.66 仍 -19.8%、熊市邊緣；⚠️ Yardeni 警告 SOXX 技術面熊市、恐再下探 12%"),
    ("處分後", "ARM",   "Arm Holdings","269.61", "+0.91%", "TSMC 4/29 全數出脫後（7/20 小漲、自高點仍 -30%+）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/20 收 $402.30 具 +21.9% 上行空間）；⭐ 法說會後新一輪上調進行中：Susquehanna $600、Barclays $650、BofA $590、TD Cowen $440——S&P Global 統計 19 位平均已達 $520.37（TipRanks/GuruFocus 7/17-20）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+21.89%",  # 從 NYSE 7/20 收 $402.30 至平均目標 $490.34（共識強力買入；法說會後多家上調、S&P Global 19 位平均 $520.37 對應約 +29%）
}
...
IR_CALENDAR = [
    ...
    ("2026-07-21", "📊 報告日：反彈續航觀察", "7/21 Tue 報告日；最新確認收盤：台股 2330 7/20 NT$2,320（+1.31%）、NYSE TSM 7/20 $402.30（+0.99%）；觀察：（1）能否站回 60MA ~2,328 進而挑戰 5MA ~2,388；（2）外資連 8 賣（-1,672、已收斂 96%）能否正式轉買；（3）AI 情緒面（SOXX 熊市警告、設備股續跌）能否確認止穩、韓股 7/21 早盤回穩延續；ADR 溢價 +12.0% 未收斂",  "#FFF9C4"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "11,743.85", "+0.60%", True),   # 7/20 收（Yahoo）小漲止跌；自 6/22 史高仍 -19.8%、熊市邊緣
    ("台股加權指數",    "42,449.70", "-0.52%", False),  # 7/20 官方收盤（-221.57 點、盤中震盪逾 1,100 點、TWSE）
    ("VIX 恐慌指數",    "18.65",    "-0.64%", True),    # 7/20 收（Yahoo）自 7/17 跳升後小幅回落
    ("WTI 原油",        "$82.46",   "持平",  False),    # 7/20-21 收（Yahoo；地緣溢價維持高檔）
    ("10Y 美債殖利率",  "4.60%",   "+6bp",   False),   # 7/20 收（Yahoo ^TNX、避險買盤退場回升）
    ("台幣/美元",       "~32.28",   "持平",  False),    # 7/21 Yahoo TWD=X（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$203.28", "+0.23%",  True),    # #1 TSMC 客戶（22%），7/20 小漲止穩
    ("ASML",            "$1,739.02", "-0.49%",  False),  # 設備股 7/20 續小跌（Yahoo）
]
...
NEWS = [
    {
        ...
        "title": "✅ 崩跌後首日止穩：2330 7/20 量縮反彈 +1.31% 收 NT$2,320、三大法人轉買超",
        "body": "止穩訊號浮現、惟屬待確認格局：（1）✅ 台股 2330 7/20 官方收 NT$2,320（+30、+1.31%、開 2,300／高 2,345／低 2,300）、量縮至 55,790 張（7/17 爆量的 57%）；（2）⭐ 大盤續跌 -221.57 點（-0.52%）收 42,449.70、盤中震盪逾 1,100 點——2330 逆勢撐盤貢獻約 244 點、資金集中防禦龍頭；（3）📊 三大法人對 2330 轉買超 +4,002 張（T86 官方）：外資 -1,672（連 8 賣、惟自 -44,184 收斂 96%）、投信 +1,049（連 3 買）、自營 +4,626（連 8 買）；全市場三大法人轉買超 21.7 億（BFI82U）；（4）🎯 反彈續航看能否站回 60MA ~2,328。",
        "source": "TWSE / 聯合新聞網 / 工商時報 2026-07-20", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "✅ NYSE TSM 收復 $400（+0.99%）、SOX +0.60% 止跌——惟設備股續跌、板塊分歧",
        "body": "美股止跌但情緒未全面修復：（1）✅ NYSE TSM 7/20 收 $402.30（+$3.93、+0.99%）收復 $400 關卡（Yahoo API 確認）；（2）📊 SOX 7/20 收 11,743.85（+0.60%）止跌——惟自 6/22 史高仍 -19.8%、處熊市邊緣；（3）⚠️ 板塊分歧：AVGO +1.98%、INTC +2.13%、MU +1.94%、AMD +1.58% 反彈 vs 設備股 LRCX -2.09%、KLAC -2.42% 與 AAPL -2.14% 續跌；（4）📊 台美 ADR 溢價維持 +12.0%（$402.30 vs 理論值 ~$359、匯率 ~32.3）——台美同步反彈、溢價未收斂。",
        "source": "Yahoo Finance API 2026-07-20", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🚀 法說會後外資密集調升 TSM 目標價：Susquehanna $600、Barclays $650",
        "body": "Q2 大超預期後評價全面上修：（1）🚀 Susquehanna 自 $575 上調至 $600、Barclays 自 $625 上調至 $650、BofA $590、TD Cowen $440、DA Davidson $500（TipRanks/GuruFocus、7/17-20）；（2）📊 S&P Global 統計 19 位分析師平均目標價 $520.37、共識評等「強力買進」——自 7/20 收 $402.30 具約 +29% 上行空間；（3）⭐ 上調邏輯：全年營收成長上修 >40%、CapEx $60-64B、晶圓漲價 5-10% 挹注毛利率、「AI 需求比預期更強」；（4）🎯 崩跌後估值消化（TSM P/E ~29.5x、台股 ~26.5x）＋目標價上修——風險報酬比改善。",
        "source": "TipRanks / GuruFocus / S&P Global 2026-07-17~20", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ SOXX 熊市未解：Yardeni 警告恐再下探 12%、AI 拋售潮未完全止穩",
        "body": "AI 情緒面尚未確認止穩：（1）⚠️ Yardeni 分析警告 iShares 半導體 ETF（SOXX）技術面已進入熊市、可能再下探 12%——「2 兆美元 AI 晶片拋售潮」未完全出清；（2）📉 台股中小型半導體賣壓未止：聯電 7/20 連 2 日跌停收 130、日月光 -2.77% 收 597、華邦電 -2.58%——僅台積電獲資金防禦；（3）📊 台股融資減碼賣壓明顯、大盤盤中一度重返 43,000 後急殺翻黑；（4）🎯 觀察 SOX 能否連續收紅、外資（-1,672 連 8 賣）能否正式轉買為情緒止穩兩大確認訊號。",
        "source": "Benzinga / ETtoday / 經濟日報 2026-07-20", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "✅ 漲價 5-10% 擴及 5/7nm：NVIDIA/AMD 需求強勁、定價權持續",
        "body": "客戶端結構性需求未受情緒面影響：（1）💰 台積電已通知 Apple、NVIDIA、Qualcomm、AMD 等主要客戶接受 5-10% 晶圓漲價、範圍自 3nm 擴及 5nm/7nm——定價權全面轉向代工廠、Q2 毛利率 67.7% 為佐證；（2）🏆 NVIDIA Q1 FY2027 營收 $81.61B（+85.2% YoY）穩居 TSMC 最大客戶（22%）；（3）🚀 AMD 資料中心營收年增 57%、MI450 系列獲 OpenAI、Meta 多 GW 承諾——CoWoS 2026 產能訂滿、缺口延續至 2027 年底；（4）⭐ 2nm 5 座廠爬坡、已貢獻 3% 營收，A16 由 NVIDIA 首發。",
        "source": "24/7 Wall St. / TechPowerUp / TechNews 2026-07-17~20", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 韓股記憶體補跌後回穩：SK 海力士 7/20 -4.2%、7/21 早盤反彈",
        "body": "記憶體鏈補跌一日後現回穩跡象：（1）📉 韓股 7/17 休市躲過全球崩跌、7/20 補跌：SK 海力士 -4.23% 收 1,764,000 韓元、三星電子 -4.31% 收 244,000 韓元（vs 7/16、Yahoo API）；（2）✅ 7/21 早盤雙雙反彈：SK 海力士約 +0.6%、三星約 +2.3%（盤中）——補跌賣壓快速消化；（3）📊 美系記憶體同步回穩：MU 7/20 +1.94% 收 $865.46 領先反彈；（4）⭐ HBM4 中長線邏輯未變：SK 海力士-NVIDIA 多年期合作、三星 HBM4E 送樣、與 TSMC CoWoS 先進封裝需求互為表裡。",
        "source": "Yahoo Finance API / 韓媒綜合 2026-07-20~21", "impact": "中性",
    },
]

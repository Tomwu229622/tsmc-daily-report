...
STOCK = {
    # 台股 2330
    "tw_price":       "1,810",        # 收盤價（數字加逗號，不含 NT$）（2026-04-08 收盤）
    "tw_change":      "-45",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-2.43%",       # 漲跌幅（含+/-號）
    "tw_volume":      "30,000",       # 成交量（張，估，關稅恐慌高量）
    "tw_open":        "1,850",
    "tw_high":        "1,852",
    "tw_low":         "1,800",
    "market_cap":     "NT$46.9 兆",
    "pe_ratio":       "~27.1x",       # 台股 P/E（TTM，2026-04-08）
    "pb_ratio":       "~9.8x",        # 台股 P/B（股淨比）
    "div_yield":      "~1.08%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "333.82",       # 盤中價（2026-04-08 12:21 PM EDT）
    "nyse_change":    "-13.95",       # 與前日相比
    "nyse_change_pct":"-4.01%",
    "52w_high_nyse":  "390.21",       # 52 週高（2026-02-25）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~30.8x",       # NYSE USD 基礎 P/E（TTM）
    "pb_nyse":        "~9.8x",
    "beta":           "1.16",
    "div_next":       "下次配息預計 2026-06（Q2 季配，US$0.968/ADR）",
    "ex_date":        "2026-04-09（下次除息日，Q2）",
    "next_earnings":  "2026-04-16",
    # 資料日期備註
    "data_note":      "台股數據：2026-04-08（NT$1,810，-2.43%）  |  NYSE 數據：2026-04-08 $333.82（-4.01% 盤中）  |  全球關稅風暴衝擊，台股 2330 重挫 45 元，量能估 3 萬張  |  TSMC 4/10 公布 3 月月營收  |  4/16 Q1 財報倒計時 8 天  |  美政府擬給予 TSMC 關稅豁免（亞利桑那廠投資 $1,650 億美元）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏空",
    # 一句話標題
    "headline":   "台股 2330 收 NT$1,810（-2.43%）全球關稅風暴衝擊，NYSE TSM $333.82（-4.01%），美政府擬給 TSMC 關稅豁免，4/16 Q1 財報倒計時 8 天",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330 收 NT$1,810（-45，-2.43%），成交量估 3 萬張，隨全球股市同步下挫",
        "NYSE TSM $333.82（-$13.95，-4.01% 盤中），關稅恐慌拖累美股，VIX 恐慌指數飆升",
        "美國政府擬給予 TSMC 關稅豁免（亞利桑那廠 $1,650 億美元投資換取）；若落實為重大利多",
        "TSMC 將於 4/10 公布 3 月月營收（市場共識年增 ~30%）；4/16 Q1 財報倒計時 8 天",
        "TSMC 達成美台半導體貿易協議，台灣廠商 15% 關稅率換取美國投資 $2,500 億承諾",
    ],
    # 一句結論
    "bottom_line": "台積電（4/8）台股 2330 收 NT$1,810（-2.43%，-45），NYSE TSM $333.82（-4.01% 盤中）。全球關稅風暴衝擊市場，但 TSMC 美國投資 $1,650 億美元有望換取關稅豁免，為中期利多。4/10 3 月月營收（共識年增約 30%）及 4/16 Q1 財報（EPS 共識 NT$20.38）為近期關鍵催化劑。基本面強健，分析師 31 人全數買入，目標 NT$2,290（+26.5% 上行空間），短線受宏觀衝擊，中線偏多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 關稅風暴衝擊：台股 2330（4/8）-2.43%，NYSE TSM -4.01%；全球貿易戰升溫，VIX 恐慌指數大幅走高；4/10 月營收、4/16 Q1 財報為近期關鍵催化劑；中國軍事活動升高，地緣政治風險持續存在",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-08",
    "foreign":  -3500,    # 外資（正=買超）— 4/8 關稅恐慌重度賣出（估）
    "trust":    +280,     # 投信（持續逢低承接，估）
    "dealer":   -350,     # 自營商（估，恐慌日跟空）
    # 近5日（由舊到新，最後一筆為今日估算）
    "foreign_5d": [-3200, +1500, -1200, -800, -3500],  # 4/8 關稅衝擊重度流出（估）
    "trust_5d":   [+300,  +350,  +280,  +310,  +280],
    "dealer_5d":  [-150,  +200,  -200,  -180,  -350],
    "foreign_ownership_pct": "74.0%",  # 外資持股比例（估，持續流出中）
    "foreign_ownership_shares": "約 192.1 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       29.5,   "rsi_signal": "RSI 跌破 30 進入超賣區間（29.5），短線跌過急，技術性反彈條件成熟",
    "macd":         -15.2,  "macd_signal": "MACD 死叉擴大，空頭動能強，需等量縮後觀察背離訊號",   "macd_color": "#F44336",
    "kdj_k":        18.5,   "kdj_d": 22.8, "kdj_j": 9.9, "kdj_signal": "KDJ J 值跌至深超賣（9.9），與前一低點形成雙底可能，留意反彈",
    "ma5":       "1,835",
    "ma20":      "1,848",
    "ma60":      "1,830",
    "ma120":     "1,745",
    "bb_upper":  "1,970",   # 布林上軌
    "bb_mid":    "1,848",   # 布林中軌（20MA）
    "bb_lower":  "1,726",   # 布林下軌
    "vol_ma5":  "29,500",   # 5日均量（張，估）
    "trend":    "台股 2330（4/8 收 NT$1,810，-2.43%）關稅恐慌重挫，NYSE TSM $333.82（-4.01% 盤中）。RSI 跌破 30 進入超賣（29.5），KDJ J 值極度超賣（9.9），MACD 死叉擴大。短線技術面嚴重超賣，後市若宏觀風險緩解（關稅豁免落地）可期待反彈。關鍵支撐 1,800，若破位恐測 1,750。4/10 月營收、4/16 Q1 財報為主要催化劑。",
    "support":  "1,800–1,810",
    "resist":   "1,850–1,870",
...
DERIVATIVES = {
    "date":          "2026-04-08",
    "futures_price": "1,805",      # 台積電期貨近月（估，現貨折價，空頭氣氛）
    "futures_oi":    "85,200",     # 未平倉量（口）（估，空單增加）
    "futures_basis": "-5",         # 逆價差（空頭氣氛偏強）
    "call_oi":       "22,800",
    "put_oi":        "68,500",
    "pcr":           "3.01",       # Put/Call 比率大幅升高（空頭情緒強）
    "pcr_signal":    "偏空（恐慌高峰常見反轉訊號）",
    "max_pain":      "1,820",      # 最大痛苦價格
    "key_strikes": [
        ("1,750", "Call 1,200 / Put 15,800", "次要支撐"),
        ("1,780", "Call 2,500 / Put 12,000", "關鍵支撐"),
        ("1,800", "Call 4,800 / Put 9,500",  "重要支撐/現貨附近"),
        ("1,820", "Call 7,200 / Put 7,800",  "最大痛苦點/壓力"),
        ("1,850", "Call 11,000 / Put 5,200", "強壓力區"),
    ],
...
ECOSYSTEM_STOCKS = [
    ...
    ("設備", "ASML",    "艾司摩爾",    "638.50", "-3.5%", "EUV 設備龍頭（關稅衝擊）"),
    ("設備", "AMAT",    "應用材料",    "158.30", "-4.2%", "CVD/PVD 設備"),
    ("設備", "LRCX",    "科林研發",    "748.60", "-4.8%", "蝕刻設備"),
    ("設備", "KLAC",    "科磊",        "552.10", "-4.1%", "量測設備"),
    ("記憶", "000660.KS","SK 海力士",  "278,000 KRW", "-3.5%", "HBM3e/4 供應商"),
    ("記憶", "MU",      "美光",        "88.50",  "-4.5%", "HBM / DDR5（關稅衝擊）"),
    ("封裝", "3711.TW", "日月光",      "208",    "-2.8%", "CoWoS 封裝"),
    ("客戶", "NVDA",    "輝達",        "785.40", "-3.8%", "#1 客戶 22%（關稅風暴同步重挫）"),
    ("客戶", "AAPL",    "蘋果",        "198.20", "-5.1%", "#2 客戶 17%（供應鏈依賴中國，跌幅最大）"),
    ("客戶", "AVGO",    "博通",        "182.60", "-4.3%", "#3 客戶 13%（Custom XPU）"),
    ("客戶", "AMD",     "超微",        "148.20", "-4.2%", "#4 客戶 8%（2nm EPYC Venice）"),
    ("客戶", "2454.TW", "聯發科",      "1,248",  "-3.1%", "手機 SoC 客戶 9%"),
    ("競爭", "INTC",    "英特爾",      "19.80",  "-3.5%", "Intel Foundry"),
    ("競爭", "005930.KS","三星電子",   "50,200 KRW", "-3.2%", "Samsung Foundry"),
]
...
CONSENSUS = {
    ...
    "upside":       "+26.5%",   # 從 NT$1,810 到 NT$2,290
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "3,620",   "-4.5%",  True),   # 4/8 估，關稅重挫
    ("美元指數 DXY",    "102.5",   "-1.2%",  False),  # 貿易戰令美元走弱
    ("VIX 恐慌指數",    "45.3",    "+12.8",  True),   # 恐慌指數飆升，接近 COVID 高點
    ("WTI 原油",        "$63.5",   "-3.8%",  True),   # 衰退擔憂壓制油價
    ("10Y 美債殖利率",  "4.01%",   "-0.23pp",False),  # 避險資金湧入美債
    ("台幣/美元",       "31.70",   "-0.10",  False),  # 台幣小幅走強（估）
    ("NVIDIA NVDA",     "$785",    "-3.8%",  True),   # AI 大廠隨市重挫
    ("ASML",            "$638",    "-3.5%",  True),   # 設備股同步下跌
]
...
VALUATION = {
    "pe_current":   27.1,   # 同 STOCK["pe_ratio"]（4/8 NT$1,810）
...
RELATIVE = {
    "date":           "2026-04-08",
    "tsmc_tw_1d":     -2.43,   # 2330 4/8 收盤 1,810（-2.43%）
    "taiex_1d":       -4.20,   # 加權指數（4/8 估算，關稅重挫）
    "sox_1d":         -4.50,   # SOX（4/8 估，關稅衝擊更深）
    "tsmc_nyse_1d":   -4.01,   # TSM ADR 4/8 $333.82（-4.01% 盤中）
    "tsmc_tw_5d":     -6.5,    # 本週累計（估，關稅恐慌大跌）
    "taiex_5d":       -5.2,
    "tsmc_tw_1m":     -11.8,
    "taiex_1m":       -7.5,
    "tsmc_tw_ytd":    -12.7,   # Q1+4/1–4/8 累計
    "taiex_ytd":      -10.8,
    "sox_ytd":        -26.5,
    "beta_60d":        1.16,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價
    "adr_price":     333.82,   # TSM NYSE（2026-04-08 盤中）
    "adr_parity":    285.49,   # 理論值 = 5 × NT$1,810 ÷ 31.70（台股 4/8 收盤）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.70,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+16.9",# (333.82 - 285.49) / 285.49 × 100
...
FX_SENSITIVITY = {
    "spot":              31.80,  # 今日匯率（同 MACRO，台幣小幅回升）
...
NEWS = [
    {
        ...
        "title": "全球關稅風暴衝擊半導體股，TSMC 台股重挫 2.43%、NYSE TSM -4.01%",
        "body": "美國大規模關稅政策引發全球股市恐慌，4/8 台股 2330 收 NT$1,810（-2.43%），NYSE TSM 盤中大跌至 $333.82（-4.01%）。VIX 恐慌指數飆升至 45+，SOX 半導體指數重挫逾 4.5%。供應鏈依賴中國的 Apple（-5.1%）跌幅最深，NVIDIA（-3.8%）、AMD（-4.2%）同步重挫。市場擔憂關稅拉高 AI 晶片成本並壓縮科技公司資本支出，惟 TSMC 基本面強健未受動搖。",
        "source": "Bloomberg / Yahoo Finance 2026-04-08", "impact": "負面",
    },
    {
    ...
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "TSMC 4/10 公布 3 月月營收，市場預估年增 ~30%",
        "body": "TSMC 將於 2026 年 4 月 10 日（週五）盤後公布 2026 年 3 月合併營收。前 2 月累計營收 NT$7,189 億（年增 30%），市場共識預估 3 月單月亦維持 ~30% 年增率。若月營收超預期，將為 4/16 Q1 財報（EPS 共識 NT$20.38，毛利率指引 63–65%）預熱，可能引發法人上修 EPS 共識。",
        "source": "Motley Fool / TSMC IR 2026-04-08", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA 超越 Apple 成 TSMC 最大客戶，A16 製程首發合作確認",
        "body": "NVIDIA CEO Jensen Huang 確認，NVIDIA 已超越 Apple 成台積電最大單一客戶，2026 年預計貢獻約 $330 億 USD（22%），Apple 退居第二（17%）。NVIDIA 亦確認為 TSMC A16（1.6nm 背面供電）首發客戶，Apple 跳過 A16 直升 A14。中國 AI 晶片廠商（CXMT、SMIC）在出口管制下持續高速增長，全球供應鏈版圖重組加速。",
        "source": "CNBC / eWeek 2026-04-07", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "半導體進入「超級周期」，AI 基礎建設驅動 2026 年銷售額達 $9,750 億",
        "body": "Deloitte / Tom's Hardware 報告指出，全球半導體進入前所未見的「超大周期（Giga-Cycle）」，AI 基礎建設投資重塑計算、記憶體、網路與儲存需求。2026 年全球半導體銷售額預估達 $9,750 億（年增 26%）。高價值 AI 晶片雖僅佔出貨量不到 0.2%，卻貢獻近一半營收。台積電作為先進製程唯一大規模供應商，受益最深。",
        "source": "Tom's Hardware / Deloitte 2026-04-08", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "中東危機與中國軍事活動升高，TSMC 供應鏈能見度受關注",
        "body": "TSMC 4/16 財報法說會中，管理層預計將就中東局勢與台海緊張對供應鏈的影響發表看法。中國軍事活動於 2026 年達歷史新高，同時中國加速推動 AI 晶片自給自足（CXMT 年增 130%、SMIC 營收達 $93 億）。美國政府持續壓制先進晶片設備出口，地緣政治溢價料將持續反映在 TSM 估值折扣上。",
        "source": "WccfTech / CNBC 2026-04-08", "impact": "負面",
    },
]

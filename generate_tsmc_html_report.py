...
STOCK = {
    # 台股 2330
    "tw_price":       "2,405",        # 台股 2330 最新確認收盤：7/23 Thu 官方收 NT$2,405（+5、+0.21% vs 7/22 NT$2,400、開 2,385／高 2,405／低 2,370、TWSE 確認）——低開後尾盤拉升收在當日最高、反彈第四日量縮整理；大盤同日尾盤翻紅 +25.03 點（+0.06%）收 44,850.81
    "tw_change":      "+5",           # 7/23 Thu 收盤漲跌點數（+0.21%、TWSE 官方）
    "tw_change_pct":  "+0.21%",       # 7/23 Thu 收盤漲跌幅（低開走高、尾盤拉升收最高）
    "tw_volume":      "28,001",       # 7/23 成交量（張、TWSE 口徑；成交股數 28,001,492 股、成交額 NT$668.34 億、成交 142,112 筆——量續縮（前日 31,653 張）、續攻量能仍未放大；成交筆數 142,112 續增（前日 136,194）、小單換手更活絡）
    "tw_open":        "2,385",        # 7/23 開盤（TWSE、平前日低點開出）
    "tw_high":        "2,405",        # 7/23 盤中高＝收盤（TWSE、尾盤拉升收最高）
    "tw_low":         "2,370",        # 7/23 盤中低（TWSE、一度跌破前日 5MA 2,378、尾盤收復）
    "market_cap":     "NT$62.4 兆",   # 7/23 收盤市值（NT$2,405 基礎、約 259.3 億股）
    "pe_ratio":       "~27.5x",       # 台股 P/E（TTM 估，NT$2,405 ÷ 近四季 EPS ~87.4——含 Q2 EPS NT$27.25）
    "pb_ratio":       "~18.0x",       # 台股 P/B（NT$2,405 ÷ 每股淨值 ~133.4、估；TWSE 官方口徑以最新季淨值為準）
    "div_yield":      "~1.00%",       # 殖利率（法說會宣布 2026 全年股利 NT$24/股（+33% YoY）÷ NT$2,405）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "415.58",       # 7/23 Thu 官方收盤 $415.58（-$5.63、-1.34% vs 7/22 $421.21、Yahoo Finance API 確認）——AI 權值回檔（量 11.0M 股）；SOX 同日 -0.54% 收 12,343.84 終結連 2 紅（自 6/22 史高 14,634.66 收斂至 -15.7%）
    "nyse_change":    "-5.63",        # 7/23 漲跌（自 7/22 $421.21）
    "nyse_change_pct":"-1.34%",       # 7/23 板塊再換手：記憶體/設備強（MU +3.20%／KLAC +1.88%／AMAT +1.60%）vs 客戶端回吐（NVDA -0.46%／AMD -2.29%／AAPL -1.30%／AVGO -1.09%）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~30.4x",       # NYSE USD 基礎 P/E（TTM 估，$415.58 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.5x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-07-24 Fri（最新確認收盤：台股 2330 7/23 Thu 官方收 NT$2,405（+5、+0.21% vs 7/22 NT$2,400、開 2,385／高 2,405／低 2,370、量 28,001 張 TWSE 口徑、成交額 NT$668.34 億、成交 142,112 筆、TWSE 確認）；NYSE TSM 7/23 Thu 收 $415.58（-$5.63、-1.34% vs 7/22 $421.21、Yahoo Finance API 確認））。📊 今日格局：反彈第四日量縮整理、尾盤拉升守穩——（1）2330 7/23 平低開 2,385、盤中低 2,370 一度跌破前日 5MA 2,378、尾盤拉升收 2,405（+0.21%）收在當日最高——連 2 日收斂於 2,400-2,405 窄幅、賣壓遞減；（2）⭐ 大盤 7/23 尾盤翻紅 +25.03 點（+0.06%）收 44,850.81 連 3 紅——成交額縮至 9,306 億（前日 1.026 兆）、2330 尾盤拉升為指數翻紅主力、反彈自普漲進入輪動整理期；（3）📊 籌碼兩層分化延續：外資對 2330 連 2 賣且擴大 -4,614 張（前日 -3,539、T86 官方）、投信 -943 張終結連 5 買、自營 +38 張、三大法人合計 -5,519 張；⭐ 惟全市場 BFI82U 官方：三大法人買超 185.2 億——外資 +69.6 億連 2 買、投信 +73.7 億、自營 +41.9 億——全市場資金續回流 vs 對 2330 個股獲利了結並存；（4）⚠️ NYSE TSM 7/23 收 $415.58（-1.34%）、SOX -0.54% 收 12,343.84 終結連 2 紅（自 6/22 史高 -15.7%）——板塊再換手（與 7/22 相反）：記憶體/設備逆勢強（MU +3.20%、KLAC +1.88%、AMAT +1.60%——AI 賣壓凸顯記憶體稀缺敘事）vs 客戶端回吐（NVDA -0.46%、AMD -2.29%、AAPL -1.30%、AVGO -1.09%、INTC -2.33%）；（5）⚠️ 韓股 7/23 收大漲（SK 海力士 +4.86% 收 191.9 萬韓元、三星 +3.65% 收 27.0 萬）後、7/24 早盤雙雙重挫（SK -5.1% 至 182.2 萬、三星 -4.3% 至 25.85 萬）——今日台股半導體開盤逆風訊號；（6）⚠️ 總經風險指標齊升：VIX +12.4% 回升至 18.70、WTI 近月急漲至 ~$92（地緣溢價）、10Y 美債殖利率 4.70%（+4bp）。📊 台美 ADR 溢價 +11.6%（$415.58 vs 理論值 ~$372.3、匯率 ~32.30）——自 +13.6% 收斂、台股相對抗跌所致。技術面（官方收盤計算至 7/23）：收 2,405 位於 5MA 2,365 之上、距 20MA 2,415 僅 10 點；RSI(14) 51.1 中軸之上續升（前日 50.7）、MACD 綠柱四日連續收斂至 -10.5（DIF 7.6 < DEA 18.1；-19.2 → -15.2 → -12.8 → -10.5）、KDJ 金叉後開口擴大（K54.2/D47.9/J66.8）；支撐 2,370（7/23 低）／2,365（5MA）／2,345（7/21 低）／2,340（60MA）、壓力 2,415（20MA/布林中軌）／2,440-2,445（7/22 開盤高）／2,470（7/16 收前高）／2,519-2,535（布林上軌/史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR +77.4%、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B（Arizona 追加 $100B）、股利 +33%、2027 全面漲價 5-10% 傳聞＋2nm Q3 放量。⚠️ 風險：韓股重挫外溢、量能連 3 日未放大（2.8-3.2 萬張）、外資對 2330 連 2 賣、VIX/油價/美債殖利率同步走升、美 232 條款半導體關稅 25%、台幣 ~32.30。⭐ 今日（7/24）觀察：（1）韓股重挫下能否守住 5MA 2,365 與 2,370；（2）站上 20MA 2,415 為反彈續航第一確認；（3）量能能否放大至 4 萬張；（4）外資對 2330 賣壓是否收斂。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "📊 反彈第四日量縮整理：台股 2330 7/23 尾盤拉升收 NT$2,405（+0.21%）收在當日最高、大盤靠權值尾盤翻紅連 3 紅收 44,850.81；外資對 2330 連 2 賣 -4,614 張、投信終結連 5 買，惟全市場三大法人續買超 185.2 億；NYSE TSM -1.34% 收 $415.58、SOX 終結連 2 紅、記憶體逆勢強 MU +3.20%；⚠️ 韓股 7/24 早盤重挫 SK -5.1%／三星 -4.3% 為今日逆風",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "📊 台股 2330 7/23 官方收 NT$2,405（+5、+0.21%、開 2,385／高 2,405／低 2,370）——盤中一度跌破前日 5MA、尾盤拉升收在當日最高；量續縮至 28,001 張、成交筆數 142,112 續增、小單換手活絡",
        "⭐ 大盤 7/23 尾盤翻紅 +25.03 點（+0.06%）收 44,850.81 連 3 紅——成交額縮至 9,306 億、2330 尾盤拉升為指數翻紅主力；反彈自普漲進入輪動整理期",
        "📊 籌碼兩層分化延續：外資對 2330 連 2 賣且擴大 -4,614 張、投信 -943 終結連 5 買、自營 +38；⭐ 全市場 BFI82U：三大法人買超 185.2 億——外資 +69.6 億連 2 買、投信 +73.7 億",
        "⚠️ NYSE TSM 7/23 收 $415.58（-1.34%）、SOX -0.54% 終結連 2 紅——板塊再換手：記憶體/設備強（MU +3.20%／KLAC +1.88%／AMAT +1.60%）vs 客戶端回吐（NVDA -0.46%／AMD -2.29%／AAPL -1.30%）",
        "⚠️ 韓股 7/23 大漲（SK 海力士 +4.86%／三星 +3.65%）後 7/24 早盤雙雙重挫（SK -5.1%／三星 -4.3%）——今日台股半導體開盤逆風；VIX +12.4% 回升 18.70、WTI 急漲 ~$92",
        "📊 台股供應鏈 7/23 漲多回吐：華邦電 -5.85% 收 161（漲停次日獲利了結）、日月光 -1.07% 收 649；聯發科 +0.65% 收 3,875 連 3 漲",
        "💡 技術面：KDJ 金叉後開口擴大（K54.2/D47.9/J66.8）、RSI 51.1 中軸上續升、MACD 綠柱四日連續收斂 -10.5；收 2,405 距 20MA 2,415 僅 10 點——站上 20MA 為反彈續航第一確認",
        "📊 ADR 溢價自 +13.6% 收斂至 +11.6%（匯率 ~32.30）——台股相對抗跌、補漲壓力減輕；惟量能連 3 日 2.8-3.2 萬張未放大仍為最大保留",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-24 Fri（最新確認收盤：台股 2330 7/23 Thu 官方收 NT$2,405（+5、+0.21% vs 7/22 NT$2,400、開 2,385／高 2,405／低 2,370、量 28,001 張 TWSE 口徑、成交額 NT$668.34 億、成交 142,112 筆、TWSE 確認）；NYSE TSM 7/23 收 $415.58（-$5.63、-1.34%、Yahoo API 確認））。📊 核心判讀：反彈第四日量縮整理、多空收斂——（1）2330 低開走高尾盤拉升：平低開 2,385、盤中低 2,370 一度跌破前日 5MA、尾盤買盤進場收 2,405 當日最高——連 2 日收在 2,400-2,405 窄幅、上下影線收斂、賣壓遞減但攻擊量能同樣缺席；（2）大盤靠權值翻紅：加權指數尾盤 +25.03 點（+0.06%）收 44,850.81 連 3 紅、成交額縮至 9,306 億——2330 尾盤拉升為指數翻紅主力、反彈自普漲進入輪動整理期；（3）籌碼兩層分化延續：外資對 2330 連 2 賣且擴大（-3,539 → -4,614 張）、投信 -943 終結連 5 買——個股籌碼轉觀望；惟全市場三大法人買超 185.2 億、外資 +69.6 億連 2 買——資金留在台股、只是不在權值；（4）美股板塊再換手（方向與 7/22 相反）：TSM -1.34%、SOX -0.54% 終結連 2 紅；記憶體/設備強（MU +3.20%、KLAC +1.88%、AMAT +1.60%——AI 賣壓凸顯記憶體稀缺）vs 客戶端回吐（NVDA -0.46%、AMD -2.29%）——輪動而非趨勢反轉、但 AI 權值連續回檔對台股半導體心理面偏空。⚠️ 今日最大變數：韓股 7/23 大漲後 7/24 早盤重挫（SK 海力士 -5.1%、三星 -4.3%）＋ VIX +12.4% 回升 18.70、WTI 急漲 ~$92、10Y 美債 4.70%——risk-off 訊號齊聚、台股半導體開盤逆風。技術面動能修復方向未變：KDJ 金叉後開口擴大（K54.2/D47.9/J66.8）、RSI 51.1 續升、MACD 綠柱四日連續收斂 -10.5——收 2,405 距 20MA 2,415 僅 10 點、站上 20MA 與放量 4 萬張仍為反彈續航雙確認。📊 ADR 溢價自 +13.6% 收斂至 +11.6%——台股補漲壓力減輕。✅ 基本面錨完全未變：Q2 營收 $40.20B（+33.7% YoY）、EPS $4.31/ADR（+77.4%、連 11 季超預期）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、股利 +33%、2027 漲價傳聞＋2nm Q3 放量。⭐ 今日（7/24）觀察：（1）韓股重挫下守住 5MA 2,365／2,370 則整理格局不變；（2）站上 20MA 2,415 → 收復 2,440-2,445 → 挑戰 2,470；（3）失守 2,365 則回測 2,345／2,340（60MA）；（4）外資對 2330 賣壓能否收斂。⚠️ 風險：韓股重挫外溢、AI 估值修正再起、美 232 條款關稅 25% + 輸中晶片轉運查驗、Intel 18A 競爭、台幣 ~32.30（每升值 1% 毛利率 -40bps）。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/24 Fri）關鍵觀察：（1）⚠️ 韓股開盤重挫為最大變數：SK 海力士 7/24 早盤 -5.1%、三星 -4.3%（7/23 大漲後獲利了結）＋美股 AI 權值連 2 日回檔（NVDA -0.46%、AMD -2.29%）——台股半導體開盤逆風、觀察 2330 能否守住 5MA 2,365／7/23 低 2,370；（2）📊 方向選擇：收 2,405 距 20MA 2,415 僅 10 點——站上 20MA 並收復 2,440-2,445 則反彈續航、上看 2,470 → 2,519-2,535；失守 2,365 則回測 2,345／2,340（60MA）、跌破 2,320 反彈結構轉弱；（3）⚠️ 量能連 3 日 2.8-3.2 萬張未放大（7/17 爆量 97,362 的三成）——續攻需 4 萬張以上；惟成交筆數 142,112 續增、小單換手活絡；（4）📊 籌碼：外資對 2330 連 2 賣且擴大（-4,614 張）、投信終結連 5 買——個股籌碼轉觀望、惟全市場三大法人續買超 185.2 億資金未離場；（5）⚠️ 總經 risk-off 訊號齊聚：VIX +12.4% 收 18.70、WTI 近月急漲 ~$92（地緣溢價）、10Y 美債 4.70%；💰 2nm Q3 放量與 2027 漲價傳聞為中期上修觸媒；💱 美 232 條款關稅 25%、台幣 ~32.30 續為背景風險",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-23",       # 最新資料日（7/23 Thu、TWSE T86 官方）
    "foreign":  -4614,             # 外資 7/23 賣超 4,614 張（連 2 賣且較 7/22 的 -3,539 擴大——高檔獲利了結延續、趨勢反轉確認再延後）
    "trust":    -943,              # 投信 7/23 賣超 943 張（終結連 5 買——最穩定多方也轉觀望）
    "dealer":   38,                # 自營商 7/23 小幅買超 38 張（前日 -1,059 轉正）；三大法人合計 -5,519 張（連 2 日賣超）
    # 近5日（由舊到新：7/17, 7/20, 7/21, 7/22, 7/23）；TWSE T86 口徑（張）
    "foreign_5d": [-44184, -1672, 4976, -3539, -4614],  # 外資（7/17 史詩級賣超 → 7/21 轉買一日 → 連 2 日回賣擴大）
    "trust_5d":   [1489, 1049, 974, 1562, -943],         # 投信（連 5 買後 7/23 終結轉賣）
    "dealer_5d":  [2759, 4626, 214, -1059, 38],          # 自營（7/22 轉賣一日、7/23 小幅回買）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       51.1,   "rsi_signal": "RSI(14) 51.1（官方收盤計算）——7/23 收漲後自 50.7 續升、站穩 50 中軸之上；動能溫和修復、站上 60 為強勢確認",
    "macd":         -10.5, "macd_signal": "MACD 綠柱續收斂至 -10.5（DIF 7.6 < DEA 18.1、官方收盤計算）——自 -19.2 → -15.2 → -12.8 → -10.5 四日連續收斂、空方動能持續減弱；DIF 上穿 DEA 為正式翻多訊號",   "macd_color": "#F44336",
    "kdj_k":        54.2,   "kdj_d": 47.9, "kdj_j": 66.8, "kdj_signal": "KDJ K(54.2)/D(47.9)/J(66.8)（官方收盤計算）——⭐ 金叉後開口持續擴大：7/22 金叉兌現（K49.4/D44.8）、7/23 K 續升至 54.2 且 J 值 66.8——超賣修復動能延續、中期指標走強",
    "ma5":       "2,365",   # 5MA（官方收盤計算；7/23 盤中低 2,370 守住其上）
    "ma20":      "2,415",   # 20MA（官方收盤計算；收 2,405 距其僅 10 點——下一關鍵確認）
    "ma60":      "2,340",  # 60MA（官方收盤計算；中期支撐）
    "ma120":     "2,116",  # 120MA（官方收盤計算）
    "bb_upper":  "2,519",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,415",   # 布林中軌（20MA）
    "bb_lower":  "2,311",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "48,882",   # 5日均量（張、TWSE 口徑、仍含 7/17 爆量 97,362）
    "trend":    "📊 反彈第四日量縮整理、KDJ 金叉開口擴大：台股 2330 7/23 官方收 NT$2,405（+5、+0.21%、開 2,385／高 2,405／低 2,370）——平低開後盤中一度跌破前日 5MA、尾盤拉升收在當日最高；連 2 日收斂於 2,400-2,405 窄幅、賣壓遞減。⭐ 大盤同日尾盤翻紅 +25.03 點（+0.06%）收 44,850.81 連 3 紅、成交額縮至 9,306 億——2330 尾盤拉升為指數翻紅主力。📊 技術指標（官方收盤計算）：KDJ 金叉後開口擴大（K54.2/D47.9/J66.8）、RSI(14) 51.1 中軸上續升、MACD 綠柱四日連續收斂至 -10.5（DIF 7.6 < DEA 18.1）——動能修復方向未變。📊 籌碼：外資對 2330 連 2 賣且擴大 -4,614 張、投信 -943 終結連 5 買；⭐ 全市場三大法人買超 185.2 億、外資 +69.6 億連 2 買（BFI82U）——資金留在台股、個股先觀望。⚠️ 量能連 3 日 2.8-3.2 萬張未放大——惟成交筆數 142,112 續增、換手活絡。NYSE TSM 7/23 收 $415.58（-1.34%）、SOX -0.54% 終結連 2 紅——記憶體/設備強（MU +3.20%）vs 客戶端回吐；⚠️ 韓股 7/24 早盤重挫（SK -5.1%／三星 -4.3%）為今日開盤逆風。下一觀察：（1）站上 20MA 2,415（距僅 10 點）→ 收復 2,440-2,445 → 挑戰 2,470 → 2,519-2,535；（2）支撐 2,370（7/23 低）／2,365（5MA）／2,345／2,340（60MA）；（3）量能能否放大至 4 萬張以上；（4）外資對 2330 賣壓能否收斂。",
    "support":  "2,370 / 2,365 / 2,345 / 2,340 (7/23 低、5MA、7/21 低、60MA)",
    "resist":   "2,415 / 2,440-2,445 / 2,470 / 2,519-2,535 (20MA+布林中軌、7/22 開盤高、7/16 收前高、布林上軌+史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-07-23 止），請以看盤軟體為準；7/23 官方收 NT$2,405（+0.21%）低開走高：盤中低 2,370 一度跌破前日 5MA、尾盤拉升收在當日最高；⭐ KDJ 金叉後開口擴大（K54.2/D47.9/J66.8）、RSI 51.1 中軸上續升、MACD 綠柱四日連續收斂 -10.5；支撐 2,370 / 2,365 / 2,345 / 2,340、壓力 2,415 / 2,440-2,445 / 2,470 / 2,519-2,535；✅ 全市場三大法人續買超 185.2 億＋KDJ/RSI/MACD 同步修復 vs ⚠️ 外資對 2330 連 2 賣 -4,614 張＋量能未放大（連 3 日 2.8-3.2 萬張）＋韓股 7/24 早盤重挫——站上 20MA 與放量 4 萬張為反彈續航雙確認",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-23",       # 最新資料日（7/23 Thu 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,401",            # 台積電期貨近月（估、現貨 7/23 收 NT$2,405、小幅貼水）
    "futures_oi":    "180,000",          # 未平倉量（口）（估、量縮整理日多空觀望）
    "futures_basis": "-4",               # 估逆價差（整理日小幅貼水）
    "call_oi":       "110,000",
    "put_oi":        "97,000",
    "pcr":           "0.88",             # Put/Call 比率（估、較 7/22 的 0.87 微升——韓股 7/24 早盤重挫、避險需求小幅回升）
    "pcr_signal":    "中性（Put/Call 估 0.88；7/23 現貨尾盤拉升收 NT$2,405（+0.21%）、量縮整理多空收斂；⚠️ 韓股 7/24 早盤重挫＋VIX 回升 18.70、避險需求較 7/22 微升；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,320", "Call 40,000 / Put 10,000", "7/20 收、回檔支撐"),
        ("2,340", "Call 37,000 / Put 8,500",  "60MA、中期支撐"),
        ("2,365", "Call 34,000 / Put 7,500",  "5MA、短線支撐"),
        ("2,370", "Call 33,000 / Put 7,000",  "7/23 低、當日防線"),
        ("2,400", "Call 32,000 / Put 6,500",  "最大痛苦（估）、多空分界"),
        ("2,415", "Call 29,000 / Put 5,500",  "20MA/布林中軌、距收盤僅 10 點"),
        ("2,445", "Call 25,000 / Put 4,500",  "7/22 開盤高、短線壓力"),
        ("2,470", "Call 22,000 / Put 3,500",  "7/16 收前高、上方壓力"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（7/23 NT$2,405）；NT$2,400 為估最大痛苦、下方支撐 2,370（7/23 低）/ 2,365（5MA）/ 2,345 / 2,340（60MA）、上方壓力 2,415（20MA）/ 2,440-2,445（7/22 開盤高）/ 2,470（7/16 收）/ 2,519-2,535（布林上軌/史高）；📊 反彈第四日量縮整理：尾盤拉升收最高、外資個股連 2 賣 vs 全市場續買超 185.2 億——多空觀望、小幅貼水；⚠️ 量能未放大（連 3 日 2.8-3.2 萬張）＋韓股 7/24 早盤重挫；✅ 基本面錨未變（2nm Q3 放量、2027 漲價傳聞、全年上修 >40%、CapEx $60-64B）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-07-23 Thu TWSE 官方收盤；美股為 7/23 Thu 收盤、Yahoo Finance API 確認；韓股為 7/23 收盤、7/24 早盤重挫）
    ("設備", "ASML",    "艾司摩爾",    "1,803.00", "+0.06%", "⭐ EUV 設備龍頭；7/23 連 2 日持平整理；TSMC CapEx 上調至 $60-64B 為設備鏈中長線利多"),
    ("設備", "AMAT",    "應用材料",    "562.80", "+1.60%", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作；7/23 收復前日跌幅、設備股逆勢轉強"),
    ("設備", "LRCX",    "科林研發",    "319.78", "+0.15%", "蝕刻設備（7/23 持平微漲）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "218.73", "+1.88%", "量測設備（7/23 逆勢反彈、收復前日跌幅）；2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","161.00","-5.85%", "📉 漲停次日獲利了結回吐（7/23 收 161、TWSE 官方）；與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC 題材未變"),
    ("記憶", "000660.KS","SK 海力士",  "1,919,000 KRW", "+4.86%", "HBM4、與 NVIDIA 多年期合作；7/23 大漲收 191.9 萬韓元（Yahoo API）；⚠️ 7/24 早盤重挫 -5.1% 至 182.2 萬、獲利了結"),
    ("記憶", "MU",      "美光",        "990.21", "+3.20%", "🚀 HBM / DDR5；7/23 逆勢大漲逼近 $1,000——AI 權值賣壓中「記憶體稀缺」成避風港、HBM4 供需緊俏"),
    ("封裝", "3711.TW", "日月光",      "649.00", "-1.07%", "CoWoS / 先進封裝（7/23 連 2 日大漲後小幅回吐收 649、TWSE 官方）"),
    ("封裝", "AMKR",    "Amkor",       "65.33",  "-2.45%", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（7/23 隨板塊回吐）"),
    ("客戶", "NVDA",    "輝達",        "207.81", "-0.46%", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/23 回檔收 207.81、AI 權值連日換手；NVIDIA-TSMC 宣布將 AI 導入晶圓廠製造"),
    ("客戶", "AAPL",    "蘋果",        "321.66", "-1.30%", "⭐ #2 客戶 18%；7/23 續跌整理；跳過 A16 直上 A14；傳為 2027 漲價適用客戶之一"),
    ("客戶", "AVGO",    "博通",        "392.47", "-1.09%", "#3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能（7/23 回落 $400 之下）"),
    ("客戶", "AMD",     "超微",        "539.69", "-2.29%", "#4 客戶；MI350/MI450 獲 OpenAI/Meta 多 GW 承諾、首發 2nm AI 晶片（7/23 回檔失守 $550）"),
    ("客戶", "2454.TW", "聯發科",      "3,875",  "+0.65%", "手機 SoC 客戶 9%（7/23 續漲收 3,875、TWSE 官方、連 3 日上漲）"),
    ("競爭", "INTC",    "英特爾",      "100.23", "-2.33%", "18A 節點量產、吸引微軟/高通；亦為 TSMC 客戶（7/23 連 2 日回吐、退守 $100 關卡）"),
    ("競爭", "005930.KS","三星電子",   "270,000 KRW", "+3.65%", "跟進 TSMC 調漲 4/5nm 代工價約 15%；HBM4E 送樣；7/23 大漲收 27.0 萬韓元（Yahoo API）；⚠️ 7/24 早盤 -4.3% 至 25.85 萬"),
    ("產業", "SOX",     "費城半導體指數","12,343.84", "-0.54%", "7/23 終結連 2 紅收 12,343.84——自 6/22 史高 14,634.66 收斂至 -15.7%；記憶體/設備強 vs 客戶端回吐的再換手格局"),
    ("處分後", "ARM",   "Arm Holdings","283.04", "-0.13%", "TSMC 4/29 全數出脫後（7/23 持平整理、高 Beta 震盪暫歇）"),
]
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/23 收 $415.58 具 +18.0% 上行空間）；⭐ 法說會後新一輪上調：Susquehanna $600、Barclays $650、BofA $590、TD Cowen $440——S&P Global 統計 19 位平均已達 $520.37（TipRanks/GuruFocus 7/17-20）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+17.99%",  # 從 NYSE 7/23 收 $415.58 至平均目標 $490.34（共識強力買進；法說會後多家上調、S&P Global 19 位平均 $520.37 對應約 +25.2%）
}
...
MACRO = [
    ...
    ("NVIDIA NVDA",     "$207.81", "-0.46%",  False),   # #1 TSMC 客戶（22%），7/23 隨 AI 權值回檔
...
NEWS = [
    {
        ...
        "title": "📊 台積電尾盤拉升收 2,405、帶動大盤翻紅連 3 紅收 44,850.81",
        "body": "反彈第四日量縮整理、權值尾盤發力：（1）📊 2330 7/23 平低開 2,385、盤中低 2,370 一度跌破前日 5MA、尾盤買盤進場拉升收 2,405（+5、+0.21%）收在當日最高；（2）⭐ 大盤同步尾盤逆轉翻紅 +25.03 點（+0.06%）收 44,850.81 連 3 紅——台積電尾盤拉升為指數翻紅主力；（3）📊 成交額縮至 9,306 億（前日 1.026 兆）、2330 量縮至 28,001 張——多空收斂、賣壓遞減但攻擊量能同樣缺席；（4）💡 連 2 日收在 2,400-2,405 窄幅、距 20MA 2,415 僅 10 點。",
        "source": "TWSE / Yahoo 股市 2026-07-23", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⚠️ 韓股 7/24 早盤重挫：SK 海力士 -5.1%、三星 -4.3%——今日開盤逆風",
        "body": "亞股半導體單日變臉：（1）📈 韓股 7/23 原大漲——SK 海力士 +4.86% 收 191.9 萬韓元、三星 +3.65% 收 27.0 萬韓元，延續記憶體多頭；（2）⚠️ 7/24 早盤雙雙重挫：SK 海力士 -5.1% 至 182.2 萬、三星 -4.3% 至 25.85 萬——大漲後獲利了結、與美股 AI 權值連 2 日回檔共振；（3）📊 對台股意涵：今日半導體開盤逆風、觀察 2330 能否守住 5MA 2,365／7/23 低 2,370；（4）⚠️ 同步 risk-off 訊號：VIX +12.4% 收 18.70、WTI 近月急漲至 ~$92（地緣溢價）、10Y 美債殖利率 4.70%。",
        "source": "Yahoo Finance API 2026-07-23~24", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📊 美股再換手：AI 權值回檔 NVDA -0.46%、記憶體逆勢強 MU +3.20%",
        "body": "板塊輪動方向與前日相反：（1）⚠️ NYSE TSM 7/23 收 $415.58（-$5.63、-1.34%）、SOX -0.54% 收 12,343.84 終結連 2 紅（自 6/22 史高收斂 -15.7%）；（2）📉 客戶端回吐：NVDA -0.46% 收 $207.81、AMD -2.29% 失守 $550、AAPL -1.30%、AVGO -1.09%、INTC -2.33% 退守 $100；（3）🚀 記憶體/設備逆勢強：MU +3.20% 逼近 $1,000——「AI 賣壓凸顯記憶體稀缺」成避風港敘事；KLAC +1.88%、AMAT +1.60% 收復前日跌幅；（4）📊 台美 ADR 溢價自 +13.6% 收斂至 +11.6%（匯率 ~32.30）——台股相對抗跌所致。",
        "source": "Yahoo Finance API / ts2.tech 2026-07-23", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 籌碼：外資對 2330 連 2 賣 -4,614 張、惟全市場三大法人續買超 185.2 億",
        "body": "兩層分化延續、個股轉觀望：（1）⚠️ 外資對 2330 7/23 賣超 -4,614 張（T86 官方）——連 2 賣且較前日 -3,539 擴大、趨勢反轉確認再延後；（2）📊 投信 -943 張終結連 5 買——最穩定多方也轉觀望；自營 +38 張、三大法人合計 -5,519 張連 2 日賣超；（3）⭐ 惟全市場 BFI82U 官方：三大法人買超 185.2 億——外資 +69.6 億連 2 買、投信 +73.7 億、自營 +41.9 億——資金留在台股、只是不在權值；（4）🎯 觀察外資對 2330 賣壓何時收斂、與投信是否回補。",
        "source": "TWSE T86 / BFI82U 官方 2026-07-23", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "💰 2nm Q3 放量＋NVIDIA 宣布與 TSMC 將 AI 導入晶圓廠製造",
        "body": "基本面敘事鏈持續增厚：（1）💰 CFO 黃仁昭 7/22 定調 2nm 已於 Q2 貢獻營收、Q3 起放大成最新成長引擎——首發客戶含 Apple A20、AMD 2nm AI 晶片、產能滿載至年底；（2）🤝 NVIDIA 官方宣布 TSMC 正採用 NVIDIA 加速運算與 AI 推進晶片設計與製造——AI 導入晶圓廠（fab digital twin/lithography 加速）深化雙方綁定；（3）⭐ 與 2027 年全面漲價 5-10%（HPC 追加訂單另加價 10-15%）傳聞形成「量價齊揚」；（4）✅ 全年營收成長上修 >40%、CapEx $60-64B、Arizona 累計投資 $265B。",
        "source": "NVIDIA Newsroom / 經濟日報 2026-07-22~23", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 反彈續航確認清單：量能連 3 日未放大、韓股重挫、VIX/油價齊升",
        "body": "整理期的風險盤點：（1）⚠️ 量能連 3 日 2.8-3.2 萬張（7/17 爆量 97,362 的三成）——續攻需放大至 4 萬張以上；惟成交筆數 142,112 續增、小單換手活絡為正向細節；（2）⚠️ 韓股 7/24 早盤重挫（SK -5.1%／三星 -4.3%）＋美股 AI 權值連 2 日回檔——外部逆風升溫；（3）📊 總經指標轉向：VIX +12.4% 回升 18.70、WTI 近月急漲 ~$92、10Y 美債 4.70%——risk-off 訊號齊聚；（4）💱 背景風險：美 232 條款半導體關稅 25% + 輸中晶片轉運查驗、台幣 ~32.30、20MA 2,415 尚未收復。",
        "source": "TWSE / Yahoo Finance API 2026-07-23~24", "impact": "中性",
    },
]

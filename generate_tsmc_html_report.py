...
STOCK = {
    # 台股 2330
    "tw_price":       "2,415",        # 台股 2330 最新確認收盤：8/12 Wed 官方收 NT$2,415（+20、+0.84% vs 8/11 NT$2,395、開 2,405／高 2,415／低 2,390、TWSE 確認）——⭐ 收在當日最高、一舉突破 2,400 整數與 2,405／2,410 壓力帶，為 7/31（收 2,425）以來收盤新高；大盤同日 +397.35 點（+0.88%）收 45,518.07 續創 8 月反彈新高
    "tw_change":      "+20",          # 8/12 Wed 收盤漲跌點數（+0.84%、TWSE 官方）
    "tw_change_pct":  "+0.84%",       # 8/12 Wed 攻堅收最高：開 2,405 盤中低 2,390 高 2,415、尾盤收在最高 2,415——連 3 日收高（8/10 +0.42%、8/11 +0.63%）、站上全部均線；⭐ 量能終結連 5 縮（19,448 張、+6.6% vs 8/11）惟仍僅 5 日均量 21,829 的 89%；7 月營收 4,675.81 億＋董事會配息 7 元／Sony 合資利多延續發酵
    "tw_volume":      "19,448",       # 8/12 成交量（張、TWSE 口徑；成交額 NT$467.98 億、成交 63,548 筆——為 5 日均量 21,829 張的 89%、較 8/11 的 18,248 張回升 +6.6% 終結連 5 日萎縮，惟量能仍未明顯放大）
    "tw_open":        "2,405",        # 8/12 開盤（TWSE、開高並全日守穩 2,400 之上）
    "tw_high":        "2,415",        # 8/12 盤中高（TWSE、與收盤同價——收在最高、強勢作收）
    "tw_low":         "2,390",        # 8/12 盤中低（TWSE、8/11 收盤價轉為當日支撐、未破 2,390）
    "market_cap":     "NT$62.6 兆",   # 8/12 收盤市值（NT$2,415 基礎、約 259.3 億股）
    "pe_ratio":       "32.47x",       # 台股 P/E（TWSE 官方 BWIBBU 8/12、口徑為最新年度 EPS）
    "pb_ratio":       "10.63x",       # 台股 P/B（TWSE 官方 BWIBBU 8/12、以 115Q1 財報淨值計）
    "div_yield":      "0.91%",        # 殖利率（TWSE 官方 BWIBBU 8/12、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "429.15",       # 8/12 Wed 官方收盤 $429.15（+$7.09、+1.68% vs 8/11 $422.06、Yahoo API 確認）——連 3 日創本波收盤新高；美國 7 月 CPI 3.4%／核心 2.5% 符合預期、緊縮預期降溫帶動 SOX +2.49% 大漲，TSM 明顯強於大盤（S&P 同日 +0.26%）
    "nyse_change":    "+7.09",        # 8/12 漲跌（美元、自我檢核：422.06 × 1.0168 = 429.15 ✓）
    "nyse_change_pct":"+1.68%",       # 8/12 CPI 符合預期後半導體全面走強（SOX +2.49%、NVDA +3.03% 創收盤新高）、TSM 創本波收盤新高
    ...
    "data_note":      '報告日期：2026-08-13 Thu（最新確認收盤：台股 2330 8/12 Wed 官方收 NT$2,415（+20、+0.84% vs 8/11 NT$2,395、開 2,405／高 2,415／低 2,390、量 19,448 張 TWSE 口徑、成交額 NT$467.98 億、成交 63,548 筆、TWSE 確認）；NYSE TSM 8/12 Wed 收 $429.15（+$7.09、+1.68% vs 8/11 $422.06、Yahoo API 確認、連 3 日創本波收盤新高））。📊 今日格局：突破 2,400 收最高、MACD DIF 上穿零軸、今晨夜盤大漲——（1）⭐ 台股攻堅收在最高：2330 8/12 收 2,415（+0.84%）一舉突破 2,400 整數與 2,405／2,410 短線壓力帶，收盤價與當日最高同價（強勢作收），為 7/31（收 2,425）以來收盤新高；量能終結連 5 日萎縮（19,448 張、+6.6% vs 8/11 的 18,248 張）惟仍僅 5 日均量 21,829 的 89%——量價配合改善但未到放量突破；大盤 +397.35 點（+0.88%）收 45,518.07 續創 8 月反彈新高；（2）⭐⭐ 技術面關鍵確認成立：MACD DIF 上穿零軸至 +2.78（8/11 為 -1.36）——此為前一日日報標記的「中期修復關鍵確認訊號」、本日正式成立；紅柱 +16.57 連 6 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43→+16.57）、RSI 54.0 中軸上方續升；⚠️ 惟 KDJ J 值 99.78 逼近 100、短線過熱訊號明顯；（3）⭐ 美國 7 月 CPI 符合預期、緊縮預期降溫：CPI 月增 0.1%／年增 3.4%（前值 3.5%）、核心 CPI 月增 0.2%／年增 2.5%（前值 2.6%）均符合市場預期（BLS 官方、CNBC 8/12）；CME FedWatch 顯示 9 月升息機率自 45% 降至 42%——風險偏好回升帶動 SOX 8/12 大漲 +2.49% 收 12,399.38、NVDA +3.03% 收 $224.09 創收盤新高、TSM +1.68% 收 $429.15；設備與記憶體股全面走強（MU +4.92%、LRCX +4.72%、AMAT +4.29%、KLAC +3.88%、INTC +3.32% 收復 $100）；（4）⭐⭐ 今晨夜盤大漲、開高機率極高：台指期（TX 202608）夜盤（8/12 晚→今晨）收 46,117（+601 點、+1.32%、量 35,208 口、TAIFEX 官方）對現貨 45,518.07 呈正價差 +599；台積電個股期貨（CDF 202608）夜盤收 2,439（+26、+1.08%、量 1,790 口）對現貨 2,415 呈正價差 +24——外部條件全面偏多；（5）✅ 籌碼連 4 買：2330 8/12 外資 +2,627 張（連 4 買：+642→+119→+3,498→+2,627）、投信 +587 張（連 4 買、且逐日加碼）、自營 -290（自行 -235／避險 -55）、三大合計 +2,924 張（T86 官方）；全市場 BFI82U 連 3 日買超（8/10 +736.1 億、8/11 +280.1 億、8/12 +220.0 億，8/12 外資 +110.2 億）；（6）⭐ 記憶體行情全面爆發：韓股 8/12 三星 +6.68% 收 25.55 萬韓元、SK 海力士 +5.54% 收 150.4 萬；今晨 8/13 續強——三星 +3.72% 至 26.5 萬、SK +7.11% 至 161.1 萬、KOSPI 6,826.14（盤中快照）；⚠️ 惟台股記憶體與封裝 8/12 反而回檔（華邦電 -0.56% 收 177.0、日月光 -1.27% 收 621、聯發科 -0.12% 收 4,015）——資金自中小型輪回權值台積電、與 8/10 的分流方向相反。📊 ADR 溢價自 +13.56% 擴大至 +14.48%（$429.15 vs 理論值 $374.88、匯率 32.21）——ADR 漲幅（+1.68%）大於台股（+0.84%）使溢價再度擴大、台股續有補漲空間。技術面（官方收盤計算至 8/12）：收 2,415 站上全部均線（5MA 2,385／10MA 2,365／20MA 2,356／60MA 2,364／120MA 2,183）；支撐 2,405（8/12 開盤）／2,400（整數）／2,390（8/12 低）／2,385（5MA）／2,365（10MA）／2,364（60MA）、壓力 2,425（7/31 收）／2,440（今晨夜盤區）／2,449（今晨夜盤高）／2,493（布林上軌）／2,535（史高）。✅ 基本面錨未變：7 月營收 NT$4,675.81 億（+44.69% YoY）連 3 月創高、1-7 月累計 2.87 兆（+37.0%）；Q2 大超預期（營收 $40.20B、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20%；8/11 董事會 Q2 配息 7 元（12/10 除息）＋資本預算 US$294.4 億＋Sony 熊本合資。⭐ 今日（8/13 Thu）觀察：（1）夜盤正價差 +599／+24 指向開高——能否放量站穩 2,415 之上並挑戰 2,425（7/31 收）為第一檢核點；（2）⚠️ KDJ J 99.78 過熱＋量能僅 89% 均量——開高後的量能與追價意願是關鍵，無量衝高提防當日沖高回落；（3）外資連 5 買與記憶體行情外溢效應；（4）232 條款半導體專章仍未公布（未能證實）。'  # noqa: E501
}
...
SUMMARY = {
    ...
    "headline":   '⭐⭐ 突破 2,400 收最高、MACD DIF 上穿零軸：2330 8/12 收 NT$2,415（+20、+0.84%）一舉突破 2,400 整數與 2,405／2,410 壓力帶、收在當日最高，為 7/31 以來收盤新高；⭐ 技術面關鍵確認成立——MACD DIF 自 -1.36 上穿零軸至 +2.78、紅柱 +16.57 連 6 日擴張；美國 7 月 CPI 3.4%／核心 2.5% 符合預期、緊縮預期降溫帶動 SOX +2.49%、NVDA +3.03% 創收盤新高、TSM +1.68% 收 $429.15 連 3 日創本波新高；⭐ 今晨夜盤大漲（台指期 +601 點正價差 +599、台積電個股期貨收 2,439 正價差 +24）＋外資連 4 買 +2,627 張＋韓股記憶體爆發（SK 今晨 +7.11%）——⚠️ 惟量能僅 89% 均量、KDJ J 99.78 逼近超買',

    "bullets": [
        '⭐⭐ 突破 2,400 整數關卡、收在當日最高：2330 8/12 官方收 NT$2,415（+20、+0.84%、開 2,405／高 2,415／低 2,390）——一舉突破 2,400 整數與 2,405（8/11 高）／2,410（8/10 高）三道短線壓力，收盤價與當日最高同價為強勢作收型態，創 7/31（收 2,425）以來收盤新高；連 3 日收高、站上全部均線；大盤同日 +397.35 點（+0.88%）收 45,518.07 續創 8 月反彈新高',
        '⭐⭐ 技術面關鍵確認成立——MACD DIF 上穿零軸：DIF 自 8/11 的 -1.36 上穿至 +2.78（前一日日報即標記「DIF 上穿零軸將為中期修復的關鍵確認訊號」、本日正式成立）；紅柱 +16.57 連 6 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43→+16.57）、DEA -5.51；RSI 54.0 中軸上方續升——中期動能修復由「金叉」升級為「零軸之上」的結構性確認',
        '⭐ 美國 7 月 CPI 符合預期、緊縮預期降溫：CPI 月增 0.1%／年增 3.4%（前值 3.5%）、核心 CPI 月增 0.2%／年增 2.5%（前值 2.6%）均符合市場預期（BLS 官方、CNBC 8/12）；居住項貢獻約三分之二漲幅、能源月減 1.5%（惟年增 14.7%）；CME FedWatch 顯示 9 月升息機率自 45% 降至 42%——風險偏好回升為當日半導體全面走強的主因',
        '⭐ 美股半導體全面大漲、TSM 連 3 日創本波新高：SOX 8/12 +2.49% 收 12,399.38（自 6/22 史高 14,634.66 收斂至 -15.3%）；TSM +1.68% 收 $429.15（連 3 日創本波收盤新高、明顯強於 S&P +0.26%）；NVDA +3.03% 收 $224.09 創收盤新高（超越 8/7 的 $223.96）；設備與記憶體領漲——MU +4.92%、LRCX +4.72%、AMAT +4.29%、KLAC +3.88%、INTC +3.32% 收復 $100；AI 概念股財報佳（CoreWeave +20%、Super Micro +15%、Nebius +12.5%）；VIX 降至 14.55',
        '⭐⭐ 今晨夜盤大漲、開高機率極高：台指期（TX 202608）夜盤（8/12 晚→今晨）收 46,117（+601 點、+1.32%、量 35,208 口、TAIFEX 官方）對現貨 45,518.07 呈正價差 +599；台積電個股期貨（CDF 202608）夜盤收 2,439（+26、+1.08%、高 2,449、量 1,790 口）對現貨 2,415 呈正價差 +24——外部條件（CPI＋美股半導體＋韓股記憶體）全面偏多',
        '✅ 籌碼連 4 買、投信逐日加碼：2330 8/12 外資 +2,627 張（連 4 買：+642→+119→+3,498→+2,627）、投信 +587 張（連 4 買且逐日加碼：+61→+153→+382→+587）、自營 -290（自行 -235／避險 -55）、三大合計 +2,924 張（T86 官方）；全市場 BFI82U 連 3 日買超（8/10 +736.1 億→8/11 +280.1 億→8/12 +220.0 億、8/12 外資 +110.2 億）——資金回流台股趨勢延續',
        '⭐ 韓股記憶體行情全面爆發：8/12 三星 +6.68% 收 25.55 萬韓元、SK 海力士 +5.54% 收 150.4 萬；今晨 8/13 續強——三星 +3.72% 至 26.5 萬、SK +7.11% 至 161.1 萬、KOSPI 6,826.14（盤中快照）；⚠️ 惟台股記憶體與封裝 8/12 反向回檔（華邦電 -0.56% 收 177.0、日月光 -1.27% 收 621、聯發科 -0.12% 收 4,015）——資金自中小型輪回權值台積電，與 8/10 的分流方向相反',
        '⚠️ 兩項保留：（1）量能仍偏低——8/12 成交 19,448 張雖終結連 5 日萎縮（+6.6% vs 8/11）但僅為 5 日均量 21,829 張的 89%，突破 2,400 屬「賣壓稀薄」而非「買盤湧入」，放量確認仍待觀察；（2）KDJ J 值 99.78 逼近 100（K 83.31／D 75.08）、短線過熱訊號明顯——開高後易出現沖高回落',
        '📅 事件行事曆：Q2 股利除息 12/10（配息 NT$7.0/股、首推外資美元領息）；Q3 法說會 10 月中旬；232 條款半導體專章結果仍未公布（未能證實）；2nm Q3 放量＋報價漲 10-20%（TrendForce）；CoWoS 2026 年底月產能目標上修至 10-13 萬片（法人估、非官方公告）',
    ],

    "bottom_line": '報告日期：2026-08-13 Thu（最新確認收盤：台股 2330 8/12 Wed 官方收 NT$2,415（+20、+0.84% vs 8/11 NT$2,395、開 2,405／高 2,415／低 2,390、量 19,448 張、額 NT$467.98 億、63,548 筆、TWSE 確認）；NYSE TSM 8/12 收 $429.15（+1.68%、連 3 日創本波收盤新高、Yahoo API 確認））。📊 核心判讀：突破 2,400 收最高、技術面關鍵確認成立、外部條件全面偏多，惟量能與過熱為兩大保留——（1）⭐⭐ 價格結構升級：2,415 一舉突破 2,400 整數與 2,405／2,410 壓力帶且收在當日最高（收盤＝最高的強勢型態），為 7/31 以來收盤新高；連 3 日收高、站上全部均線（5MA 2,385／10MA 2,365／20MA 2,356／60MA 2,364／120MA 2,183）——短線均線多頭排列雛形浮現；（2）⭐⭐ MACD DIF 上穿零軸至 +2.78（8/11 為 -1.36）：前一日日報標記的「中期修復關鍵確認訊號」正式成立，紅柱 +16.57 連 6 日擴張——動能修復由短線金叉升級為零軸之上的結構性確認，此為本波反彈以來最強的技術訊號；（3）⭐ 美國 7 月 CPI 符合預期（年增 3.4%／核心 2.5%、均低於前值）：CME FedWatch 9 月升息機率自 45% 降至 42%、緊縮預期降溫——SOX +2.49% 收 12,399.38、NVDA +3.03% 創收盤新高、TSM +1.68% 收 $429.15、VIX 降至 14.55；設備與記憶體全面領漲（MU +4.92%、LRCX +4.72%、AMAT +4.29%、KLAC +3.88%）；（4）⭐⭐ 今晨夜盤指向開高：台指期夜盤 +601 點收 46,117（正價差 +599）、台積電個股期貨夜盤收 2,439（+26、正價差 +24）——若日盤依夜盤開出，2,435-2,440 一帶將直接測試 2,425（7/31 收）之上的空手區；（5）✅ 籌碼連 4 買：外資 +2,627 張（連 4 買）、投信 +587 張（連 4 買且逐日加碼）、三大合計 +2,924 張；全市場 BFI82U 連 3 日買超（8/12 +220.0 億、外資 +110.2 億）；（6）⭐ 韓股記憶體爆發（8/12 三星 +6.68%、SK +5.54%；今晨續漲 +3.72%／+7.11%、KOSPI 6,826）——⚠️ 惟台股記憶體／封裝 8/12 反向回檔（華邦電 -0.56%、日月光 -1.27%），資金自中小型輪回權值台積電。💡 技術面：RSI 54.0、MACD DIF +2.78 上穿零軸／DEA -5.51／柱 +16.57 連 6 日擴張、KDJ K 83.31／D 75.08／J 99.78（逼近 100、過熱）；支撐 2,405（8/12 開盤）／2,400（整數、突破後轉支撐）／2,390（8/12 低）／2,385（5MA）／2,365（10MA）／2,364（60MA）／2,356（20MA）、壓力 2,425（7/31 收）／2,440（今晨夜盤區）／2,449（今晨夜盤高）／2,493（布林上軌）／2,535（史高）。📊 ADR 溢價自 +13.56% 擴大至 +14.48%（$429.15 vs 理論值 $374.88、匯率 32.21）——ADR 漲幅大於台股使溢價再度擴大、台股續有補漲空間。✅ 基本面錨未變：7 月營收 NT$4,675.81 億（+44.69% YoY）連 3 月創高、1-7 月累計 2.87 兆（+37.0%）；Q2 大超預期（營收 $40.20B、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20%、CoWoS 訂滿至 2027。⭐ 今日（8/13 Thu）觀察：（1）夜盤正價差 +24 指向開高——能否放量站穩 2,415 之上並挑戰 2,425 為第一檢核點；（2）⚠️ 量能能否突破 21,829 張（5 日均量）——8/12 僅 89% 均量，無量衝高提防沖高回落；（3）KDJ J 99.78 過熱的消化方式（橫盤 vs 回檔）；（4）外資連 5 買與投信加碼延續性；（5）記憶體行情外溢是否再度分流權值股資金。⚠️ 風險：短線過熱（J 值逼近 100）、量能未放大、CPI 利多已反映後的獲利了結、232 條款懸而未決、AI 資本支出持續性雜訊。',

    "risk_alert":  '今日（8/13 Thu）關鍵觀察：（1）⭐⭐ 開高後的攻防與量能是第一檢核點：今晨夜盤全面大漲（台指期 +601 點收 46,117、正價差 +599；台積電個股期貨收 2,439、+26、正價差 +24），加上 CPI 符合預期＋SOX +2.49%＋NVDA 創收盤新高＋韓股記憶體今晨續飆（SK +7.11%），開高機率極高；⚠️ 但 8/12 成交僅 19,448 張、為 5 日均量 21,829 張的 89%——突破 2,400 屬「賣壓稀薄」而非「買盤湧入」，若開高後量能未同步放大，慎防沖高回落；（2）⚠️ 短線過熱訊號明確：KDJ J 值 99.78 逼近 100（K 83.31／D 75.08）、RSI 54.0 雖未超買但 J 值已在極端區——過熱需以橫盤或小幅回檔消化，追高風險升高；（3）⭐⭐ 技術面正面確認：MACD DIF 上穿零軸至 +2.78（前一日 -1.36）、紅柱 +16.57 連 6 日擴張——中期修復結構性確認成立，回檔的性質由「反彈結束」轉為「上升途中的整理」，2,385（5MA）／2,364（60MA）為關鍵防守；（4）✅ 籌碼延續性：外資連 4 買（+642→+119→+3,498→+2,627）、投信連 4 買且逐日加碼（+61→+153→+382→+587）——連 5 買將進一步強化中期翻多判斷；全市場外資 3 日累計 +850.9 億；（5）⚠️ 資金輪動風險：8/12 台股記憶體與封裝反向回檔（華邦電 -0.56%、日月光 -1.27%、聯發科 -0.12%）而 2330 上漲，顯示資金回流權值；但韓股記憶體今晨再爆發（三星 +3.72%、SK +7.11%），留意資金再度分流至記憶體族群的可能；（6）💱 匯率 32.21（今晨 32.19 微升）；ADR 溢價擴大至 +14.48%——台股補漲空間仍在；（7）📅 尾部風險：232 條款半導體專章結果仍未公布（未能證實）；CPI 利多落地後的「賣事實」風險；AI 資本支出持續性雜訊反覆；SOX 自 6/22 史高仍 -15.3%、2330 自 6/23 史高 2,535 仍 -4.7%——中期反彈的完整確認仍需量能配合。',
}
...
INSTITUTIONAL = {
    ...
    "trust":    587,
    "dealer":   -290,
    # 近 5 交易日（8/6, 8/7, 8/10, 8/11, 8/12）
    "foreign_5d": [-4011, 642, 119, 3498, 2627],
    "trust_5d":   [-82, 61, 153, 382, 587],
    "dealer_5d":  [-583, 1243, 448, -189, -290],
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       54.0,   "rsi_signal": 'RSI(14) 54.0（官方收盤計算）——連 3 日上漲後自 52.5 續升至 54.0、穩居中軸之上——中性偏多；距 70 超買區仍有空間，RSI 本身尚未過熱（惟 KDJ J 值已達極端區）',
    "macd":         16.57,   "macd_signal": '⭐⭐ MACD DIF 上穿零軸（DIF +2.78、DEA -5.51、柱狀圖 +16.57、官方收盤計算）——前一日日報標記的「中期修復關鍵確認訊號」正式成立：DIF 自 8/11 的 -1.36 上穿至 +2.78，動能修復由短線金叉升級為零軸之上的結構性確認；紅柱連 6 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43→+16.57）——本波反彈以來最強的技術訊號',   "macd_color": "#4CAF50",
    "kdj_k":        83.3,   "kdj_d": 75.1, "kdj_j": 99.8, "kdj_signal": '⚠️ KDJ K(83.3)/D(75.1)/J(99.8)（官方收盤計算）——金叉延續且 K、D 同步走高，惟 J 值自 96.0 續升至 99.8 逼近 100 極端區、短線過熱訊號明確；歷史上 J 值觸及 100 後多以橫盤或小幅回檔消化，開高追價的風險報酬比轉差',
    "ma5":       "2,385",
    "ma10":      "2,365",
    "ma20":      "2,356",
    "ma60":      "2,364",
    ...
    "bb_lower":  "2,220",
    "vol_ma5":  "21,829",
    "trend":    '⭐⭐ 突破 2,400 收在最高、MACD DIF 上穿零軸：2330 8/12 官方收 NT$2,415（+20、+0.84%、開 2,405／高 2,415／低 2,390、量 19,448 張為 5 日均量 21,829 張的 89%）——一舉突破 2,400 整數與 2,405（8/11 高）／2,410（8/10 高）三道短線壓力，收盤價與當日最高同價為強勢作收型態、創 7/31（收 2,425）以來收盤新高；連 3 日收高、站上全部均線（5MA 2,385／10MA 2,365／20MA 2,356／60MA 2,364／120MA 2,183），短線均線多頭排列雛形浮現；大盤 +397.35 點（+0.88%）收 45,518.07 續創 8 月反彈新高。⭐⭐ 技術指標關鍵確認（官方收盤計算）：MACD DIF 自 -1.36 上穿零軸至 +2.78、DEA -5.51、紅柱 +16.57 連 6 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43→+16.57）——中期動能修復的結構性確認成立；RSI 54.0 中軸上方續升；⚠️ 惟 KDJ J 99.8 逼近 100（K 83.3／D 75.1）、短線過熱。✅ 籌碼連 4 買：外資 +2,627 張（連 4 買：+642→+119→+3,498→+2,627）、投信 +587 張（連 4 買且逐日加碼）、自營 -290、三大合計 +2,924 張；全市場 BFI82U 連 3 日買超（8/12 +220.0 億、外資 +110.2 億）。📊 外部條件全面偏多：美國 7 月 CPI 3.4%／核心 2.5% 符合預期、緊縮預期降溫——SOX +2.49% 收 12,399.38、NVDA +3.03% 創收盤新高、TSM +1.68% 收 $429.15 連 3 日創本波新高、VIX 14.55；⭐ 今晨夜盤大漲：台指期收 46,117（+601、正價差 +599）、台積電個股期貨收 2,439（+26、正價差 +24）；韓股記憶體爆發（8/12 三星 +6.68%、SK +5.54%；今晨 +3.72%／+7.11%）。下一觀察：（1）開高後能否放量站穩 2,415 之上並挑戰 2,425（7/31 收）／2,449（今晨夜盤高）；（2）⚠️ 量能能否突破 21,829 張——無量衝高提防沖高回落；（3）J 值 99.8 過熱的消化方式；（4）回檔時 2,405／2,400／2,385（5MA）為關鍵防守。',
    "support":  "2,405 / 2,400 / 2,390 / 2,385 / 2,365 / 2,364 (8/12 開盤、整數關卡突破後轉支撐、8/12 低、5MA、10MA、60MA 季線)",
    "resist":   "2,425 / 2,440 / 2,449 / 2,493 / 2,535 (7/31 收盤、今晨夜盤區、今晨夜盤高、布林上軌、6/23 史高)",
    "note": '均線與 RSI/MACD/KDJ/布林均依 TWSE 官方收盤價序列（2026-01-02~2026-08-12 共 146 個交易日）計算，請以看盤軟體為準；8/12 收 NT$2,415（+0.84%）突破 2,400 收在最高、站上全部均線（5MA 2,385／10MA 2,365／20MA 2,356／60MA 2,364／120MA 2,183）；⭐⭐ MACD DIF 上穿零軸至 +2.78（8/11 為 -1.36）、DEA -5.51、柱 +16.57 連 6 日擴張——前一日標記的中期修復關鍵確認訊號成立；RSI 54.0 續升、KDJ 金叉延續（K83.3/D75.1/J99.8 逼近超買）；支撐 2,405 / 2,400 / 2,390 / 2,385 / 2,365 / 2,364、壓力 2,425 / 2,440 / 2,449 / 2,493 / 2,535；⭐ 突破 2,400＋DIF 上穿零軸＋外資連 4 買＋今晨夜盤正價差 +24＋CPI 符合預期 vs ⚠️ 量能僅 89% 均量（19,448 張）＋KDJ J 99.8 過熱——今日檢核點為「開高後量能能否放大並站穩 2,415」',
}
...
DERIVATIVES = {
    "date":          "2026-08-12",
    "futures_price": "2,414",      # 台積電股票期貨 CDF 202608 8/12 日盤收盤（TAIFEX 官方、+17、+0.71%、開 2,404／高 2,415／低 2,388、結算價 2,413、量 6,014 口）
    "futures_oi":    "14,125",     # CDF 202608 未沖銷契約數（口、TAIFEX 官方 8/12；較 8/11 的 15,602 口減少 1,477）
    "futures_basis": "-1",         # 日盤期貨 2,414 - 現貨 2,415（微幅逆價差）；⭐ 夜盤（8/12 晚→今晨 8/13）收 2,439（+26、+1.08%）正價差大幅擴至 +24
    "call_oi":       "57,021",     # 台指選擇權 TXO 買權未平倉量（TAIFEX 官方 8/12；⚠️ 8/12 為週三週選結算日、未平倉大幅結清重建，與 8/11 的 83,111 口不可直接比較）
    "put_oi":        "60,479",     # 台指選擇權 TXO 賣權未平倉量（TAIFEX 官方 8/12；⚠️ 同上、8/11 為 102,705 口）
    "pcr":           "1.06",       # Put/Call 未平倉量比率 106.06%（TAIFEX 官方 8/12）——⚠️ 自 8/11 的 123.58% 回落，惟主因為週選結算日的未平倉重建、非市場情緒逆轉，宜待 8/13 起的新周期數據再判讀
    "pcr_signal":    '中性偏多（Put/Call 未平倉比 1.06、TAIFEX 官方 8/12）——⚠️ 重要提醒：8/12 為週三週選結算日，買權未平倉自 83,111 口降至 57,021 口、賣權自 102,705 口降至 60,479 口，屬結算後的部位重建而非情緒逆轉；比率自 123.58% 回落至 106.06% 不宜直接解讀為看多轉弱，應待新周期（8/13 起）數據累積後再判讀。⭐⭐ 今晨夜盤（8/12 晚→今晨 8/13）全面大漲、指向開高：台指期（TX 202608）夜盤收 46,117（+601 點、+1.32%、開 45,600／高 46,312／低 45,590、量 35,208 口）對現貨 45,518.07 呈正價差 +599；台積電個股期貨（CDF 202608）夜盤收 2,439（+26、+1.08%、高 2,449、量 1,790 口）對現貨 2,415 呈正價差 +24——美國 7 月 CPI 符合預期＋SOX +2.49%＋NVDA 創收盤新高＋韓股記憶體今晨續飆共同推升。📊 三大法人台指期未平倉淨額（8/12 TAIFEX 官方）：外資淨空 -86,633 口、投信淨多 +82,383 口、自營淨多 +1,239 口——多空大致對沖、屬結構性避險部位（投信為 ETF 避險、外資為現貨多單對沖），不宜單獨解讀為方向性看法',
    "max_pain":      "2,400",
    "key_strikes": [
        ("2,356", "Call 38,000 / Put 12,000", "20MA、布林中軌"),
        ("2,364", "Call 37,000 / Put 12,000", "60MA 季線"),
        ("2,365", "Call 37,000 / Put 12,000", "10MA"),
        ("2,385", "Call 41,000 / Put 11,000", "5MA"),
        ("2,390", "Call 43,000 / Put 13,000", "8/12 盤中低"),
        ("2,400", "Call 45,000 / Put 15,000", "整數關卡、突破後轉支撐、估最大痛苦"),
        ("2,405", "Call 42,000 / Put 11,000", "8/12 開盤"),
        ("2,415", "Call 40,000 / Put 9,000",  "8/12 收盤＝當日最高"),
        ("2,425", "Call 39,000 / Put 8,000",  "7/31 收盤、上檔第一壓力"),
        ("2,439", "Call 35,000 / Put 7,000",  "今晨夜盤收盤"),
        ("2,449", "Call 33,000 / Put 6,000",  "今晨夜盤最高"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": '⭐ 期貨與 Put/Call 未平倉數據均為 TAIFEX 官方（2026-08-12、來源 futDataDown／pcRatioDown）：台積電股票期貨（CDF 202608）日盤收 2,414（+17、+0.71%、開 2,404／高 2,415／低 2,388、量 6,014 口、結算價 2,413、未沖銷 14,125 口）對現貨 2,415 呈微幅逆價差 -1；⭐⭐ 夜盤（8/12 晚→今晨 8/13）收 2,439（+26、+1.08%、開 2,430／高 2,449／低 2,430、量 1,790 口）正價差大幅擴至 +24。台指期（TX 202608）日盤收 45,528（+443、+0.98%、量 48,409 口、結算價 45,516、未沖銷 95,311 口）；夜盤收 46,117（+601、+1.32%、開 45,600／高 46,312／低 45,590、量 35,208 口）對現貨 45,518.07 正價差 +599。台指選擇權未平倉：買權 57,021 口／賣權 60,479 口、Put/Call 比 1.06——⚠️ 8/12 為週三週選結算日、未平倉大幅結清重建（8/11 為 83,111／102,705、比率 1.24），比率回落屬結算技術性因素而非情緒逆轉，請勿與 8/11 直接比較。⚠️ key_strikes 各履約價的 Call/Put 口數仍為估算（台積電個股選擇權流動性低、TAIFEX 未逐檔提供彙總），僅作支撐壓力參考、請以 taifex.com.tw 為準；價位錨點按 TWSE 官方收盤（8/12 收 NT$2,415）。⭐ 突破 2,400 收在最高＋MACD DIF 上穿零軸（+2.78）＋紅柱連 6 日擴張（+16.57）＋外資連 4 買 +2,627 張＋今晨夜盤正價差 +24＋美國 7 月 CPI 符合預期（3.4%／核心 2.5%） vs ⚠️ 量能僅 5 日均量的 89%（19,448 張）＋KDJ J 99.8 逼近超買；✅ 基本面錨強化（7 月營收 4,675.81 億創高、2nm Q3 放量＋報價漲 10-20%、CoWoS 訂滿至 2027、全年上修 >40%）',
}
# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # (分類, 代號, 名稱, 最新價, 漲跌幅, 說明) — 美股為 8/12 Wed 收盤（Yahoo API）、台股為 8/12 TWSE 官方收盤、韓股為 8/12 收盤＋8/13 晨盤快照
    ("設備", "ASML",    "艾司摩爾",    "1,810.07", "+0.59%", "⭐⭐ EUV 設備龍頭；8/12 續漲 +0.59% 收 $1,810.07 再創歷史新高、站上 $1,800（連 2 日創高）；TSMC CapEx $60-64B 與 2nm/A16 擴產為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "548.15", "+4.29%", "⭐⭐ 8/12 大漲 +4.29% 收 $548.15（CPI 符合預期＋晶圓廠設備支出上調）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "326.11", "+4.72%", "⭐⭐ 8/12 領漲設備股 +4.72% 收 $326.11（受惠 NAND 與 HBM 需求擴大、WFE 支出上修）；TSMC CapEx 為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "208.25", "+3.88%", "⭐⭐ 8/12 續漲 +3.88% 收 $208.25（連 2 日大漲、穩站 $200 之上）；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","177.00","-0.56%", "⚠️ 8/12 連 2 日小幅回檔 -0.56% 收 177.0（TWSE 官方）——8/10 暴漲 +9.79% 後高檔整理；⚠️ 與韓股記憶體 8/12 大漲（三星 +6.68%、SK +5.54%）背離，資金短線輪回權值台積電；與 TSMC 合作 CUBE 特殊 DRAM 題材延續"),
    ("記憶", "000660.KS","SK 海力士",  "1,504,000 KRW", "+5.54%", "⭐⭐ 8/12 暴漲 +5.54% 收 150.4 萬韓元（自 8/11 的 142.5 萬跳升）；⭐⭐ 今晨 8/13 續飆 +7.11% 至 161.1 萬——記憶體行情全面爆發；高盛估 2027 HBM 均價年增約 100%、三大 DRAM 廠逾九成產能轉向 HBM"),
    ("記憶", "MU",      "美光",        "911.29", "+4.92%", "⭐⭐ 8/12 大漲 +4.92% 收 $911.29 收復 $900（HBM/DDR5 需求延續、DRAM 產能排擠效應推升報價）；自 7/23 高 $990 收斂至 -8.0%"),
    ("封裝", "3711.TW", "日月光",      "621.00", "-1.27%", "⚠️ 8/12 連 3 日回檔 -1.27% 收 621（TWSE 官方）——8/10 暴漲 +7.69% 收 630 創波段新高後續整理；⭐ 傳台積電擬購友達中科兩座舊面板廠擴充 CoWoS 產能（未經官方證實）為中長線題材"),
    ("封裝", "AMKR",    "Amkor",       "55.59",  "+2.38%", "✅ 8/12 續漲 +2.38% 收 $55.59（連 2 日上漲、創本波新高）；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材中長線未變"),
    ("客戶", "NVDA",    "輝達",        "224.09", "+3.03%", "⭐⭐ TSMC #1 客戶（約 19%）、A16 首發客戶；8/12 大漲 +3.03% 收 $224.09 創收盤新高（超越 8/7 的 $223.96）——CPI 符合預期後 AI 概念股全面反攻（CoreWeave +20%、Super Micro +15%、Nebius +12.5%）；先前對 $5,000 億 AI 基建融資平台的「循環融資」疑慮暫獲消化"),
    ("客戶", "AAPL",    "蘋果",        "302.25", "-0.87%", "⚠️ 主要客戶（比重約 17%）；8/12 連 3 跌 -0.87% 收 $302.25、自 8/7 本波高 $313.33 回落 -3.5%——與半導體板塊大漲背離；⚠️ 市場傳 2nm iPhone 18 Pro 晶片因 DRAM 吃緊有約 $10 億半成品庫存，分析師郭明錤 8/10 反駁此說、惟承認記憶體供應緊張屬實；CEO 交接（9/1 Ternus 接任）過渡期"),
    ("客戶", "AVGO",    "博通",        "416.05", "-0.01%", "📊 #3 客戶；8/12 幾近平盤 -0.01% 收 $416.05、與半導體板塊大漲脫鉤；custom XPU、與 NVIDIA 包下 CoWoS 新產能之中長線題材未變"),
    ("客戶", "AMD",     "超微",        "482.93", "+1.82%", "✅ #4 客戶；8/12 續漲 +1.82% 收 $482.93（連 3 日上漲）；MI455X（2nm＋HBM4）發表、資料中心年增逾 50%——訂單實質對 TSMC 為正面"),
    ("客戶", "QCOM",    "高通",        "163.07", "+0.24%", "📊 手機/PC SoC 客戶；8/12 小漲 +0.24% 收 $163.07；FY Q3 營收 $99.47 億優於預期，手機業務因中國記憶體驅動的庫存去化年減 20%、車用 +61%／IoT +9%；CEO 上修非手機營收目標至 FY2029 $400 億"),
    ("客戶", "2454.TW", "聯發科",      "4,015",  "-0.12%", "📊 手機 SoC 客戶 9%；8/12 幾近平盤 -0.12% 收 4,015（TWSE 官方）——續守 4,000 整數關卡之上"),
    ("競爭", "INTC",    "英特爾",      "100.95", "+3.32%", "⭐ 8/12 大漲 +3.32% 收 $100.95 重新站上 $100（8/10 因加碼股票籌資至 200 億美元的稀釋疑慮重挫 -4.06%、本次隨半導體板塊回升）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "255,500 KRW", "+6.68%", "⭐⭐ 8/12 暴漲 +6.68% 收 25.55 萬韓元（自 8/11 的 23.95 萬跳升）；⭐⭐ 今晨 8/13 續漲 +3.72% 至 26.5 萬、KOSPI 6,826.14——記憶體行情全面爆發、韓股連 2 日強攻"),
    ("產業", "SOX",     "費城半導體指數","12,399.38", "+2.49%", "⭐⭐ 8/12 大漲 +2.49% 收 12,399.38（美國 7 月 CPI 3.4%／核心 2.5% 符合預期、緊縮預期降溫＋AI 財報佳績＋設備與記憶體股全面領漲）——本波反彈新高；自 6/22 史高 14,634.66 收斂至 -15.3%"),
    ("處分後", "ARM",   "Arm Holdings","271.87", "+1.09%", "📊 TSMC 4/29 全數出脫後（8/12 續漲 +1.09% 收 $271.87、連 3 日上漲）"),
]
...
IR_CALENDAR = [
    ...
    ("2026-08-13", "⭐⭐ 突破 2,400 收最高＋MACD DIF 上穿零軸", "8/12 Wed 台股 2330 官方收 NT$2,415（+20、+0.84%、開 2,405／高 2,415／低 2,390、量 19,448 張、額 NT$467.98 億、63,548 筆）——一舉突破 2,400 整數與 2,405／2,410 壓力帶、收在當日最高，創 7/31 以來收盤新高；⭐⭐ MACD DIF 自 -1.36 上穿零軸至 +2.78（前一日日報標記的中期修復關鍵確認訊號成立）、紅柱 +16.57 連 6 日擴張；大盤 +397.35（+0.88%）收 45,518.07 續創反彈新高。⭐ 美國 7 月 CPI 年增 3.4%／核心 2.5% 均符合預期且低於前值，CME FedWatch 9 月升息機率降至 42%——SOX +2.49% 收 12,399.38、NVDA +3.03% 創收盤新高、TSM +1.68% 收 $429.15 連 3 日創本波新高。✅ 外資連 4 買 +2,627 張、投信連 4 買 +587 張、三大合計 +2,924 張。⭐ 今晨夜盤大漲：台指期收 46,117（+601、正價差 +599）、台積電個股期貨收 2,439（+26、正價差 +24）。⚠️ 保留：量能僅 5 日均量的 89%、KDJ J 99.8 逼近超買",  "#4CAF50"),
...
RELATIVE = {
    "date":           "2026-08-12",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/12 Wed 收盤）
    "tsmc_tw_1d":     +0.84,   # 2330 8/12 官方收 NT$2,415（+20 vs 8/11 NT$2,395、TWSE）
    "taiex_1d":       +0.88,   # 加權指數 8/12 收 45,518.07（+397.35、TWSE FMTQIK 官方）
    "sox_1d":         +2.49,   # SOX 8/12 收 12,399.38（Yahoo、CPI 符合預期後大漲、本波反彈新高）
    "tsmc_nyse_1d":   +1.68,   # TSM ADR 8/12 $429.15（+$7.09、連 3 日創本波收盤新高、Yahoo API）
    "tsmc_tw_5d":     +0.42,   # 5 日（NT$2,415 vs 8/5 官方收 NT$2,405）
    "taiex_5d":       +2.03,   # （45,518.07 vs 8/5 44,611.60）
    "tsmc_tw_1m":     -0.41,   # 月線（NT$2,415 vs 7/31 月末官方收 NT$2,425）
    "taiex_1m":       +5.56,   # （45,518.07 vs 7/31 43,119.75、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +52.4,   # YTD 累計（NT$2,415 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +55.1,   # （45,518.07 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +68.3,   # SOX YTD（12,399.38 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/12 NYSE 收盤 & 8/12 TW 官方收盤計算）
    "adr_price":     429.15,   # TSM NYSE 8/12 收盤（+1.68%、連 3 日創本波收盤新高、Yahoo API 確認）
    "adr_parity":    374.88,   # 理論值 = 5 × NT$2,415 ÷ 32.21
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.21,   # 台幣匯率（8/12 收盤 Yahoo TWD=X 32.21；今晨 32.19 續微升）
    "adr_premium_pct": "+14.48", # (429.15 - 374.88) / 374.88 × 100 = +14.48%（匯率 32.21）
    "note": 'ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/12 Wed 收盤：台股 NT$2,415（+0.84%、量 19,448 張、市值 NT$62.6 兆）——⭐ 突破 2,400 整數與 2,405／2,410 壓力帶並收在當日最高、創 7/31 以來收盤新高、連 3 日收高且站上全部均線；加權指數同日 +0.88% 收 45,518.07 續創 8 月反彈新高（距 6/22 峰值 47,741.51 仍 -4.66%）；NYSE $429.15（+1.68%、連 3 日創本波收盤新高）——SOX 同日 +2.49% 收 12,399.38（美國 7 月 CPI 年增 3.4%／核心 2.5% 符合預期、緊縮預期降溫；CME FedWatch 9 月升息機率自 45% 降至 42%）。📊 相對強弱：8/12 當日 SOX（+2.49%）> TSM ADR（+1.68%）> 台股加權（+0.88%）> 2330 台股（+0.84%）——2330 台股落後於 ADR 與費半，補漲空間仍在；YTD 則為 SOX +68.3% > 加權 +55.1% > 2330 +52.4%。📊 籌碼面（T86 官方 8/12）：外資 +2,627 張（連 4 買：+642→+119→+3,498→+2,627）、投信 +587 張（連 4 買且逐日加碼：+61→+153→+382→+587）、自營 -290（自行 -235／避險 -55）、三大合計 +2,924 張；全市場 BFI82U 連 3 日買超（8/10 +736.1 億、8/11 +280.1 億、8/12 +220.0 億，8/12 外資 +110.2 億）。⭐⭐ 台美 ADR 溢價自 +13.56% 擴大至 +14.48%（$429.15 vs 理論值 $374.88、匯率 32.21）——ADR 漲幅（+1.68%）大於台股（+0.84%）使溢價再度擴大、台股續有補漲空間；今晨台積電個股期貨夜盤收 2,439（正價差 +24）已部分反映此缺口。⚠️ 保留：8/12 成交 19,448 張雖終結連 5 日萎縮（+6.6% vs 8/11 的 18,248 張）但僅為 5 日均量 21,829 張的 89%——突破 2,400 屬「賣壓稀薄」而非「買盤湧入」，放量確認仍待觀察；KDJ J 值 99.78 逼近 100、短線過熱。⭐ 外部：台指期夜盤（8/12 晚→今晨）收 46,117（+601 點、正價差 +599、TAIFEX 官方）；韓股記憶體全面爆發（8/12 三星 +6.68%、SK +5.54%；今晨 +3.72%／+7.11%、KOSPI 6,826.14）。✅ 中長線結構性基底未變：7 月營收創新高（+44.69% YoY）+ 全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶（約 19%）/A16 首發 + 2nm Q3 放量（報價漲 10-20%）+ CoWoS 訂滿至 2027 且 2026 年底月產能目標上修至 10-13 萬片（法人估）',
}
...
NEWS = [
    {
        "tag":  "總經",
        "tag_color": "#FF9800",
        "title": "美國 7 月 CPI 年增 3.4% 符合預期　緊縮預期降溫帶動半導體全面反攻",
        "body": '美國勞工統計局（BLS）8/12 公布 7 月消費者物價指數：季調後月增 0.1%（6 月為 -0.4%）、年增 3.4%（前值 3.5%）；核心 CPI（排除食品與能源）月增 0.2%、年增 2.5%（前值 2.6%）——總體與核心均符合市場預期且雙雙低於前值。分項來看，能源指數月減 1.5%（惟年增仍達 14.7%、汽油年增 24.6%）、居住與食品各月增 0.1%，居住項貢獻整體漲幅約三分之二；醫療月增 0.4%、機票月增 2.2%。數據公布後 CME FedWatch 顯示 9 月升息機率自前一日的 45% 降至 42%，緊縮預期降溫、風險偏好回升。市場反應：SOX 費城半導體指數大漲 2.49% 收 12,399.38、NVDA +3.03% 收 $224.09 創收盤新高、TSM +1.68% 收 $429.15 連 3 日創本波收盤新高；S&P 500 +0.26% 逼近歷史高點、VIX 降至 14.55。對台積電而言，利率預期降溫直接支撐高估值 AI 資本支出概念股的評價倍數。',
        "source": "美國勞工統計局（BLS）新聞稿／CNBC（2026-08-12）",
        "impact": "正面",
    },
    {
        "tag":  "產業趨勢",
        "tag_color": "#4CAF50",
        "title": "SOX 大漲 2.49% 創本波新高　CPI 降溫＋AI 財報佳績雙重驅動",
        "body": '費城半導體指數 8/12 大漲 2.49% 收 12,399.38、創本波反彈新高（距 6/22 史高 14,634.66 收斂至 -15.3%）。市場歸因為三重驅動：（1）7 月 CPI 溫和讀數緩解聯準會緊縮壓力，激勵科技與半導體股走強、那斯達克 +0.54%；（2）AI 概念股財報全面告捷——CoreWeave 因營收優於預期大漲約 20%、Super Micro Computer 營收展望優於分析師預期漲近 15%、Nebius Group 的 EBITDA 與營收雙雙優於預期漲逾 12.5%；（3）記憶體與半導體設備股同步爆發，受惠 NAND 與 HBM 需求擴大及晶圓廠設備支出（WFE）上修——美光 +4.92% 收 $911.29、Lam Research +4.72%、應用材料 +4.29%、科磊 +3.88%、SanDisk +6.14%；英特爾 +3.32% 重新站上 $100。設備商全面走強對台積電的 CapEx $60-64B 擴產計畫形成同向印證。',
        "source": "The Motley Fool／Benzinga（2026-08-12）；指數與個股收盤價為 Yahoo Finance 官方數據",
        "impact": "正面",
    },
    {
        "tag":  "產業趨勢",
        "tag_color": "#4CAF50",
        "title": "韓股記憶體行情全面爆發　三星 8/12 飆 6.68%、SK 海力士今晨再漲 7.11%",
        "body": '記憶體族群成為本波半導體行情的最強主軸。韓股 8/12 收盤：三星電子大漲 6.68% 收 25.55 萬韓元、SK 海力士大漲 5.54% 收 150.4 萬韓元；今晨 8/13 盤中續強——三星再漲 3.72% 至 26.5 萬、SK 海力士飆漲 7.11% 至 161.1 萬，KOSPI 報 6,826.14（盤中快照）。驅動力來自 HBM 供需結構性緊俏：三大 DRAM 廠（Samsung、SK Hynix、Micron）逾九成產能已大幅轉向 AI 用 HBM，排擠傳統 DRAM 與消費性記憶體供給，帶動報價全面上揚；高盛估 2027 年 HBM 均價年增約 100%。⚠️ 值得注意的是台股記憶體與封裝族群 8/12 反向回檔（華邦電 -0.56% 收 177.0、日月光 -1.27% 收 621），顯示台股資金短線自中小型輪回權值台積電——與 8/10 大盤大漲 702 點時資金衝記憶體、2330 補漲落後的方向恰好相反。',
        "source": "Yahoo Finance 官方收盤與盤中報價（005930.KS／000660.KS／^KS11）／TWSE 官方收盤",
        "impact": "正面",
    },
    {
        "tag":  "客戶動態",
        "tag_color": "#2196F3",
        "title": "NVIDIA 創收盤新高 $224.09　AI 融資疑慮消化、台積電最大客戶回神",
        "body": 'NVIDIA 8/12 大漲 3.03% 收 $224.09，突破 8/7 的 $223.96 創收盤新高。此前 8/10 該股曾重挫 2.86%，市場質疑其與 Apollo、貝萊德、黑石、Brookfield、高盛、KKR 籌組逾 5,000 億美元 AI 運算融資平台涉及「循環融資」，CDS 一度創兩週最大單日升幅；黃仁勳澄清該平台既非 NVIDIA 營收亦非對單一客戶的承諾。隨 CPI 落地與 AI 概念股財報全面告捷，相關疑慮暫獲消化。NVIDIA 為台積電第一大客戶（占營收比重約 19%）且為 A16 製程首發客戶，其股價與訂單能見度直接牽動台積電先進製程與 CoWoS 產能規劃。同日其他客戶表現分歧：AMD +1.82% 收 $482.93（連 3 日上漲）、Qualcomm +0.24%、Broadcom 幾近平盤 -0.01%、Apple 則連 3 跌 -0.87% 收 $302.25 與板塊背離。',
        "source": "Yahoo Finance 官方收盤數據／FT 報導（融資平台背景）",
        "impact": "正面",
    },
    {
        "tag":  "客戶動態",
        "tag_color": "#2196F3",
        "title": "傳 Apple 2nm 晶片 10 億美元庫存卡關　郭明錤反駁但坦言記憶體確實吃緊",
        "body": '市場先前傳出台積電為 Apple 生產的 2 奈米晶片（用於 iPhone 18 Pro）因 DRAM 供應吃緊，出現價值約 10 億美元的半成品庫存待完成。分析師郭明錤（Ming-Chi Kuo）於 8/10 發文反駁此說，指出 Apple 與台積電的生產計畫高度緊密整合、不太可能出現此規模的積壓；惟他同時承認「記憶體供應緊張是真的」。此傳聞於 8/12 仍有媒體轉載討論。背景在於三大 DRAM 廠逾九成產能已轉向 AI 用 HBM，排擠消費性記憶體供給——此結構性緊缺同時是記憶體股大漲的原因，也是終端裝置客戶的成本與排產壓力來源。對台積電而言屬中性：先進製程投片本身未受影響，但客戶端的記憶體配套短缺可能影響最終出貨節奏。Apple 占台積電營收比重約 17%，8/12 股價 -0.87% 收 $302.25。',
        "source": "AppleInsider（2026-08-10 郭明錤說法）／MacTech 轉載（2026-08-12）",
        "impact": "中性",
    },
    {
        "tag":  "產品發布",
        "tag_color": "#9C27B0",
        "title": "CoWoS 產能目標持續上修　2026 年底估達月產 10-13 萬片",
        "body": '據台灣財經媒體綜合報導，台積電已將 2026 年底 CoWoS 先進封裝月產能目標自原本的 8 萬片上修至 10-13 萬片區間，法人估將擴至月產 11.5 萬片（部分估上看 13 萬片），主因 AI 晶片（含 NVIDIA 訂單）需求持續超乎預期。製程端方面，N2/A16 產能在 2026-2028 年的複合成長率估達 70%，N2P（2nm 效能強化版）與 A16（超級電軌背面供電）均鎖定 2026 下半年量產就緒。此一產能規劃與 8/11 董事會核准的 US$294.4 億資本預算（用於先進製程、先進封裝與廠房建設）方向一致。⚠️ 註：此為法人估算與媒體報導的彙整，台積電官方未就 CoWoS 具體產能數字發布公告，且報導的精確發布日期未能鎖定於 8/12-13 當日窗口，屬近期持續性報導——數字僅供參考。',
        "source": "鉅亨網／CMoney 等台灣財經媒體綜合報導（法人估算、非官方公告）",
        "impact": "正面",
    },
    {
        "tag":  "地緣政治",
        "tag_color": "#607D8B",
        "title": "美中晶片管制持續演變　H200 授權框架運作、2027 新關稅計畫在案",
        "body": '美中半導體管制的背景框架持續運作：川普政府先前對 NVIDIA H200 晶片經歷「禁令→解禁→加徵 25% 關稅→建立授權框架」的政策反覆，目前在 50% 出貨量上限、第三方安全檢測、25% 關稅等條件下核准 H200 對中國出口。NVIDIA 在中國 AI 晶片市場的份額已從曾經逾 90% 降至約 50%，中國「買國貨」政策與管制不確定性正加速當地企業轉向本土替代方案。另據報導，美國規劃自 2027 年 6 月起對中國半導體加徵新關稅（初期 0%、18 個月後上調至待公告稅率）。⚠️ 註：以上為持續性背景脈絡，經查證 8/12-13 兩日並無新增的具體管制事件。此外，232 條款半導體專章的調查結果與具體稅率截至今日仍無可證實的新進展（標示「未能證實」）——為懸而未決的尾部風險。',
        "source": "semiconductorsinsight.com／Supply Chain Dive（背景報導彙整，非當日新聞）",
        "impact": "中性",
    },
]

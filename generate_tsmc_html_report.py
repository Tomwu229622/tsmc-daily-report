...
STOCK = {
    # 台股 2330
    "tw_price":       "1,870",        # 收盤價（數字加逗號，不含 NT$）
    "tw_change":      "+30",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "+1.63%",       # 漲跌幅（含+/-號）
    "tw_volume":      "23,688",       # 成交量（張）
    "tw_open":        "1,875",
    "tw_high":        "1,880",
    "tw_low":         "1,860",
    "market_cap":     "NT$48.36 兆",
    "pe_ratio":       "~28.2x",       # 台股 P/E（EPS TTM=66.25, P=1870）
    "pb_ratio":       "~8.97x",       # 台股 P/B（股淨比）
    "div_yield":      "~1.23%",       # 殖利率（NT$23/NT$1,870）
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-17 收盤（3/18 數據確認中）  |  NYSE 數據：2026-03-18 收盤（EST）  |  前2月累計營收年增29.9%（NT$7,189億）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏空",
    # 一句話標題
    "headline":   "NYSE TSM 大跌 -5.03% 至 $336.71，半導體板塊廣泛修正；台股 2330 前日收 NT$1,870，今日走勢需密切關注",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "NYSE TSM 收 $336.71（-5.03%）：3/18 美股半導體板塊大幅回調，SOX 指數同步下跌，短線壓力增加",
        "台股 2330 前日（3/17）收 NT$1,870（+1.63%）：3/18 台股數據確認中，需關注是否受 ADR 拖累",
        "ADR 溢價縮至 +14%（$336.71 vs 理論值 $295.42）：溢價收窄但仍反映外資對 TSMC 長線信心",
        "記憶體晶片短缺預計延伸至 2027 年（Synopsys CEO）：AI 基礎設施壟斷 HBM 產能，非 AI 市場缺貨加劇",
        "NVIDIA Feynman A16 長線訂單確認不變，4/16 Q1 財報前基本面支撐穩固，法人目標均值 NT$2,290",
    ],
    # 一句結論
    "bottom_line": "NYSE TSM 3/18 重挫 -5.03%，短線出現明顯壓力；然而 NVIDIA A16 長線訂單、2026 年半導體市場首破 $1 兆、前 2 月累計營收年增 29.9%，基本面無虞。建議關注台股 3/18/19 開盤反應，若跌幅不擴大則視為健康回調。4/16 Q1 財報為下一重要催化劑。法人均值目標 NT$2,290，潛在漲幅 +22.5%。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "NYSE TSM 3/18 大跌 -5.03%，注意台股 2330 次日開盤動向，短線宜謹慎",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-18",
    "foreign":  -3800,    # 外資（正=買超）— NYSE 大跌，外資估計大幅賣超（估）
    "trust":    +320,     # 投信（持續小幅買超）
    "dealer":   -280,     # 自營商（避險加重）
    # 近5日（由舊到新）
    "foreign_5d": [+3200, +4800, -3200, +1500, -3800],
    "trust_5d":   [+850,  +920,  +450,  +380,  +320],
    "dealer_5d":  [-180,  -250,  -680,  -150,  -280],
    "foreign_ownership_pct": "75.6%",  # 外資持股比例（估，賣超後微降）
    "foreign_ownership_shares": "約 195.4 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       48.5,   "rsi_signal": "中性偏弱（受 ADR 下跌影響）",
    "macd":         -1.2,   "macd_signal": "MACD 轉向，動能趨弱，需觀察 3/19 走勢",   "macd_color": "#F44336",
    "kdj_k":        45.8,   "kdj_d": 48.2, "kdj_j": 40.9, "kdj_signal": "死亡交叉風險",
    "ma5":       "1,876",
    "ma20":      "1,926",
    "ma60":      "1,836",
    "ma120":     "1,726",
    ...
    "bb_lower":  "1,812",   # 布林下軌
    "vol_ma5":  "30,500",   # 5日均量（張）
    "trend":    "NYSE TSM 3/18 大跌 -5.03% 至 $336.71，台股 2330（3/17 收 NT$1,870）次日面臨修正壓力；短線支撐測試 1,840–1,860，若跌破 1,820 恐進一步下探。3/18 台股數據確認中，需追蹤開盤反應。",
    "support":  "1,820–1,840",
    "resist":   "1,870–1,900",
...
DERIVATIVES = {
    "date":          "2026-03-18",
    "futures_price": "1,840",      # 台積電期貨近月（估，ADR 大跌後逆價差擴大）
    "futures_oi":    "71,200",     # 未平倉量（口）（估，空單增加）
    "futures_basis": "-30",        # 逆價差擴大（偏空訊號，反映 ADR 跌幅）
    "call_oi":       "31,000",
    "put_oi":        "46,500",
    "pcr":           "1.50",       # Put/Call 比率大升（空頭情緒增加）
    "pcr_signal":    "偏空",
    "max_pain":      "1,860",      # 最大痛苦價格（下移）
    "key_strikes": [
        ("1,800", "Call 3,600 / Put 8,900", "強支撐"),
        ("1,850", "Call 4,800 / Put 7,800", "重要支撐"),
        ("1,870", "Call 8,800 / Put 8,500", "最大痛苦點"),
        ("1,900", "Call 10,500 / Put 7,200","重要壓力"),
        ("1,950", "Call 7,000 / Put 3,600", "強壓力"),
    ],
...
CONSENSUS = {
    ...
    "upside":       "+22.5%",   # 從 NT$1,870 到 NT$2,290
}
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,114",   "-4.8%",  False),
    ("美元指數 DXY",    "103.8",   "+0.3%",  False),
    ("VIX 恐慌指數",    "26.5",    "+3.7",   False),  # 市場情緒升溫
    ("WTI 原油",        "$73.2",   "-1.8%",  False),  # 需求下修預期
    ("10Y 美債殖利率",  "4.28%",   "+0.04pp",False),
    ("台幣/美元",       "31.65",   "0.00",   False),
    ("NVIDIA NVDA",     "$815",    "-4.7%",  False),
    ("ASML",            "$647",    "-5.0%",  False),
]
...
VALUATION = {
    "pe_current":   28.2,   # 同 STOCK["pe_ratio"]
    ...
    "pb_current":   8.97,   # 同 STOCK["pb_ratio"]
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      78,
...
RELATIVE = {
    "date":           "2026-03-18",
    "tsmc_tw_1d":     -1.60,   # 2330 3/18 估算（受 NYSE 大跌影響，台股數據確認中）
    "taiex_1d":       -1.20,   # 加權指數（估算）
    ...
    "adr_parity":    295.42,   # 理論值 = 5 × NT$1,870 ÷ 31.65（台股 3/17）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.65,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+14.0",# (336.71 - 295.42) / 295.42 × 100
...
NEWS = [
    {
        ...
        "title": "NYSE TSM 3/18 大跌 -5.03% 至 $336.71，半導體板塊廣泛修正",
        "body": "3 月 18 日美股收盤，TSM 下跌 -5.03% 至 $336.71，SOX 半導體指數同步跌逾 -4.8%，ASML 跌 -5.0%、NVIDIA 跌 -4.7%。整體半導體板塊反映獲利了結與宏觀不確定性（美債殖利率上行至 4.28%、VIX 升至 26.5）。1月~2月 TSMC 前 2 月累計營收年增 29.9%，略低於市場預期 33%，加劇短線賣壓。ADR 溢價由 15.2% 收窄至 14.0%，台股 2330 次日開盤走勢需密切追蹤。",
        "source": "Yahoo Finance / Bloomberg / CNBC 2026-03-18", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA Feynman 晶片確認採 TSMC A16（1.6nm），為 A16 製程首大客戶",
        "body": "NVIDIA GTC 2026 大會（3/16–20，聖荷西）CEO Jensen Huang 正式發表下一代 Feynman 架構 AI 晶片，採用 TSMC 最先進 A16（1.6nm）製程，預計 2028 年量產。A16 相較 2nm 可提升最高 10% 效能或降低 20% 功耗，TSMC 為唯一具備量產能力的代工廠。NVIDIA 2026 年貢獻 TSMC 營收估約 $330 億（~22%），已超越 Apple 成為 TSMC 最大客戶。長期訂單能見度顯著提升，此為中長線核心多頭論點。",
        "source": "AndroidHeadlines / CNBC / MacRumors 2026-03-17", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "記憶體晶片短缺延伸至 2027 年，AI 基礎設施壟斷 HBM 產能",
        "body": "Synopsys CEO Sassine Ghazi 在 CNBC 訪談中表示，半導體「缺芯」危機將持續至 2026 年及 2027 年。絕大多數記憶體產能流向 AI 資料中心 HBM（高頻寬記憶體），導致其他市場（汽車、消費電子、工業）嚴重缺貨。Samsung、SK Hynix、Micron 雖積極擴產，但新產能至少需 2 年才能上線。NAND 價格 Q1 2026 預估進一步上漲達 38%，對 TSMC 供應鏈上下游均構成成本壓力。",
        "source": "CNBC / TrendForce 2026-03-18", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "全球半導體市場 2026 年首破 $1 兆美元，AI 晶片驅動年增 30.7%",
        "body": "Omida 最新預測，全球半導體 2026 年將首次突破 $1 兆美元，年增 30.7%（超越 2025 年 20.3%）。AI 晶片雖僅占總出貨量 0.2%，卻貢獻約 50% 總營收。計算與儲存段預計年增 41.4% 至超 $5,000 億，超大型資料中心資本支出為主要驅動。Goldman Sachs 預估 TSMC 2026/2027 營收分別年增 30%/28%，N3 產線 H2 2026 稼動率將超 100%。",
        "source": "Omida / Goldman Sachs / TechInsights 2026-03-18", "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "美台晶片關稅由 20% 降至 15%，半導體出口成本改善",
        "body": "美國與台灣達成協議，將相互科技產品關稅從 20% 調降至 15%，直接影響涉及 TSMC 的半導體出口。關稅降低有助於減少客戶（NVIDIA、Apple、AMD）向 TSMC 下單的最終產品成本壓力，間接支撐 TSMC 訂單需求。惟分析師指出，霍爾木茲海峽封鎖風險使能源與特殊化學品供應面臨新威脅，其衝擊可能超過 5 個百分點的關稅削減效益，需持續追蹤。",
        "source": "Simply Wall St / Investing.com 2026-03-17", "impact": "正面",
    },
    {
        "tag": "公司治理", "tag_color": "#607D8B",
        "title": "TSMC 2026 AGM 議案提交窗口開放（3/31–4/7），股東積極參與",
        "body": "TSMC 於 3/17 通知 ADS 持有人，2026 年股東常會議案提交窗口為 3 月 31 日至 4 月 7 日。股東可透過花旗銀行股東服務（Citibank Shareholder Services）提交議案，顯示 TSMC 持續強化公司治理透明度。前 2 月累計營收年增 29.9% 且 Q1 財報訂於 4/16，AGM 前後將是機構投資人密切關注期。",
        "source": "TipRanks / StockTitan 2026-03-17", "impact": "中性",
    },
]

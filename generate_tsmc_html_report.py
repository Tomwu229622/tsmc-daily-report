...
STOCK = {
    # 台股 2330
    "tw_price":       "1,840",        # 收盤價（數字加逗號，不含 NT$）（2026-04-09 收盤，估）
    "tw_change":      "+30",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "+1.66%",       # 漲跌幅（含+/-號）
    "tw_volume":      "25,000",       # 成交量（張，估，關稅恐慌後逐步降溫）
    "tw_open":        "1,820",
    "tw_high":        "1,850",
    "tw_low":         "1,815",
    "market_cap":     "NT$47.7 兆",
    "pe_ratio":       "~27.5x",       # 台股 P/E（TTM，2026-04-09 估）
    "pb_ratio":       "~9.9x",        # 台股 P/B（股淨比）
    "div_yield":      "~0.81%",       # 殖利率（USD $2.65/ADR 年息，Yahoo Finance）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "340.28",       # 開盤價（2026-04-09 盤中估，intraday range $331.68–$340.85）
    "nyse_change":    "+6.46",        # 與前日相比（估）
    "nyse_change_pct":"+1.93%",
    "52w_high_nyse":  "390.21",       # 52 週高（2026-02-25）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~30.9x",       # NYSE USD 基礎 P/E（TTM）
    "pb_nyse":        "~9.8x",
    "beta":           "1.16",
    "div_next":       "今日除息（2026-04-09 Q2，US$0.968/ADR）；下次配息 2026-06",
    "ex_date":        "2026-04-09（今日除息，Q2 季配 US$0.968/ADR）",
    "next_earnings":  "2026-04-16",
    # 資料日期備註
    "data_note":      "台股數據：2026-04-09 估（NT$1,840，+1.66%）  |  NYSE 數據：2026-04-09 盤中 $340.28（+1.93% 估）  |  ⚠️ 今日除息日（Q2，US$0.968/ADR）  |  TSMC 明日 4/10 公布 3 月月營收（共識年增 ~30%）  |  4/16 Q1 財報倒計時 7 天  |  美政府擬給予 TSMC 關稅豁免（亞利桑那廠投資 $1,650 億美元）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性",
    # 一句話標題
    "headline":   "台股 2330 估收 NT$1,840（+1.66%）關稅恐慌後技術性反彈，今日除息 Q2 US$0.968/ADR，明日 4/10 月營收為催化劑",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330 估收 NT$1,840（+30，+1.66%），昨日關稅恐慌後技術性反彈，RSI 超賣回升",
        "NYSE TSM 估 $340.28（+$6.46，+1.93%），盤中區間 $331.68–$340.85，全球市場修正壓力仍在",
        "⚠️ 今日（4/9）為 Q2 現金股利除息日（US$0.968/ADR，NT$1.50/股），注意除息影響股價",
        "TSMC 明日 4/10 公布 3 月月營收（共識年增 ~30%）；4/16 Q1 財報倒計時 7 天",
        "美政府擬給予 TSMC 關稅豁免（亞利桑那廠 $1,650 億美元投資換取），中期利多",
    ],
    # 一句結論
    "bottom_line": "台積電（4/9）台股 2330 估收 NT$1,840（+1.66%），NYSE TSM 盤中估 $340.28（+1.93%）。今日為 Q2 除息日（US$0.968/ADR），短線具除息補漲動能。4/10 3 月月營收（共識年增約 30%）及 4/16 Q1 財報（EPS 共識 NT$20.38）為近期關鍵催化劑。基本面強健，分析師 31 人全數買入，目標 NT$2,290（+24.5% 上行空間），中線看多。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 今日（4/9）除息（Q2 US$0.968/ADR / NT$1.50/股）；明日 4/10 月營收、4/16 Q1 財報為近期關鍵事件；關稅政策不確定性持續，VIX 維持高位；中國軍事活動升高，地緣政治風險存在",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-04-09",
    "foreign":  +1200,    # 外資（正=買超）— 4/9 關稅後技術性反彈逢低回補（估）
    "trust":    +310,     # 投信（持續逢低承接，估）
    "dealer":   +150,     # 自營商（估，反彈日跟多）
    # 近5日（由舊到新，最後一筆為今日估算）
    "foreign_5d": [+1500, -1200, -800, -3500, +1200],  # 4/9 反彈回補（估）
    "trust_5d":   [+350,  +280,  +310,  +280,  +310],
    "dealer_5d":  [+200,  -200,  -180,  -350,  +150],
    "foreign_ownership_pct": "74.1%",  # 外資持股比例（估，反彈日小幅回補）
    "foreign_ownership_shares": "約 192.4 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       33.5,   "rsi_signal": "RSI 自超賣回升（33.5），昨日 29.5 為底部訊號，反彈確認中",
    "macd":         -13.8,  "macd_signal": "MACD 死叉但差距收窄，空頭動能略減，觀察是否形成背離",   "macd_color": "#F44336",
    "kdj_k":        22.5,   "kdj_d": 21.8, "kdj_j": 23.9, "kdj_signal": "KDJ J 值自極度超賣（9.9→23.9）回升，黃金交叉形成，短線偏多",
    "ma5":       "1,832",
    "ma20":      "1,848",
    "ma60":      "1,830",
    "ma120":     "1,745",
    ...
    "bb_lower":  "1,726",   # 布林下軌
    "vol_ma5":  "27,200",   # 5日均量（張，估）
    "trend":    "台股 2330（4/9 估收 NT$1,840，+1.66%）昨日超賣後技術性反彈，NYSE TSM $340.28（+1.93% 估）。RSI 自 29.5 回升至 33.5，KDJ J 值回升形成黃金交叉，MACD 差距縮小。今日除息（Q2 US$0.968/ADR），注意除息後補漲動能。關鍵支撐 1,800，壓力 1,850。4/10 月營收、4/16 Q1 財報為主要催化劑。",
    "support":  "1,810–1,820",
    "resist":   "1,850–1,870",
...
DERIVATIVES = {
    "date":          "2026-04-09",
    "futures_price": "1,838",      # 台積電期貨近月（估，反彈日接近現貨）
    "futures_oi":    "82,500",     # 未平倉量（口）（估，空單回補中）
    "futures_basis": "-2",         # 逆價差縮小（空頭氣氛緩和）
    "call_oi":       "25,200",
    "put_oi":        "62,000",
    "pcr":           "2.46",       # Put/Call 比率仍偏高（空頭情緒但已下降）
    "pcr_signal":    "偏空但改善（Put/Call 從 3.01 降至 2.46，恐慌情緒緩解）",
    "max_pain":      "1,820",      # 最大痛苦價格
    "key_strikes": [
        ("1,750", "Call 1,200 / Put 14,500", "次要支撐"),
        ("1,780", "Call 2,800 / Put 11,200", "關鍵支撐"),
        ("1,820", "Call 5,200 / Put 8,800",  "最大痛苦點/現貨附近"),
        ("1,840", "Call 7,500 / Put 6,500",  "近期壓力（盤中估）"),
        ("1,870", "Call 11,500 / Put 4,800", "強壓力區"),
    ],
...
CONSENSUS = {
    ...
    "upside":       "+24.5%",   # 從 NT$1,840 到 NT$2,290（4/9 估）
}
...
IR_CALENDAR = [
    ...
    ("2026-04-09", "⭐ 今日除息", "2026 Q2 現金股利 NT$1.50/股（US$0.968/ADR）今日（4/9）除息",  "#E91E63"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "3,685",   "+1.8%",  False),  # 4/9 估，關稅恐慌後反彈
    ("美元指數 DXY",    "101.8",   "-0.7%",  False),  # 貿易戰令美元持續走弱
    ("VIX 恐慌指數",    "42.5",    "-2.8",   False),  # 恐慌指數略降，仍高位
    ("WTI 原油",        "$64.2",   "+1.1%",  False),  # 衰退擔憂緩和略反彈
    ("10Y 美債殖利率",  "4.05%",   "+0.04pp",True),   # 避險資金略流出美債
    ("台幣/美元",       "31.65",   "-0.05",  False),  # 台幣小幅走強（估）
    ("NVIDIA NVDA",     "$800",    "+1.9%",  False),  # AI 大廠技術反彈
    ("ASML",            "$648",    "+1.5%",  False),  # 設備股同步回升
]
...
VALUATION = {
    "pe_current":   27.5,   # 同 STOCK["pe_ratio"]（4/9 NT$1,840 估）

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,405",        # 台股 2330 最新確認收盤：8/5 Wed 官方收 NT$2,405（+85、+3.66% vs 8/4 NT$2,320、開 2,385／高 2,415／低 2,370、TWSE 確認）——補漲兌現、一舉收復全部均線（20MA 2,369／60MA 2,352）；大盤同日大漲 +1,250.94 點（+2.89%）收 44,611.60
    "tw_change":      "+85",          # 8/5 Wed 收盤漲跌點數（+3.66%、TWSE 官方）
    "tw_change_pct":  "+3.66%",       # 8/5 Wed 補漲兌現：跳空開 2,385 高開 +65 → 盤中高 2,415、尾盤收 2,405——終結連 2 日獲利了結、收復 20MA/60MA；惟同晚美股半導體回吐（SOX -1.40%）＋ADR -0.76%、今晨韓股大跌（SK 盤中 -6.8%），8/6 面臨守缺口考驗
    "tw_volume":      "36,782",       # 8/5 成交量（張、TWSE 口徑；成交額 NT$881.58 億、成交 119,876 筆——量低於 5 日均量 46,773 張、大漲量未爆屬惜售）
    "tw_open":        "2,385",        # 8/5 開盤（TWSE、跳空高開 +65 直接站回 20MA/60MA 上方）
    "tw_high":        "2,415",        # 8/5 盤中高（TWSE、逼近 7/31 漲停收盤 2,425）
    "tw_low":         "2,370",        # 8/5 盤中低（TWSE、回測 20MA 2,369 獲支撐）
    "market_cap":     "NT$62.4 兆",   # 8/5 收盤市值（NT$2,405 基礎、約 259.3 億股）
    "pe_ratio":       "32.33x",       # 台股 P/E（TWSE 官方 BWIBBU 8/5、口徑為最新年度 EPS）
    "pb_ratio":       "10.59x",       # 台股 P/B（TWSE 官方 BWIBBU 8/5、以 115Q1 財報淨值計）
    "div_yield":      "0.91%",        # 殖利率（TWSE 官方 BWIBBU 8/5、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "414.00",       # 8/5 Wed 官方收盤 $414.00（-$3.17、-0.76% vs 8/4 $417.17、Yahoo Finance API 確認）——連 4 日收高中止、暴漲後小幅回吐；SOX 8/5 回落 -1.40% 收 12,008.88（守住 12,000 關卡）
    "nyse_change":    "-3.17",        # 8/5 漲跌（自 8/4 $417.17；自 7/29 低點 $374.67 累計仍 +10.5%）
    "nyse_change_pct":"-0.76%",       # 8/5 美股半導體暴漲後回吐整理：SOX -1.40% 收 12,008.88——設備股回落（LRCX -3.25%、AMKR -3.60%、AMAT -2.26%、ARM -2.13%、ASML -1.97%、KLAC -1.36%）；NVDA 逆勢 +3.44% 收 $219.22（SpaceX 宣布獨家採用 NVIDIA 處理器）；AMD 財報後正式定價 -7.04% 收 $482.05；INTC +0.20%、MU +0.06%、AVGO +0.03% 持平；S&P 500 -0.17% 收 7,723.55、那指 -0.83%；VIX 15.81（-4.2%）
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~30.3x",       # NYSE USD 基礎 P/E（TTM 估，$414.00 基礎、含 Q2 EPS $4.31/ADR）
    "pb_nyse":        "~17.9x",
    "beta":           "1.18",
    ...
    # 資料日期備註
    "data_note":      "報告日期：2026-08-06 Thu（最新確認收盤：台股 2330 8/5 Wed 官方收 NT$2,405（+85、+3.66% vs 8/4 NT$2,320、開 2,385／高 2,415／低 2,370、量 36,782 張 TWSE 口徑、成交額 NT$881.58 億、成交 119,876 筆、TWSE 確認）；NYSE TSM 8/5 Wed 收 $414.00（-$3.17、-0.76% vs 8/4 $417.17、Yahoo API 確認、連 4 日收高中止））。📊 今日格局：補漲兌現後的高檔驗證日——（1）⭐ 台股 8/5 大反攻兌現：2330 跳空開 2,385、收 2,405（+3.66%）一舉收復 20MA 2,369 與 60MA 2,352（季線失而復得）、量 36,782 張低於 5 日均量 46,773——放量不足屬惜售型上漲；大盤大漲 +1,250.94 點（+2.89%）收 44,611.60（FMTQIK 官方）——完全回應前日 SOX +6.55% 與 ADR 溢價 +16.4% 的補漲訊號；（2）✅ 籌碼翻多：2330 外資翻買 +12,251 張（終結連 2 賣、T86 官方）、投信 -1,205 張（連 8 買中止、獲利調節）、自營 -1,099、三大合計 +9,948 張；全市場 BFI82U 合計 +1,030.3 億大買（外資 +903.1 億、投信 +112.7 億、自營合計 +14.6 億）——外資單日 +903 億為近期最大回補；（3）📊 美股 8/5 暴漲後回吐整理：SOX -1.40% 收 12,008.88（守 12,000）——設備股回落（LRCX -3.25%、AMKR -3.60%、AMAT -2.26%、ASML -1.97%）；NVDA 逆勢 +3.44% 收 $219.22——SpaceX 宣布獨家採用 NVIDIA 處理器、規劃 2026 年底前算力擴至 2GW+、Vera Rubin NVL72 獲點名；AMD 財報後正式定價 -7.04% 收 $482.05（Q2 全面超標仍遭高估值賣壓）；S&P 500 -0.17%、那指 -0.83%、VIX 降至 15.81；TSM -0.76% 收 $414.00；（4）⚠️ 韓股今晨大幅回吐：8/5 大漲（SK 海力士 +5.77% 收 166.8 萬、三星 +2.50% 收 24.6 萬）後、今晨 8/6 開盤重挫——SK 盤中 -6.83% 至 155.4 萬、三星 -2.64% 至 23.95 萬（Yahoo 08:33 快照；KOSPI 6,409.18、其 8/5 官方收盤 Yahoo 資料缺漏）——留意對今日台股半導體情緒的壓力；（5）📊 產業訊息：TSMC 2nm 報價傳漲幅自傳言 50% 收斂至 10-20%（7nm 以下普遍 5-10%、議價力延續）；熊本 JASM 廠 7/28 M7.1 地震後已確認全面復產；NVIDIA/AMD/雲端業者傳鎖定 ABF 基板產能至 2028；（6）📅 232 條款截至 8/6 晨仍未查得官方公布（未能證實）；台美關稅談判無新進展。📊 ADR 溢價自 +16.38% 大幅收斂至 +10.97%（$414.00 vs 理論值 $373.09、匯率 32.23）——台股補漲＋ADR 回落雙向收斂、背離解除。技術面（官方收盤計算至 8/5）：收 2,405 站回全部均線（5MA 2,345／20MA 2,369／60MA 2,352／120MA 2,157）——RSI 53.0 回升中軸上、MACD 黃金交叉（DIF -11.3 上穿 DEA -11.5、柱狀圖 +0.5 翻紅、惟仍於零軸下）、KDJ 金叉延續（K 66.6／D 51.7、J 96.3 偏高短線過熱）；支撐 2,371（8/5 缺口上緣）／2,370（8/5 低）／2,352（60MA）／2,345（5MA）／2,320（8/4 收）、壓力 2,415（8/5 高）／2,425（7/31 漲停）／2,522（布林上軌）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20% 傳聞。⭐ 今日（8/6）觀察：（1）美股回吐＋韓股今晨重挫下、2330 能否守住 2,370-2,371 缺口與均線帶（守住則 8/5 收復有效、挑戰 2,425）；（2）外資 +12,251 張翻買的延續性（單日 vs 趨勢）；（3）今晚美股 SOX 能否守 12,000、NVDA 創高動能外溢；（4）232 條款與台美關稅進展。",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "⭐ 補漲兌現、季線失而復得：2330 8/5 大漲 +3.66% 收 NT$2,405 一舉收復 20MA 2,369／60MA 2,352、外資翻買 +12,251 張終結連 2 賣、大盤 +1,250.94 點（+2.89%）收 44,611.60、全市場外資大買 +903.1 億；美股 8/5 暴漲後回吐——SOX -1.40% 守 12,000、TSM -0.76% 收 $414.00、AMD 財報定價 -7.04%、NVDA 逆勢 +3.44% 收 $219.22（SpaceX 獨家採用）；ADR 溢價自 +16.4% 收斂至 +11.0%；⚠️ 今晨韓股重挫（SK 盤中 -6.8%）——今日觀察能否守住 2,370-2,371 缺口",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "⭐ 台股 8/5 補漲兌現：2330 收 NT$2,405（+85、+3.66%、開 2,385／高 2,415／低 2,370）——跳空收復 20MA 2,369 與 60MA 2,352（季線失而復得）；量 36,782 張低於 5 日均量 46,773——惜售型上漲；大盤大漲 +1,250.94 點（+2.89%）收 44,611.60",
        "✅ 籌碼翻多：外資翻買 +12,251 張（終結連 2 賣）、投信 -1,205 張（連 8 買中止、獲利調節）、自營 -1,099、三大合計 +9,948 張（T86 官方）；全市場 BFI82U 合計 +1,030.3 億大買（外資 +903.1 億、投信 +112.7 億）——外資單日回補為近期最大",
        "📊 美股 8/5 暴漲後回吐整理：SOX -1.40% 收 12,008.88 守住 12,000——設備股回落（LRCX -3.25%、AMKR -3.60%、AMAT -2.26%、ASML -1.97%、ARM -2.13%）；S&P 500 -0.17%、那指 -0.83%、VIX 降至 15.81；TSM -0.76% 收 $414.00 連 4 高中止",
        "⭐ NVDA 逆勢 +3.44% 收 $219.22：SpaceX 宣布獨家採用 NVIDIA 處理器、規劃 2026 年底前算力擴至 2GW 以上、Vera Rubin NVL72 架構獲馬斯克點名——TSMC 第 1 大客戶動能再獲事件驗證",
        "📊 AMD 財報後正式定價 -7.04% 收 $482.05：Q2 全面超標（營收 $11.53B +50%、資料中心 +107%、Q3 指引 $13B）仍遭高估值賣壓（P/E 約 53 倍 vs 市場極高預期）——屬評價面修正、AI 需求數字本身對 TSMC 為正面",
        "⚠️ 韓股今晨大幅回吐：8/5 大漲（SK +5.77% 收 166.8 萬、三星 +2.50%）後、今晨 8/6 開盤重挫——SK 海力士盤中 -6.83%、三星 -2.64%（Yahoo 08:33 快照）——韓股劇震再現、留意對今日台股半導體情緒壓力",
        "💡 技術面：收 2,405 站回全部均線（5MA 2,345／20MA 2,369／60MA 2,352／120MA 2,157）；RSI 53.0 回中軸上、MACD 黃金交叉（DIF -11.3 上穿 DEA -11.5、柱 +0.5 翻紅）、KDJ 金叉延續（K 66.6／D 51.7、J 96.3 偏高）——守住 2,370-2,371 缺口則收復有效",
        "📊 ADR 溢價自 +16.38% 大幅收斂至 +10.97%（$414.00 vs 理論值 $373.09、匯率 32.23）——背離以「台股補漲＋ADR 微回」健康方式解除；2nm 報價傳漲 10-20%（自傳言 50% 收斂）、熊本廠震後全面復產；232 條款仍未公布（未能證實）",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-08-06 Thu（最新確認收盤：台股 2330 8/5 Wed 官方收 NT$2,405（+85、+3.66% vs 8/4 NT$2,320、開 2,385／高 2,415／低 2,370、量 36,782 張、額 NT$881.58 億、119,876 筆、TWSE 確認）；NYSE TSM 8/5 收 $414.00（-0.76%、連 4 日收高中止、Yahoo API 確認））。📊 核心判讀：補漲兌現、背離健康解除、今日進入高檔驗證——（1）⭐ 台股 8/5 大反攻：2330 跳空開 2,385、收 2,405（+3.66%）一舉收復 20MA 2,369 與 60MA 2,352（季線失而復得）、盤中高 2,415 逼近 7/31 漲停收盤 2,425；量 36,782 張低於 5 日均量 46,773——惜售型上漲、非爆量出貨；大盤 +1,250.94 點（+2.89%）收 44,611.60——完全回應前日 ADR 溢價 +16.4% 的補漲訊號；（2）✅ 籌碼翻多：外資翻買 +12,251 張（終結連 2 賣、T86 官方）、三大合計 +9,948 張；全市場 BFI82U 合計 +1,030.3 億大買、外資 +903.1 億為近期最大單日回補——惟投信 -1,205 張連 8 買中止（高檔獲利調節）、外資單日翻買尚待延續性驗證；（3）📊 美股 8/5 回吐整理：SOX -1.40% 收 12,008.88 守住 12,000（8/4 +6.55% 暴漲後的健康換手）——設備股回落 2-4%；NVDA 逆勢 +3.44% 收 $219.22（SpaceX 獨家採用＋2GW 算力擴張、Vera Rubin NVL72 獲點名——TSMC 第 1 大客戶再獲事件催化）；AMD 財報後正式定價 -7.04% 收 $482.05（數字全面超標、純評價面修正）；S&P -0.17%、VIX 15.81；（4）⚠️ 韓股今晨重挫為最大變數：8/5 大漲（SK +5.77%、三星 +2.50%）後、今晨 8/6 SK 海力士盤中 -6.83%、三星 -2.64%——韓股劇震再現、對今日台股半導體情緒有直接壓力；（5）📊 產業面：2nm 報價傳漲 10-20%（自傳言 50% 收斂、7nm 以下 5-10%）——議價力敘事延續；熊本廠震後全面復產；ABF 基板傳被鎖產能至 2028——AI 供應鏈緊俏實證。💡 技術面：收 2,405 站回全部均線——RSI 53.0 回中軸上、MACD 黃金交叉（DIF -11.3 上穿 DEA -11.5、柱 +0.5 翻紅、零軸下首次）、KDJ 金叉延續（K 66.6／D 51.7）惟 J 96.3 偏高短線過熱；支撐 2,371／2,370（缺口）／2,352（60MA）／2,345（5MA）／2,320、壓力 2,415（8/5 高）／2,425（7/31 漲停）／2,522（布林上軌）／2,535（史高）。✅ 基本面錨未變：Q2 營收 $40.20B（+33.7% YoY）、毛利率 67.7%、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價傳漲。⭐ 今日（8/6）觀察：（1）韓股重挫＋美股回吐下、2330 守 2,370-2,371 缺口與否（守住則收復有效、上看 2,425；失守則 8/5 為單日行情）；（2）外資翻買延續性；（3）今晚 SOX 守 12,000 與 NVDA 動能；（4）232 條款／台美關稅。⚠️ 風險：KDJ J 值 96.3 短線過熱、投信連 8 買中止、韓股劇震外溢、232 條款結果偏鷹。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（8/6 Thu）關鍵觀察：（1）⚠️ 韓股今晨重挫：SK 海力士盤中 -6.83% 至 155.4 萬、三星 -2.64%（Yahoo 08:33 快照）——8/5 大漲一日即回吐、韓股高波動劇震再現，對今日台股半導體開盤情緒有直接壓力；2330 若跳空低開、首要觀察 2,370-2,371 缺口支撐；（2）⚠️ 高檔驗證日：8/5 大漲量僅 36,782 張（低於 5 日均量）、KDJ J 值 96.3 短線過熱、投信 -1,205 張連 8 買中止——若今日外資未能續買、單日翻買（+12,251 張）恐屬 ADR 溢價套利的一次性回補；（3）📊 美股 8/5 僅溫和回吐（SOX -1.40% 守 12,000、VIX 降至 15.81）、NVDA 逆勢創高 $219.22——暴漲結構未破壞；今晚 SOX 能否守 12,000 為次一驗證；（4）💱 匯率 32.23 微降（台幣小升）；232 條款懸而未決（8/6 晨查證仍未公布、未能證實）＋台美關稅無新進展——政策面仍是尾部變數；（5）⚠️ 中期壓力：SOX 自 6/22 史高 14,634.66 仍 -17.9%、7 月大盤月線收黑——反彈的中期確認仍需量能與外資連續回補。",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-08-05",       # 最新資料日（8/5 Wed、TWSE T86 官方）
    "foreign":  12251,             # 外資 8/5 買超 +12,251 張（終結連 2 賣、翻買——幾乎完全回補 8/4 的 -12,430）
    "trust":    -1205,             # 投信 8/5 賣超 -1,205 張（連 8 買中止、高檔獲利調節）
    "dealer":   -1099,             # 自營商 8/5 賣超 -1,099 張（連 3 賣、收斂）；三大法人合計 +9,948 張
    # 近5日（由舊到新：7/30, 7/31, 8/3, 8/4, 8/5）；TWSE T86 口徑（張）
    "foreign_5d": [-3958, 13780, -9717, -12430, 12251],  # 外資（賣→大買→連 2 賣→翻買、劇烈擺盪）
    "trust_5d":   [939, 7063, 387, 434, -1205],          # 投信（連 8 買後中止）
    "dealer_5d":  [1637, 2724, -1587, -2327, -1099],     # 自營（連 3 賣、量收斂）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       53.0,   "rsi_signal": "RSI(14) 53.0（官方收盤計算）——大漲 +3.66% 自 47.3 回升至 53.0、重返中軸上方——中性偏多；續站 50 上則修復確認",
    "macd":         0.5,   "macd_signal": "MACD 黃金交叉（DIF -11.3 上穿 DEA -11.5、柱狀圖 +0.5 翻紅、官方收盤計算）——綠柱連 4 日收斂後首度翻正、惟 DIF 仍在零軸下；DIF 上穿零軸為中期修復的下一確認",   "macd_color": "#4CAF50",
    "kdj_k":        66.6,   "kdj_d": 51.7, "kdj_j": 96.3, "kdj_signal": "KDJ K(66.6)/D(51.7)/J(96.3)（官方收盤計算）——金叉延續且開口擴大、K 站上 60、D 站回 50 上；⚠️ J 值 96.3 逼近 100 短線過熱、追高需留意",
    "ma5":       "2,345",   # 5MA（官方收盤計算至 8/5）
    "ma20":      "2,369",   # 20MA（官方收盤計算；8/5 收 2,405 收復 +1.52%）
    "ma60":      "2,352",  # 60MA（官方收盤計算；8/5 收盤收復季線——失守僅 1 日、中期架構修復）
    "ma120":     "2,157",  # 120MA（官方收盤計算至 8/5）
    "bb_upper":  "2,522",  # 布林上軌（20MA+2σ、官方收盤計算）
    "bb_mid":    "2,369",   # 布林中軌（20MA）
    "bb_lower":  "2,216",  # 布林下軌（20MA-2σ、官方收盤計算）
    "vol_ma5":  "46,773",   # 5日均量（張、TWSE 口徑；8/5 量 36,782 低於均量——大漲量未爆、惜售型上漲）
    "trend":    "⭐ 補漲兌現、季線失而復得：2330 8/5 官方收 NT$2,405（+85、+3.66%、開 2,385／高 2,415／低 2,370、量 36,782 張低於 5 日均量 46,773）——跳空收復 20MA 2,369 與 60MA 2,352、大盤 +2.89% 收 44,611.60。📊 技術指標（官方收盤計算）：RSI 53.0 重返中軸上、MACD 黃金交叉（DIF -11.3 上穿 DEA -11.5、柱 +0.5 翻紅、零軸下首次）、KDJ 金叉開口擴大（K 66.6／D 51.7）惟 J 96.3 短線過熱。✅ 籌碼：外資翻買 +12,251 張終結連 2 賣、全市場外資 +903.1 億近期最大回補；⚠️ 投信連 8 買中止（-1,205 張）。⚠️ 外部：美股 8/5 SOX -1.40% 回吐守 12,000、TSM ADR -0.76% 收 $414.00；今晨韓股重挫（SK 盤中 -6.83%）——今日開盤面臨情緒逆風。下一觀察：（1）守住 2,370-2,371 缺口則收復有效、上看 2,415／2,425；（2）失守缺口則 8/5 屬單日行情、回測 2,352（60MA）／2,345（5MA）；（3）外資翻買延續性與今晚 SOX 守 12,000 與否。",
    "support":  "2,371 / 2,370 / 2,352 / 2,345 / 2,320 (8/5 缺口上緣、8/5 低、60MA、5MA、8/4 收)",
    "resist":   "2,415 / 2,425 / 2,445 / 2,522 / 2,535 (8/5 高、7/31 漲停、7 月中平台、布林上軌、6/23 史高)",
    "note": "均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-05 止），請以看盤軟體為準；8/5 收 NT$2,405（+3.66%）站回全部均線（5MA 2,345／20MA 2,369／60MA 2,352／120MA 2,157）；RSI 53.0 回中軸上、MACD 黃金交叉（DIF -11.3／DEA -11.5、柱 +0.5 翻紅）、KDJ 金叉延續（K66.6/D51.7/J96.3 偏高）；支撐 2,371 / 2,370 / 2,352 / 2,345 / 2,320、壓力 2,415 / 2,425 / 2,445 / 2,522 / 2,535；✅ 外資翻買 +12,251 張＋MACD 金叉＋收復季線 vs ⚠️ J 值過熱＋投信中止連買＋今晨韓股重挫——今日守 2,370-2,371 缺口與否為檢核點",
}
...
DERIVATIVES = {
    "date":          "2026-08-05",       # 最新資料日（8/5 Wed 盤後估；官方以 TAIFEX 為準）
    "futures_price": "2,410",            # 台積電期貨近月（估、現貨 8/5 收 2,405、大漲日常見小幅正價差）
    "futures_oi":    "186,000",          # 未平倉量（口）（估、大漲日空單回補部位微減）
    "futures_basis": "+5",               # 估小幅正價差（補漲大漲、期貨略升水）
    "call_oi":       "119,000",
    "put_oi":        "107,000",
    "pcr":           "0.90",             # Put/Call 比率（估、大漲後 Put 避險需求下降）
    "pcr_signal":    "中性偏多（Put/Call 估 0.90；8/5 現貨大漲 +3.66% 收復季線、期權市場預期轉向上檔佈局；惟今晨韓股重挫、開盤避險買盤可能回升；期權數據以估算標註、請以 TAIFEX 為準）",
    "max_pain":      "2,400",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,320", "Call 33,000 / Put 13,000", "8/4 收、下檔支撐"),
        ("2,345", "Call 34,000 / Put 12,000", "5MA"),
        ("2,352", "Call 36,000 / Put 12,000", "60MA 季線"),
        ("2,371", "Call 36,000 / Put 11,000", "20MA、8/5 缺口上緣"),
        ("2,400", "Call 43,000 / Put 13,000", "整數關卡、估最大痛苦"),
        ("2,415", "Call 41,000 / Put 10,000", "8/5 盤中高"),
        ("2,425", "Call 40,000 / Put 9,000",  "7/31 漲停收盤"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實；價位錨點按 TWSE 官方收盤（8/5 收 NT$2,405）；估最大痛苦 2,400、下方支撐 2,371（缺口）/ 2,370（8/5 低）/ 2,352（60MA）/ 2,345（5MA）、上方壓力 2,415（8/5 高）/ 2,425（7/31 漲停）/ 2,522（布林上軌）/ 2,535（史高）；✅ 外資翻買 +12,251 張＋MACD 金叉＋收復季線 vs ⚠️ KDJ J 96.3 過熱＋今晨韓股重挫（SK 盤中 -6.8%）；✅ 基本面錨未變（2nm Q3 放量＋報價傳漲 10-20%、AI 需求實證、全年上修 >40%）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（台股個股為 2026-08-05 Wed TWSE/Yahoo 官方收盤；美股為 8/5 Wed 收盤、Yahoo Finance API 確認；韓股為 8/5 官方收盤、8/6 今晨盤中重挫）
    ("設備", "ASML",    "艾司摩爾",    "1,678.22", "-1.97%", "📊 EUV 設備龍頭；8/5 回吐 -1.97% 收 $1,678.22（8/4 +4.22% 暴漲後整理）；TSMC CapEx $60-64B 為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "534.24", "-2.26%", "📊 8/5 回吐 -2.26% 收 $534.24（8/4 創本輪新高後整理）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "307.42", "-3.25%", "📊 8/5 回吐 -3.25% 收 $307.42（8/4 +7.85% 暴漲後整理、7/30 財報行情高檔換手）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "192.80", "-1.36%", "📊 8/5 回吐 -1.36% 收 $192.80；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","169.00","+7.64%", "⭐ 8/5 續大漲 +7.64% 收 169.0（TWSE 官方、連 4 日大漲）；記憶體補漲主軸、與 TSMC 合作 CUBE 特殊 DRAM 題材"),
    ("記憶", "000660.KS","SK 海力士",  "1,668,000 KRW", "+5.77%", "⚠️ 8/5 大漲 +5.77% 收 166.8 萬韓元；今晨 8/6 開盤重挫 -6.83% 至 155.4 萬（Yahoo 08:33 快照）——市場關注股東回饋方案與高檔震盪；Q2 財報創史高＋HBM4 量產出貨基本面未變"),
    ("記憶", "MU",      "美光",        "893.19", "+0.06%", "✅ 8/5 持平 +0.06% 收 $893.19 守住 8/4 +7.62% 暴漲成果；HBM/DDR5、自 7/23 高 $990 收斂至 -9.8%"),
    ("封裝", "3711.TW", "日月光",      "593.00", "+1.37%", "✅ 8/5 反彈 +1.37% 收 593（TWSE 官方）；CoWoS 外包題材中長線未變、TSMC-Amkor 美國封裝長約對照"),
    ("封裝", "AMKR",    "Amkor",       "53.86",  "-3.60%", "📊 8/5 回吐 -3.60% 收 $53.86（連 5 漲後整理）；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材中長線未變"),
    ("客戶", "NVDA",    "輝達",        "219.22", "+3.44%", "⭐ TSMC #1 客戶 22%/$33B、A16 首發客戶；8/5 逆勢大漲 +3.44% 收 $219.22 連 5 日收高——SpaceX 宣布獨家採用 NVIDIA 處理器、2026 年底算力擴至 2GW+、Vera Rubin NVL72 獲點名"),
    ("客戶", "AAPL",    "蘋果",        "311.00", "+0.52%", "✅ #2 客戶 18%；8/5 續彈 +0.52% 收 $311.00 連 2 日上漲；CEO 交接（9/1 Ternus 接任）過渡期、「晶片供應吃緊」反證需求強勁"),
    ("客戶", "AVGO",    "博通",        "418.28", "+0.03%", "✅ #3 客戶；custom XPU、與 NVIDIA 包下 CoWoS 新產能；8/5 持平 +0.03% 收 $418.28 守住 8/4 +6.61% 暴漲成果"),
    ("客戶", "AMD",     "超微",        "482.05", "-7.04%", "📊 #4 客戶；8/5 財報後正式定價 -7.04% 收 $482.05——Q2 全面超標（營收 $11.53B +50%、資料中心 $6.7B +107%、Q3 指引 $13B）仍遭高估值賣壓（P/E 約 53 倍、市場「耳語」預期更高）；MI350 放量對 TSMC 為實質正面"),
    ("客戶", "2454.TW", "聯發科",      "4,000",  "+3.49%", "⭐ 手機 SoC 客戶 9%；8/5 大漲 +3.49% 收 4,000 整數關卡（Yahoo/TWSE）——V 型反轉後首度站上 4,000"),
    ("競爭", "INTC",    "英特爾",      "101.06", "+0.20%", "✅ 8/5 續漲 +0.20% 收 $101.06 守穩 $100 關卡（8/4 +10.84% 暴漲後）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "246,000 KRW", "+2.50%", "⚠️ 8/5 大漲 +2.50% 收 24.6 萬韓元；今晨 8/6 開盤回落 -2.64% 至 23.95 萬（Yahoo 08:33 快照）——韓股高檔劇震、CXMT 擴產疑慮未完全消化"),
    ("產業", "SOX",     "費城半導體指數","12,008.88", "-1.40%", "📊 8/5 回吐 -1.40% 收 12,008.88 守住 12,000 關卡（8/4 +6.55% 史詩級大漲後健康換手）——自 6/22 史高 14,634.66 仍 -17.9%；NVDA 逆勢創高、設備股回落 2-4%"),
    ("處分後", "ARM",   "Arm Holdings","274.58", "-2.13%", "TSMC 4/29 全數出脫後（8/5 回吐 -2.13% 收 $274.58——8/4 +17.36% 領漲後整理、高 Beta 雙向劇震）"),
]
...
RELATIVE = {
    "date":           "2026-08-05",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/5 Wed 收盤）
    "tsmc_tw_1d":     +3.66,   # 2330 8/5 官方收 NT$2,405（+85 vs 8/4 NT$2,320、TWSE）
    "taiex_1d":       +2.89,   # 加權指數 8/5 收 44,611.60（+1,250.94、TWSE 官方——補漲大反攻）
    "sox_1d":         -1.40,   # SOX 8/5 收 12,008.88（Yahoo、8/4 +6.55% 暴漲後回吐、守 12,000）
    "tsmc_nyse_1d":   -0.76,   # TSM ADR 8/5 $414.00（連 4 日收高中止、小幅回吐）
    "tsmc_tw_5d":     +9.07,   # 5 日（NT$2,405 vs 7/30 官方收 NT$2,205）
    "taiex_5d":       +11.72,  # （44,611.60 vs 7/30 39,933.30）
    "tsmc_tw_1m":     -0.82,   # 月線（NT$2,405 vs 7/31 月末官方收 NT$2,425）
    "taiex_1m":       +3.46,   # （44,611.60 vs 7/31 43,119.75、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +51.7,   # YTD 累計（NT$2,405 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +52.0,   # （44,611.60 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +63.0,   # SOX YTD（12,008.88 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/5 NYSE 收盤 & 8/5 TW 官方收盤計算）
    "adr_price":     414.00,   # TSM NYSE 8/5 收盤（-0.76%、Yahoo Finance API）
    "adr_parity":    373.09,   # 理論值 = 5 × NT$2,405 ÷ 32.23
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.23,   # 台幣匯率（8/6 晨 Yahoo TWD=X 32.231）
    "adr_premium_pct": "+10.97", # (414.00 - 373.09) / 373.09 × 100 = +10.97%（匯率 32.23）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/5 Wed 收盤：台股 NT$2,405（+3.66%、量 36,782 張 TWSE 口徑、市值 NT$62.4 兆）——補漲兌現、收復 20MA/60MA；加權指數同日大漲 +2.89% 收 44,611.60；NYSE $414.00（-0.76%、連 4 日收高中止）——SOX -1.40% 收 12,008.88 暴漲後回吐守 12,000。📊 籌碼面（T86 官方）：8/5 外資翻買 +12,251 張（終結連 2 賣）、投信 -1,205（連 8 買中止）、自營 -1,099、合計 +9,948 張；全市場 BFI82U 合計 +1,030.3 億（外資 +903.1 億近期最大回補、投信 +112.7 億、自營合計 +14.6 億）。📊 台美 ADR 溢價自 +16.38% 大幅收斂至 +10.97%（$414.00 vs 理論值 $373.09、匯率 32.23）——背離以「台股補漲＋ADR 微回」健康方式解除。⚠️ 外部：美股 8/5 半導體回吐整理（設備股 -2~-4%、NVDA 逆勢 +3.44% 創高、AMD 財報定價 -7.04%）；今晨韓股重挫（SK 盤中 -6.83%、三星 -2.64%）——今日情緒逆風。✅ 中長線結構性基底未變：全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶/A16 首發 + 2nm Q3 放量（報價傳漲 10-20%）+ CoWoS 訂滿至 2027",
}
...
NEWS = [
    {
        ...
        "title": "⭐ 台股 8/5 大反攻：2330 +3.66% 收復季線、外資 +903 億回補",
        "body": "補漲兌現、籌碼翻多：（1）⭐ 2330 跳空開 2,385、收 NT$2,405（+85、+3.66%）一舉收復 20MA 2,369 與 60MA 2,352（季線失守僅 1 日）、盤中高 2,415；量 36,782 張低於 5 日均量 46,773——惜售型上漲；（2）📈 大盤大漲 +1,250.94 點（+2.89%）收 44,611.60（FMTQIK 官方）——完全回應前日 SOX +6.55% 與 ADR 溢價 +16.4% 的補漲訊號；聯發科 +3.49% 站上 4,000、華邦電 +7.64% 連 4 大漲；（3）✅ 籌碼：外資翻買 2330 +12,251 張（終結連 2 賣、T86 官方）、三大合計 +9,948 張；全市場 BFI82U 合計 +1,030.3 億（外資 +903.1 億近期最大單日回補）——惟投信 -1,205 張連 8 買中止、翻買延續性待驗證。",
        "source": "TWSE 官方（STOCK_DAY/T86/BFI82U）2026-08-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "⭐ SpaceX 獨家採用 NVIDIA 處理器：NVDA 逆勢 +3.44% 收 $219.22",
        "body": "TSMC 第 1 大客戶再獲事件催化：（1）⭐ SpaceX 8/5 宣布將獨家採用 NVIDIA 處理器、馬斯克點名即將推出的 Vera Rubin NVL72 平台架構優於傳統機架式設計、並規劃 SpaceX 運算容量 2026 年底前擴增至 2GW 以上；（2）📈 NVDA 8/5 逆勢大漲 +3.44% 收 $219.22 連 5 日收高（Yahoo API 官方收盤）——在 SOX -1.40% 回吐日一枝獨秀、帶動道瓊走高；（3）🎯 對 TSMC 意涵：NVIDIA（22% 營收占比、$33B）的 Vera Rubin 世代採 TSMC N3＋CoWoS-L——SpaceX 級新需求方進場、AI 算力需求的廣度再獲驗證。",
        "source": "TradingKey / FXLeaders / Yahoo Finance API 2026-08-05", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "📊 AMD 財報後正式定價 -7.04% 收 $482.05：評價面修正",
        "body": "數字超標、估值賣壓：（1）📊 AMD（TSMC 第 4 大客戶）8/5 正常盤正式定價 8/4 盤後 Q2 財報：收 $482.05（-7.04%、Yahoo API 官方收盤）——Q2 營收 $11.53B（+50% YoY）創新高、EPS $1.66 超預期、資料中心 $6.7B（+107% YoY）、Q3 指引 $13B 遠超共識，數字全面超標仍遭賣壓；（2）⚠️ 原因為估值與預期：P/E 約 53 倍（高於 NVIDIA）、市場「耳語數字」預期更高、財報前一日 +7.00% 的高預期回吐；（3）🎯 對 TSMC 意涵：MI350 放量＋資料中心倍增以實際出貨驗證 AI 需求、對先進製程／CoWoS 稼動率為實質正面；跌勢屬評價面而非需求面。",
        "source": "CNBC / Yahoo Finance 2026-08-05", "impact": "中性",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "📊 2nm 報價傳漲 10-20%：自傳言 50% 收斂、議價力延續",
        "body": "先進製程定價力的最新訊號：（1）📊 市場傳言 TSMC 原規劃 2nm（N2）晶圓報價大漲 50% 至每片 3 萬美元、最新報導指實際漲幅將收斂至 10-20%，成熟與先進製程報價普遍調漲 5-10%；（2）⭐ 對照公司先前已確認的 3nm H2 漲 15%、7nm 以下全製程漲 5-10%——2nm 首發世代（Q3 放量、Apple/NVIDIA/AMD 排隊）續享溢價定價；（3）🎯 意涵：漲幅收斂反映與大客戶的長約協商現實、非需求轉弱；10-20% 漲幅仍直接挹注 2027 毛利率、緩衝 2nm 爬坡稀釋（H2 稀釋 3-4pp）。（漲價幅度屬市場報導、TSMC 未官方證實）",
        "source": "Yahoo Finance / Tom's Hardware 2026-08-05（報導、未官方證實）", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "✅ 熊本廠震後全面復產；ABF 基板產能傳鎖定至 2028",
        "body": "供應鏈兩則實證：（1）✅ TSMC 日本子公司 JASM 熊本一廠 7/28 遭 M7.1 強震一度停產檢查、8/3-8/4 確認已恢復正常生產——復原速度遠快於 2016 年熊本大地震（當年費時數月）；TSMC 並捐款 2.5 億日圓協助震災重建——供應中斷風險解除；（2）⭐ DIGITIMES 8/5 報導：NVIDIA、AMD 與雲端業者為應對 AI 晶片需求緊俏、已鎖定 ABF 基板產能至 2028 年、並要求供應商規劃 2029-2030 擴產；（3）🎯 意涵：上游材料被搶訂至 2028 是 AI 需求能見度的實體證據、與 CoWoS 訂滿至 2027 相互印證；ABF 供給為 TSMC 先進封裝擴產的外部瓶頸之一、需追蹤。",
        "source": "DIGITIMES / Taipei Times 2026-08-04~05", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⚠️ 韓股 8/5 大漲、今晨 8/6 重挫：SK 盤中 -6.8% 劇震再現",
        "body": "亞股半導體情緒的最大變數：（1）📈 韓股 8/5 大漲：SK 海力士 +5.77% 收 166.8 萬韓元（盤中一度 +7.9%）、三星 +2.50% 收 24.6 萬（Yahoo API 官方收盤）——市場預期 SK 海力士 ADR 售股靜默期結束後將公布股東回饋方案；同日兩公司執行長因績效獎金爭議遭南韓警方調查；（2）⚠️ 今晨 8/6 開盤重挫：SK 盤中 -6.83% 至 155.4 萬、三星 -2.64% 至 23.95 萬（Yahoo 08:33 快照）——8/5 漲幅一日回吐、高檔劇震再現（KOSPI 8/5 官方收盤 Yahoo 資料缺漏、未能證實）；（3）🎯 對台股意涵：韓股與台股半導體情緒高度連動、今日 2330 守缺口之戰的主要外部逆風。",
        "source": "Bloomberg / Yahoo Finance API 2026-08-05~06", "impact": "中性",
    },
]

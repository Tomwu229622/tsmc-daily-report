...
STOCK = {
    # 台股 2330
    "tw_price":       "1,850",        # 收盤價（數字加逗號，不含 NT$）（2026-03-20 估算）
    "tw_change":      "-55",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-2.89%",       # 漲跌幅（含+/-號）
    "tw_volume":      "30,200",       # 成交量（張）（估算）
    "tw_open":        "1,905",
    "tw_high":        "1,910",
    "tw_low":         "1,840",
    "market_cap":     "NT$48.0 兆",
    "pe_ratio":       "~27.9x",       # 台股 P/E（EPS TTM=66.25, P=1850）
    "pb_ratio":       "~8.88x",       # 台股 P/B（股淨比，估算）
    "div_yield":      "~1.24%",       # 殖利率（NT$23/NT$1,850）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "334.58",       # 收盤（2026-03-20 EST 估算）
    "nyse_change":    "-4.99",        # 較前一交易日（3/19 $339.57）下跌
    "nyse_change_pct":"-1.47%",
    "52w_high_nyse":  "390.20",       # 52 週高（2026-02-25）
    "52w_low_nyse":   "134.25",       # 52 週低（2025-04-07）
    "pe_nyse":        "~31.4x",       # NYSE USD 基礎 P/E（EPS TTM $10.66）
    "pb_nyse":        "~8.49x",
    "beta":           "1.56",
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-20 收盤（估算）  |  NYSE 數據：2026-03-20 盤中/收盤（EST）  |  前2月累計營收年增29.9%（NT$7,189億）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏空",
    # 一句話標題
    "headline":   "台股 2330 3/20 下跌 -2.89% 至 NT$1,850，氦氣供應危機疊加全球關稅憂慮壓制短線多頭",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330（3/20 估算）：收 NT$1,850（-55，-2.89%），成交量 30,200 張；NYSE TSM 同步下跌 -1.47%，本週累計走弱",
        "氦氣供應危機升溫：伊朗攻擊導致全球最大液化天然氣廠氦氣供應缺口 520 萬立方米/月，TSMC 被認為最易受影響，公司聲明庫存尚可管控",
        "Samsung 宣布 2026 年史上最大 $734 億 AI 晶片投資計劃，積極追趕台積電，HBM4 已進入量產，供應 NVIDIA/AMD",
        "半導體產業 2026 年料首破 $1 兆美元規模，AI 晶片貢獻 50% 總營收；TSMC 1-2 月銷售年增 30%，略低市場預估 33%",
        "4/16 Q1 財報倒計時 27 天，EPS 共識 NT$20.38；AGM 議案提交窗口 3/31-4/7，31 位分析師無一賣出，目標 NT$2,290",
    ],
    # 一句結論
    "bottom_line": "台股 2330 今日下跌 -2.89% 至 NT$1,850，主因氦氣供應風險升溫、美股廣泛修正情緒延續。短線技術面轉偏空（跌破 1,880 均線支撐），但中長線基本面穩健：Samsung 競爭加劇為雙面刃（亦顯示 AI 需求龐大），TSMC N2P 量產、CoWoS 滿載格局不變。建議關注 4/16 Q1 財報作為情緒修復催化劑，支撐區間 1,830-1,850。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ 伊朗衝突導致氦氣短缺：氦氣價格 5 日漲 13%，TSMC 聲明庫存可控，需持續追蹤",
}
...
RELATIVE = {
    "date":           "2026-03-20",
    "tsmc_tw_1d":     -2.89,   # 2330 3/20 估算（氦氣危機+關稅憂慮壓制）
    "taiex_1d":       -1.05,   # 加權指數（估算，台股同步走跌）
    "sox_1d":         -1.50,   # SOX（半導體股持續承壓）
    "tsmc_nyse_1d":   -1.47,   # TSM ADR 3/20 估算
    "tsmc_tw_5d":     -4.1,
    "taiex_5d":       -1.8,
    "tsmc_tw_1m":     -8.1,
    "taiex_1m":       -4.2,
    ...
    # ADR 折溢價
    "adr_price":     334.58,   # TSM NYSE 收盤（2026-03-20 EST 估算）
    "adr_parity":    292.26,   # 理論值 = 5 × NT$1,850 ÷ 31.65（台股 3/20 估算）
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        31.65,   # 同 MACRO 台幣匯率
    "adr_premium_pct": "+14.5",# (334.58 - 292.26) / 292.26 × 100
...
EPS_REVISION = {
    ...
    "analyst_view": "Q1 EPS 共識 NT$20.38，前 2 月累計營收年增 29.9%（略低市場預估 33%）。3/20 台股 2330 下跌 -2.89% 至 NT$1,850，主因氦氣供應危機（伊朗攻擊 Ras Laffan，每月缺口 520 萬立方米）疊加短線技術走弱。中長線基本面穩健：N2P 量產啟動、CoWoS 滿載、法人目標 NT$2,290（上行 +23.8%）。4/16 Q1 財報為關鍵催化劑，若毛利率超指引（63-65%）可望扭轉近期偏空情緒。",
...
NEWS = [
    {
        ...
        "title": "伊朗攻擊致氦氣短缺，TSMC 被列最高風險半導體廠",
        "body": "伊朗對拉斯拉凡（Ras Laffan）液化天然氣設施發動攻擊，導致全球最大氦氣生產基地自 3 月初起停產，每月缺口達 520 萬立方米。氦氣為半導體製造冷卻與清洗關鍵氣體，5 日內現貨價格上漲 13%。分析師點名 TSMC 為最易受影響廠商（先進製程用量最大），但台積電聲明：現有庫存與替代供應可應對短期衝擊，目前生產正常。若危機延續 2 個月以上，成本端壓力將逐步顯現，是當前最值得追蹤的供應鏈風險。",
        "source": "TipRanks / Reuters 2026-03-20", "impact": "負面",
    },
    {
        "tag": "競爭動態", "tag_color": "#F44336",
        "title": "Samsung 史上最大 $734 億 AI 晶片投資計劃，HBM4 量產搶攻 TSMC",
        "body": "Samsung Electronics 宣布 2026 年資本支出計劃達 $734 億美元（年增 22%），創公司史上最高，矢志在 AI 半導體市場超越台積電與 SK 海力士。記憶體方面：HBM4 已進入量產，速度達 11.7Gbps（超業界標準 8GB），並已取得供應 NVIDIA Groq 3 處理器合約；邏輯代工方面：德州 Taylor 廠（4nm）目標 2027 年量產。此舉雖顯示 AI 需求之龐大，但台積電技術領先至少 1-2 代，市佔差距（72% vs 7%）短期難以撼動。",
        "source": "Sammy Fans / Android Headlines 2026-03-19", "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "全球半導體 2026 年首破 $1 兆美元，AI 晶片佔 50% 總營收",
        "body": "DigiTimes 及 Omdia 最新報告指出，2026 年全球半導體產業產值將提前突破 $1 兆美元（年增 30.7%），遠超 2025 年預測時間表。AI 晶片雖僅占出貨量 0.2%，卻貢獻全行業約 50% 總營收，凸顯 AI 對半導體附加值的結構性重塑。計算與儲存段預計年增 41.4% 至逾 $5,000 億。全球 AI 資本支出 2026 年料達 $7,200 億，TSMC 估可受益每 $1 CapEx 中的 $0.22，長線訂單能見度極強。",
        "source": "DigiTimes / Omdia 2026-03-19", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "TSMC 1-2 月銷售年增 30%，略低市場 33% 預期；AI 需求仍強勁",
        "body": "TSMC 公告 2026 年 1-2 月累計合併營收年增 30%（較 2025 同期大幅成長），但略低於華爾街分析師普遍預期的 33%，主因 2 月有效工作天數較少及記憶體晶片需求放緩。儘管如此，N3/N5 先進製程稼動率維持 100% 滿載，NVIDIA GB300/GB400 系列及雲端 ASIC 訂單積壓量超 4 季。Bloomberg 報告強調此小幅落差不影響全年 30% 增長目標，短線市場情緒的波動為 Q1 財報前買點的重要參考。",
        "source": "Bloomberg / TSMC 月報 2026-03-10", "impact": "中性",
    },
    {

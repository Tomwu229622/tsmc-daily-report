...
STOCK = {
    # 台股 2330
    "tw_price":       "2,395",        # 台股 2330 最新確認收盤：8/11 Tue 官方收 NT$2,395（+15、+0.63% vs 8/10 NT$2,380、開 2,390／高 2,405／低 2,375、TWSE 確認）——量縮緩攻連 2 紅、收復 2,390 逼近 2,400 整數關卡；大盤同日 +191.96 點（+0.43%）收 45,120.72 續創 8 月反彈新高、2330 相對強勢
    "tw_change":      "+15",          # 8/11 Tue 收盤漲跌點數（+0.63%、TWSE 官方）
    "tw_change_pct":  "+0.63%",       # 8/11 Tue 量縮緩攻收中紅：開 2,390 盤中高 2,405 低 2,375、尾盤收 2,395——連 2 日收高（8/10 +0.42% 收 2,380）、站穩全部均線且 5MA 上彎；⚠️ 量能連 5 日萎縮至 18,248 張（5 日均量 25,296 的 72%、近月新低）；7 月營收 4,675.81 億大超預期＋8/11 董事會配息 7 元／Sony 合資雙利多支撐
    "tw_volume":      "18,248",       # 8/11 成交量（張、TWSE 口徑；成交額 NT$436.67 億、成交 50,065 筆——僅 5 日均量 25,296 張的 72%、連 5 日萎縮再創近月新低，無量緩漲為最大隱憂）
    "tw_open":        "2,390",        # 8/11 開盤（TWSE、連 3 日開在 2,390）
    "tw_high":        "2,405",        # 8/11 盤中高（TWSE、與 8/10 高 2,410 同為短線壓力區）
    "tw_low":         "2,375",        # 8/11 盤中低（TWSE、高於 5MA 2,383 下方不遠處獲支撐）
    "market_cap":     "NT$62.1 兆",   # 8/11 收盤市值（NT$2,395 基礎、約 259.3 億股）
    "pe_ratio":       "32.20x",       # 台股 P/E（TWSE 官方 BWIBBU 8/11、口徑為最新年度 EPS）
    "pb_ratio":       "10.54x",       # 台股 P/B（TWSE 官方 BWIBBU 8/11、以 115Q1 財報淨值計）
    "div_yield":      "0.92%",        # 殖利率（TWSE 官方 BWIBBU 8/11、以 114 年度股利計）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "422.06",       # 8/11 Tue 官方收盤 $422.06（+$3.59、+0.86% vs 8/10 $418.47、Yahoo API 確認）——創本波收盤新高（超越 8/7 $420.04）；8/10 曾隨 SOX -2.94% 回檔 -0.37%、8/11 半導體反彈中領先大盤（S&P 同日 -0.32%）
    "nyse_change":    "+3.59",        # 8/11 漲跌（美元、自我檢核：418.47 × 1.0086 = 422.07 ≈ 422.06 ✓）
    "nyse_change_pct":"+0.86%",       # 8/11 逆大盤上漲（S&P -0.32%）、創本波收盤新高
    ...
    "data_note":      '報告日期：2026-08-12 Wed（最新確認收盤：台股 2330 8/11 Tue 官方收 NT$2,395（+15、+0.63% vs 8/10 NT$2,380、開 2,390／高 2,405／低 2,375、量 18,248 張 TWSE 口徑、成交額 NT$436.67 億、成交 50,065 筆、TWSE 確認）；NYSE TSM 8/11 Tue 收 $422.06（+$3.59、+0.86% vs 8/10 $418.47、Yahoo API 確認、創本波收盤新高））。⚠️ 註：8/11 排程未執行、本日報一併補記 8/10（Mon）行情——2330 8/10 收 2,380（+10、+0.42%、開 2,390／高 2,410／低 2,380、量 21,498 張、額 NT$513.85 億）；大盤 8/10 大漲 +702.85 點（+1.59%）收 44,928.76——2330 當日補漲落後、資金衝記憶體與封裝（華邦電 +9.79%、日月光 +7.69%）。📊 今日格局：基本面雙重利多落地、量縮緩攻逼近 2,400——（1）⭐ 7 月營收大超預期：NT$4,675.81 億（+44.69% YoY、+5.62% MoM、8/10 盤後公布）連 3 個月改寫單月新高、優於投顧預期的 4,500 億；1-7 月累計 NT$2.87 兆（+37.0% YoY）——Q3 美元指引 $44.6-45.8B 達成無虞；（2）⭐ 8/11 董事會四大議案：Q2 配息 NT$7.0/股（12/10 除息、2027-01-07 發放、首次提供外資美元領息選項）、核准資本預算約 US$294.4 億（先進製程／先進封裝／廠房建設）、與 Sony 半導體合資 7,470 億日圓於熊本設新公司開發次世代影像感測器（台積電出資 2,820 億日圓約 NT$572 億、2029 年量產）、通過 Q2 財報（營收 NT$1.270 兆、稅後純益 NT$7,065.6 億、H1 EPS NT$49.33）；（3）📊 台股連 2 紅、量能連 5 日萎縮：8/11 收 2,395 收復 2,390、逼近 2,400 整數與 2,405/2,410 短線壓力；量 18,248 張僅 5 日均量 25,296 的 72%（近月新低）——無量緩漲、追價意願仍待量能確認；大盤 8/11 +0.43% 收 45,120.72 續創 8 月反彈新高；（4）✅ 籌碼續翻多：2330 外資連 3 買且明顯加碼（8/7 +642→8/10 +119→8/11 +3,498 張）、投信 +382、自營 -189、三大合計 +3,691 張（T86 官方）；全市場 BFI82U 8/10 大買 +736.1 億（外資 +517.4 億）、8/11 續買 +280.1 億（外資 +223.3 億）——資金全面回流台股；（5）📊 美股 8/10 半導體回檔、8/11 反彈：SOX 8/10 -2.94% 收 11,993.86（油價上漲通膨疑慮＋AI 資本支出持續性雜訊、NVDA -2.86%）、8/11 +0.87% 收 12,098.47——FT 報導 NVIDIA 攜華爾街大行籌措 $5,000 億 AI 基建資金帶動情緒回穩；設備股領漲（ASML +3.80% 收 $1,799.38、KLAC +4.01% 收 $200.47 站上 $200、LRCX +1.64%）；TSM 8/11 +0.86% 收 $422.06 創本波收盤新高（大盤 S&P -0.32% 收 7,728.20、那指 -0.60%、VIX 15.28）——半導體相對強勢；（6）✅ 韓股今晨（8/12）全面大漲：三星 +3.55% 至 24.8 萬韓元、SK 海力士 +2.32% 至 145.8 萬、KOSPI +2.06% 至 6,476（盤中快照）——記憶體行情重啟、與台股記憶體（華邦電 8/10 +9.79%）相互印證，半導體風險偏好回暖。📊 ADR 溢價自 +14.28% 收斂至 +13.56%（$422.06 vs 理論值 $371.66、匯率 32.22）——以台股補漲方式收斂、符合歷史型態。技術面（官方收盤計算至 8/11）：收 2,395 站上全部均線且 5MA 上彎——RSI 52.5 中軸上方續升、MACD 黃金交叉續強（DIF -1.36 逼近零軸／DEA -7.58、柱狀圖 +12.43 連 5 日擴張：+0.53→+3.68→+6.31→+9.09→+12.43）、KDJ 金叉延續（K 79.3／D 71.0、J 96.0 偏高）；支撐 2,390（連 3 日開盤）／2,383（5MA）／2,375（8/11 低）／2,361（60MA）／2,358（20MA）／2,320、壓力 2,400（整數）／2,405（8/11 高）／2,410（8/10 高）／2,425（7/31 漲停）／2,497（布林上軌）／2,535（史高）。✅ 基本面錨未變：Q2 大超預期（營收 $40.20B +33.7% YoY、EPS $4.31/ADR、毛利率 67.7%）、全年上修 >40%、CapEx $60-64B、2nm Q3 放量＋報價漲 10-20%、7 月營收 4,675.81 億再驗證。⭐ 今日（8/12 Wed）觀察：（1）2,400 整數與 2,405-2,410 壓力區攻防——量能能否終結連 5 縮為關鍵、無量突破持續性存疑；（2）韓股今晨大漲＋美股半導體反彈——半導體情緒回暖能否帶動台股放量；（3）📅 美國 7 月 CPI 今晚（美東 8/12）公布——短線最大宏觀變數、8/10 美股回檔即因通膨疑慮；（4）外資連 3 買加碼後的延續性；（5）232 條款半導體專章仍未公布（未能證實）。',
}
...
SUMMARY = {
    ...
    "headline":   '⭐ 基本面雙重利多落地、量縮緩攻逼近 2,400：7 月營收 NT$4,675.81 億（+44.69% YoY）連 3 月創單月新高、優於投顧預期；8/11 董事會通過 Q2 配息 7 元（首推美元領息）＋US$294.4 億資本預算＋Sony 熊本合資案；2330 8/11 收 NT$2,395（+0.63%）連 2 紅收復 2,390、外資連 3 買且加碼至 +3,498 張；TSM ADR 8/11 +0.86% 收 $422.06 創本波收盤新高、韓股今晨大漲（三星 +3.55%）——⚠️ 惟量能連 5 日萎縮至 18,248 張近月新低、今晚美國 7 月 CPI 為短線最大變數',

    "bullets": [
        '⭐ 7 月營收大超預期：NT$4,675.81 億（+44.69% YoY、+5.62% MoM、8/10 盤後公布）——連 3 個月改寫單月歷史新高、優於投顧預期的 4,500 億；1-7 月累計 NT$2.87 兆（+37.0% YoY）、Q3 美元指引 $44.6-45.8B 達成無虞——AI/HPC 需求動能再獲月營收驗證',
        '⭐ 8/11 董事會四大議案齊發：Q2 配息 NT$7.0/股（12/10 除息、2027-01-07 發放、⭐ 首次提供外資美元領息選項、ADR 適用）；核准資本預算約 US$294.4 億（先進製程／先進封裝擴產）；與 Sony 半導體合資 7,470 億日圓於熊本設新公司（台積電出資 2,820 億日圓約 NT$572 億）開發次世代影像感測器、2029 量產；通過 Q2 財報：營收 NT$1.270 兆、稅後純益 NT$7,065.6 億、H1 EPS NT$49.33',
        '📊 台股連 2 紅、量縮緩攻：8/10 +0.42% 收 2,380（大盤大漲 +1.59% 收 44,928.76、2330 補漲落後——資金衝記憶體與封裝：華邦電 +9.79%、日月光 +7.69%）；8/11 +0.63% 收 2,395（開 2,390／高 2,405／低 2,375）收復 2,390、逼近 2,400 整數關卡；大盤 +0.43% 收 45,120.72 續創 8 月反彈新高、2330 相對強勢',
        '⚠️ 量能連 5 日萎縮為最大隱憂：8/11 成交僅 18,248 張（5 日均量 25,296 張的 72%、再創近月新低、連 5 縮：36,782→25,537→24,414→21,498→18,248）、成交額 NT$436.67 億——無量緩漲、突破 2,400-2,410 壓力區需量能配合，今日能否終結連 5 縮為關鍵檢核點',
        '✅ 籌碼續翻多、外資連 3 買且加碼：2330 8/11 外資 +3,498 張（連 3 買：+642→+119→+3,498、明顯加碼）、投信 +382 張（連 3 買）、自營 -189、三大合計 +3,691 張（T86 官方）；全市場 BFI82U 8/10 大買 +736.1 億（外資 +517.4 億、終結 8/7 的 -428.9 億大賣）、8/11 續買 +280.1 億（外資 +223.3 億）——資金全面回流台股',
        '📊 美股 8/10 回檔、8/11 反彈：SOX 8/10 -2.94% 收 11,993.86（油價上漲通膨疑慮＋AI 資本支出持續性雜訊、NVDA -2.86%）→ 8/11 +0.87% 收 12,098.47（FT 報導 NVIDIA 攜華爾街大行籌 $5,000 億 AI 基建資金、設備股領漲：ASML +3.80%、KLAC +4.01% 站上 $200）；TSM 8/11 +0.86% 收 $422.06 創本波收盤新高——S&P 同日 -0.32% 收 7,728.20、VIX 15.28，半導體相對強勢；ADR 溢價自 +14.28% 收斂至 +13.56%（台股補漲收斂、符合歷史型態）',
        '✅ 韓股今晨（8/12）全面大漲、記憶體行情重啟：三星 +3.55% 至 24.8 萬韓元、SK 海力士 +2.32% 至 145.8 萬、KOSPI +2.06% 至 6,476（盤中快照）；台股記憶體已先行（華邦電 8/10 +9.79% 收 179.5、外資 2 日回補 +56,916 張）——半導體風險偏好全面回暖、有利台股半導體族群情緒',
        '📅 事件行事曆：美國 7 月 CPI 今晚（美東 8/12）公布——短線最大宏觀變數（8/10 美股回檔即因油價通膨疑慮）；Q2 股利除息 12/10；Q3 法說會 10 月中旬；232 條款半導體專章結果仍未公布（未能證實）；2nm Q3 放量＋報價漲 10-20%（TrendForce）',
    ],

    "bottom_line": '報告日期：2026-08-12 Wed（最新確認收盤：台股 2330 8/11 Tue 官方收 NT$2,395（+15、+0.63% vs 8/10 NT$2,380、開 2,390／高 2,405／低 2,375、量 18,248 張、額 NT$436.67 億、50,065 筆、TWSE 確認）；NYSE TSM 8/11 收 $422.06（+0.86%、創本波收盤新高、Yahoo API 確認）；8/11 排程未執行、本日報一併補記 8/10 行情）。📊 核心判讀：基本面雙重利多落地、價格結構轉強、惟量能與 CPI 是兩大檢核點——（1）⭐ 7 月營收 NT$4,675.81 億（+44.69% YoY、+5.62% MoM）連 3 月創高、優於投顧預期 4,500 億：1-7 月累計 2.87 兆（+37.0%）、Q3 指引達成無虞——8/10 盤後公布、8/11 的 +0.63% 已部分反映；（2）⭐ 8/11 董事會：Q2 配息 NT$7.0（12/10 除息、首推外資美元領息）、資本預算 US$294.4 億延續擴產節奏、Sony 熊本合資（7,470 億日圓、台積電 2,820 億日圓、2029 量產次世代影像感測器）——特殊製程佈局補強、股利政策穩定；（3）📊 價格結構轉強：連 2 紅收 2,395、收復 2,390 且站上全部均線（5MA 2,383 上彎／20MA 2,358／60MA 2,361／120MA 2,177）、大盤續創 8 月反彈新高 45,120.72；⚠️ 量能連 5 日萎縮至 18,248 張（5 日均量的 72%、近月新低）——8/10 大盤大漲 702 點時 2330 僅 +0.42%（資金衝記憶體／封裝補漲）、8/11 相對轉強但無量，突破 2,400-2,410 壓力區需量能回升確認；（4）✅ 籌碼結構明確翻多：外資連 3 買且 8/11 加碼至 +3,498 張、投信連 3 買、三大合計 +3,691 張；全市場外資 8/10 +517.4 億大買、8/11 +223.3 億續買——與 8/7 前的調節完全反轉；（5）📊 美股節奏：8/10 半導體回檔（SOX -2.94%、油價通膨疑慮＋AI 資本支出雜訊）、8/11 反彈（SOX +0.87%、FT 報導 NVIDIA 攜華爾街大行籌 $5,000 億 AI 基建資金）；TSM +0.86% 收 $422.06 創本波收盤新高且逆大盤（S&P -0.32%）——ADR 溢價收斂至 +13.56%、以台股補漲方式進行中；（6）✅ 韓股今晨大漲（三星 +3.55%、SK +2.32%、KOSPI +2.06%）——記憶體行情重啟、半導體風險偏好回暖。💡 技術面：RSI 52.5 中軸上方續升、⭐ MACD 柱 +12.43 連 5 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43）且 DIF -1.36 逼近零軸——上穿零軸將為中期修復的關鍵確認；KDJ 金叉延續（K 79.3／D 71.0）惟 J 96.0 偏高、短線過熱；支撐 2,390／2,383（5MA）／2,375（8/11 低）／2,361（60MA）／2,358（20MA）／2,320、壓力 2,400（整數）／2,405（8/11 高）／2,410（8/10 高）／2,425（7/31 漲停）／2,497（布林上軌）／2,535（史高）。⭐ 今日（8/12 Wed）觀察：（1）2,400-2,410 攻防與量能——放量突破則上看 2,425、無量則提防回測 2,383-2,375 支撐帶；（2）韓股大漲＋美股半導體反彈的外溢效應；（3）📅 今晚美國 7 月 CPI——若超預期恐引發獲利了結、低於預期則降息預期升溫；（4）外資加碼延續性（連 4 買將確立中期翻多）。⚠️ 風險：量能近月新低的無量緩漲結構、KDJ J 96.0 過熱、CPI 變數、AI 資本支出持續性雜訊（8/10 美股回檔主因）、232 條款懸而未決。',

    "risk_alert":  '今日（8/12 Wed）關鍵觀察：（1）⭐ 2,400 整數與 2,405-2,410 壓力區攻防：7 月營收大超預期＋董事會利多＋TSM ADR 創本波收盤新高（$422.06、溢價 +13.56%）＋韓股今晨大漲——開高機率高；⚠️ 但量能連 5 日萎縮至 18,248 張（5 日均量 25,296 的 72%、近月新低），無量突破持續性存疑、今日能否終結連 5 縮為第一檢核點；（2）📅 美國 7 月 CPI 今晚（美東 8/12）公布——短線最大宏觀變數：8/10 美股回檔即因油價上漲引發通膨疑慮（SOX -2.94%、NVDA -2.86%），若 CPI 超預期、今晚美股回檔將壓抑明日台股；（3）✅ 外資連 3 買且加碼（+642→+119→+3,498 張）——連 4 買將確立中期翻多；全市場外資 2 日 +740.7 億回流，延續性為中期關鍵；（4）✅ 記憶體行情重啟：韓股今晨三星 +3.55%、SK +2.32%；台股華邦電 8/10 +9.79%（外資 2 日回補 +56,916 張）——半導體情緒回暖、惟留意資金分流效應（8/10 大盤 +1.59% 時 2330 僅 +0.42% 即為資金衝記憶體所致）；（5）💱 匯率 32.22 持平；232 條款半導體專章結果仍未公布（未能證實）——尾部風險；AI 資本支出持續性雜訊反覆（FT 報導 NVIDIA 籌 $5,000 億 AI 基建資金暫時緩解）；（6）⚠️ 中期：KDJ J 96.0 偏高短線過熱；SOX 自 6/22 史高 14,634.66 仍 -17.3%；2330 自 6/23 史高 2,535 仍 -5.5%——中期反彈確認仍需量能配合、MACD DIF 上穿零軸為下一關鍵訊號。',
}
...
INSTITUTIONAL = {
    ...
    "trust":    382,
    "dealer":   -189,
    # 近 5 交易日（8/5, 8/6, 8/7, 8/10, 8/11）
    "foreign_5d": [12251, -4011, 642, 119, 3498],
    "trust_5d":   [-1205, -82, 61, 153, 382],
    "dealer_5d":  [-1099, -583, 1243, 448, -189],
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       52.5,   "rsi_signal": 'RSI(14) 52.5（官方收盤計算）——連 2 日上漲後自 50.6 續升至 52.5、脫離中軸糾纏——中性偏多；持續墊高顯示修復動能延續、尚遠離 70 超買區',
    "macd":         12.43,   "macd_signal": 'MACD 黃金交叉續強（DIF -1.36 逼近零軸、DEA -7.58、柱狀圖 +12.43、官方收盤計算）——⭐ 紅柱連 5 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43）、中期動能持續修復；DIF -1.36 距零軸僅一步之遙、上穿零軸將為中期修復的關鍵確認訊號',   "macd_color": "#4CAF50",
    "kdj_k":        79.3,   "kdj_d": 71.0, "kdj_j": 96.0, "kdj_signal": 'KDJ K(79.3)/D(71.0)/J(96.0)（官方收盤計算）——金叉延續、K 與 D 同步走高進入高檔區；⚠️ J 值 96.0 偏高（自 91.7 續升）、短線過熱未消化，無量追高需留意回檔風險',
    "ma5":       "2,383",
    "ma10":      "2,344",
    "ma20":      "2,358",
    "ma60":      "2,361",
    ...
    "bb_lower":  "2,218",
    "vol_ma5":  "25,296",
    "trend":    '📊 連 2 紅收復 2,390、無量緩攻逼近 2,400：2330 8/11 官方收 NT$2,395（+15、+0.63%、開 2,390／高 2,405／低 2,375、量 18,248 張僅 5 日均量 25,296 張的 72%、近月新低）——連 2 日收高（8/10 +0.42% 收 2,380）、站上全部均線且 5MA 2,383 上彎；大盤 +0.43% 收 45,120.72 續創 8 月反彈新高、2330 相對強勢（8/10 大盤 +1.59% 時 2330 僅 +0.42% 補漲落後、8/11 反轉為相對強）。⭐ 技術指標（官方收盤計算）多頭訊號續強：MACD 黃金交叉——DIF -1.36 逼近零軸、DEA -7.58、柱狀圖 +12.43 連 5 日擴張（+0.53→+3.68→+6.31→+9.09→+12.43）；RSI 52.5 中軸上方續升、KDJ 金叉延續（K 79.3／D 71.0）惟 J 96.0 偏高。✅ 籌碼續翻多：外資連 3 買且加碼（+642→+119→+3,498 張）、投信連 3 買（+61→+153→+382）、三大 8/11 合計 +3,691 張；全市場 BFI82U 8/10 +736.1 億（外資 +517.4 億大買）、8/11 +280.1 億——資金全面回流。⭐ 基本面雙重利多：7 月營收 4,675.81 億（+44.69% YoY）連 3 月創高＋8/11 董事會（配息 7 元／US$294.4 億資本預算／Sony 合資）。📊 外部：美股 8/10 回檔 8/11 反彈（SOX +0.87% 收 12,098.47、設備股領漲）、TSM +0.86% 收 $422.06 創本波收盤新高——ADR 溢價收斂至 +13.56%；韓股今晨大漲（三星 +3.55%、SK +2.32%）。下一觀察：（1）2,400 整數與 2,405-2,410 壓力區攻防——放量突破則上看 2,425／2,497（布林上軌）、量能能否終結連 5 縮為關鍵；（2）無量衝高則提防回測 2,390／2,383（5MA）／2,375 支撐帶；（3）📅 今晚美國 7 月 CPI 為短線最大宏觀變數；（4）MACD DIF 上穿零軸為中期修復的下一確認。',
    "support":  "2,390 / 2,383 / 2,375 / 2,361 / 2,358 / 2,320 (連 3 日開盤區、5MA、8/11 低、60MA 季線、20MA、8/4 收)",
    "resist":   "2,400 / 2,405 / 2,410 / 2,425 / 2,497 / 2,535 (整數關卡、8/11 高、8/10 高、7/31 漲停、布林上軌、6/23 史高)",
    "note": '均線與 RSI/MACD/KDJ/布林均依官方收盤價計算（2026-08-11 止），請以看盤軟體為準；8/11 收 NT$2,395（+0.63%）連 2 紅、站上全部均線（5MA 2,383 上彎／20MA 2,358／60MA 2,361／120MA 2,177）；RSI 52.5 續升、MACD 黃金交叉續強（DIF -1.36 逼近零軸／DEA -7.58、柱 +12.43 連 5 日擴張）、KDJ 金叉延續（K79.3/D71.0/J96.0 偏高）；支撐 2,390 / 2,383 / 2,375 / 2,361 / 2,358 / 2,320、壓力 2,400 / 2,405 / 2,410 / 2,425 / 2,497 / 2,535；⭐ 7 月營收大超預期＋董事會利多＋外資連 3 買加碼＋MACD 柱連 5 日擴張 vs ⚠️ 量能連 5 縮近月新低（18,248 張、72% 均量）＋J 值 96.0 偏高＋今晚美國 CPI——今日檢核點為「量能能否終結連 5 縮、放量突破 2,400-2,410」',
}
...
DERIVATIVES = {
    "date":          "2026-08-11",
    "futures_price": "2,398",      # 台積電股票期貨 CDF 202608 8/11 日盤收盤（TAIFEX 官方、+11、+0.46%、結算價 2,397、量 5,528 口）
    "futures_oi":    "15,602",     # ⭐ CDF 202608 未沖銷契約數（口、TAIFEX 官方；原 184,000 為估算值、已更正）
    "futures_basis": "+3",         # 期貨 2,398 - 現貨 2,395（正價差）；⭐ 夜盤（8/11 晚→今晨）收 2,403、正價差擴大至 +8
    "call_oi":       "83,111",     # ⭐ 台指選擇權 TXO 買權未平倉量（TAIFEX 官方 8/11；原 124,000 為估算值、已更正）
    "put_oi":        "102,705",    # ⭐ 台指選擇權 TXO 賣權未平倉量（TAIFEX 官方 8/11；原 113,000 為估算值、已更正）
    "pcr":           "1.24",       # ⭐ Put/Call 未平倉量比率 123.58%（TAIFEX 官方 8/11）——連 3 日上升（8/7 106.42% → 8/10 117.38% → 8/11 123.58%）；原估算 0.91 已更正
    "pcr_signal":    '偏多（Put/Call 未平倉比 1.24、TAIFEX 官方數據；連 3 日上升 106.42%→117.38%→123.58%）——⭐ 賣權未平倉量 102,705 口明顯高於買權 83,111 口，代表下檔賣權佈局積極、市場對回檔的保護需求高於追價，一般解讀為支撐強於壓力；⭐ 台指期夜盤（8/11 晚→今晨）收 45,279（+194 點、+0.43%、量 36,518 口）、台積電個股期貨夜盤收 2,403（+6）對現貨 2,395 呈正價差 +8——今日開盤偏多；⚠️ 惟夜盤盤中震盪 690 點（高低 45,520／44,830）、波動明顯放大；📊 三大法人台指期未平倉淨額：外資淨空 -88,924 口、投信淨多 +83,418 口、自營淨多 +1,899 口——多空大致對沖、屬結構性避險部位（投信為 ETF 避險、外資為現貨多單對沖），不宜單獨解讀為方向性看法',
    "max_pain":      "2,400",
    "key_strikes": [
        ("2,344", "Call 39,000 / Put 11,000", "10MA"),
        ("2,358", "Call 38,000 / Put 12,000", "20MA、布林中軌"),
        ("2,361", "Call 37,000 / Put 12,000", "60MA 季線"),
        ("2,375", "Call 40,000 / Put 12,000", "8/11 盤中低"),
        ("2,383", "Call 41,000 / Put 11,000", "5MA"),
        ("2,390", "Call 43,000 / Put 13,000", "連 3 日開盤區"),
        ("2,400", "Call 45,000 / Put 15,000", "整數關卡、估最大痛苦"),
        ("2,405", "Call 42,000 / Put 11,000", "8/11 盤中高"),
        ("2,410", "Call 41,000 / Put 10,000", "8/10 盤中高"),
        ("2,415", "Call 40,000 / Put 9,000",  "8/5 盤中高"),
        ("2,425", "Call 39,000 / Put 8,000",  "7/31 收盤"),
        ("2,535", "Call 30,000 / Put 5,000",  "6/23 史高、終極反壓"),
    ],
    "note": '⭐ 期貨與 Put/Call 未平倉數據已改用 TAIFEX 官方數據（2026-08-11、來源 dlFutDataDown／pcRatioDown）：台積電股票期貨（CDF 202608）日盤收 2,398（+11、+0.46%、量 5,528 口、結算價 2,397、未沖銷 15,602 口）對現貨 2,395 呈正價差 +3；夜盤（8/11 晚→今晨、TAIFEX 盤後時段）收 2,403（+6、+0.25%、量 1,465 口）正價差擴大至 +8。台指期（TX 202608）日盤收 45,090（+100、+0.22%、未沖銷 99,034 口）、夜盤收 45,279（+194、+0.43%、量 36,518 口、盤中高低 45,520／44,830 震盪 690 點）。台指選擇權未平倉：買權 83,111 口／賣權 102,705 口、Put/Call 比 1.24（連 3 日上升）。⚠️ 惟 key_strikes 各履約價的 Call/Put 口數仍為估算（台積電個股選擇權流動性低、TAIFEX 未逐檔提供彙總），僅作支撐壓力參考、請以 taifex.com.tw 為準；價位錨點按 TWSE 官方收盤（8/11 收 NT$2,395）。⭐ MACD 柱連 5 日擴大（+12.43）＋DIF -1.36 逼近零軸＋外資連 2 買 +3,498 張＋7 月營收 NT$4,675.81 億創新高（+44.69% YoY）＋夜盤正價差 vs ⚠️⚠️ 量能連 4 日萎縮至今年新低（18,248 張、僅 72% 均量）＋KDJ J 值偏高＋美國 7 月 CPI 今晚公布；✅ 基本面錨強化（2nm Q3 放量＋報價漲 10-20%、CoWoS 訂滿至 2027、全年上修 >40%）',
}
# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # (分類, 代號, 名稱, 最新價, 漲跌幅, 說明) — 美股為 8/11 Tue 收盤（Yahoo API）、台股為 8/11 TWSE 官方收盤、韓股為 8/11 收盤＋8/12 晨盤快照
    ("設備", "ASML",    "艾司摩爾",    "1,799.38", "+3.80%", "⭐⭐ EUV 設備龍頭；8/11 大漲 +3.80% 收 $1,799.38 創歷史新高、逼近 $1,800（8/10 僅 -0.43% 抗跌）；TSMC CapEx $60-64B 與 2nm/A16 擴產為中長線支撐"),
    ("設備", "AMAT",    "應用材料",    "525.61", "+0.67%", "✅ 8/11 反彈 +0.67% 收 $525.61（8/10 重挫 -3.16%）；AMAT-TSMC $5B EPIC Center 研發合作為中長線題材"),
    ("設備", "LRCX",    "科林研發",    "311.41", "+1.63%", "✅ 8/11 反彈 +1.63% 收 $311.41 收復 8/7 價位（8/10 -1.59%）；TSMC CapEx + WFE 上修為中長線支撐"),
    ("設備", "KLAC",    "科磊",        "200.47", "+4.01%", "⭐⭐ 8/11 領漲設備股 +4.01% 收 $200.47 首度站上 $200 整數關卡（8/10 -2.71%）；量測設備、2nm 量產驅動中長線"),
    ("記憶", "2344.TW", "華邦電 Winbond","178.00","-0.84%", "📊 8/11 小幅回檔 -0.84% 收 178.0（TWSE 官方）——8/10 暴漲 +9.79% 收 179.5（收復 8/7 -4.39% 跌幅並創波段高）後正常獲利了結；與 TSMC 合作 CUBE 特殊 DRAM 題材延續"),
    ("記憶", "000660.KS","SK 海力士",  "1,425,000 KRW", "+0.35%", "📊 8/11 小漲 +0.35% 收 142.5 萬韓元（Yahoo 官方收盤序列、8/10 -0.14% 打底）；⭐ 今晨 8/12 大漲 +2.25% 至 145.7 萬——記憶體族群重回攻勢；高盛估 2027 HBM 均價年增約 100%"),
    ("記憶", "MU",      "美光",        "868.52", "+0.87%", "✅ 8/11 反彈 +0.87% 收 $868.52（8/10 -1.89%）；HBM/DDR5 需求延續、自 7/23 高 $990 收斂至 -12.3%"),
    ("封裝", "3711.TW", "日月光",      "629.00", "-0.16%", "📊 8/11 幾近平盤 -0.16% 收 629（TWSE 官方）——8/10 暴漲 +7.69% 收 630 創波段新高後高檔整理；⭐ 傳台積電擬購友達中科兩座舊面板廠擴充 CoWoS 產能（未經官方證實）帶動封裝族群"),
    ("封裝", "AMKR",    "Amkor",       "54.30",  "+2.71%", "✅ 8/11 大漲 +2.71% 收 $54.30（8/10 -4.34%）；TSMC-Amkor 10 年美國封裝長約、Peoria AZ 題材中長線未變"),
    ("客戶", "NVDA",    "輝達",        "217.50", "-0.02%", "📊 TSMC #1 客戶、A16 首發客戶；8/11 持平 -0.02% 收 $217.50——8/10 重挫 -2.86%（與 Apollo／貝萊德／黑石／Brookfield／高盛／KKR 籌組逾 5,000 億美元 AI 運算融資平台、市場質疑「循環融資」，CDS 創兩週最大單日升幅；黃仁勳澄清此非 NVIDIA 營收亦非對單一客戶承諾）；已自 8/7 史高 $223.96 回落 -2.9%；占 TSMC 營收比重約 19% 為最大客戶"),
    ("客戶", "AAPL",    "蘋果",        "304.91", "-1.09%", "⚠️ 主要客戶（比重約 17%）；8/11 連 2 跌 -1.09% 收 $304.91（8/10 -1.62%）、自 8/7 本波高 $313.33 回落 -2.7%——大型科技股同步走弱；CEO 交接（9/1 Ternus 接任）過渡期"),
    ("客戶", "AVGO",    "博通",        "416.08", "-1.50%", "⚠️ #3 客戶；8/11 連 2 跌 -1.50% 收 $416.08（8/10 -1.25%）、自 8/7 本波高 $427.76 回落 -2.7%；custom XPU、與 NVIDIA 包下 CoWoS 新產能之中長線題材未變"),
    ("客戶", "AMD",     "超微",        "474.32", "+1.01%", "✅ #4 客戶；8/11 反彈 +1.01% 收 $474.32（8/10 -2.86%）；MI455X（2nm＋HBM4）發表、資料中心年增逾 50%——訂單實質對 TSMC 為正面"),
    ("客戶", "QCOM",    "高通",        "162.68", "+0.31%", "📊 手機/PC SoC 客戶；8/11 小漲 +0.31% 收 $162.68（8/10 -3.39% 回吐 8/7 的 +4.66% 漲幅）"),
    ("客戶", "2454.TW", "聯發科",      "4,020",  "+1.52%", "⭐ 手機 SoC 客戶 9%；8/11 續漲 +1.52% 收 4,020（TWSE 官方）——連 2 日上漲並站上 4,000 整數關卡（8/10 +1.54%）"),
    ("競爭", "INTC",    "英特爾",      "97.71", "+0.19%", "📊 8/11 止穩 +0.19% 收 $97.71——8/10 重挫 -4.06% 失守 $100（宣布加碼股票籌資 150 億美元、隔日再擴大至 200 億美元、引發稀釋疑慮，為 8/10 費半重挫主因之一）；亦為 TSMC 客戶"),
    ("競爭", "005930.KS","三星電子",   "239,500 KRW", "+4.13%", "⭐⭐ 8/11 大漲 +4.13% 收 23.95 萬韓元（8/10 -0.43%）；⭐ 今晨 8/12 續漲 +3.55% 至 24.8 萬——記憶體族群強勢回歸、KOSPI 今晨 +2.06%"),
    ("產業", "SOX",     "費城半導體指數","12,098.47", "+0.87%", "📊 8/11 逆勢反彈 +0.87% 收 12,098.47（大盤三大指數同步收跌）——設備股領漲；⚠️ 8/10 重挫 -2.94% 收 11,993.86 失守 12,000（Intel 籌資稀釋＋NVIDIA 循環融資疑慮）；自 6/22 史高 14,634.66 仍 -17.3%"),
    ("處分後", "ARM",   "Arm Holdings","268.93", "+0.40%", "📊 TSMC 4/29 全數出脫後（8/11 小漲 +0.40% 收 $268.93；8/10 重挫 -5.21%、高 Beta 特性延續）"),
]
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "12,098.47", "+0.87%", True),   # 8/11 收（Yahoo）逆勢反彈、收復 12,000；自 6/22 史高 14,634.66 收斂至 -17.3%
    ("台股加權指數",    "45,120.72", "+0.43%", True),   # 8/11 官方收盤（+191.96 點、TWSE FMTQIK）為 7/16 以來收盤新高、惟距 6/22 峰值 47,741.51 仍 -5.49%
    ("VIX 恐慌指數",    "15.28",    "-1.16%", True),    # 8/11 收（Yahoo）低位持穩、風險偏好未惡化
    ("S&P 500",         "7,728.20", "-0.32%", False),   # 8/11 收（Yahoo ^GSPC、連 2 日收跌、自 8/7 收盤高 7,757.64 回落）
    ("那斯達克指數",    "26,445.45","-0.60%", False),   # 8/11 收（Yahoo ^IXIC、連 2 日收跌、大型科技股走弱）
    ("台幣/美元",       "~32.22",   "持平",  False),    # 8/12 晨 Yahoo TWD=X 32.223（每升值 1% 毛利率 -40bps）
    ("NVIDIA NVDA",     "$217.50", "-0.02%",  False),   # #1 TSMC 客戶（約 19%），8/11 持平、自 8/7 史高回落 -2.9%
    ("ASML",            "$1,799.38", "+3.80%",  True),  # 設備龍頭 8/11 創歷史新高（Yahoo）
]
...
VALUATION = {
    "pe_current":   32.20,  # TWSE 官方 BWIBBU（8/11 收 NT$2,395；口徑為最新年度 EPS、與 TTM 估算不同）
    ...
    "pb_current":  10.54,   # TWSE 官方 BWIBBU（8/11、以 115Q1 財報淨值計；先前 ~18x 為估算、以官方為準）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
...
RELATIVE = {
    "date":           "2026-08-11",       # 最新資料日（台股 2330 與 NYSE TSM 均為 8/11 Tue 收盤）
    "tsmc_tw_1d":     +0.63,   # 2330 8/11 官方收 NT$2,395（+15 vs 8/10 NT$2,380、TWSE）
    "taiex_1d":       +0.43,   # 加權指數 8/11 收 45,120.72（+191.96、TWSE FMTQIK 官方）
    "sox_1d":         +0.87,   # SOX 8/11 收 12,098.47（Yahoo、逆勢反彈、收復 12,000）
    "tsmc_nyse_1d":   +0.86,   # TSM ADR 8/11 $422.06（+$3.59、創本波收盤新高、Yahoo API）
    "tsmc_tw_5d":     +3.23,   # 5 日（NT$2,395 vs 8/4 官方收 NT$2,320）
    "taiex_5d":       +4.06,   # （45,120.72 vs 8/4 43,360.66）
    "tsmc_tw_1m":     -1.24,   # 月線（NT$2,395 vs 7/31 月末官方收 NT$2,425）
    "taiex_1m":       +4.64,   # （45,120.72 vs 7/31 43,119.75、TWSE FMTQIK 官方）
    "tsmc_tw_ytd":    +51.1,   # YTD 累計（NT$2,395 vs 1/2 官方收 NT$1,585、TWSE STOCK_DAY）
    "taiex_ytd":      +53.7,   # （45,120.72 vs 1/2 官方收 29,349.81、FMTQIK）
    "sox_ytd":        +64.2,   # SOX YTD（12,098.47 vs 1/2 收 7,367.47、Yahoo）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 8/11 NYSE 收盤 & 8/11 TW 官方收盤計算）
    "adr_price":     422.06,   # TSM NYSE 8/11 收盤（+0.86%、創本波收盤新高、Yahoo API 確認）
    "adr_parity":    371.66,   # 理論值 = 5 × NT$2,395 ÷ 32.22
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.22,   # 台幣匯率（8/12 晨 Yahoo TWD=X 32.223）
    "adr_premium_pct": "+13.56", # (422.06 - 371.66) / 371.66 × 100 = +13.56%（匯率 32.22）
    "note": 'ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。台股 2330 與 NYSE TSM 均為 8/11 Tue 收盤：台股 NT$2,395（+0.63%、量 18,248 張 TWSE 口徑為今年以來新低、市值 NT$62.1 兆）——連 3 日收高、站上全部均線；加權指數同日 +0.43% 收 45,120.72（為 7/16 以來收盤新高、自 7/28 低點反彈 +8.45%，惟距 6/22 峰值 47,741.51 仍 -5.49%）；NYSE $422.06（+0.86%、創本波收盤新高）——SOX +0.87% 收 12,098.47 逆勢反彈（大盤三大指數同步收跌）。⭐⭐ 基本面：7 月合併營收 8/10 公布 NT$4,675.81 億（官方 MOPS 申報 467,580,548 千元、+5.62% MoM、+44.69% YoY）連 3 個月創單月新高、遠超投顧原估的 4,500 億；累計 1-7 月 NT$2.8721 兆（+37.01% YoY）。📊 籌碼面（T86 官方）：8/11 外資大買 +3,498 張（連 2 買、為 8/5 以來最大單日回補）、投信 +382（連 3 買）、自營 -189、三大合計 +3,691 張；8/10 亦全面小買（合計 +720 張）；全市場 BFI82U 連 2 日全面買超（8/11 +280.1 億：外資 +223.3 億、投信 +32.2 億、自營合計 +24.6 億；8/10 +736.1 億）——與 8/7 的 -428.9 億大賣完全反轉。📊 台美 ADR 溢價自 +14.28% 收斂至 +13.56%（$422.06 vs 理論值 $371.66、匯率 32.22）——台股連 3 日補漲已部分收斂溢價、背離健康化，惟仍達兩位數、續有補漲空間。⚠️⚠️ 最大保留：2330 成交量連 4 日萎縮（36,782→25,537→24,414→21,498→18,248 張）、8/11 僅為 5 日均量 25,296 張的 72% 且為今年以來最低——「價漲量縮」背離，反彈由賣壓稀薄而非買盤積極推動。⭐ 外部：台指期夜盤（8/11 晚→今晨）收 45,279（+194 點、TAIFEX 官方）、台積電個股期貨夜盤正價差 +8；韓股記憶體今晨大漲（三星 +3.55%、SK +2.25%、KOSPI +2.06%）。✅ 中長線結構性基底未變：7 月營收創新高 + 全年上修 >40% + CapEx $60-64B + NVIDIA #1 客戶（約 19%）/A16 首發 + 2nm Q3 放量（報價漲 10-20%）+ CoWoS 訂滿至 2027',
}
...
NEWS = [
    {
        "tag": "公司公告", "tag_color": "#FF9800",
        "title": "⭐⭐ 7 月營收 NT$4,675.81 億創新高、年增 44.69%：連 3 個月改寫紀錄、大超投顧預期",
        "body": '基本面獲官方數據強力確認：（1）⭐⭐ 台積電 8/10 公布 7 月合併營收 NT$4,675.81 億（官方 MOPS 申報值 467,580,548 千元），較 6 月的 NT$4,426.80 億月增 +5.62%、較去年同期 NT$3,231.66 億年增 +44.69%——連續第 3 個月改寫單月歷史新高；（2）⭐ 遠超市場預期：8/10 前投顧預估上看 NT$4,500 億，實際數字超出約 3.9%；累計 1–7 月營收 NT$2.8721 兆（去年同期 NT$2.0962 兆、+37.01% YoY）；（3）💡 關鍵判讀：在 6 月極高基期（NT$4,426.80 億、+67.9% YoY）之上仍能月增 5.62%，說明 AI/HPC 拉貨動能未見鈍化——年增率自 6 月的 +67.9% 回落至 +44.69% 屬基期效應而非需求轉弱，不宜誤讀為動能衰退；（4）⚠️ 惟須留意指引達成度：Q3 美元營收指引 $44.6-45.8B（中位 $45.2B）以匯率 32.2 換算月均約需 NT$4,851 億，7 月的 4,675.81 億仍低約 3.6%——8-9 月需再加速方能達成指引中位，為市場下一個檢核點。',
        "source": "公開資訊觀測站 MOPS 官方月營收申報（t21sc03_115_7）／TSMC 官方 IR（pr.tsmc.com）",
        "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⚠️ 台股連 3 日收高但量能萎縮至今年新低：2330 收 2,395、成交僅 18,248 張",
        "body": '價漲量縮背離為當前最大隱憂：（1）📊 2330 自 8/7 起連 3 日收高——8/10 收 2,380（+10、+0.42%）、8/11 收 2,395（+15、+0.63%、開 2,390／高 2,405／低 2,375），漲幅逐日放大（+5→+10→+15）、低點逐日墊高（2,355→2,380→2,375）、站上全部均線（5MA 2,383／10MA 2,344／20MA 2,358／60MA 2,361／120MA 2,177）；（2）⚠️⚠️ 但成交量連 4 日萎縮：36,782→25,537→24,414→21,498→18,248 張，8/11 僅為 5 日均量 25,296 張的 72%、為今年以來最低；成交額僅 NT$436.67 億、成交筆數 50,065 亦同步萎縮——代表上漲主要來自「賣壓稀薄、賣方縮手」而非「買盤積極承接」；（3）💡 型態判讀：上方 2,405（8/11 高）／2,415（8/5 高）／2,425（7/31 收）為連續密集套牢區，無量上攻的突破成功率偏低；反之若量能回升至 3 萬張以上並站穩 2,415，可視為有效突破；（4）📊 大盤 8/10 +702.85 點（+1.59%）、8/11 +191.96 點（+0.43%）收 45,120.72——為 7/16 以來收盤新高、自 7/28 低點 41,603.36 反彈 +8.45%，⚠️ 惟距 6/22 峰值 47,741.51 仍 -5.49%，本波屬反彈修復而非創高突破。',
        "source": "TWSE 台灣證券交易所（STOCK_DAY／FMTQIK 官方 API）",
        "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 籌碼大幅翻多：外資 8/11 大買 2330 逾 3,498 張、三大法人合計 +3,691 張",
        "body": '資金回流權值訊號明確：（1）⭐ 2330 8/11 三大法人全面翻多——外資大買 +3,498 張（連 2 買、為 8/5 以來最大單日回補）、投信 +382 張（連 3 買）、自營小賣 -189 張，三大合計買超 +3,691 張（T86 官方）；（2）📊 8/10 亦為全面小買：外資 +119、投信 +153、自營 +448、合計 +720 張——外資自 8/7 的 +642 張起已連 3 日站在買方；（3）⭐ 全市場 BFI82U 連 2 日全面買超：8/11 合計 +280.1 億（外資 +223.3 億、投信 +32.2 億、自營自行 +10.6 億、自營避險 +14.1 億）、8/10 合計 +736.1 億——與 8/7 的 -428.9 億大賣（外資 -407.2 億）完全反轉，資金自中小型與記憶體回流台股權值；（4）📊 台股供應鏈同步走強：聯發科 8/11 +1.52% 收 4,020 連 2 漲並站上 4,000 關卡、日月光 8/10 暴漲 +7.69% 收 630 創波段新高、華邦電 8/10 +9.79% 收 179.5；（5）💡 觀察：外資能否延續為連 3 買為今日重點。',
        "source": "TWSE 台灣證券交易所（T86／BFI82U／STOCK_DAY 官方 API）",
        "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 美股 8/10 半導體重挫 -2.94%：Intel 加碼籌資至 200 億、NVIDIA「循環融資」爭議",
        "body": '兩則籌資面利空引發賣壓、8/11 情緒緩和：（1）⚠️ Intel 宣布透過發行股票籌資 150 億美元（隔日再擴大至 200 億美元），市場擔憂股權稀釋——INTC 8/10 重挫 -4.06% 失守 $100 收 $97.52，為費半當日重挫主因之一；（2）⚠️ NVIDIA 宣布攜手 Apollo、貝萊德、黑石、Brookfield、高盛、KKR 籌組獨立「運算融資平台」、目標調動逾 5,000 億美元第三方資本建置 AI 基礎設施——市場質疑資本被用於「資助自身 AI 客戶」構成循環融資，NVDA 8/10 重挫 -2.86%、其 CDS 價格創兩週最大單日升幅；黃仁勳隨後於社群平台澄清此非 NVIDIA 營收、亦非對單一客戶的承諾；（3）📊 SOX 8/10 因此重挫 -2.94% 收 11,993.86 失守 12,000，設備股全面下跌（AMAT -3.16%、QCOM -3.39%、KLAC -2.71%）；（4）✅ 8/11 情緒緩和、半導體逆勢反彈 SOX +0.87% 收 12,098.47——NVDA 持平 -0.02%、INTC 止穩 +0.19%；（5）💡 對 TSMC 的判讀：屬客戶端財務結構與市場情緒議題、非晶圓需求變化，TSM 8/10 僅 -0.37%、8/11 +0.86% 創本波收盤新高即為分流定價驗證。',
        "source": "unwire.pro／mashdigi／經濟日報／Yahoo Finance API",
        "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 美股 8/11 分歧：半導體逆勢反彈、KLAC 站上 $200／ASML 創新高，惟三大指數同步收跌",
        "body": '資金在科技股內部分流：（1）✅ 半導體逆勢反彈：SOX +0.87% 收 12,098.47 收復 12,000——設備股領漲，KLAC +4.01% 收 $200.47 首度站上 $200 整數關卡、ASML +3.80% 收 $1,799.38 創歷史新高、LRCX +1.63%、AMAT +0.67%、AMKR +2.71%；TSM +0.86% 收 $422.06 創本波收盤新高；AMD +1.01%、MU +0.87% 同步反彈；（2）⚠️ 惟大盤三大指數同步收跌：S&P 500 -0.32% 收 7,728.20（連 2 跌）、那斯達克 -0.60% 收 26,445.45（連 2 跌）、道瓊 -0.34% 收 53,791.85——大型科技股走弱，AAPL 連 2 跌 -1.09% 收 $304.91（自 8/7 高 $313.33 回落 -2.7%）、AVGO 連 2 跌 -1.50% 收 $416.08、NVDA 持平 -0.02% 收 $217.50（自 8/7 史高 $223.96 回落 -2.9%）；（3）💡 判讀：市場並非全面 risk-on，而是資金自大型科技股輪動至半導體設備與製造——對 TSMC 屬相對有利的結構，但需留意大盤走弱若延續將壓抑整體評價；VIX 15.28（-1.16%）仍屬低位、恐慌未升溫；（4）📊 SOX 自 6/22 史高 14,634.66 仍 -17.3%。',
        "source": "Yahoo Finance API（^SOX／^GSPC／^IXIC／^DJI／^VIX 官方收盤序列）",
        "impact": "中性",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "⭐ 台指期夜盤 +194 點、韓股記憶體今晨大漲：三星 +3.55%、KOSPI +2.06%",
        "body": '今日開盤外部條件偏多但震盪加大：（1）⭐ 台指期夜盤（8/11 晚間至今晨、TAIFEX 官方盤後時段）收 45,279（+194 點、+0.43%、成交 36,518 口）——對現貨 45,120.72 呈正價差；⚠️ 惟盤中高低 45,520／44,830 震盪達 690 點，波動明顯放大；（2）⭐ 台積電個股期貨（CDF 202608）夜盤收 2,403（+6、+0.25%、量 1,465 口）對現貨 2,395 呈正價差 +8；日盤收 2,398（+11、+0.46%、結算價 2,397、未沖銷 15,602 口）；（3）⭐ 韓股記憶體今晨延續強勢：三星電子 +3.55% 至 248,000 韓元、SK 海力士 +2.25% 至 1,457,000 韓元、KOSPI +2.06% 至 6,476.55（Yahoo 今晨快照）——延續 8/11 的三星 +4.13%；高盛預估 2027 年三星、SK 海力士 HBM 平均售價將分別年增約 87%、100%、每 Gb 價格上看 2.9 美元；（4）📊 台指選擇權未平倉 Put/Call 比 1.24（賣權 102,705 口／買權 83,111 口、TAIFEX 官方）連 3 日上升（106.42%→117.38%→123.58%）——下檔保護佈局積極、一般解讀為支撐強於壓力。',
        "source": "TAIFEX 台灣期貨交易所（dlFutDataDown／pcRatioDown 官方）／Yahoo Finance API",
        "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "📊 傳台積電擬購友達中科兩座舊面板廠擴充 CoWoS 產能（未經官方證實）",
        "body": '先進封裝產能持續擴張的市場傳聞：（1）📊 市場 8/10 傳出台積電計畫收購友達光電位於中科的兩座舊世代面板廠，交易金額傳超過新台幣 300 億元——若成真將是近年本土面板廠出售舊廠的最高價；（2）📊 模式比照此前收購群創南科四廠：面板廠的無塵室、廠務系統與大面積廠房可較低成本改建為先進封裝產線，用於擴充 CoWoS 等 AI 封裝產能；（3）📊 消息激勵友達、群創股價 8/10 雙雙攻上漲停，封裝族群同步走強（日月光 8/10 +7.69% 收 630 創波段新高）；（4）⚠️ 重要聲明：此為媒體報導的市場傳聞，台積電、友達均未發布正式公告——標示「未經官方證實」，投資判斷不宜以此為據；（5）💡 產業意涵：無論此案是否成真，反映的結構性事實是台灣工業用地稀缺、而 CoWoS 先進封裝產能仍供不應求（TrendForce 估 2026 年底月產能上看 ~35,000 片、+59% YoY、訂單已排至 2027）。',
        "source": "今周刊／Newtalk／聯合新聞網（市場傳聞、未經台積電或友達官方證實）",
        "impact": "正面",
    },
    {
        "tag": "法規政策", "tag_color": "#FF9800",
        "title": "📅 美國 7 月 CPI 今晚 20:30 公布：本週最大宏觀變數、影響降息路徑",
        "body": '今日盤後最關鍵的外部事件：（1）📅 美國 7 月消費者物價指數（CPI）將於美東時間 8/12 上午（台北時間 8/12 晚間 20:30）公布，7 月生產者物價指數（PPI）續於 8/13 公布——為本週最重要的宏觀數據；（2）💡 影響路徑：CPI 直接影響 Fed 降息預期，而降息路徑決定高估值科技股（含 AI 資本支出概念）的評價倍數——若通膨數據高於預期、將壓抑對利率敏感的 AI 基礎設施投資評價（8/11 Oracle、Amazon 等雲端資本支出概念股即因利率敏感度下跌逾 2%）；（3）📊 對台股的傳導：數據於台股收盤後公布，影響將反映在 8/13（四）的開盤與台指期夜盤；（4）⚠️ 當前背景：VIX 15.28 處於低位，代表市場對此數據的預期偏樂觀——若出現意外，低波動環境下的反應幅度可能被放大；（5）📊 另：232 條款半導體專章的調查結果與具體稅率，截至 8/10-11 兩日仍無可證實的新進展（標示「未能證實」）、為懸而未決的尾部風險。',
        "source": "美國勞工統計局（BLS）行事曆／Yahoo Finance／經濟日報",
        "impact": "中性",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "1,840",        # 收盤價（數字加逗號，不含 NT$）（2026-03-24 收盤）
    "tw_change":      "-25",          # 漲跌點數（含+/-號）
    "tw_change_pct":  "-1.34%",       # 漲跌幅（含+/-號）
    "tw_volume":      "27,000",       # 成交量（張，估）
    "tw_open":        "1,855",
    "tw_high":        "1,860",
    "tw_low":         "1,835",
    "market_cap":     "NT$46.94 兆",
    "pe_ratio":       "~31.2x",       # 台股 P/E（TTM，2026-03-24）
    "pb_ratio":       "~8.0x",        # 台股 P/B（股淨比）
    "div_yield":      "~0.97%",       # 殖利率（USD $2.65/ADR 年息）
    "52w_high_tw":    "2,025",        # 52 週高（2026-02-25）
    "52w_low_tw":     "780",          # 52 週低（2025-04-07）
    # NYSE TSM ADR
    "nyse_price":     "338.02",       # 盤中（2026-03-24 EST 盤中）
    ...
    # 資料日期備註
    "data_note":      "台股數據：2026-03-24 收盤  |  NYSE 數據：2026-03-24 盤中  |  2月營收年增35.9%（HPC佔58%）  |  股東人數達246萬人歷史高位",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性偏多",
    # 一句話標題
    "headline":   "台股 2330 小幅收黑 -1.34%，NYSE TSM 反彈 +2.67%；全球半導體 2026 破兆美元，AI 佔 50%",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "台股 2330（3/24 收盤）：NT$1,840（-25，-1.34%）；NYSE TSM 盤中 US$338.02（+8.78，+2.67%），兩市背離",
        "NVIDIA 確認超越 Apple 成 TSMC 最大客戶：2026 年貢獻 ~$330 億美元（22%），AI GPU 訂單積壓超 4 季",
        "全球半導體 2026 年首破 $1 兆美元（年增 30.7%），AI 晶片貢獻逾 50% 總營收，TSMC 維持 72% 代工市佔",
        "中國出口管制 + 伊朗衝突致鎵/氦氣原料短缺，TSMC 聲明庫存可控，惟若危機延續 2 個月以上將壓縮毛利",
        "4/16 Q1 財報倒計時 23 天，EPS 共識 NT$20.38；AGM 議案窗口 3/31-4/7；31 位分析師無一賣出",
    ],
    # 一句結論
    "bottom_line": "台股 2330 今日收 NT$1,840（-1.34%），短線持續整理。NYSE TSM 盤中反彈至 $338.02（+2.67%），顯示美國市場對 AI 需求信心回升。2 月營收年增 35.9%（HPC 佔 58%）基本面亮眼，NVIDIA 成最大客戶等長線利多不變。原料供應風險（鎵、氦氣）為近期主要不確定因素。AGM 提案窗口（3/31-4/7）與 Q1 財報（4/16）為近期焦點，技術支撐 1,830-1,840。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "⚠️ AGM 提案截止日 3/31-4/7；Q1 財報 4/16 倒計時 23 天；鎵/氦氣供應危機持續追蹤中",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-03-24",
    "foreign":  -3200,    # 外資（負=賣超）— 延續近期調節（估）
    "trust":    +350,     # 投信（逢低承接）
    "dealer":   -180,     # 自營商（避險小賣）
    # 近5日（由舊到新）
    "foreign_5d": [-3800, +2800, -6500, -4200, -3200],
    "trust_5d":   [+320,  +350,  +280,  +310,  +350],
    "dealer_5d":  [-280,  -120,  -320,  -250,  -180],
    "foreign_ownership_pct": "75.3%",  # 外資持股比例（估，賣超後微降）
    "foreign_ownership_shares": "約 195.3 億股",
}

# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       42.8,   "rsi_signal": "偏空（持續低於 50 中軸，超賣區接近）",
    "macd":         -4.1,   "macd_signal": "MACD 持續死叉，下行動能未止",   "macd_color": "#F44336",
    "kdj_k":        36.2,   "kdj_d": 43.8, "kdj_j": 21.0, "kdj_signal": "死亡交叉持續，超賣警示",
    "ma5":       "1,858",
    "ma20":      "1,910",
    "ma60":      "1,840",
    "ma120":     "1,735",
    "bb_upper":  "2,020",   # 布林上軌
    "bb_mid":    "1,910",   # 布林中軌（20MA）
    "bb_lower":  "1,800",   # 布林下軌
    "vol_ma5":  "27,000",   # 5日均量（張）
    "trend":    "台股 2330 於 3/24 收 NT$1,840（-1.34%），量能縮小至 27,000 張。RSI 42.8 持續低於 50，MACD 死叉未見好轉，短線仍偏弱。收盤逼近 MA60（1,840）支撐，若跌破則下看布林下軌 1,800。NYSE 反彈 +2.67% 是正面訊號，有助帶動台股隔日氣氛。",
    "support":  "1,830–1,840",
    "resist":   "1,860–1,875",
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "4,120",   "+1.7%",  True),   # 3/24 反彈
    ...
    ("NVIDIA NVDA",     "$855",    "+1.5%",  True),   # GTC 後持續強勢
...
NEWS = [
    {
        "tag": "財報", "tag_color": "#2196F3",
        "title": "TSMC 2月營收年增35.9%，HPC佔比升至58%創新高",
        "body": "台積電2月合併營收年增35.9%，高效能運算（HPC/AI）業務年增48%，佔總營收比重升至58%，遠超行動裝置業務（約28%）。前2月累計營收年增30%，Q1 財報（4/16）前市場情緒偏樂觀。31位分析師共識平均目標 NT$2,290（現價 +24.5%），共識 EPS NT$20.38，公司連續8季超預期。",
        "source": "Bloomberg / TSMC 2026-03-10", "impact": "正面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "NVIDIA 正式超越 Apple 成 TSMC 最大客戶，貢獻 $330 億美元",
        "body": "NVIDIA CEO 黃仁勳確認，2026 年 NVIDIA 將以約 $330 億美元（佔 TSMC 總收入 22%）超越 Apple（約 $270 億，18%），成為台積電史上最大單一客戶。AI GPU 需求爆發：GB300/GB400 採 N3P、CoWoS-L，訂單積壓超 4 季。Feynman 架構（2028 年）確定採 A16（1.6nm），長達 4 年以上的深度合作確認。",
        "source": "CNBC / eWeek 2026-01-26", "impact": "正面",
    },
    {
        "tag": "產品發布", "tag_color": "#9C27B0",
        "title": "TSMC A16 製程首發確認為 NVIDIA，Apple 跳過直攻 A14",
        "body": "TSMC 最先進 A16 製程（配備 BPR 背面電源供電）首批客戶確認為 NVIDIA，用於 Feynman 架構 GPU（2028 年量產）；Apple 則跳過 A16 直接採用 A14 節點，顯示兩大客戶策略分歧。此確認強化 TSMC 先進製程壟斷地位，Intel Foundry 與 Samsung 在此高端市場幾乎無緣分食。",
        "source": "Tweaktown 2026-03", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "全球半導體 2026 年首破 $1 兆美元，AI 晶片佔逾 50% 總營收",
        "body": "分析機構預測 2026 年全球半導體市場規模突破 $1 兆美元（年增 30.7%），其中 AI 相關晶片雖僅佔出貨量 0.2%，卻貢獻逾 50% 總行業營收，凸顯 AI 對附加值的結構性重塑。TSMC 維持全球代工 72% 市佔率，N3/N5 稼動率 ~100%。全球 AI 資本支出 2026 年料達 $7,200 億。",
        "source": "Digitimes / Deloitte 2026-03-19", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "中國出口管制 + 伊朗局勢推升鎵與氦氣原料價格，威脅半導體製造",
        "body": "中國收緊鎵出口管制加上伊朗衝突導致卡達 LNG 設施停產引發氦氣短缺，鎵價格突破每公斤 $2,000（翻倍），氦氣現貨價 5 日漲 13%。兩者皆為半導體先進製程關鍵材料。TSMC 聲明現有庫存可控、生產正常，但業界正加速備庫存；若危機延續 2 個月以上，成本壓力將逐步顯現。",
        "source": "Digitimes / ICO Optics 2026-03-23", "impact": "負面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "TSMC 股東人數突破 246 萬人歷史高位，週增 7.5 萬人顯示散戶長期信心",
        "body": "儘管台股 2330 近期下跌，台積電持股人數仍創新高達 246 萬，較前一週增加逾 75,536 人，在近期下跌行情中逆勢成長，顯示散戶在 NT$1,840 附近視為長線佈局機會。AGM 議案提交窗口 3/31-4/7（ADS 持有人透過花旗銀行），Q1 財報 4/16 倒計時 23 天。",
        "source": "Taipei Times 2026-03-23", "impact": "中性",
    },
]

...
STOCK = {
    # 台股 2330
    "tw_price":       "2,415.5",      # 台股 2330 最新確認收盤：7/9 Thu 官方收 NT$2,415.5（-50、-2.03% vs 7/8 NT$2,465、TWSE）——外資大賣拖累；⚠️ 7/10 Fri 因颱風「巴威」TWSE 全日休市（未交易）；7/13 Mon 台股復市、TSMC 6 月營收今日 13:30 公布（延自 7/10）
    "tw_change":      "-50",          # 7/9 Thu 收盤漲跌點數（-2.03%、TWSE 官方）
    "tw_change_pct":  "-2.03%",       # 7/9 Thu 收盤漲跌幅（TWSE；外資賣超 12,749 張、提款 310 億）
    "tw_volume":      "27,057",       # 7/9 Thu 官方成交量（張、成交額 NT$658.50 億、TWSE）
    "tw_open":        "2,450",        # 7/9 開盤
    "tw_high":        "2,460",        # 7/9 盤中高
    "tw_low":         "2,415",        # 7/9 盤中低（收在低檔附近）
    "market_cap":     "NT$62.6 兆",   # 7/9 收盤市值（NT$2,415.5 基礎、約 259.3 億股）
    "pe_ratio":       "~32.0x",       # 台股 P/E（TTM，NT$2,415.5 ÷ 近四季 EPS ~75.5、statementdog）
    "pb_ratio":       "~18.1x",       # 台股 P/B（NT$2,415.5 ÷ 每股淨值 ~133.4、statementdog）
    "div_yield":      "~0.62%",       # 殖利率（NT$2,415.5 基礎）
    "52w_high_tw":    "2,535",        # 52 週/歷史高（2026-06-23 盤中觸 NT$2,535 史高）
    "52w_low_tw":     "1,065",        # 52 週低（Yahoo 股市 52 週區間）
    # NYSE TSM ADR
    "nyse_price":     "434.11",       # 7/10 Fri 官方收盤 $434.11（-$2.85、-0.65% vs 7/9 $436.96、stockanalysis.com 確認、日內 $428.10-$439.66）——小幅回檔；7/9 收 $436.96（-$0.02、持平 vs 7/8 $436.98）；台股 7/10 休市期間 NYSE 續交易
    "nyse_change":    "-2.85",        # 7/10 漲跌（自 7/9 $436.96 小跌）
    "nyse_change_pct":"-0.65%",       # 7/10 NYSE 小幅回檔
    "52w_high_nyse":  "479.00",       # 52 週/歷史高（區間，6/30 收 $477.57）
    "52w_low_nyse":   "223.70",       # 52 週低（區間）
    "pe_nyse":        "~36.9x",       # NYSE USD 基礎 P/E（TTM 估，$434.11 基礎）
    "pb_nyse":        "~18.1x",
    "beta":           "1.18",
    "div_next":       "下次配息預計 2026-07",
    "ex_date":        "2026-04-09（Q2 除息完成，US$0.968/ADR）",
    "next_earnings":  "2026-07-16（Q2 2026 財報，倒數 3 天）",
    # 資料日期備註
    "data_note":      "報告日期：2026-07-13 Mon（最新確認收盤：台股 2330 7/9 Thu 官方收 NT$2,415.5（-50、-2.03% vs 7/8 NT$2,465、開 2,450／高 2,460／低 2,415、量 27,057 張、成交額 NT$658.50 億、TWSE 官方確認）；NYSE TSM 7/10 Fri 收 $434.11（-$2.85、-0.65% vs 7/9 $436.96、stockanalysis.com 確認、日內 $428.10-$439.66）；7/9 NYSE 收 $436.96（-$0.02、持平）。⚠️ 7/10 Fri 因中度颱風「巴威（Bavi）」，證交所宣布台股（含期貨、外匯）全日休市；台股 7/13 Mon 復市）。⭐⭐ 今日頭條——TSMC 6 月營收今日 13:30 公布（延自 7/10、颱風順延）、7/16 法說會倒數 3 天：（1）🎯 6 月營收為 Q2 達標關鍵驗證——法人估區間 NT$4,000-4,400 億、樂觀者看逼近 NT$4,400 億改寫單月新高，若達標將帶動 Q2 營收突破 NT$1.2 兆（對應美元指引 $39.0-40.2B）；4-5 月營收合計僅 +24% YoY 落後華爾街 Q2 約 +35% 預期，6 月數字為市場焦點；（2）📉 台股 7/9 收 NT$2,415.5（-2.03%）——外資賣超 12,749 張、提款約 310 億元、全集中市場連 6 日賣超（7/9 賣 471 億），為主要拖累；（3）📊 NYSE TSM 台股休市期間續交易，7/9 持平 $436.96、7/10 小跌 -0.65% 至 $434.11，相對抗跌；NVDA 7/10 收 $210.96 續強、AI 晶片股維持多頭；（4）📊 台美 ADR 溢價升至約 +15.4%（7/10 ADR $434.11 vs 理論值 $376.2＝5×NT$2,415.5÷匯率 ~32.1）——溢價偏高部分因台股 7/9 起停格、NYSE 續走；台股 7/13 有補跌／補漲拉鋸空間、關鍵看 6 月營收；（5）🚀 分析師大幅續挺：Citi NT$3,800（自 NT$2,415.5 具 +57.3% 空間）、Barclays $625（自 $434.11 具 +44.0%）、Nomura NT$3,425（+41.8%）；共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）；Motley Fool 7/9 發文「TSMC 股價料於 7/16 財報後大漲」。🎯 即將事件：6 月營收今日（7/13）13:30 公布（關鍵 catalyst）；Q2 2026 財報法說會 7/16（倒數 3 天、指引 $39.0-40.2B、毛利率 65-67%、Citi 預期上修全年展望）。⚠️ 本日（7/13）關鍵觀察：（1）台股復市後開盤方向、能否守穩 NT$2,400 / 20MA ~NT$2,410、6 月營收公布後反應；（2）外資賣超能否收斂翻買（7/9 大賣 12,749 張）；（3）ADR 溢價 +15.4% 偏高、留意收斂方式；（4）估值警示：台股 P/E ~32.0x（TTM）、TSM P/E ~37x。⭐ 中長線結構性利多基底未變：TSMC-Amkor 10 年美國先進封裝長約（Peoria AZ $70 億、2028 投產）、Winbond 特殊 DRAM 自主供應、NVIDIA 確認第 1 大客戶（22%/$33B）+ A16 首發 2026 下半/2027 量產（Apple 跳過 A16 直上 A14）+ Vera Rubin 平台 6 顆晶片全數 TSMC 代工（3nm + HBM4）、2nm 量產 70-80% 良率、2nm/A16 產能 2026-28 估 CAGR ~70%。  |  ⭐ 中長線基底：5 月營收 NT$4,169.75 億 +30.1% 創單月新高、1-5 月累計 NT$1.96 兆（+30% YoY）；2nm 量產良率 70-80% 領先全球；NVIDIA 第 1 大客戶 22%/$33B；4 大美國 CSP 2026 AI CapEx 合計 $725B（+77% YoY）；BofA 估 2026 全球半導體市場 $1.3 兆；費半（SOX）YTD +47%  |  ⚠️ 風險：外資 7 月大幅賣超未轉向、下游手機/PC 需求疲弱、Intel 18A 量產競爭、估值不低、台幣匯率波動（每升值 1% 毛利率 -40bps）",
}
...
SUMMARY = {
    ...
    "sentiment":  "中性",
    # 一句話標題
    "headline":   "🎯 6 月營收今日（7/13）13:30 揭曉（颱風延自 7/10）、7/16 法說會倒數 3 天：台股 2330 最新確認收 NT$2,415.5（7/9、-2.03%、外資賣超 12,749 張拖累）；⚠️ 7/10 颱風「巴威」台股休市；NYSE TSM 7/10 收 $434.11（-0.65%）相對抗跌；法人估 6 月營收 NT$4,000-4,400 億、有望創高帶動 Q2 破 NT$1.2 兆；分析師續挺（Citi NT$3,800／Barclays $625／Nomura NT$3,425）",
    # 3–5 條白話重點（不用術語，每條約 30 字內）
    "bullets": [
        "🎯 最重要：TSMC 6 月營收今日 13:30 公布（原訂 7/10、因颱風順延）——法人估 NT$4,000-4,400 億、樂觀看逼近 4,400 億改寫單月新高；達標則 Q2 營收上看 NT$1.2 兆（對應指引 $39.0-40.2B）；4-5 月僅 +24% YoY 落後 ~35% 預期，6 月為 Q2 達標關鍵驗證",
        "⚠️ 7/10 Fri 中度颱風「巴威（Bavi）」侵台，證交所宣布台股（含期貨、外匯）全日休市；台股 7/13 Mon 復市——最新確認台股收盤仍為 7/9 Thu NT$2,415.5",
        "📉 台股 2330 7/9 Thu 官方收 NT$2,415.5（-50、-2.03%、開 2,450／高 2,460／低 2,415、量 27,057 張、TWSE）——外資大賣拖累、自 7/8 NT$2,465 拉回、失守 5MA",
        "📊 三大法人 7/9：外資賣超 12,749 張（連 2 賣、提款約 310 億元、全市場連 6 日賣超 471 億）、投信買超 43 張（連 7 買）、自營買超 90 張，合計賣超 12,615 張——外資賣壓為近期最大壓力",
        "📊 NYSE TSM 台股休市期間續交易：7/9 收 $436.96（-$0.02、持平）、7/10 收 $434.11（-$2.85、-0.65%）相對抗跌；NVDA 7/10 收 $210.96 續強、美 AI 晶片股維持多頭",
        "📊 台美 ADR 溢價升至約 +15.4%（7/10 ADR $434.11 vs 理論值 $376.2＝5×NT$2,415.5÷匯率 ~32.1）——溢價偏高部分因台股 7/9 起停格、NYSE 續走；台股 7/13 補跌／補漲拉鋸、關鍵看營收",
        "🚀 分析師大幅續挺：Citi NT$3,800（自 NT$2,415.5 具 +57.3% 空間）、Barclays $625（自 $434.11 具 +44.0%）、Nomura NT$3,425（+41.8%）；共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）；Motley Fool 看好 7/16 財報後大漲",
        "🏆 中長線基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）+ TSMC-Amkor 美國封裝長約 + 2nm 70-80% 良率 + 全先進製程漲價 5-10%（毛利率 +2pp）",
        "⚠️ 風險續追蹤：外資 7 月大幅賣超未轉向（7/9 單日 -12,749 張）；下游手機/PC 需求疲弱；Intel 18A 量產吸引微軟/高通；估值不低（台股 P/E ~32.0x、TSM ~37x）；6 月營收若不如預期恐引發賣壓",
    ],
    # 一句結論
    "bottom_line": "報告日期：2026-07-13 Mon（最新確認收盤：台股 2330 7/9 Thu 官方收 NT$2,415.5（-50、-2.03% vs 7/8 NT$2,465、開 2,450／高 2,460／低 2,415、量 27,057 張、成交額 NT$658.50 億、TWSE 確認）；NYSE TSM 7/10 Fri 收 $434.11（-$2.85、-0.65% vs 7/9 $436.96）；7/9 NYSE 收 $436.96（-$0.02、持平）。⚠️ 7/10 Fri 因中度颱風「巴威（Bavi）」，證交所宣布台股全日休市；台股 7/13 Mon 復市）。⭐⭐ 今日最重要：TSMC 6 月營收今日 13:30 公布（原訂 7/10、颱風順延至 7/13）——法人估 NT$4,000-4,400 億、樂觀看逼近 4,400 億改寫單月新高；若達標將帶動 Q2 營收突破 NT$1.2 兆（對應美元指引 $39.0-40.2B）；⚠️ 4-5 月營收合計僅 +24% YoY 落後華爾街 Q2 約 +35% 預期，6 月數字為 Q2 達標關鍵驗證、亦為 7/16 法說會前市場最大焦點。⭐ 今日重點：（1）📉 台股 7/9 收 NT$2,415.5（-2.03%）自 7/8 NT$2,465 拉回、失守 5MA——主因外資賣超 12,749 張、提款約 310 億元、全集中市場連 6 日賣超（7/9 賣 471 億）；投信買超 43 張（連 7 買）、自營買超 90 張，三大法人合計賣超 12,615 張；（2）📊 NYSE TSM 台股休市期間續交易、相對抗跌：7/9 持平 $436.96、7/10 小跌 -0.65% 至 $434.11；NVDA 7/10 收 $210.96 續強、美 AI 晶片股維持多頭；（3）📊 台美 ADR 溢價升至 +15.4%（$434.11 vs 理論值 $376.2＝5×NT$2,415.5÷~32.1）——溢價偏高部分因台股 7/9 起停格、NYSE 續走；台股 7/13 補跌／補漲拉鋸、關鍵看 6 月營收；（4）🚀 分析師續挺：Citi NT$3,800（自 NT$2,415.5 具 +57.3% 空間）、Barclays $625（自 $434.11 具 +44.0%）、Nomura NT$3,425（+41.8%）、共識強力買入；⚠️ Goldman 7/1 移出 APAC Conviction List（戰術調整非降評）；Motley Fool 看好 7/16 財報後大漲；（5）🎯 催化倒數：6 月營收今日 13:30、7/16 Q2 法說會倒數 3 天（Citi 預期上修全年展望、上調 2027-28 資本支出至 $75-80B）。⭐ 中長線結構性利多基底未變：NVIDIA #1 客戶（22%/$33B）/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）、2nm 量產良率 70-80%、TSMC-Amkor 美國封裝長約、全先進製程漲價 5-10%（估毛利率 +2pp）、4 大 CSP 2026 AI CapEx $725B（+77% YoY）。技術面（TWSE 官方收盤）：5MA ~NT$2,445 / 10MA ~NT$2,425 / 20MA ~NT$2,410；7/9 收 NT$2,415.5 失守 5MA、貼近 20MA；下方支撐 NT$2,400 / NT$2,380；上方壓力 NT$2,440 / NT$2,465 / NT$2,500-2,535 史高。⚠️ 風險：外資 7 月大幅賣超未轉向（7/9 單日 -12,749 張）、下游手機/PC 需求疲弱、Intel 18A 量產競爭、估值不低（台股 P/E ~32.0x TTM、TSM ~37x）、台幣匯率 ~32.1（每升值 1% 毛利率 -40bps）、6 月營收若不如預期恐引發賣壓。",
    # 重大警示（選填，空白則不顯示）
    "risk_alert":  "今日（7/13 Mon）關鍵觀察：（1）🎯🎯 TSMC 6 月營收今日 13:30 公布（延自 7/10 颱風）——法人估 NT$4,000-4,400 億、達標則 Q2 上看 NT$1.2 兆；4-5 月 +24% YoY 落後 ~35% 預期，此為 7/16 法說會前最重要驗證、公布後恐劇烈波動；（2）📉 台股 7/13 復市後開盤方向與營收反應——最新確認收盤 7/9 NT$2,415.5（-2.03%）、失守 5MA；觀察能否守穩 NT$2,400 / 20MA ~NT$2,410、上方壓力 NT$2,440 / NT$2,465；（3）📊 籌碼：外資 7/9 大賣 12,749 張（提款 310 億、全市場連 6 日賣超）——能否收斂翻買為關鍵；投信連 7 日買超為穩定買盤；（4）📊 ADR 溢價 +15.4% 偏高（台股停格、NYSE 續走所致）、留意收斂方式（台股補跌 vs ADR 回落）；（5）📊 NYSE TSM 7/10 $434.11（-0.65%）相對抗跌、NVDA $210.96 續強，隔夜美股動能中性偏正；（6）⚠️ 估值警示：台股 P/E ~32.0x（TTM）、TSM ~37x；台幣 ~32.1、Intel 18A 競爭、下游手機/PC 需求疲弱續追蹤",
}

# 三大法人買賣超（張）— 最新一日
INSTITUTIONAL = {
    "date":     "2026-07-09",       # 最新資料日（7/9 Thu；7/10 Fri 颱風休市無交易）
    "foreign":  -12749,            # 外資 7/9 賣超 12,749 張（連 2 賣、提款約 310 億元、全市場連 6 日賣超）
    "trust":    43,                # 投信 7/9 買超 43 張（連 7 日買超）
    "dealer":   90,                # 自營商 7/9 買超 90 張（連 2 買）；三大法人合計 -12,615 張
    # 近5日（由舊到新：7/3, 7/6, 7/7, 7/8, 7/9）；TWSE 官方數據（張）；7/10 颱風休市
    "foreign_5d": [-12537, -241, 83, -4155, -12749],     # 外資：7/9 再度大賣逾 1.2 萬張、7 月賣壓集中
    "trust_5d":   [821, 380, 539, 730, 43],              # 投信連 7 日買超（官方）
    "dealer_5d":  [653, 119, -237, 468, 90],             # 自營（官方）
...
# 技術分析指標（需每日從看盤軟體更新）
TECHNICAL = {
    "rsi_14":       46.0,   "rsi_signal": "RSI(14) 估約 46——7/9 -2.03% 回落後跌破中軸 50、中性偏弱、動能轉弱但距超賣（30）仍有空間",
    "macd":         0.10,  "macd_signal": "MACD 估約 +0.10、紅柱大幅收斂逼近翻黑——7/1 高點 NT$2,505 後於 2,415-2,505 區間震盪走弱，趨勢動能減弱、待 6 月營收與 7/16 財報定方向",   "macd_color": "#FF9800",
    "kdj_k":        40.0,   "kdj_d": 48.0, "kdj_j": 24.0, "kdj_signal": "KDJ K(40)/D(48)/J(24) 估——7/9 回落後 J 值急跌接近超賣區、短線偏弱、待止穩訊號",
    "ma5":       "2,445",   # 5MA（7/3 2445, 7/6 2460, 7/7 2440, 7/8 2465, 7/9 2415.5）— TWSE 官方收盤
    "ma20":      "2,410",   # 20MA（估、TWSE 官方收盤）
    "ma60":      "2,300",  # 60MA（估算）
    "ma120":     "2,235",  # 120MA（估算）
    "bb_upper":  "2,540",  # 布林上軌（20MA+2σ 估）
    "bb_mid":    "2,410",   # 布林中軌（20MA）
    "bb_lower":  "2,280",  # 布林下軌（估）
    "vol_ma5":  "30,600",   # 5日均量（張、7/3-7/9 官方量均：26,455/21,042/31,401/25,520/27,057）
    "trend":    "📉 回檔失守 5MA、營收公布前觀望：台股 2330 7/9 官方收 NT$2,415.5（-50、-2.03%、量 27,057 張）自 7/8 NT$2,465 拉回、失守 5MA（~2,445）、貼近 20MA（~2,410）——主因外資單日大賣 12,749 張、提款約 310 億元（全集中市場連 6 日賣超 471 億）。⚠️ 7/10 Fri 因中度颱風「巴威」台股全日休市、無交易（含期貨/外匯），最新確認收盤仍為 7/9。NYSE TSM 台股休市期間續交易、相對抗跌：7/9 持平 $436.96、7/10 小跌 -0.65% 至 $434.11；NVDA 7/10 收 $210.96 續強、美 AI 晶片股維持多頭、隔夜動能中性偏正。📊 技術面（官方收盤）：RSI(14) 約 46 跌破中軸、MACD 約 +0.10 紅柱逼近翻黑、KDJ J 值急跌近超賣；價格失守 5MA（~2,445）、貼近 20MA（~2,410），短線轉弱、中期均線多頭尚未破壞。下一觀察：（1）🎯 6 月營收今日 13:30 公布（延自 7/10）——法人估 NT$4,000-4,400 億、為 Q2 達標與 7/16 財報前最關鍵驗證、公布後恐劇烈波動；（2）台股 7/13 復市能否守穩 NT$2,400 / 20MA ~NT$2,410、上方壓力 NT$2,440 / NT$2,465 / NT$2,500；（3）外資賣超（7/9 -12,749 張）能否收斂翻買；（4）🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 共識強力買入為中長線支撐；（5）⭐ 中長線基底未變：NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 70-80% 良率。",
    "support":  "2,400 / 2,380 / 20MA ~2,410 (整數、前低、20MA)",
    "resist":   "2,440 / 2,465 / 2,500-2,535 (5MA、7/8 收、史高)",
    "note": "均線依 TWSE 官方收盤計算、RSI/MACD/KDJ/布林為估算，請以看盤軟體為準；7/9 官方收 NT$2,415.5（-2.03%）失守 5MA、貼近 20MA；7/10 颱風「巴威」台股休市無交易；台股 7/13 復市、6 月營收今日 13:30 公布為關鍵；下方支撐 NT$2,400 / NT$2,380、上方壓力 NT$2,440 / NT$2,465 / NT$2,500-2,535 史高；🚀 Citi/Barclays/Nomura 續挺 + 漲價定價力 vs ⚠️ 外資 7 月大幅賣超 + 6 月營收與下游需求雜訊",
}

# 期貨與選擇權（來源：台灣期交所 TAIFEX）
DERIVATIVES = {
    "date":          "2026-07-09",       # 最新資料日（7/9 Thu 盤後估；7/10 颱風休市無交易；官方以 TAIFEX 為準）
    "futures_price": "2,410",            # 台積電期貨近月（估、現貨 7/9 收 NT$2,415.5 附近小幅逆價差）
    "futures_oi":    "182,000",          # 未平倉量（口）（估）
    "futures_basis": "-5",               # 估小幅逆價差（7/9 現貨回檔、外資大賣、營收公布前偏保守）
    "call_oi":       "118,000",
    "put_oi":        "80,000",
    "pcr":           "0.68",             # Put/Call 比率（估、營收公布前避險升溫、中性）
    "pcr_signal":    "中性（Put/Call 估 0.68；台股 7/9 官方收 NT$2,415.5（-2.03%）回檔、外資大賣 12,749 張、6 月營收今日 13:30 公布前避險升溫；⭐ Citi/Barclays/Nomura + 共識偏多為中長線支撐；⚠️ 7/10 颱風休市、本日期權數據以估算標註）",
    "max_pain":      "2,440",            # 最大痛苦價格（近期到期、估）
    "key_strikes": [
        ("2,380", "Call 54,000 / Put 14,000", "前低、下方支撐"),
        ("2,400", "Call 52,000 / Put 11,000", "整數關卡、支撐"),
        ("2,410", "Call 50,000 / Put 9,000",  "20MA、支撐"),
        ("2,415", "Call 48,000 / Put 8,000",  "7/9 官方收"),
        ("2,440", "Call 45,000 / Put 7,000",  "5MA、最大痛苦（估）"),
        ("2,465", "Call 40,000 / Put 5,500",  "7/8 官方收、整數壓力"),
        ("2,500", "Call 32,000 / Put 3,500",  "整數壓力"),
        ("2,535", "Call 24,000 / Put 2,500",  "盤中史高"),
    ],
    "note": "衍生品數據為估算、需每日至台灣期交所(taifex.com.tw)核實（7/10 颱風休市、無交易數據）；價位錨點按 TWSE 官方收盤（7/9 NT$2,415.5）；NT$2,440 為估最大痛苦、下方支撐 NT$2,400 / NT$2,380、上方壓力 NT$2,465 / NT$2,500 / NT$2,535 史高；🎯 6 月營收今日 13:30 公布為近期最大變數；🚀 Citi NT$3,800 + Barclays $625 + Nomura NT$3,425 + 漲價定價力偏多 vs ⚠️ 外資 7 月大幅賣超、6 月營收 miss 風險；中長線基底未變（NVIDIA #1 客戶/A16 首發、Vera Rubin 6 顆晶片全 TSMC 代工、2nm 70-80% 良率、TSMC-Amkor 美國封裝長約）",
}

# 供應鏈與客戶股價（需每日更新）
ECOSYSTEM_STOCKS = [
    # 類別、股票代碼、名稱、收盤價、漲跌幅、備註（美股/韓股為近期收盤：NVDA 為 2026-07-10 Fri 確認收盤、其餘美股/韓股為近幾日收盤估值；台股個股為 7/9 Thu 收盤、7/10 颱風休市無交易——⚠️ 除 NVDA 外部分價格為前期估值、僅供參考，請以看盤軟體為準）
    ("設備", "ASML",    "艾司摩爾",    "1,770.00", "≈持平", "EUV 設備龍頭（近期收盤估、高 NA EUV 需求強；請以即時報價為準）"),
    ("設備", "AMAT",    "應用材料",    "572.00", "≈持平", "🛠️ AMAT-TSMC $5B EPIC Center 研發合作（近期收盤估）"),
    ("設備", "LRCX",    "科林研發",    "334.00", "≈持平", "蝕刻設備（近期收盤估；WFE 全年展望上修）"),
    ("設備", "KLAC",    "科磊",        "222.00", "≈持平", "量測設備（近期收盤估；2nm 量產驅動中長線）"),
    ("記憶", "2344.TW", "華邦電 Winbond","168.50","—", "⭐ 與 TSMC 合作 CUBE 特殊 DRAM 導入 WoW 3D 堆疊/SoIC、降低對韓系 HBM 依賴（台股 7/9 收盤、7/10 颱風休市）"),
    ("記憶", "000660.KS","SK 海力士",  "2,100,000 KRW", "≈持平", "🤝 與 NVIDIA 多年期記憶體合作；HBM4 需求強（近期收盤估）"),
    ("記憶", "MU",      "美光",        "952.00", "≈持平", "HBM / DDR5（近期收盤估；AI 記憶體需求勁揚、HBM4 中長線強勁）"),
    ("封裝", "3711.TW", "日月光",      "625.00", "—", "CoWoS / 先進封裝（台股 7/9 收盤、7/10 颱風休市；先進封裝瓶頸結構受惠）"),
    ("封裝", "AMKR",    "Amkor",       "67.00",  "≈持平", "⭐ TSMC-Amkor 10 年美國封裝長約、Peoria AZ $70 億 2028 投產（近期收盤估）"),
    ("客戶", "NVDA",    "輝達",        "210.96", "續強", "🏆 TSMC #1 客戶 22%/$33B、A16 首發客戶；7/10 收 $210.96（自 7/8 $204.12 續強）領攻 AI 晶片股多頭"),
    ("客戶", "AAPL",    "蘋果",        "314.00", "≈持平", "#2 客戶 18%；面臨 TSMC 漲價成本；跳過 A16 直上 A14（近期收盤估）"),
    ("客戶", "AVGO",    "博通",        "392.00", "≈持平", "#3 客戶；custom XPU 中長線強勁（近期收盤估）"),
    ("客戶", "AMD",     "超微",        "518.00", "≈持平", "#4 客戶；瞄準 $1,200 億資料中心市場、MI350/MI450、首發 2nm AI 晶片（近期收盤估）"),
    ("客戶", "2454.TW", "聯發科",      "3,995",  "—", "手機 SoC 客戶 9%；面臨漲價成本（台股 7/9 收盤、7/10 颱風休市）"),
    ("競爭", "INTC",    "英特爾",      "110.50", "≈持平", "18A 節點量產、吸引微軟/高通等客戶進駐；代工轉型為 TSMC 長期競爭變數（近期收盤估）"),
    ("競爭", "005930.KS","三星電子",   "280,000 KRW", "≈持平", "⚠️ 南韓 $1 兆美元晶片投資領頭者；HBM4E 送樣（近期收盤估）"),
    ("產業", "SOX",     "費城半導體指數","12,600", "≈持平", "AI 晶片股維持多頭；YTD +47%、7/16 起半導體財報季（近期估值、請以即時報價為準）"),
    ("處分後", "ARM",   "Arm Holdings","300.00", "≈持平", "TSMC 4/29 全數出脫後（近期收盤估；RTX Spark ARM 架構中長線受惠）"),
]
...
ANALYST_RATINGS = [
    ...
    ("Barclays",       "增持",     "US$625",   "2026-07-02", "🚀 7/2 大幅上調目標價自 $470 至 $625（維持 Overweight、自 7/10 收 $434.11 具 +44.0% 空間）；AI 需求結構性 + A14/A16 領先、TSMC 為 AI CapEx 循環終極受惠者"),
...
CONSENSUS = {
    ...
    "low_target":   "NT$2,100",
    "avg_target_us":"US$490.34",    # 分析師 12 個月平均目標（高 $700 / 低 $354；自 7/10 收 $434.11 具 +13.0% 上行空間；Citi 上調台股目標至 NT$3,800、Barclays 維持 $625、Nomura NT$3,425；Goldman 7/1 移出 APAC Conviction List；Morningstar 17 位分析師強力買入）
    "analyst_count": 32,
    "buy": 32, "hold": 0, "sell": 0,
    "upside":       "+12.95%",  # 從 NYSE 7/10 收 $434.11 至平均目標 $490.34（共識強力買入、Citi 上調拉高平均；待 7/16 法說會後重估）
}
...
IR_CALENDAR = [
    ...
    ("2026-07-13", "🎯 報告日：6 月營收今日 13:30 揭曉、7/16 法說會倒數 3 天", "7/13 Mon 台股復市；TSMC 6 月營收今日 13:30 公布（延自 7/10 颱風）——法人估 NT$4,000-4,400 億、樂觀看逼近 4,400 億改寫單月新高，達標則 Q2 上看 NT$1.2 兆（對應指引 $39.0-40.2B）；4-5 月僅 +24% YoY 落後 ~35% 預期、6 月為 Q2 達標關鍵驗證；最新確認收盤台股 7/9 NT$2,415.5、NYSE TSM 7/10 $434.11；ADR 溢價 +15.4%；🚀 Citi NT$3,800 / Barclays $625 / Nomura NT$3,425 續挺；Motley Fool 看好 7/16 財報後大漲",  "#FFF9C4"),
...
# 總體經濟監控
MACRO = [
    ("SOX 半導體指數",  "~12,600", "≈持平", False),  # 近期估值（AI 晶片股維持多頭；請以即時報價為準）
    ("台股加權指數",    "~45,100", "≈持平", False),  # 7/9 收盤估（2330 回檔拖累；7/10 颱風休市）
    ("VIX 恐慌指數",    "18.0",    "—",     False),   # 估（未取得即時值）
    ("WTI 原油",        "$70.0",   "0.00%",  False),   # 油價持穩（估、未取得即時值）
    ("10Y 美債殖利率",  "4.50%",   "0.00pp", False),   # 美債殖利率持穩（估、未取得即時值）
    ("台幣/美元",       "~32.1",   "≈持平",  False),   # 台幣（每升值 1% 毛利率 -40bps；近期估）
    ("NVIDIA NVDA",     "$210.96", "續強",   False),   # #1 TSMC 客戶（22%），7/10 收 $210.96 續強領攻 AI 晶片股
    ("ASML",            "~$1,770", "≈持平",  False),  # 設備股（近期估值）
]
...
VALUATION = {
    "pe_current":   32.0,   # 同 STOCK["pe_ratio"]（7/9 官方收 NT$2,415.5 ÷ 近四季 EPS ~75.5、statementdog）
    ...
    "pb_current":  18.11,   # 同 STOCK["pb_ratio"]（7/9 官方收 NT$2,415.5 ÷ 每股淨值 ~133.4）
    "pb_5y_avg":     5.8,
    "pb_5y_pct":      95,
    "ev_ebitda":    21.0,   # EV/EBITDA（估算）
    "peg":          1.11,   # PEG = P/E ÷ 5 年 EPS CAGR ~33%
    "note": "P/E 歷史百分位為估算（Bloomberg/Refinitiv 基準）、基於 7/9 官方收 NT$2,415.5：台股 P/E ~32.0x（TTM、近四季 EPS ~75.5、statementdog）、P/B ~18.1x（每股淨值 ~133.4）；NYSE TSM 7/10 收 $434.11、USD P/E 約 37x（估）；估值仍處 5 年高位區間、PEG 約 1.0（以 5 年 EPS CAGR ~33% 計）；⚠️ 留意近期營收 miss 風險（4-5 月 +24% YoY 落後 ~35% 預期、6 月營收今日 7/13 13:30 公布為關鍵驗證）；中長線基底未變：NVIDIA #1 客戶/A16 首發 + TSMC-Amkor 美國封裝長約 + 2nm 量產 70-80% 良率 + 全先進製程漲價 5-10%",
}
...
RELATIVE = {
    "date":           "2026-07-10",       # 最新資料日（台股 2330 最新確認收盤 7/9 NT$2,415.5、NYSE TSM 7/10 收 $434.11；台股 7/10 颱風休市）
    "tsmc_tw_1d":     -2.03,   # 2330 7/9 官方收 NT$2,415.5（-50 vs 7/8 NT$2,465、TWSE）
    "taiex_1d":       -1.00,   # 加權指數 7/9 估回檔（2330 拖累、估）
    "sox_1d":         0.0,     # SOX（未取得 7/10 即時值、估持平）
    "tsmc_nyse_1d":   -0.65,   # TSM ADR 7/10 $434.11（-$2.85 vs 7/9 $436.96；小幅回檔）
    "tsmc_tw_5d":     -2.00,   # 5 日（NT$2,415.5 vs 7/2 官方收 NT$2,465）
    "taiex_5d":       -1.50,   # （估）
    "tsmc_tw_1m":     +4.80,   # 月線（NT$2,415.5 vs 6/9 官方收 NT$2,305）
    "taiex_1m":       +2.0,    # （估）
    "tsmc_tw_ytd":    +15.30,  # YTD 累計（NT$2,415.5 vs 年初 ~NT$2,095）
    "taiex_ytd":      +13.0,   # （估）
    "sox_ytd":        +47.0,   # SOX YTD（費半 YTD +47%）
    "beta_60d":        1.38,   # 60 日滾動 Beta vs TAIEX
    # ADR 折溢價（以 7/10 NYSE 收盤 & 7/9 TW 官方收盤計算）
    "adr_price":     434.11,   # TSM NYSE 7/10 收盤（-0.65%、stockanalysis.com 確認）
    "adr_parity":    376.24,   # 理論值 = 5 × NT$2,415.5 ÷ 32.1
    "adr_ratio":     "5:1",    # 1 ADR = 5 台積電普通股
    "usd_ntd":        32.1,    # 台幣匯率（近期估）
    "adr_premium_pct": "+15.38", # (434.11 - 376.24) / 376.24 × 100 = +15.38%（匯率 32.1）
    "note": "ADR 理論值 = 5 × 台股收盤 ÷ 匯率（不含交易摩擦與流動性溢價）。⚠️ 7/10 Fri 因中度颱風「巴威（Bavi）」，證交所宣布台股全日休市；NYSE TSM 續交易：7/9 收 $436.96（-$0.02、持平）、7/10 收 $434.11（-$2.85、-0.65%、日內 $428.10-$439.66）相對抗跌；台股 2330 最新確認收盤為 7/9 Thu NT$2,415.5（-50、-2.03%、量 27,057 張、TWSE、市值 NT$62.6 兆）。📊 籌碼面：7/9 外資賣超 12,749 張（連 2 賣、提款約 310 億元、全市場連 6 日賣超 471 億）、投信買超 43 張（連 7 買）、自營買超 90 張、合計賣超 12,615 張——外資賣壓為近期最大壓力。📊 台美 ADR 溢價升至 +15.38%（$434.11 vs 理論值 $376.24、匯率 32.1）——溢價偏高部分因台股 7/9 起停格（7/10 休市）、NYSE 續走所致，台股 7/13 復市有補跌／補漲拉鋸空間、關鍵看 6 月營收。🎯🎯 今日（7/13）最重要：TSMC 6 月營收 13:30 公布（延自 7/10 颱風）——法人估 NT$4,000-4,400 億、達標則 Q2 上看 NT$1.2 兆（對應指引 $39.0-40.2B）；7/16 法說會倒數 3 天；🚀 分析師續挺：Citi NT$3,800、Barclays $625、Nomura NT$3,425；⭐ 中長線結構性基底未變：TSMC-Amkor 美國封裝長約 + NVIDIA #1 客戶/A16 首發 + Vera Rubin 6 顆晶片全 TSMC 代工 + 2nm 量產 70-80% 良率 + CoWoS 擴產 + 4 大 CSP CapEx $725B",
}
...
NEWS = [
    {
        ...
        "title": "🎯 TSMC 6 月營收今日（7/13）13:30 揭曉（颱風延自 7/10）：法人估 NT$4,000-4,400 億、有望創單月新高、帶動 Q2 突破 NT$1.2 兆",
        "body": "本週最大催化、7/16 法說會前哨戰：（1）🎯 TSMC 原訂 7/10 公布之 6 月合併月營收，因颱風休市順延至今日（7/13）13:30 公告；（2）📊 依 Q2 財測（單季 $39.0-40.2B、約 NT$1.2 兆），扣除已公布 4 月 NT$4,107 億、5 月 NT$4,169 億後，法人推估 6 月營收落在 NT$4,000-4,400 億、樂觀者看逼近 NT$4,400 億改寫史上單月新高；若達標則 Q2 營收可突破 NT$1.2 兆、確認 AI/HPC 需求動能；（3）⚠️ 4-5 月合計 +24% YoY 落後華爾街 Q2 約 +35% 預期，6 月數字是驗證關鍵——達標將大幅緩解營收疑慮、不如預期恐引發賣壓；（4）美 Motley Fool 7/9 發文看好「TSMC 股價料於 7/16 財報後大漲」。",
        "source": "財報狗 / 工商時報 / BigGo Finance / Motley Fool 2026-07-10 - 07-13", "impact": "正面",
    },
    {
        "tag": "地緣政治", "tag_color": "#607D8B",
        "title": "🌀 颱風「巴威」7/10 台股全日休市：證交所停止交易、6 月營收順延至 7/13；NYSE TSM 續交易收 $434.11（-0.65%）相對抗跌",
        "body": "天災擾亂交易節奏：（1）🌀 中度颱風「巴威（Bavi）」7/10 Fri 侵台，台北、新北、桃園、基隆宣布停班停課，證交所宣布台股（含集中市場、期貨、外匯）全日休市，結算與櫃買同步暫停；（2）📅 連帶 TSMC 原訂 7/10 公布之 6 月營收順延至 7/13（下週一）13:30；（3）📊 台股 7/10 停格，最新確認收盤仍為 7/9 Thu NT$2,415.5（-2.03%）；NYSE TSM 於台股休市期間照常交易：7/9 收 $436.96（持平）、7/10 收 $434.11（-$2.85、-0.65%、日內 $428.10-$439.66）相對抗跌；（4）📊 台股停格＋NYSE 續走推升 ADR 溢價至 +15.4%（$434.11 vs 理論值 $376.2）——台股 7/13 復市有補跌／補漲拉鋸空間、關鍵看營收。",
        "source": "證交所 / 中央社 / 經濟日報 / stockanalysis.com 2026-07-09 - 07-10", "impact": "中性",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "📉 台股 7/9 收 NT$2,415.5（-2.03%）失守 5MA：外資單日大賣 12,749 張、提款約 310 億元、全市場連 6 日賣超 471 億",
        "body": "外資賣壓為近期最大壓力：（1）📉 台股 2330 7/9 Thu 官方收 NT$2,415.5（-50、-2.03% vs 7/8 NT$2,465、開 2,450／高 2,460／低 2,415、量 27,057 張、成交額 NT$658.50 億、TWSE）失守 5MA（~2,445）、貼近 20MA（~2,410）；（2）📊 三大法人 7/9：外資賣超 12,749 張（連 2 賣、單一台積電提款約 310 億元）、投信買超 43 張（連 7 日買超）、自營買超 90 張（連 2 買）、合計賣超 12,615 張；（3）外資同日於全集中市場賣超 471 億元、已連續第 6 個交易日站在賣方；（4）⚠️ 外資 7 月大幅調節籌碼，能否於 6 月營收與 7/16 財報前收斂翻買為關鍵；投信連 7 日買超為相對穩定的內資買盤。",
        "source": "鉅亨網 / Goodinfo / TWSE T86 2026-07-09", "impact": "負面",
    },
    {
        "tag": "客戶動態", "tag_color": "#2196F3",
        "title": "🏆 NVIDIA 續強、7/10 收 $210.96 領攻 AI 晶片股：穩居 TSMC 第 1 大客戶（22%/$33B）；Vera Rubin 6 顆晶片全 TSMC 代工（3nm+HBM4）",
        "body": "美 AI 晶片股維持多頭、對台股偏正面：（1）🏆 NVDA 7/10 收 $210.96（自 7/8 $204.12 續強）領攻 AI 晶片股、市場情緒穩健；NVIDIA 2026 年估貢獻 TSMC 營收約 $330 億（佔比 ~22%）、超越 Apple 約 $270 億；（2）⭐ NVIDIA 次世代 Vera Rubin 平台 6 顆並行晶片全數 TSMC 代工（3nm+HBM4、較 Blackwell 訓練快 3.5×／推論快 5×）、NVIDIA 包下多數 CoWoS 產能、傳為 A16 首發客戶；（3）🤝 NVIDIA-SK 海力士多年期記憶體合作、NVIDIA-TSMC 將 AI 導入晶圓廠製造提升良率；（4）⚠️ 傳 TSMC 對 Apple 調漲先進製程價格、Apple 跳過 A16 直上 A14。",
        "source": "Yahoo Finance / CNBC / NVIDIA Newsroom 2026-07-10", "impact": "正面",
    },
    {
        "tag": "產業趨勢", "tag_color": "#4CAF50",
        "title": "🚀 分析師續大幅看多、7/16 法說會倒數 3 天：Citi NT$3,800、Barclays $625、Nomura NT$3,425；共識強力買入（32 位 0 賣出）",
        "body": "法人目標價維持高檔、財報季倒數：（1）🚀 Citi 維持台股目標 NT$3,800（自 7/9 NT$2,415.5 具 +57.3% 空間）、預期 TSMC 於 7 月中上修 2026 全年營收成長預測（估 >30%）與 2027-28 資本支出（$75-80B）；Barclays $625（自 $434.11 具 +44.0%）、Nomura NT$3,425（+41.8%）續挺；（2）📊 共識強力買入（Morningstar 統計 17 位分析師買入 0 賣出、平均目標 $490.34）；（3）🎯 TSMC 將於 7/16 台北 14:00 舉行法說會、公布 Q2 財報（指引 $39.0-40.2B、+32% YoY、毛利率 65-67%）並展望 Q3、資本支出；（4）⚠️ 惟 Goldman Sachs 7/1 將 TSM 移出「APAC Conviction List」（戰術性調整、非降評）。",
        "source": "Citi / Barclays / Nomura / Morningstar / investor.tsmc.com 2026-07-01 - 07-13", "impact": "正面",
    },
    {
        "tag": "警示", "tag_color": "#F44336",
        "title": "⚠️ 風險追蹤：外資 7 月大幅賣超未轉向；6 月營收若不如預期恐引賣壓；Intel 18A 量產吸引微軟/高通；估值不低（台股 P/E ~32x、TSM ~37x）",
        "body": "多項雜訊與競爭變數須留意：（1）📊 籌碼面——外資 7 月對 2330 大幅賣超（7/9 單日 -12,749 張、全市場連 6 日賣超），尚未轉買；投信連 7 日買超為穩定內資力量；（2）🎯 事件風險——6 月營收今日 13:30 公布，4-5 月 +24% YoY 落後華爾街 ~35% 預期，若 6 月不如預期恐引發賣壓；（3）⚔️ 競爭——Intel 在 CEO Lip-Bu Tan 帶領下代工轉型、18A 節點進入量產並吸引微軟、高通等大客戶，對 TSMC 構成長期競爭壓力（惟 Intel 亦為 TSMC 客戶）；下游手機/PC 需求疲弱；（4）💱 估值與匯率——台股 P/E ~32.0x（TTM）、TSM ~37x、台幣 ~32.1（每升值 1% 毛利率 -40bps）；ADR 溢價 +15.4% 偏高、留意收斂風險。",
        "source": "TWSE T86 / 鉅亨網 / gurufocus 2026-07-09 - 07-13", "impact": "負面",
    },
]
